...
git commit -m "feat: Entry에 cachedSnapshot 필드 추가"
```
...
```bash
git add server/subscriptions.js server/subscriptions.test.js
git commit -m "feat: onDisconnect에서 entry 삭제 제거 (캐시 보존)"
```
...
```bash
git add server/subscriptions.js server/subscriptions.test.js
git commit -m "feat: SubscriptionRegistry에 generation counter 추가"
```
...
```js
test('refreshAndPublish discards result when generation changes (workspace switch)', async () => {
  vi.useFakeTimers();
  const server = createServer();
  const { wss } = attachWsServer(server, {
    path: '/ws',
  ...
  const snapshot_before = registry.get(key)?.cachedSnapshot;

  // Simulate workspace switch: clear registry (increments generation)
  registry.clear();

  // Mock adapter returns new data
  const mock = /** @type {import('vitest').Mock} */ (fetchListForSubscription);
  mock.mockResolvedValueOnce({
    ok: true,
  ...
  // Trigger refresh — should be discarded because generation changed
  scheduleListRefresh();
  await vi.advanceTimersByTimeAsync(60);

  // Entry should not contain stale data (entry was cleared)
  const entry = registry.get(key);
  // entry is null because registry was cleared and generation guard prevented re-creation
...
Expected: FAIL — generation guard가 아직 없어서 stale 데이터가 entry에 기록될 수 있음
...
```bash
git add server/ws.js server/ws.snapshot-cache.test.js
git commit -m "feat: refreshAndPublish에 generation guard 추가"
```
...
test('subscribe-list serves cached snapshot without bd call on cache hit', async () => {
  ...
  const mock = /** @type {import('vitest').Mock} */ (fetchListForSubscription);
  const calls_after_cold = mock.mock.calls.length;
  ...
  // fetchListForSubscription should NOT have been called again (synchronously)
  // Note: background refresh may call it asynchronously, but not during handleMessage
  expect(mock.mock.calls.length).toBe(calls_after_cold);

  // sock2 should have received a snapshot
...
test('subscribe-list cache hit serves empty array as valid cache (not a miss)', async () => {
  ...
  const calls_after = mock.mock.calls.length;

  // Second subscribe: should still be cache hit (empty is valid)
  const sock2 = {
    sent: /** @type {string[]} */ ([]),
    readyState: 1,
    OPEN: 1,
  ...
  // Should not call fetchListForSubscription again (cache hit)
  expect(mock.mock.calls.length).toBe(calls_after);
});
```
...
Expected: FAIL — 캐시 히트 분기가 없어 항상 `fetchListForSubscription` 호출
...
```bash
git add server/ws.js server/ws.snapshot-cache.test.js
git commit -m "feat: subscribe-list 캐시 히트 경로 구현"
```
...
```bash
git add -A
git commit -m "test: 서버 스냅샷 캐시 통합 테스트 보완"
```
