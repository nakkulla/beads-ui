...
**Goal:** Beads UI가 Dolt backend workspace 전환 시 안정적으로 board/list를 로드하고, 현재 workspace 기준 `Sync now`와 `Auto sync`로 원격 이슈를 동기화할 수 있게 한다.

**Architecture:** 서버는 daemon/subprocess PATH를 정규화하고, websocket `sync-workspace` 액션에서 Dolt readiness 확인 → 필요 시 `bd dolt start` → `bd dolt pull` → active subscription refresh를 직렬화한다. 클라이언트는 workspace picker 인접 UI에서 `Sync now`와 `Auto sync`를 제공하고, workspace 전환 직후 best-effort 1회 sync를 background로 트리거한다.

**Tech Stack:** Node.js ESM, Express, ws, Lit HTML, Vitest, Beads CLI (`bd`), Dolt backend
...
- `server/ws.js`
  - `sync-workspace` 처리, workspace sync lock, readiness/pull/refresh orchestration을 구현한다.
- `server/index.js`
  - 필요하면 startup 진단 로그를 최소 범위로 추가한다.
- `app/protocol.js`
  - `sync-workspace` 메시지 타입을 추가한다.
- `app/main.js`
  - sync 상태, 버튼 handler, auto-sync timer, workspace-switch background sync를 연결한다.
- `app/views/nav.js` 또는 `app/views/workspace-picker.js`
...
describe('buildSpawnPath', () => {
  ...
  test('dedupes duplicate path entries while preserving order', () => {
    ...
    expect(result.split(':')).toEqual(['/opt/homebrew/bin', '/usr/local/bin', '/usr/bin']);
  });
});
```
...
Run: `npx vitest run server/env.test.js`
Expected: FAIL with `Cannot find module './env.js'` or missing export errors.
...
export function buildSpawnPath(raw_path) {
  ...
  for (const entry of [...DEFAULT_BIN_DIRS, ...(String(raw_path || '').split(':'))]) {
...
`server/cli/daemon.js`의 `startDaemon()`에서 `spawn_env` 생성 직후 아래를 추가한다.
...
Run: `npx vitest run server/bd.test.js -t "runBdSync|normalized PATH|sandbox by default"`
Expected: FAIL because `runBdSync` does not exist and PATH normalization is not implemented.

- [ ] **Step 3: `server/bd.js`에 env 정규화 helper와 sync 전용 실행 함수를 추가한다**
...
describe('sync-workspace', () => {
  test('starts dolt when needed, pulls, and refreshes active subscriptions', async () => {
    run_bd.mockResolvedValueOnce({ code: 0, stdout: 'Dolt server: not running\n  Expected port: 0', stderr: '' });
    run_bd_sync.mockResolvedValueOnce({ code: 0, stdout: 'started', stderr: '' });
    run_bd_sync.mockResolvedValueOnce({ code: 0, stdout: 'pulled', stderr: '' });
...
test('returns structured error when dolt start fails', async () => {
  run_bd.mockResolvedValueOnce({ code: 0, stdout: 'Dolt server: not running\n  Expected port: 0', stderr: '' });
  run_bd_sync.mockResolvedValueOnce({ code: 1, stdout: '', stderr: 'start failed' });
...
test('returns structured error when pull fails without clearing local subscriptions', async () => {
  ...
  run_bd_sync.mockResolvedValueOnce({ code: 1, stdout: '', stderr: 'pull failed' });
  ...
  expect(registry.get(keyOf({ type: 'blocked-issues' }))?.subscribers.size).toBeGreaterThan(0);
});
```

- [ ] **Step 3: 새 테스트를 실행해 실패를 확인한다**

Run: `npx vitest run server/ws.sync-workspace.test.js`
Expected: FAIL because `sync-workspace` handler does not exist.

- [ ] **Step 4: `server/ws.js`에 sync helper를 추가한다**

`server/ws.js` 상단 import를 확장한다.
...
if (req.type === 'sync-workspace') {
  const workspace_root = path.resolve(String((req.payload || {}).path || CURRENT_WORKSPACE?.root_dir || ''));
  ...
  try {
    const result = await withWorkspaceSyncLock(workspace_root, async () => {
      const status = await runBd(['dolt', 'status'], { cwd: workspace_root });
...
test('clicking Sync now sends sync-workspace for current workspace', async () => {
  ...
  CLIENT = {
    send: vi.fn(async (type) => {
      ...
      if (type === 'sync-workspace') {
        return { workspace: { root_dir: '/tmp/demo', db_path: '/tmp/demo/.beads' }, pulled: true };
      }
      return { changed: false, workspace: { root_dir: '/tmp/demo', db_path: '/tmp/demo/.beads' } };
    }),
    on() { return () => {}; },
    onConnection() { return () => {}; },
    close() {},
  ...
  document.body.innerHTML = '<header id="top-nav"></header><div id="header-loading"></div><main id="app"></main>';
  ...
  expect(CLIENT.send).toHaveBeenCalledWith('sync-workspace', { reason: 'manual', path: '/tmp/demo' });
});
...
test('workspace switch triggers one best-effort sync', async () => {
  CLIENT = {
    send: vi.fn(async (type, payload) => {
      if (type === 'list-workspaces') {
        return {
          workspaces: [{ path: '/tmp/a', database: '/tmp/a/.beads' }, { path: '/tmp/b', database: '/tmp/b/.beads' }],
          current: { root_dir: '/tmp/a', db_path: '/tmp/a/.beads' }
        };
      }
      if (type === 'set-workspace') {
        return { changed: true, workspace: { root_dir: payload.path, db_path: payload.path + '/.beads' } };
      }
      if (type === 'sync-workspace') {
        return { workspace: { root_dir: payload.path || '/tmp/b', db_path: '/tmp/b/.beads' }, pulled: true };
      }
      return null;
    }),
    on() { return () => {}; },
  ...
  expect(CLIENT.send).toHaveBeenCalledWith('sync-workspace', { reason: 'workspace-switch', path: '/tmp/b' });
});
...
test('auto sync interval sends sync-workspace repeatedly for current workspace', async () => {
  ...
  expect(CLIENT.send).toHaveBeenCalledWith('sync-workspace', { reason: 'auto', path: '/tmp/demo' });
});
```

- [ ] **Step 2: 새 테스트를 실행해 실패를 확인한다**

Run: `npx vitest run app/main.sync-workspace.test.js`
Expected: FAIL because the UI has no sync button/state/interval logic.
...
- [ ] **Step 5: workspace 전환 시 1회 sync와 auto-sync timer 관리 코드를 연결한다**

`handleWorkspaceChange()` 성공 분기와 `workspace-changed` event handler에서 아래를 호출한다.
...
function resetAutoSyncTimer() {
  ...
  const current = store.getState().workspace.current;
  const interval_ms = auto_sync_mode === '30s' ? 30000 : auto_sync_mode === '60s' ? 60000 : 0;
...
test('renders Sync now button near workspace picker', async () => {
  ...
  expect(document.getElementById('sync-now-btn')?.textContent).toContain('Sync');
});
    ...
    send: vi.fn(async (type) => {
      ...
      if (type === 'sync-workspace') {
        return await new Promise((resolve) => {
          resolve_sync = () => resolve({ workspace: { root_dir: '/tmp/demo', db_path: '/tmp/demo/.beads' }, pulled: true });
        });
      }
      return null;
    }),
...
`app/views/nav.js` 또는 workspace picker rendering 함수에 아래 props를 받도록 확장한다.
...
- [ ] **Step 4: `app/main.js`에서 nav에 새 props를 넘기고 localStorage를 연결한다**
...
sync_disabled: !store.getState().workspace.current
```
...
test('ignores sync completion for a workspace that is no longer current', async () => {
  ...
  CLIENT = {
    send: vi.fn(async (type, payload) => {
      ...
      if (type === 'sync-workspace') {
        return await new Promise((resolve) => {
          resolve_sync = () => resolve({ workspace: { root_dir: payload.path, db_path: payload.path + '/.beads' }, pulled: true });
        });
      }
      if (type === 'set-workspace') {
        return { changed: true, workspace: { root_dir: payload.path, db_path: payload.path + '/.beads' } };
  ...
  resolve_sync();
  await Promise.resolve();
  expect(Array.from(document.querySelectorAll('.toast')).every((el) => !(el.textContent || '').includes('/tmp/a'))).toBe(true);
});
```
...
Run: `npx vitest run server/ws.sync-workspace.test.js app/main.sync-workspace.test.js -t "stale workspace|no longer current"`
Expected: FAIL because stale sync results are not guarded.

- [ ] **Step 3: client/server에 current workspace guard를 추가한다**

클라이언트 `syncCurrentWorkspace()`에서 시작 시점 workspace path를 캡처하고 finally 직전 current workspace와 비교한다.
...
Run: `npx vitest run server/bd.test.js server/env.test.js server/ws.sync-workspace.test.js server/ws.list-subscriptions.test.js server/ws.mutation-window.test.js`
Expected: PASS

- [ ] **Step 2: 앱 관련 전체 대상 테스트를 실행한다**

Run: `npx vitest run app/main.sync-workspace.test.js app/main.ws-toast.test.js app/main.board-switch.test.js app/main.test.js`
Expected: PASS
...
Run: `npm test && npm run prettier:write`
Expected: PASS, formatting applied with no remaining failures.

- [ ] **Step 5: 수동 smoke 확인을 수행한다**

Run: `npm start`
Expected:
- workspace picker에서 `beads-ui`, `dotfiles`, `oliveyoung` 전환 시 board가 fatal error 없이 열린다.
...
- `sync-workspace`, `runBdSync`, `buildSpawnPath`, `resetAutoSyncTimer`, `syncCurrentWorkspace` 등 이름을 plan 전체에서 일관되게 사용했다.
- skill artifact는 건드리지 않으므로 `writing-skills` / `skill-creator` 라우팅은 필요 없다.
- 기존 parent bead 후보 `UI-nl9g`는 workspace discovery watcher 후속 안정화 범위라 이번 plan과 완전히 일치하지 않으므로, 잘못된 parent에 metadata.plan을 덮어쓰지 않는다.
