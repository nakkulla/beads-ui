...
- 큐(`queue[]`)에 잔류한 비드 중, 최신 attempt가 `spec_review_stale` 플래그를
  달고 failed로 마감됐고 bd 재관측이 `status=blocked` +
  `blocked_reason=spec_review_stale:revise`인 항목에 `revise_parked` 관측을
  붙인다(`decorateQueue` 축, `prObservationsFor`와 같은 파생 방식). 관측은
  advisory다 — 실제 처분 가부는 클릭 시점 재관측이 결정한다.
...
- `worker-revise-fix` / `worker-revise-approve` 두 핸들러. 머지 클릭 패턴
  ...
  conflict 응답, 부수효과 0) → 클릭 시점 bd 재관측(`status=blocked` +
  `blocked_reason=spec_review_stale:revise`가 아니면 refused, 부수효과 0) →
  실행 → try/catch `reason:'error'` 폴백 → 결과 무관 fanout.
...
- 처분 프롬프트에 명시: 이 디스패치는 beads-ui 처분 클릭(= explicit user
  ...
  대상 리포 main 체크아웃에서 커밋·퍼블리시(fetch, ff-only, push,
  ...
  착수 금지.
- 세션 종료 후 서버가 readback으로 성공 판정(영수증 갱신 + unblock 확인).
  성공 시 `auto_advance` 재개 → 일반 tick이 fresh 영수증으로 admission을
  통과시켜 구현을 새 세션으로 디스패치. 실패 시 기존 실패 배너 경로.

### 3.4 델타 승인 (`worker-revise-approve`)

- 세션 없이 서버가 bd 어댑터(`bd-metadata.js`)로 직접 수행: 대상 리포 base tip
  40hex 산출 → **하나의 `bd update`**로 `spec_review = skipped@<base tip>`
  set-metadata + 계보 `--append-notes`(클릭 처분·원 receipt·승인 시점 base) +
  `status=open` + `blocked_reason` unset → readback → `auto_advance` 재개.
...
- `scheduler.test.js`: resume 디스패치(`resumed_from`)·transcript 유실 폴백·
  종료 후 readback 판정·`auto_advance` 재개.
