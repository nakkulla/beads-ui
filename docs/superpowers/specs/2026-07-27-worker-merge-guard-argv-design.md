...
- **토크나이저**: POSIX 인용 3종(`'…'`, `"…"`(+`\` 이스케이프), 맨몸 `\`)을
  ...
  비대칭 인용 등 해석 불가 입력은 `null`을 반환한다(폴백 트리거).
- **`findMergeViolation(cmd, { conflict_resolution })`** →
  ...
  - argv 선두가 `git push` → 옵션 아닌 인자 토큰 중
    `(^|[:/])(main|master)$`에 걸리는 것이 있으면 `merge_to_base_blocked`.
    (기존 정규식은 push 뒤 아무 데나 `main`이 보이면 차단했지만, argv 기준은
    도착 브랜치가 main/master인 경우만 본다 — `git push origin main:feature`
    는 base 착지가 아니므로 통과. 의도된 의미 강화다.)
...
- 실패 배너: `app/views/worker/index.js`의 failure 조립(:813-821)에 `detail`을
  추가하고, `running-grid.js` 배너에 매치 명령을 한 줄(`<code>`, truncate)
  표시한다. lit-html 텍스트 바인딩이므로 이스케이프는 자동이다.
- 디스코드 알림 정합: 미구현 스펙
...
1. 실측 오탐 명령(위 rg 패턴)이 blocker 없이 통과한다(session 단위 테스트).
   `git commit -m "gh pr merge 관련 수정"` 같은 인용 토큰 케이스도 통과한다.
2. `gh pr merge 311`, `git push origin main`, `git push origin HEAD:main`,
   `git merge origin/main`(+`--ff-only` 변형)은 여전히 차단된다.
3. `bash -c "gh pr merge 311"`, `eval 'git merge x'`, `$(gh pr merge 311)`은
   차단된다(재귀 검사).
4. 비대칭 인용 등 토큰화 실패 입력은 기존 정규식과 동일하게 판정된다(폴백).
5. `git merge-base --is-ancestor A HEAD` 통과와 conflict-resolution attempt의
   `git merge` 허용은 기존 테스트 그대로 green.
