...
**정정(UI-01wh).** 이 문단의 "배너와 `재개` 버튼은 없애지 않는다"는 UI-01wh가
뒤집었다: 상단 배너는 제거됐고, `재개`는 **막힌 대기 행의 조작**(`▶ 재개`)이 됐다.
막힌 행이 없으면 정지를 그리지 않는다. 이 스펙의 다른 결정 — 해제 단위, ADR 0048의
"사람의 승인 한 번", 자동 재개는 hold를 풀지 않는다 — 는 그대로다. 근거는
`2026-09-09-worker-gate-on-card-no-banner-design.md`가 소유한다.
