...
- 상태: 사용자 승인(2026-09-23) 뒤 spec_review r1(astra) REVISE 5건 반영본 — 이력 행에도 링크·오류 표시, 화면의 kind 목록 갱신, 후보 6·7 summary 정정, UI-wecw는 축약 대신 조항 보존 재진술(통합 11건·유지 1건).
...
목표: (1) ADR 탭이 `docs/adr/`와 `docs/adr/history/` 두 위치를 읽어 dotfiles ADR 체계 v3(ADR dotfiles/dotfiles-u7ub D1·D2·D10)의 레이아웃을 가진 저장소와 아직 평면인 저장소를 모두 정확히 보인다. (2) beads-ui 현재표를 42행에서 12행으로 줄인다 — 주제 통합 10건과 조항 보존 재진술 1건, 유지 1건, 강등 4건 — 그리고 superseded 파일 전부를 `history/`로 옮긴다. (3) `AGENTS.md` 태그 4개를 새 id로 옮기고 인덱스·인용 검사기·`repo-ops/script/verify`가 rc=0이다.
...
D2. **경로를 쓰는 곳은 `file` 하나를 따른다.** `app/views/adr/index.js`의 현재표는 `docCell('docs/adr/' + adr.file)`과 `frontmatter_errors` 필터(`e.file === adr.file`)를 그대로 쓰며 `history/` 접두가 붙은 값을 그대로 잇는다. 이력 표(`historySection`)는 오늘 제목과 대체 id를 문자로만 그리므로 같은 규칙으로 바꾼다: 제목 칸은 현재표와 같은 `docCell('docs/adr/' + adr.file)` 링크이고, 그 레코드의 `frontmatter_errors`가 있으면 현재표와 같은 오류 표시를 붙이며, `대체` 칸은 대상 레코드가 스냅샷에 있으면 그 파일로 가는 링크, 없으면 지금처럼 id 문자다. `adr-signals.js`의 `adr_files`(교차 인용 스캔 대상)는 두 위치의 파일을 `docs/adr/<상대 경로>`로 싣는다 — 오늘 평면 디렉터리에서 superseded 파일도 스캔하던 동작과 같다. `adr-watch.js`는 `docs/adr`를 이미 재귀 감시하므로 바꾸지 않는다.

D3. **위치 판정은 복제하지 않는다.** `history/`에 accepted가 있거나 루트에 superseded가 있어도 리더는 status대로 현재·이력에 넣고 경고를 만들지 않는다. 위치 불일치는 설치본 `adr-index.py --check`가 내는 `index_drift`로만 보인다(ADR 0039의 "규칙은 JS로 복제하지 않는다" 그대로). kind 어휘는 두 목록에 산다 — 서버 레지스트리 `CANDIDATE_ERROR_KINDS`(`server/adr/adr-registry.js`)와 화면의 `CANDIDATE_OTHER_KINDS`(`app/views/adr/index.js`, `kindLabel`이 읽는 목록). 둘 다에 `title_too_long`을 더해 `기타` 대신 이름표 `제목 초과`로 그린다 — 어휘 등록일 뿐 판정은 없다.
...
| UI-wecw | 통합 → UI-u6ud-11 | 조항 보존 재진술 — 제목이 실은 0013 승계 조항(defaults 어휘·검증의 dotfiles 소유권, fail-quiet 읽기·strict 쓰기)이 summary에 없어 v3 D6의 축약 조건을 못 채운다(r1 F5); 주제는 다른 저장소 CLI 소비라 단독 ADR로 남긴다 |
...
- `app/views/adr/index.test.js`: (1) 실제 `status: superseded`·`file: 'history/<name>'` 레코드가 이력 표에서 `docs/adr/history/<name>` 링크와 그 `file`의 frontmatter 오류 표시를 얻는다(현재표에 `history/` 경로를 주입하는 사례와 구분), (2) 이력 `대체` 칸이 스냅샷에 있는 대상은 링크, 없는 대상은 id 문자, (3) 후보 오류 `title_too_long`이 `기타`가 아니라 `제목 초과`로 그려진다.
- `server/adr/adr-registry.test.js`는 없으므로 `adr-signals.test.js`에 `CANDIDATE_ERROR_KINDS`의 `title_too_long` 포함 사례를 둔다.
...
- 배포는 `repo-ops/config.toml [deploy]`가 소유한다. 인도 증거는 공유 서버 재시작 뒤 ADR 탭이 beads-ui 저장소를 현재 12행·이력 95행으로, dotfiles 저장소를 그 시점 레이아웃대로 보이는 것이다.
...
- Worker 주소·defaults 소비: 되돌림 어려움=충족(`server/worker-url.js` 어댑터·설정 화면 세 값·dotfiles `worker-url` CLI와 kv 계약이 함께 움직인다), 의외=충족(왜 서버가 공통 파일을 직접 읽지 않고 적용값을 추측하지 않는지), 대안=충족(kv 단일 저장·서버 직접 읽기·연결 점검 API). `summary`: "Beads UI는 저장소 defaults 저장값과 Worker 주소 적용값을 분리하고 공통 주소의 조회·쓰기는 설치된 dotfiles worker-url CLI로만 하며 kv·공통 파일을 직접 쓰거나 적용값을 추측하지 않는다; defaults의 키·schema·허용값·부재 규칙은 dotfiles 계약이 정의하고 beads-ui는 어휘를 넓히거나 하네스 기본값을 복제하지 않으며 kv 읽기는 fail-quiet, 사용자 편집 쓰기는 strict 거절이다; 설정 화면은 적용 주소·저장소 예외·공통 기본값을 따로 보이고 새 연결 점검 API는 두지 않는다" → ADR, supersede UI-wecw
- 리더의 두 위치 읽기(D1~D3): 되돌리기 비용 X(리더 한 파일과 `file` 소비자 두 곳) / 놀라움 X(dotfiles 계약을 따를 뿐) / 대안 있음 → ADR 아님
- 강등 4건과 유지 1건의 판정: 되돌리기 비용 X(status 줄) / 놀라움 X(표가 근거를 적는다) / 대안 있음 → ADR 아님
