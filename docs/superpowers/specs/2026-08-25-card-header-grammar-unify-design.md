...
| 5 좌표·실행 사실 | 어느 레포·레인·경로의 것이고 무엇으로 얼마나 돌았나 | 레포 배지 · 직렬 레인 칩 · `route` · `↩ from` · `오케`/`워커` exec 칩 · `exec_receipt` · `영수증 잔여` 판정 칩(클릭 = 사유 팝업) · 본체 usage/비용(직접 세션은 현재 대화 기준) · 실패 로그 경로(`log_path` + 복사) |
