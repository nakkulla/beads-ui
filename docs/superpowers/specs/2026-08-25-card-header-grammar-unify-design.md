...
| 4a 의존 | 지금 갈 수 있나 | `▶ 연결 n` 발차 칩 · `⛓ <ID>` (타 레포는 `<workspace>/<ID>` 표기와 색) · `스펙 대기` · `라우팅 필요`/`본문 필요`/`스펙 충돌`/`스펙 미발행`(판정 칩, 클릭 = 사유 팝업) · `→ <ID>` · `⏳ <n>초` 유예 |
...
두 문구다. 4a의 타 레포 `⛓ <ID>`는 `<workspace>/<ID>` 표기와 색으로 rig를
구분하고, 4b의 `🔓 <ID>`는 선행 대기 타일에서 해제된 선행(복귀 대기)도 말한다.
