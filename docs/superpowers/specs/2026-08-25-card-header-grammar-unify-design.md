...
fail-quiet를 그대로 유지하므로, 칩이 없는 Worker 탭에서는 좌표 줄 자체가 서지
않는다.

| # | 줄 | 싣는 것 |
|---|---|---|
| 1 | **정체성 + 조작** | `[상태점/grip] [순번] [ID] [P n] [상태 아이콘·뱃지·PR 링크]` ··· 오른쪽 끝 `[경과/usage] [조작 버튼]` |
| 2 | 제목 | 카드형 변형만. 한 줄 변형은 1줄에 제목이 함께 있다 |
| 3 | 진행 | stepper(후보) 또는 활동 줄·위임 칩(실행중) |
| 4 | **의존 · 겹침 칩** | `⛓ blocked` · `⧉ 겹침` · `scope 없음` (`.worker-deps`) |
| 5 | **좌표 · 실행 설정 칩** | `레포 → 레인 → route → from` 다음에 `오케`/`워커` exec 칩 |
...
|---|---|---|
| 1 정체성 (왼쪽) | 이것이 무엇인가 | 상태점 · grip · 직렬 순번 · ID · `P n` · `↻` 재개 계보 · `worker-ineligible` · legacy `worker-serial` · PR/repair PR 링크 · 상태 뱃지 · reason |
| 1 조작 (오른쪽 끝) | 내가 여기서 무엇을 하나 | 경과/상태 라벨 · usage · merge-step · `▤ 세션` · `⏸`/`▶` · `↻ 이어하기` · `머지`/`취소` · `폐기` · `✕` |
...
| 5 좌표·실행 설정 | 어느 레포·레인·경로의 것인가 | 레포 배지 · 직렬 레인 칩 · `route` · `↩ from` · `오케`/`워커` exec 칩 · `exec_receipt` |
| 6 액션 foot | (카드형) 무엇을 하나 | `대기로 ↴` · `머지`/`취소` · `폐기` · 파킹 처분 버튼 · 폐기 영수증 |
| 7 시각 | 언제 것인가 | `생성` · `수정` |
...
- 렌더러 단위 테스트에서 다음을 확인한다.
  ...
  - 좌표 칩과 exec 칩이 모두 없으면 그 줄이 렌더되지 않는다.
