...
| 4b | 정보 칩 (하단, 저채도) | `🔓 <ID>` · `⧉ <ID>` · `scope 없음` (`.worker-deps--secondary`) |
...
| 4a 의존 | 지금 갈 수 있나 | `▶ 연결 n` 발차 칩 · `⛓ <ID>` (타 레포는 색만 다름) · `스펙 대기`(판정 칩, 클릭 = 사유 팝업) · `→ <ID>` |
| 4b 정보 | 왜 이제 갈 수 있나 · 누구와 부딪히나 | `🔓 <ID>` · `⧉ <ID>` · `scope 없음` |
...
- **완료 행(2줄 `two_line` · 3줄 `doneThreeLineRow`)은 바꾸지 않는다.** 완료 행은
  ...
  길어진다. §5.1이 usage를 5번 슬롯으로 배정하지만 완료 행에는 그 줄 자체가 없고
  만들지도 않으므로, 완료 행의 usage는 지금 자리에 그대로 둔다.
