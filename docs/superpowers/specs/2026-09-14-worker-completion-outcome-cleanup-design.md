...
- squash 때문에 해시 계보가 남아 있는 경우 기존 내용 포함 판정과 필요한 기존 branch
  archive를 유지한다. 새 archive 서비스나 정상 조상 branch의 불필요한 archive는 만들지 않는다.
...
- waiting을 사유로 나눠 선행 대기는 bd ready로 자동 복귀하고 반복 기준 이동은 후보를 보존해 같은 세션에서 이어간다 — 지속적 상태·재개 계약이며 잘못 복귀하면 후보 유실/중복 구현 위험이 있다; 단일 자동 복귀와 가짜 blockers를 기각한다. ADR 0028을 대체한다. `summary`: "waiting은 선행 대기와 기준 이동 대기를 구분하며 선행은 bd ready로 자동 복귀하고 기준 이동은 보존 후보를 기존 세션에서 이어간다" → ADR, supersede 0028
- quick_fix 재개 분류에 base_moved를 session으로 명시하고 기존 기계 정리 재시도를 유지한다 — 여러 소비자가 공유하는 재개 의미이며 미지 사유 기본값에 맡기는 대안을 기각한다. ADR 0042를 대체한다. `summary`: "quick_fix 재개는 종료 사유로 session과 settlement를 구분하며 base_moved는 보존 세션 이어하기이고 기계 정산은 정리 재시도다" → ADR, supersede 0042
- 완료 worktree·branch 삭제를 같은 잠금에서 증명·실행한다 — 국소 구현 수리이며 외부 선택을 새로 만들지 않아 별도 결정 기록의 세 조건을 모두 만족하지 않는다 → ADR 아님.
