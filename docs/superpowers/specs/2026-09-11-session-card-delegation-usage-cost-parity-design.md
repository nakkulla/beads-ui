...
정정(UI-mscc): 위 슬롯 5의 본체 기준과 native child 합산 제외는
`2026-09-15-parent-child-usage-total-design.md` §3–6 및 ADR UI-mscc로 대체한다.
부모와 연결된 자식의 검증된 직접 사용량은 각각 한 번 합산하고, 미확정 범위는
부분 집계로 표시한다. 부모 단독 비용과 자식별 비용은 분해해서 확인할 수 있다.
직접 세션의 현재 대화 범위와 워크스페이스 자동 가산 제외는 그대로 유지한다.
...
Codex의 응답별 직접 사용량은 UI-mscc §3의 `token_usage_record.payload.usage`로
읽으며 thread·turn·response와 attempt 경계로 중복을 제거한다. `token_count`와
turn/thread 누적값 및 종료 usage는 대조값이며 직접 기여에 추가로 더하지 않는다.
과거 형식의 직접 귀속·경계가 입증되는 경우만 baseline 차이를 사용한다.
...
초기 단가 적용은 이 Bead의 구현 컨트롤러가 **구현 검증 후 PR 인도 전**에 소유한다. 기존 배포 스크립트가 단가를 쓰는 것으로 간주하지 않는다. 순서는 다음과 같다.

1. 공유 서버 프로세스의 실제 설정 경로를 확인한다. `BDUI_CONFIG_PATH`가 없으면 `getConfig()`의 기본 경로를 사용한다. 현재 파일의 해시와 접근 권한을 기록하고, 비공개 백업과 수정 후보를 저장소 밖에 만든다. 보고서에는 비밀을 포함한 설정 원문을 남기지 않는다.
2. §5의 정확한 모델 ID가 현재 카탈로그의 어느 모델 이름으로 해석되는지 확인하고 **빠진 단가 필드만** 후보에 추가한다. 기존 단가와 나머지 설정을 보존한다. 후보의 TOML 파싱과 `resolveCatalog` 결과에서 기대 단가·모델 매핑·기존 값 보존을 확인한다.
3. 공유 배포와 같은 `.worktrees/.repo-ops-deploy.lock`의 `fcntl.flock` 안에서 원본 해시를 다시 확인한다. 같을 때만 기존 접근 권한을 유지한 같은 디렉터리 임시 파일로 원자 교체하고, 디스크를 다시 읽어 파싱·단가·보존 값을 검증한다. 해시가 달라졌으면 덮어쓰지 않고 최신 파일에서 1~2단계를 다시 준비한다. 이 단계에서는 공유 서버를 재시작하지 않는다.
4. 저장된 설정을 사용하는 별도 localhost 후보 서버에서 실제 `worker-snapshot.runner_catalog`의 모델별 단가를 확인한다. 새 코드의 가격 계산과 두 카드 검증까지 통과해야 PR을 인도한다. 설정 전후 해시, 적용한 가격 필드, 카탈로그 확인 결과를 완료 보고서에 남긴다. 후보 서버는 확인 후 종료한다.
5. 이후 기존 Worker 머지·배포가 설치·빌드·재시작하고 핀된 SHA, 프로세스 경로, 포트, HTTP 응답을 검증한다. 프로세스가 캐시한 카탈로그는 이 재시작에서 저장된 단가를 읽는다. 공유 배포 절차나 코드의 상시 기본 단가는 추가하지 않는다.

중단 후에는 디스크 단가와 보고된 해시를 먼저 확인한다. 이미 기대 단가가 있으면 덧쓰기 없이 다음 검증 단계부터 재개하고, 미적용이면 1단계부터 준비한다. 다른 변경이 있으면 백업 전체로 되돌리지 않는다. 설정 검증 실패는 PR 인도를 막고 기존 Worker 배포 실패는 기존 배포 실패 절차로 처리한다. PR이 아직 머지되지 않았으면 새 카드 기능은 미배포 상태이며, 단가 저장만으로 완료를 주장하지 않는다. 배포 뒤 실제 세션 한 건의 카드·카탈로그 확인은 완료 보고서에 잔여 관찰로 운반한다.
...
- native child의 개별 토큰·환산 비용을 카드에도 보이되 합산에서는 제외한다(supersede ADR 0047). 되돌리기 어려움: 예(공유 표시·회계 의미), 맥락 없으면 놀라움: 예(보이는 자식 비용이 부모 금액과 별개), 실제 절충: 예(완전 합계 대신 검증된 관측). `summary`: "Codex native child의 개별 사용량과 환산 비용은 카드·상세에 표시하고 중복 여부가 미확인된 값은 부모 합계에 더하지 않는다" → ADR
