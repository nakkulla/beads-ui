...
3. 이동 대상이 **리터럴로 해석된다** — 리터럴 경로, `~` 확장, **`$HOME`/`${HOME}` 확장**, 또는 같은 셸 스코프에서 이동보다 **앞선 별도 명령**으로 온 **리터럴 단일 대입** 1단계 확장(`H="/abs/path"; cd "$H"`)까지만. 대입의 값 자체도 리터럴·`~`·`$HOME` 조합까지 해석한다(`H="$HOME/…"; cd "$H"`). 그 밖의 확장은 해석하지 않는다.

   대입은 **이동과 별도 명령**이어야 한다. 같은 명령의 선행 대입(`H=/abs cd "$H"`)은 해석하지 않는다 — POSIX 2.9.1 이 단순 명령의 단어 확장을 그 명령 자신의 대입보다 **먼저** 수행하므로 그 형태는 실제로 `cd ""` 이 되어 repo 를 떠나지 않는다. 해석 가능한 것으로 취급하면 자기 base 랜딩을 통과시키는 우회로가 된다.
