 ...
 * @property {string} head_sha   - claim이 잡힌 head (40hex 소문자). exhausted 전이 시
 *                                 최종 관측 head로 갱신된다(§5.2).
...
- `normalizeMergeQueue`가 정규화한다: 모양이 어긋나면(필드 누락·SHA 아님·state enum 밖)
  필드를 버린다(오류 아님). 구형 `queue.json`에는 없으므로 부재가 정상이다.
- `MergeHold`에 선택 필드 `auto_review_wait: 'slot'|null`을 더한다 — 결정 1의 "슬롯이
  없어 대기 중"을 화면에 투영하기 위한 유일한 durable 표시다(ADR 0015 "슬롯 부재
  보류는 화면 보류 사유로 투영"). `setMergeHold`는 이 필드가 바뀌어도 write한다.
...
1. `reason ∈ REVIEW_AFTER_MERGE_GATE_REASONS`(`review_receipt_missing|stale`)일 때만.
   `review_receipt_undetermined`는 probe 오류이지 판정이 아니므로 버튼과 같이 제외한다
   (UI-d7fy §5.1). `head_sha`가 40hex가 아니면 제외.
...
4. claim 판정: `review_dispatch`가 없거나 `review_dispatch.head_sha !== head_sha`일 때만
   dispatch. 같은 head에 `exhausted`면 no-op(버튼이 출구). 같은 head에 `active`인데 3의
   attempt가 없으면 기록 불일치(재시작 회복 누락)이므로 `exhausted`로 정정만 하고
   dispatch하지 않는다.
5. fence(결정 1): `authority.source === 'manual'`이면 면제. `'automatic'`이면
   ...
   `setMergeHold`로 `hold.auto_review_wait='slot'`을 쓰고 끝낸다. 통과하면
   `auto_review_wait`는 null로 되돌린다. 슬롯이 비면 attempt 종료가 큐 변경 이벤트를
   내고 `hasHeldEntry()`가 드레인을 요청하므로(현 `kick()` 배선) 추가 깨움이 필요 없다.
...
- 클릭 `start`: `enqueueMergeManual`이 manual authority 부여 + attempt 등록 + claim을
  한 CAS로 커밋.
- 자동 `startAuto`: 새 저장 연산 `claimAutoReviewDispatch(workspace, { bead_id, head_sha, attempt_id, session_source })`가
  **행의 기존 authority id에** `review_session` attempt(`origin:'auto'`, `status:'pending'`,
  ...
  `applyUnconditional` 안에서. §4의 2·3·4 조건을 write 안에서 다시 검사해 어긋나면
  `{ok:false, reason}`으로 거부한다(`review_dispatch_claimed`·`review_session_in_flight`·
  `authority_missing`). write 실패면 dispatch 없음(UI-d7fy 속성 1과 동일).
...
- launch 실패(`dispatched.ok !== true`·throw)는 클릭과 같이
  `settle(attempt_id, {outcome:'failed', from:'launch', cause:'launch_failed:<reason>', hold_reason: reason, final_head_sha: head_sha})`
  — 결정 2에 따라 `bead_running`·`bd_snapshot_failed`·`worktree_*`·spawn 오류를 구분하지
  않고 모두 `exhausted`가 된다(§5.2).

`complete`(완료 판정)는 변경 없다 — attempt에 `origin`이 무엇이든 같은 코드가 돈다.
...
`review_dispatch.attempt_id`와 같으면 `state:'exhausted'`, `at`, 그리고
`head_sha := isSha(final_head_sha) ? final_head_sha : review_dispatch.head_sha`로 갱신한다.

head_sha를 최종 관측 head로 옮기는 이유: 세션이 REVISE 수정을 push한 뒤 영수증을 못
쓰고 죽으면 head는 X'인데 claim이 X에 남는다. 다음 `kick()`은 X'를 "claim 없는 새
head"로 읽어 한 번 더 자동 dispatch하게 되고, 이는 같은 lineage에 두 번째 외부 리뷰를
보내는 것이다. 최종 head로 옮기면 X'는 `exhausted`이고 버튼만이 출구다.
...
| 상황 | 버튼 | 클릭 효과 |
|---|---|---|
| claim `active`(자동 또는 클릭 세션 실행 중) | 잠김(기존 `review_session.active` 가드) | 서버 in-flight 가드로 no-op |
...
| `review_receipt_undetermined` | 없음(기존) | — |
...
- ADR 0004(ancestry 신선도)와 무관 — 게이트 판정은 바뀌지 않는다.
...
- 관찰: 계약 102행 첫 문장은 `probe_error`도 자동 dispatch 대상으로 읽힌다. 이 스펙은
  버튼과 같은 이유로 `undetermined`를 제외한다(probe 오류는 판정이 아니고 다음 관측이
  재판정). 어긋남이 문제가 되면 dotfiles 쪽 문구 정정 대상이다.
...
- `queue-store.test.js`
  ...
  - `settleReviewSession`: `current` → claim 삭제; `failed` → `exhausted`, `head_sha`가
    `final_head_sha`로 갱신; attempt_id가 claim과 다르면 claim 불변.
...
- `merge-queue.test.js`
  ...
  - `undetermined`·authority 없는 행 → dispatch 없음.
...
- `review-session.test.js`
  - `startAuto`: write 실패 시 launch 없음; launch 실패 → `failed: launch_failed:*` +
    claim `exhausted`; 성공 시 클릭과 같은 프롬프트·resume 선택.
