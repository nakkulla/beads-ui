...
화면이 repo별 Dolt server 상태에 따라 불안정하게 깨지지 않도록 한다.
동시에 현재 workspace 기준으로 원격 이슈를 명시적으로 또는 주기적으로
동기화할 수 있는 UX를 추가한다.
...
2. **원격 sync UX 제공**
   ...
   - workspace 전환 직후에는 best-effort 1회 sync를 시도해 최신 상태를
     가져오되, 실패해도 로컬 데이터 렌더링은 유지한다.
...
- 현재 `bd` subprocess 호출은 기본적으로 `--sandbox`를 prepend 하므로,
  일반 list/read 경로는 auto-sync를 기대하지 않는다.
- 일부 workspace에서 Dolt server가 이미 떠 있으면 화면이 정상 동작하지만,
  Dolt server가 내려간 workspace로 전환했을 때 daemon 환경에서 `dolt`를
  찾지 못하면 auto-start 실패가 발생한다.
...
daemon을 띄우는 `startDaemon()`과, 서버 프로세스 내부에서 `bd`를 spawn 하는
경로 모두에서 `dolt`가 안정적으로 보이도록 PATH를 정규화한다.
...
이 변경의 목적은 shell마다 다른 PATH 차이로 인해 daemon이 `dolt`를 찾지
못하는 상황을 줄이는 것이다. 이때 특정 경로를 하드코딩된 절대 실행 파일로
강제하지 않고, 표준 경로 보강 수준에 그친다.

#### 1-2. `bd` subprocess 환경 정규화

`server/bd.js`에서 `bd`를 spawn 할 때도 같은 PATH 정규화를 적용한다.
이렇게 하면 daemon이 정상 기동된 이후에도 workspace별 Dolt auto-start 경로가
일관되게 `dolt`를 찾을 수 있다.
...
- 요청: `sync-workspace`
- payload:
  - `path?: string` — 생략 시 current workspace 사용
...
버튼은 현재 workspace가 없거나, 현재 workspace가 Dolt backend가 아니라고 확인된 경우 비활성화한다.
...
interval은 정리하고 새 workspace 기준으로 다시 시작한다. 현재 workspace가 Dolt
backend가 아니면 auto-sync는 시작하지 않는다.
...
| `server/ws.js` | `sync-workspace` 메시지 처리, workspace readiness/sync orchestration, refresh 연계 |
...
| `app/views/nav.js` 또는 `app/views/workspace-picker.js` | `Sync now`, `Auto sync` UI 추가 |
...
- workspace는 있지만 Dolt backend가 아님 → sync API는 no-op 또는 명확한 bad request
- `bd dolt start`는 성공했지만 `bd dolt pull` 실패 → 로컬 데이터 유지 + toast 에러
...
- 네트워크가 불안정하여 pull이 오래 걸림 → 버튼/상태에 syncing 표시 유지, timeout 정책 필요
- sync 성공했지만 변경 없음 → success toast는 간단히 유지하거나 조용한 처리 허용
- board/detail/list가 동시에 열려 있어도 refresh는 active subscriptions 기준으로 한 번만 수행
...
3. `sync-workspace`가 stopped Dolt server에서는 `bd dolt start` 후 pull을 수행한다.
