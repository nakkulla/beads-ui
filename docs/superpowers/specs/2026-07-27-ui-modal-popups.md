...
- 유지: backdrop 클릭 닫기, `role="dialog" aria-modal="true"`, 열기 진입점
  (보드 카드 클릭, 워커 ⓘ/ID 클릭), md-viewer 중첩 오버레이.
...
- 유지: follow 토글, 일시정지/이어하기 등 드로어 내부 기능, 상세 패널 쪽
  세션 로그 오버레이 동작.

## 수용 기준

1. 워커 탭에서 ⓘ/ID 클릭 시 레인 레이아웃이 밀리지 않고 모달로 진행상황이
   뜨고, backdrop/✕로 닫힌다.
2. 이슈 상세가 데스크톱에서 중앙 모달로 뜨고, 모바일에서는 전체화면 유지.
3. 기존 드로어·상세 패널 기능(팔로우, resume, 편집, md-viewer) 회귀 없음 —
