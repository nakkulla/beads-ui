...
- 상태: 사용자 승인(2026-09-09) 뒤 spec_review r1(astra) REVISE 11건 반영본 — 사실 카드는 selector 입력과 출처만, `--controller-runtime`은 실제 runner, dotfiles_root는 git 루트, `--verdict` 조건 삭제, 선점 해제는 기존 reconcile, `worktree_setup`은 queue-store, env 재시도는 resume 경로, UI-47y7 선행.
...
- `## 시도 사실`(신설, `attemptFactsDirective(facts)`): 줄마다 값과 `source`를 적는다. `attempt=<BDUI_ATTEMPT_ID> bead=<id> route=<route> workflow_mode=fast_track`; `base=<remote>/<branch>@<sha>`; `worktree=<abs>`; `dotfiles_root=<abs>`(`git -C "$(readlink -f ~/.claude/skills/workflow)" rev-parse --show-toplevel`; 설치본은 `dotfiles/.worktrees/.repo-ops-deploy/src/shared/skills/flow/workflow`라 상위 단계 셈은 틀리고 git 루트가 맞다; 실패 시 줄 생략); `workflow_python=<dotfiles_root>/.venv/bin/python`(그 경로가 존재할 때만 — 배포 워크트리에 venv가 없으면 줄이 빠지고 세션은 dotfiles-3zsj D10의 자기 재실행에 기댄다); `node_modules=<ok|failed:<사유>|skipped>`(D3 결과); `origin/<branch> tip=<sha>`(재개·충돌 attempt에서 원격 브랜치가 있을 때); `선택 입력: impl_runtime=<v|없음> impl_model=<v|없음> impl_effort=<v|없음> impl_speed=<v|없음> impl_dispatch=<v|없음> (source=bead|workspace_kv|없음)`와 `리뷰어 프리셋: <token> → <model>/<effort> (source=핀 사본 review.reviewers, digest=<execution-defaults.js provenance>)` — 값은 Worker가 dispatch 시 이미 읽은 Bead 메타데이터·워크스페이스 kv(`exec-preset-coordinator.js resolveForDispatch`)와 핀 사본의 `review.reviewers` 표에서만 오고, 최종 선택은 세션이 정본 `impl-selector.py`를 1회 돌려 확정한다(이 스펙은 selector 판정을 대체하지 않는다; 카드는 selector가 읽을 입력과 출처를 미리 보여 `harness.yaml`·`bd kv`·`generated/` 탐색을 없애는 것이 목적). 리뷰 전송 방식(`via <transport>`)은 핀 사본에 리뷰용 `runtime_transports`가 없어 적지 않는다; `이미 되어 있음:` 줄은 이 attempt에서 실제로 확인된 항목만 적는다 — `status=<bd show 값>(선점: Worker|없음)`(D2의 선점이 이 attempt 기록에 `worker_claim=written`으로 남았을 때만 "Worker"), `route marker: Worker attempt는 마커 없이 진행한다 — route-nudge 리마인더가 보이면 무시한다(dotfiles-3zsj D1 설치 뒤에는 나오지 않는다)`, `session_ref 불필요`, `npm ci=<결과>`; 스크립트 호출 줄(`stale-rereview-inputs.py <id> --json` — 출력은 workflow `references/execution-spec-backed.md`의 `Staleness re-review` 절차를 따른다. `needs_judgment`는 최종 판정이 아니며 `verdict_draft_blockers`가 지정한 항목을 비교한 뒤 정본 절차로 분류·기록한다, `impl-selector.py --controller-runtime <이 attempt의 runner: claude|codex> --route <route> --bead <id> --repo <worktree> --json`, `check-completion-report.py <path>`(설치본 `--help`에 `--template`이 있으면 `--template --lane worker --identifier <attempt-id>` 줄을 더한다), `land-quick-fix.py …`는 quick_fix 레인만) — 존재하지 않는 옵션 조건은 두지 않는다(fail-quiet는 스크립트 파일 부재에만 적용); 마지막에 대상 리포 `AGENTS.md`의 `## Worker pitfalls` 절 본문을 그대로 붙인다(절이 없으면 생략). 이 블록은 ADR 0012를 지킨다: dotfiles 파일을 읽지 않고 beads-ui 핀 사본·설치본 스크립트의 존재 여부·Worker 상태만 쓴다.
...
- 프리앰블은 `runner/codex.js:464`에서도 같은 `applyPreamble`로 Codex 세션에 들어가므로 `--controller-runtime`은 attempt의 runner 값으로 채우고 `claude.test.js`·`codex.test.js` 양쪽에 고정한다.
- `preamble.test.js` 스냅샷을 갱신하고, 시도 사실 블록은 값이 없는 줄을 생략하는 fail-quiet 케이스(dotfiles_root 부재·workflow_python 부재·node_modules skipped·재개 아님·선점 없음)를 각각 고정한다.

### D2. dispatch 시 `in_progress` 선점 (`scheduler.js`)

`dispatch`(구현 attempt 시작) 경로에서만, durable `prerecordAttempt`가 성공한 뒤, 직전 readStatus가 정확히 `open`일 때 `bd update <id> --status in_progress --json`을 1회 쓴다(session_ref는 쓰지 않는다 — 계약 `workflow-contract.md:162`가 Worker runner의 session_ref 기록을 금지). `open`이 아니면 쓰지 않는다(fail-quiet). `dispatchExternalConflict`·`relaunchResolvedAttempt`처럼 `resolved` Bead를 다루는 공유 `installGuardHook` 지점에는 넣지 않는다 — `resolved`를 되돌리면 `lifecycle.transitions.delivery`를 깬다. 선점은 attempt 기록에 `worker_claim: pending → written → released`로 남긴다(`prerecordAttempt` 뒤 `pending`, `bd update` 성공 뒤 `written`, 해제 뒤 `released`; `server/worker/queue-store.js makeAttempt`의 허용 필드에 추가). 그 뒤 모든 실패 exit가 기존 `releaseBeadClaim`에 닿는 경로여야 하며, 해제는 기존 `reconcile`(`scheduler.js:7202`, 죽은 attempt 정산)에 연결한다: `worker_claim=written`인 채 terminal이 된 attempt(세션 없음·PR 없음)는 정산에서 `open`으로 되돌리고 `released`를 기록한다. 기록 없는 Bead를 스캔하는 별도 재조정은 두지 않는다(소유권 근거가 없다). 회귀: 선점 직후 프로세스가 죽은 attempt를 재시작 뒤 reconcile이 해제한다. 세션은 이미 claim된 상태로 시작하므로 impl-gate의 `status ≠ in_progress` 차단이 첫 턴부터 사라진다; readiness 술어(route·spec_review·impl_entry·의존성)는 상태와 무관하게 그대로 판정한다.

### D3. 워크트리 의존성 설치 (`worktree.js`, `scheduler.js`)

워크트리 생성 직후 그 워크트리에 `package-lock.json`이 있으면 `npm ci --prefer-offline --no-audit --no-fund`를 실행한다(타임아웃 300초). 결과(`ok|failed:<tail>|skipped`)를 attempt 기록 `worktree_setup`에 저장하고(`queue-store.js makeAttempt:2893`·`:6770`의 허용 필드와 타입에 추가 — 지금은 복사 시 제거된다) D1의 `node_modules=` 줄에 싣는다. 회귀: append → update → 재로드 뒤 `worktree_setup`·`worker_claim` 보존. 실패는 dispatch를 막지 않는다(세션이 지금처럼 복구). `repo-ops/config.toml`에 새 키를 두지 않는다 — `allowed_top_level_keys`는 `[base, verify, deploy]`로 고정(dotfiles `workflow-state.yaml`). 다른 언어 생태계의 설치 훅은 이번 범위 밖 관찰이다.
...
`ENV_ERROR_PATTERNS`에 `{group: 'provider_capacity', re: /at capacity|overloaded|API Error: 529|unexpected status 5\d\d/i}`를 더한다. `세션 실패 — 환경 · codex turn failed: Selected model is at capacity` 요약은 `env` tier가 되어 기존 `RETRY_DELAYS_MS`(2·5·15분) 사다리를 탄다. 테스트는 그 문자열 원문과 `overloaded_error`를 고정하고, `individual` 판정이 유지되는 대조 문자열(테스트 실패 보고)도 고정한다.

D5b. env 재시도의 재개 경로. 지금 env 재시도는 `continuation_choice=prior_attempt`일 때만 `resume`이고 그 밖에는 새 `dispatch`다(`scheduler.js:11889`); 새 dispatch는 커밋이 남은 워크트리 잔여를 거부해(`:7579`) `retry_deferred`로 미뤄질 뿐 완료 직전 세션을 잇지 못한다(`:11931`). 따라서 env tier 재시도에서 실패 attempt의 워크트리에 base보다 앞선 커밋이 있거나 원격 브랜치가 push돼 있으면 `resume(before_attempt_id)`로 그 attempt를 잇는다(기록된 provider·tuple 승계, ADR 0046); 그렇지 않으면 현행 fresh dispatch다. 회귀: 커밋을 남긴 attempt → 용량 오류 → 재시도 예약 → 실제 `resume` 발화까지 한 사례와, 커밋 없는 attempt의 fresh dispatch 유지. 세션 안의 리뷰 레그 재발화 규칙은 dotfiles-3zsj D11이 소유한다.
...
한 패킷. 순서: D5·D5b(분류·재개 경로) → D3·D2(queue-store 필드 → dispatch 경로 → reconcile) → D1·D4·D6(프리앰블·argv·재개) → D7(AGENTS.md) → 스냅샷·빌드.
...
| 형제 | beads-ui | user_request | 독립 착지 가능한 검증 묶음 — 번들 미커밋은 배포·dev 경로 공개 동작 변경; UI-47y7은 이 Bead 뒤에 착지한다(UI-47y7 blocked_by UI-cvwo)고 D7 부록 A의 번들 문장을 최종 대체한다 | UI-cvwo | UI-47y7 |
| 형제 | dotfiles | user_request | 독립 착지 가능한 검증 묶음 — adr 스킬 식별자 규칙 | 없음 | dotfiles-60u8 |

- 관찰: `UI-01wh`와 `scheduler.js`·`AGENTS.md`·번들 scope 겹침 — 결정 충돌 없음.
...
```bash
npx vitest run --reporter=dot server/worker/runner/preamble.test.js server/worker/runner/claude.test.js server/worker/runner/codex.test.js server/worker/runner/session.env.test.js server/worker/failure-class.test.js server/worker/scheduler.test.js server/worker/queue-store.test.js server/worker/worktree.integration.test.js
npm run tsc && npm run lint && npx prettier --write <changed files> && npm run build
npx vitest run --reporter=dot   # 전체, timeout 120초
```

회귀 사례: D1 스냅샷(writable·disposition·quickfix_lane·review 네 변형 × claude·codex runner)과 fail-quiet 줄 생략; D2 `open`→선점·`worker_claim` 전이, `resolved`에서는 미기록, prerecord 실패 시 미기록, 선점 직후 사망 attempt의 reconcile 해제; D3 lock 파일 없음→skipped, 설치 실패→dispatch 계속·`failed:` 기록, queue-store 보존; D4 argv/env 고정; D5 용량 문자열→`env`/`provider_capacity`, 테스트 실패 문장→`individual`; D5b 커밋 남긴 attempt의 resume 발화; D6 재개 문장 채움. 실제 실행 확인: 배포 뒤 Worker attempt 1건 transcript에서 첫 턴에 `## 시도 사실`이 있고 impl-gate claim 차단이 0건, `npm ls` 호출이 0건임을 확인한다.
