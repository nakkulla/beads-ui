...
- 범위: **전체 파이프라인** — 대기(`queue`) / 실행중(running attempts) /
  PR 대기(`pr_wait`, external PR overlay 포함) / 완료(`done`, 당일).
...
2. 각 workspace에 대해 `getWorkerRuntime().queueStore` 스냅샷을 읽는다(큐
   스토어는 이미 서버 전역 싱글턴, workspace 키별 `queue.json`).
3. 기존 데코레이션 헬퍼를 workspace별로 재사용한다: 버드 제목/시간 데코,
   `withExternalPrWait`(external PR overlay). worker-handlers의 해당 로직을
   공용으로 추출하거나 export 재사용 — 복제하지 않는다.
4. 4개 레인이 모두 비고 running attempt도 없는 workspace는 결과에서 생략한다.
...
`queue`/`pr_wait`/`done`/`attempts`는 기존 `worker-queue-snapshot`이 보내는
정규화·데코레이션된 형태를 그대로 사용한다(신규 정규화 없음). `name`은
workspace 경로 basename.
...
- 레인 판정(모두 payload에서 파생, 클라이언트 병합):
  - 실행중: `attempts` 중 status `running`인 attempt가 있는 버드.
  - PR 대기: `pr_wait` 레인(+external overlay 행).
  - 대기: `queue` 레인.
  - 완료·오늘: `done` 레인.
- 행 구성: 단계색 스파인(3px) → **workspace 이름 뱃지** → 버드 ID(mono) → 제목
  ...
  - PR 대기: PR 번호, external PR이면 `외부 PR` 칩.
  - 대기: 큐 순번(`#n`, 디스패치 순서).
  - 완료: 완료 종류(`✓ merged` 등; 비정상 종류는 warn 색)·완료 시각.
...
1. **서버 집계 빌더** (`server/ws/monitor-handlers` 신규 테스트):
   ...
   - 빈 파이프라인 workspace 생략을 검증.
   - 한 workspace 빌드 실패 시 나머지 workspace가 유지됨을 검증.
   - payload가 기존 worker-queue 정규화 형태(+데코)를 유지함을 검증.
2. **push 트리거** (동일 모듈):
