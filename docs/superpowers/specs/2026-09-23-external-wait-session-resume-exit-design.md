...
- **소유 세션의 생사를 판정할 재료가 레코드에 있다.** 세션 소유 레코드의 `owner`는
  `session_ref`·`session_pid`·`session_start`를 싣는다(계약 Record). 로컬 프로세스
  어댑터가 같은 identity 규칙(`LC_ALL=C ps -p <pid> -o lstart=` + 시작 시각 ±300초)을
  이미 쓴다(`external-wait/adapters/process.js:35`). owzn의 `session_pid` 45312는 지금
  없다.
...
레코드 `owner.session_ref`는 생사 판정(pid·시작 시각)에만 쓴다.
...
| `completing` | session | `external_wait_resume` `mode: 'session'` | `[세션에서 이어가기]` | card | — | `보존 세션을 같은 워크트리의 tmux 창에서 재개한다 · 열리면 대기 키가 풀린다 · 원래 터미널이 살아 있으면 열지 않고 재개 명령을 복사한다` |
...
`completing`이고 `completion`이 있으며 `owner.kind === 'session'`, 예약(`resume.attempt_id`)
이 없고(`resume.error`는 허용), 같은 `[workspace, wait_id]`의 재개가 진행 중이 아니다
(`external_wait_resumes` 집합). Bead가 `open`이 아니거나 `awaiting_user`가 있으면
...
reason: string|null, command: string|null, tmux_session, tmux_window, pane_id,
bridge_active, queue }`. `not_launched`의 `reason`은 §4.3의 실패 사유이며 `owner_alive`
만 `command`(재개 명령)를 싣고 레코드를 바꾸지 않는다; 나머지 `not_launched`는
`resume.error=<reason>`을 기록하고 키를 남긴다(fork 실패와 같은 처분, 카드는
`⛔ 조치 필요 · 재개 실패 · <reason>`과 세 버튼).
...
1. **세션 자격.** `qualifySessionFork(bead_metadata, null)`로 Bead `session_ref`의 마지막
   항목을 판정한다: `no_session_ref`·`unsafe_session_id`·`not_local`은 `not_launched`.
   provider는 그 항목의 것이다(`runner_name = null`, provider 제약 없음).
2. **소유 세션 생사.** `record.owner.session_pid`·`session_start`가 있으면
   `LC_ALL=C ps -p <pid> -o lstart=`를 돌려 exit 0이고 lstart가 `session_start`
   ±300초 안이면 살아 있다(로컬 프로세스 어댑터의 identity 규칙 재사용 —
   `adapters/process.js`의 프로브 호출을 함수로 빼 공유한다). 살아 있으면
   `not_launched`·`reason: 'owner_alive'`·`command: sessionResumeCommand(entry)`로 끝난다
   — 한 세션 ID에 프로세스 둘을 열지 않는다. pid·시작 시각이 없거나 프로브가 실패하면
   죽은 것으로 본다(옛 레코드·프로브 오류로 기동을 막지 않는다).
...
4. **프롬프트.** `## 외부 작업 완료` 블록은 `scheduler.js externalWaitCompletionPrompt`를
   ...
   `사람이 beads-ui [세션에서 이어가기]로 이 세션을 재개했다 · 대기 키는 해제됐다 · 워크플로 절차대로 in_progress 클레임 뒤 이어간다`.
5. **기동.** `launcher.launch({ marker: EXTERNAL_RESUME_PANE_MARKER, key: bead_id,
   tmux_session, window_name: bead_id, cwd: record.worktree, commandArgs, runner:
   provider })`. `tmux-launcher.js`에 세 번째 마커 `EXTERNAL_RESUME_PANE_MARKER =
   '@bdui_external_resume_bead'`를 더한다(모듈 머리말의 "마커가 종류를 이름한다" 규칙).
   `commandArgs`는 claude `['--resume', session_id, prompt]`(fork·`--session-id` 없음),
   codex `['resume', session_id, prompt]` — codex의 프롬프트 위치 인자는 구현이
   `codex resume --help`로 실측하고, 받지 않으면 codex에서는 프롬프트를 빼고 세션이
   `bead-wait show`로 완료 블록을 읽게 한다(실측 결과를 테스트 주석에 남긴다).
   `already_running`(같은 마커·key의 살아 있는 pane)은 기동 없이 6으로 간다.
6. **정산.** `launched`·`already_running`이면 큐 store에 대화형 세션을 기록하고
   (`InteractiveSession.kind`에 `'external_resume'`, `mode`에 `'resume'`을 더한다;
   `session_id`는 재개한 ID, `source: 'session_ref'`, `forked_from: null`, `attempt_id:
   null`), `record.resume = { mode: 'session', attempt_id: null, reserved_at,
   launched_at, session_id, error: null }`을 쓴 뒤 키를 unset하고 readback한다
   (`unsetExternalWait` 그대로: 불일치면 throw). unset이 성공하면 `stage='resumed'`로
   닫고 `notifyChanged`한다. unset이 실패하면 레코드는 `launched_at`이 있는 `completing`
   으로 남고 §4.2의 reconcile 정산이 다음 pass에서 같은 unset·`resumed`를 다시 한다 —
   세션은 이미 열려 있으므로 두 번 열지 않는다.

**재시작·reconcile 정산(`settleExternalWaitReservations`).** `resume.mode === 'session'`
인 레코드는 `launched_at`이 있으면 `settleExternalWaitLaunch`와 같은 처분(키 unset →
`resumed`)이고, `launched_at`이 없으면(예약 뒤 기동 전 크래시) `resume = null`로
예약만 지우고 **다시 띄우지 않는다** — 사람의 클릭만이 tmux 기동의 방아쇠다. 기존
분기가 `resume.attempt_id`를 키로 삼으므로 session 모드는 `resume.mode`로 먼저
갈라 attempt 조회를 건너뛴다.

**대화형 세션 표면.** `lane-model.js InteractiveSessionView.kind`와 서버 투영에
`'external_resume'`을 더하고, 세션 타일·후보 카드의 대화형 세션 배지 라벨은
`재개 세션`이다(기존 `해결 세션`·`문의 세션`과 같은 자리·같은 `▤` 열기 동작).
`reconcileInteractiveSessions`의 생존·정산 규칙(pane 소멸 → settled)은 kind와 무관하게
그대로 적용된다.
...
- `mode === 'session'` 응답: `session === 'launched'`면 토스트
  ...
  `원래 터미널 세션이 살아 있어 열지 않았습니다 · 재개 명령을 복사했습니다`, 그 밖의
  `not_launched`는 `세션을 열지 못했습니다 · <reason>`(error 토스트). 응답의 `queue`는
  지금처럼 adopt한다.
- 다른 op의 토스트는 지금 그대로다.
...
- `docs/contracts/external-wait.md` — Routes 표의 `/resume` 행 `mode: 'fork'|'fresh'|'session'`;
  Record의 `resume.mode: 'fork'|'fresh'|'session'`(session은 `attempt_id: null`);
  Resume 절에 **Session resume** 문단: 세션 소유 `completing` 레코드에만, 예약 없을 때,
  Bead `session_ref` 마지막 항목을 `qualifySessionFork`(provider 제약 없음)로 판정해
  같은 워크트리의 tmux 창에서 fork 없이 재개하고 `## 외부 작업 완료` 블록을 첫
  프롬프트로 싣는다; 등록 세션 프로세스(`owner.session_pid`·`session_start`)가 살아
  있으면 열지 않고 `owner_alive`와 재개 명령을 돌려준다; attempt를 만들지 않고 클레임하지
  않으며 admission을 돌리지 않는다; 기동 readback 뒤 키를 unset하고 `resumed`로 닫는다;
  실패는 `resume.error`를 남기고 키를 유지한다; 재시작 정산은 `launched_at` 없는 session
  예약을 지우되 다시 띄우지 않는다. "a session-owned wait is notified over Discord and
  resumes on the person's `[이어하기]`" 문장은 `[세션에서 이어가기]`(session) 또는
  `[워커로 이어가기]`(fork)로 고친다.
- `docs/contracts/workflow-state.yaml external_wait.completion.resume` —
  ...
  `manual_api.body_mode: [fork, fresh, session]`,
...
- `src/shared/skills/flow/workflow/references/external-wait.md` "After completion" —
  "Worker resumes by fork"에 사람의 `[세션에서 이어가기]`(같은 워크트리, fork 없는 재개,
  같은 블록)를 나란히 적고, 그렇게 재개된 세션은 키가 이미 풀려 있으므로 첫 편집 전에
  세션 소유 규칙대로 `in_progress`를 클레임한다는 한 줄을 더한다.
- 검증: dotfiles `scripts/required-tests.json`이 지정하는 테스트 중 `tests/test_contracts_layout.py`·
  `tests/contracts/test_workflow_contract.py`·`tests/bead_wait_test.py`(exit 0). `bead-wait`
  CLI는 바꾸지 않는다 — `/resume`는 사람의 클릭 경로다.
...
- 결정: `[워커로 이어가기]`·`[새 세션으로]`의 `/resume` fork·fresh 동작(예약·launch·
  클레임·정산)은 바꾸지 않는다.
- 결정: Discord 완료 알림 본문은 바꾸지 않는다 — 재개 명령 줄이 이미 있고 카드의 버튼과
  같은 세션을 가리킨다.
...
- `server/worker/external-wait/service.test.js`: `resumeWait`가 `mode: 'session'`을
  받고 Worker 소유·예약 있음·`completing` 아님을 거절한다.
- `server/worker/scheduler.external-wait.test.js`: session 모드 수락 조건(§4.2),
  admission 미호출, `bead_running`·`discard_in_progress`, 정산(`launched_at` 있음 →
  `resumed`; 없음 → 예약 삭제·재기동 없음).
- `server/worker/external-wait/session-resume.test.js`(신규): 자격 실패 사유 넷,
  `owner_alive`(ps 프로브 stub)가 기동·키 변경 없이 `command`를 돌려줌, `worktree_missing`,
  launcher 호출 인자(마커·key·cwd·argv: claude resume 형태, 프롬프트 첫 줄), `launched`
  뒤 대화형 세션 기록·`resume` 필드·키 unset·`resumed`, `already_running` 정산,
  unset 실패 시 `completing` 유지.
...
**실측.** Mac에서 실제 `[세션에서 이어가기]` 클릭으로 tmux 창이 열리고 첫 프롬프트에
완료 블록이 보이며 `bd show --json`에서 `external_wait` 키가 사라졌음을 확인한다.
owzn 레코드는 이미 `stopped`라 재현에는 새 대기 등록(process 어댑터, 로컬 `sleep`)을
쓴다.
...
- 크로스 리포 unit: dotfiles — §4.6의 계약·상태 표·스킬 참조 전사(`docs/contracts/external-wait.md`,
  `docs/contracts/workflow-state.yaml`, `src/shared/skills/flow/workflow/references/external-wait.md`),
  검증은 §4.6의 dotfiles 테스트 · quick_fix Bead는 인계 시 dotfiles rig에 만들고 이 Bead가
  그것에 foreign `blocks`로 잇는다(계약 먼저, 소비자 다음 — ADR UI-u6ud-2).
...
- 세션 소유 외부 대기의 완료 출구는 `[세션에서 이어가기]`(보존 세션의 fork 없는 tmux
  재개, attempt·클레임·admission 없음, 기동 readback 뒤 키 해제, 소유 세션 생존 시
  명령 복사)와 `[워커로 이어가기]` 둘이고, 키를 지우는 stop은 확인 1회를 거치며 완료
  ...
  `summary`: "세션 소유 외부 대기의 완료 출구는 [세션에서 이어가기](보존 세션의 fork 없는 tmux 재개, attempt·클레임 없음, 기동 readback 뒤 키 해제, 소유 세션 생존 시 명령 복사)와 [워커로 이어가기] 둘이고, 키를 지우는 stop은 확인 1회를 거치며 완료 뒤의 [대기 해제]는 상세 패널에만 선다" → ADR, supersede UI-l48z
