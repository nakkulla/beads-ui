...
- **UI-7agi(외부 행)**: 아직 `UI-7agi` 브랜치(main 미머지). 외부 행 머지도 같은
  `runCleanup` 초레오그래피를 타므로, UI-7agi가 머지되면 외부 행 머지에도 이
  알림이 추가 작업 없이 적용된다. 양쪽 모두 `pr-actions.js`를 수정하므로 머지
  순서에 따라 작은 충돌 해소가 필요할 수 있다.
