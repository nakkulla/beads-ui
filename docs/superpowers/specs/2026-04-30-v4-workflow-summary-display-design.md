...
Board view의 `IssueLite` typedef는 현재 `metadata`를 모델링하지 않는다. push store가
원본 issue object를 보존하더라도, 이번 구현에서는 board card가 사용하는 DTO surface를
명시적으로 확장해 `metadata.execution_lane`, `metadata.skill_workflow`,
`metadata.pr_url`을 읽는다는 점을 코드와 테스트에 고정한다.
...
- `skill_workflow=none`은 exact canonical value일 때만 valid value로 취급한다.
  empty string, `null`, `undefined`, unknown value, and case-mismatched values are
  absent로 normalize되며 detail row도 표시하지 않는다.
...
- `normalizeExecutionLane(value)`는 exact `plan` 또는 `quick_edit` string만 반환한다.
...
- 이 규칙은 contract drift를 조용히 보정하지 않기 위한 것이다. 예를 들어
  `Skill_Creator`나 `skill-related`는 숨겨지고, upstream metadata producer가 canonical
  값을 쓰도록 고쳐야 한다.
