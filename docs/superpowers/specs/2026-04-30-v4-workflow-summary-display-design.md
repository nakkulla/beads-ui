...
Parent bead: UI-uf7m Supersedes: UI-uyoa, UI-vhdx

## 문제 정의

`bd-ralph-v4` / `pr-review-v4`는 workflow evidence를 v4 contract 기준으로
정리하려는 canary workflow다. `beads-ui`는 현재 issue detail에서
spec/plan/handoff metadata path를 보여주고, board/list에서는 일부 label만 요약
표시한다. 그러나 v4 contract가 도입한 실행 분류와 결과 요약은 UI에서 바로
확인하기 어렵다.

기존 `UI-vhdx` spec은 detail 화면에 `bd-ralph-v3` 실행요약(duration/PR)을
표시하는 것에 초점을 맞췄다. 이번 재설계는 `UI-uyoa`의 목표인 **v4 workflow
contract metadata 표시 정합성**을 중심으로 기존 실행요약 요구를 흡수한다.
...
반대로 run ledger artifact 자체는 audit/debug 용도이며, 이번 `beads-ui` UI
surface에 직접 노출할 필요가 없다.

## 목표

- Board card에서 v4 contract metadata 기반 workflow chip을 보여준다.
- Issue detail에서 `Workflow summary` card를 보여준다.
- Detail summary에는 duration, started, finished, PR, execution lane, skill
  workflow를 표시한다.
- UI는 v4 contract 기준 parent metadata를 primary source로 사용한다.
- Run ledger / evidence artifact link는 이번 UI scope에서 표시하지 않는다.
- 기존 label display policy, spec/plan/handoff metadata path 표시,
  navigation/editing behavior를 유지한다.
...
- legacy metadata(`skill_related`, `skill_creator_required`,
  `skill_eval_fast_path`)를 UI primary source로 사용하지 않는다.
...
`app/views/board.js`는 card title, label badges, type, priority, issue id,
created relative time을 표시한다. label badge는 `app/utils/label-badge.js`와
config 기반 visible prefix 정책을 사용한다.

Board view의 `IssueLite` typedef는 현재 `metadata`를 모델링하지 않는다. push
store가 원본 issue object를 보존하더라도, 이번 구현에서는 board card가 사용하는
DTO surface를 명시적으로 확장해 `metadata.execution_lane`,
`metadata.skill_workflow`, `metadata.pr_url`을 읽는다는 점을 코드와 테스트에
고정한다.

### List/Epics row 표시

`app/views/issue-row.js`는 reusable table row renderer로 title/status/assignee/
priority/labels/dependency count 등을 표시한다. table row는 이미 밀도가 높고, 새
workflow column을 추가하면 layout 영향이 board card보다 크다.

List/Epics row는 이번 scope에서 workflow chip을 표시하지 않는다. 따라서
`app/views/list.js` 또는 `app/views/issue-row.js`의 row DTO를 workflow metadata
표시 목적으로 확장하지 않는다.

### v4 contract 관련 관찰

`dotfiles/docs/contracts/workflow-contract.yaml`과
`dotfiles/shared/skills/bd-ralph-v4/resources/workflow-contract.yaml`은 v4
workflow contract의 metadata로 `pr_url`, `execution_lane`, `skill_workflow`,
`run_ledger` 등을 정의한다.

`bd-ralph-v4/scripts/finish_branch.sh`는 현재 parent bead에 `pr_url`,
`pr_number`, `run_started_at`, `run_finished_at` 등을 sync한다. run ledger
file은 생성하지만 parent metadata pointer는 현재 `audit_artifact` 중심으로
sync되는 흐름이 관찰된다. 이번 UI 설계는 ledger pointer를 표시하지 않으므로, 이
drift를 UI fallback으로 흡수하지 않는다. 대신 dotfiles 쪽 contract/emitter
정합성 follow-up으로 명시한다.
...
5. UI가 ledger file reader가 되지 않으면 backend/protocol 변경 없이 구현할 수
   있다.
...
이번 `beads-ui` spec은 contract consumer 설계다. dotfiles 쪽에서는 별도
follow-up으로 다음 정합성을 맞춰야 한다.

- `run_started_at` / `run_finished_at`을 v4 contract metadata registry에
  canonical UI summary source로 포함할지 결정한다.
- `bd-ralph-v4`가 parent bead에 v4 contract metadata를 실제로 sync하도록 맞춘다.
- `run_ledger`와 `audit_artifact`의 관계를 정리한다. `beads-ui`는 ledger를
  표시하지 않지만, contract-first workflow promotion 전에는 emitter/contract
  naming이 맞아야 한다.
...
- empty string, `null`, `undefined`, unknown value, and case-mismatched values
  are normalized to absent, not to `none`.
...
Issue detail sidebar에 `Workflow summary` card를 추가한다. 기존 metadata path
card와 가까운 위치에 두되, 기존 card의 동작과 layout은 유지한다.
...
- `skill_workflow=none`은 detail에서는 표시해도 된다. 이는 “skill workflow
  없음”을 명확히 보여주는 값이기 때문이다.
- `skill_workflow=none`은 exact canonical value일 때만 valid value로 취급한다.
  empty string, `null`, `undefined`, unknown value, and case-mismatched values
  are absent로 normalize되며 detail row도 표시하지 않는다.
...
기존 utility가 같은 목적에 맞으면 재사용한다. 맞지 않으면 workflow summary
helper에 작은 formatter를 둔다.
...
- `normalizeExecutionLane(value)`는 exact `plan` 또는 `quick_edit` string만
  반환한다.
- `normalizeSkillWorkflow(value)`는 exact `none`, `writing_skills`, 또는
  `skill_creator` string만 반환한다.
- empty string, whitespace-only string, `null`, `undefined`, non-string value,
  unknown string, and case-mismatched value는 모두 `null`로 normalize한다.
- `null`은 UI row/chip을 렌더링하지 않는 absent value를 의미한다.
- 이 규칙은 contract drift를 조용히 보정하지 않기 위한 것이다. 예를 들어
  `Skill_Creator`나 `skill-related`는 숨겨지고, upstream metadata producer가
  canonical 값을 쓰도록 고쳐야 한다.
...
- `IssueDetail` typedef의 `metadata` shape를 workflow summary key까지 확장한다.
...
- `IssueLite` typedef를 workflow metadata key까지 확장한다.
- board subscription/store path가 metadata를 보존하지 않는 것으로 확인되면,
  board selector/store boundary에서 해당 metadata passthrough를 보존하도록 같은
  task 안에서 보완한다. 새 backend RPC나 ledger file read는 추가하지 않는다.
...
- workflow chip은 decorative status text이므로 card navigation/click behavior를
  방해하지 않는다.
...
- empty/null/unknown/case-mismatched enum inputs normalize to `null`.
...
- board card에 `plan`, `quick_edit`, `writing_skills`, `skill_creator`, `PR`
  chip이 조건부 표시된다.
- invalid/unknown metadata는 chip으로 표시되지 않는다.
- 기존 label badge display는 유지된다.
- board issue fixture의 `metadata`가 card renderer까지 전달되는 것을 검증한다.
...
- Board card에서 v4 contract metadata 기반 lane/skill workflow/PR chip을 볼 수
  있다.
- Detail 화면에서 `Workflow summary` card가 duration, started, finished, PR,
  lane, skill workflow를 보여준다.
