...
- DOM 순서는 생애주기(실행가능→대기→실행중→PR 대기→완료) 그대로. 데스크톱은
  Worker와 같은 `.worker-lanes` flex 5열(실행중 `flex:1.35`, 실행가능
  `flex:1.05`, 나머지 1). 모바일(≤640px)은 Worker 모바일과 같은 세로 스택이되
...
활성 타일 아래 **기본 접힌** 토글 한 줄 `▸ 파이프라인 없음 N`. 펼치면 pill로
나열: 레포명 · 슬롯 레일(빈 칸만) · `▶/⏸` · `🔀` · `⚙` · `↗` — 같은 op·같은
