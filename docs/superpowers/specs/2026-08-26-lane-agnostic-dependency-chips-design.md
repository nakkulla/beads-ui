...
4. **Worker에서 blocker id를 문장으로 적던 자리는 칩으로 통일한다.** 문장과 칩이
   같은 blocker id를 한 카드에 두 번 적지 않게 한다. 대상은 대기 행의
   `⏸ … 완료 대기 (blocks)` 뱃지와 후보 행의 잠금 문장 `🔒 <ids>` 둘 다다.
   후보 행은 UI-tf6d가 뒤늦게 정정했다 — 이 스펙의 첫 구현이 §5.1에서 그 문장을
   blocker id의 원천으로 참조만 하고 제거 대상에서 빠뜨려, 후보 카드에 칩과
   문장이 나란히 서는 중복을 남겼다. 단, 잠금 문장의 id 없는 형태
   `🔒 blocked`는 남는다: 칩은 id가 없으면 서지 않으므로(§5.1 fail-quiet) 그
   경우 두 번 적히는 id 자체가 없고, 그 문장만이 "막혀 있으나 선행을 모른다"를
   말한다.
...
- **입력은 `bead_blocked_by` 하나가 아니다.** Worker 탭에서 blocker id가 오는
  ...
  - 후보 행: 후보는 큐에 없어 그 장식에 들어오지 않는다. 원천은 구독 이슈의
    ...
    `blocks` 간선(`depends_on_id`)이다 — 잠금 문장이 쓰던 바로 그
    사다리이고, UI-tf6d 이후로는 `blockerIdsOf` 하나가 그 판정을 소유한다
    (§2 결정 4).
...
**정정(UI-8x90).** 두 탭의 클릭이 하나로 통일됐다. Monitor의 blocked 칩도 이제
의존성 패널이 아니라 **그 이슈의 상세**를 열고, 같은 줄에 서는 `→`·`🔓`·`⧉`
네 칩이 전부 같은 `.worker-dep__open` 버튼으로 같은 결과를 낸다 — 한 줄 안에서
칩마다 클릭 결과가 다른 것이 이 절이 남긴 마지막 갈림이었다. 이 절의 원칙
"누를 수 없는 대상에 버튼을 만들지 않는다"와 칩 단위 `openable` 판정은 그대로
성립하며, 겹침 칩도 같은 규칙을 쓴다
(`2026-08-28-chip-grammar-unify-design.md` §4.3).
...
**정정(UI-8x90).** 이 목록의 앞 세 항목은 더 이상 성립하지 않는다. 칩 라벨은
`⛓ blocked: <id>`가 아니라 `⛓ <ID>`이고(방향어는 툴팁 첫 줄), 이미 있던 칩의
클릭 동작도 전부 이슈 상세로 통일됐으며, 슬롯 4는 의존(4a)·정보(4b) 두 줄로
나뉘었다.

**후속 칩 부활의 경위.** 이 스펙이 비목표로 둔 "후속(successor) 칩 부활"은
UI-d13v가 후보 행 한정 `→ 후속 n` 개수 칩으로 되살렸고, UI-8x90이 그것을
`⛓`와 같은 범위 — 후보·대기·실행중·PR 대기, 두 탭 — 의 `→ <ID>` 칩으로 넓혔다.
"카드는 '내가 막혔나' 하나만 말한다"는 이 절의 전제는 그 두 스펙이 뒤집었다
(`2026-08-27-worker-candidate-sort-chain-release-chips-design.md` §5.3 ·
`2026-08-28-chip-grammar-unify-design.md` §2·§4.4).
