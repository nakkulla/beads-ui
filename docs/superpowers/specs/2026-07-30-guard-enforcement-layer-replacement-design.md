...
| **검출** — 사후 base 불변식 (§3) | 원격 base tip 과 attempt 브랜치의 도달 관계 | 정확하되 사후 | — |
...
**배선**: `scheduler.js` 의 `launchSession` 에서 `settings.env` 에 세 키를 싣는다.
...
**실행 지점**: `scheduler.js` `onSessionDone` 의 disposition 분기(`:1065`) 직후, PR 관측(`:1140`)보다 앞. `verdict.success` 여부와 **무관하게** 실행한다 — SIGTERM 당한 세션도 죽기 전에 push 했을 수 있고, 그 경우 실패 원인이 `base_landing_detected` 로 대체되는 편이 정직하다.

**절차**:

1. `resolveTargetBase({ force: true })` 로 base 를 재해석해 현재 원격 tip 을 얻는다(fetch 포함). `pr-actions.js` 가 머지 직전에 쓰는 것과 같은 강제 재해석이다.
...
3. 이동이 있으면 두 집합을 비교한다.
   ...
   - **A ∩ B ≠ ∅ → 위반.** 교집합에 든 SHA 를 증거로 기록한다.
4. 교집합이 비었으면 위반이 아니라 **관측 기록**이다. attempt 에 `base_drift = { pinned, observed, landed: false }` 를 남긴다. 남이 움직인 base(사람의 머지 클릭, 다른 워커의 머지, 사용자 push)는 여기까지다.

A 가 비어 있으면(세션이 브랜치에 커밋하지 않음) 3단계는 자동으로 위반이 아니다.
...
- **`git push` base-landing 판정은 경고로 강등** — `runner/session.js` 가 `merge_to_base_blocked` 를 받으면 kill 없이 경고 이벤트만 남기고 세션을 계속 진행시킨다. 판정 로직(`command-guard.js` 의 토크나이저·allowlist·회귀 테스트)은 **그대로 둔다.** 근거: 정확한 판정은 이제 hook 이 하고, 이 층의 오판 비용은 세션 전체 SIGTERM 이다.
...
- **새 kill 대상: hook 무력화 시도** (`reason='hook_bypass_blocked'`) — 네 형태를 argv 위치로 판정한다.
...
**hook 설치 실패는 dispatch 거부** — `refuseDispatch(…, 'guard_hook_install_failed')`. 검출 층이 있으니 그냥 진행할 수도 있지만, 안전 장치 없이 무인 세션을 조용히 돌리는 것이 이 Bead 가 없애려는 종류의 침묵이다. 기존 refusal 배지 관용구를 그대로 쓴다.
...
2. 검출 층은 **squash 머지로 base 에 들어간 경우를 못 잡는다**(SHA 가 바뀜). 이 구멍의 유일한 실질적 방어는 `gh pr merge` kill 유지다.
...
**검출 층 — 단위 테스트** (fake git runner): 이동 없음 / 남이 이동(관측 기록만, 위반 아님) / attempt 커밋 랜딩(위반) / 관측 실패(기록 후 진행) / A 가 빈 경우 / disposition·external 제외 기록.

**텍스트 가드 — 기존 회귀 유지 + 신규**: base-landing 이 kill 이 아닌 경고가 되는지, `gh pr merge` kill 이 유지되는지, hook 무력화 4형태가 kill 되는지, disposition 세션에서 세 층이 모두 미적용인지. `UI-lhwp` 가 고정한 실측 사고 2건과 allowlist 반례 3종은 **경고 판정으로서** 그대로 유지한다(판정 로직을 지우지 않으므로 회귀 가치가 남는다).
...
| 6 | 검출 층이 위반만 잡는다 | §6 검출 층 단위 테스트 전건 |
| 7 | 검출 층이 관측 실패로 세션을 죽이지 않는다 | fetch 실패 케이스에서 attempt 가 정상 경로를 마치고 `base_drift` 에 실패 step 이 남는다 |
| 8 | base-landing 이 kill 하지 않는다 | 실측 사고 2건이 경고로만 기록되고 세션이 계속된다 |
| 9 | `gh pr merge` kill 이 유지된다 | 기존 테스트 green + `--repo` 지정 형태도 kill |
| 10 | hook 무력화 4형태가 kill 된다 | §6 텍스트 가드 신규 테스트 |
| 11 | disposition 세션이 본업을 수행할 수 있다 | disposition 컨텍스트에서 `git push origin <base>` 가 violation 도 경고도 아니고, hook 도 설치되지 않는다 |
| 12 | 프리앰블 고지가 강등 후 사실과 일치한다 | 생성된 프리앰블 문자열 검증 테스트 |
| 13 | hook 설치 실패가 보이게 거부된다 | `guard_hook_install_failed` refusal 배지 테스트 |
| 14 | 정본 검증 묶음 green | `npm run all` (= `lint && tsc && test && prettier:check`) |
| 15 | 배포 후 런타임이 새 코드로 돈다 | 머지 후 `bdui-shared restart` → 프로세스 경로가 머지된 체크아웃, 포트, HTTP 응답 확인 |
