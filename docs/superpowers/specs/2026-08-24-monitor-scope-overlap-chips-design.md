...
**정정(UI-8x90).** 이 결정 중 "팝오버에서 1클릭으로 같은 직렬 레인에 배치"는
2026-08-28 사용자 결정으로 대체됐다. `⧉` 글리프와 "정보 표시가 기본"은 그대로
성립하고, 자동 배치를 하지 않는다는 판정은 더 강해졌다 — 겹침을 직렬 레인에
넣는 조작은 이제 드래그와 `[대기로 ↴]` 배치 메뉴만 소유한다
(`2026-08-28-chip-grammar-unify-design.md` §2·§9).
...
**정정(UI-8x90).** 겹침 팝오버는 없다. 칩 라벨은 `⧉ <ID>`(개수 상한·`+n` 칩
없음)이고 클릭은 같은 줄의 다른 칩과 똑같이 그 이슈의 상세를 연다. 팝오버가
보여 주던 상대 위치와 겹치는 경로 목록은 칩의 `title` 툴팁으로 남는다. 칩은
정보 줄(슬롯 4b)에 서고 `scope 없음`도 같은 줄이다
(`2026-08-28-chip-grammar-unify-design.md` §3·§4.1·§4.2).

### 5.4 1클릭 직렬 배치 (`app/views/monitor/index.js`)

**정정(UI-8x90).** 이 절 전체가 제거됐다. `workerPlacementPlan`·
`placeIntoSameSerialLane`과 그 판정 표는 코드에서 사라졌고, 겹침 칩에서 시작하는
배치 조작은 없다 — 대체 조작을 새로 만들지 않고 기존 드래그·`[대기로 ↴]` 배치
메뉴가 그 일을 소유한다. `worker-queue-place` op 자체와 그 배치 경로는 그대로다
(`2026-08-28-chip-grammar-unify-design.md` §4.3·§9).
