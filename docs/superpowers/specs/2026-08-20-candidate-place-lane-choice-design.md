...
- **탭 수 보존**: 고를 수 있는 직렬 레인이 하나도 없으면(§4의 유효성
  판정 결과가 빈 목록) 선택 UI를 띄우지 않고 지금과 똑같이 한 번의 탭으로
  병렬 말미에 적재한다. 선택지가 하나뿐인 선택은 선택이 아니다.
...
- **적재 의미 = 말미**: 버튼 경로의 의미는 드롭 위치가 아니라 "그 레인의
  맨 뒤"다. 그래서 이 경로는 `index`를 아예 보내지 않고 서버의 append 의미
  (`clampIndex(index ?? arr.length, arr.length)`)에 맡긴다. 프런트에서 tail을
  한 번 계산해 넣으면 CAS 충돌 재시도에서 낡은 인덱스를 그대로 다시 보내
  "말미"가 깨지기 때문이다(`placeBead`의 `payload()`는 revision만 다시 읽고
  index는 클로저에 고정된다). 드래그 경로는 드롭 위치가 곧 의미이므로 지금처럼
  index를 계속 보낸다 — 이 불변식은 버튼 경로에만 적용된다.
...
- 뷰 로컬 상태 `place_menu_bead_id: string|null` 하나를 추가한다. durable
  ...
  사는 상태다.
- 렌더 계약 `place_menu`는 매 렌더마다 서버 큐 스냅샷(`currentQueue()`)에서
  새로 만든다. 저장되는 것은 위의 문자열 하나뿐이고 개수는 절대 캐시하지
  않는다.

  ```js
  /** @type {{ bead_id: string, lanes: Array<{ id: 'parallel'|'s1'|'s2'|'s3'|'s4'|'s5', label: string, count: number }> } | null} */
  ```

  - `lanes[0]`은 언제나 `{ id: 'parallel', label: '병렬', count: <병렬 대기 수> }`.
  - 이어서 `serial_lanes`를 순서대로 돌며, `id`가 `s1`..`s5`이고 배열
    `entries`를 가진 레인 중 앞에서 `serial_lane_count`개까지만
    `{ id, label: '직렬 N', count: entries.length }`로 만든다. `count`는
    대기 entries만 세므로 ghost 점유 행은 자연히 빠진다.
  - 유효한 직렬 레인이 하나도 없으면(구버전 스냅샷 포함) `place_menu`를
    만들지 않고 즉시 병렬 적재한다 — §2의 "레인 0" 규칙과 같은 분기다.
  - `place_menu_bead_id`가 `null`이거나 그 bead가 이번 렌더의 후보 목록에
    없으면 `place_menu`는 `null`이다.
- `lanes.js`
...
- `index.js` 클릭 핸들러(기존 `.worker-card__place` 분기 자리)
  - `.worker-card__place`: `disabled`면 아무것도 하지 않는다. 유효한 직렬
    레인이 없으면 `placeBead(id, 'parallel')`, 아니면 `place_menu_bead_id =
    id` 후 재렌더.
  - `.worker-card__place-lane`: `data-lane`이 `parallel` 또는 `s1`..`s5`일
    때만 `placeBead(id, lane)`을 부르고, 응답을 기다리지 않고 메뉴를
    닫는다(적재된 카드는 후보에서 사라진다).
...
- `placeBead(bead_id, lane, index)`의 `index`를 선택 인자로 바꾼다.
  `undefined`면 payload에서 `index` 키를 뺀다 — CAS 재시도(`payload()` 재호출)
  에서도 서버가 그때의 말미를 다시 계산한다. `queueTailIndex()`는 드래그
  경로(접힌 스트립 드롭, `UI-58y2`)가 계속 쓰므로 그대로 남는다.
...
1. 직렬 레인이 있는 큐에서 `[대기로 ↴]` 탭은 transport를 부르지 않고 칩
   행을 연다. 칩 라벨은 각 레인의 대기 수를 보여주며 ghost 점유 행은 세지
   않는다.
2. `직렬 2` 칩 탭은 `worker-queue-place`를 `{ bead_id, lane: 's2',
   expected_revision }`로 부른다 — `index` 키가 없다.
3. `병렬` 칩 탭은 `{ bead_id, expected_revision }`(`lane`·`index` 키 없음)을
   보낸다.
4. 직렬 레인 수가 0이면 탭 한 번에 곧바로 병렬 적재한다. 기존 테스트
   `places the candidate at the tail of the waiting queue`는 `index: 1`
   단언을 잃고 `{ bead_id, expected_revision }`로 갱신된다 — 의미(병렬 말미)는
   같고, 말미 계산 주체만 클라이언트에서 서버로 옮겨간다.
5. CAS 충돌 응답(`{ conflict: true }`) 뒤 재시도 payload에도 `index` 키가
   없다.
6. 취소 칩은 transport를 부르지 않고 닫힘 렌더로 되돌린다.
7. 다른 카드의 버튼을 누르면 열린 메뉴는 하나로 유지된다.
8. 칩 클릭은 상세 패널을 열지 않는다.
9. 자격 없는(spec 없는) 후보의 버튼은 여전히 `disabled`이고 메뉴를 열지
   않는다.
