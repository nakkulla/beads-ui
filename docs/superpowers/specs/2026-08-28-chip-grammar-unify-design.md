...
| 워커 생성 (`worker_created_from`) | `워커 생성` · `↩ 생성 원본 <ID>` | Worker가 어느 이슈에서 새로 만들었나 | 슬롯 5 출처 그룹 | 없음 |
| 상위 (`parent-child`) | `⌸ <ID>` | Phase 자식의 부모 | 없음 | 있음 |
| 관련 (`related`) | `↔ <ID>` | 양방향 소프트 관계 | 없음 | 있음 |
| 그 외 type | `<type> <ID>` | — | 없음 | 있음 (현행) |

**정정(UI-7341).** 원래 이슈 카드의 `외부 계산 대기 N건`은 슬롯 4a 판정 칩이다.
클릭하면 확인된 외부 작업별 상태와 실제 gate 상세 링크를 보여 준다. 기존 `⛓
<gate-id>`는 계속 gate 상세를 직접 열며, 두 칩 모두 상태를 쓰거나 gate를 닫지
않는다.
...
**정정(UI-j10d).** `worker_created_from`은 실행·관계 추론이 아닌 최초 생성
metadata다(`2026-09-14-worker-created-issue-chip-design.md` §2·§4). 슬롯 5에서
`워커 생성` 사실 칩 뒤에 `↩ 생성 원본 <ID>` 링크를 두며, 원본 저장소가 확인된
경우에만 링크로 만든다. 같은 ID의 `↩ from`은 중복 제거하고, 다른 ID면 두 관계를
구분해 함께 표시한다. 이 그룹은 완료 행에도 남는다.

**정정(UI-8gem).** 외부 작업 관측 행의 4a 칩은 `→ <consumer>`다 — 이 gate가 풀리면
진행할 원래 이슈이고, 표의 "내가 막는 후속"과 같은 관계다. 자기 gate를 가리키던
`⛓ <gate-id>` 칩은 없앤다.

**정정(UI-8gem).** 원래 이슈 카드의 칩은 `<글리프> 외부 계산 N건`이다 — 글리프는 그
이슈의 열린 gate 사유 중 최악 판정(`⏳`·`⚠`·`⛔`)이고, 클릭은 gate별 상태 팝업을
연다.
