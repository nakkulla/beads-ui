...
비우고 재요청, 실패는 `showToast`(`app/utils/toast.js`, `index.js:4`에서 이미
...
넘기므로 그 필드는 이미 클라이언트에 도달한다(`bd list --json` 항목 키에
`comment_count` 존재 확인). 폴링도 새 구독도 없이 worker가 쓴 보고서가 열어둔
화면에 뜬다.

댓글 작성 후에도 같은 경로로 갱신하고 낙관적 갱신은 두지 않는다 — 서버가
`bd comment`를 돌린 뒤 `comment_count`가 오르므로 같은 감지가 이미 작동한다.
...
머지 후 `bdui-shared restart`로 공유 서버를 재시작하고, 그 worker 세션이 남긴
보고서를 UI에서 실제로 읽으면 검증이 닫힌다. 그때까지 이 단위는 완료가 아니다
...
- 머지 후 `bdui-shared restart`, 프로세스 경로·리스닝 포트·HTTP 응답 확인
- 재시작한 공유 서버에서 `Analysis-ex0`와 `dotfiles-ygcy`의 보고서가 실제로
  읽히는지 확인
...
- **`comment_count` 전달은 코드 근거**: `...it` 스프레드로 확인했으나 런타임
  실측은 구현 시점에 한다. 부재 시 fail-quiet으로 최초 1회 fetch는 유지된다
