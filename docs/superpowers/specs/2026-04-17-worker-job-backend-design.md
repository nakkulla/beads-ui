...
- lock 획득에 성공한 인스턴스만 SQLite write / runner ownership / cancel orchestration을 수행할 수 있다.
- lock 획득에 실패한 인스턴스는 새 writer가 되지 않고, 기존 supervisor로 재연결만 시도해야 한다.
...
2. **PID + health check**
   - lock holder가 남긴 PID file과 health endpoint를 확인해 살아 있는 owner인지 검증한다.

stale owner 처리 규칙:

- PID file이 남아 있지만 process가 없고 health check도 실패하면 stale owner로 본다.
- stale owner로 판정된 경우에만 lock takeover를 허용한다.
- takeover 시 이전 stale PID file을 정리하고 새 supervisor가 PID/log/health ownership을 이어받는다.

즉, 첫 버전은 **PID file만으로 owner를 판정하지 않고, exclusive lock + PID + health**를 함께 사용해
split-brain과 double-run을 막는다.
...
- 따라서 정상 enqueue된 job에 대해서는 log endpoint가 “파일이 아직 없어서 404”를 반환하지 않는다.
- 출력이 아직 없는 초기 상태에서는:
...
즉, log endpoint의 `404`는 “job 없음” 또는 “path를 아예 resolve할 수 없음”에만 사용하고,
빈 로그는 empty tail로 표현한다.
...
2. **issue-only target**
   - body에 `issueId`만 있고 `prNumber`가 없으면, supervisor는 **해당 issue에 linked된 PR 중 `open` 상태인 PR만** 후보로 본다.
