...
두 형태 모두 사용자가 개입할 근거가 없다. 브랜치를 지워도 잃는 내용이 없고,
...
| `server/worker/recovery-archive.js` | branch-only archive 모드(커밋 번들 전용) |
...
그 ref가 기여한 바가 없다. 따라서 ref를 지워도 잃는 내용이 없다.
...
| `P`에 gitlink(submodule) 항목 | `ls-tree` mode `160000` | `unique` / `ahead_submodule_path` |
...
detached 잔재는 삭제 후 되돌릴 ref가 없어 회수 실패 시 복구 근거가 약하다.
현행대로 `unique`로 보존한다.
...
| 형태 | 순서 |
|---|---|
| A. clean worktree + contained ahead branch (`dotfiles-dk6v`) | `worktree remove` → CAS ref 삭제 |
| B. branch-only contained ahead (`UI-20gk`) | CAS ref 삭제 |
| C. dirty base_contained + contained ahead branch | exact-path restore → 재관측 → `worktree remove` → CAS ref 삭제 |
...
cleanup은 `worktree remove`를 건너뛰고 §5.2와 같은 CAS ref 삭제만 수행한다.
...
3. `update-ref` CAS 실패는 원본 ref를 보존하고 `ref_delete_failed`를 반환한다.
4. Git 관측 실패는 부재나 clean으로 해석하지 않고 `unknown`으로 확정한다.
5. 증명 실패는 어떤 파일·ref·worktree도 바꾸지 않는다.
6. queue revision과 `action_id`는 stale click을 거절한다. 같은 identity와 cause를
   다시 관측하면 revision을 반복 증가시키지 않는다.
7. archive 검증 전 cleanup은 0회다.
8. raw Git stderr와 파일 내용은 snapshot이나 알림에 포함하지 않는다.
...
1. `server/worker/worktree.integration.test.js`
   - `dotfiles-dk6v` 형태(clean worktree + behind 상태의 contained ahead branch)를
     `base_contained`로 판정하고 `worktree remove` 후 CAS ref 삭제까지 마친다.
   - `UI-20gk` 형태(worktree 없는 contained ahead branch)를 CAS ref 삭제만으로
     회수한다.
   - dirty base_contained와 contained ahead가 겹친 조합을 restore 후 회수한다.
   - behind 0인 순수 ahead branch는 `ahead_not_contained`로 보존한다.
   - `P`의 한 경로라도 mode 또는 blob이 다르면 `ahead_not_contained`로 보존한다.
   - 브랜치 안에서 바꿨다가 되돌린 경로는 `P`에서 제외되어 증명을 막지 않는다.
   - rename은 `--no-renames`로 삭제·추가 두 경로로 각각 비교된다.
   - `base..ref`에 merge commit이 있으면 `ahead_merge_commit`으로 보존한다.
   - `P`에 gitlink가 있으면 `ahead_submodule_path`로 보존한다.
   - 관측과 CAS 삭제 사이 ref가 움직이면 `ref_delete_failed`로 브랜치를 보존한다.
   - 재관측에서 state 또는 identity가 달라지면 `identity_changed`로 무변경
     거절한다.
   - `head_ahead` detached 잔재는 계속 보존한다.
   - UI-8vn1의 기존 dirty base_contained·unique·unknown 판정이 그대로 유지된다.
...
3. `server/worker/recovery-archive.test.js` 및 discard coordinator 테스트
...
4. `server/ws/*worker*.test.js`
   ...
   - public projection이 경로, raw stderr, 파일 내용을 노출하지 않는다.

5. `app/views/worker/index.test.js`
   - `branch` 형태 카드가 `이전 브랜치 보존됨` 제목과 고유 commit 수 요약,
     허용된 두 버튼만 렌더한다.
   - `worktree` 형태 카드 표현이 회귀 없이 유지된다.
...
1. `dotfiles-dk6v` 재현형과 `UI-20gk` 재현형이 사용자 개입 없이 회수되고 같은
   tick에서 dispatch된다.
2. `P`의 어느 경로든 base와 다르거나, merge commit 또는 gitlink가 있거나, Git
   관측이 실패하면 파일·worktree·ref를 변경하지 않는다.
3. 판정 직전과 파괴 단계 사이 identity 또는 ref가 움직이면 CAS로 거절하고 원본을
   보존한다.
4. 자동 회수가 원격 ref, PR, base를 변경하지 않는다.
5. 고유 커밋을 가진 branch-only 잔재가 `백업 후 새로 시작`과 `다시 확인`을 가진
   실행 가능한 카드로 표시된다.
6. branch-only 백업은 verified archive 없이는 어떤 파괴적 단계도 실행하지 않는다.
7. 백업 없는 삭제 action이 없다.
8. focused integration/scheduler/queue 회귀 테스트가 UI-8vn1의 dirty
   base_contained 및 unique/unknown 동작을 함께 보존한다.
9. dotfiles workflow metadata·label·status 어휘가 바뀌지 않는다.
10. 전체 pre-handoff validation과 frontend bundle 생성이 통과한다.
...
1. **ahead containment 증명과 CAS 자동 회수.** `server/worker/worktree.js` 중심의
   server-only 변경이다. 두 실측 형태를 여기서 해소하며 frontend bundle이 필요
   없고, focused integration/scheduler 테스트만으로 봉인된다.
2. **증명 실패 branch-only 잔재의 복구 표면.** admission 소유·능력 계산,
   branch-only archive 모드, branch-only cleanup, ws 게이팅, Worker UI, bundle
   재생성을 포함한다.

두 번째 unit은 첫 번째가 봉인한 identity·`residue` 계약을 소비하고 durable
operation과 frontend bundle까지 포함한다. 따라서 route는 `full_plan`이며, spec
