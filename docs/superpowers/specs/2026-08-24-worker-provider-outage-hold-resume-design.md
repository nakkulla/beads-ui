...
> 정정(2026-09-21): 아래의 재무장 3회·24시간 상한과 상한 target의 재시작 1회
> 프로브 규칙을 철회한다. `usage_limit` 프로브는 매 리셋 시각 + 60초(시각 미상이면
> 15분)에 계속 재무장하며 `rearm_count`는 표시용이다. 상한 중단 알림도 제거한다.
> §8.1의 계보 자동 재개 cap과 `auto_resume_cap` 알림은 유지한다.
> 정본: `docs/superpowers/specs/2026-09-21-account-auto-switch-never-stalls-design.md` §3.3.
