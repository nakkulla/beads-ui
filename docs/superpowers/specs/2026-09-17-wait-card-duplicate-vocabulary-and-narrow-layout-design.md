...
**정정(UI-pw2g).** 결정 2("대기 행의 카드 구조는 좁은 화면(≤640px)에서만. 데스크톱의
한 줄 밀도는 유지한다")와 그것을 구현한 §7.1은 효력이 없다. 그 결정이 지키려던 것은
데스크톱 밀도였는데, 판정 기준이 뷰포트 폭이라 레인이 여러 컬럼으로 나뉘는 화면에서는
컬럼 폭을 대변하지 못했다 — 창이 넓어도 대기 레인 컬럼만 좁아지면 분기에 걸리지 않고,
그 상태에서 한 줄에 실린 제목이 먼저 잘렸다. 새 결정은 폭과 무관한 카드 구조 하나이고,
`.worker-mini__line` 한 줄 변형은 제거됐다. §7.1이 정한 카드 머리 줄의 순서
(`grip · 순번 · ID · 우선순위 · PR · 외부 PR · 상태 뱃지 · 대기 판정 배지 · 조작`)와
reason이 그 아래 자기 줄을 쓰는 것은 그대로 남는다. 근거는
`2026-09-21-wait-row-card-layout-and-verdict-slot-design.md` §3.1이 소유한다.
...
**정정(UI-pw2g).** `options.card`와 뷰포트 폭 판정은 제거됐다 — §2 결정 2에 붙은
정정을 볼 것. 카드 변형은 폭과 무관한 대기 행의 유일한 구조이고, 이 절이 정한 머리
줄 순서와 reason 자리는 그대로 남는다.
