...
| README의 Worker 설명이 CI 배지·항상 사람의 `[머지]` 클릭·head 고정 검증을 말하고, 상세 패널을 "5-key 실행 설정"으로 설명 | `README.md:29-31,46-47`; 실제 실행 설정 키는 `app/views/detail-panel/exec-settings.js`에 14개 | ADR 0003·0006·0031과 현재 코드에 맞지 않으므로 함께 고쳐야 한다 |
...
- README의 Worker·PR 대기 설명: CI 배지와 "항상 사람의 `[머지]` 클릭", head 고정
  검증 문장을 현재 동작으로 바꾼다 — 머지 자격은 GitHub checks를 보지 않고(ADR 0003),
  머지는 큐가 소유하며 자동/수동 모두 같은 경로로 수렴하고(ADR 0006), 구현 리뷰
  영수증은 exact head가 아니라 ancestry 결속이며(ADR 0031), `[verify]`는 그와 별개의
  머지 직전 안전망이다. 상세 패널의 실행 설정은 "5-key" 같은 키 수를 문장에
  고정하지 않고 현재 편집기가 다루는 설정 묶음(오케스트레이션·구현·리뷰 단계별
  모델/effort/speed와 `workflow_mode`)으로 설명한다.
...
- 필요한 패키지 파일 포함·미사용 코드 삭제·설명 정정: 되돌리기 어려움 아니오(영속 스키마·공개 계약·배포 선언을 바꾸지 않는 국소 파일 변경이라 게시 목록 복원과 파일 복구로 되돌린다), 맥락 없이 의외 아니오(확인된 누락·미도달·문서 불일치를 직접 고치는 작업이라 결과가 예상 가능하다), 실질 대안 있음(app 전체 게시와 일괄 재구조화) → ADR 아님
