...
1. `bead_blocked_by[bead]`는 title cache 적중 bead에 **키를 항상 싣고**, 열린
   ...
   **키가 있고 ID가 없으면 닫힘**이라는 판정이 같은 rig·foreign 양쪽에 같은
   식으로 성립한다.
...
| D2 | 닫힌 blocker는 기존 해제 칩 어휘 `🔓 <ID>`(4b, 툴팁 "닫힘 · 복귀 대기")로 내려가고, 전부 닫히면 상태 뱃지가 `⛓ 복귀 대기`다 | `⛓ <ID> · 닫힘`(새 어휘); 뱃지만 변경 |
...
- **판정 재료는 서버가 이미 증명한 것만.** 닫힘 판정은 `bd ready`와 같은
...
`staleWorkProjection`(`lanes.js:711`)이 카드를 그리는 조건과 같은 재료다 —
카드가 서지 않을 admission으로 타일을 지우면 조작 없는 행만 남으므로 둘은
같은 식이어야 한다.
...
## 6. 범위 2 — 닫힌 blocker와 `복귀 대기` 뱃지 (held `waiting` 타일)
...
if Object.hasOwn(bead_blocked_by, bead_id):
  open   = bead_blocked_by[bead_id]                       // 서버가 증명한 열린 선행
  closed = wait.blockers.map(id).filter(id ∉ open)        // 동결 목록 중 서버 목록에 없는 것
else:
  closed = []                                             // 모름 → 현행(전부 ⛓)
```

- 항목 `blocked_by` = `open ∪ (wait.blockers − closed)` — 닫힌 ID는 `⛓`에서
  뺀다. `prerequisite_unmet` admission의 `blockers`(UI-d3i1 §5.4)는 attempt
  없는 큐 항목의 재료이므로 이 판정 대상이 아니다(현행 합침 유지).
- 항목 `dependency_chips.released`에 `closed`를 `🔓 <ID>` 칩으로 싣는다.
  `releasedChip`(`queue-blockers.js:96`)은 `closed_at`이 없으면 null이므로,
  같은 파일에 **`closedBlockerChip(owner_id, id, workspace_name?)`** 을 더한다:
  라벨 규칙은 §8과 같고, 툴팁은 `닫힘 · 복귀 대기 — 선행이 close되어 이 이슈가
  다시 후보로 돌아간다`, 7일 창 없음, `foreign`·`openable`·`root_dir` 규칙은
  선행 칩과 같다. 4b 줄에 서는 것은 `dependencyChipsTemplate`(`lanes.js:1046`)의
  현행 `released` 처리 그대로다.
- `wait.returning = wait.blockers.length > 0 ∧ closed.length ===
  wait.blockers.length`. `WaitTile`(`running-grid.js:184`)에
  `returning?: boolean`을 더한다. 키 부재면 `false`.
...
`⛓ 복귀 대기`, 툴팁 `선행이 모두 닫혔습니다 — 다음 pass에서 후보로 돌아갑니다
(슬롯·레인 순서 대기)`. 아니면 현행 `⛓ 선행 대기`. 같은 배타 자리이며 새
슬롯은 없다.
...
붙인다. `closedBlockerChip`(§6.1)과 `releasedChip`도 같은 표를 따른다 — 같은
...
- 4a: `⛓ <ID>`의 "타 레포는 색만 다름"을 "타 레포는 `<workspace>/<ID>` 표기와
  색"으로.
- 4b: `🔓 <ID>`에 "선행 대기 타일의 닫힌 blocker(복귀 대기)도 이 칩"을 더한다.
...
1. admission 키가 있으나 `stale_work`가 없거나 `action_id`가 빈 옛 기록: 판정
   거짓 → 현행 렌더(타일/ghost). 카드가 서지 못할 admission으로 강등하지
   않는다.
...
3. `parked`·`retry_wait`·`provider_hold` + `worktree_stale_work` 조합은 생기지
   ...
   이긴다(강등은 `waiting`에만).
4. `bead_blocked_by` 키 부재(title cache 미적중): 닫힘 판정 없음, 전부 `⛓`,
   `returning=false`. 부재를 "닫힘"으로 읽지 않는다.
...
- 닫힘: `waiting` attempt blockers `[X, Y]`, `bead_blocked_by[bead]=[Y]`. 기대:
  `⛓ Y`·`🔓 X`, 뱃지 `⛓ 선행 대기`. `bead_blocked_by[bead]=[]`면 `🔓 X`·`🔓 Y`,
  뱃지 `⛓ 복귀 대기`. 키 없음이면 `⛓ X`·`⛓ Y`.
- foreign: owner `Analysis-2zly` → `blocker_workspaces` 있음 → `⛓
  microbiome_bile/Analysis-2zly`; 없음 → `⛓ 외부/Analysis-2zly`.

## 12. 검증 bundle

| 파일 | 고정하는 것 |
| --- | --- |
| `app/views/worker/lane-model.test.js` | 경로 1·2 강등(타일/ghost 없음·행 있음·`claimed` 이중 렌더 없음); admission 없음·`stale_work` 없음이면 현행 타일/ghost 불변; entry 없는 점유자는 ghost 유지; §11 닫힘 세 케이스와 `returning`; foreign `workspace_name` 재료 전달 |
| `app/views/worker/index.test.js` | 두 경로에서 s1 행에 `.worker-mini__stale-continue`·`.worker-mini__stale-backup`이 `data-action-id`와 함께 렌더되고, 클릭이 `worker-stale-work-continue`를 `{bead_id, action_id, expected_revision}`으로 보낸다; 강등 행 `draggable=false`; 뱃지 `실패 · 점유 유지` |
| `app/views/worker/running-grid.test.js` | `wait.returning`이면 `⛓ 복귀 대기`, 아니면 `⛓ 선행 대기`; held 본문의 `released` 칩 렌더 |
| `app/views/worker/queue-blockers.test.js` | `predecessorChip`·`closedBlockerChip`·`releasedChip` 라벨 세 변형과 툴팁 접두; `deriveWorkerBlockers`가 `blocker_workspaces`에서 `workspace_name`을 만든다 |
...
1. `server/worker/foreign-blocker-status.js` — §7 (`startLookup` 분리·활동
   구독·항목 필드).
2. `app/views/worker/lane-model.js` + `queue-blockers.js` — §5 강등, §6.1 닫힘
   판정과 `closedBlockerChip`, §8.1-8.2 라벨 재료.
...
- 사람의 처분을 기다리는 admission(`worktree_stale_work`)이 선 bead는 held
  ...
  행에만 산다 — 두 탭·세 투영에 걸치고, 되돌리면 처분 카드가 세 자리에 다시
  살아야 하며, 대안(타일·ghost에 카드 추가, dismiss 시점 이동)을 기각한 선택이다
  → ADR. `summary`: "처분 대기 admission이 선 bead는 held 타일·점유 ghost가
  아니라 대기 행이 대표하고 stale-work 처분 조작은 대기 행에만 산다"
