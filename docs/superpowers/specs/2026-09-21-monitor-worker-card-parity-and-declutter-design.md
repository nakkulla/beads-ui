...
**정정(UI-c8kc).** 규칙 2·8의 "줄바꿈 없음"과 "배지는 줄바꿈 대신 `min-width: 0`으로 줄어든다"는 640px 초과 폭에만 적용한다. ≤640px에서는 세 머리줄이 줄을 넘기고 조작 묶음은 자기가 선 줄의 오른쪽 끝에 남으며, 열린 대기 판정 배지는 머리줄 한 줄을 통째로 차지해 팝업이 카드 폭으로 펼쳐진다. 근거: 2026-09-23 사용자 모바일 화면에서 팝업이 35px 폭으로 눌려 세로 글자가 되고 `backend`·`⏳ 외부 작업`이 말줄임됐다 — UI-pw2g §2 결정 2(잘림 없이 줄을 늘린다)와 같은 방향이다.

**정정(UI-nr86).** ≤640px 머리줄의 레포 배지(`.worker-card__repo`·`.worker-mini__repo`)는 12ch 상한과 말줄임을 풀어 전체 이름을 싣는다. 머리줄이 줄을 넘기므로 긴 이름은 다음 줄로 내려갈 뿐 잘리지 않는다. 640px 초과 폭은 상한을 유지한다. 근거: 2026-09-23 사용자 모바일 화면의 `microbiome_bile` → `microbiom…` 잘림과 사용자 선택(prefix 대체 대신 모바일 상한 해제).
