...
> 처리 완료 terminal이고 **같은 bead의 더 오래된 attempt 전부가 같은 pass에서 이관
> 가능할 때만** 큐를 떠난다.
...
```
for each bead in group(q.attempts, by bead_id):
  sorted = attempts sorted by (at(a), a.attempt_id)   # at = started_at ?? finished_at
  for a in sorted (oldest first):
    if !isProcessedTerminalAttempt(a): break         # 첫 미처리 레코드에서 그 bead 종료
    if bead ∈ heldBeads(q): break                     # §4
    candidates.push(a)
```
...
- **정렬 키**는 `readAttemptsForBead`와 같은 `(started_at ?? finished_at, attempt_id)`다.
  같은 bead의 같은 판정에 두 정렬이 있으면 서로 어긋나므로 comparator는 하나를 공유한다.
- **시각이 둘 다 null인 attempt**(해소 대기 prerecord: `started_at: null`로 `running`)는
  아직 시작하지 않았으므로 **가장 최신**으로 정렬한다(`at = +Infinity`). 0으로 두면
  가장 오래된 것이 되어 그 bead의 이관 전부를 막는다. `readAttemptsForBead`의 정렬은
  표시 순서이므로 바꾸지 않는다 — comparator 공유는 "시각이 있는 레코드끼리"의 규칙이고,
  null 처리는 이관 판정의 것이다. 이 차이는 코드 주석에 고정한다.
...
| repair-lane 보류 계획의 attempt_ids | `transferProcessedAttempts`의 `withheld` — 지금 그대로 |
...
| reader | 보는 것 | 보존 근거 |
|---|---|---|
| `attempt-failure.js` `createUnhandledFailurePredicate` (`auto-advance-restore.js`, `settleMootRepairFailures`) | bead의 마지막 구현 attempt(`superseded`), `queue.done` 시각 | 판정 대상 attempt는 큐에 있으므로 접미 비어 있지 않음 → 마지막 보존. `queue.done`은 attempt가 아니라 레인 엔트리로 무관 |
...
미처리 레코드(미dismiss `failed`·`parked`·`paused`·`stopped`·`orphaned`·`retry_wait`)
**뒤의** 처리 완료 기록은 그 레코드가 처리될 때까지 큐에 남는다. 이는 사람이 보고
dismiss·재개·폐기할 상태이고, 그 처리 순간 다음 persist에서 접두가 함께 나간다. §7의
...
pass에서 이관하지 않는다** — 그래야 큐가 접미로 유지된다. 구현: 후보 목록을 bead별
오래된 순으로 순회하고, 한 bead에서 첫 실패가 나면 그 bead의 나머지 후보를 건너뛴다.
...
7. 시각이 둘 다 null인 `running` attempt와 더 오래된 `done` → done 이관.
8. 접미 중간 후보의 파일 쓰기 실패 → 그보다 최신 후보는 같은 pass에서 잔류(§7).
9. `record-retention` 마이그레이션 테스트: in-flight bead의 처리 완료 접두가 이관됨
...
`transferProcessedAttempts`) + `queue-store.test.js` + `record-retention.test.js` 기대
갱신.
...
- **살아 있는 `queue.attempts`는 bead 이력의 최신 접미다 — 이관은 접미 불변식으로
  ...
  - 되돌리기 어려움: 성립 — reader 8곳이 "라이브 큐 = 최신 접미"에 암묵적으로 의존하므로
    불변식을 깨려면 reader 전부를 합집합 조회로 먼저 옮겨야 한다.
