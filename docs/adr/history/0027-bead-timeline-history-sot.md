...
- 배제되는 것: 처리 완료되어 이관된 attempt의 정본은 더 이상 `queue.json`이
  아니다. 그 파일에 오래된 레코드가 없는 것은 삭제된 이력이 아니라 상태 전용화의
  정상 결과이며, bead 전체 이력이 필요한 소비자는 상태 파일이 아니라 bead
  디렉터리를 함께 읽는 조회를 쓴다.
