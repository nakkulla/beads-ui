...
- **화면은 기록을 먼저 그리고 자체 판정은 예측으로 내려간다.** 대기 행의 4a 게이트 칩은
  ...
  이 행이 지금 해석하는 계정이 기록과 같다는 것이다. `unresolved` 기록의 유효 조건은
  해석값이 아니라 **선언 두 층(핀·저장소 기본)이 둘 다 비었는가**다 — 서버의 미해석에는
  카탈로그 조회 실패가 섞이고 그때 프론트 스냅샷에는 활성 계정이 그대로 남으므로, 해석값으로
  판정하면 서버가 막은 행에서 칩과 두 출구가 함께 사라진다. 선언 두 층은 서버와 프론트가 같은
  스냅샷 값을 읽으니 거기서 어긋나야 진짜 설정 변경이고, 활성 로그인 층만으로 설명되는 차이는
  조회 시차다. `outage` 기록은 계정과 무관하게 유효하다. 조건이 깨진 기록은 무시하고
...
- **계정을 해석할 수 없으면 fail-closed로 막되 그 사실이 보인다.** 세 층이 모두 비거나
  ...
  팝업에 `계정: 미해석 — 서버가 핀·저장소 기본·활성 로그인 어디에서도 <runner> 계정을
  정하지 못함` 한 줄이 선다. 미해석 행을 통과시키지는 않는다 — 한도 계정으로 다시 나가 attempt 기록만
  쌓을 위험이 있다.
