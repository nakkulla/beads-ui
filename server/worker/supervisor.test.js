...
import { createWorkerSupervisor, createWorkerSupervisorServer } from './supervisor.js';
...
describe('worker supervisor', () => {
  test('creates job, marks it running, and finalizes success on close', async () => {
    ...
    const supervisor = createWorkerSupervisor({ root_dir, store, runner: runner_stub.runner, owner_pid: 9001, health_check_impl: async () => true, is_process_running_impl: () => true, now: () => '2026-04-17T03:00:00.000Z' });
  ...
  test('moves running job into cancelling and cancelled during cancel flow', async () => {
    ...
    const store = createJobStore({ root_dir, now: () => '2026-04-17T03:00:00.000Z' });
    const runner_stub = createRunnerStub();
    const supervisor = createWorkerSupervisor({ root_dir, store, runner: runner_stub.runner, owner_pid: 9002, health_check_impl: async () => true, is_process_running_impl: () => true, now: () => '2026-04-17T03:00:00.000Z' });
  ...
  test('marks stale active job as failed during reconcile', async () => {
    const root_dir = mkdtemp();
    const store = createJobStore({ root_dir, now: () => '2026-04-17T03:00:00.000Z' });
    const job = store.createJob({ command: 'bd-ralph-v2', issueId: 'UI-qclw', workspace: root_dir });
    store.updateJob(job.id, { status: 'running', pid: 999999, started_at: '2026-04-17T03:00:00.000Z' });
    const supervisor = createWorkerSupervisor({ root_dir, store, owner_pid: 9003, health_check_impl: async () => true, is_process_running_impl: () => false, now: () => '2026-04-17T03:05:00.000Z' });
  ...
  test('takes over stale owner when health check fails for live pid', async () => {
    ...
    fs.writeFileSync(lock_path, JSON.stringify({ pid: 1111, port: 4099, acquired_at: '2026-04-17T02:59:00.000Z' }));
    const supervisor = createWorkerSupervisor({ root_dir, store, owner_pid: 9004, health_check_impl: async () => false, is_process_running_impl: (pid) => pid === 1111, now: () => '2026-04-17T03:00:00.000Z' });
