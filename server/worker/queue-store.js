...
export function createQueueStore(options = {}) {
  ...
  return {
    ...
    /**
     * Re-adopt only the historical post-merge repair terminal created when the
     * scheduler compared a cleanup failure against the pre-merge head SHA.
     * The exact terminal, operation, lineage, and queue predicates share one
     * persist so a restart cannot restore the root without its existing repair
     * journal.
     *
     * @param {string} workspace
     * @param {{ root_bead_id: string }} input
     * @returns {QueueOpResult}
     */
    adoptHistoricalStaleRepair(workspace, input) {
      const { root_bead_id } = input;
      return applyUnconditional(workspace, (next) => {
        const intent = next.completion_intents[root_bead_id];
        const terminal = intent?.terminal_reason;
        const active_op = intent?.active_op;
        const repair_bead_id = active_op?.repair_bead_id;
        const subject = intent?.subject;
        const repair_owned_elsewhere =
          typeof repair_bead_id === 'string' &&
          Object.entries(next.completion_intents).some(
            ([other_root, other_intent]) =>
              other_root !== root_bead_id &&
              (other_root === repair_bead_id ||
                other_intent.repair_bead_ids.includes(repair_bead_id))
          );
        if (
          next.auto_merge !== true ||
          ...
          repair_bead_id.length === 0 ||
          !sameCompletionFailureKey(
            terminal.failure_key,
            active_op.failure_key
          ) ||
          !intent.repair_bead_ids.includes(repair_bead_id) ||
          repair_owned_elsewhere ||
          typeof subject?.merged_sha !== 'string' ||
          subject.merged_sha === subject.head_sha ||
          active_op.failure_key.subject_sha !== subject.merged_sha ||
          active_op.failure_key.base_sha !== subject.base_sha ||
          next.merge_queue.some((entry) => entry.bead_id === root_bead_id) ||
          intent.repair_sessions_used >= MAX_REPAIR_SESSIONS
        ) {
          return false;
