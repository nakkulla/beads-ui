...
export function guardContractDirective(options = {}) {
  ...
  if (disposition) {
    ...
    lines.push(
      ...
      '- **1회성 재배치**(`git -c core.hooksPath=…`, `git --config-env=core.hooksPath=…`, `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_*`/`GIT_CONFIG_VALUE_*`/`GIT_CONFIG_PARAMETERS` 접두) 중 뒤따르는 명령이 아래 「허용됨」의 열거 밖인 것 — 명령 없는 할당, git 이 아닌 명령(`go test` 등), 열거 밖 서브커맨드(`push`·`commit`·`diff`·`log`·`show`·`submodule` 등)가 모두 여기 든다. `GIT_CONFIG_PARAMETERS` 접두는 뒤따르는 명령이 무엇이든 면제가 없고, `export`로 내보낸 `GIT_CONFIG_*` 할당은 bare 할당과 같은 지속 재배치다.',
