...
export function mergeLockProtocol(params) {
  ...
  return [
    ...
    '2) 머지 + 스윕 수행.',
    `3) 해제: \`${release}\` — 머지 커밋 SHA(40-hex)를 알고 있으면 요청 body에 "merge_sha" 필드로 함께 보내라(서버가 직접 읽은 base tip과 대조된다).`,
    '해제가 성공하면 서버가 머지를 관측·기록하고 검증 인계를 시작한다 — 세션은 락 해제 완료를 기다리지 않고 종료해도 된다. 해제가 409로 거부되면(base 미전진·SHA 불일치) 머지가 관측되지 않은 것이므로 blocker로 보고하고 비정상 종료하라.',
