...
describe('worker/attach construction + live loop (F1)', () => {
  ...
  test('wires the integrated historical adopter into startup reconciliation', async () => {
    const runtime = createWorkerRuntime();
    const adoptHistoricalTerminal = vi.fn(async () => true);
    const completionActionDriver = {
      observe: vi.fn(),
      onAction: vi.fn(),
    ...
    runtime.queueStore.terminalizeCompletionIntent(WS, {
      root_bead_id: 'UI-root',
      terminal: {
    ...
    expect(adoptHistoricalTerminal).toHaveBeenCalledWith(
      'UI-root',
      expect.objectContaining({
        phase: 'needs_human',
        terminal_reason: expect.objectContaining({
          reason: 'completion_subject_sha_stale',
          stage: 'repair_dispatch'
        })
      }),
