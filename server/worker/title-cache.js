...
export function createTitleCache(options = {}) {
  ...
  /**
   * Fill a batch of missing titles, then notify ONCE if anything landed. Fully
   * fail-quiet: a bead that cannot be read is negative-cached and logged, and
   * never blocks the other ids in the batch or the snapshot that asked.
   *
   * @param {string} workspace
   * @param {string[]} bead_ids
   */
  async function fill(workspace, bead_ids) {
    const lane = laneFor(workspace);
    const failed = failedFor(workspace);
    let landed = false;
    for (const bead_id of bead_ids) {
      const flight_key = `${keyOf(workspace)}\u0000${bead_id}`;
      try {
  ...
  return {
    ...
    titlesFor(workspace, ids) {
      ...
      if (missing.length > 0) {
        ...
        // swallows everything it can fail on.
        void fill(workspace, missing);
      }
      return out;
    },
