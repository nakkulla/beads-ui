 ...
 * @property {string} [impl_speed] - impl_speed. Read for the facts card's
 * `선택 입력` line only (harness-reduction spec D1): `policy.js` resolves no
 * such axis, so this snapshot field is DISPLAY, never a launch input.
 ...
 * @property {() => Promise<string|null>} [resolveDotfilesRoot]
 * The dotfiles repository root, for the dispatch facts card (harness-reduction
 * spec D1). Optional and fail-quiet: an unwired resolver omits the
 * `dotfiles_root` line, which is what every hermetic test wants — the value is
 * a property of the machine, not of the queue.
 * @property {{
 ...
 * @property {{ sync: (workspace: string) => void }} [providerHealth]
 ...
 *   disposeStaleResidue: (workspace: string, bead_id: string, observation: WorktreeObservation, resume_attempt: Attempt|null, cut_base: string) => Promise<any>,
...
export function createScheduler(deps) {
  ...
  /**
   * Read the runner account row used by provider classification and hold scope.
   *
   * @param {any} attempt
  ...
  async function providerAccountContext(attempt) {
    ...
    try {
      if (
        ...
        typeof deps.accountCatalog.readClaude === 'function'
      ) {
        const result = await deps.accountCatalog.readClaude(
          attempt.claude_account
        );
        return result.ok
          ? { account: result.account.email, row: result.account }
          : { account: attempt.claude_account, row: null };
      }
      if (typeof deps.accountCatalog.activeClaude === 'function') {
        const result = await deps.accountCatalog.activeClaude();
        return result.ok
          ? { account: result.account.email, row: result.account }
          : { account: null, row: null };
      }
    } catch {
      // Catalog failure is represented as account:null and never broadens proof.
    }
    return { account: null, row: null };
  }
    ...
    for (const hold of Object.values(
      ...
      for (const target of /** @type {any} */ (hold).targets || []) {
        if (typeof target.account === 'string') {
          held_accounts.add(target.account);
        }
      }
    }
    return (
  ...
  /**
   * Detect whether a resume lineage already used its fresh transcript fallback.
   *
   * @param {string} workspace
   * @param {any} attempt
   */
  function lineageHasResumeFallback(workspace, attempt) {
    const attempts = deps.store.snapshot(workspace).attempts || {};
    const seen = new Set();
    let cursor = attempt;
    while (cursor && !seen.has(cursor.attempt_id)) {
      seen.add(cursor.attempt_id);
      if (cursor.resume_fallback?.reason === 'transcript_missing') {
        return true;
      }
  ...
  /**
   * Read recent assistant prose from the source attempt's stored session log.
   *
   * @param {string} workspace
   * @param {any} prior
   */
  function recentAssistantTexts(workspace, prior) {
    ...
    while (cursor && !seen.has(cursor.attempt_id)) {
      ...
      let raw;
      try {
        raw = deps.sessionLog.read(workspace, cursor.attempt_id, {
          bead_id: cursor.bead_id,
          log_path: cursor.log_path ?? undefined
        });
      } catch {
        raw = null;
      }
  ...
  /**
   * Build the bounded context handoff injected into a fresh continuation.
   *
   * @param {string} workspace
   * @param {any} prior
   */
  function resumeHandoffBlock(workspace, prior) {
    const header = `이 작업은 공급자 장애/한도로 중단된 ${prior.runner || 'unknown'} 세션의 연속이다. 같은 워크트리에 부분 작업이 남아 있을 수 있다.`;
    const instruction =
    ...
    const available = Math.max(
      0,
      RESUME_HANDOFF_MAX_CHARS - prefix.length - suffix.length
    );
    const summary =
      recentAssistantText(workspace, prior).slice(-available) ||
      '원 세션의 assistant 텍스트를 읽을 수 없음.';
    return `${prefix}${summary}${suffix}`.slice(0, RESUME_HANDOFF_MAX_CHARS);
  }
  ...
  /**
   * Match only the runner's exact missing-transcript diagnostics.
   *
   * @param {string} workspace
   * @param {string} attempt_id
   * @param {any} attempt
   ...
   */
  function resumeTranscriptMissing(workspace, attempt_id, attempt, verdict) {
    if (verdict.reason !== 'no_result') {
      return false;
    }
    const stderr_tail = providerStderrTail(workspace, attempt_id, attempt);
    return (
      typeof stderr_tail === 'string' && TRANSCRIPT_MISSING_RE.test(stderr_tail)
    );
  }
   ...
   * @returns {string|null}
   */
  function providerStderrTail(workspace, attempt_id, attempt) {
    const log_path =
      typeof attempt.log_path === 'string'
        ? attempt.log_path
        : deps.sessionLog.pathFor?.(workspace, attempt_id, attempt.bead_id);
    const stderr_path = deps.sessionLog.stderrPathFor
      ? deps.sessionLog.stderrPathFor(workspace, attempt_id, attempt.bead_id)
      : log_path
    ...
    try {
      const contents = nodeFs.readFileSync(stderr_path, 'utf8');
  ...
  /**
   * Read a persisted JSONL only when its original file still exists.
   *
   * @param {string} workspace
   * @param {string} attempt_id
   * @param {any} attempt
   ...
   */
  function persistedSessionRaw(workspace, attempt_id, attempt) {
    const log_path =
      typeof attempt.log_path === 'string'
        ? attempt.log_path
        : deps.sessionLog.pathFor?.(workspace, attempt_id, attempt.bead_id);
    if (
      !log_path ||
      !nodeFs.existsSync(log_path) ||
      typeof deps.sessionLog.read !== 'function'
    ) {
      return null;
    }
  ...
  /**
   * Apply the adapter's outage classifier to live or persisted session evidence.
   *
   * @param {string} workspace
   * @param {string} attempt_id
   * @param {unknown[]|null} live_raw
   * @param {boolean} require_log
   ...
   */
  async function providerOutageFor(
    workspace,
    attempt_id,
    live_raw,
    require_log
    ...
    const classifier = adapterSpec(attempt.runner).classifyProviderOutage;
    if (typeof classifier !== 'function') {
      return null;
    }
    /** @type {unknown[]} */
    let raw = Array.isArray(live_raw) ? live_raw : [];
    if (require_log) {
      const persisted = persistedSessionRaw(workspace, attempt_id, attempt);
      if (persisted === null) {
        return null;
      }
      raw = persisted;
    ...
    const outage = classifier({
      raw,
      stderr_tail: providerStderrTail(workspace, attempt_id, attempt),
      finished_at: now(),
      account_row: account.row
    });
    return outage ? { outage, account: account.account } : null;
  }
   ...
   *
   * @param {unknown[]} raw
   * @returns {{ cause: string, detail: { kind: 'session_hard_stop', message: string, summary: string } }|null}
   */
  function sessionHardStop(raw) {
    for (let index = raw.length - 1; index >= 0; index -= 1) {
      ...
      if (
        ...
        typeof event !== 'object' ||
        Array.isArray(event) ||
        /** @type {any} */ (event).type !== 'result' ||
        typeof (/** @type {any} */ (event).result) !== 'string'
      ) {
        continue;
      }
      const result_event = /** @type {{ result: string }} */ (event);
      const first = result_event.result
        .split(/\r?\n/)
        .map((/** @type {string} */ line) => line.trim())
        .find((/** @type {string} */ line) => line.length > 0);
      if (!first) {
        return null;
      }
      const line =
        first.startsWith('**') && first.endsWith('**')
          ? first.slice(2, -2).trim()
          : first;
      const suffix = line.startsWith('실패 ·')
        ? 'failure'
        : line.startsWith('환경 ·')
          ? 'environment'
          : null;
      ...
      return {
        cause: `session_hard_stop:${suffix}`,
        detail: {
          kind: 'session_hard_stop',
          message: line.slice(0, 512),
   ...
   * @param {string} bead_id
   * @param {string} reason
   * @param {{ blockers?: PrerequisiteBlocker[], gate?: { runner: string, kind: 'outage'|'usage_limit', account: string|null, unresolved: boolean } }} [extra]
   */
  ...
  /**
   * Derive automatic dispositions from a server-only observation.
   *
   * @param {WorktreeObservation} observation
   * @param {string} bead_id
   * @param {Attempt|null} resume_attempt
   * @returns {StaleResidue}
   */
  function describeStaleResidue(observation, bead_id, resume_attempt) {
    const owned =
    ...
    const state =
      owned &&
      observation.state !== 'unknown' &&
      (observation.state === 'unique' || resume_attempt !== null)
        ? 'unique'
        : 'unknown';
    const cause = !owned
      ? 'ownership_unknown'
  ...
  /**
   * Preserve observed identity while marking failed observation as unknown.
   *
   * @param {StaleResidue|null} stale_work
  ...
  /**
   * The remote tip of a bead's own branch, for the facts card's
   * `origin/<branch> tip=<sha>` line (spec D1). Fail-quiet: an unwired git
   * runner, a nonzero exit or an absent branch all yield null and omit the
   * line, because a continuation told the wrong tip would merge the wrong
   * thing.
   *
   * @param {string} repo
   * @param {string} remote
   ...
   * @returns {Promise<string|null>}
   */
  async function observeRemoteBranchTip(repo, remote, branch) {
    if (
      typeof deps.gitRun !== 'function' ||
      typeof repo !== 'string' ||
      repo.length === 0 ||
    ...
    try {
      ...
      const match = /([0-9a-f]{40})/.exec(result.stdout);
  ...
  /**
   * The workspace-global session defaults (`bd kv workflow_session_defaults`),
   * read fail-quiet for the facts card's `선택 입력` line (spec D1). It is the
   * SECOND layer the dotfiles selector reads after Bead metadata, so a card
   * that omitted it would send a session looking for the very value it already
   * has.
   *
   * @param {string} workspace
   * @returns {Promise<Record<string, unknown>|null>}
   */
  async function readSessionDefaultsLayer(workspace) {
    if (typeof deps.kvGet !== 'function') {
      return null;
    }
    try {
      const result = await deps.kvGet(workspace, 'workflow_session_defaults');
      return result?.ok === true && result.value ? result.value : null;
    } catch {
      return null;
    }
  }
      ...
      return await buildAttemptFacts(
        {
          ...
          bead_values: input.snap,
          kv_values: await readSessionDefaultsLayer(input.workspace)
        },
        {
          homeDir: deps.homeDir || os.homedir(),
          resolveDotfilesRoot: deps.resolveDotfilesRoot,
          loadDefaults: loadExecutionDefaults
        }
      );
    } catch (err) {
  ...
  /**
   * Take the bead's `in_progress` claim for an implementation dispatch
   * (harness-reduction spec D2).
   *
   * The workflow contract says a session claims the bead when it enters
   * implementation; the Worker's dispatch IS that entry, and 15 audited
   * sessions spent their first turns being refused by the impl-gate hook's
   * `status ≠ in_progress` check — one of them for nine turns on a
   * misdiagnosis. Writing it here removes the round trip without changing what
   * the check means.
   *
   * FAIL-QUIET AND NARROW. Only an exactly-`open` bead is claimed: any other
   * status belongs to somebody else's transition, and `resolved`→`in_progress`
   * would break the delivery lifecycle outright. A bd failure writes nothing
   * and lets the dispatch continue — the session then claims itself, exactly as
   * it does today. `session_ref` is NOT written: the contract forbids a Worker
   * runner from recording one.
   *
   * @param {string} workspace
   * @param {string} bead_id
   * @param {string} attempt_id
   * @returns {Promise<{ claimed: boolean, status: string|null }>} The observed
   * status, so the facts card can state it without a second `bd` read.
   */
  async function claimBeadForDispatch(workspace, bead_id, attempt_id) {
    /** @type {string|null} */
    let status = null;
    try {
      status = await deps.bd.readStatus(bead_id);
    } catch (err) {
      log('dispatch claim status read failed for %s: %o', bead_id, err);
    ...
    try {
      ...
      if (readback !== 'in_progress') {
        log(
          'dispatch claim readback mismatch for %s: expected in_progress, got %o',
          bead_id,
          readback
        );
        return { claimed: false, status: readback };
      }
      status = readback;
    } catch (err) {
   ...
   *
   * `attempt` is optional (spec D2) and only marks the WORKER's own claim
   * released. A caller that does not know which attempt held the claim keeps
   * the old one-argument shape, and `reconcile` settles whatever it left.
   *
   * @param {string} bead_id
   * @param {{ workspace: string, attempt_id: string }} [attempt]
   */
  async function releaseBeadClaim(bead_id, attempt) {
    try {
      ...
      markWorkerClaimReleased(attempt);
    } catch (err) {
  ...
  /**
   * Move a `written` worker claim to `released` on the attempt that took it
   * (spec D2). A record that never carried the Worker's own claim is left
   * alone: the field says who wrote the `in_progress`, and a session's own
   * claim is not this engine's to describe.
   *
   * @param {{ workspace: string, attempt_id: string }|null|undefined} attempt
   */
  function markWorkerClaimReleased(attempt) {
    if (!attempt) {
      return;
    }
    const record = deps.store.snapshot(attempt.workspace).attempts?.[
      attempt.attempt_id
    ];
    if (!record || record.worker_claim !== 'written') {
      return;
    }
    deps.store.updateAttempt(attempt.workspace, {
      attempt_id: attempt.attempt_id,
      patch: { worker_claim: 'released' }
    });
  }
    ...
    try {
      ...
      void Promise.resolve()
        ...
        .catch((err) => {
          ...
          return { session: 'not_launched', reason: 'error' };
        })
        .then((outcome) => {
          if (!recovery) {
            return;
        ...
        .catch((err) => {
          log('direction inquiry outcome failed for %s: %o', bead_id, err);
        });
    } catch (err) {
      log('direction inquiry trigger failed for %s: %o', bead_id, err);
    }
  ...
  async function failAttempt(
    ...
    if (
      ...
      !Array.isArray(cause_detail?.blockers)
    ) {
      try {
        if (!deps.bd.readIssue) {
          throw new Error('bd_read_unavailable');
        }
        const issue = await deps.bd.readIssue(bead_id);
        const blockers = (issue?.dependencies || [])
          .filter(
            (/** @type {any} */ edge) =>
              edge?.dependency_type === 'blocks' && typeof edge.id === 'string'
          )
          .map((/** @type {any} */ edge) => ({
            id: edge.id,
            rig: edge.external ? prefixOfBeadId(edge.id) : null,
  ...
  /**
   * Persist a provider pause and its dispatch gate as one durable mutation.
   *
   * @param {string} workspace
   * @param {string} attempt_id
   * @param {string} bead_id
   * @param {string|null} prior
   * @param {{ outage: { detail: string, message: string, scope: 'provider'|'account', resets_at: number|null }, account: string|null }} classified
   */
  async function holdAttempt(
    ...
    const saved = deps.store.holdProviderAttempt(workspace, {
      ...
      patch: {
        status: 'paused',
        cause: `provider_outage:${outage.detail}`,
        cause_detail: {
    ...
    try {
      ...
      log('workflow_mode revert failed for provider hold %s: %o', bead_id, err);
    }
    await revertExecStamps(
      bead_id,
      execStampedKeysOf(workspace, attempt_id),
    ...
    await releaseBeadClaim(bead_id, { workspace, attempt_id });
    const pending_switch = saved.queue?.auto_resume_pending?.some(
      (/** @type {any} */ entry) =>
        entry.attempt_id === attempt_id && entry.kind === 'account_switch'
    );
    if (pending_switch) {
  ...
  async function onSessionDone(
    ...
    try {
      ...
      if (!verdict.success) {
        ...
        if (
          failed_record?.continuation_mode === 'session' &&
          !verdict.blocked &&
          resumeTranscriptMissing(workspace, attempt_id, failed_record, verdict)
        ) {
          const attempts = deps.store.snapshot(workspace).attempts || {};
          const source = failed_record.resumed_from
            ? attempts[failed_record.resumed_from]
            : null;
          const source_session_id =
            source?.session_id || failed_record.session_id || '';
          const fallback_used = lineageHasResumeFallback(
            workspace,
            failed_record
          );
          ...
          if (
            ...
            await resume(workspace, attempt_id, {
              ...
              auto_resume_kind: failed_record.auto_resume_kind ?? undefined,
              account_switched_from:
                failed_record.account_switched_from ?? undefined,
              resume_fallback: {
  ...
  async function onDispositionDone(
    ...
    if (!verdict.success) {
      ...
      const transcript_missing =
        record.disposition_resume === true &&
        !verdict.blocked &&
        resumeTranscriptMissing(workspace, attempt_id, record, verdict);
      if (transcript_missing && !lineageHasResumeFallback(workspace, record)) {
   ...
   * present while the transcript itself is gone. The caller has already
   * matched the exact missing-transcript stderr before entering this path.
   *
  ...
  async function retryDispositionFresh(workspace, attempt_id, bead_id, record) {
    ...
    try {
      const attempts = deps.store.snapshot(workspace).attempts || {};
      const source = record.resumed_from ? attempts[record.resumed_from] : null;
      const source_session_id = source?.session_id || record.session_id || '';
      relaunched = await dispatchReviseFix(workspace, {
        ...
        resume: false,
        provider_auto_resume: true,
        auto_resume_kind: record.auto_resume_kind ?? undefined,
        account_switched_from: record.account_switched_from ?? undefined,
        resume_fallback: {
          reason: 'transcript_missing',
          session_id: source_session_id
        }
  ...
  async function disposeDeadAttemptSettlement(workspace, attempt_id, attempt) {
    ...
    try {
      ...
      /** @type {{ ok: boolean, reason: string, pr_url?: string|null, already_finished?: boolean, bead_status?: string|null, awaiting_user?: string|null }} */
      let vr;
      if (repo.length === 0) {
  ...
  /** Worker claims one reconcile pass may settle, so a cold start is bounded. */
   ...
   * (harness-reduction spec D2).
   *
   * The claim's ordinary release rides {@link releaseBeadClaim} on the
   * termination path. That path can be skipped exactly once: the process dies
   * between the claim write and the spawn, or `bd` is unreachable at the moment
   * the attempt ends. Either leaves a bead invisible to `bd ready` with nothing
   * on screen saying why, which is the failure mode the field exists to close.
   *
   * NARROW BY CONSTRUCTION — five conditions, all of them evidence rather than
   * inference: this attempt wrote the claim, it is terminal, no live sibling of
   * the same bead could still own it, the bead is STILL `in_progress`, and it
   * carries no `pr_url` (a delivered bead's status is the delivery's business,
   * not this pass's). A bead with no attempt record is never scanned — there
   * would be no ownership evidence to stand on.
   *
   * @param {string} workspace
   */
  async function settleStaleWorkerClaims(workspace) {
    const q = deps.store.snapshot(workspace);
    const attempts = Object.entries(q.attempts || {});
    /** @type {Set<string>} */
    ...
    let settled = 0;
    for (const [attempt_id, attempt] of attempts) {
      if (settled >= STALE_CLAIM_SETTLE_MAX) {
        return;
      }
      const a = /** @type {any} */ (attempt);
      ...
      settled += 1;
      try {
        if ((await deps.bd.readStatus(a.bead_id)) !== 'in_progress') {
          // Somebody already moved it; the record is simply stale.
          markWorkerClaimReleased({ workspace, attempt_id });
          continue;
        }
        if (typeof deps.bd.readMetadata === 'function') {
          const pr_url = await deps.bd.readMetadata(a.bead_id, 'pr_url');
          if (typeof pr_url === 'string' && pr_url.length > 0) {
            continue;
          }
        }
        await deps.bd.setStatus(a.bead_id, 'open');
        const readback = await deps.bd.readStatus(a.bead_id);
        if (readback !== 'open') {
          log(
            'stale worker claim release readback mismatch for %s: got %o',
            a.bead_id,
            readback
          );
          continue;
        ...
        log('stale worker claim release failed for %s: %o', a.bead_id, err);
      }
    }
  }
   ...
   * @param {{ stale_work?: StaleResidue, stale_rechecked?: boolean, retry_source?: any, retry?: { cause: string, attempts: number, max?: number, origin_attempt_id?: string|null } }} [options]
  ...
  async function dispatch(
    ...
    try {
      ...
      // Kept beside the resolution rather than re-derived: the facts card names
      // the base as `<remote>/<branch>@<sha>` (spec D1), and a local-only repo
      // resolves no remote at all, which must print as the bare branch instead
      // of an invented `origin/`.
      /** @type {string|null} */
      let base_remote = null;
      if (typeof deps.resolveBase === 'function') {
        ...
        base_remote = resolved.remote ?? null;
        snap = {
          ...snap,
      ...
      let wt = null;
      /**
       * Rejudge changed residue once while retaining the original dispatch claim.
       *
       * @param {WorktreeObservation|null} observation
       * @param {string} cause
       */
      async function recheckStale(observation, cause) {
        const actionable = observation?.identity
          ? observation
          : unknownStaleWorkObservation(stale_context, cut_base, cause);
        removeGuardHook(workspace, attempt_id);
        reservation?.release();
        if (options.stale_rechecked) {
          return failStaleResidue(
            workspace,
            bead_id,
            snap,
            describeStaleResidue(actionable, bead_id, null),
            cause
          );
        }
        return disposeStaleResidue(
          workspace,
          bead_id,
          actionable,
          null,
      ...
      if (stale_context) {
        ...
        if (typeof deps.worktree.removeIfDiscardable !== 'function') {
          await recheckStale(null, 'observer_unavailable');
          return;
        }
        ...
        try {
          ...
          await recheckStale(null, 'git_error');
          return;
        }
        if (
          ...
          await recheckStale(observed, 'worktree_identity_changed');
          return;
        }
        wt = {
      ...
      if (!wt) {
        if (stale_context) {
          await recheckStale(null, 'worktree_identity_changed');
        } else {
          removeGuardHook(workspace, attempt_id);
      ...
      // DEPENDENCY INSTALL for a FRESH worktree (harness-reduction spec D3).
      // `git worktree add` produces a checkout with no `node_modules`, and 31
      // of 73 audited sessions spent an `npm ls` → `npm ci` round trip
      // rediscovering that. A stale-work adoption is skipped: that worktree is
      // the previous attempt's, already installed, and re-running the installer
      // over a session's own tree is a change it did not ask for.
      //
      // NEVER a refusal. The three results are all answers, and a `failed:` one
      // leaves the session exactly where it is today — the facts card says so
      // and the session installs by itself.
      /** @type {string|null} */
      let worktree_setup = null;
      if (!stale_context) {
        if (typeof deps.worktree.installDependencies === 'function') {
          try {
            worktree_setup = await deps.worktree.installDependencies({
              path: wt.path
            });
          } catch (err) {
            log('worktree dependency install threw for %s: %o', bead_id, err);
            worktree_setup = 'skipped';
          }
        } else {
          worktree_setup = 'skipped';
        }
      ...
      if (
        !prerecordAttempt(workspace, {
          ...
          quickfix_lane,
          // The claim transition starts at `pending` (spec D2): the write is
          // attempted only AFTER this record lands, so the release duty exists
      ...
      // WORKER-SIDE CLAIM (harness-reduction spec D2). A stale-work
      // continuation keeps today's behaviour: its bead was claimed by the
      // attempt whose worktree this is.
      const claim = stale_context
        ? { claimed: false, status: snap.status ?? null }
        : await claimBeadForDispatch(workspace, bead_id, attempt_id);
      ...
      if (!stamped.ok) {
        ...
        // The Worker's own claim (spec D2) goes back with the mode: this path
        // never reaches `failAttempt`, and a claim left behind here would
        // hide the bead from `bd ready` until `reconcile` settled it. Only a
        // claim THIS attempt wrote is released — an `in_progress` the Worker
        // merely observed belongs to somebody else's transition.
        if (claim.claimed) {
          await releaseBeadClaim(bead_id, { workspace, attempt_id });
        }
        removeGuardHook(workspace, attempt_id);
      ...
      // The facts card (spec D1), collected AFTER the workflow_mode stamp so
      // its `workflow_mode=fast_track` line is a recorded fact rather than an
      // intention. `null` on any collection failure, which emits no block.
      const attempt_facts = await collectDispatchFacts({
        workspace,
        attempt_id,
        bead_id,
        snap,
   ...
   * @param {string} bead_id
   * @param {any} snap
   * @param {StaleResidue} residue
   * @param {string} reason
   */
  function failStaleResidue(workspace, bead_id, snap, residue, reason) {
    const attempt_id = makeAttemptId(bead_id);
    const lane = serialHeadOf(deps.store.snapshot(workspace), bead_id);
    const recorded = prerecordAttempt(workspace, {
      attempt_id,
      bead_id,
      repo: snap.repo,
      target_base: snap.target_base,
      base_oid: residue.identity.base_oid,
      head_oid: residue.identity.head_sha,
      serial_lane_id: lane.ok ? lane.lane_id : null,
      status: 'failed',
    ...
    if (recorded) {
      ...
      appendTimeline({
        ...
        attempt_id,
        kind: 'attempt_failed',
        seq: 'stale_work_unresolved',
        summary: `stale_work_unresolved · ${reason}`,
        at: now()
      });
    }
    claimed.delete(bead_id);
    ...
    return {
      ok: false,
      reason: recorded ? 'stale_work_unresolved' : 'attempt_prerecord_failed',
      attempt_id
    };
  }

  /**
   * Dispose of a verified residue under the dispatch claim. At most one new
   * observation is allowed; neither an unknown identity nor a backup error
   * can start an unbounded dispatch loop.
   *
   * @param {string} workspace
   * @param {string} bead_id
   * @param {WorktreeObservation} observation
  ...
  async function disposeStaleResidue(
    ...
    let residue = describeStaleResidue(observation, bead_id, resume_attempt);
    let failure = residue.cause;
    for (let pass = Number(rechecked); pass < 2; pass++) {
      ...
      if (!owner_reason && residue.can_continue) {
        ...
        failure = result?.reason || 'backup_unavailable';
        if (
          !failure.includes('identity_changed') &&
          failure !== 'local_ref_changed'
        ) {
          return failStaleResidue(workspace, bead_id, snap, residue, failure);
        }
      }
      if (
        pass === 1 ||
      ...
      try {
        // The owner check above must precede even discardable cleanup.
        observation = owner_reason
          ? await deps.worktree.removeIfDiscardable({
              repo: snap.repo,
              bead_id,
              base: cut_base,
              preserve: true
        ...
        failure = 'observe_failed';
        break;
      }
      if (observation.ok && !owner_reason) {
        recordStaleDisposition(bead_id, residue, 'continue');
        if (continue_dispatch) {
          try {
            await continue_dispatch(null);
      ...
      resume_attempt = matchingResidueAttempt(
        observation,
        bead_id,
        continue_dispatch
          ? []
          : resumableResidueAttempts(workspace, bead_id, snap.repo)
      );
      residue = describeStaleResidue(observation, bead_id, resume_attempt);
      failure = residue.cause;
    }
    return failStaleResidue(workspace, bead_id, snap, residue, failure);
  }
   ...
   * @param {string} bead_id
   * @param {StaleResidue} residue
   * @param {'resume'|'continue'|'backup_fresh'} action
   * @param {string|null} [backup_path]
   */
  function recordStaleDisposition(bead_id, residue, action, backup_path) {
    appendTimeline({
      bead_id,
      kind: 'stale_work_auto',
      seq: `${action}:${continuationDigest(residue.identity)}`,
      summary: `잔재 자동 처분 · ${action} · ${residue.cause}`,
   ...
   *   attempt_facts?: import('./attempt-facts.js').AttemptFacts|null
   * }} input
   * `attempt_facts` is the dispatch-time facts card (spec D1). Only the
   * ordinary dispatch collects one; every relaunch path leaves it absent, and
   * the preamble then emits no `## 시도 사실` block rather than a card of `없음`.
  ...
  async function launchSession(input) {
    ...
    const settings = {
      ...
      // The facts card rides the SAME settings object every other
      // session-constant does, so both adapters read one field and neither can
      // deliver a card the other does not (spec D1).
      attempt_facts: input.attempt_facts ?? null
    };
   ...
   *
   * `facts` adds what the Worker ALREADY knows about the ancestor
   * (harness-reduction spec D6): its id and cause, its two receipts, and the
   * one git sequence that answers "did the prior attempt push?". Without them
   * "직접 점검하라" sends a fresh session hunting for facts the queue holds, and
   * the audited resumes spent their first turns doing exactly that. Absent
  ...
  /**
   * Fill {@link resumePrompt}'s ancestor sentence from the prior attempt record
   * plus the bead's two receipt keys (spec D6). A bd failure yields `없음` for
  ...
  async function resumeAncestorFacts(workspace, prior, bead_id) {
    ...
    /**
     * One metadata key, or null when bd cannot answer.
     *
     * @param {string} key
     * @returns {Promise<string|null>}
     */
    ...
    if (typeof deps.resolveBase === 'function') {
      try {
        const resolved = await deps.resolveBase();
        remote = resolved.ok ? resolved.remote : null;
      } catch {
        remote = null;
      }
    }
  ...
  async function launchExternalWaitSession(
    ...
    /** @type {{ path: string, base_oid: string|null, head_oid: string|null }} */
    const prepared_worktree = {
      path: record.worktree,
      base_oid: null,
      head_oid: null
    };
    /** @param {StaleResidue|null} residue */
    async function prepareFreshWorktree(residue) {
      if (residue) {
        if (!deps.worktree.removeIfDiscardable) {
          throw new Error('observer_unavailable');
        }
        const observed = await deps.worktree.removeIfDiscardable({
          repo,
          bead_id,
          base: residue.identity.base_oid || target_base,
          preserve: true
        });
        if (!staleResidueIntact(residue.identity, observed)) {
          throw new Error('worktree_identity_changed');
        }
        Object.assign(prepared_worktree, {
          path: /** @type {string} */ (residue.identity.worktree_realpath),
          base_oid: residue.identity.base_oid || target_base,
          head_oid: residue.identity.head_sha
        });
      } else {
        Object.assign(
          prepared_worktree,
          await deps.worktree.add({
            repo,
            bead_id,
            base: snap.base_oid || target_base
          })
        );
      }
    }
    try {
      if (mode === 'fresh' && !fs.existsSync(record.worktree)) {
        ...
        if (typeof deps.worktree.removeIfDiscardable === 'function') {
          ...
          try {
            ...
            residue = unknownStaleWorkObservation(null, base, 'observe_failed');
          }
          if (!residue.ok) {
            const disposed = await disposeStaleResidue(
              workspace,
              bead_id,
              residue,
              null,
            ...
            await prepareFreshWorktree(null);
          }
        } else {
          await prepareFreshWorktree(null);
        }
      ...
      if (
        !prerecordAttempt(workspace, {
          ...
          repo,
          target_base,
          base_oid: prepared_worktree.base_oid,
          head_oid: prepared_worktree.head_oid,
          runner: runner_name,
   ...
   * worktree was removed. An unresolved residue retries automatic disposition
   * without requiring a session or worktree. Fail-closed with five refusal
  ...
  async function resume(workspace, attempt_id, continuation = {}) {
    ...
    const wt_present =
      typeof deps.worktree.exists === 'function'
        ...
        : true;
    const stale_residue_retry = prior.cause === 'stale_work_unresolved';
    if (!quickfix_cleanup_resume && !stale_residue_retry && !wt_present) {
      return { ok: false, reason: 'worktree_missing' };
    }
    ...
    if (stale_residue_retry) {
      claimed.add(bead_id);
      try {
        await dispatch(workspace, bead_id);
        const current = deps.store.snapshot(workspace);
        const child = Object.values(current.attempts).find(
          (attempt) =>
            attempt.bead_id === bead_id && !q.attempts[attempt.attempt_id]
        );
        if (child) {
          deps.store.updateAttempt(workspace, {
            attempt_id: child.attempt_id,
            patch: { resumed_from: attempt_id }
          });
          notifyChanged(workspace);
        }
        return child?.status === 'running'
          ? { ok: true, attempt_id: child.attempt_id }
          : {
              ok: false,
              reason:
                child?.cause ||
                current.admission[bead_id]?.reason ||
                'dispatch_refused'
            };
      } finally {
        claimed.delete(bead_id);
    ...
    if (result.ok) {
      if (
        ...
        const cleared_target = deps.store.clearManualProviderResumeTarget(
          workspace,
          { runner: prior.runner, attempt_id }
        );
        if (cleared_target?.ok) {
          notifyChanged(workspace);
        }
      }
   ...
   *
   * @param {string} workspace
   * @returns {Promise<{ resumed_beads: string[], refusals: string[] }>}
   */
  async function consumeProviderAutoResume(workspace) {
  ...
  /**
   * Validate and atomically overlay one launch-only execution override.
   *
   * @param {any} resolved
   * @param {unknown} raw_override
   */
  async function applyExecutionOverride(resolved, raw_override) {
    if (raw_override === undefined) {
      return { ok: true, resolved, applied: false };
    }
    if (
      !raw_override ||
      typeof raw_override !== 'object' ||
      Array.isArray(raw_override)
    ) {
    ...
    const runner =
      typeof override.runner === 'string'
        ? override.runner.trim()
        : resolved.exec.runner;
    const model =
      typeof override.model === 'string'
        ? override.model.trim()
        : resolved.exec.orchestration_model;
    const effort =
      typeof override.effort === 'string'
        ? override.effort.trim()
        : resolved.exec.orchestration_effort;
    ...
    if (typeof override.claude_account === 'string') {
      const claude_account = override.claude_account.trim();
      if (
        runner !== 'claude' ||
        !deps.accountCatalog ||
        typeof deps.accountCatalog.resolveClaude !== 'function'
      ...
      let account;
      try {
        account = await deps.accountCatalog.resolveClaude(claude_account);
      } catch {
        return { ok: false, reason: 'exec_override_invalid' };
      }
      if (!account?.ok || account.account?.email !== claude_account) {
        return { ok: false, reason: 'exec_override_invalid' };
      }
    ...
    return {
      ok: true,
      applied: Object.keys(override).length > 0,
      resolved: {
        ...resolved,
        exec: {
          ...resolved.exec,
  ...
  async function resolveContinuationForAttempt(workspace, prior, options = {}) {
    ...
    const override_cross_runner = override_result.applied && runner_mismatch;
    const explicit_fresh_current =
      !runner_mismatch && requested_decision === 'fresh_current';
    ...
    if (
      ...
      (!prior_exec_complete ||
        !prior_runner_available ||
        (options.resume === false && !internal_fresh_fallback) ||
        (decision !== 'fresh_current' &&
          options.resume !== false &&
          (typeof prior.session_id !== 'string' ||
            prior.session_id.length === 0)))
    ) {
    ...
    let continuation_mode =
      decision === 'fresh_current' || options.resume === false
        ? 'fresh'
        : 'session';
    /** @type {{ reason: 'transcript_missing', session_id: string }|null} */
    let resume_fallback =
      options.resume_fallback?.reason === 'transcript_missing' &&
      typeof options.resume_fallback.session_id === 'string'
        ? options.resume_fallback
        : null;
    if (
    ...
    const needs_handoff =
      (runner_mismatch && !use_prior && continuation_mode === 'fresh') ||
      resume_fallback !== null ||
      (explicit_fresh_current &&
        typeof prior.cause === 'string' &&
        prior.cause.startsWith('provider_outage:'));
    return {
      ok: true,
      bead_snapshot,
      decision,
      ...
      continuation_mode,
      resume_fallback,
      handoff_instructions: needs_handoff
        ? resumeHandoffBlock(workspace, prior)
        : null
    };
  }
  ...
  async function relaunchResolvedAttempt(
    ...
    const relaunch_attempt = {
      ...
      effort: launch_effort,
      speed: launch_speed,
      claude_account: accounts.claude,
      codex_account: accounts.codex,
      account_sources,
      account_switched_from,
      resume_fallback,
      workflow_mode_prior: prior_wf,
    ...
    return {
      ok: true,
      attempt_id: new_attempt_id,
      fallback: resume_fallback?.reason ?? null
    };
  }
  ...
  /**
   * Whether an attempt left work a fresh dispatch would strand (spec D5b).
   *
   * Three independent kinds of evidence, in ascending cost: the recorded head
   * already differs from the base, the worktree observation still reports
   * commits ahead, or the bead's branch exists on the remote. Any one of them
   * means the next attempt has to CONTINUE rather than start over.
   *
   * FAIL-QUIET TO FALSE. An unobservable worktree yields the ladder's existing
   * fresh dispatch, which is where every lineage was before this rung existed;
   * a false positive would resume an attempt with nothing to resume.
   *
   * @param {any} attempt - The lineage's latest implementation attempt.
   * @returns {Promise<boolean>}
   */
  async function attemptHasPreservedWork(attempt) {
    if (
      !attempt ||
      typeof attempt.session_id !== 'string' ||
      attempt.session_id.length === 0
    ) {
      // No recorded session ⇒ nothing to continue INTO, whatever the worktree
      // holds. That case is the fresh dispatch's, and refusing it here is what
      // keeps `resume`'s `no_session_id` off the ladder.
      return false;
    }
    const repo = typeof attempt.repo === 'string' ? attempt.repo : '';
    const bead_id = typeof attempt.bead_id === 'string' ? attempt.bead_id : '';
    if (repo.length === 0 || bead_id.length === 0) {
      return false;
    }
    const base =
    ...
    if (
      ...
      try {
        // `preserve: true` makes this an OBSERVATION: the manager returns the
        // snapshot without removing the worktree or archiving its branch.
        const observed = await deps.worktree.removeIfDiscardable({
          repo,
          bead_id,
          base,
        ...
        if (
          observed?.summary &&
          (observed.summary.head_ahead > 0 || observed.summary.branch_ahead > 0)
        ) {
          return true;
        }
      } catch {
        // Unobservable is not evidence; fall through to the remote probe.
      }
    }
    return (
      (await observeRemoteBranchTip(repo, 'origin', branchForBead(bead_id))) !==
      null
    );
  }
