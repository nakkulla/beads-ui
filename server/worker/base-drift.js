 ...
 * @property {'pr_merge'|'direct_push'} [via] - How the landing happened: an
 * observed MERGED pull request, or a push that no merged PR accounts for (the
 * violation).
 * @property {string[]} [shas] - The intersection: the attempt commits found on
 * the base. This is the evidence a violation stands on.
 ...
 * (`base_resolve:<step>` / `rev_list_branch` / `rev_list_base` /
 * `pr_observe:<reason>` / `no_repo` / `no_observer_deps`).
 */
 ...
 *   gh?: { mergedPrForBranch: (repo_dir: string, branch: string) => Promise<{ state: string, data?: unknown, reason?: string }> }
 * }} input
...
export async function observeBaseDrift(input) {
  ...
  return {
    violation: true,
    record: { pinned, observed, landed: true, via: 'direct_push', shas }
  };
}
