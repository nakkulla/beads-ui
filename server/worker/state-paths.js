...
/**
 * Absolute path to the SERVER-GLOBAL parallel-analysis settings file
 * (UI-04vo §7): `{ revision, runner, model, effort }` under CAS. Global like
 * the exec presets — the analyzer model choice is not workspace state.
 *
 * @returns {string} `$XDG_STATE_HOME/bdui/parallel-analysis-settings.json`.
 */
export function parallelAnalysisSettingsPath() {
  return path.join(stateHome(), 'bdui', 'parallel-analysis-settings.json');
}

/**
 * Absolute path to a workspace's parallel-analysis cache (last-good result +
 * identity digest, UI-04vo §9).
 *
 * @param {string} workspace_root - Workspace root (relative or absolute).
 * @returns {string} `$XDG_STATE_HOME/bdui/<slug>/parallel-analysis.json`.
 */
