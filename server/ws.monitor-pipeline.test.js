...
describe('monitor pipeline external waits', () => {
  test('counts live records and reuses projected wait verdicts', () => {
    const root_dir = '/tmp/mon-external';
    const rows = ['hold', 'detached', 'completing', 'stopped'].map(
      (stage, i) => ({
        root_dir,
        bead_id: `A-${i}`,
        wait_id: `w-${i}`,
    ...
    expect(workspaces[0].external_waits).toEqual(rows);
