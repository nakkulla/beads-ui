...
Please report this to https://github.com/markedjs/marked.`,e){let n="<p>An error occurred:</p><pre>"+pr(r.message+"",!0)+"</pre>";return t?Promise.resolve(n):n}if(t)return Promise.reject(r);throw r}}},Pr=new Np;function Qe(e,t){return Pr.parse(e,t)}Qe.options=Qe.setOptions=function(e){return Pr.setOptions(e),Qe.defaults=Pr.defaults,sl(Qe.defaults),Qe};Qe.getDefaults=Ao;Qe.defaults=Dr;Qe.use=function(...e){return Pr.use(...e),Qe.defaults=Pr.defaults,sl(Qe.defaults),Qe};Qe.walkTokens=function(e,t){return Pr.walkTokens(e,t)};Qe.parseInline=Pr.parseInline;Qe.Parser=Yt;Qe.parser=Yt.parse;Qe.Renderer=us;Qe.TextRenderer=Po;Qe.Lexer=Vt;Qe.lexer=Vt.lex;Qe.Tokenizer=ds;Qe.Hooks=gn;Qe.parse=Qe;var Gg=Qe.options,Vg=Qe.setOptions,Yg=Qe.use,Kg=Qe.walkTokens,Zg=Qe.parseInline;var Xg=Yt.parse,Qg=Vt.lex;function wr(e){let t=Qe.parse(e),r=Zi.sanitize(t);return Xi(r)}function fr(e,t){return i`<div class="prompt-block">
    ...
    <pre class="prompt-block__body">${t}</pre>
      ...
      </div>`}if(B.kind==="error")return i`<div class="sv__error">⛔ ${B.text}</div>`;if(B.kind==="blocker")return i`<div class="sv__error">⛔ ${B.text}</div>`;if(B.kind==="tool"){let T=f.has(U),z=B.tool==="Bash"?Jp(B.command):0,R=B.tool==="Bash"?z>1?ms(B.command):B.command:B.path||B.command||"";return i`<div
...
`)}function Ze(){if(!o)return i``;let U=H(),B=(a?[c.model]:[c.runner,c.model,c.effort]).filter(Boolean).join(" \xB7 "),T=c.session_id||"",z=`\uB77C\uC774\uBE0C \uB530\uB77C\uAC00\uAE30 ${u?"ON":"OFF"}`,R=ge(),K=R?of(Y(),Date.now()):"",Z=R?De(U):null,ce=R?Ue(U):null,_e=sf(U);return i`<div class="sv" data-attempt-id=${o}>
      <div class="sv__bar">
...
${fe.notes_tail}`:"notes\uC758 REVISE finding\uC744 \uC2A4\uD399\uC5D0 \uBC18\uC601\uD558\uB294 \uCC98\uBD84 \uC138\uC158\uC744 \uB744\uC6C1\uB2C8\uB2E4":""};f.push(B);let T=b.get(j);T?T.push(B):b.set(j,[B])}for(let X of Array.isArray(A.runnable)?A.runnable:[]){let G=X&&X.bead_id;typeof G!="string"||me.has(G)||(me.add(G),l.push({...Re(G),title:X.title||H[G]||G,lane:"runnable",draggable:!0,reason:uc(ge,G),created_at:X.created_at??void 0,updated_at:X.updated_at??void 0,labels:Array.isArray(X.labels)?X.labels:[],spec_reviewer:typeof X.spec_reviewer=="string"?X.spec_reviewer:void 0,plan_state:X.plan_state==="approved"||X.plan_state==="authored"?X.plan_state:"none",workflow:X.route?{route:X.route,chips:{route:X.route}}:null,place_index:we.length}))}for(let X of ve){let G=X&&X.bead_id;if(typeof G!="string"||me.has(G)||(me.add(G),o!==void 0&&typeof X.added_at=="number"&&X.added_at<o))continue;let ke=a_(S,G);m.push({...Re(G),lane:"done",done:!0,usage:Ft(S,G),done_at:typeof X.added_at=="number"?X.added_at:void 0,done_kind:ke&&typeof ke.done_kind=="string"?ke.done_kind:null})}}let E=new Map;s.forEach((A,j)=>{A&&typeof A.root_dir=="string"&&E.set(A.root_dir,j)});let k=r&&r.running_sort==="repo"?"repo":"started";c.sort((A,j)=>{if(k==="repo"){let w=E.get(A.root_dir)??Number.MAX_SAFE_INTEGER,S=E.get(j.root_dir)??Number.MAX_SAFE_INTEGER;if(w!==S)return w-S}let ee=typeof A.started_at=="number"&&Number.isFinite(A.started_at)?A.started_at:null,x=typeof j.started_at=="number"&&Number.isFinite(j.started_at)?j.started_at:null;return ee!==null&&x!==null&&ee!==x?ee-x:ee===null&&x!==null?1:ee!==null&&x===null?-1:A.id.localeCompare(j.id)}),m.sort((A,j)=>(j.done_at??0)-(A.done_at??0));let N=s.length>0?s:n.map(A=>({root_dir:A&&A.root_dir,name:A&&A.name,auto_advance:A&&A.auto_advance,auto_merge:A&&A.auto_merge,slots:A&&A.slots,revision:A&&A.revision,runner_catalog:A&&A.runner_catalog})),D=[];for(let A of N)!A||typeof A.root_dir!="string"||D.push({root_dir:A.root_dir,name:A.name||A.root_dir,auto_advance:A.auto_advance===!0,auto_merge:A.auto_merge===!0,slots:typeof A.slots=="number"&&A.slots>=An?A.slots:An,revision:typeof A.revision=="number"?A.revision:0,runner_catalog:Rt(A.runner_catalog),items:b.get(A.root_dir)||[]});return{runnable:l,queue:f,queue_groups:D,running:c,pr_wait:u,done:m,automation:{total:D.length,both_on:D.filter(A=>A.auto_advance&&A.auto_merge).length}}}function l_(e,t){if(typeof e!="number"||!Number.isFinite(e))return"";let n=t-e<n_;return i`<span
  ...
  >
    ${pc(e,t)}
  </div>`}function wc(e,t){let r=at("views:monitor"),n=t.gotoIssue,s=t.pipelineStore,o=t.transport,a=t.getWorkspacePath,l=t.switchWorkspace,c=t.now||(()=>Date.now()),u=t.confirm||(T=>typeof globalThis.confirm!="function"||globalThis.confirm(T)),f=m_(),m=b_();function b(){let T=tr.find(z=>z.value===f);return T?T.label:""}let E=document.createElement("div");E.className="mon",e.appendChild(E);let k=Jo(null,null),N=new Map,D=null,A=null;async function j(T,z,R,K,Z=!0){if(!o||!R)return null;let ce=await o(T,{...z,root_dir:R,expected_revision:K});if(ce&&ce.conflict&&Z){ce.queue&&N.set(R,ce.queue);let _e=ce.queue&&typeof ce.queue.revision=="number"?ce.queue.revision:K;ce=await o(T,{...z,root_dir:R,expected_revision:_e})}return ce&&ce.queue&&R&&N.set(R,ce.queue),ce}function ee(T,z){let R=N.get(T),K=s&&s.get?s.get():null,Z=(Array.isArray(K)?K:[]).find(_e=>_e?.root_dir===T);return(R||Z)?.merge_queue?.find(_e=>_e.bead_id===z)?.continuation_action}async function x(T,z,R,K){let Z=await j(T,z,R,K),ce=N.get(R)?.revision??Z?.queue?.revision??K;return lr(Z,(_e,$e)=>j(T,{...z,continuation:_e,decision_token:$e},R,ce,!1),{refresh:_e=>j(T,z,R,_e?.queue?.revision??N.get(R)?.revision??ce,!1)})}async function w(T,z,R,K){let Z=await lr({continuation_mismatch:K},(_e,$e)=>j("worker-merge-queue-add",{bead_id:z,continuation:_e,decision_token:$e},T,R,!1)),ce=Z?.queue?.merge_queue?.find(_e=>_e.bead_id===z)?.continuation_action;Z?.applied!==!0&&ce?.continuation===null&&ce.mismatch&&await w(T,z,Z.queue.revision,ce.mismatch)}async function S(T,z,R){let K=await j("worker-discard",T,z,R);if(K&&K.discarded===!0){ie(Ls(K),"success",5e3);return}if(K&&K.reason){ie(`\uD3D0\uAE30 \uC2E4\uD328: ${K.reason}`,"error");return}if(K&&K.accepted&&K.pending==="merged_revert"){ie("revert PR \uB300\uAE30 \uC0C1\uD0DC\uB85C \uC804\uD658\uD588\uC2B5\uB2C8\uB2E4","success");return}if(K&&K.accepted){ie(`\uD3D0\uAE30 \uC9C4\uD589: ${K.phase||"\uBC31\uC5C5 \uC911"}`,"success");return}K&&!K.conflict&&ie("\uD3D0\uAE30 \uAC70\uBD80: unknown","error")}async function H(T,z,R){return!o||!R?null:await o(T,{...z,root_dir:R})}async function Y(T){if(!o||!T&&!u("\uC804 \uB808\uD3EC\uC758 \uC790\uB3D9 \uC9C4\uD589\xB7\uC790\uB3D9 \uBA38\uC9C0\uB97C \uB055\uB2C8\uB2E4. \uAC01 \uB808\uD3EC\uC758 \uBA38\uC9C0 \uB300\uAE30\uC5F4\uB3C4 \uD568\uAED8 \uBE44\uC6CC\uC9D1\uB2C8\uB2E4. \uACC4\uC18D\uD560\uAE4C\uC694?"))return;let z=await o("monitor-auto-toggle",{on:T}),R=z&&Array.isArray(z.failed)?z.failed:[];R.length>0&&ie(`\uC790\uB3D9\uD654 ${T?"\uCF1C\uAE30":"\uB044\uAE30"} \uC77C\uBD80 \uC2E4\uD328: ${R.map(K=>K.root_dir).join(", ")}`,"error",3200)}async function ge(){let T=new Map;for(let z of k.pr_wait)T.has(z.root_dir)||T.set(z.root_dir,z.expected_revision);for(let[z,R]of T)await j("worker-merge-queue-add-all",{},z,R)}let le=null,ne=!1,de=null;function De(){de!==null&&clearTimeout(de),de=setTimeout(()=>{de=null,ne=!1},0)}function Ue(T){let z=T.target;return typeof z?.closest=="function"?z.closest(".mon-group"):null}function Ge(T){let z=Ue(T);return!z||!le?null:(z.getAttribute("data-root-dir")||"")===le.root_dir?z:null}function ze(){for(let T of Array.from(E.querySelectorAll(".mon-group--drag-over")))T.classList.remove("mon-group--drag-over")}function Ze(T){let z=T.target,R=typeof z?.closest=="function"?z.closest('.mon-card[draggable="true"]'):null;if(R){le={bead_id:R.getAttribute("data-issue-id")||"",lane:R.getAttribute("data-lane")||"",root_dir:R.getAttribute("data-root-dir")||"",revision:Number(R.getAttribute("data-revision")||0)||0,queue_index:Number(R.getAttribute("data-queue-index")),queue_length:Number(R.getAttribute("data-queue-length")),place_index:Number(R.getAttribute("data-place-index"))},ne=!0;try{T.dataTransfer?.setData("text/plain",le.bead_id),T.dataTransfer&&(T.dataTransfer.effectAllowed="move")}catch{}}}function Fe(T){let z=Ge(T);z&&(T.preventDefault(),T.dataTransfer&&(T.dataTransfer.dropEffect="move"),z.classList.add("mon-group--drag-over"))}function we(T){Ue(T)?.classList.remove("mon-group--drag-over")}function ve(){le=null,ze(),De()}function xe(T){let z=Ge(T),R=le;if(le=null,ze(),!z||!R||!R.bead_id)return;T.preventDefault();let K=T.target,Z=typeof K?.closest=="function"?K.closest('.mon-card[data-lane="queue"]'):null,ce=Z&&z.contains(Z)?Number(Z.getAttribute("data-queue-index")):NaN;if(R.lane==="runnable"){let C=Number.isFinite(ce)?ce:R.place_index;if(!Number.isFinite(C))return;j("worker-queue-place",{bead_id:R.bead_id,index:C},R.root_dir,R.revision);return}if(R.lane!=="queue"||Z&&Z.getAttribute("data-issue-id")===R.bead_id)return;let _e=R.queue_index,$e=Number.isFinite(ce)?_e>ce?ce:ce-1:R.queue_length-1;!Number.isFinite($e)||$e<0||$e===_e||j("worker-queue-reorder",{bead_id:R.bead_id,to_index:$e},R.root_dir,R.revision)}function Re(T){let z={runnable:k.runnable,queue:k.queue,running:k.running,pr_wait:k.pr_wait,done:k.done};return i`${_c({automation:k.automation,counts:{running:k.running.length,queue:k.queue.length,pr_wait:k.pr_wait.length},running_sort:m,done_range:f,token_total:gc(k.done),token_tooltip:mc(b())})}
        ...
        ${v_.map(R=>{let K=z[R.lane],Z=R.lane==="queue"?k.queue_groups.length>0?i`${k.queue_groups.map(ce=>i`<div
                      ...
                      >
                        ...
                        <div class="mon-group__list">
                          ${ce.items.map(_e=>bc(_e,T))}
                        </div>
                      </div>`)}`:void 0:K.length>0?i`${K.map(ce=>bc(ce,T))}`:void 0;return Kt({id:`monitor-${R.lane}`,lane:R.pane,title:R.lane==="done"?`\uC644\uB8CC\xB7${b()}`:R.title,items:K,empty:R.empty,body:Z,live:R.lane==="running"&&K.length>0,header_control:R.lane==="pr_wait"&&K.length>0?i`<button
                    type="button"
  ...
  `}var $_=[{id:"session",label:"\uC138\uC158",glyph:"\u25C6"},{id:"worker",label:"Worker",glyph:"\u25A4"},{id:"display",label:"\uD45C\uC2DC",glyph:"\u25EB"}],Zt="";function Xt(e){return!!e&&typeof e=="object"&&!Array.isArray(e)}function Rc(e,t){let{transport:r,policyStore:n,labelOptions:s}=t,o=t.notify||($=>ie($,"error",4e3)),a=document.createElement("dialog");a.id="settings-dialog",a.className="settings-dialog",a.setAttribute("role","dialog"),a.setAttribute("aria-modal","true"),a.setAttribute("aria-label","\uC124\uC815"),e.appendChild(a);let l="session",c=!1,u="",f={},m={},b=[],E=!1,k=null,N={},D="",A="",j=!1,ee=!1,x=!1,w=null;function S(){let $=t.queueStore?.get();return Xt($)?$.runner_catalog:null}function H(){let $=t.implPresetStore?.get();return Xt($)&&Array.isArray($.presets)?$:null}async function Y(){E=!0,Z();try{let $=await r("get-session-defaults",{});f=Xt($?.values)?{...$.values}:{},m={...f},b=Array.isArray($?.warnings)?$.warnings:[]}catch($){b=["kv_read_failed"],o(`\uC138\uC158 \uAE30\uBCF8\uAC12\uC744 \uC77D\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4: ${$ instanceof Error?$.message:String($)}`)}finally{E=!1,Z()}}async function ge(){let $=El(f,m);if(Object.keys($).length!==0){try{let M=await r("set-session-defaults",{values:$});f=Xt(M?.values)?{...M.values}:{},m={...f},b=Array.isArray(M?.warnings)?M.warnings:[]}catch(M){o(`\uC138\uC158 \uAE30\uBCF8\uAC12 \uC800\uC7A5 \uC2E4\uD328: ${M instanceof Error?M.message:String(M)}`)}Z()}}function le($,M){M===Zt?delete m[$]:m[$]=M,Z(),ge()}async function ne(){let $=t.queueStore?.get();if(!Xt($))return;let M={orchestration_model:$.orchestration_model??null,orchestration_effort:$.orchestration_effort??null,orchestration_speed:$.orchestration_speed??null},V=Tl(M,{...M,...N});if(Object.keys(V).length!==0){try{let Ie=await r("worker-queue-set-orchestration-defaults",{expected_revision:$.revision,values:V});if(Ie&&Ie.applied===!1){o("Worker \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uD074\uB77C\uC774\uC5B8\uD2B8\uC640 \uCDA9\uB3CC");return}N={}}catch(Ie){o(`Worker \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: ${Ie instanceof Error?Ie.message:String(Ie)}`)}Z()}}function de($,M){N[$]=M===Zt?null:M,Z(),ne()}async function De($){let M=t.queueStore?.get();if(!(!Xt(M)||$<1)){try{await r("worker-queue-set-slots",{expected_revision:M.revision,slots:$})}catch(V){o(`slots \uC800\uC7A5 \uC2E4\uD328: ${V instanceof Error?V.message:String(V)}`)}Z()}}function Ue(){let $={};for(let M of xl){let V=m[M];typeof V=="string"&&V.length>0&&($[M]=V)}return $}async function Ge(){let $=H();if(!$)return;let M=Ue();if(Object.keys(M).length===0){o("\uC800\uC7A5\uD560 \uAD6C\uD604 \uAC12\uC774 \uC5C6\uC2B5\uB2C8\uB2E4 \u2014 \uBA3C\uC800 \uAD6C\uD604 \uADF8\uB8F9\uC744 \uC120\uD0DD\uD558\uC138\uC694");return}let V=($.presets||[]).find(Ce=>Ce.id===D),Ie=A.trim()||(V?V.name:"");if(!Ie){o("\uD504\uB9AC\uC14B \uC774\uB984\uC744 \uC785\uB825\uD558\uC138\uC694");return}try{let Ce=V?await r("impl-preset-update",{expected_revision:$.revision,id:V.id,name:Ie,settings:M}):await r("impl-preset-create",{expected_revision:$.revision,name:Ie,settings:M});if(Ce&&Ce.applied){if(A="",!V&&Array.isArray(Ce.presets)){let Me=Ce.presets.find(Te=>Te.name===Ie);D=Me?Me.id:D}Z()}else o("\uD504\uB9AC\uC14B \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uACF3\uC5D0\uC11C \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4"),Z()}catch(Ce){o(`\uD504\uB9AC\uC14B \uC800\uC7A5 \uC2E4\uD328: ${Ce instanceof Error?Ce.message:String(Ce)}`)}}async function ze(){let $=H();if(!(!$||D.length===0))try{let M=await r("impl-preset-delete",{expected_revision:$.revision,id:D});M&&M.applied?(D="",Z()):(o("\uD504\uB9AC\uC14B \uC0AD\uC81C \uC2E4\uD328: \uB2E4\uB978 \uACF3\uC5D0\uC11C \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4"),Z())}catch(M){o(`\uD504\uB9AC\uC14B \uC0AD\uC81C \uC2E4\uD328: ${M instanceof Error?M.message:String(M)}`)}}async function Ze(){let $=H();if(!(!$||D.length===0)){try{let M=await r("apply-impl-preset-global",{preset_id:D,expected_revision:$.revision});M&&M.applied?(f=Xt(M.values)?{...M.values}:{},m={...f},b=Array.isArray(M.warnings)?M.warnings:[]):M&&M.conflict&&o("\uAD6C\uD604 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328: \uD504\uB9AC\uC14B\uC774 \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4")}catch(M){o(`\uAD6C\uD604 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328: ${M instanceof Error?M.message:String(M)}`)}Z()}}async function Fe(){ee=!0,x=!1,Z();try{let $=await r("get-worker-system-prompt",{});!$||typeof $!="object"||Array.isArray($)?x=!0:w=$}catch{x=!0}finally{ee=!1,Z()}}function we(){if(j=!j,j&&!w){Fe();return}Z()}function ve(){let $=Kr({loading:ee,error:x});if($)return $;if(!w)return"";let M=Array.isArray(w.variants)?w.variants:[];return i`<div class="settings-dialog__sp-body">
    ...
    `}async function oe($){let M=n.get();if(M)try{let V=await r("display-policy-set",{expected_revision:M.revision,policy:$(M)});U(V),V&&V.conflict&&V.policy&&(V=await r("display-policy-set",{expected_revision:V.policy.revision,policy:$(V.policy)}),U(V)),V&&V.conflict&&o("\uD45C\uC2DC \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uD074\uB77C\uC774\uC5B8\uD2B8\uC640 \uCDA9\uB3CC")}catch{o("\uD45C\uC2DC \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328")}}function U($){$&&$.policy&&typeof $.policy=="object"&&n.set($.policy)}function B($){oe($)}function T($){let M=n.get();if(!M)return;let V=!x_($,M);B(Ie=>S_($,Ie,V))}function z(){let $=u.trim();$.length!==0&&(u="",B(M=>M.hidden_prefixes.includes($)?{hidden_prefixes:M.hidden_prefixes}:{hidden_prefixes:[...M.hidden_prefixes,$]}),Z())}function R($){B(M=>({hidden_prefixes:M.hidden_prefixes.filter(V=>V!==$)}))}function K($){let M=n.get();if(!M)return;let V=M.chips[$]===!1;B(()=>({chips:{[$]:V}}))}function Z(){je(i`
      ...
      </div>`,e),e.hidden=!1}async function a(c){try{let u=await fetch(c.endpoint);if(!u.ok)return null;let f=await u.json();return!f||f.available!==!0||!Array.isArray(f.windows)?null:f}catch{return null}}async function l(){let c=await Promise.all(Lc.map(async u=>({provider:u,payload:await a(u)})));if(!t){for(let u of c)u.payload?n.set(u.provider.key,u.payload):n.delete(u.provider.key);o()}}return s(),l(),r=setInterval(()=>{l()},6e4),{destroy(){t=!0,r!==null&&(clearInterval(r),r=null),s()}}}function Pc(e){let t=e.attempts?Object.values(e.attempts):[],r=new Map;for(let s of t)s&&r.set(s.bead_id,s.attempt_id);let n=new Map;for(let s of e.done||[])s&&typeof s.bead_id=="string"&&typeof s.added_at=="number"&&n.set(s.bead_id,s.added_at);return s=>{let o=r.get(s.bead_id)!==s.attempt_id,a=n.get(s.bead_id),l=typeof a=="number"&&a>0&&typeof s.finished_at=="number"&&a>=s.finished_at;return!o&&!l&&typeof s.dismissed_at!="number"}}var R_="worker-ineligible";function ra(e){return Array.isArray(e)?e.filter(t=>typeof t=="string"):[]}function Dc(e){return ra(e).includes(R_)}var I_="worker-serial";function na(e){return ra(e).includes(I_)}function sa(e,t,r){if(typeof t!="string"||typeof r!="string")return[];let n=e?.runners;if(!n||!Object.hasOwn(n,t))return[];let s=n[t],o=s?.models;if(!o||!Object.hasOwn(o,r))return[];let a=o[r]?.efforts;return Array.isArray(a)?a.slice():Array.isArray(s.efforts)?s.efforts.slice():[]}var L_=new Set(["done","failed","orphaned","stopped","discarded"]);function Mc(e,t){let{queueStore:r,analysisStore:n,transport:s}=t,o=document.createElement("dialog");o.id="worker-parallel-analysis-dialog",o.className="pa",o.setAttribute("role","dialog"),o.setAttribute("aria-modal","true"),e.appendChild(o);let a=new Map,l=new Map,c=!1,u=null,f=null;function m(){return r&&r.get()||{revision:0,queue:[],serial_lanes:[],serial_lane_count:0,attempts:{},pr_wait:[]}}function b(){return n&&n.get()||{settings:{revision:0,runner:null,model:null,effort:null},job:null,last_good:null}}function E(){let C=m(),F=new Set;for(let te of Object.values(C.attempts||{})){let Q=te;Q&&typeof Q.bead_id=="string"&&!L_.has(Q.status)&&F.add(Q.bead_id)}for(let te of Array.isArray(C.pr_wait)?C.pr_wait:[])te&&typeof te.bead_id=="string"&&F.add(te.bead_id);for(let te of Object.values(C.discard_operations||{})){let Q=te;Q&&Q.phase!=="done"&&typeof Q.bead_id=="string"&&F.add(Q.bead_id)}return F}function k(C){return C.filter(F=>N(F)===null)}function N(C){let F=m();for(let te of Array.isArray(F.serial_lanes)?F.serial_lanes:[])if(Array.isArray(te?.entries)&&te.entries.some(Q=>Q.bead_id===C))return te.id;return(Array.isArray(F.queue)?F.queue:[]).some(te=>te.bead_id===C)?"parallel":null}function D(C,F){let te=a.get(C);return te||[...F.order]}function A(C){if(C.length<2)return!1;let F=N(C[0]);if(!F||F==="parallel")return!1;let te=m(),Q=(Array.isArray(te.serial_lanes)?te.serial_lanes:[]).find(M=>M.id===F)?.entries.map(M=>M.bead_id);if(!Array.isArray(Q))return!1;let $=C.map(M=>Q.indexOf(M));return $.every(M=>M>=0)&&$.every((M,V)=>V===0||M>$[V-1])}function j(){let C=m(),F=Array.isArray(C.serial_lanes)?C.serial_lanes:[],te=F.find(Q=>Array.isArray(Q.entries)&&Q.entries.length===0);return te?te.id:F[0]?.id||"s1"}function ee(C){let F=m().bead_titles||{};return typeof F[C]=="string"?F[C]:C}async function x(C,F){if(!s||c)return null;c=!0,T();try{return await s(C,F)}finally{c=!1,T()}}async function w(C){n?.setPending?.(!0);try{let F=await x("worker-parallel-analysis-start",{force:C});F&&F.applied===!1&&F.reason&&ie(`\uBD84\uC11D \uC2E4\uD328: ${F.reason}`,"error",2800)}finally{n?.setPending?.(!1)}}async function S(){let C=b().job;!s||!C||await s("worker-parallel-analysis-cancel",{job_id:C.job_id})}function H(){return m().runner_catalog}function Y(C){return Object.keys(H()?.runners?.[C]?.models||{})}function ge(C){let F=Y(C),te=H()?.runners?.[C]?.default_model;return typeof te=="string"&&F.includes(te)?te:F[0]||""}function le(){let C=b().settings,F=u||C.runner||"claude",te=Y(F),Q=u?ge(F):C.model||te[0]||"",$=sa(H(),F,Q),M=C.effort||"",V=$.includes(M)?M:$[0]||"";return{runner:F,model:Q,effort:V,models:te,efforts:$}}async function ne(C){let F=b().settings,te=await x("worker-parallel-analysis-settings-update",{expected_revision:F.revision,runner:C.runner,model:C.model,effort:C.effort});(!te||te.applied!==!0)&&(u=null,T(),te&&te.reason&&ie(`\uBD84\uC11D \uC124\uC815 \uAC70\uBD80: ${te.reason}`,"error",2800))}function de(C){u=C,T();let F=le();ne({runner:C,model:F.model,effort:F.effort})}function De(C){let F=le(),te=sa(H(),F.runner,C);ne({runner:F.runner,model:C,effort:te.includes(F.effort)?F.effort:te[0]||""})}function Ue(C){let F=le();ne({runner:F.runner,model:F.model,effort:C})}async function Ge(C,F){if(!s||c)return;let te=D(C,F),Q=b();if(te.length<2||!Q.last_good){ie("\uC81C\uCD9C\uD558\uB824\uBA74 2\uAC1C \uC774\uC0C1\uC774\uC5B4\uC57C \uD569\uB2C8\uB2E4","warning");return}let $=l.get(C)||j(),M=()=>({snapshot_digest:Q.last_good.identity_digest,group_index:C,lane:$,ordered_bead_ids:te,expected_revision:m().revision});c=!0,T();try{let V=await s("worker-parallel-analysis-submit",M());V&&V.queue&&r&&r.set(V.queue),V&&V.applied!==!0&&V.conflict===!0&&(V=await s("worker-parallel-analysis-submit",M()),V&&V.queue&&r&&r.set(V.queue)),V&&V.applied===!0?(a.delete(C),ie(`\uC9C1\uB82C \uB808\uC778 ${$}\uC5D0 ${te.length}\uAC1C \uBC30\uCE58`,"success")):ie(`\uC81C\uCD9C \uAC70\uBD80: ${V?.reason||"conflict"} (\uD050 \uBB34\uBCC0\uACBD)`,"error",2800)}finally{c=!1,T()}}function ze(C,F,te){a.set(C,D(C,F).filter(Q=>Q!==te)),T()}function Ze(C){a.delete(C),T()}function Fe(C,F,te,Q){let $=[...D(C,F)],M=$.indexOf(te),V=M+Q;M<0||V<0||V>=$.length||($.splice(V,0,...$.splice(M,1)),a.set(C,$),T())}function we(){let C=b().settings,F=Object.keys(H()?.runners||{}),te=le();return i`<div class="pa-settings">
      ...
      ${ve(C)}
    </div>`}function ve(C){return!Re(C)||xe(C)?i`<span class="pa-settings__unset">분석 모델 설정 필요</span>`:C.compatible===!1?i`<span class="pa-settings__incompatible"
        >설정 비호환 — 저장된 ${C.runner}/${C.model} · effort
        ${C.effort} 을(를) 카탈로그가 더는 제공하지 않습니다</span
      >`:C.is_default===!0?i`<span class="pa-settings__default">기본값</span>`:""}function xe(C){return C.is_default===!0&&C.compatible===!1}function Re(C){return!!(C.runner&&C.model&&C.effort)}function me(C){return Re(C)&&C.compatible!==!1}function X(C){let F=Math.max(0,Math.floor(C/1e3)),te=Math.floor(F/60),Q=F%60;return`${te}:${String(Q).padStart(2,"0")}`}function G(C){let F=C.job;if(F){let te=typeof F.started_at=="number"?F.started_at:0,Q=`${F.runner||"?"}/${F.model||"?"}`,$=te?` \xB7 \uACBD\uACFC ${X(Date.now()-te)}`:"";return i`<span class="pa-meta__progress"
        >분석 중 — ${Q} · effort ${F.effort||"?"}${$}</span
      >`}return ke()?i`<span class="pa-meta__progress"
          >준비 중 — 대상과 아티팩트 수집 중</span
        >`:""}function ke(){return n?.isPending?.()===!0}function fe(C){let F=m(),te=(Array.isArray(F.queue)?F.queue.length:0)+(Array.isArray(F.serial_lanes)?F.serial_lanes:[]).reduce((V,Ie)=>V+(Array.isArray(Ie.entries)?Ie.entries.length:0),0),Q=!!C.job,$=me(C.settings),M=Q||c||ke();return i`<div class="pa-meta">
    ...
    </div>`}function oe(C,F){let te=D(C,F),Q=E(),$=te.filter(Te=>Q.has(Te)),M=k(te),V=A(te),Ie=Array.isArray(m().serial_lanes)?m().serial_lanes:[],Ce=l.get(C)||j(),Me=F.eligible!==!0||te.length<2||$.length>0||M.length>0||V||c;return i`<section class="pa-group" data-group-index=${String(C)}>
      ...
      <footer class="pa-group__foot">
        ...
        <label class="pa-group__lane-field"
          ...
          <select
            ...
            aria-label="제출 대상 레인"
            @change=${Te=>{l.set(C,Te.target.value),T()}}
          >
    ...
    </section>`}function U(C){let F=Array.isArray(C.issues)?C.issues:[],te=F.filter($=>$.verdict==="parallel_ok").length,Q=F.filter($=>$.verdict==="uncertain").length;return i`<div class="pa-summary">
      <span>parallel_ok ${te}</span>
      <span>uncertain ${Q}</span>
    </div>`}function B(){let C=z&&!!b().job;if(C&&f===null){f=setInterval(()=>T(),1e3);return}!C&&f!==null&&(clearInterval(f),f=null)}function T(){let C=b();u&&C.settings.runner===u&&(u=null);let F=C.last_good?.result;B(),je(i`
