...
Please report this to https://github.com/markedjs/marked.`,e){let r="<p>An error occurred:</p><pre>"+jn(n.message+"",!0)+"</pre>";return t?Promise.resolve(r):r}if(t)return Promise.reject(n);throw n}}},gr=new jm;function ht(e,t){return gr.parse(e,t)}ht.options=ht.setOptions=function(e){return gr.setOptions(e),ht.defaults=gr.defaults,du(ht.defaults),ht};ht.getDefaults=si;ht.defaults=br;ht.use=function(...e){return gr.use(...e),ht.defaults=gr.defaults,du(ht.defaults),ht};ht.walkTokens=function(e,t){return gr.walkTokens(e,t)};ht.parseInline=gr.parseInline;ht.Parser=An;ht.parser=An.parse;ht.Renderer=ko;ht.TextRenderer=pi;ht.Lexer=xn;ht.lexer=xn.lex;ht.Tokenizer=wo;ht.Hooks=ms;ht.parse=ht;var lw=ht.options,cw=ht.setOptions,uw=ht.use,dw=ht.walkTokens,pw=ht.parseInline;var fw=An.parse,_w=xn.lex;function Zn(e){let t=ht.parse(e),n=ru.sanitize(t);return su(n)}function Bn(e,t){return l`<div class="prompt-block">
    ...
    <pre class="prompt-block__body">${t}</pre>
    ...
    </div>`}function re(){if(!i||!r)return[];let ne=r.get(i);return bi(ne?ne.lines:[])}function Te(){if(!i||!r)return null;let ne=r.get(i),te=ne?ne.last_event_at:null;return typeof te=="number"?te:null}function he(){return p.status==="running"}function G(){if(he()&&o){j||(j=setInterval(()=>de(),1e3));return}J()}function J(){j&&(clearInterval(j),j=null)}function ye(ne){let te=[],Ie=0;for(;Ie<ne.length;){let{idx:Qe,line:Le}=ne[Ie];if(Le.kind==="tool"){let xe=Ie;for(;xe<ne.length&&ne[xe].line.kind==="tool"&&ne[xe].line.tool===Le.tool;)xe+=1;if(xe-Ie>=Qm&&!k.has(Qe)){te.push({kind:"group",idx:Qe,tool:Le.tool||"",lines:ne.slice(Ie,xe)}),Ie=xe;continue}}te.push({kind:"line",idx:Qe,line:Le}),Ie+=1}return te}function Ae(ne){let te=[],Ie=new Map;for(let xe=0;xe<ne.length;xe+=1){let We=ne[xe],Ye=We.parent_tool_use_id;if(typeof Ye=="string"&&Ye.length>0){let et=Ie.get(Ye);et||(et={kind:"subagent",idx:xe,launch_id:Ye,agent_type:null,header:null,lines:[]},Ie.set(Ye,et),te.push(et)),et.lines.push({idx:xe,line:We});continue}if(We.kind==="tool"&&We.tool==="Agent"&&typeof We.launch_id=="string"&&We.launch_id.length>0){let et=be(We),Xe=Ie.get(We.launch_id);if(Xe){Xe.header={idx:xe,line:We},Xe.agent_type=et;continue}let ft={kind:"subagent",idx:xe,launch_id:We.launch_id,agent_type:et,header:{idx:xe,line:We},lines:[]};Ie.set(We.launch_id,ft),te.push(ft);continue}te.push({kind:"entry",idx:xe,line:We})}let Qe=[],Le=0;for(;Le<te.length;){if(te[Le].kind!=="entry"){Qe.push(te[Le]),Le+=1;continue}let xe=Le;for(;xe<te.length&&te[xe].kind==="entry";)xe+=1;Qe.push(...ye(te.slice(Le,xe))),Le=xe}return Qe}function be(ne){let te=ne.input;return te&&typeof te.subagent_type=="string"?te.subagent_type:null}function oe(ne){for(let te=ne.length-1;te>=0;te-=1){let Ie=ne[te];if(Ie.kind==="result"||Ie.kind==="error")return null;if(Ie.kind==="tool"&&!Object.hasOwn(Ie,"result"))return Ie}return null}function Se(ne){for(let te=ne.length-1;te>=0;te-=1)if(ne[te].kind==="thinking")return ne[te];return null}function ve(ne,te){if(te.kind==="gate")return l`<div class="sv__gate">${te.text}</div>`;if(te.kind==="phase")return l`<div class="sv__phase">${te.text}</div>`;if(te.kind==="result")return l`<div
      ...
      >
        ...
        >
      </div>`;if(te.kind==="thinking"){let Ie=h.has(ne);return l`<div
      ...
      >
        ...
        ${Ie?l`<pre class="sv__think-expand">${te.text}</pre>`:""}
      </div>`}if(te.kind==="error")return l`<div class="sv__error">⛔ ${te.text}</div>`;if(te.kind==="blocker")return l`<div class="sv__error">⛔ ${te.text}</div>`;if(te.kind==="tool"){let Ie=h.has(ne),Qe=te.tool==="Bash"?rg(te.command):0,Le=te.tool==="Bash"?Qe>1?So(te.command):te.command:te.path||te.command||"";return l`<div
      ...
      >
        ...
        ${Ie?l`<pre class="sv__tool-expand">${Y(te)}</pre>`:""}
...
`)}function ee(){if(!o)return l``;let ne=re(),te=(a?[p.agent_type,p.model,p.effort]:[p.runner,p.model,p.effort]).filter(Boolean).join(" \xB7 "),Ie=p.session_id||"",Qe=`\uB77C\uC774\uBE0C \uB530\uB77C\uAC00\uAE30 ${b?"ON":"OFF"}`,Le=he(),xe=Le?cg(Te(),Date.now()):"",We=Le?oe(ne):null,Ye=Le?Se(ne):null,et=lg(ne);return l`<div class="sv" data-attempt-id=${o}>
      <div class="sv__bar">
      ...
      ${a||u?"":F()}
      <div class="sv__body">
        ${ne.length===0?l`<div class="sv__empty">세션 로그 없음</div>`:Ae(ne).map(Xe=>Xe.kind==="subagent"?ge(Xe):Xe.kind==="group"?ke(Xe):ve(Xe.idx,Xe.line))}
      </div>
    ...
    </div>`}function ke(ne){return l`<div
      class="sv__group"
      role="button"
      tabindex="0"
      title="펼치기"
    ...
    >
    ...
    </div>`}function ge(ne){let te=k.has(ne.idx),Ie=ne.header?ne.header.line:null,Qe=Ie?Ie.is_error===!0?"\u2717":typeof Ie.result=="string"?"\u2713":"\u27F3":"",Le=Ie&&Ie.command?Ie.command:"";return l`<div class="sv__sub${te?" sv__sub--open":""}">
      <div
        class="sv__sub-head"
        role="button"
        tabindex="0"
      ...
      ${te?l`<div class="sv__sub-body">
            ${ye(ne.lines).map(xe=>xe.kind==="group"?ke(xe):ve(xe.idx,xe.line))}
          </div>`:""}
    </div>`}function Be(ne){k.add(ne),de()}function de(){Ke(ee(),e),G(),b&&Ve()}function Ve(){let ne=e.querySelector(".sv__body");ne&&(ne.scrollTop=ne.scrollHeight)}function st(ne){h.has(ne)?h.delete(ne):h.add(ne),de()}function dt(){b=!b,de()}function O(ne){fn(ne).then(te=>{te?le("\uBCF5\uC0AC\uB428","success",1200):le("\uBCF5\uC0AC \uC2E4\uD328","error",1600)})}function se(ne){!o||!ne||(p={...p,...ne},de())}function $e(ne){let te=ne.target;if(!te||!te.classList||!te.classList.contains("sv__body"))return;!(te.scrollHeight-te.scrollTop-te.clientHeight<=4)&&b&&(b=!1,de())}e.addEventListener("scroll",$e,!0);function Me(ne){let te=ne.target;!te||typeof te.closest!="function"||e.contains(te)||te.closest("dialog")||te.closest(".md-viewer-root")||bt()}let Ne=!1;function ze(){Ne||(document.addEventListener("mousedown",Me),Ne=!0)}function He(){Ne&&(document.removeEventListener("mousedown",Me),Ne=!1)}function at(ne){let te=ne&&ne.attempt_id;if(!te)return;let Ie=i;o=te,a=typeof ne.launch_id=="string"&&ne.launch_id.length>0?ne.launch_id:null,i=a?`session-log:${o}:${a}`:`session-log:${o}`,n&&Ie&&Ie!==i&&Promise.resolve(n("unsubscribe-session-log",{id:Ie})).catch(()=>{}),c=typeof ne.root_dir=="string"&&ne.root_dir.length>0?ne.root_dir:null,p=ne.meta||{},u=ne.hide_prompt===!0,b=!0,h.clear(),k.clear(),N(),!T&&r&&(T=r.subscribe(de)),n&&Promise.resolve(n("subscribe-session-log",{id:i,attempt_id:o,...a?{launch_id:a}:{},...c?{root_dir:c}:{}})).catch(()=>{}),ze(),de()}function bt(){let ne=i;He(),o=null,a=null,i=null,c=null,u=!1,h.clear(),k.clear(),N(),J(),n&&ne&&Promise.resolve(n("unsubscribe-session-log",{id:ne})).catch(()=>{}),Ke(l``,e),s&&s()}return{open:at,updateMeta:se,close:bt,isOpen(){return o!==null},destroy(){J(),He(),T&&(T(),T=null),e.removeEventListener("scroll",$e,!0),o=null,a=null,i=null,c=null,u=!1,Ke(l``,e)}}}function Eo(e){let t=e&&typeof e=="object"?e:{},n=t.metadata&&typeof t.metadata=="object"?t.metadata:{},r=hi(t.spec_id),s=hi(n.spec_id);return r?{path:r,source:"native",conflict:s.length>0&&s!==r}:s?{path:s,source:"metadata",conflict:!1}:{path:"",source:"none",conflict:!1}}function hi(e){return typeof e=="string"?e.trim():""}function Su(e){let t=Eo(e);if(t.path)return t;let n=hi(ug(e).spec_path);return n?{path:n,source:"draft",conflict:!1}:t}function ug(e){let t=e&&typeof e=="object"?e:{};return t.metadata&&typeof t.metadata=="object"?t.metadata:{}}function dg(e){return["plan_review","plan_approval","plan_check"].some(t=>{let n=e[t];return typeof n=="string"&&n.trim().length>0})}function pg(e){let t=e&&e.metadata||{},n=Su(e),r=[];return n.path&&r.push({kind:"spec",path:n.path,missing_state:n.source==="draft"?"spec_draft":null}),typeof t.plan_path=="string"&&t.plan_path.trim().length>0&&r.push({kind:"plan",path:t.plan_path.trim(),missing_state:dg(t)?null:"plan_pending"}),r}function Eu(e,t){let n=pg(e);return l`
...
${i}</pre
  ...
  `}function tb(e){let t=Fr(e);if(t)return t;let n=e.data;if(!n)return"";if(n.missing)return l`<div class="detail-prompt__missing">
    ...
    `}function Pt(m){let E=m.metadata||{},A=m.workflow||{},d=A.stages||{},_=d.spec&&d.spec.stale,v=d.impl&&d.impl.stale,$=d.plan||null,B=A.route_source==="derived",Q=A.route||E.route||"\u2014";return l`
    ...
    `}function De(m){let E=Array.isArray(m.labels)?m.labels:[];return l`
    ...
    `}function f(){if(!u)return l``;let m=p||{},E=String(m.id||u),A=m.title||"(\uC81C\uBAA9 \uC5C6\uC74C)",d=et(),_=m.status||"open",v=typeof m.priority=="number"?Math.max(0,Math.min(4,m.priority)):"",$=m.description||"",B={...m,metadata:{...m.metadata||{},...b}};return l`
      <div class="detail-overlay" role="dialog" aria-modal="true">
        ...
        <div class="detail-overlay__panel">
          ...
          ${nn(A,d)}
          ${Hu(B)}
          ${zu({metadata:B.metadata,workspace_values:ct(),catalog:Ee(),execution_defaults:M(),expanded:j,presets:ue()?.presets||[],preset_id:h,preset_busy:k,skipped_orchestration_keys:T},{onToggle:Q=>{j=Q,y()},onEdit:(Q,pe)=>{if(Q==="impl_runtime"||Q==="impl_model"||Q==="impl_effort"){Ce(Q,pe??"");return}Ze(Q,pe??"")},onPresetSelect:Q=>{h=Q,T=[],y()},onPresetApply:()=>{_e()}})}
          ${Xu({md:B.metadata,catalog:Z,workspace_defaults:ce,handlers:{onExecChange:Ze}})}
          ${Ln(_,v)} ${ln(m)}
          ${R($)}
          ${Ru(Ae,O,{expanded:ke,draft:ve,sending:Y,error:Se})}
          ${P(m)} ${De(m)} ${Rt(m)}
          ${Pt(m)} ${wt(m)}
          ${Eu(m,tt)}
    ...
    `}function y(){Ke(f(),e)}return{load(m){m!==u&&(b={},h="",T=[],j=!1,Te(),ge(),Le(),he()),u=m,p=null,V(),Ge(),z!==m&&ye(m)},clear(){u=null,p=null,b={},h="",k=!1,T=[],j=!1,Te(),ge(),Le(),he(),Me.close(),ze.close(),Ke(l``,e)},destroy(){x&&(x(),x=null),L&&(L(),L=null),w&&(w(),w=null),document.removeEventListener("keydown",D),se||(Me.destroy(),$e&&$e.parentNode&&$e.parentNode.removeChild($e)),ze.destroy(),Ne.parentNode&&Ne.parentNode.removeChild(Ne),u=null,p=null,he(),h="",k=!1,T=[],ge(),Le(),Ke(l``,e)}}}function ld(e){let t=document.createElement("dialog");t.id="fatal-error-dialog",t.setAttribute("role","alertdialog"),t.setAttribute("aria-modal","true"),t.innerHTML=`
...
`)}
  ...
  >`,T=e.lane==="done"?"":Yo(e.workflow),j=dd(e.from_id),W=Zo(e.priority),Z=l`<span class="worker-mini__title">${e.title}</span>`,ce=e.pr_url&&e.pr_number?l`<a
        ...
        >`:l`<span
          ...
          >${ee}</span
        >`),N=e.reason?l`<span class="worker-mini__reason">${e.reason}</span>`:"",H=r.length>0?r.map(ee=>l`<span class="worker-usage" title=${ee.tooltip}
              >${ee.label}</span
      ...
      >
        저장소 작업 보기
      </button>`:"",he=e.discard,G=he?.action||e.discard_action?l`<button
          type="button"
        ...
        >
          ${he?.label||"\uD3D0\uAE30"}
        </button>`:"",J=e.stale_work||null,ye=J?l`${J.can_resume||J.can_continue?l`<button
            type="button"
          ...
          >
            기존 작업 이어가기
          </button>`:""}${J.can_backup_fresh?l`<button
            type="button"
          ...
          >
            다시 확인
          </button>`:""}`:"",Ae=J?l`<div class="worker-mini__stale">
        <strong>${J.title}</strong>
        <span>${J.summary}</span>
        <span>${J.cause}</span>
        ${J.can_backup_fresh?l`<small
        ...
        >
          승인하고 진행
        </button>`:"",oe=e.lane!=="pr_wait"&&!e.done&&e.exec_chips&&(e.exec_chips.orchestration||e.exec_chips.worker)?l`<div class="worker-mini__exec">
          ${Vo(e.exec_chips,{pin:e.exec_chips_pinned===!0})}
        </div>`:"",Se=Ko(e.dependency_chips,{lane:e.lane}),ve=$s(e),Y=!!(s||o||e.merge_action||e.cancel_action||e.timeline_action||e.discard_action||he?.operation||e.revise_action||J);return l`<div
  ...
  >
    ${i?l`<div class="worker-mini__row1">
          ...
          <div class="worker-mini__row2">
            ...
            ${Se}${oe}${Y?l`<div class="worker-mini__foot">
                  ${H}${C}
                  <span class="worker-mini__actions"
                    >${F}${re}${Te}${G}${be}${ye}</span
                  >
                  ${$s(e)}
                </div>`:""}
...
`)}function Ss(e){if(typeof e!="object"||e===null)return null;let t=mr(e);if(t==="")return null;let n=(r,s)=>typeof s=="string"&&s.length>0?`${r}: ${s}`:null;return{text:t,title:Di(["\uC624\uCF00\uC2A4\uD2B8\uB808\uC774\uC158 \u2014 \uC774 attempt\uC5D0 \uAE30\uB85D\uB41C \uC2E4\uD589\uAC12",n("runner",e.runner),n(Xn.orchestration_model,e.model),n(Xn.orchestration_effort,e.effort),n(Xn.orchestration_speed,e.speed)])}}function yr(e,t){let n=Jn(e,"orchestration_model");if(n===null||n.resolution==="unavailable")return null;let r=Jn(e,"orchestration_effort"),s=Jn(e,"orchestration_speed"),o=hd([Sn(t,n.value??""),n.display,r!==null&&r.value!==null?r.display:null,s!==null&&s.value==="fast"?"Fast":null]);return o===""?null:{text:o,title:Di(["\uC624\uCF00\uC2A4\uD2B8\uB808\uC774\uC158 \u2014 \uD604\uC7AC \uD574\uC11D\uAC12 (\uD540 > \uD050 \uAE30\uBCF8\uAC12)",er("orchestration_model",n),er("orchestration_effort",r),er("orchestration_speed",s)])}}function bb(e,t){return e===null||e.value===null||Pi.has(e.resolution)?null:e.value!=="inherit"?e.value:t?`inherit\u2192${t}`:"inherit"}function hb(e){return e===null||Pi.has(e.resolution)?null:e.value==="auto"?"auto":e.display}function yb(e){return e===null?null:e.value==="auto"?"auto":Pi.has(e.resolution)?null:e.display}function tr(e,t){if(typeof e!="object"||e===null)return null;let n=Jn(e,"impl_dispatch"),r=Jn(e,"impl_runtime"),s=Jn(e,"impl_model"),o=Jn(e,"impl_effort"),a=Jn(e,"impl_speed"),i=n!==null&&n.value==="main"?"\uBA54\uC778":hd([bb(r,t??null),hb(s),yb(o),a!==null&&a.value==="fast"?"Fast":null]);return i===""?null:{text:i,title:Di(["\uC6CC\uCEE4(\uAD6C\uD604 \uC704\uC784) \u2014 \uD604\uC7AC \uD574\uC11D\uAC12 (\uD540 > \uC804\uC5ED kv > \uAE30\uBCF8). \uC2E4\uD589 \uC911\uC774\uBA74 \uC138\uC158\uC774 \uC2DC\uC791 \uC2DC \uACE0\uC815\uD55C \uAC12\uACFC \uB2E4\uB97C \uC218 \uC788\uC74C",er("impl_dispatch",n),er("impl_runtime",r),er("impl_model",s),er("impl_effort",o),er("impl_speed",a)])}}var Ht="",vb=["impl_runtime","impl_model","impl_effort"],wb=["claude_account","codex_account"],kb=5,Qo=1;function an(e){return!!e&&typeof e=="object"&&!Array.isArray(e)}function Jo(e,t){let n=t.transport,r=typeof t.root_dir=="string"&&t.root_dir.length>0?t.root_dir:null,s=t.notify||(M=>le(M,"error",4e3)),o={},a={},i=[],c=!1,u={state:"absent",values:{},warnings:[]},p={},b={},h=Promise.resolve(),k={claude:null,codex:null},T=!1,j=null,W={},Z="",ce="",z=!1,q=!1,N=!1,H=null,C=!1;function F(){let M=t.queue?t.queue():null;return an(M)?M:null}function re(){let M=F();return M?M.runner_catalog:null}function Te(){let M=F();return M&&an(M.execution_defaults)?M.execution_defaults:null}function he(){let M=t.implPresetStore?.get();return an(M)&&Array.isArray(M.presets)?M:null}function G(){return r===null?{}:{root_dir:r}}async function J(M,X){return C||!n?null:await n(M,X)}function ye(M){M&&an(M.queue)&&t.onQueueAdopt?.(M.queue)}async function Ae(M,X){let ue=F();if(!ue||C)return null;let S=await J(M,{...X,...G(),expected_revision:ue.revision});if(ye(S),r!==null&&S&&S.conflict){let K=S.queue&&typeof S.queue.revision=="number"?S.queue.revision:F()?.revision??ue.revision;S=await J(M,{...X,...G(),expected_revision:K}),ye(S)}return S}async function be(){c=!0,Ee();try{let M=await J("get-session-defaults",{...G()});o=an(M?.values)?{...M.values}:{},a={...o},i=Array.isArray(M?.warnings)?M.warnings:[]}catch(M){i=["kv_read_failed"],s(`\uC138\uC158 \uAE30\uBCF8\uAC12\uC744 \uC77D\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4: ${M instanceof Error?M.message:String(M)}`)}finally{c=!1,Ee()}}async function oe(){let M=qu(o,a);if(Object.keys(M).length!==0){try{let X=await J("set-session-defaults",{values:M,...G()});o=an(X?.values)?{...X.values}:{},a={...o},i=Array.isArray(X?.warnings)?X.warnings:[]}catch(X){s(`\uC138\uC158 \uAE30\uBCF8\uAC12 \uC800\uC7A5 \uC2E4\uD328: ${X instanceof Error?X.message:String(X)}`)}Ee()}}function Se(M,X){if(!an(M))return;let ue=M.state;u={state:ue==="usable"||ue==="unusable"||ue==="absent"?ue:"absent",values:an(M.values)?{...M.values}:{},warnings:Array.isArray(M.warnings)?M.warnings:[]},b={...u.values},X&&(p={...b})}async function ve(){try{Se(await J("get-workspace-accounts",{...G()}),!0)}catch(M){u={state:"unusable",values:{},warnings:["kv_read_failed"]},b={},p={},s(`\uC2E4\uD589 \uACC4\uC815 \uAE30\uBCF8\uAC12\uC744 \uC77D\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4: ${M instanceof Error?M.message:String(M)}`)}Ee()}async function Y(M){try{let X=await fetch(M);if(!X.ok)return null;let ue=await X.json();if(!an(ue)||!Array.isArray(ue.accounts))return null;let S=ue.accounts.filter(K=>an(K)&&typeof K.key=="string"&&K.key.length>0&&typeof K.email=="string"&&K.email.length>0);return{accounts:S,active:S.find(K=>K.active===!0)||null}}catch{return null}}async function ee(){T=!0;let[M,X]=await Promise.all([Y("/api/claude-usage"),Y("/api/codex-usage")]);C||(k={claude:M,codex:X},Ee())}function ke(){let M={};for(let X of wb){let ue=Object.hasOwn(p,X)?p[X]:null,S=Object.hasOwn(b,X)?b[X]:null;ue!==S&&(M[X]=ue)}return M}async function ge(){let M=ke();if(Object.keys(M).length!==0){try{Se(await J("set-workspace-accounts",{values:M,...G()}),!1)}catch(X){s(`\uC2E4\uD589 \uACC4\uC815 \uAE30\uBCF8\uAC12 \uC800\uC7A5 \uC2E4\uD328: ${X instanceof Error?X.message:String(X)}`)}Ee()}}function Be(M,X){X===Ht?delete p[M]:p[M]=X,Ee(),h=h.then(()=>ge())}function de(M,X){if(vb.includes(M)){dt(M,X);return}X===Ht?delete a[M]:a[M]=X,Ee(),oe()}function Ve(){let M=ct().orchestration_model,X=on({global:{orchestration_model:M??void 0},execution_defaults:Te(),runner_catalog:re()}).orchestration_model.value;return X?Sn(re(),X):null}function st(M,X){typeof X=="string"&&X.length>0?a[M]=X:delete a[M]}function dt(M,X){let ue=X===Ht?void 0:X,S=Mu({impl_runtime:M==="impl_runtime"?ue:a.impl_runtime,impl_model:M==="impl_model"?ue:a.impl_model,impl_effort:M==="impl_effort"?ue:a.impl_effort},re(),Ve());st("impl_runtime",S.impl_runtime),st("impl_model",S.impl_model),st("impl_effort",S.impl_effort),Ee(),oe()}async function O(){let M=F();if(!M)return;let X={orchestration_model:M.orchestration_model??null,orchestration_effort:M.orchestration_effort??null,orchestration_speed:M.orchestration_speed??null},ue=Fu(X,{...X,...W});if(Object.keys(ue).length!==0){try{let S=await Ae("worker-queue-set-orchestration-defaults",{values:ue});if(S&&S.applied===!1){s("Worker \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uD074\uB77C\uC774\uC5B8\uD2B8\uC640 \uCDA9\uB3CC");return}W={}}catch(S){s(`Worker \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: ${S instanceof Error?S.message:String(S)}`)}Ee()}}function se(M,X){W[M]=X===Ht?null:X,Ee(),O()}function $e(M){if(j=M,!M){Ee();return}let X=re(),ue=ct(),S=ue.orchestration_model;S&&!ws(X,M).includes(S)&&(W.orchestration_model=null,S=null);let K=ue.orchestration_effort;K&&!wi(X,M,S||mn).includes(K)&&(W.orchestration_effort=null),Ee(),O()}async function Me(M){if(!(!F()||M<Qo)){try{await Ae("worker-queue-set-slots",{slots:M})}catch(X){s(`slots \uC800\uC7A5 \uC2E4\uD328: ${X instanceof Error?X.message:String(X)}`)}Ee()}}async function Ne(M){if(!(!F()||M<Qo||M>kb)){try{await Ae("worker-queue-set-serial-lane-count",{count:M})}catch(X){s(`\uC9C1\uB82C \uB808\uC778 \uC800\uC7A5 \uC2E4\uD328: ${X instanceof Error?X.message:String(X)}`)}Ee()}}async function ze(M,X){let ue=M==="auto_advance"?"worker-automation-toggle":M==="auto_merge"?"worker-merge-auto-toggle":"worker-auto-repair-toggle";try{await Ae(ue,{on:X})}catch(S){s(`\uC790\uB3D9\uD654 \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: ${S instanceof Error?S.message:String(S)}`)}Ee()}function He(){let M={},X=ct();for(let ue of Ro){let S=Un.includes(ue)?X[ue]:a[ue];typeof S=="string"&&S.length>0&&(M[ue]=S)}return M}async function at(){let M=he();if(!M)return;let X=He();if(Object.keys(X).length===0){s("\uC800\uC7A5\uD560 \uC2E4\uD589 \uC124\uC815\uC774 \uC5C6\uC2B5\uB2C8\uB2E4 \u2014 \uBA3C\uC800 \uC2E4\uD589 \uAC12\uC744 \uC120\uD0DD\uD558\uC138\uC694");return}let ue=(M.presets||[]).find(K=>K.id===Z),S=ce.trim()||(ue?ue.name:"");if(!S){s("\uD504\uB9AC\uC14B \uC774\uB984\uC744 \uC785\uB825\uD558\uC138\uC694");return}try{let K=ue?await J("impl-preset-update",{expected_revision:M.revision,id:ue.id,name:S,settings:X}):await J("impl-preset-create",{expected_revision:M.revision,name:S,settings:X});if(K&&K.applied){if(ce="",!ue&&Array.isArray(K.presets)){let _e=K.presets.find(x=>x.name===S);Z=_e?_e.id:Z}Ee()}else s("\uD504\uB9AC\uC14B \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uACF3\uC5D0\uC11C \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4"),Ee()}catch(K){s(`\uD504\uB9AC\uC14B \uC800\uC7A5 \uC2E4\uD328: ${K instanceof Error?K.message:String(K)}`)}}async function bt(){let M=he();if(!(!M||Z.length===0))try{let X=await J("impl-preset-delete",{expected_revision:M.revision,id:Z});X&&X.applied?(Z="",Ee()):(s("\uD504\uB9AC\uC14B \uC0AD\uC81C \uC2E4\uD328: \uB2E4\uB978 \uACF3\uC5D0\uC11C \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4"),Ee())}catch(X){s(`\uD504\uB9AC\uC14B \uC0AD\uC81C \uC2E4\uD328: ${X instanceof Error?X.message:String(X)}`)}}function ne(M){o=an(M.values)?{...M.values}:{},a={...o},i=Array.isArray(M.warnings)?M.warnings:[],an(M.queue)&&(t.onQueueAdopt?.(M.queue),W={})}async function te(){let M=he(),X=F();if(!M||!X||Z.length===0)return;let ue=S=>({preset_id:Z,expected_revision:M.revision,expected_queue_revision:S,...G()});try{let S=await J("apply-impl-preset-global",ue(X.revision));if(S&&S.applied&&ne(S),r!==null&&S&&S.queue_applied===!1){let K=S.queue&&typeof S.queue.revision=="number"?S.queue.revision:F()?.revision??X.revision;S=await J("apply-impl-preset-global",ue(K)),S&&S.applied&&ne(S)}S&&S.applied?S.queue_applied===!1&&s("\uC624\uCF00\uC2A4\uD2B8\uB808\uC774\uC158 \uAC12\uC740 \uC801\uC6A9\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4 \u2014 \uB2E4\uC2DC \uC2DC\uB3C4\uD558\uC138\uC694"):S&&S.conflict&&s("\uC2E4\uD589 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328: \uD504\uB9AC\uC14B\uC774 \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4")}catch(S){s(`\uC2E4\uD589 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328: ${S instanceof Error?S.message:String(S)}`)}Ee()}async function Ie(){q=!0,N=!1,Ee();try{let M=await J("get-worker-system-prompt",{});!M||typeof M!="object"||Array.isArray(M)?N=!0:H=M}catch{N=!0}finally{q=!1,Ee()}}function Qe(){if(z=!z,z&&!H){Ie();return}Ee()}function Le(){let M=Fr({loading:q,error:N});if(M)return M;if(!H)return"";let X=Array.isArray(H.variants)?H.variants:[];return l`<div class="settings-dialog__sp-body">
    ...
    >
      ...
      <span class="settings-dialog__controls">
        <select
          aria-label=${X}
          data-account-key=${M}
          @change=${L=>Be(M,String(L.target.value))}
        >
    ...
    </div>`}function ct(){let M=F(),X={};for(let ue of Un)X[ue]=Object.prototype.hasOwnProperty.call(W,ue)?W[ue]:M&&typeof M[ue]=="string"?M[ue]:null;return X}function Ge(){let M=re(),X=a.impl_runtime,ue=a.impl_model,S=he(),K=F(),_e=ct(),x=ws(M,j),L=Br(M,void 0).filter(Ce=>Ce!==mn),w=wi(M,j,_e.orchestration_model||mn).filter(Ce=>Ce!==mn),D=Z?(S?.presets||[]).find(Ce=>Ce.id===Z):null,V=D?Nu(He(),an(D.settings)?D.settings:{}):null,me=K&&typeof K.slots=="number"?K.slots:Qo+1,ae=K&&typeof K.serial_lane_count=="number"?K.serial_lane_count:Qo,we=Te()?.supported===!0,rt=ft(),Ze=ki("workflow_mode",ys,a,Te(),M);return l`
      ...
      ${c?l`<div class="settings-dialog__empty">불러오는 중…</div>`:l`
            ...
            <div class="settings-dialog__group">
              ...
              <div class="settings-dialog__row">
                ...
                <span class="settings-dialog__controls">
                  <select
                    ...
                    .value=${hr(j||Ht)}
                    @change=${Ce=>{let ut=String(Ce.target.value);$e(ut===Ht?null:ut)}}
                  >
              ...
              ${Ye("orchestration_model","\uBAA8\uB378",x,se,_e)}
              ${Ye("orchestration_effort","effort",w,se,_e)}
              ${Ye("orchestration_speed","\uC18D\uB3C4",hs,se,_e)}
            </div>
            ...
            <div class="settings-dialog__group">
              ...
              ${Ye("impl_runtime","\uC704\uC784 \uB300\uC0C1",Oo,de,a)}
              ${Ye("impl_model","\uBAA8\uB378",Br(M,X),de,a)}
              ${Ye("impl_effort","effort",Ur(M,X,ue),de,a)}
              ${Ye("impl_speed","\uC18D\uB3C4",hs,de,a)}
              ${Ye("quick_fix_impl_model","quick_fix \uAD6C\uD604 \uBAA8\uB378",L,de,a,!1,{...a,..._e})}
            </div>
            ...
            <div class="settings-dialog__group">
              ...
              ${_t("slots","\uB3D9\uC2DC \uC2E4\uD589",me,Ce=>Me(Ce))}
              ${_t("serial-lane-count","\uC9C1\uB82C \uB808\uC778",ae,Ce=>Ne(Ce))}
            </div>
            ${xe()}
          `}
    `}function Ee(){C||Ke(Ge(),e)}return{load(){W={};let M=[be(),ve()];return T||M.push(ee()),Promise.all(M).then(()=>{})},render:Ee,sessionDraft:()=>({...a}),destroy(){C=!0,Ke(l``,e)}}}function ea(e){return l`<svg
    class="mon-i"
    viewBox="0 0 16 16"
    aria-hidden="true"
    fill="none"
  ...
  >
    ...
    >
      ...
      >
        ${kd()}
      </button>`}function he(Y){let ee=xb(Y);return ee?l`<div class="mon2-deck__chips">
      ...
      ${ee.worker?l`<span class="mon2-deck__chip" title=${ee.worker.title}
          ...
          >`:""}
    </div>`:""}function G(Y){let ee=[];for(let[ke,ge]of[["queue","\uB300\uAE30"],["pr_wait","PR"],["session_active","\uC138\uC158"]]){let Be=Mi(Y,ke);Be>0&&ee.push(`${ge} ${Be}`)}return ee.join(" \xB7 ")}function J(Y){let ee=Mi(Y,"running"),ke=typeof Y.slots=="number"?Y.slots:1;return l`<div
    ...
    >
      <div class="mon2-deck__tile-hd">
        ...
        <span
          ...
          title=${`\uC2AC\uB86F ${ke}\uAC1C \uC911 ${ee}\uAC1C \uC2E4\uD589 \uC911`}
        >
          <span class="mon2-deck__load-n">${ee}/${ke}</span>
          ${re(ee,ke)}
        </span>
        <button
          type="button"
    ...
    </div>`}function ye(Y){let ee=t.doneItems?t.doneItems():[],ke=t.rangeLabel?t.rangeLabel():"",ge=xd(Array.isArray(ee)?ee:[]),Be=de=>Y.reduce((Ve,st)=>Ve+Mi(st,de),0);return l`<div class="mon2-deck__bar">
      <span
        class="mon2-deck__total-counts"
        title=${`visible \uB808\uD3EC ${Y.length}\uACF3\uC758 \uD569\uACC4\uC785\uB2C8\uB2E4 \u2014 \uC2E4\uD589\xB7\uB300\uAE30\xB7PR\uC740 \uC9C0\uAE08, \uC644\uB8CC\uB294 ${ke}`}
        >실행 ${Be("running")} · 대기 ${Be("queue")} · PR
        ${Be("pr_wait")}${Be("session_active")>0?` \xB7 \uC138\uC158 ${Be("session_active")}`:""}
        · ${ke} 완료
        ${Array.isArray(ee)?ee.length:0}</span
      >
      ${ge===null?"":l`<span class="mon2-deck__total-tokens">
            ${typeof ge=="string"?l`<span
...
${we.notes_tail}`:"notes\uC758 REVISE finding\uC744 \uC2A4\uD399\uC5D0 \uBC18\uC601\uD558\uB294 \uCC98\uBD84 \uC138\uC158\uC744 \uB744\uC6C1\uB2C8\uB2E4":""};return Object.hasOwn(xe,ae)&&(Ce.blocked_by=Array.isArray(xe[ae])?xe[ae].filter(ut=>typeof ut=="string"&&ut.length>0):[]),Ce};for(let w=0;w<yt.length;w++){let D=K(yt[w],"queue",w,yt.length);if(!D)continue;T.push(D);let V=Z.get(se);V?V.push(D):Z.set(se,[D])}let _e=w=>{let D=k.find(ae=>ae.id===w&&ae.root_dir===se);if(D)return{id:w,title:D.title,badge:"PR \uB300\uAE30 \xB7 \uC810\uC720"};let V=h.find(ae=>ae.id===w&&ae.root_dir===se),me=V&&V.run_state==="failed"?"\uC2E4\uD328 \xB7 \uC810\uC720 \uC720\uC9C0":V&&V.run_state==="paused"?"\uC77C\uC2DC\uC815\uC9C0 \xB7 \uC810\uC720":"\uC2E4\uD589 \uC911 \xB7 \uC810\uC720";return{id:w,title:V?V.title:ue(w).title,badge:me}},x=[];for(let w=0;w<Math.max(ct,_t.length);w++){let D=`s${w+1}`,V=Ge.get(D),me=V&&Array.isArray(V.entries)?V.entries:[],ae=[];for(let Ze=0;Ze<me.length;Ze++){let Ce=K(me[Ze],D,Ze,me.length);Ce&&(ae.push(Ce),T.push(Ce))}let we=xt(Ot[D]),rt=Array.isArray(we.occupied_by)?we.occupied_by.filter(Ze=>typeof Ze=="string"):[];ae.length===0&&rt.length===0&&(ct<=1||w>=ct)||x.push({id:D,index:w,items:ae,raw_length:me.length,occupied_by:rt,occupants:rt.map(Ze=>_e(Ze)),corrections:Array.isArray(we.corrections)?we.corrections.length:0,cycle:we.cycle===!0,...ae.length===0&&rt.length===0?{empty:!0}:{}})}ce.set(se,x);let L=Array.from({length:ct},(w,D)=>{let V=`s${D+1}`,me=Ge.get(V),ae=me&&Array.isArray(me.entries)?me.entries:[],we=xt(Ot[V]);return{id:V,index:ae.length,length:ae.length,occupied_by:Array.isArray(we.occupied_by)?we.occupied_by.filter(rt=>typeof rt=="string"):[]}});for(let w of Array.isArray(O.runnable)?O.runnable:[]){let D=w&&w.bead_id;if(typeof D!="string"||S.has(D))continue;S.add(D);let V=w.workflow&&typeof w.workflow=="object"?w.workflow:null,me=V&&typeof V.route=="string"&&V.route||(typeof w.route=="string"?w.route:null),ae=Vb(xt(Me),w.exec_pins,me);Array.isArray(w.blocked_by)&&w.blocked_by.length>0&&N.set(D,w.blocked_by.filter(we=>typeof we=="string"&&we.length>0)),typeof w.title=="string"&&w.title.length>0&&F.set(D,w.title),Array.isArray(w.scope)&&C.set(D,w.scope.filter(we=>typeof we=="string"&&we.length>0)),b.push({...ue(D),title:w.title||He[D]||D,lane:"runnable",draggable:!0,reason:Gd(ne,D),created_at:w.created_at??void 0,updated_at:w.updated_at??void 0,status:typeof w.status=="string"?w.status:void 0,labels:Array.isArray(w.labels)?w.labels:[],spec_id:typeof w.spec_id=="string"?w.spec_id:"",workflow:V||(me?{route:me,chips:{route:me}}:null),...ae?{exec_chips:ae}:{},blocked:w.blocked===!0,...Array.isArray(w.blocked_by)?{blocked_by:w.blocked_by.filter(we=>typeof we=="string"&&we.length>0)}:{},place_index:yt.length,place_lanes:L})}for(let w of M){let D=w&&w.bead_id;if(typeof D!="string"||S.has(D)||(S.add(D),o!==void 0&&typeof w.added_at=="number"&&w.added_at<o))continue;let V=Hb(ze,D),me=V&&typeof V.done_kind=="string"?V.done_kind:null;j.push({...ue(D),lane:"done",done:!0,done_layout:"three_line",usage:hn(ze,D),work_ms:zo(ze,D),done_at:typeof w.added_at=="number"?w.added_at:void 0,done_kind:me,badges:[...me&&Hd[me]?[Hd[me]]:[],...Wo(ze,D)]})}}let re=new Map;s.forEach((O,se)=>{O&&typeof O.root_dir=="string"&&re.set(O.root_dir,se)});let Te=n&&n.running_sort==="repo"?"repo":"started";h.sort((O,se)=>{let $e=O.kind==="session",Me=se.kind==="session";if($e!==Me)return $e?1:-1;if($e&&Me){let He=ua(se.updated_at)-ua(O.updated_at);return He!==0?He:O.id.localeCompare(se.id)}if(Te==="repo"){let He=re.get(O.root_dir)??Number.MAX_SAFE_INTEGER,at=re.get(se.root_dir)??Number.MAX_SAFE_INTEGER;if(He!==at)return He-at}let Ne=typeof O.started_at=="number"&&Number.isFinite(O.started_at)?O.started_at:null,ze=typeof se.started_at=="number"&&Number.isFinite(se.started_at)?se.started_at:null;return Ne!==null&&ze!==null&&Ne!==ze?Ne-ze:Ne===null&&ze!==null?1:Ne!==null&&ze===null?-1:O.id.localeCompare(se.id)}),j.sort((O,se)=>(se.done_at??0)-(O.done_at??0));let he=s.length>0?s:r.map(O=>({root_dir:O&&O.root_dir,name:O&&O.name,auto_advance:O&&O.auto_advance,auto_merge:O&&O.auto_merge,slots:O&&O.slots,revision:O&&O.revision,runner_catalog:O&&O.runner_catalog})),G=new Set(b.map(O=>O.root_dir)),J=[];for(let O of he){if(!O||typeof O.root_dir!="string")continue;let se=Z.get(O.root_dir)||[],$e=ce.get(O.root_dir)||[];!(se.length>0||$e.some(Ne=>Ne.items.length>0||Ne.occupied_by.length>0))&&!G.has(O.root_dir)||J.push({root_dir:O.root_dir,name:O.name||O.root_dir,auto_advance:O.auto_advance===!0,auto_merge:O.auto_merge===!0,slots:typeof O.slots=="number"&&O.slots>=zd?O.slots:zd,revision:typeof O.revision=="number"?O.revision:0,runner_catalog:xt(O.runner_catalog),items:se,sublanes:{parallel:se,serial:$e},serial_lane_count:z.get(O.root_dir)||0,raw_queue_length:q.get(O.root_dir)||0})}let ye={runnable:b,runnable_all:b,runnable_hidden:{blocked:0,spec:0,deps:0},runnable_sections:[],runnable_flat:c==="updated_flat",queue:T,queue_groups:J,running:h,pr_wait:k,done:j,parallel_rows:[],chain_lanes:[],cross_lanes_revision:i&&typeof i.revision=="number"?i.revision:null,cross_lanes_unreadable:i===null,parallel_raw_length:Object.fromEntries(q),owner_of:{}},Ae=jd(ye);for(let O of W)Ae.has(O.id)||Ae.set(O.id,{root_dir:O.root_dir,workspace_name:O.workspace_name,lane:"done",state:"done"});let be=new Map;for(let[O,se]of N)for(let $e of se){let Me=be.get($e);Me?Me.includes(O)||Me.push(O):be.set($e,[O])}for(let O of[...ye.queue,...ye.runnable]){if(!Object.hasOwn(O,"blocked_by"))continue;let se=Ae.get(O.id);O.blockers=(O.blocked_by||[]).map($e=>Bd($e,se,Ae,s)),O.blocker_warnings=O.blockers.filter($e=>$e.missing_internal).map($e=>`\u26A0 \uC120\uD589 ${$e.id}\uAC00 \uC5B4\uB290 \uB808\uC778\uC5D0\uB3C4 \uC5C6\uACE0 \uC2E4\uD589 \uC911\uB3C4 \uC544\uB2D8 \u2014 \uC218\uB3D9 \uAC1C\uC785 \uC804\uAE4C\uC9C0 \uC774 \uC790\uB9AC\uC5D0\uC11C \uC815\uC9C0`),O.blocker_warnings.length>0&&(O.alert=!0)}for(let O of[...ye.queue,...ye.runnable,...ye.running,...ye.pr_wait]){let se=O.lane==="running"||O.lane==="pr_wait"?[]:(O.blockers||[]).map(Kb),$e=[];for(let ze of be.get(O.id)||[]){let He=Yb(ze,O.root_dir,Ae);He&&$e.push(He)}let Me=O.lane==="running"||O.lane==="pr_wait"?[]:O.blocker_warnings||[];if(se.length===0&&$e.length===0&&Me.length===0)continue;let Ne={predecessors:se,successors:$e,warnings:Me};O.dependency_chips=Ne}Jb(ye,H,C,Ae,s);let oe=Ud(ye.queue_groups);for(let O of ye.queue_groups)for(let se of O.sublanes.serial){let $e=oe.get(Wd(O.root_dir,se.id));$e&&(se.cross_wait_peers=$e)}ye.chain_lanes=Xb(i&&Array.isArray(i.lanes)?i.lanes:[],N,Ae,s,F,p);let Se=new Map;for(let O of[...ye.queue,...ye.runnable])Se.has(O.id)||Se.set(O.id,O);let ve=new Set;for(let O of ye.chain_lanes)for(let se of O.rows){if(O.status==="confirmed"&&!se.unplaced&&!se.fixed&&ve.add(se.id),!O.draft&&!se.unplaced)continue;let $e=Se.get(se.id);$e&&($e.cross_lane_chip={lane_id:O.lane_id,number:O.number,status:O.status,label:O.draft?`\uC5F0\uACB0 ${O.number} (draft)`:`\uC5F0\uACB0 ${O.number}`})}let Y=[];for(let O of Z.values())for(let se of O)ve.has(se.id)||Y.push(se);Y.sort((O,se)=>{let $e=O.workspace_name.localeCompare(se.workspace_name);return $e!==0?$e:(O.queue_index??0)-(se.queue_index??0)}),ye.parallel_rows=Y;let ee={};for(let[O,se]of Ae)typeof se.root_dir=="string"&&se.root_dir.length>0&&(ee[O]=se.root_dir);for(let O of ye.chain_lanes)for(let se of O.rows)!Object.hasOwn(ee,se.id)&&se.root_dir.length>0&&p.has(se.root_dir)&&(ee[se.id]=se.root_dir);ye.owner_of=ee;let ke=ye.runnable.length;ye.runnable_all=ye.runnable.slice();let ge=ye.runnable;a.show_blocked||(ge=ge.filter(O=>O.blocked!==!0));let Be=ge.length;a.spec==="with"?ge=ge.filter(O=>!!O.spec_id):a.spec==="without"&&(ge=ge.filter(O=>!O.spec_id));let de=ge.length;a.with_deps&&(ge=ge.filter(O=>{let se=O.dependency_chips;return se?(se.predecessors||[]).length>0||(se.successors||[]).length>0:!1})),ye.runnable_hidden={blocked:ke-Be,spec:Be-de,deps:de-ge.length};let Ve=(O,se)=>{let $e=ua(se.updated_at)-ua(O.updated_at);return $e!==0?$e:O.id.localeCompare(se.id)},dt=c==="repo_spec"?(O,se)=>{let $e=O.spec_id?0:1,Me=se.spec_id?0:1;return $e!==Me?$e-Me:Ve(O,se)}:Ve;if(c==="updated_flat")ye.runnable=ge.slice().sort(Ve),ye.runnable_sections=[];else{let O=new Map;for(let Me of ge){let Ne=O.get(Me.root_dir);Ne?Ne.push(Me):O.set(Me.root_dir,[Me])}let se=[],$e=[];for(let Me of he){if(!Me||typeof Me.root_dir!="string")continue;let Ne=(O.get(Me.root_dir)||[]).slice().sort(dt);O.delete(Me.root_dir),Ne.length!==0&&(se.push({root_dir:Me.root_dir,name:Me.name||Me.root_dir,items:Ne.map(ze=>({...ze,workspace_name:""}))}),$e.push(...Ne))}for(let[Me,Ne]of O){let ze=Ne.slice().sort(dt);se.push({root_dir:Me,name:ze[0]?.workspace_name||Me,items:ze.map(He=>({...He,workspace_name:""}))}),$e.push(...ze)}ye.runnable=$e,ye.runnable_sections=se}return ye}var Yd="\uAC19\uC740 \uC9C1\uB82C \uB808\uC778\uC73C\uB85C";function Zd(e){return typeof e.lane=="string"&&/^s[1-5]$/.test(e.lane)?e.lane:e.lane==="running"&&e.serial_lane_id?e.serial_lane_id:null}function Xd(e){return e.lane==="runnable"||e.lane==="queue"||typeof e.lane=="string"&&/^s[1-5]$/.test(e.lane)}var ep="bdui.monitor.done-range",tp="bdui.monitor.running_sort",np="bdui.monitor.candidate_sort",rp="beads-ui.monitor.candidate-filter",sp="beads-ui.monitor.sections";function eh(){try{let e=window.localStorage.getItem(rp);if(!e)return{...wr};let t=JSON.parse(e);return!t||typeof t!="object"?{...wr}:{show_blocked:typeof t.show_blocked=="boolean"?t.show_blocked:wr.show_blocked,spec:Hi.some(n=>n.value===t.spec)?t.spec:"all",with_deps:typeof t.with_deps=="boolean"?t.with_deps:wr.with_deps}}catch{return{...wr}}}function Vi(e){try{window.localStorage.setItem(rp,JSON.stringify({show_blocked:e.show_blocked,spec:e.spec,with_deps:e.with_deps}))}catch{}}function th(){try{let e=window.localStorage.getItem(np);return Ls.some(t=>t.value===e)?e:"repo_spec"}catch{return"repo_spec"}}function nh(e){try{window.localStorage.setItem(np,e)}catch{}}function rh(){try{let e=window.localStorage.getItem(sp);if(!e)return{};let t=JSON.parse(e);return t&&typeof t=="object"?t:{}}catch{return{}}}function Qd(e){try{window.localStorage.setItem(sp,JSON.stringify(e))}catch{}}function sh(){try{let e=window.localStorage.getItem(ep);return bn(e)?e:dn}catch{return dn}}function oh(e){try{window.localStorage.setItem(ep,e)}catch{}}function ah(){try{return window.localStorage.getItem(tp)==="repo"?"repo":"started"}catch{return"started"}}function ih(e){try{window.localStorage.setItem(tp,e)}catch{}}var op="tab:monitor:pipeline",lh=1e3,ch=[{lane:"runnable",pane:"candidate",title:"\uC2E4\uD589\uAC00\uB2A5",empty:"\uC2E4\uD589 \uC790\uACA9\uC744 \uAC16\uCD98 \uC774\uC288 \uC5C6\uC74C"},{lane:"queue",pane:"queue",title:"\uB300\uAE30",empty:"\uD45C\uC2DC\uD560 \uB808\uD3EC \uC5C6\uC74C"},{lane:"running",pane:"running",title:"\uC2E4\uD589\uC911",empty:"\uC2E4\uD589 \uC911 \uC5C6\uC74C"},{lane:"pr_wait",pane:"pr_wait",title:"PR \uB300\uAE30",empty:"PR \uC5C6\uC74C"},{lane:"done",pane:"done",title:"\uC644\uB8CC",empty:"\uC644\uB8CC \uAE30\uB85D \uC5C6\uC74C"}],Jd="\u2460\u2461\u2462\u2463\u2464\u2465\u2466\u2467\u2468\u2469\u246A\u246B\u246C\u246D\u246E\u246F\u2470\u2471\u2472\u2473";function uh(e){return e>=1&&e<=Jd.length?Jd[e-1]:`(${e})`}function ap(e,t){let n=St("views:monitor"),r=t.gotoIssue,s=t.pipelineStore,o=t.transport,a=t.getWorkspacePath,i=t.openDoc,c=t.switchWorkspace,u=t.router,p=t.now||(()=>Date.now()),b=t.confirm||(d=>typeof globalThis.confirm!="function"||globalThis.confirm(d)),h=sh(),k=ah(),T=eh(),j=th(),W=rh(),Z=null,ce=null,z=null,q=null,N=[],H=null;function C(){let d=Vn.find(_=>_.value===h);return d?d.label:""}let F=document.createElement("div");F.className="mon",e.appendChild(F);let re=document.createElement("div");re.className="worker-drawer-overlay",re.hidden=!0;let Te=document.createElement("div");Te.className="worker-drawer-overlay__backdrop";let he=document.createElement("div");he.className="worker-drawer-host mon2-drawer",re.append(Te,he),e.appendChild(re);let G=Gi(null,null),J=new Map,ye=new Map,Ae=null,be=null,oe=null,Se=jr(he,{transport:o,sessionLogStore:t.sessionLogStore,onClose:()=>{Z=null,re.hidden=!0,_e()}});async function ve(d,_,v,$,B=!0){if(!o||!v)return null;let Q=await o(d,{..._,root_dir:v,expected_revision:$});if(Q&&Q.conflict&&B){Q.queue&&ye.set(v,Q.queue);let pe=Q.queue&&typeof Q.queue.revision=="number"?Q.queue.revision:$;Q=await o(d,{..._,root_dir:v,expected_revision:pe})}return Q&&Q.queue&&v&&ye.set(v,Q.queue),Q}function Y(d,_){let v=ye.get(d),$=s&&s.get?s.get():null,B=(Array.isArray($)?$:[]).find(pe=>pe?.root_dir===d);return(v||B)?.merge_queue?.find(pe=>pe.bead_id===_)?.continuation_action}async function ee(d,_,v,$){let B=await ve(d,_,v,$),Q=ye.get(v)?.revision??B?.queue?.revision??$;return Dn(B,(pe,Oe)=>ve(d,{..._,continuation:pe,decision_token:Oe},v,Q,!1),{refresh:pe=>ve(d,_,v,pe?.queue?.revision??ye.get(v)?.revision??Q,!1)})}async function ke(d,_,v,$){let B=await Dn({continuation_mismatch:$},(pe,Oe)=>ve("worker-merge-queue-add",{bead_id:_,continuation:pe,decision_token:Oe},d,v,!1)),Q=B?.queue?.merge_queue?.find(pe=>pe.bead_id===_)?.continuation_action;B?.applied!==!0&&Q?.continuation===null&&Q.mismatch&&await ke(d,_,B.queue.revision,Q.mismatch)}async function ge(d,_,v){let $=await ve("worker-discard",d,_,v);if($&&$.discarded===!0){le(Go($),"success",5e3);return}if($&&$.reason){le(`\uD3D0\uAE30 \uC2E4\uD328: ${$.reason}`,"error");return}if($&&$.accepted&&$.pending==="merged_revert"){le("revert PR \uB300\uAE30 \uC0C1\uD0DC\uB85C \uC804\uD658\uD588\uC2B5\uB2C8\uB2E4","success");return}if($&&$.accepted){le(`\uD3D0\uAE30 \uC9C4\uD589: ${$.phase||"\uBC31\uC5C5 \uC911"}`,"success");return}$&&!$.conflict&&le("\uD3D0\uAE30 \uAC70\uBD80: unknown","error")}async function Be(d,_,v){return!o||!v?null:await o(d,{..._,root_dir:v})}async function de(){let d=new Map;for(let _ of G.pr_wait)d.has(_.root_dir)||d.set(_.root_dir,_.expected_revision);for(let[_,v]of d)await ve("worker-merge-queue-add-all",{},_,v)}function Ve(d){let _=W[d];return!!(_&&_.runnable===!0)}function st(d){let _={...W[d]||{}};_.runnable=!_.runnable,W={...W,[d]:_},Qd(W),_e()}function dt(d){return W[d]===!0}function O(d){W={...W,[d]:W[d]!==!0},Qd(W),_e()}function se(d){let _=G.queue_groups.find(v=>v.root_dir===d);if(!_)return null;for(let v=0;v<_.serial_lane_count;v+=1){let $=`s${v+1}`,B=_.sublanes.serial.find(Q=>Q.id===$);if(!B||B.raw_length===0&&B.occupied_by.length===0)return $}return null}function $e(d,_){let v=G.queue_groups.find(B=>B.root_dir===d),$=v?v.sublanes.serial.find(B=>B.id===_):void 0;return $?$.raw_length:0}function Me(d,_){let v=J.get(d),$=J.get(_);if(!v||!$)return{kind:"note",text:"\uC0C1\uB300\uC758 \uD604\uC7AC \uC704\uCE58\uB97C \uC54C \uC218 \uC5C6\uC2B5\uB2C8\uB2E4"};let B=Zd(v),Q=Zd($);if(B!==null&&B===Q&&v.root_dir===$.root_dir)return{kind:"note",text:"\uC774\uBBF8 \uAC19\uC740 \uC9C1\uB82C \uB808\uC778 \u2014 \uC21C\uC11C\uAC00 \uC788\uC2B5\uB2C8\uB2E4"};let pe=Xd(v),Oe=Xd($);if(pe&&Q!==null){let Fe=Q;return{kind:"ops",title:`${Fe} \uB05D\uC5D0 ${d}\uB97C \uB123\uC2B5\uB2C8\uB2E4`,root_dir:$.root_dir,ops:[{bead_id:d,lane:Fe,index:$e($.root_dir,Fe)}]}}if(B!==null&&Oe&&Q===null){let Fe=B;return{kind:"ops",title:`${Fe} \uB05D\uC5D0 ${_}\uB97C \uB123\uC2B5\uB2C8\uB2E4`,root_dir:v.root_dir,ops:[{bead_id:_,lane:Fe,index:$e(v.root_dir,Fe)}]}}if(pe&&B===null&&Oe&&Q===null){let Fe=se(v.root_dir);return Fe===null?{kind:"disabled",title:"\uBE48 \uC9C1\uB82C \uB808\uC778 \uC5C6\uC74C \u2014 Worker \uD0ED\uC5D0\uC11C \uB808\uC778 \uC218 \uC870\uC808"}:{kind:"ops",title:`${Fe} \uB808\uC778\uC5D0 ${_} \u2192 ${d} \uC21C\uC11C\uB85C \uB123\uC2B5\uB2C8\uB2E4`,root_dir:v.root_dir,ops:[{bead_id:_,lane:Fe,index:0},{bead_id:d,lane:Fe,index:1}]}}return!pe&&!Oe?{kind:"note",text:"\uB458 \uB2E4 \uC2E4\uD589 \uC911 \u2014 \uC21C\uC11C\uB97C \uB9CC\uB4E4 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4"}:pe?{kind:"note",text:"\uC2E4\uD589 \uC911 \u2014 \uC885\uB8CC \uD6C4 \uCD9C\uBC1C\uD558\uB824\uBA74 \uC9C1\uB82C \uB808\uC778\uC5D0 \uB450\uC138\uC694"}:{kind:"note",text:"\uC2E4\uD589 \uC911 \u2014 \uC21C\uC11C\uB97C \uB9CC\uB4E4\uB824\uBA74 \uC0C1\uB300\uB97C \uC9C1\uB82C \uB808\uC778\uC5D0 \uB450\uC138\uC694"}}function Ne(d,_){let v=Me(d,_.id);return{id:_.id,title:_.title,location_label:_.location_label,prefixes:_.prefixes,action:v.kind==="note"?{kind:"note",text:v.text}:v.kind==="disabled"?{kind:"disabled",label:Yd,title:v.title}:{kind:"place",label:Yd,title:v.title}}}function ze(d,_){if(!z||z.bead_id!==d)return null;let v=z.counterpart_id,$=_.filter(B=>B.id===v);return $.length===0?null:{rows:$.map(B=>Ne(d,B))}}function He(d){let _=d.dependency_chips||null,v=d.overlap_chips||[],$=d.scope_state==="missing",B=d.cross_lane_chip;if(!_&&v.length===0&&!$&&!B)return null;let Q=ze(d.id,v);return{..._||{},...v.length>0?{overlaps:v}:{},...$?{scope_missing:!0}:{},...B?{cross_lane:{lane_id:B.lane_id,label:B.label}}:{},...Q?{popover:Q}:{}}}function at(d){let _=He(d);return _?{...d,dependency_chips:_}:d}async function bt(d,_){let v=Me(d,_);if(z=null,v.kind!=="ops"){_e();return}let $=Gt(v.root_dir,v.ops[0].bead_id);for(let B of v.ops){let Q=await ne(B,v.root_dir,$);if(Q===null)break;$=Q}_e()}async function ne(d,_,v){try{let $=await ve("worker-queue-place",d,_,v,!1);if($&&$.conflict)return le("\uD050\uAC00 \uBC14\uB00C\uC5C8\uC2B5\uB2C8\uB2E4 \u2014 \uB2E4\uC2DC \uC2DC\uB3C4\uD574 \uC8FC\uC138\uC694","error"),null;if(!$||$.applied!==!0)return le($&&typeof $.admission_reason=="string"?`\uD050 \uC801\uC7AC \uAC70\uBD80: ${$.admission_reason}`:"\uD050 \uC694\uCCAD\uC774 \uC801\uC6A9\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4","error"),null;let B=$.queue?$.queue.revision:void 0;return typeof B!="number"?(le("\uD050 \uC751\uB2F5\uC5D0 revision\uC774 \uC5C6\uC2B5\uB2C8\uB2E4","error"),null):B}catch($){return le(we($),"error"),null}}function te(d){let _=Ve(d.root_dir);return l`<header class="mon2-sec__hd">
    ...
    </header>`}function Ie(d,_){return l`<div
      class="mon2-item"
      data-bead-id=${d.id}
    ...
    >
      ${_}
    </div>`}function Qe(d){if(ce!==d.id)return null;let _=G.queue_groups.find(Q=>Q.root_dir===d.root_dir),v=d.place_lanes||[],$=G.cross_lanes_revision!==null,B=[{id:"parallel",label:"\uBCD1\uB82C",count:d.place_index??0}];for(let Q of G.chain_lanes)B.push({id:`lane:${Q.lane_id}`,label:`\uC5F0\uACB0 ${Q.number} (${Q.draft?"draft":"\uD655\uC815"}) \uB05D\uC5D0`,count:Q.rows.length,group:"\uC5F0\uACB0 \uB808\uC778",disabled:!$});B.push({id:"new-lane",label:"+ \uC0C8 \uC5F0\uACB0 \uB808\uC778",group:"\uC5F0\uACB0 \uB808\uC778",disabled:!$,title:$?"\uC774 \uC774\uC288\uB9CC \uB4E0 draft \uB808\uC778\uC744 \uB9CC\uB4ED\uB2C8\uB2E4":"\uC5F0\uACB0 \uB808\uC778 \uC800\uC7A5\uC18C\uB97C \uC77D\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4"});for(let Q of v)B.push({id:`serial:${Q.id}`,label:`\uC9C1\uB82C ${Number(Q.id.slice(1))}`,count:Q.length,group:`${_?_.name:""} \uC9C1\uB82C`});return{bead_id:d.id,lanes:B}}function Le(){let d=[],_=new Set,v=($,B)=>{for(let Q of $)_.has(Q.id)||(_.add(Q.id),d.push({bead_id:Q.id,root_dir:Q.root_dir,workspace_name:Q.workspace_name,title:Q.title,lane:B}))};return v(G.running,"running"),v(G.pr_wait,"pr_wait"),v(G.queue,"queue"),v(G.runnable_all,"runnable"),d}function xe(d){if(!q||q.bead_id!==d)return"";let _=it(),v=Le(),$=new Map;for(let Fe of v)$.set(Fe.bead_id,Fe);let B=(_.get(d)||[]).filter(Fe=>$.has(Fe)),Q=v.filter(Fe=>(_.get(Fe.bead_id)||[]).includes(d)).map(Fe=>Fe.bead_id),pe=Id(Ld(d,q.direction,{issues:v,blocked_by_map:_}),q.query),Oe=G.owner_of[d];return l`<div
      class="mon-deppanel"
      data-bead-id=${d}
      role="dialog"
      aria-label="의존성"
    >
      <div class="mon-deppanel__now">
        ${B.length===0&&Q.length===0?l`<span class="mon-deppanel__empty">연결된 의존 없음</span>`:""}
        ${B.map(Fe=>l`<span class="mon-deppanel__chip mon-deppanel__chip--pred"
            ...
            >`)}
        ${Q.map(Fe=>l`<span class="mon-deppanel__chip mon-deppanel__chip--succ"
    ...
    </div>`}function We(d){return Ie(d,l`${Ci(at(d),Qe(d),{exec_chips_mode:"pinned_only",dep_action:!0,onOpenDoc:i?(_,v)=>i(v,d.root_dir):void 0})}${xe(d.id)}`)}function Ye(){return G.runnable_flat?l`<div class="mon2-flat" data-drop="candidate">
    ...
    >
      ...
      ${xe(d.id)}
    </div>`}function Xe(){let d=dt("parallel");return l`<section
...
`):""}
    ...
    </div>`}function Ge(){let d=dt("serial"),_=G.cross_lanes_revision!==null,v=G.chain_lanes.some($=>$.draft&&$.rows.length===0);return l`<section
    ...
    >
      ...
      <div class="worker-lanes mon2-lanes">
        ${ch.map(v=>{let $=_[v.lane],B=v.lane==="runnable"?G.runnable_flat?$.length>0?Ye():void 0:G.runnable_sections.length>0?Ye():void 0:v.lane==="queue"?G.queue_groups.length>0||G.chain_lanes.length>0||G.parallel_rows.length>0?Ee():void 0:v.lane==="running"?M(d):$.length>0?l`${$.map(Q=>Qn(Q))}`:void 0;return vn({id:`monitor-${v.lane}`,lane:v.pane,title:v.lane==="done"?`\uC644\uB8CC\xB7${C()}`:v.title,items:$,empty:v.empty,body:B,live:v.lane==="running"&&$.length>0,controls:v.lane==="runnable"?ue():void 0,header_control:S(v.lane,$.length)})})}
      ...
      </select>`:""}function K(d){let _=s&&s.get?s.get():null,v=s&&s.getWorkspacesState?s.getWorkspacesState():[],$=d===void 0?s&&s.crossLanes?s.crossLanes():void 0:d,B={done_since:pr(h,p()),running_sort:k,candidate_filter:T,candidate_sort:j};return $!==void 0&&(B.cross_lanes=$),Gi(_,v,B)}function _e(){let d=p();G=K(),J=new Map;for(let _ of[...G.runnable,...G.queue,...G.running,...G.pr_wait,...G.done])!_.non_occupying&&!J.has(_.id)&&J.set(_.id,_);Ke(X(d),F),L()?.render(),x(),w()}function x(){let d=new Map;for(let _ of G.queue_groups)d.set(_.root_dir,_.auto_advance);for(let _ of Array.from(F.querySelectorAll(".mon2-parallel .worker-mini__repo"))){let v=_.closest(".mon2-item")?.getAttribute("data-root-dir")||"",$=d.get(v);typeof $=="boolean"&&_.setAttribute("title",`${_.textContent||""} \xB7 ${$?"\uC790\uB3D9\uD654 \uCF1C\uC9D0":"\uC790\uB3D9\uD654 \uAEBC\uC9D0"}`)}}function L(){if(oe)return oe;let d=F.querySelector(".mon2-deck");return d?(oe=Ad(d,{workspacesState:()=>s&&s.getWorkspacesState?s.getWorkspacesState():[],doneItems:()=>G.done,rangeLabel:C,transport:o,implPresetStore:t.execPresetStore,gotoWorkerTab:V,onFocusChange:_=>{H=_,w()}}),oe):null}function w(){F.classList.toggle("has-focus",H!==null);for(let d of Array.from(F.querySelectorAll(".mon2-sec[data-root-dir]")))d.classList.toggle("is-focus",H!==null&&d.getAttribute("data-root-dir")===H);for(let d of Array.from(F.querySelectorAll(".mon2-item[data-bead-id], .rtile[data-bead-id], .worker-mini[data-bead-id], .worker-card[data-bead-id]"))){let _=J.get(d.getAttribute("data-bead-id")||"");d.classList.toggle("is-focus",H!==null&&!!_&&_.root_dir===H)}for(let d of Array.from(F.querySelectorAll(".mon2-crow[data-root-dir]")))d.classList.toggle("is-focus",H!==null&&d.getAttribute("data-root-dir")===H)}function D(d,_){let v=a?a():void 0;if(!_||!v||_===v||!c){r(d);return}c(_).then(()=>{r(d)}).catch($=>{n("workspace switch for %s failed: %o",_,$)})}function V(d){if(!d)return;let _=a?a():void 0,v=()=>{try{u?.gotoView("worker")}catch($){n("gotoView(worker) failed: %o",$)}};if(!c||_&&_===d){v();return}c(d).then(v).catch($=>{n("workspace switch for %s failed: %o",d,$),le("\uB808\uD3EC \uC804\uD658\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4","error")})}function me(d){fn(d).then(_=>{le(_?"\uBCF5\uC0AC\uB428":"\uBCF5\uC0AC \uC2E4\uD328",_?"success":"error",1400)})}function ae(d){let _=J.get(d)||null;return{item:_,root_dir:_?_.root_dir:"",revision:_?_.expected_revision:0}}function we(d){if(typeof d=="string"&&d.length>0)return d;if(d&&typeof d=="object"){let _=d;if(typeof _.message=="string"&&_.message.length>0)return _.message;if(typeof _.error=="string"&&_.error.length>0)return _.error;if(_.error&&typeof _.error=="object"&&typeof _.error.message=="string")return _.error.message}return"\uC694\uCCAD\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4"}async function rt(d,_,v){let{root_dir:$}=ae(_);if(!(!_||!v||v===_))try{await Be(d,{a:_,b:v},$)}catch(B){le(we(B),"error")}}async function Ze(d,_,v){let $=G.owner_of[_];if(typeof $!="string"||$.length===0){le(`${_}\uC758 \uB808\uD3EC\uB97C \uC54C \uC218 \uC5C6\uC5B4 \uC758\uC874\uC744 \uBC14\uAFC0 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4`,"error");return}try{await Be(d,{a:_,b:v},$)}catch(B){le(we(B),"error")}_e()}function Ce(d){return G.runnable.some(_=>_.id===d)||G.parallel_rows.some(_=>_.id===d)?!0:G.queue_groups.some(_=>_.sublanes.serial.some(v=>v.items.some($=>$.id===d)))}function ut(d,_){!d||!Ce(d)||(q=q&&q.bead_id===d&&_===void 0?null:{bead_id:d,direction:_||"predecessor",query:""},_e())}function it(){let d=new Map,_=s&&s.get?s.get():null,v=$=>Array.isArray($)?$.filter(B=>typeof B=="string"&&B.length>0):[];for(let $ of Array.isArray(_)?_:[]){if(!$||typeof $!="object")continue;let B=$.bead_blocked_by&&typeof $.bead_blocked_by=="object"?$.bead_blocked_by:{};for(let[Q,pe]of Object.entries(B))Array.isArray(pe)&&d.set(Q,v(pe));for(let Q of[...Array.isArray($.runnable)?$.runnable:[],...Array.isArray($.session_active)?$.session_active:[]])Q&&typeof Q.bead_id=="string"&&Array.isArray(Q.blocked_by)&&Q.blocked_by.length>0&&d.set(Q.bead_id,v(Q.blocked_by))}return d}function Ft(){let d=it();for(let _ of N){let v=(d.get(_.a)||[]).slice();_.type==="dep-remove"?d.set(_.a,v.filter($=>$!==_.b)):v.includes(_.b)||d.set(_.a,[...v,_.b])}return d}function jt(d=G){let _=new Map,v=new Map,$=new Set,B=new Set;for(let pe of d.chain_lanes){_.set(pe.lane_id,{status:pe.status,entries:pe.rows.map(Oe=>({bead_id:Oe.id,root_dir:Oe.root_dir}))});for(let Oe of pe.rows)v.set(Oe.id,pe.lane_id),Oe.fixed&&$.add(Oe.id),Oe.unplaced||B.add(Oe.id)}let Q=new Map;for(let pe of d.parallel_rows)typeof pe.queue_index=="number"&&Q.set(pe.id,pe.queue_index);for(let pe of d.queue_groups)for(let Oe of pe.sublanes.serial)for(let Fe of Oe.items)typeof Fe.queue_index=="number"&&Q.set(Fe.id,Fe.queue_index);return{blocked_by_map:Ft(),owner_of:new Map(Object.entries(d.owner_of)),cross_lanes:_,owner_lane_of:v,fixed_members:$,placed_members:B,parallel_rows:d.parallel_rows.map(pe=>({bead_id:pe.id,root_dir:pe.root_dir,queue_index:pe.queue_index??0})),parallel_raw_length:new Map(Object.entries(d.parallel_raw_length)),queue_index_of:Q}}function Gt(d,_){let v=J.get(_);if(v&&v.root_dir===d)return v.expected_revision;let $=G.queue_groups.find(B=>B.root_dir===d);return $?$.revision:0}async function Mt(d,_,v){try{if(d.type==="worker-queue-place"||d.type==="worker-queue-reorder"||d.type==="worker-queue-remove"){let $=await ve(d.type,d.payload,d.root_dir,v.get(d.root_dir)??Gt(d.root_dir,_));return!$||typeof $.applied!="boolean"?(le("\uD050 \uC694\uCCAD\uC774 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4","error"),!1):($.queue&&typeof $.queue.revision=="number"&&v.set(d.root_dir,$.queue.revision),$.conflict?(le("\uD050\uAC00 \uBC14\uB00C\uC5C8\uC2B5\uB2C8\uB2E4 \u2014 \uB2E4\uC2DC \uC2DC\uB3C4\uD574 \uC8FC\uC138\uC694","error"),!1):$.applied===!1?(le($.admission_reason?`\uD050 \uC801\uC7AC \uAC70\uBD80: ${$.admission_reason}`:"\uD050 \uC694\uCCAD\uC774 \uC801\uC6A9\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4","error"),!1):!0)}return(d.type==="dep-add"||d.type==="dep-remove")&&await Be(d.type,{a:d.a,b:d.b},d.root_dir),!0}catch($){return le(we($),"error"),!1}}function Ut(d){(d.type==="dep-add"||d.type==="dep-remove")&&(N=[...N,{type:d.type,a:d.a,b:d.b}])}async function It(d,_){if(!o)return{ok:!1};try{let v=await o(d.type,{...d.payload,expected_revision:_});return!v||typeof v.revision!="number"?(le("\uC5F0\uACB0 \uB808\uC778 \uC751\uB2F5\uC5D0 revision\uC774 \uC5C6\uC2B5\uB2C8\uB2E4","error"),{ok:!1}):{ok:!0,revision:v.revision}}catch(v){let $=v,B=$&&$.code==="conflict"?$.details?.cross_lanes:null;return B&&typeof B.revision=="number"&&Array.isArray(B.lanes)?{ok:!1,conflict:B}:(le(we(v),"error"),{ok:!1})}}async function Kt(d,_,v){let $=new Map,B=d.ops.slice(0,d.lane_op_index),Q=d.ops.slice(d.lane_op_index);for(let Oe of B){if(!await Mt(Oe,v,$))return{done:!0};Ut(Oe)}let pe=_;for(let Oe of d.lane_ops){if(pe===null)return le("\uC5F0\uACB0 \uB808\uC778 \uC800\uC7A5\uC18C\uB97C \uC77D\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4","error"),{done:!0};let Fe=await It(Oe,pe);if(!Fe.ok)return Fe.conflict?{done:!1,conflict:Fe.conflict}:{done:!0};pe=Fe.revision}for(let Oe of Q){if(!await Mt(Oe,v,$))return{done:!0};Ut(Oe)}return{done:!0}}async function qe(d,_){N=[];let v=G;for(let $=0;;$+=1){let B=d(jt(v));if("refused"in B){le(B.refused,"error");break}let Q=await Kt(B,v.cross_lanes_revision,_);if(Q.done)break;if($>=1){le("\uB808\uC778\uC774 \uB2E4\uB978 \uACF3\uC5D0\uC11C \uBC14\uB00C\uC5C8\uC2B5\uB2C8\uB2E4","error");break}v=K(Q.conflict)}N=[],_e()}async function Nt(d,_){await qe(v=>qi(d,_,v),d.bead_id)}async function Yt(d,_){if(d==="create"){await qe(v=>Fi(null,v),"");return}if(d==="remove"){let v=G.chain_lanes.find($=>$.lane_id===_);if(v&&!v.draft){let $=v.rows.filter((B,Q)=>Q===0?!1:!B.mismatch).length;if(!b(`\uC758\uC874 ${$}\uAC1C\uB97C \uD568\uAED8 \uC81C\uAC70\uD569\uB2C8\uB2E4`))return}await qe($=>Od(_,$),"");return}await qe(v=>d==="confirm"?Cd(_,v):Rd(_,v),"")}async function nt(d,_){let v=J.get(d);if(!v){_e();return}let $={kind:"candidate",bead_id:d,root_dir:v.root_dir};if(_==="new-lane"){await qe(B=>Fi({bead_id:d,root_dir:v.root_dir},B),d);return}if(_.startsWith("lane:")){let B=_.slice(5);if(!G.chain_lanes.find(pe=>pe.lane_id===B)){_e();return}await qe(pe=>qi($,{kind:"chain",lane_id:B,marker_index:(pe.cross_lanes.get(B)?.entries??[]).length},pe),d);return}if(_.startsWith("serial:")){let B=_.slice(7),Q=(v.place_lanes||[]).find(pe=>pe.id===B);await Nt($,{kind:"repo-serial",root_dir:v.root_dir,lane_id:B,index:Q?Q.index:0});return}await Nt($,{kind:"parallel",marker_index:G.parallel_rows.length})}async function Pe(d,_){let v=G.parallel_rows,$=v.findIndex(cn=>cn.id===d);if($<0)return;let B=v[$].root_dir,Q=[];v.forEach((cn,Dt)=>{cn.root_dir===B&&Q.push(Dt)});let pe=Q.indexOf($),Oe=Q[pe+_];if(typeof Oe!="number")return;let Fe=_===-1?Oe:Q[pe+2]??Math.min(v.length,Oe+1);await Nt({kind:"parallel",bead_id:d,root_dir:B,queue_index:v[$].queue_index??0},{kind:"parallel",marker_index:Fe})}async function I(d){for(let _ of G.chain_lanes){let v=_.rows.find($=>$.id===d);if(v){await Nt({kind:"chain",bead_id:d,root_dir:v.root_dir,lane_id:_.lane_id,...typeof v.queue_index=="number"?{queue_index:v.queue_index}:{}},{kind:"parallel",marker_index:G.parallel_rows.length});return}}}let fe=null,Re=!1,tt=null;function At(){tt!==null&&clearTimeout(tt),tt=setTimeout(()=>{tt=null,Re=!1},0)}function mt(d,_){let v=_&&typeof _.closest=="function"?_.closest("[data-row-index]"):null;if(v&&d.contains(v)){let $=Number(v.getAttribute("data-row-index"));return Number.isFinite($)?$:0}return d.querySelectorAll("[data-row-index]").length}function Rt(d){let _=d.target,v=typeof _?.closest=="function"?_.closest("[data-drop]"):null;if(!v||!fe)return null;let $=v.getAttribute("data-drop");if($==="candidate")return{zone:v,target:{kind:"candidate"}};if($==="parallel")return{zone:v,target:{kind:"parallel",marker_index:mt(v,_)}};if($==="chain")return{zone:v,target:{kind:"chain",lane_id:v.getAttribute("data-lane-id")||"",marker_index:mt(v,_)}};if($==="repo-serial"){let B=v.getAttribute("data-root-dir")||"";if(B!==fe.root_dir)return null;let Q=typeof _?.closest=="function"?_.closest("[data-queue-index]"):null,pe=Q&&v.contains(Q)?Q.getAttribute("data-queue-index"):v.getAttribute("data-lane-length"),Oe=Number(pe);return{zone:v,target:{kind:"repo-serial",root_dir:B,lane_id:v.getAttribute("data-lane-id")||"",index:Number.isFinite(Oe)?Oe:0}}}return null}function Pt(){for(let d of Array.from(F.querySelectorAll(".is-drop-over")))d.classList.remove("is-drop-over")}function Vt(d){let _=d.target,v=typeof _?.closest=="function"?_.closest('[draggable="true"][data-bead-id]'):null,$=v?v.closest("[data-drag-kind]"):null;if(!$)return;let B=$.getAttribute("data-bead-id")||"",Q=$.getAttribute("data-drag-kind")||"",pe=$.getAttribute("data-root-dir")||"";if(!B||!Q||!pe)return;let Oe=$.getAttribute("data-queue-index")||"",Fe=Number(Oe),cn=$.getAttribute("data-lane-id")||"";fe={kind:Q,bead_id:B,root_dir:pe,...Oe!==""&&Number.isFinite(Fe)?{queue_index:Fe}:{},...cn?{lane_id:cn}:{}},Re=!0,ce=null,F.classList.add("is-dragging");try{d.dataTransfer?.setData("text/plain",B),d.dataTransfer&&(d.dataTransfer.effectAllowed="move")}catch{}}function tn(d){let _=Rt(d);_&&(d.preventDefault(),d.dataTransfer&&(d.dataTransfer.dropEffect="move"),_.zone.classList.add("is-drop-over"))}function wt(d){let _=d.target;typeof _?.closest=="function"&&_.closest("[data-drop]")?.classList.remove("is-drop-over")}function nn(){fe=null,Pt(),F.classList.remove("is-dragging"),At()}function ln(d){let _=Rt(d),v=fe;fe=null,Pt(),F.classList.remove("is-dragging"),!(!_||!v)&&(d.preventDefault(),Nt(v,_.target))}function Ln(d){return{runner:d.runner||void 0,model:d.model||void 0,effort:d.effort||void 0,status:d.run_state==="running"?"running":d.run_state,worktree:d.root_dir}}function R(d,_){let{item:v,root_dir:$,revision:B}=ae(_),Q=v?.attempt_id||"",pe=d.classList;if(pe.contains("worker-dep__remove")){rt("dep-remove",_,d.dataset.blockerId||"");return}if(pe.contains("mon2-rowops__up")||pe.contains("mon2-rowops__down")){Pe(_,pe.contains("mon2-rowops__up")?-1:1);return}if(pe.contains("mon2-rowops__remove")){ve("worker-queue-remove",{bead_id:_},$,B);return}if(pe.contains("mon2-crow__detach")){I(_);return}if(pe.contains("mon-dep__btn")){ut(_);return}if(pe.contains("worker-dep__open")){ut(_,d.getAttribute("data-dep-direction")==="successor"?"successor":"predecessor");return}if(pe.contains("mon-lane__chip")){let Oe=d.getAttribute("data-lane-id")||"";F.querySelector(`.mon2-clane[data-lane-id="${Oe}"]`)?.scrollIntoView({block:"nearest"});return}if(pe.contains("mon-deppanel__unlink")){Ze("dep-remove",d.getAttribute("data-dep-a")||"",d.getAttribute("data-dep-b")||"");return}if(pe.contains("mon-deppanel__seg")){q&&(q={...q,direction:d.getAttribute("data-dep-direction")==="successor"?"successor":"predecessor"},_e());return}if(pe.contains("mon-deppanel__cand")){let Oe=d.getAttribute("data-dep-cand")||"";q&&Oe&&(q.direction==="predecessor"?Ze("dep-add",q.bead_id,Oe):Ze("dep-add",Oe,q.bead_id));return}if(pe.contains("mon-overlap__chip")){let Oe=d.getAttribute("data-overlap-id")||"";z=!!z&&z.bead_id===_&&z.counterpart_id===Oe?null:{bead_id:_,counterpart_id:Oe},_e();return}if(pe.contains("mon-overlap__place")){bt(_,d.getAttribute("data-counterpart-id")||"");return}if(pe.contains("worker-card__place")){ce=ce===_?null:_,_e();return}if(pe.contains("worker-card__place-cancel")){ce=null,_e();return}if(pe.contains("worker-card__place-lane")){let Oe=d.getAttribute("data-lane")||"parallel";ce=null,nt(_,Oe);return}if(pe.contains("rtile__session")){Z=Q,Q&&v&&(re.hidden=!1,Se.open({attempt_id:Q,root_dir:$,meta:Ln(v)})),_e();return}if(pe.contains("rtile__pause")){Be("worker-attempt-pause",{attempt_id:Q},$);return}if(pe.contains("rtile__resume")){Mr().then(Oe=>{if(Oe!==null)return ee("worker-attempt-resume",{attempt_id:Q,...Oe!==""?{instructions:Oe}:{}},$,B)});return}if(pe.contains("rtile__dismiss")){ve("worker-attempt-dismiss",{attempt_id:Q},$,B);return}if(pe.contains("rtile__discard")){if(!b(As(_,"unmerged")))return;ge({bead_id:_,...Q?{attempt_id:Q}:{},...d.dataset.operationId?{operation_id:d.dataset.operationId}:{}},$,B);return}if(pe.contains("worker-mini__merge")){let Oe=Y($,_);Oe?.mismatch&&Oe.continuation===null?ke($,_,B,Oe.mismatch):ve("worker-merge-queue-add",{bead_id:_},$,B);return}if(pe.contains("worker-mini__merge-cancel")){ve("worker-merge-queue-remove",{bead_id:_},$,B);return}if(pe.contains("worker-mini__discard")){let Oe=d.dataset.discardMode==="merged"?"merged":"unmerged";if(!b(As(_,Oe)))return;ge({bead_id:_,...d.dataset.attemptId?{attempt_id:d.dataset.attemptId}:{},...d.dataset.operationId?{operation_id:d.dataset.operationId}:{}},$,B);return}if(pe.contains("worker-mini__revise-fix")){ee("worker-revise-fix",{bead_id:_},$,B);return}pe.contains("worker-mini__revise-approve")&&ve("worker-revise-approve",{bead_id:_},$,B)}function P(d){let _=Re;Re=!1;let v=d.target;if(!v||typeof v.closest!="function"||v.closest("dialog")||v.closest(".worker-drawer-overlay")||v.closest("a"))return;let $=v.closest(".worker-card__id, .worker-mini__id, .rtile__id");if($){d.preventDefault();let sr=v.closest(".mon2-item, .rtile, .mon2-crow, .worker-mini")?.getAttribute("data-bead-id")||$.textContent?.trim()||"";sr&&me(sr);return}let B=v.closest(".worker-mini__repo, .worker-card__repo, .mon2-sec__worker");if(B){d.preventDefault();let Zt=B.getAttribute("data-root-dir")||J.get(v.closest(".mon2-item, .rtile, .worker-mini")?.getAttribute("data-bead-id")||"")?.root_dir||B.getAttribute("title")||"";V(Zt);return}let Q=v.closest(".mon2-sec__toggle");if(Q){d.preventDefault(),st(Q.getAttribute("data-root-dir")||"");return}let pe=v.closest(".mon2-area__toggle");if(pe){d.preventDefault(),O(pe.getAttribute("data-area")||"parallel");return}if(v.closest(".mon2-newlane")){d.preventDefault(),Yt("create","");return}let Oe=v.closest(".mon2-clane__confirm, .mon2-clane__reapply, .mon2-clane__remove");if(Oe){d.preventDefault();let Zt=Oe.getAttribute("data-lane-id")||"";Yt(Oe.classList.contains("mon2-clane__confirm")?"confirm":Oe.classList.contains("mon2-clane__reapply")?"reapply":"remove",Zt);return}if(v.closest(".mon-merge-all")){d.preventDefault(),de();return}let Fe=v.closest(".mon-filter__spec");if(Fe){d.preventDefault(),T={...T,spec:Fe.getAttribute("data-spec")||"all"},Vi(T),_e();return}let cn=v.closest(".mon2-item, .rtile, .mon2-crow, .worker-mini, .worker-card");if(!cn)return;let Dt=cn.getAttribute("data-bead-id")||"",Wn=v.closest("button");if(Wn){d.preventDefault(),R(Wn,Dt);return}Dt&&!_&&(d.preventDefault(),D(Dt,cn.getAttribute("data-root-dir")||ae(Dt).root_dir))}function De(d){let _=d.target;if(!_||typeof _.closest!="function")return;let v=_.closest(".mon-filter__blocked");if(v){T={...T,show_blocked:v.checked},Vi(T),_e();return}let $=_.closest(".mon-filter__deps");if($){T={...T,with_deps:$.checked},Vi(T),_e();return}let B=_.closest(".mon-candidate-sort");if(B){j=Ls.some(Oe=>Oe.value===B.value)?B.value:"repo_spec",nh(j),_e();return}let Q=_.closest(".mon-running-sort");if(Q){k=Q.value==="repo"?"repo":"started",ih(k),_e();return}let pe=_.closest(".mon-done-range");pe&&(h=bn(pe.value)?pe.value:dn,oh(h),_e())}function f(d){let _=d.target,v=_&&typeof _.closest=="function"?B=>_.closest(B):()=>null,$=!1;z&&!v(".mon-overlap__popover, .mon-overlap__chip")&&(z=null,$=!0),q&&!v(".mon-deppanel, .mon-dep__btn, .worker-dep__open")&&(q=null,$=!0),$&&_e()}function y(d){d.key!=="Escape"||!z&&!q||(z=null,q=null,_e())}function m(d){let _=d.target;!_||typeof _.closest!="function"||!_.closest(".mon-deppanel__search")||!q||(q={...q,query:_.value},_e())}e.addEventListener("click",P),e.addEventListener("change",De),e.addEventListener("input",m),document.addEventListener("click",f),document.addEventListener("keydown",y),e.addEventListener("dragstart",Vt),e.addEventListener("dragover",tn),e.addEventListener("dragleave",wt),e.addEventListener("drop",ln),e.addEventListener("dragend",nn),s&&typeof s.subscribe=="function"&&(Ae=s.subscribe(()=>{try{ye.clear(),_e()}catch{}}));function E(){be!==null&&(clearInterval(be),be=null)}function A(){tt!==null&&(clearTimeout(tt),tt=null)}return{load(){n("load"),_e(),be===null&&(be=setInterval(()=>{try{_e()}catch{}},lh))},pause(){E()},clear(){E(),A(),Ae&&(Ae(),Ae=null),Se.destroy(),re.hidden=!0,oe?.destroy(),oe=null,e.removeEventListener("click",P),e.removeEventListener("change",De),e.removeEventListener("input",m),document.removeEventListener("click",f),document.removeEventListener("keydown",y),e.removeEventListener("dragstart",Vt),e.removeEventListener("dragover",tn),e.removeEventListener("dragleave",wt),e.removeEventListener("drop",ln),e.removeEventListener("dragend",nn),e.replaceChildren()}}}function ip(e,t,n){let r=St("views:nav"),{global_element:s,repo_element:o}=e,a=null;function i(h){return k=>{k.preventDefault(),r("click tab %s",h),n.gotoView(h)}}function c(){let h=t.getState();return h.view==="worker"||h.view==="monitor"?h.view:"board"}function u(){let h=c();return l`
      <a
        href="#/monitor"
  ...
  `}var fh=[{id:"execution",label:"\uC2E4\uD589",glyph:"\u25C6"},{id:"display",label:"\uD45C\uC2DC",glyph:"\u25EB"}];function mp(e,t){let{transport:n,policyStore:r,labelOptions:s}=t,o=t.notify||(J=>le(J,"error",4e3)),a=document.createElement("dialog");a.id="settings-dialog",a.className="settings-dialog",a.setAttribute("role","dialog"),a.setAttribute("aria-modal","true"),a.setAttribute("aria-label","\uC124\uC815"),e.appendChild(a);let i="execution",c=!1,u="",p=null;function b(){if(p)return p;let J=a.querySelector('[data-pane="execution"]');return J?(p=Jo(J,{root_dir:null,queue:()=>t.queueStore?.get()??null,transport:n,implPresetStore:t.implPresetStore,notify:o,onQueueAdopt:ye=>t.queueStore?.set?.(ye)}),p):null}function h(){return l`
      <section
    ...
    `}function k(){let J=r.get();return l`
      <section
      ...
      >
        ...
        ${J?l`
              ${pp(J,s(),Z)}
              ${fp(J,u,{onDraft:ye=>{u=ye},onAdd:ce,onRemove:z})}
              ${_p(J,q)}
            `:l`<div class="settings-dialog__empty">
              표시 정책을 불러오는 중…
            </div>`}
      </section>
    `}async function T(J){let ye=r.get();if(ye)try{let Ae=await n("display-policy-set",{expected_revision:ye.revision,policy:J(ye)});j(Ae),Ae&&Ae.conflict&&Ae.policy&&(Ae=await n("display-policy-set",{expected_revision:Ae.policy.revision,policy:J(Ae.policy)}),j(Ae)),Ae&&Ae.conflict&&o("\uD45C\uC2DC \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uD074\uB77C\uC774\uC5B8\uD2B8\uC640 \uCDA9\uB3CC")}catch{o("\uD45C\uC2DC \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328")}}function j(J){J&&J.policy&&typeof J.policy=="object"&&r.set(J.policy)}function W(J){T(J)}function Z(J){let ye=r.get();if(!ye)return;let Ae=!_h(J,ye);W(be=>mh(J,be,Ae))}function ce(){let J=u.trim();J.length!==0&&(u="",W(ye=>ye.hidden_prefixes.includes(J)?{hidden_prefixes:ye.hidden_prefixes}:{hidden_prefixes:[...ye.hidden_prefixes,J]}),N())}function z(J){W(ye=>({hidden_prefixes:ye.hidden_prefixes.filter(Ae=>Ae!==J)}))}function q(J){let ye=r.get();if(!ye)return;let Ae=ye.chips[J]===!1;W(()=>({chips:{[J]:Ae}}))}function N(){Ke(l`
    ...
    </span>`}function N(be,oe,Se){let ve=oe.available&&typeof oe.ageSeconds=="number"&&oe.ageSeconds>bp,Y=ve&&typeof oe.ageSeconds=="number"?`${Math.floor(oe.ageSeconds/60)}\uBD84 \uC804 \uCE21\uC815`:"",ee=oe.accounts.filter(de=>!de.active).length,ke=`usage-meter__group${ve?" usage-meter__group--stale":""}`,ge=l`<span class="usage-meter__provider"
        >${be.label}</span
      >
      ${oe.available?oe.windows.map(de=>q(de,ve,Y,Se)):l`<span class="usage-meter__empty">사용량 없음</span>`}
      ${ee>0?l`<span class="usage-meter__badge">+${ee}</span>`:""}`;if(oe.accounts.length===0)return l`<span
        class=${ke}
        aria-label=${`${be.label} usage`}
        >${ge}</span
      >`;let Be=n===be.key;return l`<button
      type="button"
    ...
    >
      ${ge}
    </button>`}function H(be,oe){return l`<span class="usage-meter" aria-label="Usage">
      ${be.map(Se=>N(Se.provider,Se.snapshot,oe))}
    </span>`}function C(be,oe){let Se=wp(be.pct),ve=yp(be.resetsAt,oe);return l`<span
    ...
    >
      ...
      <span class="usage-meter__account-reset"
        >${ve.length>0?`\u21BB ${ve}`:""}</span
      >
    </span>`}function F(be,oe){return hh.includes(oe)?`\uD1A0\uD070 \uB9CC\uB8CC \u2014 ${be.tool} \uC7AC\uB85C\uADF8\uC778 \uD544\uC694`:"\uC0AC\uC6A9\uB7C9 \uC5C6\uC74C"}function re(be,oe,Se){let ve=oe.status==="ok",Y=typeof oe.ageSeconds=="number"&&oe.ageSeconds>bp,ee=a.get($p(be.key,oe.number)),ke=r.get(be.key),ge=ke!==void 0,Be=ke===oe.number,de=["usage-meter__account"];return oe.active&&de.push("usage-meter__account--active"),ve||de.push("usage-meter__account--unavailable"),Y&&de.push("usage-meter__account--stale"),l`<div class=${de.join(" ")}>
      <div class="usage-meter__account-head">
        ...
        ${oe.ageSeconds===null?"":l`<span class="usage-meter__account-age"
              >${vh(oe.ageSeconds)}</span
            >`}
        ${oe.active?"":l`<button
              type="button"
              class="usage-meter__switch"
              ?disabled=${ge}
              @click=${()=>{z(be,oe.number)}}
            >
              ${Be?"\uC804\uD658 \uC911\u2026":"\uC804\uD658"}
            </button>`}
      </div>
      ${ve?l`<div class="usage-meter__account-windows">
            ${oe.windows.map(Ve=>C(Ve,Se))}
    ...
    </div>`}function Te(be,oe,Se){let ve=oe.accounts.filter(Y=>Y.active).length;return l`<section class="usage-meter__section">
      <h2 class="usage-meter__section-title">
        ${be.label} · 활성 ${ve} / 전체
        ${oe.accounts.length}
      </h2>
      ${oe.accounts.map(Y=>re(be,Y,Se))}
    </section>`}function he(be,oe){return l`<div
      class="usage-meter__card"
      id=${gp}
      role="dialog"
      aria-label=${`${be.provider.label} \uACC4\uC815 \uC0AC\uC6A9\uB7C9`}
    >
      ${Te(be.provider,be.snapshot,oe)}
      <p class="usage-meter__note">전환은 새로 시작하는 세션부터 적용됩니다.</p>
    </div>`}function G(){let be=[];for(let ve of kp){let Y=o.get(ve.key);Y&&be.push({provider:ve,snapshot:Y})}if(be.length===0){k(),u();return}let oe=be.find(ve=>ve.provider.key===n&&ve.snapshot.accounts.length>0);oe||k();let Se=Date.now();Ke(H(be,Se),e),e.hidden=!1,oe?J(oe,Se):b()}function J(be,oe){let Se=p(),ve=e.getBoundingClientRect(),Y=e.ownerDocument.documentElement.clientWidth;Se.style.setProperty("--usage-meter-anchor-top",`${ve.bottom}px`),Se.style.setProperty("--usage-meter-anchor-right",`${Math.max(0,Y-ve.right)}px`),Ke(l`<div
          class="usage-meter__scrim"
          aria-hidden="true"
          @mousedown=${ce}
        ></div>
        ${he(be,oe)}`,Se)}async function ye(be){try{let oe=await fetch(be.endpoint);return oe.ok?kh(await oe.json()):null}catch{return null}}async function Ae(){i+=1;let be=i,oe=await Promise.all(kp.map(async Se=>({provider:Se,snapshot:await ye(Se)})));if(!(t||be!==i)){for(let Se of oe)Se.snapshot?o.set(Se.provider.key,Se.snapshot):o.delete(Se.provider.key);G()}}return u(),Ae(),s=setInterval(()=>{Ae()},6e4),{destroy(){t=!0,s!==null&&(clearInterval(s),s=null),k(),u()}}}function Sp(e){let t=e.attempts?Object.values(e.attempts):[],n=new Map;for(let s of t)s&&(s.kind??"implementation")==="implementation"&&n.set(s.bead_id,s.attempt_id);let r=new Map;for(let s of e.done||[])s&&typeof s.bead_id=="string"&&typeof s.added_at=="number"&&r.set(s.bead_id,s.added_at);return s=>{let o=n.get(s.bead_id)!==s.attempt_id,a=r.get(s.bead_id),i=typeof a=="number"&&a>0&&typeof s.finished_at=="number"&&a>=s.finished_at;return!o&&!i&&typeof s.dismissed_at!="number"}}var $h="worker-ineligible";function Ki(e){return Array.isArray(e)?e.filter(t=>typeof t=="string"):[]}function Ep(e){return Ki(e).includes($h)}var xh="worker-serial";function Yi(e){return Ki(e).includes(xh)}function Zi(e,t,n){if(typeof t!="string"||typeof n!="string")return[];let r=e?.runners;if(!r||!Object.hasOwn(r,t))return[];let s=r[t],o=s?.models;if(!o||!Object.hasOwn(o,n))return[];let a=o[n]?.efforts;return Array.isArray(a)?a.slice():Array.isArray(s.efforts)?s.efforts.slice():[]}var Ah=new Set(["done","failed","orphaned","stopped","discarded"]),Sh={spec_missing:"\uC2A4\uD399 \uC5C6\uC74C",route:"route \uBBF8\uB2EC",spec_review:"\uC2A4\uD399 \uB9AC\uBDF0 \uC5C6\uC74C",spec_conflict:"\uC2A4\uD399 \uCDA9\uB3CC",phase_child:"phase child",worker_ineligible:"worker \uC81C\uC678"},Eh={running:"\uC2E4\uD589 \uC911",success:"\uC131\uACF5",failure:"\uC2E4\uD328",cancelled:"\uCDE8\uC18C",interrupted:"\uC911\uB2E8"},Th={running:"running",success:"done",failure:"failed",cancelled:"stopped",interrupted:"orphaned"};function Xi(e){return{runner:e.runner||void 0,model:e.model||void 0,effort:e.effort||void 0,status:Th[e.outcome]||(typeof e.job_id=="string"?"running":void 0),session_id:e.session_id||void 0}}function Tp(e,t){let{queueStore:n,analysisStore:r,transport:s,getWorkspacePath:o,onOpenTranscript:a}=t,i=document.createElement("dialog");i.id="worker-parallel-analysis-dialog",i.className="pa",i.setAttribute("role","dialog"),i.setAttribute("aria-modal","true"),e.appendChild(i);let c=new Map,u=new Map,p=!1,b=null,h=null,k=null,T=new Set,j=!1,W=0,Z=null,ce=new Set;function z(){return n&&n.get()||{revision:0,queue:[],serial_lanes:[],serial_lane_count:0,attempts:{},pr_wait:[]}}function q(){return r&&r.get()||{settings:{revision:0,runner:null,model:null,effort:null},job:null,runs:[],last_good:null}}function N(){return o&&o()||""}async function H(){if(!s)return;let x=++W;j=!0,k=null,T.clear(),Ge();try{let L=await s("worker-parallel-analysis-targets",{root_dir:N()});if(x!==W||!Ee)return;let w=Array.isArray(L?.qualified)?L.qualified:[],D=Array.isArray(L?.excluded)?L.excluded:[];k={qualified:w,excluded:D};for(let V of w)V&&typeof V.id=="string"&&T.add(V.id)}catch{x===W&&Ee&&(k={qualified:[],excluded:[]},le("\uBD84\uC11D \uB300\uC0C1\uC744 \uBD88\uB7EC\uC624\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4","error",2800))}finally{x===W&&(j=!1,Ee&&Ge())}}function C(x){return Array.isArray(x.runs)?x.runs:[]}function F(){let x=z(),L=new Set;for(let w of Object.values(x.attempts||{})){let D=w;D&&typeof D.bead_id=="string"&&!Ah.has(D.status)&&L.add(D.bead_id)}for(let w of Array.isArray(x.pr_wait)?x.pr_wait:[])w&&typeof w.bead_id=="string"&&L.add(w.bead_id);for(let w of Object.values(x.discard_operations||{})){let D=w;D&&D.phase!=="done"&&typeof D.bead_id=="string"&&L.add(D.bead_id)}return L}function re(x){return x.filter(L=>Te(L)===null)}function Te(x){let L=z();for(let w of Array.isArray(L.serial_lanes)?L.serial_lanes:[])if(Array.isArray(w?.entries)&&w.entries.some(D=>D.bead_id===x))return w.id;return(Array.isArray(L.queue)?L.queue:[]).some(w=>w.bead_id===x)?"parallel":null}function he(x,L){let w=c.get(x);return w||[...L.order]}function G(x){if(x.length<2)return!1;let L=Te(x[0]);if(!L||L==="parallel")return!1;let w=z(),D=(Array.isArray(w.serial_lanes)?w.serial_lanes:[]).find(me=>me.id===L)?.entries.map(me=>me.bead_id);if(!Array.isArray(D))return!1;let V=x.map(me=>D.indexOf(me));return V.every(me=>me>=0)&&V.every((me,ae)=>ae===0||me>V[ae-1])}function J(){let x=z(),L=Array.isArray(x.serial_lanes)?x.serial_lanes:[],w=L.find(D=>Array.isArray(D.entries)&&D.entries.length===0);return w?w.id:L[0]?.id||"s1"}function ye(x){let L=z().bead_titles||{};return typeof L[x]=="string"?L[x]:x}async function Ae(x,L){if(!s||p)return null;p=!0,Ge();try{return await s(x,L)}finally{p=!1,Ge()}}async function be(x){r?.setPending?.(!0);try{let L=await Ae("worker-parallel-analysis-start",{force:x,target_ids:Array.from(T)});L&&L.applied===!1&&L.reason&&(L.reason==="target_not_qualified"&&Array.isArray(L.detail)?le(`\uBD84\uC11D \uB300\uC0C1 \uC790\uACA9 \uBCC0\uACBD: ${L.detail.join(", ")}`,"error",3200):le(`\uBD84\uC11D \uC2E4\uD328: ${L.reason}`,"error",2800))}finally{r?.setPending?.(!1)}}async function oe(){let x=q().job;!s||!x||await s("worker-parallel-analysis-cancel",{job_id:x.job_id})}async function Se(x){if(!(!s||ce.has(x))){ce.add(x),Ge();try{let L=await s("worker-parallel-analysis-prompt",{root_dir:N(),run_id:x});if(!Ee)return;if(L?.ok===!0&&typeof L.prompt=="string"){Z={run_id:x,prompt:L.prompt};return}le(L?.reason==="not_found"?"\uC800\uC7A5\uB41C \uBD84\uC11D \uD504\uB86C\uD504\uD2B8\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4":"\uBD84\uC11D \uD504\uB86C\uD504\uD2B8\uB97C \uBD88\uB7EC\uC624\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4","error",2800)}finally{ce.delete(x),Ge()}}}function ve(){Z=null,Ge()}async function Y(){if(!Z)return;let x=await fn(Z.prompt);le(x?"\uBCF5\uC0AC\uB428":"\uBCF5\uC0AC \uC2E4\uD328",x?"success":"error",1400)}function ee(x,L){a&&a(x,Xi(L))}function ke(){return z().runner_catalog}function ge(x){return Object.keys(ke()?.runners?.[x]?.models||{})}function Be(x){let L=ge(x),w=ke()?.runners?.[x]?.default_model;return typeof w=="string"&&L.includes(w)?w:L[0]||""}function de(){let x=q().settings,L=b||x.runner||"claude",w=ge(L),D=b?Be(L):x.model||w[0]||"",V=Zi(ke(),L,D),me=x.effort||"",ae=V.includes(me)?me:V[0]||"";return{runner:L,model:D,effort:ae,models:w,efforts:V}}async function Ve(x){let L=q().settings,w=await Ae("worker-parallel-analysis-settings-update",{expected_revision:L.revision,runner:x.runner,model:x.model,effort:x.effort});(!w||w.applied!==!0)&&(b=null,Ge(),w&&w.reason&&le(`\uBD84\uC11D \uC124\uC815 \uAC70\uBD80: ${w.reason}`,"error",2800))}function st(x){b=x,Ge();let L=de();Ve({runner:x,model:L.model,effort:L.effort})}function dt(x){let L=de(),w=Zi(ke(),L.runner,x);Ve({runner:L.runner,model:x,effort:w.includes(L.effort)?L.effort:w[0]||""})}function O(x){let L=de();Ve({runner:L.runner,model:L.model,effort:x})}async function se(x,L){if(!s||p)return;let w=he(x,L),D=q();if(w.length<2||!D.last_good){le("\uC81C\uCD9C\uD558\uB824\uBA74 2\uAC1C \uC774\uC0C1\uC774\uC5B4\uC57C \uD569\uB2C8\uB2E4","warning");return}let V=u.get(x)||J(),me=()=>({snapshot_digest:D.last_good.identity_digest,group_index:x,lane:V,ordered_bead_ids:w,expected_revision:z().revision});p=!0,Ge();try{let ae=await s("worker-parallel-analysis-submit",me());ae&&ae.queue&&n&&n.set(ae.queue),ae&&ae.applied!==!0&&ae.conflict===!0&&(ae=await s("worker-parallel-analysis-submit",me()),ae&&ae.queue&&n&&n.set(ae.queue)),ae&&ae.applied===!0?(c.delete(x),le(`\uC9C1\uB82C \uB808\uC778 ${V}\uC5D0 ${w.length}\uAC1C \uBC30\uCE58`,"success")):le(`\uC81C\uCD9C \uAC70\uBD80: ${ae?.reason||"conflict"} (\uD050 \uBB34\uBCC0\uACBD)`,"error",2800)}finally{p=!1,Ge()}}function $e(x,L,w){c.set(x,he(x,L).filter(D=>D!==w)),Ge()}function Me(x){c.delete(x),Ge()}function Ne(x,L,w,D){let V=[...he(x,L)],me=V.indexOf(w),ae=me+D;me<0||ae<0||ae>=V.length||(V.splice(ae,0,...V.splice(me,1)),c.set(x,V),Ge())}function ze(){let x=q().settings,L=Object.keys(ke()?.runners||{}),w=de();return l`<div class="pa-settings">
      <label class="pa-settings__field"
        >러너
        <select
          class="pa-settings__runner"
          aria-label="분석 러너"
          @change=${D=>st(D.target.value)}
        >
          ${L.map(D=>l`<option
                value=${D}
                ?selected=${w.runner===D}
      ...
      >`:x.is_default===!0?l`<span class="pa-settings__default">기본값</span>`:""}function at(x){return x.is_default===!0&&x.compatible===!1}function bt(x){return!!(x.runner&&x.model&&x.effort)}function ne(x){return bt(x)&&x.compatible!==!1}function te(x){let L=Math.max(0,Math.floor(x/1e3)),w=Math.floor(L/60),D=L%60;return`${w}:${String(D).padStart(2,"0")}`}function Ie(x){let L=x.job;if(L){let w=typeof L.started_at=="number"?L.started_at:0,D=`${L.runner||"?"}/${L.model||"?"}`,V=w?` \xB7 \uACBD\uACFC ${te(Date.now()-w)}`:"",me=typeof L.session_id=="string"?L.session_id:"",ae=C(x).find(we=>we.run_id===L.job_id);return l`<span class="pa-meta__progress">
        <span
        ...
        >`:""}function Qe(x){let L=Ie(x);return L===""?"":l`<div class="pa__strip">${L}</div>`}function Le(){return r?.isPending?.()===!0}function xe(x){let L=!!x.job,w=ne(x.settings),D=k!==null&&T.size===0,V=L||p||Le()||j;return l`<div class="pa-meta">
...
${Z.prompt}</pre
    ...
    </div>`:""}function _t(x,L){let w=he(x,L),D=F(),V=w.filter(Ce=>D.has(Ce)),me=re(w),ae=G(w),we=Array.isArray(z().serial_lanes)?z().serial_lanes:[],rt=u.get(x)||J(),Ze=L.eligible!==!0||w.length<2||V.length>0||me.length>0||ae||p;return l`<section class="pa-group" data-group-index=${String(x)}>
...
`):[];return l`<div
    ...
    ></button>`}function k(){let C=o().repo_ops_opt_out;return{verify:C?.verify===!0,deploy:C?.deploy===!0}}function T(C,F){return l`<label class="worker-repo-ops__lane-run">
      ...
      />
      이 workspace에서 실행
    </label>`}function j(C){let F=typeof C.base_sha=="string"?C.base_sha:"",re=`${C.source_path||"repo-ops/config.toml"} @ ${C.base_ref||"?"}${F?`@${F.slice(0,7)}`:""}`,Te=k(),he=!!C.verify&&Te.verify,G=!!C.deploy&&Te.deploy;return l`<section class="worker-repo-ops__vd" data-seam="repo-ops">
      ...
      >
        ...
        <span class="worker-repo-ops__lane-d"
          >${he?"\uC774 workspace\uC5D0\uC11C\uB294 \uAC80\uC99D \uC5C6\uC774 \uD310\uC815\uD569\uB2C8\uB2E4.":C.verify?"\uBA38\uC9C0 \uC804\uC5D0 \uC774 \uC2A4\uD06C\uB9BD\uD2B8\uAC00 \uD1B5\uACFC\uD574\uC57C \uC790\uACA9\uC744 \uC5BB\uC2B5\uB2C8\uB2E4.":"\uBA38\uC9C0 \uC790\uACA9\uC740 PR/base/head \uC2E0\uC120\uB3C4\xB7mergeability\xB7\uB9AC\uBDF0 \uC601\uC218\uC99D\uC73C\uB85C\uB9CC \uD310\uC815\uD569\uB2C8\uB2E4."}</span
        >
    ...
    </div>`}function H(){let C=o(),F=C.auto_repair!==!1,re=C.repo_operation_policy&&typeof C.repo_operation_policy=="object"?C.repo_operation_policy:null,Te=Array.isArray(C.repo_operations)?C.repo_operations:[],he=Te.find(Ae=>Ae.state==="repairing"),G=Te.filter(Ae=>Ae.state==="failed"||Ae.state==="repairing"),J=G.length?Math.min(...G.map(Ae=>typeof Ae.repair?.remaining=="number"?Ae.repair.remaining:0)):re?.auto_repair?.resolution_ladder?.find(Ae=>Ae.id==="auto_repair_session")?.attempts??1,ye=Array.isArray(re?.auto_repair?.resolution_ladder)?re.auto_repair.resolution_ladder:[];return l`<section
...
${Et.notes_tail}`:"notes\uC758 REVISE finding\uC744 \uC2A4\uD399\uC5D0 \uBC18\uC601\uD558\uB294 \uCC98\uBD84 \uC138\uC158\uC744 \uB744\uC6C1\uB2C8\uB2E4":"",usage:U==="done"?hn(f.attempts||{},ie.bead_id):null,work_ms:U==="done"?zo(f.attempts||{},ie.bead_id):null,done_at:U==="done"&&typeof ie.added_at=="number"?ie.added_at:void 0,exec_chips:Je?Oe(ie.bead_id):null,workflow:Je&&zn[ie.bead_id]||null,from_id:Wn.get(ie.bead_id)||void 0,priority:Zt.get(ie.bead_id),...ar(ie.bead_id)}}),xr=f.attempts?Object.values(f.attempts).filter(Hr):[],ba=new Set;for(let g of xr)g&&typeof g.resumed_from=="string"&&g.resumed_from.length>0&&ba.add(g.resumed_from);let ll=new Map;for(let g of xr)ll.set(g.bead_id,g.attempt_id);let Kr=new Map;for(let g of xr)Kr.set(g.attempt_id,g);function ha(g){let U=new Set,ie=g;for(;ie&&!U.has(ie.attempt_id);){if(ie.conflict_resolution===!0)return!0;U.add(ie.attempt_id),ie=typeof ie.resumed_from=="string"&&ie.resumed_from.length>0&&Kr.get(ie.resumed_from)||null}return!1}let Ms=typeof f.declared_base=="string"?f.declared_base:null;function bf(g){let U=null;for(let ie of xr)!ie||ie.bead_id!==g||ha(ie)||(U===null||(typeof ie.started_at=="number"?ie.started_at:0)>=(typeof U.started_at=="number"?U.started_at:0))&&(U=ie);return U&&typeof U.target_base=="string"?U.target_base:null}let ya=[],Ns=[],hf=Sp(f),cl=g=>{let U=typeof g.session_id=="string"&&g.session_id.length>0,ie=ba.has(g.attempt_id);return{eligible:U&&!ie,reason:U?ie?"\uC774\uBBF8 \uC774\uC5B4\uBC1B\uC740 attempt (child attempt \uC874\uC7AC) \u2014 \uC774\uC5B4\uD558\uAE30 \uBD88\uAC00":null:"session_id \uC5C6\uB294 \uAD6C attempt \u2014 \uC774\uC5B4\uD558\uAE30 \uBD88\uAC00"}},wn=null;for(let g of xr){let U=g.status==="paused"&&!ba.has(g.attempt_id);if(g.status==="running"||U)Ns.push({bead_id:g.bead_id,attempt_id:g.attempt_id,title:Dt.get(g.bead_id)||g.bead_id,runner:g.runner||null,model:g.model||null,effort:g.effort||null,speed:g.speed||null,continuation_mode:g.continuation_mode||null,started_at:typeof g.started_at=="number"?g.started_at:null,resumed_from:g.resumed_from||null,paused:U,conflict_resolution:ha(g),base_exception:el(Ms,g.target_base),can_pause:typeof g.session_id=="string"&&g.session_id.length>0,discard:En(Ds,g.bead_id,{attempt_id:g.attempt_id}),workflow:zn[g.bead_id]||null,priority:Zt.get(g.bead_id),usage:hn(f.attempts||{},g.bead_id),rollup:Fe(g.bead_id),rollup_expanded:Ae.has(g.bead_id),exec_chips:Q(g),...ar(g.bead_id)});else if((g.status==="failed"||g.status==="orphaned")&&hf(g)){let ie=cl(g);ya.push({bead_id:g.bead_id,attempt_id:g.attempt_id,title:Dt.get(g.bead_id)||g.bead_id,runner:g.runner||null,model:g.model||null,effort:g.effort||null,speed:g.speed||null,continuation_mode:g.continuation_mode||null,started_at:typeof g.started_at=="number"?g.started_at:null,resumed_from:g.resumed_from||null,failed:!0,status:g.status,status_label:g.status==="orphaned"?"\uC911\uB2E8\uB428":"\uC2E4\uD328",discard:En(Ds,g.bead_id,{attempt_id:g.attempt_id}),resume_eligible:ie.eligible,resume_reason:ie.reason,conflict_resolution:ha(g),base_exception:el(Ms,g.target_base),workflow:zn[g.bead_id]||null,priority:Zt.get(g.bead_id),usage:hn(f.attempts||{},g.bead_id),rollup:Fe(g.bead_id),rollup_expanded:Ae.has(g.bead_id),exec_chips:Q(g),...ar(g.bead_id)}),wn=g}}let ul=new Set([...ya,...Ns].map(g=>g.bead_id));for(let g of Array.isArray(f.session_active)?f.session_active:[]){let U=g&&g.bead_id;typeof U!="string"||U.length===0||ul.has(U)||(ul.add(U),Ns.push({bead_id:U,attempt_id:null,kind:"session",title:g.title||Dt.get(U)||U,status:"in_progress",started_at:Cn(g.started_at)??Cn(g.updated_at),updated_at:Cn(g.updated_at),workflow:g.workflow||null,priority:Zt.get(U),runner:null,model:null,effort:null,speed:null,continuation_mode:null,resumed_from:null,paused:!1,can_pause:!1,conflict_resolution:!1,base_exception:null,discard:null,exec_chips:null,usage:null,rollup:null,rollup_expanded:!1}))}let Ar=[...ya,...Ns].map(g=>{let U=Kr.get(g.attempt_id),ie=U?.quickfix_landing;if(U?.quickfix_lane!==!0||!ie||typeof ie!="object")return g;let Ue=typeof ie.reason=="string"&&ie.reason.length>0?ie.reason:null,Je=Rs({bead_id:U.bead_id,merge_sha:ie.head_sha,cleanup_cursor:ie.cursor,cleanup_failed:Ue?{step:ie.cursor,reason:Ue}:null,repo_operations:Array.isArray(f.repo_operations)?f.repo_operations:[]});return Je?{...g,landing:Je}:g}),dl=null;if(wn){let g=cl(wn),U=wn.cause_detail;dl={bead_id:wn.bead_id,repo:wn.repo||"",reason:wn.cause||wn.status,cause_detail:U&&typeof U.reason=="string"?{reason:U.reason,command:typeof U.command=="string"?U.command:null}:null,resume_attempt_id:wn.attempt_id,resume_eligible:g.eligible,resume_reason:g.reason,discard:En(Ds,wn.bead_id,{attempt_id:wn.attempt_id})}}let pl=new Set(Ar.map(g=>g.bead_id)),va=Array.isArray(f.merge_queue)?f.merge_queue:[],fl=new Map,_l=new Map,ml=new Map,gl=new Map,bl=new Map;va.forEach((g,U)=>{g&&typeof g.bead_id=="string"&&(fl.set(g.bead_id,U+1),_l.set(g.bead_id,g.resolution),ml.set(g.bead_id,g.continuation_action||null),gl.set(g.bead_id,g.head_review||null),bl.set(g.bead_id,g.authority||null))});let Sr=f.merge_queue_state||{active:null,failures:{}},yf=Sr.failures||{},hl=Sr.waiting&&typeof Sr.waiting.bead_id=="string"&&typeof Sr.waiting.reason=="string"?Sr.waiting:null,vf=f.auto_merge_skips||{},yl=g=>{let U=vf[g];if(!U)return null;let ie=Vr[g],Ue=ie&&ie.pr?ie.pr.head_sha:null;return Ue&&Ue===U.head_sha?U.reason||"":null},qs=new Map;for(let g of Ar)g.failed!==!0&&g.conflict_resolution&&(g.paused?qs.has(g.bead_id)||qs.set(g.bead_id,"paused"):qs.set(g.bead_id,"running"));let vl=Ar.filter(g=>g.kind!=="session"&&!g.paused&&g.failed!==!0).length,wl=(f.workspace_info||{}).slots,kl=typeof wl=="number"?wl:typeof f.slots=="number"?f.slots:da,wf=vl>kl,Fs=pr(H),kf=(Array.isArray(f.done)?f.done.slice():[]).filter(g=>Fs===void 0||typeof g.added_at!="number"||g.added_at>=Fs).sort((g,U)=>(U.added_at||0)-(g.added_at||0)),Yr=ga(kf,"done"),$f=new Set((Array.isArray(f.done)?f.done:[]).map(g=>g?.bead_id).filter(g=>typeof g=="string")),$l=[],xf=u?.()||"";for(let g of E){let U=Cn(g.closed_at);if(typeof g.id!="string"||$f.has(g.id)||U===null||Fs!==void 0&&U<Fs||typeof g.comment_count!="number"||g.comment_count<=0)continue;let ie=`${xf}\0${g.id}\0${String(g.updated_at)}\0${g.comment_count}`,Ue=C.get(ie);Ue===void 0&&n&&(C.set(ie,"pending"),Promise.resolve(n("get-comments",{id:g.id})).then(Je=>{let kt=Array.isArray(Je)&&Je.some(Et=>To(typeof Et?.text=="string"?Et.text:"")?.lane==="session");C.set(ie,kt?"session":"not-session"),qe()}).catch(()=>{C.set(ie,"failed"),qe()})),Ue==="session"&&$l.push({id:g.id,title:g.title||g.id,reason:"",draggable:!1,done:!0,lane:"done",selectable:!1,selected:!1,worker_serial:!1,badges:["\uC138\uC158 \uC791\uC5C5"],alert:!1,usage:null,work_ms:null,done_at:U,created_at:g.created_at,updated_at:g.updated_at})}Yr.push(...$l),Yr.sort((g,U)=>(U.done_at||0)-(g.done_at||0));let js={};for(let g of Mn)js[g]=0;let xl=!1,Al=0,wa=0,Sl=0;for(let g of Yr){let U=g.usage;if(U&&typeof U=="object"){let ie=!1;for(let Ue of Mn)Number.isFinite(U[Ue])&&(js[Ue]+=U[Ue],xl=!0,ie=!0);ie&&(wa+=1,Number.isFinite(U.total_cost_usd)&&(Al+=U.total_cost_usd,Sl+=1))}}wa>0&&Sl===wa&&(js.total_cost_usd=Al);let El=Yr.map(g=>g.usage).filter(g=>g&&typeof g=="object"&&g.providers),Af=El.length>0?zt(fo(El)):xl?Nn(js):null,Tl=f.lane_states&&typeof f.lane_states=="object"&&!Array.isArray(f.lane_states)?f.lane_states:{},Cl=Array.isArray(f.serial_lanes)?f.serial_lanes:[],Rl=g=>{if(Gn.some(Ue=>Ue.bead_id===g))return"PR \uB300\uAE30 \xB7 \uC810\uC720";let U=xr.filter(Ue=>Ue&&Ue.bead_id===g),ie=U.length>0?U[U.length-1].status:null;return ie==="failed"||ie==="orphaned"?"\uC2E4\uD328 \xB7 \uC810\uC720 \uC720\uC9C0":ie==="paused"?"\uC77C\uC2DC\uC815\uC9C0 \xB7 \uC810\uC720":"\uC2E4\uD589 \uC911 \xB7 \uC810\uC720"},Bs=Cl.filter(g=>g&&typeof g.id=="string"&&Array.isArray(g.entries)).map((g,U)=>{let ie=Tl[g.id]||{},Ue=new Map((Array.isArray(ie.corrections)?ie.corrections:[]).filter(gt=>gt&&typeof gt.bead_id=="string"&&typeof gt.after=="string").map(gt=>[gt.bead_id,gt.after])),Je=ga(g.entries.filter(gt=>!pl.has(gt.bead_id)),g.id).map(gt=>Ue.has(gt.id)?{...gt,badges:[`\u{1F517} ${Ue.get(gt.id)} \uB4A4 (blocks \uC790\uB3D9)`,...gt.badges]}:gt),kt=Array.isArray(ie.occupied_by)?ie.occupied_by.filter(gt=>typeof gt=="string"):[],Et=kt.map(gt=>({id:gt,title:Dt.get(gt)||gt,draggable:!1,lane:g.id,ghost:!0,badges:[Rl(gt)]}));return{id:g.id,index:U+1,rows:[...Et,...Je],occupied:kt.length>0,badge:kt.length>0?Rl(kt[0]):"\uB300\uAE30",cycle:ie.cycle===!0}}),Ol=typeof f.serial_lane_count=="number"?f.serial_lane_count:Bs.length,ka=ga(fa.filter(g=>!pl.has(g.bead_id)),"queue"),Ll=new Map,Il=new Set;for(let[g,U]of Object.entries(Tl)){if(!/^s[1-5]$/.test(g))continue;let ie=U&&Array.isArray(U.occupied_by)?U.occupied_by:[];for(let Ue of ie)typeof Ue=="string"&&Ll.set(Ue,g);ie.length>0&&Il.add(g)}let Er=[];for(let g of Ar)typeof g.bead_id=="string"&&Er.push({id:g.bead_id,title:Dt.get(g.bead_id)||g.bead_id,location_label:"\uC2E4\uD589\uC911",kind:"running",lane_id:Ll.get(g.bead_id)??null});for(let g of Bs)for(let U of g.rows)U.ghost!==!0&&Er.push({id:U.id,title:U.title,location_label:`${g.id} #${U.seq??""}`.trim(),kind:"serial",lane_id:g.id});ka.forEach((g,U)=>{Er.push({id:g.id,title:g.title,location_label:`#${U+1}`,kind:"parallel",lane_id:null})});for(let g of ma)Er.push({id:g.id,title:g.title,location_label:"\uD6C4\uBCF4",kind:"candidate",lane_id:null});let Pl={};for(let g of Cl)g&&typeof g.id=="string"&&Array.isArray(g.entries)&&(Pl[g.id]=g.entries.length);let $a=new Map;for(let g of Er)$a.has(g.id)||$a.set(g.id,g);q={members_by_id:$a,serial_raw_lengths:Pl,serial_lane_count:Ol,occupied_lanes:Il};let Dl=Cp(f.bead_scope,Er),xa=(g,U)=>{let ie=Dl.get(g.id);if(!ie||ie.overlaps.length===0&&!ie.scope_missing)return g;let Ue=ae(g.id,ie.overlaps);return g.dependency_chips={...g.dependency_chips||{},...ie.overlaps.length>0?{overlaps:ie.overlaps}:{},...ie.scope_missing&&U!=="running"?{scope_missing:!0}:{},...Ue?{popover:Ue}:{}},g};for(let g of ka)xa(g,"queue");for(let g of Bs)for(let U of g.rows)U.ghost!==!0&&xa(U,g.id);for(let g of ma)xa(g,"candidate");let Ml=new Map;for(let g of Ar){let U=typeof g.bead_id=="string"?g.bead_id:"";if(U.length===0)continue;let ie=g.kind==="session",Ue=Dl.get(U),Je=Ue&&Ue.overlaps.length>0?Ue.overlaps:null,kt=typeof g.attempt_id=="string"&&g.attempt_id.length>0?Kr.get(g.attempt_id):void 0,Et=kt&&kt.last_activity&&typeof kt.last_activity=="object"?kt.last_activity:null,gt=kt&&Array.isArray(kt.legs)?kt.legs:[];if(!Je&&!Et&&gt.length===0&&!ie)continue;let rn=Je?ae(U,Je):null;Ml.set(U,{...Et?{last_activity:Et}:{},...gt.length>0?{legs:gt}:{},...Je?{dependency_chips:{overlaps:Je,...rn?{popover:rn}:{}}}:{}})}return{queue:f,idToTitle:Dt,candidates:ma,candidate_hidden:{blocked:_a.hidden_blocked,spec:_a.hidden_spec},running:Ar,live_count:vl,slots:kl,over_cap:wf,failure:dl,waiting:ka,serial_lanes:Bs,serial_lane_count:Ol,running_overlays:Ml,pr_wait:Gn.map(g=>$y(g.bead_id,Dt.get(g.bead_id)||g.bead_id,Vr,lt[g.bead_id]||null,hn(f.attempts||{},g.bead_id),je[g.bead_id]||(he.has(g.bead_id)||G.has(g.bead_id)?{activity:null,merge_progress:{step:"merging"}}:null),qs.get(g.bead_id)||null,g.external===!0,{position:fl.get(g.bead_id)||0,active:Sr.active===g.bead_id,failure:yf[g.bead_id]||null,waiting:hl?.bead_id===g.bead_id?hl.reason:null,resolution:_l.get(g.bead_id),continuation_action:ml.get(g.bead_id),head_review:gl.get(g.bead_id)||null,authority:bl.get(g.bead_id)||null},g.wt_present!==!1,f.auto_merge===!0?yl(g.bead_id):null,el(Ms,bf(g.bead_id)),f.completion_status&&typeof f.completion_status=="object"&&!Array.isArray(f.completion_status)&&f.completion_status[g.bead_id]||null,f.discard_operations&&typeof f.discard_operations=="object"&&!Array.isArray(f.discard_operations)?f.discard_operations:{},Kr.get(ll.get(g.bead_id)||"")?.worker_serial===!0,f.auto_merge===!0,{merge_sha:g.merge_sha,cleanup_cursor:g.cleanup_cursor,repo_operations:Array.isArray(f.repo_operations)?f.repo_operations:[]})).map(g=>({...g,workflow:zn[g.id]||null,priority:Zt.get(g.id),...ar(g.id)})),merge_queue_length:va.length,merge_queue_running:va.length>0,auto_excluded:Gn.map(g=>g.bead_id).filter(g=>yl(g)!==null),declared_base:Ms,done:Yr,token_total:Af,cleanup_failures:un,repo_operations:Array.isArray(f.repo_operations)?f.repo_operations:[]}}function Ce(){let y=!!o?.get()?.job,m=!y&&o?.isPending?.()===!0,E=y?"\uBD84\uC11D \uC911":m?"\uC900\uBE44 \uC911":"";return l`<button
    ...
    >`,v=l`<label class="worker-tgl worker-slots"
      ...
      <label
        ...
        <select class="worker-serial-lane-count" aria-label="직렬 레인 수">
          ${Array.from({length:Up},(Q,pe)=>pe+1).map(Q=>l`<option
                value=${String(Q)}
                ?selected=${f.serial_lane_count===Q}
              >
                ${Q}
              </option>`)}
        </select>
      ...
      ${o?Ce():""} `,$=bd({failure:f.failure}),B=cd(f.repo_operations,f.cleanup_failures);return Te?l`<div class="worker-ribbon">
        ...
        <div class="worker-kpi">
          ${A}${d}${_}
          ${(Array.isArray(f.token_total)?f.token_total:f.token_total?[{label:f.token_total,tooltip:`${F()} \uC644\uB8CC\uB41C \uC774\uC288\uB4E4\uC774 \uC0DD\uC560 \uC804\uCCB4\uC5D0 \uC4F4 \uD1A0\uD070 \uB204\uC801 (\uC785\uB825+\uCD9C\uB825+\uCE90\uC2DC). \uC774 \uAE30\uAC04\uC5D0 \uC18C\uBAA8\uB41C \uC591\uC774 \uC544\uB2C8\uB2E4`}]:[]).map(Q=>l`<span
                class="worker-kpi__chip worker-kpi__chip--tokens"
                title=${Q.tooltip}
                >${F()} 완료 · 누적 ${Q.label}</span
              >`)}
    ...
    >
      ...
      ${f.running.length>0?Ii(f.running,Date.now(),$e,f.running_overlays):""}
    ...
    >
      ...
      ${vn({id:"worker-pane-running",lane:"running",title:`\uC2E4\uD589 \uC911 \xB7 \uC2AC\uB86F ${f.slots}`,items:f.running,live:f.running.some(m=>m.kind!=="session"&&!m.paused&&m.failed!==!0),body:Ii(f.running,Date.now(),$e,f.running_overlays)})}
    ...
    </div>`}function Kt(f){re={...re,[f]:!re[f]},ly(re),qe()}function qe(){let f=Ze();Ke(ut(f),de),Ke(It(f),se)}function Nt(){if(typeof window.matchMedia!="function")return;let f=window.matchMedia(ay);Te=!!f.matches;let y=m=>{let E=!!(m&&typeof m.matches=="boolean"?m.matches:f.matches);E!==Te&&(Te=E,qe())};typeof f.addEventListener=="function"?(f.addEventListener("change",y),Y.push(()=>f.removeEventListener("change",y))):typeof f.addListener=="function"&&(f.addListener(y),Y.push(()=>f.removeListener(y)))}let Yt=null;function nt(f){Yt=f.target instanceof Element?f.target:null}function Pe(f){let m=f.target?.closest?.('.worker-mini[draggable="true"], .worker-card[draggable="true"]');if(!m)return;if(Yt&&m.contains(Yt)&&Yt.closest("input, button, a")){f.preventDefault();return}let E=m.dataset.beadId||"",A=m.dataset.lane||"";j={bead_id:E,from_lane:A};try{f.dataTransfer?.setData("text/plain",E),f.dataTransfer&&(f.dataTransfer.effectAllowed="move")}catch{}}function I(f){let y=f.target?.closest?.(".worker-pane");if(!y)return;let m=y.dataset.lane||"";m!=="candidate"&&m!=="queue"&&!/^s[1-5]$/.test(m)||(f.preventDefault(),f.dataTransfer&&(f.dataTransfer.dropEffect="move"),y.classList.add("worker-pane--drag-over"))}function fe(f){f.target?.closest?.(".worker-pane")?.classList.remove("worker-pane--drag-over")}function Re(f,y){let m=W.find(_=>_.id===f);if(!m)return;let E=W.filter(_=>_.id!==f),A=E.length;if(y){let _=y.dataset.beadId;if(_===f)return;let v=E.findIndex($=>$.id===_);v>=0&&(A=v)}let d=E.slice();d.splice(A,0,m),T.applyReorder(f,d,A)}function tt(f){let y=f.target?.closest?.(".worker-pane");if(!y)return;f.preventDefault(),y.classList.remove("worker-pane--drag-over");let m=y.dataset.lane||"",E=j?.bead_id||f.dataTransfer?.getData("text/plain")||"",A=j?.from_lane||"";if(j=null,!E)return;let d=f.target?.closest?.(".worker-mini, .worker-card"),_=Array.from(y.querySelectorAll(".worker-mini, .worker-card")),v=_.length;if(d){let $=_.indexOf(d);$>=0&&(v=$)}if(v=Math.max(0,v-y.querySelectorAll(".worker-mini--ghost").length),y.classList.contains("worker-pane--collapsed")&&(v=We()),m==="candidate"){if(A==="candidate"){Re(E,d);return}(A==="queue"||/^s[1-5]$/.test(A))&&Xe(E);return}if(m==="queue"||/^s[1-5]$/.test(m)){let $=m==="queue"?"parallel":m;A===m?et(E,$,v):Ye(E,$)}}function At(f){Z=f,Qh(f),qe()}function mt(f){N=f==="board"||f==="created"||f==="spec"?f:pa,ry(N),qe()}function Rt(f){H=bn(f)?f:dn,oy(H),h?.(H),qe()}function Pt(f){let y=f.target?.closest?.(".worker-serial-lane-count");if(y){let v=Number.parseInt(y.value,10);Number.isFinite(v)&&D(v).then(qe);return}let m=f.target?.closest?.(".worker-filter__blocked");if(m){At({...Z,show_blocked:m.checked});return}let E=f.target?.closest?.(".worker-done-range");if(E){Rt(E.value);return}let A=f.target?.closest?.(".worker-sort");if(A){mt(A.value||pa);return}let d=f.target?.closest?.(".worker-slots__input");if(!d)return;let _=Number.parseInt(d.value,10);if(!Number.isFinite(_)){qe();return}w(_).then(qe)}function Vt(f){return f?{runner:f.runner||void 0,model:f.model||void 0,effort:f.effort||void 0,worktree:f.worktree||void 0,status:f.status||void 0,session_id:f.session_id||void 0}:{}}function tn(){let f=Ze();return{operations:f.repo_operations,cleanup_failures:f.cleanup_failures,repo:u&&u()||""}}function wt(){$e&&Ne.close(),O.hidden=!1,Ve.hidden=!1,ze.open(tn()),qe()}function nn(f){let y=te(),m=y.attempts?y.attempts[f]:null;$e=f,Me=null,ze.close(),O.hidden=!0,Ve.hidden=!1,Ne.open({attempt_id:f,meta:Vt(m)}),qe()}function ln(f,y){$e=null,Me=f,ze.close(),O.hidden=!0,Ve.hidden=!1,Ne.open({attempt_id:f,meta:y,hide_prompt:!0}),qe()}function Ln(){if(ze.isOpen()&&ze.refresh(tn()),Me){let m=(o?.get()?.runs||[]).find(E=>E.run_id===Me);m?Ne.updateMeta(Xi(m)):Ne.close();return}if(!$e)return;let f=te(),y=f.attempts?f.attempts[$e]:null;if(y){Ne.updateMeta(Vt(y));return}Ne.close()}function R(f){let y=f.target;if(y?.closest?.(".worker-mini__serial, .worker-mini__grip")||y?.closest?.("#worker-parallel-analysis-dialog"))return;let m=y?.closest?.(".mon-overlap__chip");if(m){let je=m.closest("[data-bead-id]"),lt=je&&je.getAttribute("data-bead-id")||"";if(lt){let un=m.getAttribute("data-overlap-id")||"";z=!!z&&z.bead_id===lt&&z.counterpart_id===un?null:{bead_id:lt,counterpart_id:un},qe()}return}let E=y?.closest?.(".mon-overlap__place");if(E){let je=E.closest("[data-bead-id]"),lt=je&&je.getAttribute("data-bead-id")||"";lt&&we(lt,E.getAttribute("data-counterpart-id")||"");return}if(y?.closest?.(".mon-overlap__popover"))return;if(y?.closest?.(".worker-analysis-btn")){ne?.open();return}if(y?.closest?.(".worker-repo-strip")||y?.closest?.(".worker-mini__timeline")){wt();return}let A=y?.closest?.(".worker-repo-op__session");if(A){let je=A.dataset.attemptId;je&&nn(je);return}let d=y?.closest?.(".worker-repo-op__resolve");if(d){x(d.dataset.operationId||"");return}let _=y?.closest?.(".worker-repo-op__dismiss");if(_){L(_.dataset.operationId||"");return}let v=y?.closest?.(".worker-cleanup__resume");if(v){let je=v.dataset.beadId;je&&ct(je);return}let $=y?.closest?.(".worker-banner__resume");if($){let je=$.dataset.attemptId;je&&Ct(je);return}let B=y?.closest?.(".worker-banner__discard");if(B){let je=B.dataset.confirmation==="merged"?"merged":"unmerged";ue(B.dataset.beadId||"",B.dataset.attemptId||null,je,B.dataset.operationId||null);return}let Q=y?.closest?.(".worker-banner__dismiss");if(Q){let je=Q.dataset.attemptId;je&&yt(je);return}if(y?.closest?.(".worker-play")){_e(!te().auto_advance);return}let pe=y?.closest?.(".worker-merge-all");if(pe){pe.classList.contains("worker-merge-all--stop")?te().auto_merge===!0?Ee(!1):X():Ee(!0);return}let Oe=y?.closest?.(".worker-pane__hd--toggle");if(Oe){let je=Oe.dataset.lane;(je==="queue"||je==="done")&&Kt(je);return}let Fe=y?.closest?.(".worker-card__place-lane");if(Fe){let je=Fe.dataset.beadId,lt=Fe.dataset.lane;je&&(lt==="parallel"||/^s[1-5]$/.test(lt||""))&&(ce=null,qe(),Ye(je,lt));return}if(y?.closest?.(".worker-card__place-cancel")){ce=null,qe();return}let Dt=y?.closest?.(".worker-card__place");if(Dt){let je=Dt.dataset.beadId;je&&!Dt.disabled&&(Ie()?(ce=je,qe()):Ye(je,"parallel"));return}let Wn=y?.closest?.(".worker-filter__chip");if(Wn){let je=Wn.dataset.spec;(je==="all"||je==="with"||je==="without")&&At({...Z,spec:je});return}let Zt=y?.closest?.(".worker-mini__merge");if(Zt){let je=Zt.dataset.beadId||"";te().cleanup_failed?.[je]?ct(je):Ot(je);return}let sr=y?.closest?.(".worker-mini__merge-cancel");if(sr){M(sr.dataset.beadId||"");return}let or=y?.closest?.(".worker-mini__discard");if(or){ue(or.dataset.beadId||"",or.dataset.attemptId||null,or.dataset.discardMode==="merged"?"merged":"unmerged",or.dataset.operationId||null);return}let zn=y?.closest?.(".worker-mini__stale-continue");if(zn){S("worker-stale-work-continue",zn.dataset.beadId||"",zn.dataset.actionId||"");return}let Hn=y?.closest?.(".worker-mini__stale-backup");if(Hn){S("worker-stale-work-backup-fresh",Hn.dataset.beadId||"",Hn.dataset.actionId||"");return}let kr=y?.closest?.(".worker-mini__stale-recheck");if(kr){S("worker-stale-work-recheck",kr.dataset.beadId||"",kr.dataset.actionId||"");return}let Gr=y?.closest?.(".worker-mini__revise-fix");if(Gr){K("worker-revise-fix",Gr.dataset.beadId||"");return}let Ps=y?.closest?.(".worker-mini__revise-approve");if(Ps){K("worker-revise-approve",Ps.dataset.beadId||"");return}if(y?.closest?.(".worker-mini__pr"))return;if(y?.closest?.(".rtile__discard")){let je=y?.closest?.(".rtile"),lt=je?.dataset?.beadId,un=je?.dataset?.attemptId;lt&&ue(lt,un||null,"unmerged",y?.closest?.(".rtile__discard")?.dataset.operationId||null);return}if(y?.closest?.(".rtile__dismiss")){let lt=y?.closest?.(".rtile")?.dataset?.attemptId;lt&&yt(lt);return}if(y?.closest?.(".rtile__pause")){let lt=y?.closest?.(".rtile")?.dataset?.attemptId;lt&&ft(lt);return}if(y?.closest?.(".rtile__resume")){let lt=y?.closest?.(".rtile")?.dataset?.attemptId;lt&&Ct(lt);return}if(y?.closest?.(".rtile__session")){let lt=y?.closest?.(".rtile")?.dataset?.attemptId;lt&&nn(lt);return}if(y?.closest?.(".worker-drawer-overlay__backdrop")){ze.close(),Ne.close();return}if(y?.closest?.(".worker-drawer-host"))return;let $r=y?.closest?.(".rtile .board-card__roll-toggle");if($r){let je=$r.dataset.rollParent;je&&(Ae.has(je)?Ae.delete(je):Ae.add(je),qe());return}let ar=y?.closest?.(".rtile .board-card__roll-child");if(ar){let je=ar.dataset.childId;je&&c&&c(je);return}let Gn=y?.closest?.(".rtile");if(Gn){if(y?.closest?.(".rtile__id")){let lt=Gn.dataset.beadId;lt&&fn(lt).then(un=>{un?le("\uBCF5\uC0AC\uB428","success",1200):le("\uBCF5\uC0AC \uC2E4\uD328","error",1600)});return}let je=Gn.dataset.beadId;je&&c&&c(je);return}let Vr=y?.closest?.(".worker-mini, .worker-card");if(Vr){let je=Vr.dataset.beadId;if(y?.closest?.(".worker-mini__id, .worker-card__id")){je&&fn(je).then(un=>{un?le("\uBCF5\uC0AC\uB428","success",1200):le("\uBCF5\uC0AC \uC2E4\uD328","error",1600)});return}let lt=y?.closest?.(".ctl-chip--from");if(lt){let un=lt.dataset.fromId;un&&c&&c(un);return}je&&c&&c(je)}}e.addEventListener("pointerdown",nt),e.addEventListener("dragstart",Pe),e.addEventListener("dragover",I),e.addEventListener("dragleave",fe),e.addEventListener("drop",tt),e.addEventListener("click",R),e.addEventListener("change",Pt);function P(f){if(!z)return;let y=f.target;y&&typeof y.closest=="function"&&y.closest(".mon-overlap__popover, .mon-overlap__chip")||(z=null,qe())}function De(f){f.key!=="Escape"||!z||(z=null,qe())}return document.addEventListener("click",P),document.addEventListener("keydown",De),Y.push(()=>{document.removeEventListener("click",P),document.removeEventListener("keydown",De)}),Nt(),k&&Y.push(k.subscribe(()=>{for(let[f,y]of C)y==="failed"&&C.delete(f);qe()})),s&&Y.push(s.subscribe(()=>{let f=u&&u()||"";f!==at&&(at=f,He.close()),qe(),Ln()})),o&&typeof o.subscribe=="function"&&Y.push(o.subscribe(()=>{Ln(),qe()})),qe(),{load(){ke(),qe()},refreshSessionDefaults:ge,destroy(){for(let f of Y.splice(0))try{f()}catch{}e.removeEventListener("pointerdown",nt),e.removeEventListener("dragstart",Pe),e.removeEventListener("dragover",I),e.removeEventListener("dragleave",fe),e.removeEventListener("drop",tt),e.removeEventListener("click",R),e.removeEventListener("change",Pt);try{Ne.destroy()}catch{}Ve.hidden=!0;try{ne?.destroy()}catch{}try{He.destroy()}catch{}Ke(l``,e)}}}function nl(e){if(!e)return"Unknown";let t=e.split("/").filter(Boolean);return t.length>0?t[t.length-1]:"Unknown"}function Qp(e,t,n,r=async()=>{},s=async()=>{}){let o=St("views:workspace-picker"),a=null,i=!1,c=!1,u=!1;async function p(H){let F=H.target.value,Te=t.getState().workspace?.current?.path||"";if(F&&F!==Te){o("switching workspace to %s",F),i=!0,N();try{await n(F)}catch(he){o("workspace switch failed: %o",he)}finally{i=!1,N()}}}async function b(){let H=t.getState(),C=H.workspace?.current?.path||H.workspace?.available?.[0]?.path||"";if(!(!C||c)){o("git-pulling workspace %s",C),c=!0,N();try{await r(C)}catch(F){o("workspace git pull failed: %o",F)}finally{c=!1,N()}}}function h(H){let C=H.target;C&&e.contains(C)||j()}function k(H){H.key==="Escape"&&j()}function T(){u||(u=!0,document.addEventListener("mousedown",h),document.addEventListener("keydown",k),N())}function j(){u&&(u=!1,document.removeEventListener("mousedown",h),document.removeEventListener("keydown",k),N())}function W(){u?j():T()}async function Z(H){let C=H.target,F=C.value,re=C.checked;o("toggling visibility %s \u2192 %s",F,String(re));try{await s(F,re)}catch(Te){o("workspace visibility toggle failed: %o",Te)}}function ce(H){return H?l`
      <button
        type="button"
    ...
    `}function q(){let H=t.getState(),C=H.workspace?.current,F=H.workspace?.available||[],re=new Set(H.workspace?.hidden||[]),Te=C?.path||F[0]?.path||"";if(F.length===0)return l``;let he=F.filter(G=>!re.has(G.path)||G.path===Te);if(he.length<=1){let G=he[0]||F[0],J=nl(G.path);return l`
  ...
  `;Ke(n,e);let r=document.getElementById("global-nav"),s=document.getElementById("top-nav"),o=document.getElementById("repo-scope"),a=document.getElementById("usage-meter"),i=document.getElementById("board-root"),c=document.getElementById("worker-root"),u=document.getElementById("monitor-root"),p=document.getElementById("detail-panel");if(a&&Ap(a),i&&c&&u&&p){let ee=function(R,P){let De="Request failed",f="";if(R&&typeof R=="object"){let m=R;if(typeof m.message=="string"&&m.message.length>0&&(De=m.message),typeof m.details=="string")f=m.details;else if(m.details&&typeof m.details=="object")try{f=JSON.stringify(m.details,null,2)}catch{f=""}}else typeof R=="string"&&R.length>0&&(De=R);let y=P&&P.length>0?`Failed to load ${P}`:"Request failed";Y.open(y,De,f)},Qe=function(R){return`${nt.getState().workspace.current?.path||""}\0${R}`},Le=function(){Ne&&(Ne().catch(()=>{}),Ne=null),ze=null,He=null},We=function(R){at=R;let P=()=>{at!==R||nt.getState().selected_id!==R||(at=null,xe(R))};if(!te){ne.then(P);return}P()},ft=function(R,P,De,f,y){return De!==Xe[P]?(y().catch(()=>{}),!1):(R.set(f,y),!0)},yt=function(){let R=nt.getState();Ee(R.view==="board"),_e(R.view==="worker"),V(R.view==="monitor"),L(R.view==="board"||R.view==="worker"||Ct||!!R.selected_id)},ct=function(){let R=pr(_t);return R===void 0?{type:"closed-issues"}:{type:"closed-issues",params:{since:R}}},Ge=function(){let R=pr(Ot);return R===void 0?{type:"closed-issues"}:{type:"closed-issues",params:{since:R}}},Ee=function(R){if(R)for(let[P,De]of sl){if(Ye.has(P)||et.has(P))continue;let f=P===rr?ct():{type:De};try{de.register(P,f)}catch(E){t("register %s store failed: %o",P,E)}et.add(P);let y=Xe.board,m=!1;Be.subscribeList(P,f).then(E=>{m=!ft(Ye,"board",y,P,E)}).catch(E=>{t("subscribe %s failed: %o",P,E),ee(E,"board")}).finally(()=>{et.delete(P),m&&yt()})}else ue()},ue=function(){Xe.board+=1;for(let[R]of sl){let P=Ye.get(R);P&&(P().catch(()=>{}),Ye.delete(R));try{de.unregister(R)}catch(De){t("unregister %s failed: %o",R,De)}}},_e=function(R){if(!R){x();return}for(let[P,De]of nf){if(S.has(P)||et.has(P))continue;let f=P===nr?Ge():{type:De};try{de.register(P,f)}catch(E){t("register %s store failed: %o",P,E)}et.add(P);let y=Xe.worker,m=!1;Be.subscribeList(P,f).then(E=>{m=!ft(S,"worker",y,P,E)}).catch(E=>{t("subscribe %s failed: %o",P,E),ee(E,"worker")}).finally(()=>{et.delete(P),m&&yt()})}},x=function(){Xe.worker+=1;for(let[R]of nf){let P=S.get(R);P&&(P().catch(()=>{}),S.delete(R));try{de.unregister(R)}catch(De){t("unregister %s failed: %o",R,De)}}},L=function(R){if(!R){w();return}K||(ge("subscribe-worker-queue",{id:sf}).catch(P=>{t("subscribe-worker-queue failed: %o",P)}),ge("subscribe-worker-parallel-analysis",{id:of}).catch(P=>{t("subscribe-worker-parallel-analysis failed: %o",P)}),K=()=>(ge("unsubscribe-worker-parallel-analysis",{id:of}),ge("unsubscribe-worker-queue",{id:sf})))},w=function(){K&&(K().catch(()=>{}),K=null),st.clear()},V=function(R){if(!R){me();return}D||(ge("subscribe-monitor-pipeline",{id:rf}).catch(P=>{t("subscribe-monitor-pipeline failed: %o",P)}),D=()=>ge("unsubscribe-monitor-pipeline",{id:rf}))},me=function(){D&&(D().catch(()=>{}),D=null)},we=function(){ae||(ge("subscribe-ui-order",{id:af}).catch(R=>{t("subscribe-ui-order failed: %o",R)}),ae=()=>ge("unsubscribe-ui-order",{id:af}))},rt=function(){ae&&(ae().catch(()=>{}),ae=null),O.clear()},Ce=function(){Ze||(ge("subscribe-display-policy",{id:lf}).catch(R=>{t("subscribe-display-policy failed: %o",R)}),Ze=()=>ge("unsubscribe-display-policy",{id:lf}))},ut=function(){Ze&&(Ze().catch(()=>{}),Ze=null),se.clear()},Ft=function(){it||(ge("subscribe-impl-presets",{id:cf}).catch(R=>{t("subscribe-impl-presets failed: %o",R)}),it=()=>ge("unsubscribe-impl-presets",{id:cf}))},Kt=function(R){if(!R)return"Unknown";let P=R.split("/").filter(Boolean);return P.length>0?P[P.length-1]:"Unknown"},Pt=function(R,P){Rt.open(R.path,{missing_state:R.missing_state,...P?{workspace:P}:{}})};var b=ee,h=Qe,k=Le,T=We,j=ft,W=yt,Z=ct,ce=Ge,z=Ee,q=ue,N=_e,H=x,C=L,F=w,re=V,Te=me,he=we,G=rt,J=Ce,ye=ut,Ae=Ft,be=Kt,oe=Pt;let Se=document.getElementById("header-loading"),ve=wc(Se),Y=ld(e),ke=tf(),ge=ve.wrapSend((R,P)=>ke.send(R,P)),Be=fc(ge),de=_c(),Ve=bc(),st=gc(),dt=Ql(),O=mc(),se=Zl(),$e=Xl(),Me=Jl();ke.on("impl-presets-snapshot",R=>{let P=R;P&&typeof P.revision=="number"&&Array.isArray(P.presets)&&$e.set({revision:P.revision,presets:P.presets})}),ke.on("monitor-pipeline-snapshot",R=>{let P=R;if(!(!P||!Array.isArray(P.workspaces)))try{dt.set(P.workspaces,P.workspaces_state,P.cross_lanes)}catch{}}),ke.on("ui-order-snapshot",R=>{let P=R;if(P&&typeof P.revision=="number")try{O.set({revision:P.revision,order:P.order&&typeof P.order=="object"?P.order:{}})}catch{}}),ke.on("display-policy-snapshot",R=>{let P=R;if(P&&P.policy&&typeof P.policy=="object")try{se.set(P.policy)}catch{}}),ke.on("session-log-snapshot",R=>{let P=R;if(P&&typeof P.id=="string")try{Me.set(P.id,Array.isArray(P.lines)?P.lines:[],typeof P.last_event_at=="number"?P.last_event_at:null)}catch{}}),ke.on("session-log-append",R=>{let P=R;if(P&&typeof P.id=="string")try{Me.append(P.id,P.event)}catch{}}),ke.on("snapshot",R=>{let P=R,De=P&&typeof P.id=="string"?P.id:"",f=De?de.getStore(De):null;if(f&&P&&P.type==="snapshot")try{f.applyPush(P)}catch{}}),ke.on("upsert",R=>{let P=R,De=P&&typeof P.id=="string"?P.id:"",f=De?de.getStore(De):null;if(f&&P&&P.type==="upsert")try{f.applyPush(P)}catch{}}),ke.on("delete",R=>{let P=R,De=P&&typeof P.id=="string"?P.id:"",f=De?de.getStore(De):null;if(f&&P&&P.type==="delete")try{f.applyPush(P)}catch{}});let Ne=null,ze=null,He=null,at=null,bt=()=>{},ne=new Promise(R=>{bt=()=>R(void 0)}),te=!1,Ie=!1;async function xe(R){let P=Qe(R);if(P===ze||P===He)return;He=P;let De=`detail:${R}`,f={type:"issue-detail",params:{id:R}};try{de.register(De,f)}catch(y){t("register detail store failed: %o",y)}try{let y=await Be.subscribeList(De,f);if(nt.getState().selected_id!==R||Qe(R)!==P){await y().catch(()=>{});return}Ne&&await Ne().catch(()=>{}),Ne=y,ze=P}catch(y){t("detail subscribe failed: %o",y),ee(y,"issue details")}finally{He===P&&(He=null)}}let Ye=new Map,et=new Set,Xe={board:0,worker:0},Ct=!1,_t=dn;try{let R=window.localStorage.getItem(uf);bn(R)&&(_t=R)}catch{}let Ot=dn;try{let R=window.localStorage.getItem(Sy);bn(R)&&(Ot=R)}catch{}async function M(R){if(!bn(R)||R===_t)return;_t=R;try{window.localStorage.setItem(uf,R)}catch{}let P=Ye.get(rr);if(!P)return;Ye.delete(rr),await P().catch(()=>{});let De=ct();try{de.register(rr,De)}catch(f){t("register %s store failed: %o",rr,f)}try{let f=await Be.subscribeList(rr,De);Ye.set(rr,f)}catch(f){t("re-subscribe %s failed: %o",rr,f),ee(f,"board")}}async function X(R){if(!bn(R)||R===Ot)return;Ot=R;let P=S.get(nr);if(!P)return;S.delete(nr),await P().catch(()=>{});let De=Ge();try{de.register(nr,De)}catch(f){t("register %s store failed: %o",nr,f)}try{let f=await Be.subscribeList(nr,De);S.set(nr,f)}catch(f){t("re-subscribe %s failed: %o",nr,f),ee(f,"worker")}}let S=new Map,K=null,D=null,ae=null,Ze=null,it=null;async function jt(){Ze=null,se.clear(),it=null,$e.clear(),K=null,D=null,Ye.clear(),S.clear(),Xe.board+=1,Xe.worker+=1,Ft();let R=nt.getState().workspace.current?.path;if(R)try{await ke.send("set-workspace",{path:R})}catch(De){t("workspace restore after reconnect failed: %o",De);return}Ce();let P=nt.getState();Ee(P.view==="board"),_e(P.view==="worker"),V(P.view==="monitor"),L(P.view==="board"||P.view==="worker"||!!P.selected_id)}async function Gt(){t("clearing all subscriptions for workspace switch"),ue(),x(),w(),Ve.clear(),rt(),we(),ut(),Ce(),Le();let R=nt.getState();if(R.selected_id)try{de.unregister(`detail:${R.selected_id}`)}catch{}let P=nt.getState();Ee(P.view==="board"),_e(P.view==="worker"),V(P.view==="monitor"),L(P.view==="board"||P.view==="worker"||!!P.selected_id),P.selected_id&&We(P.selected_id)}async function Mt(R){t("requesting workspace switch to %s",R),Ie=!0;try{let P=await ke.send("set-workspace",{path:R});t("workspace switch result: %o",P),P&&P.workspace&&(nt.setState({workspace:{current:{path:P.workspace.root_dir,database:P.workspace.db_path}}}),window.localStorage.setItem("beads-ui.workspace",R),P.changed&&(await Gt(),le("Switched to "+Kt(R),"success",2e3)))}catch(P){throw t("workspace switch failed: %o",P),le("Failed to switch workspace","error",3e3),P}finally{Ie=!1}}async function Ut(R){t("requesting workspace git pull for %s",R);try{let P=await ke.send("git-pull-workspace",{});t("workspace git pull result: %o",P);let De=P?.status;if(De==="up_to_date"){le("Already up to date","success",2e3);return}if(De==="stash_pop_conflict"){le("Git pulled, but stash pop conflicted (check git stash list)","warning",4e3);return}le("Git pulled "+Kt(R),"success",2e3)}catch(P){t("workspace git pull failed: %o",P);let De=P?.code,f=P?.message;if(De==="rebase_conflict"){le("Git pull conflicts \u2014 reverted (manual resolve required)","error",4e3);return}if(De==="rebase_conflict_abort_failed"){le("Git pull conflicts AND rebase --abort failed \u2014 repo left mid-rebase, run 'git rebase --abort' manually","error",6e3);return}if(De==="busy"){le("Git pull skipped: another operation is running","warning",3e3);return}let y=f?`: ${f}`:"";throw le(`Git pull failed${y}`,"error",3e3),P}}async function It(R,P){t("setting workspace visibility %s \u2192 %s",R,String(P));try{await ke.send("set-workspace-visibility",{path:R,visible:P}),await qe()}catch(De){t("workspace visibility update failed: %o",De),le("Failed to update project visibility","error",3e3)}}async function qe(){try{let R=await ke.send("list-workspaces",{});if(t("workspaces loaded: %o",R),R&&Array.isArray(R.workspaces)){let P=R.workspaces.map(m=>({path:m.path,database:m.database,pid:m.pid,version:m.version})),De=R.current?{path:R.current.root_dir,database:R.current.db_path}:null,f=Array.isArray(R.hidden)?R.hidden.filter(m=>typeof m=="string"):[];nt.setState({workspace:{current:De,available:P,hidden:f}});let y=window.localStorage.getItem("beads-ui.workspace");y&&(!P.some(E=>E.path===y)||f.includes(y)?window.localStorage.removeItem("beads-ui.workspace"):De&&y!==De.path&&(t("restoring saved workspace preference: %s",y),await Mt(y)))}}catch(R){t("failed to load workspaces: %o",R)}}ke.on("workspace-changed",R=>{t("workspace-changed event: %o",R),R&&R.root_dir&&(nt.setState({workspace:{current:{path:R.root_dir,database:R.db_path}}}),qe(),Gt())});let Nt=!1;if(typeof ke.onConnection=="function"){let R=P=>{t("ws state %s",P),P==="reconnecting"||P==="closed"?(Nt=!0,le("Connection lost. Reconnecting\u2026","error",4e3)):P==="open"&&Nt&&(Nt=!1,le("Reconnected","success",2200),Ay(nt,(De,f)=>{t(`${De}: %o`,f)}),jt())};ke.onConnection(R)}let Yt="board";try{let R=window.localStorage.getItem("beads-ui.view");(R==="board"||R==="worker"||R==="monitor")&&(Yt=R)}catch(R){t("view parse error: %o",R)}let nt=vc({config:xy(),view:Yt});ke.on("worker-queue-snapshot",R=>{let P=R;if(!P||!P.queue)return;let De=nt.getState().workspace.current?.path;if(typeof De=="string"&&De.length>0&&P.root_dir!==De){t("dropping worker-queue snapshot for %s",String(P.root_dir));return}try{Ve.set(P.queue)}catch{}}),ke.on("worker-parallel-analysis-snapshot",R=>{let P=R;if(!P)return;let De=nt.getState().workspace.current?.path;if(!(typeof De=="string"&&De.length>0&&typeof P.root_dir=="string"&&P.root_dir!==De))try{st.set({settings:P.settings,job:P.job??null,runs:Array.isArray(P.runs)?P.runs:[],last_good:P.last_good??null})}catch{}});let Pe=hc(nt);Pe.start();let I=new Set(["get-comments","dep-add","dep-remove","impl-preset-create","impl-preset-update","impl-preset-delete","apply-impl-preset","apply-impl-preset-global","get-session-defaults","set-session-defaults","monitor-lane-create","monitor-lane-update","monitor-lane-confirm","monitor-lane-remove"]),fe=async(R,P)=>{try{return await ge(R,P)}catch(De){if(I.has(R))throw De;return[]}};ip({global_element:r,repo_element:s},nt,Pe);let Re=document.getElementById("workspace-picker");Re&&Qp(Re,nt,Mt,Ut,It);let tt=dp(e,(R,P)=>ge(R,P));try{let R=document.getElementById("new-issue-btn");R&&R.addEventListener("click",()=>tt.open())}catch{}let At=mp(e,{policyStore:se,queueStore:Ve,implPresetStore:$e,transport:(R,P)=>ge(R,P),onOpenChange:R=>{let P=Ct;Ct=R,yt(),P&&R===!1&&tn.refreshSessionDefaults()},labelOptions:()=>{let R=new Set;for(let[P]of sl)for(let De of de.snapshotFor(P)||[]){let f=De.labels;if(Array.isArray(f))for(let y of f)typeof y=="string"&&y.length>0&&R.add(y)}return Array.from(R).sort()}});try{let R=document.getElementById("display-settings-btn");R&&(R.setAttribute("aria-label","\uC124\uC815"),R.setAttribute("title","\uC124\uC815"),R.addEventListener("click",()=>At.open()))}catch{}let mt=document.createElement("div");mt.className="md-viewer-root",document.body.appendChild(mt);let Rt=Fo(mt,{getWorkspacePath:()=>nt.getState().workspace.current?.path}),Vt=Pc(i,{gotoIssue:R=>Pe.gotoIssue(R),issueStores:de,transport:fe,workerQueueStore:Ve,uiOrderStore:O,displayPolicyStore:se,closedRange:_t,onClosedRangeChange:R=>{M(R)},onNewIssue:()=>tt.open(),openDoc:Pt}),tn=tl(c,{transport:fe,issueStores:de,queueStore:Ve,analysisStore:st,sessionLogStore:Me,uiOrderStore:O,gotoIssue:R=>nt.setState({selected_id:R}),getWorkspacePath:()=>nt.getState().workspace.current?.path,openDoc:Pt,doneRange:Ot,onDoneRangeChange:R=>{X(R)}}),wt=ap(u,{transport:fe,pipelineStore:dt,execPresetStore:$e,sessionLogStore:Me,router:Pe,gotoIssue:R=>Pe.gotoIssue(R),getWorkspacePath:()=>nt.getState().workspace.current?.path,switchWorkspace:R=>Mt(R),openDoc:Pt}),nn=id(p,{issueStores:de,transport:fe,queueStore:Ve,execPresetStore:$e,sessionLogStore:Me,getWorkspacePath:()=>nt.getState().workspace.current?.path,mdViewer:Rt,onNavigate:R=>{nt.getState().view==="worker"?nt.setState({selected_id:R}):Pe.gotoIssue(R)},onClose:()=>{let R=nt.getState();nt.setState({selected_id:null});try{Pe.gotoView(R.view==="worker"||R.view==="monitor"?R.view:"board")}catch{}},onOpenExecPresets:()=>{At.open("execution")}}),ln=nt.getState().selected_id;ln&&(p.hidden=!1,nn.load(ln),We(ln)),nt.subscribe(R=>{let P=R.selected_id;P?(p.hidden=!1,nn.load(P),Ie||We(P)):(nn.clear(),p.hidden=!0,Le())});let Ln=R=>{i.hidden=R.view!=="board",c.hidden=R.view!=="worker",u.hidden=R.view!=="monitor",o&&o.classList.toggle("is-quiet",R.view==="monitor"),Ee(R.view==="board"),_e(R.view==="worker"),V(R.view==="monitor"),L(R.view==="board"||R.view==="worker"||Ct||!!R.selected_id),!R.selected_id&&R.view==="board"&&Vt.load(),R.view==="worker"&&tn.load(),R.view==="monitor"?wt.load():wt.pause(),window.localStorage.setItem("beads-ui.view",R.view)};nt.subscribe(Ln),Ln(nt.getState()),we(),Ce(),Ft(),qe().finally(()=>{te=!0,bt()}),window.addEventListener("keydown",R=>{let P=R.ctrlKey||R.metaKey,De=String(R.key||"").toLowerCase(),f=R.target,y=f&&f.tagName?String(f.tagName).toLowerCase():"",m=y==="input"||y==="textarea"||y==="select"||f&&typeof f.isContentEditable=="boolean"&&f.isContentEditable;P&&De==="n"&&(m||(R.preventDefault(),tt.open()))})}}typeof window<"u"&&typeof document<"u"&&window.addEventListener("DOMContentLoaded",()=>{try{let n=window.localStorage.getItem("beads-ui.theme"),r=window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches,s=n==="dark"||n==="light"?n:r?"dark":"light";document.documentElement.setAttribute("data-theme",s);let o=document.getElementById("theme-switch");o&&(o.checked=s==="dark")}catch{}let e=document.getElementById("theme-switch");e&&e.addEventListener("change",()=>{let n=e.checked?"dark":"light";document.documentElement.setAttribute("data-theme",n),window.localStorage.setItem("beads-ui.theme",n)});let t=document.getElementById("app");t&&Ey(t)});export{Ey as bootstrap,xy as readBootstrapConfig,Ay as refreshConfigSnapshot};
