...
`+r.join(`
`),D=this.blockquote(k);o[o.length-1]=D,n=n.substring(0,n.length-E.raw.length)+D.raw,s=s.substring(0,s.length-E.text.length)+D.text;break}else if(h?.type==="list"){let E=h,k=E.raw+`
`+r.join(`
`),D=this.list(k);o[o.length-1]=D,n=n.substring(0,n.length-h.raw.length)+D.raw,s=s.substring(0,s.length-E.raw.length)+D.raw,r=k.substring(o.at(-1).raw.length).split(`
...
`,e=e.substring(h.length+1),c=!0),!c){let D=this.rules.other.nextBulletRegex(k),N=this.rules.other.hrRegex(k),A=this.rules.other.fencesBeginRegex(k),$=this.rules.other.headingBeginRegex(k),M=this.rules.other.htmlBeginRegex(k);for(;e;){let I=e.split(`
`,1)[0],S;if(h=I,this.options.pedantic?(h=h.replace(this.rules.other.listReplaceNesting,"  "),S=h):S=h.replace(this.rules.other.tabCharGlobal,"    "),A.test(h)||$.test(h)||M.test(h)||D.test(h)||N.test(h))break;if(S.search(this.rules.other.nonSpaceChar)>=k||!h.trim())_+=`
...
`)}function je(){if(!o)return i``;let W=S(),U=[a.runner,a.model,a.effort].filter(Boolean).join(" \xB7 "),ge=a.session_id||"",Pe=`\uB77C\uC774\uBE0C \uB530\uB77C\uAC00\uAE30 ${l?"ON":"OFF"}`,O=Z(),j=O?Wp(B(),Date.now()):"",R=O?de(W):null,re=O?ke(W):null,ee=jp(W);return i`<div class="sv" data-attempt-id=${o}>
      <div class="sv__bar">
...
`).trim()}}var dl=20;function ul(e){if(e==null||e==="")return"";let t=new Date(e);if(Number.isNaN(t.getTime()))return"";let r=String(t.getMonth()+1).padStart(2,"0"),n=String(t.getDate()).padStart(2,"0"),s=String(t.getHours()).padStart(2,"0"),o=String(t.getMinutes()).padStart(2,"0");return`${r}-${n} ${s}:${o}`}function Kp(e){return e.length>dl?`${e.slice(0,dl)}\u2026`:e}function Zp(e,t,r,n){let s=`${t.lane} ${Kp(t.identifier)}`;return i`<div class="detail-report">
  ...
  >
    ...
    `:i``}function _(){Ge(d(),e)}async function m(k,D={}){s=k,o="loading",a="",l="",_();let N=r?r():"";if(!N){o="error",l="\uC6CC\uD06C\uC2A4\uD398\uC774\uC2A4\uAC00 \uC120\uD0DD\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4",_();return}if(!n){o="error",l="fetch\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4",_();return}let A="/api/doc?workspace="+encodeURIComponent(N)+"&path="+encodeURIComponent(k);try{let $=await n(A),M=await $.json().catch(()=>({}));if(!$.ok||!M||M.ok!==!0){if(M?.error==="not_found"&&D.missing_state==="plan_pending"){o="pending",l="\uACC4\uD68D \uC791\uC131 \uC804 \xB7 \uACBD\uB85C\uB9CC \uC608\uC57D\uB418\uC5B4 \uC788\uC2B5\uB2C8\uB2E4",_();return}o="error",l="\uBB38\uC11C\uB97C \uBD88\uB7EC\uC624\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4 ("+String(M&&M.error||$.status)+")",_();return}a=String(M.content||""),o="ready",_()}catch{o="error",l="\uBB38\uC11C \uC694\uCCAD \uC2E4\uD328",_()}}function h(){s=null,Ge(i``,e)}function E(){document.removeEventListener("keydown",c),h()}return{open:m,close:h,destroy:E}}var df=[{key:"input_tokens",label:"\uC785\uB825"},{key:"output_tokens",label:"\uCD9C\uB825"},{key:"cache_read_input_tokens",label:"\uCE90\uC2DC \uC77D\uAE30"}],Pl="\uC11C\uBC84 \uC7AC\uC2DC\uC791 \uBCF5\uAD6C \u2014 \uBD80\uBD84 \uC9D1\uACC4";function uf(e){return typeof e=="number"&&Number.isFinite(e)?e:0}function pf(e){let t=wt(e);if(t.length>0)return t.map(s=>i`<span class="detail-usage-total" title=${s.tooltip}
          >${s.label}</span
  ...
  `}function hf(e){let t=Vr(e);if(t)return t;let r=e.data;if(!r)return"";if(r.missing)return i`<div class="detail-prompt__missing">
    ...
    `}function x(v){let f=v.metadata||{},u=v.workflow||{},C=u.stages||{},Y=C.spec&&C.spec.stale,he=C.impl&&C.impl.stale,Oe=C.plan||null,xe=u.route_source==="derived",_t=u.route||f.route||"\u2014";return i`
    ...
    `}function Ke(){if(!d)return i``;let v=_||{},f=String(v.id||d),u=v.title||"(\uC81C\uBAA9 \uC5C6\uC74C)",C=fe(),Y=v.status||"open",he=typeof v.priority=="number"?Math.max(0,Math.min(4,v.priority)):"",Oe=v.description||"",xe={...v,metadata:{...v.metadata||{},...m}};return i`
      <div class="detail-overlay" role="dialog" aria-modal="true">
        ...
        <div class="detail-overlay__panel">
          ...
          ${G(u,C)}
          ${Al(xe)}
          ${Sl({metadata:xe.metadata,workspace_values:ne(),catalog:P(),expanded:k,presets:De()?.presets||[],preset_id:h,preset_busy:E},{onToggle:()=>{k=!k,ve()},onEdit:(_t,Te)=>{if(_t==="impl_runtime"||_t==="impl_model"||_t==="impl_effort"){se(_t,Te??"");return}X(_t,Te??"")},onPresetSelect:_t=>{h=_t,ve()},onPresetApply:()=>{Ee()}})}
          ${q(Y,he)} ${b(v)}
          ${pe(Oe)}
    ...
    `}function ve(){Ge(Ke(),e)}return{load(v){v!==d&&(m={},h="",k=!1,S(),Ue(),j()),d=v,_=null,$t(),w()},clear(){d=null,_=null,m={},h="",E=!1,S(),Ue(),j(),V.close(),$e.close(),Ge(i``,e)},destroy(){Qe&&(Qe(),Qe=null),kt&&(kt(),kt=null),yt&&(yt(),yt=null),document.removeEventListener("keydown",ct),V.destroy(),be.parentNode&&be.parentNode.removeChild(be),$e.destroy(),H.parentNode&&H.parentNode.removeChild(H),d=null,_=null,h="",E=!1,Ue(),j(),Ge(i``,e)}}}function ql(e){let t=document.createElement("dialog");t.id="fatal-error-dialog",t.setAttribute("role","alertdialog"),t.setAttribute("aria-modal","true"),t.innerHTML=`
    ...
    ${l?i`<div class="worker-mini__row1">${h}${E}${k}</div>
          <div class="worker-mini__row2">
            ...
            ${Kr(e)}`:i`<div class="worker-mini__line">
              ${d}${_}${h}${E}${k}${D}${N}${A}${m}${$}${M}${I}${S}${B}${Z}${we}
            </div>
            ${dr(e)} ${Kr(e)}`}
...
${te.notes_tail}`:"notes\uC758 REVISE finding\uC744 \uC2A4\uD399\uC5D0 \uBC18\uC601\uD558\uB294 \uCC98\uBD84 \uC138\uC158\uC744 \uB744\uC6C1\uB2C8\uB2E4":""};_.push(U);let ge=h.get($);ge?ge.push(U):h.set($,[U])}for(let V of Array.isArray(A.runnable)?A.runnable:[]){let H=V&&V.bead_id;typeof H!="string"||be.has(H)||(be.add(H),l.push({...Re(H),title:V.title||Z[H]||H,lane:"runnable",draggable:!0,reason:rc(we,H),created_at:V.created_at??void 0,updated_at:V.updated_at??void 0,labels:Array.isArray(V.labels)?V.labels:[],spec_reviewer:typeof V.spec_reviewer=="string"?V.spec_reviewer:void 0,plan_state:V.plan_state==="approved"||V.plan_state==="authored"?V.plan_state:"none",workflow:V.route?{route:V.route,chips:{route:V.route}}:null,place_index:We.length}))}for(let V of Le){let H=V&&V.bead_id;if(typeof H!="string"||be.has(H)||(be.add(H),o!==void 0&&typeof V.added_at=="number"&&V.added_at<o))continue;let $e=Lf(B,H);m.push({...Re(H),lane:"done",done:!0,usage:Dt(B,H),done_at:typeof V.added_at=="number"?V.added_at:void 0,done_kind:$e&&typeof $e.done_kind=="string"?$e.done_kind:null})}}let E=new Map;s.forEach((A,$)=>{A&&typeof A.root_dir=="string"&&E.set(A.root_dir,$)});let k=r&&r.running_sort==="repo"?"repo":"started";c.sort((A,$)=>{if(k==="repo"){let S=E.get(A.root_dir)??Number.MAX_SAFE_INTEGER,B=E.get($.root_dir)??Number.MAX_SAFE_INTEGER;if(S!==B)return S-B}let M=typeof A.started_at=="number"&&Number.isFinite(A.started_at)?A.started_at:null,I=typeof $.started_at=="number"&&Number.isFinite($.started_at)?$.started_at:null;return M!==null&&I!==null&&M!==I?M-I:M===null&&I!==null?1:M!==null&&I===null?-1:A.id.localeCompare($.id)}),m.sort((A,$)=>($.done_at??0)-(A.done_at??0));let D=s.length>0?s:n.map(A=>({root_dir:A&&A.root_dir,name:A&&A.name,auto_advance:A&&A.auto_advance,auto_merge:A&&A.auto_merge,slots:A&&A.slots,revision:A&&A.revision,runner_catalog:A&&A.runner_catalog})),N=[];for(let A of D)!A||typeof A.root_dir!="string"||N.push({root_dir:A.root_dir,name:A.name||A.root_dir,auto_advance:A.auto_advance===!0,auto_merge:A.auto_merge===!0,slots:typeof A.slots=="number"&&A.slots>=$n?A.slots:$n,revision:typeof A.revision=="number"?A.revision:0,runner_catalog:Ft(A.runner_catalog),items:h.get(A.root_dir)||[]});return{runnable:l,queue:_,queue_groups:N,running:c,pr_wait:d,done:m,automation:{total:N.length,both_on:N.filter(A=>A.auto_advance&&A.auto_merge).length}}}function Pf(e,t){if(typeof e!="number"||!Number.isFinite(e))return"";let n=t-e<Cf;return i`<span
  ...
  `,e.appendChild(r);let n=r.querySelector("#new-issue-form"),s=r.querySelector("#new-title"),o=r.querySelector("#new-type"),a=r.querySelector("#new-priority"),l=r.querySelector("#new-labels"),c=r.querySelector("#new-description"),d=r.querySelector("#new-issue-error"),_=r.querySelector("#btn-cancel"),m=r.querySelector("#btn-create"),h=r.querySelector(".new-issue__close");function E(){o.replaceChildren();let S=document.createElement("option");S.value="",S.textContent="\u2014 Select \u2014",o.appendChild(S);for(let B of _c){let Z=document.createElement("option");Z.value=B,Z.textContent=mc(B),o.appendChild(Z)}a.replaceChildren();for(let B=0;B<=4;B+=1){let Z=document.createElement("option");Z.value=String(B);let oe=gc[B]||"Medium";Z.textContent=`${B} \u2013 ${oe}`,a.appendChild(Z)}}E();function k(){try{typeof r.close=="function"?r.close():r.removeAttribute("open")}catch{r.removeAttribute("open")}}function D(S){s.disabled=S,o.disabled=S,a.disabled=S,l.disabled=S,c.disabled=S,_.disabled=S,m.disabled=S,m.textContent=S?"Creating\u2026":"Create"}function N(){d.textContent=""}function A(S){d.textContent=S}function $(){try{let S=window.localStorage.getItem("beads-ui.new.type");S?o.value=S:o.value="";let B=window.localStorage.getItem("beads-ui.new.priority");B&&/^\d$/.test(B)?a.value=B:a.value="2"}catch{o.value="",a.value="2"}}function M(){let S=o.value||"",B=a.value||"";S.length>0&&window.localStorage.setItem("beads-ui.new.type",S),B.length>0&&window.localStorage.setItem("beads-ui.new.priority",B)}async function I(){N();let S=String(s.value||"").trim();if(S.length===0){A("Title is required"),s.focus();return}let B=Number(a.value||"2");if(!(B>=0&&B<=4)){A("Priority must be 0..4"),a.focus();return}let Z=String(o.value||""),oe=String(c.value||""),we={title:S};Z.length>0&&(we.type=Z),String(B).length>0&&(we.priority=B),oe.length>0&&(we.description=oe),D(!0);try{await t("create-issue",we)}catch{D(!1),A("Failed to create issue");return}M(),D(!1),k()}return r.addEventListener("cancel",S=>{S.preventDefault(),k()}),h.addEventListener("click",()=>k()),_.addEventListener("click",()=>k()),r.addEventListener("keydown",S=>{S.key==="Enter"&&(S.ctrlKey||S.metaKey)&&(S.preventDefault(),I())}),n.addEventListener("submit",S=>{S.preventDefault(),I()}),{open(){n.reset(),N(),$();try{"showModal"in r&&typeof r.showModal=="function"?r.showModal():r.setAttribute("open","")}catch{r.setAttribute("open","")}setTimeout(()=>{try{s.focus()}catch{}},0)},close(){k()}}}var Vf=[["route","route \uCE69"],["fast_track","\u26A1 fast_track \uCE69"],["pr","PR \uCE69"],["from","\u21A9 from \uCE69"],["blocked","blocked \uC0AC\uC720 \uCE69"],["stepper","stepper"]];function Yf(e,t){return to(e,t)?"shown":t.hidden_labels.includes(e)?"hidden_exact":"hidden_prefix"}function bc(e,t,r){return i`
  ...
  `}var Kf=[{id:"session",label:"\uC138\uC158",glyph:"\u25C6"},{id:"worker",label:"Worker",glyph:"\u25A4"},{id:"display",label:"\uD45C\uC2DC",glyph:"\u25EB"}],Kt="";function Zt(e){return!!e&&typeof e=="object"&&!Array.isArray(e)}function wc(e,t){let{transport:r,policyStore:n,labelOptions:s}=t,o=t.notify||(w=>le(w,"error",4e3)),a=document.createElement("dialog");a.id="settings-dialog",a.className="settings-dialog",a.setAttribute("role","dialog"),a.setAttribute("aria-modal","true"),a.setAttribute("aria-label","\uC124\uC815"),e.appendChild(a);let l="session",c=!1,d="",_={},m={},h=[],E=!1,k=null,D={},N="",A="",$=!1,M=!1,I=!1,S=null;function B(){let w=t.queueStore?.get();return Zt(w)?w.runner_catalog:null}function Z(){let w=t.implPresetStore?.get();return Zt(w)&&Array.isArray(w.presets)?w:null}async function oe(){E=!0,ee();try{let w=await r("get-session-defaults",{});_=Zt(w?.values)?{...w.values}:{},m={..._},h=Array.isArray(w?.warnings)?w.warnings:[]}catch(w){h=["kv_read_failed"],o(`\uC138\uC158 \uAE30\uBCF8\uAC12\uC744 \uC77D\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4: ${w instanceof Error?w.message:String(w)}`)}finally{E=!1,ee()}}async function we(){let w=bl(_,m);if(Object.keys(w).length!==0){try{let P=await r("set-session-defaults",{values:w});_=Zt(P?.values)?{...P.values}:{},m={..._},h=Array.isArray(P?.warnings)?P.warnings:[]}catch(P){o(`\uC138\uC158 \uAE30\uBCF8\uAC12 \uC800\uC7A5 \uC2E4\uD328: ${P instanceof Error?P.message:String(P)}`)}ee()}}function Q(w,P){P===Kt?delete m[w]:m[w]=P,ee(),we()}async function de(){let w=t.queueStore?.get();if(!Zt(w))return;let P={orchestration_model:w.orchestration_model??null,orchestration_effort:w.orchestration_effort??null,orchestration_speed:w.orchestration_speed??null},K=yl(P,{...P,...D});if(Object.keys(K).length!==0){try{let De=await r("worker-queue-set-orchestration-defaults",{expected_revision:w.revision,values:K});if(De&&De.applied===!1){o("Worker \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uD074\uB77C\uC774\uC5B8\uD2B8\uC640 \uCDA9\uB3CC");return}D={}}catch(De){o(`Worker \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: ${De instanceof Error?De.message:String(De)}`)}ee()}}function ke(w,P){D[w]=P===Kt?null:P,ee(),de()}async function Be(w){let P=t.queueStore?.get();if(!(!Zt(P)||w<1)){try{await r("worker-queue-set-slots",{expected_revision:P.revision,slots:w})}catch(K){o(`slots \uC800\uC7A5 \uC2E4\uD328: ${K instanceof Error?K.message:String(K)}`)}ee()}}function Ue(){let w={};for(let P of ml){let K=m[P];typeof K=="string"&&K.length>0&&(w[P]=K)}return w}async function je(){let w=Z();if(!w)return;let P=Ue();if(Object.keys(P).length===0){o("\uC800\uC7A5\uD560 \uAD6C\uD604 \uAC12\uC774 \uC5C6\uC2B5\uB2C8\uB2E4 \u2014 \uBA3C\uC800 \uAD6C\uD604 \uADF8\uB8F9\uC744 \uC120\uD0DD\uD558\uC138\uC694");return}let K=(w.presets||[]).find(Ie=>Ie.id===N),De=A.trim()||(K?K.name:"");if(!De){o("\uD504\uB9AC\uC14B \uC774\uB984\uC744 \uC785\uB825\uD558\uC138\uC694");return}try{let Ie=K?await r("impl-preset-update",{expected_revision:w.revision,id:K.id,name:De,settings:P}):await r("impl-preset-create",{expected_revision:w.revision,name:De,settings:P});if(Ie&&Ie.applied){if(A="",!K&&Array.isArray(Ie.presets)){let Fe=Ie.presets.find(Ee=>Ee.name===De);N=Fe?Fe.id:N}ee()}else o("\uD504\uB9AC\uC14B \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uACF3\uC5D0\uC11C \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4"),ee()}catch(Ie){o(`\uD504\uB9AC\uC14B \uC800\uC7A5 \uC2E4\uD328: ${Ie instanceof Error?Ie.message:String(Ie)}`)}}async function Xe(){let w=Z();if(!(!w||N.length===0))try{let P=await r("impl-preset-delete",{expected_revision:w.revision,id:N});P&&P.applied?(N="",ee()):(o("\uD504\uB9AC\uC14B \uC0AD\uC81C \uC2E4\uD328: \uB2E4\uB978 \uACF3\uC5D0\uC11C \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4"),ee())}catch(P){o(`\uD504\uB9AC\uC14B \uC0AD\uC81C \uC2E4\uD328: ${P instanceof Error?P.message:String(P)}`)}}async function We(){let w=Z();if(!(!w||N.length===0)){try{let P=await r("apply-impl-preset-global",{preset_id:N,expected_revision:w.revision});P&&P.applied?(_=Zt(P.values)?{...P.values}:{},m={..._},h=Array.isArray(P.warnings)?P.warnings:[]):P&&P.conflict&&o("\uAD6C\uD604 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328: \uD504\uB9AC\uC14B\uC774 \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4")}catch(P){o(`\uAD6C\uD604 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328: ${P instanceof Error?P.message:String(P)}`)}ee()}}async function Le(){M=!0,I=!1,ee();try{let w=await r("get-worker-system-prompt",{});!w||typeof w!="object"||Array.isArray(w)?I=!0:S=w}catch{I=!0}finally{M=!1,ee()}}function ye(){if($=!$,$&&!S){Le();return}ee()}function Re(){let w=Vr({loading:M,error:I});if(w)return w;if(!S)return"";let P=Array.isArray(S.variants)?S.variants:[];return i`<div class="settings-dialog__sp-body">
      ${S.target_base_placeholder?i`<div class="prompt-block__meta">
            \`${S.target_base_placeholder}\`는 디스패치 시점에 해석된
            base로 치환됩니다.
          </div>`:""}
      ${P.map(K=>i`<div class="settings-dialog__sp-variant" data-variant=${K.key}>
    ...
    >
      ...
      ${$?Re():""}
    </section>`}function V(w,P,K,De,Ie,Fe){let Ee=Ie[w]??Kt;return i`<select
    ...
    </select>`}function H(w,P,K,De,Ie,Fe=!1){return i`<div
    ...
    </div>`}function $e(w,P,K,De,Ie){return i`<div class="settings-dialog__row">
    ...
    </div>`}function te(){let w=B(),P=hl(m),K=m.impl_runtime,De=m.impl_model,Ie=Z();return i`
      <section
      ...
      >
        ...
        ${E?i`<div class="settings-dialog__empty">불러오는 중…</div>`:i`
              ...
              <div class="settings-dialog__group">
                ...
                ${H("impl_runtime","\uC704\uC784 \uB300\uC0C1",hs,Q,m,P)}
                ${H("impl_model","\uBAA8\uB378",ws(w,K),Q,m,P)}
                ${H("impl_effort","effort",Yr(w,K,De),Q,m,P)}
                ${H("impl_speed","\uC18D\uB3C4",bn,Q,m,P)}
              </div>
    ...
    `}function ie(){let w=t.queueStore?.get(),P=B(),K={orchestration_model:D.orchestration_model??(Zt(w)?w.orchestration_model:null),orchestration_effort:D.orchestration_effort??(Zt(w)?w.orchestration_effort:null),orchestration_speed:D.orchestration_speed??(Zt(w)?w.orchestration_speed:null)},De=ks(P,k),Ie=Yr(P,k||void 0,K.orchestration_model||Nt).filter(Ee=>Ee!==Nt),Fe=Zt(w)&&typeof w.slots=="number"?w.slots:2;return i`
      <section
      ...
      >
        ...
        <div class="settings-dialog__group">
          ...
          ${H("orchestration_model","\uBAA8\uB378",De,ke,K)}
    ...
    `}function W(){let w=n.get();return i`
      ...
      >
        ...
        ${w?i`
              ${bc(w,s(),O)}
              ${yc(w,d,{onDraft:P=>{d=P},onAdd:j,onRemove:R})}
              ${vc(w,re)}
            `:i`<div class="settings-dialog__empty">
              표시 정책을 불러오는 중…
            </div>`}
      </section>
    `}async function U(w){let P=n.get();if(P)try{let K=await r("display-policy-set",{expected_revision:P.revision,policy:w(P)});ge(K),K&&K.conflict&&K.policy&&(K=await r("display-policy-set",{expected_revision:K.policy.revision,policy:w(K.policy)}),ge(K)),K&&K.conflict&&o("\uD45C\uC2DC \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uD074\uB77C\uC774\uC5B8\uD2B8\uC640 \uCDA9\uB3CC")}catch{o("\uD45C\uC2DC \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328")}}function ge(w){w&&w.policy&&typeof w.policy=="object"&&n.set(w.policy)}function Pe(w){U(w)}function O(w){let P=n.get();if(!P)return;let K=!Zf(w,P);Pe(De=>Xf(w,De,K))}function j(){let w=d.trim();w.length!==0&&(d="",Pe(P=>P.hidden_prefixes.includes(w)?{hidden_prefixes:P.hidden_prefixes}:{hidden_prefixes:[...P.hidden_prefixes,w]}),ee())}function R(w){Pe(P=>({hidden_prefixes:P.hidden_prefixes.filter(K=>K!==w)}))}function re(w){let P=n.get();if(!P)return;let K=P.chips[w]===!1;Pe(()=>({chips:{[w]:K}}))}function ee(){Ge(i`
      ...
      `,a)}function fe(w){l=w,ee()}let ue=()=>{c=!1,t.onOpenChange?.(!1)};a.addEventListener("close",ue),a.addEventListener("cancel",ue);let Ae=w=>{w.target===a&&ne()};a.addEventListener("click",Ae);let T=null;n.subscribe&&(T=n.subscribe(()=>{c&&ee()}));let F=null;t.implPresetStore?.subscribe&&(F=t.implPresetStore.subscribe(()=>{c&&ee()}));function J(w="session"){c||(c=!0,t.onOpenChange?.(!0),l=w,d="",D={},ee(),typeof a.showModal=="function"?a.showModal():a.setAttribute("open",""),oe())}function ne(){c&&(c=!1,t.onOpenChange?.(!1),typeof a.close=="function"?a.close():a.removeAttribute("open"))}return{open:J,close:ne,sessionDraft:()=>({...m}),destroy(){c=!1,a.removeEventListener("close",ue),a.removeEventListener("cancel",ue),a.removeEventListener("click",Ae),T&&(T(),T=null),F&&(F(),F=null),a.remove()}}}function Zf(e,t){return t.visible_labels.includes(e)?!0:t.hidden_labels.includes(e)?!1:!t.hidden_prefixes.some(r=>r.length>0&&e.startsWith(r))}function Xf(e,t,r){if(!r)return{hidden_labels:t.hidden_labels.includes(e)?t.hidden_labels:[...t.hidden_labels,e],visible_labels:t.visible_labels.filter(o=>o!==e)};let n=t.hidden_labels.filter(o=>o!==e);return t.hidden_prefixes.some(o=>o.length>0&&e.startsWith(o))?{hidden_labels:n,visible_labels:t.visible_labels.includes(e)?t.visible_labels:[...t.visible_labels,e]}:{hidden_labels:n}}var Qf=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];function kc(e){return String(e).padStart(2,"0")}function Jf(e,t){let r=Math.max(0,Math.ceil((e-t)/6e4)),n=Math.floor(r/1440),s=Math.floor(r%1440/60),o=r%60;return n>0?`${n}d${s>0?` ${s}h`:""}`:s>0?`${s}h${o>0?` ${o}m`:""}`:`${o}m`}function e_(e,t=Date.now()){let r=Date.parse(e);if(!Number.isFinite(r))return"";let n=new Date(r),s=new Date(t),o=`${kc(n.getHours())}:${kc(n.getMinutes())}`,l=n.getFullYear()===s.getFullYear()&&n.getMonth()===s.getMonth()&&n.getDate()===s.getDate()?o:`${Qf[n.getMonth()]} ${n.getDate()} ${o}`;return`${Jf(r,t)} \xB7 ${l}`}function t_(e){return e>=85?"usage-meter__window--danger":e>=60?"usage-meter__window--warn":"usage-meter__window--success"}var $c=[{key:"claude",label:"Claude",endpoint:"/api/claude-usage"},{key:"codex",label:"Codex",endpoint:"/api/codex-usage"}];function xc(e){let t=!1,r=null,n=new Map;function s(){Ge(i``,e),e.hidden=!0}function o(){let c=$c.filter(_=>n.has(_.key));if(c.length===0){s();return}let d=Date.now();Ge(i`<div class="usage-meter" aria-label="Usage">
        ${c.map(_=>{let m=n.get(_.key),h=typeof m.ageSeconds=="number"&&m.ageSeconds>600,E=h?`${Math.floor(m.ageSeconds/60)}\uBD84 \uC804 \uCE21\uC815`:"";return i`<span
          ...
          >
            ...
            ${m.windows.map(k=>{let D=typeof k.pct=="number"&&Number.isFinite(k.pct)?k.pct:0,N=Math.min(100,Math.max(0,D)),$=`resets ${e_(k.resetsAt,d)}${h?` \xB7 ${E}`:""}`;return i`<span
      ...
      >`}return ie()?i`<span class="pa-meta__progress"
        ...
        >`:""}function ie(){return n?.isPending?.()===!0}function W(T){let F=m(),J=(Array.isArray(F.queue)?F.queue.length:0)+(Array.isArray(F.serial_lanes)?F.serial_lanes:[]).reduce((K,De)=>K+(Array.isArray(De.entries)?De.entries.length:0),0),ne=!!T.job,w=H(T.settings),P=ne||c||ie();return i`<div class="pa-meta">
    ...
    </div>`}function U(T,F){let J=N(T,F),ne=E(),w=J.filter(Ee=>ne.has(Ee)),P=k(J),K=A(J),De=Array.isArray(m().serial_lanes)?m().serial_lanes:[],Ie=l.get(T)||$(),Fe=F.eligible!==!0||J.length<2||w.length>0||P.length>0||K||c;return i`<section class="pa-group" data-group-index=${String(T)}>
      <header class="pa-group__head">
        ...
        ${P.length>0?i`<span class="pa-group__stale"
              >stale — ${P.join(", ")} 대기 영역 이탈</span
            >`:""}
      </header>
    ...
    </div>`}function Pe(){let T=j&&!!h().job;if(T&&_===null){_=setInterval(()=>O(),1e3);return}!T&&_!==null&&(clearInterval(_),_=null)}function O(){let T=h();d&&T.settings.runner===d&&(d=null);let F=T.last_good?.result;Pe(),Ge(i`
    ...
    </div>`}function A(){let $=s(),M=$.auto_repair!==!1,I=$.repo_operation_policy&&typeof $.repo_operation_policy=="object"?$.repo_operation_policy:null,S=Array.isArray($.repo_operations)?$.repo_operations:[],B=S.find(Q=>Q.state==="repairing"),Z=S.filter(Q=>Q.state==="failed"||Q.state==="repairing"),oe=Z.length?Math.min(...Z.map(Q=>typeof Q.repair?.remaining=="number"?Q.repair.remaining:0)):I?.auto_repair?.resolution_ladder?.find(Q=>Q.id==="auto_repair_session")?.attempts??1,we=Array.isArray(I?.auto_repair?.resolution_ladder)?I.auto_repair.resolution_ladder:[];return i`<section
    ...
    >
      ...
      ${I?i`<details
          ...
          >
            ...
            ${D("Worker\uAC00 \uC790\uB3D9 \uCC98\uB9AC",I.worker_automatic||[],"worker-automatic")}
            ...
            ${D("\uC790\uB3D9\uC73C\uB85C \uD558\uC9C0 \uC54A\uC74C",I.never_automatic||[],"never-automatic")}
          </details>`:""}
...
${pt.notes_tail}`:"notes\uC758 REVISE finding\uC744 \uC2A4\uD399\uC5D0 \uBC18\uC601\uD558\uB294 \uCC98\uBD84 \uC138\uC158\uC744 \uB744\uC6C1\uB2C8\uB2E4":"",usage:L==="done"?Dt(p.attempts||{},ce.bead_id):null,done_at:L==="done"&&typeof ce.added_at=="number"?ce.added_at:void 0,...u(ce.bead_id)}}),na=new Map;for(let g of p.done)g&&typeof g.bead_id=="string"&&typeof g.added_at=="number"&&na.set(g.bead_id,g.added_at);let Nr=p.attempts?Object.values(p.attempts):[],Ds=new Set;for(let g of Nr)g&&typeof g.resumed_from=="string"&&g.resumed_from.length>0&&Ds.add(g.resumed_from);let Ms=new Map;for(let g of Nr)Ms.set(g.bead_id,g.attempt_id);let Ns=new Map;for(let g of Nr)Ns.set(g.attempt_id,g);function Fs(g){let L=new Set,ce=g;for(;ce&&!L.has(ce.attempt_id);){if(ce.conflict_resolution===!0)return!0;L.add(ce.attempt_id),ce=typeof ce.resumed_from=="string"&&ce.resumed_from.length>0&&Ns.get(ce.resumed_from)||null}return!1}let Tn=typeof p.declared_base=="string"?p.declared_base:null;function sd(g){let L=null;for(let ce of Nr)!ce||ce.bead_id!==g||Fs(ce)||(L===null||(typeof ce.started_at=="number"?ce.started_at:0)>=(typeof L.started_at=="number"?L.started_at:0))&&(L=ce);return L&&typeof L.target_base=="string"?L.target_base:null}let sa=[],oa=[],od=g=>{let L=Ms.get(g.bead_id)!==g.attempt_id,ce=na.get(g.bead_id),ze=typeof ce=="number"&&ce>0&&typeof g.finished_at=="number"&&ce>=g.finished_at;return!L&&!ze&&typeof g.dismissed_at!="number"},aa=g=>{let L=typeof g.session_id=="string"&&g.session_id.length>0,ce=Ds.has(g.attempt_id);return{eligible:L&&!ce,reason:L?ce?"\uC774\uBBF8 \uC774\uC5B4\uBC1B\uC740 attempt (child attempt \uC874\uC7AC) \u2014 \uC774\uC5B4\uD558\uAE30 \uBD88\uAC00":null:"session_id \uC5C6\uB294 \uAD6C attempt \u2014 \uC774\uC5B4\uD558\uAE30 \uBD88\uAC00"}},jt=null;for(let g of Nr){let L=g.status==="paused"&&!Ds.has(g.attempt_id);if(g.status==="running"||L)oa.push({bead_id:g.bead_id,attempt_id:g.attempt_id,title:q.get(g.bead_id)||g.bead_id,runner:g.runner||null,model:g.model||null,effort:g.effort||null,speed:g.speed||null,continuation_mode:g.continuation_mode||null,started_at:typeof g.started_at=="number"?g.started_at:null,resumed_from:g.resumed_from||null,paused:L,conflict_resolution:Fs(g),base_exception:Ko(Tn,g.target_base),can_pause:typeof g.session_id=="string"&&g.session_id.length>0,discard:er(En,g.bead_id,{attempt_id:g.attempt_id}),usage:Dt(p.attempts||{},g.bead_id),current_child:G(g.bead_id),...u(g.bead_id)});else if((g.status==="failed"||g.status==="orphaned")&&od(g)){let ce=aa(g);sa.push({bead_id:g.bead_id,attempt_id:g.attempt_id,title:q.get(g.bead_id)||g.bead_id,runner:g.runner||null,model:g.model||null,effort:g.effort||null,speed:g.speed||null,continuation_mode:g.continuation_mode||null,started_at:typeof g.started_at=="number"?g.started_at:null,resumed_from:g.resumed_from||null,failed:!0,status:g.status,status_label:g.status==="orphaned"?"\uC911\uB2E8\uB428":"\uC2E4\uD328",discard:er(En,g.bead_id,{attempt_id:g.attempt_id}),resume_eligible:ce.eligible,resume_reason:ce.reason,conflict_resolution:Fs(g),base_exception:Ko(Tn,g.target_base),usage:Dt(p.attempts||{},g.bead_id),current_child:G(g.bead_id),...u(g.bead_id)}),jt=g}}let Cn=[...sa,...oa],ia=null;if(jt){let g=aa(jt),L=jt.cause_detail;ia={bead_id:jt.bead_id,repo:jt.repo||"",reason:jt.cause||jt.status,cause_detail:L&&typeof L.reason=="string"?{reason:L.reason,command:typeof L.command=="string"?L.command:null}:null,resume_attempt_id:jt.attempt_id,resume_eligible:g.eligible,resume_reason:g.reason,discard:er(En,jt.bead_id,{attempt_id:jt.attempt_id})}}let la=new Set(Cn.map(g=>g.bead_id)),qs=Array.isArray(p.merge_queue)?p.merge_queue:[],ca=new Map,da=new Map,ua=new Map,pa=new Map,fa=new Map;qs.forEach((g,L)=>{g&&typeof g.bead_id=="string"&&(ca.set(g.bead_id,L+1),da.set(g.bead_id,g.resolution),ua.set(g.bead_id,g.continuation_action||null),pa.set(g.bead_id,g.head_review||null),fa.set(g.bead_id,g.authority||null))});let _a=p.merge_queue_state||{active:null,failures:{}},ad=_a.failures||{},id=p.auto_merge_skips||{},ma=g=>{let L=id[g];if(!L)return null;let ce=Y[g],ze=ce&&ce.pr?ce.pr.head_sha:null;return ze&&ze===L.head_sha?L.reason||"":null},Rn=new Map;for(let g of Cn)g.failed!==!0&&g.conflict_resolution&&(g.paused?Rn.has(g.bead_id)||Rn.set(g.bead_id,"paused"):Rn.set(g.bead_id,"running"));let ga=Cn.filter(g=>!g.paused&&g.failed!==!0).length,ha=(p.workspace_info||{}).slots,ba=typeof ha=="number"?ha:typeof p.slots=="number"?p.slots:Is,ld=ga>ba,In=Er($),cd=(Array.isArray(p.done)?p.done.slice():[]).filter(g=>In===void 0||typeof g.added_at!="number"||g.added_at>=In).sort((g,L)=>(L.added_at||0)-(g.added_at||0)),Xr=Ps(cd,"done"),dd=new Set((Array.isArray(p.done)?p.done:[]).map(g=>g?.bead_id).filter(g=>typeof g=="string")),ya=[],ud=d?.()||"";for(let g of ae){let L=Rr(g.closed_at);if(typeof g.id!="string"||dd.has(g.id)||L===null||In!==void 0&&L<In||typeof g.comment_count!="number"||g.comment_count<=0)continue;let ce=`${ud}\0${g.id}\0${String(g.updated_at)}\0${g.comment_count}`,ze=M.get(ce);ze===void 0&&r&&(M.set(ce,"pending"),Promise.resolve(r("get-comments",{id:g.id})).then(ut=>{let Rt=Array.isArray(ut)&&ut.some(pt=>_s(typeof pt?.text=="string"?pt.text:"")?.lane==="session");M.set(ce,Rt?"session":"not-session"),se()}).catch(()=>{M.set(ce,"failed"),se()})),ze==="session"&&ya.push({id:g.id,title:g.title||g.id,reason:"",draggable:!1,done:!0,lane:"done",selectable:!1,selected:!1,worker_serial:!1,badges:["\uC138\uC158 \uC791\uC5C5"],alert:!1,usage:null,done_at:L,created_at:g.created_at,updated_at:g.updated_at})}Xr.push(...ya),Xr.sort((g,L)=>(L.done_at||0)-(g.done_at||0));let Ln={};for(let g of sr)Ln[g]=0;let va=!1,wa=0,Bs=0,ka=0;for(let g of Xr){let L=g.usage;if(L&&typeof L=="object"){let ce=!1;for(let ze of sr)Number.isFinite(L[ze])&&(Ln[ze]+=L[ze],va=!0,ce=!0);ce&&(Bs+=1,Number.isFinite(L.total_cost_usd)&&(wa+=L.total_cost_usd,ka+=1))}}Bs>0&&ka===Bs&&(Ln.total_cost_usd=wa);let $a=Xr.map(g=>g.usage).filter(g=>g&&typeof g=="object"&&g.providers),pd=$a.length>0?wt(Jn($a)):va?zt(Ln):null,fd=p.lane_states&&typeof p.lane_states=="object"&&!Array.isArray(p.lane_states)?p.lane_states:{},_d=Array.isArray(p.serial_lanes)?p.serial_lanes:[],xa=g=>{if(C.some(ze=>ze.bead_id===g))return"PR \uB300\uAE30 \xB7 \uC810\uC720";let L=Nr.filter(ze=>ze&&ze.bead_id===g),ce=L.length>0?L[L.length-1].status:null;return ce==="failed"||ce==="orphaned"?"\uC2E4\uD328 \xB7 \uC810\uC720 \uC720\uC9C0":ce==="paused"?"\uC77C\uC2DC\uC815\uC9C0 \xB7 \uC810\uC720":"\uC2E4\uD589 \uC911 \xB7 \uC810\uC720"},Sa=_d.filter(g=>g&&typeof g.id=="string"&&Array.isArray(g.entries)).map((g,L)=>{let ce=fd[g.id]||{},ze=new Map((Array.isArray(ce.corrections)?ce.corrections:[]).filter(st=>st&&typeof st.bead_id=="string"&&typeof st.after=="string").map(st=>[st.bead_id,st.after])),ut=Ps(g.entries.filter(st=>!la.has(st.bead_id)),g.id).map(st=>ze.has(st.id)?{...st,badges:[`\u{1F517} ${ze.get(st.id)} \uB4A4 (blocks \uC790\uB3D9)`,...st.badges]}:st),Rt=Array.isArray(ce.occupied_by)?ce.occupied_by.filter(st=>typeof st=="string"):[],pt=Rt.map(st=>({id:st,title:q.get(st)||st,draggable:!1,lane:g.id,ghost:!0,badges:[xa(st)]}));return{id:g.id,index:L+1,rows:[...pt,...ut],occupied:Rt.length>0,badge:Rt.length>0?xa(Rt[0]):"\uB300\uAE30",cycle:ce.cycle===!0}}),md=typeof p.serial_lane_count=="number"?p.serial_lane_count:Sa.length;return{queue:p,idToTitle:q,candidates:rd,candidate_hidden:{blocked:Os.hidden_blocked,spec:Os.hidden_spec},running:Cn,live_count:ga,slots:ba,over_cap:ld,failure:ia,waiting:Ps(_t.filter(g=>!la.has(g.bead_id)),"queue"),serial_lanes:Sa,serial_lane_count:md,pr_wait:C.map(g=>M_(g.bead_id,q.get(g.bead_id)||g.bead_id,Y,Oe[g.bead_id]||null,Dt(p.attempts||{},g.bead_id),he[g.bead_id]||(Z.has(g.bead_id)||oe.has(g.bead_id)?{activity:null,merge_progress:{step:"merging"}}:null),Rn.get(g.bead_id)||null,g.external===!0,{position:ca.get(g.bead_id)||0,active:_a.active===g.bead_id,failure:ad[g.bead_id]||null,resolution:da.get(g.bead_id),continuation_action:ua.get(g.bead_id),head_review:pa.get(g.bead_id)||null,authority:fa.get(g.bead_id)||null},g.wt_present!==!1,p.auto_merge===!0?ma(g.bead_id):null,Ko(Tn,sd(g.bead_id)),p.completion_status&&typeof p.completion_status=="object"&&!Array.isArray(p.completion_status)&&p.completion_status[g.bead_id]||null,p.discard_operations&&typeof p.discard_operations=="object"&&!Array.isArray(p.discard_operations)?p.discard_operations:{},Ns.get(Ms.get(g.bead_id)||"")?.worker_serial===!0,p.auto_merge===!0)).map(g=>({...g,...u(g.id)})),merge_queue_length:qs.length,merge_queue_running:qs.length>0,auto_excluded:C.map(g=>g.bead_id).filter(g=>ma(g)!==null),declared_base:Tn,done:Xr,token_total:pd,cleanup_failures:xe,repo_operations:Array.isArray(p.repo_operations)?p.repo_operations:[]}}function Qe(){let y=!!o?.get()?.job,x=!y&&o?.isPending?.()===!0,ae=y?"\uBD84\uC11D \uC911":x?"\uC900\uBE44 \uC911":"";return i`<button
    ...
    >`,b=i`<label class="worker-tgl worker-slots"
      ...
      <label
        ...
        <select class="worker-serial-lane-count" aria-label="직렬 레인 수">
          ${Array.from({length:Pc},(qe,Ye)=>Ye+1).map(qe=>i`<option
                value=${String(qe)}
      ...
      ${o?Qe():""} `,q=Kl({failure:p.failure}),pe=Bl(p.repo_operations,p.cleanup_failures);return B?i`<div class="worker-ribbon">
        ...
        <div class="worker-ctrl__ops">${x}${ae}${b}</div>
      ...
      ${pe}${V.template()}${q}`}function yt(p){if(p.running.length===0&&p.pr_wait.length===0)return"";let y=p.running.some(x=>!x.paused&&x.failed!==!0);return i`<section
    ...
    >`,x=p.cycle?i`<div class="worker-lane__cycle">
      ...
      >
        ⏸ 자동 머지
      </button>`;let x=new Set(p.auto_excluded),ae=p.pr_wait.filter(me=>me.merge_action&&me.merge_enabled&&!x.has(me.id)).length;return i`<button
      type="button"
    ...
    >
      ...
      ${Yt({id:"worker-pane-running",lane:"running",title:`\uC2E4\uD589 \uC911 \xB7 \uC2AC\uB86F ${p.slots}`,items:p.running,live:p.running.some(x=>!x.paused&&x.failed!==!0),body:qo(p.running,Date.now(),ye)})}
