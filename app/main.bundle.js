...
`+r.join(`
`),I=this.blockquote(A);o[o.length-1]=I,n=n.substring(0,n.length-R.raw.length)+I.raw,s=s.substring(0,s.length-R.text.length)+I.text;break}else if(h?.type==="list"){let R=h,A=R.raw+`
`+r.join(`
`),I=this.list(A);o[o.length-1]=I,n=n.substring(0,n.length-h.raw.length)+I.raw,s=s.substring(0,s.length-R.raw.length)+I.raw,r=A.substring(o.at(-1).raw.length).split(`
...
`,e=e.substring(h.length+1),u=!0),!u){let I=this.rules.other.nextBulletRegex(A),F=this.rules.other.hrRegex(A),Z=this.rules.other.fencesBeginRegex(A),Q=this.rules.other.headingBeginRegex(A),U=this.rules.other.htmlBeginRegex(A);for(;e;){let W=e.split(`
`,1)[0],O;if(h=W,this.options.pedantic?(h=h.replace(this.rules.other.listReplaceNesting,"  "),O=h):O=h.replace(this.rules.other.tabCharGlobal,"    "),Z.test(h)||Q.test(h)||U.test(h)||I.test(h)||F.test(h))break;if(O.search(this.rules.other.nonSpaceChar)>=A||!h.trim())p+=`
...
Please report this to https://github.com/markedjs/marked.`,e){let n="<p>An error occurred:</p><pre>"+br(r.message+"",!0)+"</pre>";return t?Promise.resolve(n):n}if(t)return Promise.reject(r);throw r}}},Ur=new Lf;function it(e,t){return Ur.parse(e,t)}it.options=it.setOptions=function(e){return Ur.setOptions(e),it.defaults=Ur.defaults,Ol(it.defaults),it};it.getDefaults=Ho;it.defaults=Wr;it.use=function(...e){return Ur.use(...e),it.defaults=Ur.defaults,Ol(it.defaults),it};it.walkTokens=function(e,t){return Ur.walkTokens(e,t)};it.parseInline=Ur.parseInline;it.Parser=tr;it.parser=tr.parse;it.Renderer=Os;it.TextRenderer=Jo;it.Lexer=er;it.lexer=er.lex;it.Tokenizer=Ls;it.Hooks=Ln;it.parse=it;var ih=it.options,lh=it.setOptions,ch=it.use,uh=it.walkTokens,dh=it.parseInline;var ph=tr.parse,fh=er.lex;function Tr(e){let t=it.parse(e),r=Al.sanitize(t);return Sl(r)}function hr(e,t){return l`<div class="prompt-block">
    ...
    <pre class="prompt-block__body">${t}</pre>
    ...
    </div>`}function C(){if(!i||!n)return[];let Y=n.get(i);return Hl(Y?Y.lines:[])}function H(){if(!i||!n)return null;let Y=n.get(i),j=Y?Y.last_event_at:null;return typeof j=="number"?j:null}function se(){return d.status==="running"}function M(){if(se()&&o){A||(A=setInterval(()=>he(),1e3));return}D()}function D(){A&&(clearInterval(A),A=null)}function ue(Y){let j=[],oe=0;for(;oe<Y.length;){let Oe=Y[oe];if(Oe.kind==="tool"){let We=oe;for(;We<Y.length&&Y[We].kind==="tool"&&Y[We].tool===Oe.tool;)We+=1;if(We-oe>=Wf&&!h.has(oe)){j.push({kind:"group",idx:oe,tool:Oe.tool||"",lines:Y.slice(oe,We).map((Ve,Ie)=>({idx:oe+Ie,line:Ve}))}),oe=We;continue}}j.push({kind:"line",idx:oe,line:Oe}),oe+=1}return j}function ge(Y){for(let j=Y.length-1;j>=0;j-=1){let oe=Y[j];if(oe.kind==="result"||oe.kind==="error")return null;if(oe.kind==="tool"&&!Object.hasOwn(oe,"result"))return oe}return null}function pe(Y){for(let j=Y.length-1;j>=0;j-=1)if(Y[j].kind==="thinking")return Y[j];return null}function qe(Y,j){if(j.kind==="gate")return l`<div class="sv__gate">${j.text}</div>`;if(j.kind==="phase")return l`<div class="sv__phase">${j.text}</div>`;if(j.kind==="result")return l`<div
      ...
      </div>`}if(j.kind==="error")return l`<div class="sv__error">⛔ ${j.text}</div>`;if(j.kind==="blocker")return l`<div class="sv__error">⛔ ${j.text}</div>`;if(j.kind==="tool"){let oe=f.has(Y),Oe=j.tool==="Bash"?Kf(j.command):0,We=j.tool==="Bash"?Oe>1?Ns(j.command):j.command:j.path||j.command||"";return l`<div
      ...
      >
        ...
        ${oe?l`<pre class="sv__tool-expand">${tt(j)}</pre>`:""}
      </div>`}return l`<div class="sv__as">${Tr(j.text||"")}</div>`}function tt(Y){let j=[];if(Y.tool==="Bash"&&typeof Y.command=="string"&&Y.command.length>0)j.push(Y.command);else if(Y.input!==void 0)try{j.push(`input: ${JSON.stringify(Y.input,null,2)}`)}catch{}return typeof Y.output=="string"&&Y.output.length>0&&j.push(`output:
${Y.output}`),j.join(`

`)}function je(){if(!o)return l``;let Y=C(),j=(a?[d.model,d.effort]:[d.runner,d.model,d.effort]).filter(Boolean).join(" \xB7 "),oe=d.session_id||"",Oe=`\uB77C\uC774\uBE0C \uB530\uB77C\uAC00\uAE30 ${p?"ON":"OFF"}`,We=se(),Ve=We?e_(H(),Date.now()):"",Ie=We?ge(Y):null,lt=We?pe(Y):null,Xe=Jf(Y);return l`<div class="sv" data-attempt-id=${o}>
      <div class="sv__bar">
      ...
      ${a||u?"":g()}
      <div class="sv__body">
        ${Y.length===0?l`<div class="sv__empty">세션 로그 없음</div>`:ue(Y).map(V=>V.kind==="group"?_e(V):qe(V.idx,V.line))}
      </div>
    ...
    </div>`}function Le(Y){h.add(Y),he()}function he(){Ye(je(),e),M(),p&&xe()}function xe(){let Y=e.querySelector(".sv__body");Y&&(Y.scrollTop=Y.scrollHeight)}function Re(Y){f.has(Y)?f.delete(Y):f.add(Y),he()}function Be(){p=!p,he()}function Ae(Y){Xt(Y).then(j=>{j?le("\uBCF5\uC0AC\uB428","success",1200):le("\uBCF5\uC0AC \uC2E4\uD328","error",1600)})}function Ue(Y){!o||!Y||(d={...d,...Y},he())}function Ze(Y){let j=Y.target;if(!j||!j.classList||!j.classList.contains("sv__body"))return;!(j.scrollHeight-j.scrollTop-j.clientHeight<=4)&&p&&(p=!1,he())}e.addEventListener("scroll",Ze,!0);function Ee(Y){let j=Y&&Y.attempt_id;if(!j)return;let oe=i;o=j,a=typeof Y.launch_id=="string"&&Y.launch_id.length>0?Y.launch_id:null,i=a?`session-log:${o}:${a}`:`session-log:${o}`,r&&oe&&oe!==i&&Promise.resolve(r("unsubscribe-session-log",{id:oe})).catch(()=>{}),d=Y.meta||{},u=Y.hide_prompt===!0,p=!0,f.clear(),h.clear(),W(),!R&&n&&(R=n.subscribe(he)),r&&Promise.resolve(r("subscribe-session-log",{id:i,attempt_id:o,...a?{launch_id:a}:{}})).catch(()=>{}),he()}function ot(){let Y=i;o=null,a=null,i=null,u=!1,f.clear(),h.clear(),W(),D(),r&&Y&&Promise.resolve(r("unsubscribe-session-log",{id:Y})).catch(()=>{}),Ye(l``,e),s&&s()}return{open:Ee,updateMeta:Ue,close:ot,isOpen(){return o!==null},destroy(){D(),R&&(R(),R=null),e.removeEventListener("scroll",Ze,!0),o=null,a=null,i=null,u=!1,Ye(l``,e)}}}function Fs(e){let t=e&&typeof e=="object"?e:{},r=t.metadata&&typeof t.metadata=="object"?t.metadata:{},n=na(t.spec_id),s=na(r.spec_id);return n?{path:n,source:"native",conflict:s.length>0&&s!==n}:s?{path:s,source:"metadata",conflict:!1}:{path:"",source:"none",conflict:!1}}function na(e){return typeof e=="string"?e.trim():""}function Gl(e){let t=Fs(e);if(t.path)return t;let r=na(t_(e).spec_path);return r?{path:r,source:"draft",conflict:!1}:t}function t_(e){let t=e&&typeof e=="object"?e:{};return t.metadata&&typeof t.metadata=="object"?t.metadata:{}}function r_(e){return["plan_review","plan_approval","plan_check"].some(t=>{let r=e[t];return typeof r=="string"&&r.trim().length>0})}function n_(e){let t=e&&e.metadata||{},r=Gl(e),n=[];return r.path&&n.push({kind:"spec",path:r.path,missing_state:r.source==="draft"?"spec_draft":null}),typeof t.plan_path=="string"&&t.plan_path.trim().length>0&&n.push({kind:"plan",path:t.plan_path.trim(),missing_state:r_(t)?null:"plan_pending"}),n}function Vl(e,t){let r=n_(e);return l`
...
`).trim()}}var Kl=20;function Yl(e){if(e==null||e==="")return"";let t=new Date(e);if(Number.isNaN(t.getTime()))return"";let r=String(t.getMonth()+1).padStart(2,"0"),n=String(t.getDate()).padStart(2,"0"),s=String(t.getHours()).padStart(2,"0"),o=String(t.getMinutes()).padStart(2,"0");return`${r}-${n} ${s}:${o}`}function i_(e){return e.length>Kl?`${e.slice(0,Kl)}\u2026`:e}function l_(e,t,r,n){let s=`${t.lane} ${i_(t.identifier)}`;return l`<div class="detail-report">
  ...
  >
  ...
  </details>`}function b_(e){let t=[];if(e.workflow_mode&&t.push(e.workflow_mode.display),e.impl_dispatch?.value==="main")t.push("\uBA54\uC778");else if(e.impl_dispatch?.value==="delegated"){let r=e.impl_runtime?` ${e.impl_runtime.display}`:"";t.push(`\uC704\uC784${r}`)}for(let r of["impl_model","impl_effort","impl_speed"])e[r]?.resolution!=="not_applicable"&&t.push(e[r]?.display||"\uAE30\uBCF8\uAC12 \uD655\uC778 \uBD88\uAC00");return t.join(" \xB7 ")}function h_(e){if(!e||typeof e!="object")return null;let{kind:t,actor:r,effort:n,sha:s}=e;return typeof t!="string"||typeof r!="string"||typeof s!="string"?null:{kind:t,actor:r,effort:typeof n=="string"?n:null,sha:s}}function cc(e){let t=e&&typeof e.metadata=="object"&&e.metadata?e.metadata:{},r=e&&typeof e.workflow=="object"&&e.workflow?e.workflow:{},n=r.stages||{},s=r.route||t.route||null,o=typeof t.pr_url=="string"?t.pr_url:"",a=typeof t.exec_receipt=="string"?t.exec_receipt:"",i=h_(r.exec_receipt),u=i?Br(i):a,d=i?`${i.kind}:${i.actor}`:a.split("@")[0],p=ys(r.planned_execution,r.exec_receipt);return l`<section class="detail-summary" data-seam="detail-summary">
    ...
    <div class="detail-summary__gates">
      ${p_.map(f=>{let h=f.receipt&&typeof t[f.receipt]=="string"?String(t[f.receipt]):"",R=n[f.id],A=h.length>0||R?.fill==="full",I=!A&&R?.fill==="dim",F=R?.stale===!0;return l`<span
    ...
    `:l``}function p(){Ye(d(),e)}async function f(A,I={}){s=A,o="loading",a="",i="",p();let F=r?r():"";if(!F){o="error",i="\uC6CC\uD06C\uC2A4\uD398\uC774\uC2A4\uAC00 \uC120\uD0DD\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4",p();return}if(!n){o="error",i="fetch\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4",p();return}let Z="/api/doc?workspace="+encodeURIComponent(F)+"&path="+encodeURIComponent(A);try{let Q=await n(Z),U=await Q.json().catch(()=>({}));if(!Q.ok||!U||U.ok!==!0){if(U?.error==="not_found"&&I.missing_state==="plan_pending"){o="pending",i="\uACC4\uD68D \uC791\uC131 \uC804 \xB7 \uACBD\uB85C\uB9CC \uC608\uC57D\uB418\uC5B4 \uC788\uC2B5\uB2C8\uB2E4",p();return}o="error",i="\uBB38\uC11C\uB97C \uBD88\uB7EC\uC624\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4 ("+String(U&&U.error||Q.status)+")",p();return}a=String(U.content||""),o="ready",p()}catch{o="error",i="\uBB38\uC11C \uC694\uCCAD \uC2E4\uD328",p()}}function h(){s=null,Ye(l``,e)}function R(){document.removeEventListener("keydown",u),h()}return{open:f,close:h,destroy:R}}var k_=[{key:"input_tokens",label:"\uC785\uB825"},{key:"output_tokens",label:"\uCD9C\uB825"},{key:"cache_read_input_tokens",label:"\uCE90\uC2DC \uC77D\uAE30"}],gc="\uC11C\uBC84 \uC7AC\uC2DC\uC791 \uBCF5\uAD6C \u2014 \uBD80\uBD84 \uC9D1\uACC4",Ks=["implementation","review-consult"],$_=["running","done","failed","interrupted"],x_={running:"\u25CF",done:"\u2713",failed:"\u2717",interrupted:"\u26A0"};function A_(e){return typeof e=="number"&&Number.isFinite(e)?e:0}function S_(e){let t=xt(e);if(t.length>0)return t.map(s=>l`<span class="detail-usage-total" title=${s.tooltip}
          >${s.label}</span
  ...
  >
    ...
    ${a?l`<span class="detail-session__usage" title=${a.tooltip}
          >${a.label}</span
        >`:""}
  </button>`}function R_(e,t){return e.role===t.role&&e.model===t.model&&e.session_id===t.session_id}function I_(e,t,r){let n=[],s=new Set,o=Array.isArray(e.delegation_sessions)?e.delegation_sessions:[];for(let p of o){let f=E_(p);!f||s.has(f.launch_id)||(s.add(f.launch_id),n.push(f))}n.sort((p,f)=>p.started_at-f.started_at);let a={implementation:[],"review-consult":[]};if(t)for(let p of Ks){let f=t.roles[p]?.codex;a[p]=f?[...f.legs]:[]}let i=Ks.flatMap(p=>a[p]),u=new Set,d=[];for(let p of Ks){for(let f of n.filter(h=>h.role===p)){let h=i.find(R=>R.receipt_id===f.launch_id)||null;h&&!R_(f,h)||(h&&u.add(h.receipt_id),d.push(C_(f,h,e.attempt_id,r)))}for(let f of a[p])u.has(f.receipt_id)||d.push(T_(p,f))}return d}function L_(e,t){let r=typeof e.total_cost_usd=="number"&&Number.isFinite(e.total_cost_usd)?e.total_cost_usd:null,n=[...k_,{key:"cache_creation_input_tokens",label:t==="codex"?"\uCE90\uC2DC \uC4F0\uAE30":"\uCE90\uC2DC \uC0DD\uC131"},...t==="codex"&&typeof e.reasoning_output_tokens=="number"&&Number.isFinite(e.reasoning_output_tokens)?[{key:"reasoning_output_tokens",label:"\uCD94\uB860 \uCD9C\uB825"}]:[]];return l`<div class="detail-session__usage-detail">
  ...
  >
    ...
    <div class="detail-sessions" data-seam="session-history">
      ${n.map(d=>{let p=Lo(d),f=mc(p),h=xt(f);return l`<div class="detail-session-row">
          <button
            type="button"
  ...
  `}function P_(e){let t=cn(e);if(t)return t;let r=e.data;if(!r)return"";if(r.missing)return l`<div class="detail-prompt__missing">
    ...
    ${typeof r.system_prompt=="string"?hr("\uC2DC\uC2A4\uD15C \uACC4\uC57D (--append-system-prompt)",r.system_prompt):""}`}var D_=["open","in_progress","deferred","resolved","closed"],N_=[0,1,2,3,4];function yc(e,t){let r=t.issueStores,n=t.onClose,s=t.transport,o=t.onNavigate,a=t.queueStore,i=t.execPresetStore,u=t.sessionLogStore,d=null,p=null,f={},h="",R=!1,A=[],I=!1,F={},Z=!1,Q=!1,U="",W="",O="";function z(){Z=!1,Q=!1,U="",W="",O=""}let g=[],C=null,H=null,se=!1,M="",D=!1,ue=0,ge=new Set;function pe(){g=[],C=null,H=null,se=!1,M="",D=!1,ue+=1,ge.clear()}async function qe(m){if(!s)return;let L=++ue;try{let E=await Promise.resolve(s("get-comments",{id:m}));if(L!==ue||m!==d)return;g=Array.isArray(E)?E:[],se=!1}catch{if(L!==ue||m!==d)return;se=!0}w()}function tt(){if(!s||!d)return;let m=p&&typeof p.comment_count=="number"?p.comment_count:null;if(C!==d){C=d,H=m,qe(d);return}m!==null&&m!==H&&(H=m,qe(d))}function je(m){ge.has(m)?ge.delete(m):ge.add(m),w()}function _e(m){let L=M.trim().length===0;M=m,L!==(m.trim().length===0)&&w()}async function Le(){let m=M.trim();if(!s||!d||m.length===0||D)return;let L=d;D=!0,w();let E=!1;try{let ne=await Promise.resolve(s("add-comment",{id:L,text:m}));Array.isArray(ne)&&ne.length>0&&(E=!0,L===d&&(g=ne,se=!1,M="",H=ne.length))}catch{E=!1}E||le("\uB313\uAE00 \uCD94\uAC00 \uC2E4\uD328","error"),L===d&&(D=!1),w()}let he={onToggle:je,onDraftInput:_e,onSubmit:Le},xe=document.createElement("div");xe.className="md-viewer-root",document.body.appendChild(xe);let Re=_c(xe,{getWorkspacePath:t.getWorkspacePath||(()=>"")}),Be=document.createElement("div");Be.className="session-log-root",document.body.appendChild(Be);let Ae=qs(Be,{transport:s?(m,L)=>Promise.resolve(s(m,L)):void 0,sessionLogStore:u}),Ue=!1,Ze=!1,Ee=!1,ot=null,Y=null,j=0;function oe(m){return`${t.getWorkspacePath&&t.getWorkspacePath()||""}::${m}`}function Oe(){Ue=!1,Ze=!1,Ee=!1,ot=null,Y=null,j+=1}async function We(m){if(!s)return;let L=++j;Ze=!0,Ee=!1,w();try{let E=await Promise.resolve(s("get-bead-prompt",{bead_id:m}));if(L!==j)return;!E||typeof E!="object"||Array.isArray(E)?Ee=!0:(ot=E,Y=oe(m))}catch{L===j&&(Ee=!0)}finally{L===j&&(Ze=!1,w())}}function Ve(){if(Ue=!Ue,Ue&&d&&Y!==oe(d)){ot=null,We(d);return}w()}function Ie(){if(!a||!d)return[];let m=a.get();return(m&&m.attempts?Object.values(m.attempts):[]).filter(E=>E&&E.bead_id===d).sort((E,ne)=>(ne.started_at||0)-(E.started_at||0)).map(E=>({attempt_id:E.attempt_id,bead_id:E.bead_id,status:E.status,started_at:typeof E.started_at=="number"?E.started_at:null,runner:E.runner||null,model:E.model||null,effort:E.effort||E.observed_effort||null,speed:E.speed||null,session_id:E.session_id||null,resumed_from:E.resumed_from||null,continuation_mode:E.continuation_mode||null,dismissed_at:typeof E.dismissed_at=="number"?E.dismissed_at:null,cause:typeof E.cause=="string"?E.cause:null,cause_detail:E.cause_detail||null,exec_default_preset_id:typeof E.exec_default_preset_id=="string"?E.exec_default_preset_id:null,exec_default_preset_revision:typeof E.exec_default_preset_revision=="number"?E.exec_default_preset_revision:null,exec_values:E.exec_values&&typeof E.exec_values=="object"?E.exec_values:null,usage:E.usage||null,usage_legs:Array.isArray(E.usage_legs)?E.usage_legs:[],delegation_sessions:Array.isArray(E.delegation_sessions)?E.delegation_sessions:[]}))}function lt(){if(!a||!d)return null;let m=a.get();return zt(m&&m.attempts||{},d)}let Xe=new Set;function V(m){Xe.has(m)?Xe.delete(m):Xe.add(m),w()}function te(m){let L=a?a.get():null,E=L&&L.attempts?L.attempts[m]:null;Ae.open({attempt_id:m,meta:E?{runner:E.runner||void 0,model:E.model||void 0,effort:E.effort||void 0,status:E.status||void 0,session_id:E.session_id||void 0}:{}})}function Me(m,L){let E=a?a.get():null,ne=E&&E.attempts?E.attempts[m]:null,Ke=(ne&&Array.isArray(ne.delegation_sessions)?ne.delegation_sessions:[]).find(Qe=>Qe&&typeof Qe=="object"&&Qe.launch_id===L);Ke&&Ae.open({attempt_id:m,launch_id:L,meta:{runner:"codex",role:Ke.role,model:Ke.model,effort:Ke.effort,session_id:Ke.session_id,status:Ke.status}})}async function ze(m){if(!s||!m)return;let L=await sn();if(L===null)return;let E=()=>{let Qe=a?a.get():null;return Qe&&typeof Qe.revision=="number"?Qe.revision:0},ne=async(Qe={},De=E())=>await s("worker-attempt-resume",{attempt_id:m,expected_revision:De,...L!==""?{instructions:L}:{},...Qe}),Fe=Qe=>{Qe?.queue&&a?.set&&a.set(Qe.queue)},Ke=await ne();if(Fe(Ke),Ke&&Ke.conflict){let Qe=Ke.queue&&typeof Ke.queue.revision=="number"?Ke.queue.revision:E();Ke=await ne({},Qe),Fe(Ke)}Ke=await fr(Ke,(Qe,De)=>ne({continuation:Qe,decision_token:De}),{onResult:Fe,refresh:()=>ne()}),Ke&&Ke.resumed===!1&&!Ke.conflict&&Ke.reason&&le(`\uC774\uC5B4\uD558\uAE30 \uAC70\uBD80: ${Ke.reason}`,"error",2400)}let fe={onOpen:te,onOpenDelegation:Me,onResume:ze,onToggleUsage:V};function b(){let m=a?a.get():null,L={...F};for(let E of["orchestration_model","orchestration_effort","orchestration_speed"]){let ne=m&&m[E];typeof ne=="string"&&(L[E]=ne)}return L}async function x(){if(s){try{let m=await Promise.resolve(s("get-session-defaults",{}));F=m&&m.values&&typeof m.values=="object"?m.values:{}}catch{F={}}w()}}function $(){let m=a?a.get():null;return m&&m.runner_catalog||null}function P(){let m=a?a.get():null;return m&&typeof m.execution_defaults=="object"?m.execution_defaults:null}function K(){let m=p?.metadata&&typeof p.metadata=="object"?p.metadata:{},E=nn({pin:{...m,...f},global:b(),execution_defaults:P(),runner_catalog:$(),route:typeof m.route=="string"?m.route:null}).orchestration_model.value||"";return la($(),E)}function X(){let m=i?i.get():null;return!m||typeof m.revision!="number"?null:{revision:m.revision,presets:Array.isArray(m.presets)?m.presets:[]}}function ae(m){return m?.compatible===!1}function de(m){i&&m&&typeof m.revision=="number"&&Array.isArray(m.presets)&&i.set({revision:m.revision,presets:m.presets})}async function Ne(){let m=X(),L=m?.presets.find(E=>E.id===h);if(!(!s||!d||!m||!L||ae(L)||R)){R=!0,A=[],w();try{let E=await Promise.resolve(s("apply-impl-preset",ic(d,L.id,m.revision)));if(E&&E.conflict){de(E),le("\uD504\uB9AC\uC14B\uC774 \uBCC0\uACBD\uB410\uC2B5\uB2C8\uB2E4. \uCD5C\uC2E0 \uBAA9\uB85D\uC5D0\uC11C \uB2E4\uC2DC \uC801\uC6A9\uD558\uC138\uC694.","error",4e3);return}let ne=E&&Array.isArray(E.issue)?E.issue[0]:E?.issue;if(E&&E.applied&&ne&&typeof ne=="object"){p=ne,A=Array.isArray(E.skipped_orchestration_keys)?E.skipped_orchestration_keys.filter(Fe=>typeof Fe=="string"):[];for(let Fe of uc)delete f[Fe];le(A.length>0?"\uC2E4\uD589 \uD504\uB9AC\uC14B\uC744 \uC801\uC6A9\uD588\uC2B5\uB2C8\uB2E4. \uC624\uCF00\uC2A4\uD2B8\uB808\uC774\uC158 3\uD0A4\uB294 Bead\uC5D0 \uD540\uD560 \uC218 \uC5C6\uC5B4 \uAC74\uB108\uB6F0\uC5C8\uC2B5\uB2C8\uB2E4.":"\uC2E4\uD589 \uD504\uB9AC\uC14B\uC744 \uC801\uC6A9\uD588\uC2B5\uB2C8\uB2E4.","success",4e3);return}E&&E.error==="bd_readback_failed"?le("\uC124\uC815\uC740 \uC804\uC1A1\uB410\uC9C0\uB9CC \uC801\uC6A9 \uC5EC\uBD80 \uD655\uC778\uC774 \uD544\uC694\uD569\uB2C8\uB2E4.","error",4e3):le("\uC2E4\uD589 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328","error",4e3)}catch(E){E&&typeof E=="object"&&E.code==="bd_readback_failed"?le("\uC124\uC815\uC740 \uC804\uC1A1\uB410\uC9C0\uB9CC \uC801\uC6A9 \uC5EC\uBD80 \uD655\uC778\uC774 \uD544\uC694\uD569\uB2C8\uB2E4.","error",4e3):le("\uC2E4\uD589 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328","error",4e3)}finally{R=!1,w()}}}let ke=null;r&&r.subscribe&&(ke=r.subscribe(()=>rt()));let Te=null;a&&typeof a.subscribe=="function"&&(Te=a.subscribe(()=>{d&&w()}));let ye=null;i&&typeof i.subscribe=="function"&&(ye=i.subscribe(()=>{d&&w()}));function He(m){m.key==="Escape"&&d&&(m.preventDefault(),n())}document.addEventListener("keydown",He);function rt(){if(d){if(r&&typeof r.snapshotFor=="function"){let m=r.snapshotFor("detail:"+d)||[];p=m.find(E=>E&&E.id===d)||m[0]||p}tt(),w()}}function B(m){Xt(m).then(L=>{L?le("\uBCF5\uC0AC\uB428","success",1200):le("\uBCF5\uC0AC \uC2E4\uD328","error",1600)})}function re(m){m.preventDefault(),m.stopPropagation(),d&&B(d)}function be(m,L){m.preventDefault(),m.stopPropagation(),B(L)}function k(m,L,E){m.preventDefault(),m.stopPropagation(),Re.open(L,{missing_state:E})}function T(m,L){f[m]=L,w(),!(!s||!d)&&Promise.resolve(s("update-exec-settings",ac(d,m,L.length===0?null:L))).catch(()=>{le("\uC2E4\uD589 \uC124\uC815 \uBCC0\uACBD \uC2E4\uD328","error")})}function N(m,L){let E=p||{},ne=E.metadata&&typeof E.metadata=="object"?E.metadata:{},Fe={};for(let De of["impl_runtime","impl_model","impl_effort"])Fe[De]=Object.hasOwn(f,De)?f[De]:typeof ne[De]=="string"?ne[De]:"";Fe[m]=L;let Ke=fc(Fe,$(),K()),Qe={};for(let De of["impl_runtime","impl_model","impl_effort"])Qe[De]=f[De],f[De]=Ke[De]||"";w(),!(!s||!d)&&Promise.resolve(s("update-impl-target",{id:d,...Ke,orchestration_runtime:K()})).then(De=>{let mt=Array.isArray(De)?De[0]:De;if(!mt||typeof mt!="object"||!mt.id)throw new Error("implementation target readback failed");p=mt;for(let sr of["impl_runtime","impl_model","impl_effort"])delete f[sr];w()}).catch(()=>{for(let De of["impl_runtime","impl_model","impl_effort"])Qe[De]===void 0?delete f[De]:f[De]=Qe[De];w(),le("\uAD6C\uD604 target \uBCC0\uACBD \uC2E4\uD328","error")})}async function ee(m,L,E){if(!s||!d)return!1;try{let ne=await Promise.resolve(s(m,L)),Fe=Array.isArray(ne)?ne[0]:ne;return Fe&&typeof Fe=="object"&&Fe.id?(p=Fe,!0):(le(E,"error"),!1)}catch{return le(E,"error"),!1}}function $e(m){setTimeout(()=>{try{let L=e.querySelector(m);L&&typeof L.focus=="function"&&L.focus()}catch{}},0)}function J(){Z=!0,U=p&&p.title||"",w(),$e('.detail-edit__input[data-edit="title"]')}function Se(m){U=m.target.value}function Ce(){Z=!1,U="",w()}function ft(){ee("edit-text",{id:d,field:"title",value:U},"\uC81C\uBAA9 \uC800\uC7A5 \uC2E4\uD328").then(L=>{L&&(Z=!1,U=""),w()})}function Et(){Q=!0,W=p&&p.description||"",w(),$e('.detail-edit__textarea[data-edit="description"]')}function nt(m){W=m.target.value}function yt(){Q=!1,W="",w()}function ur(){ee("edit-text",{id:d,field:"description",value:W},"\uC124\uBA85 \uC800\uC7A5 \uC2E4\uD328").then(L=>{L&&(Q=!1,W=""),w()})}function vt(m,L,E,ne){if(m.key==="Escape"){m.stopPropagation(),E();return}m.key==="Enter"&&(!ne||m.ctrlKey||m.metaKey)&&(m.preventDefault(),L())}function Tt(m){let L=m.target.value;ee("update-status",{id:d,status:L},"\uC0C1\uD0DC \uBCC0\uACBD \uC2E4\uD328").then(()=>w())}function dr(m){let L=Number(m.target.value);ee("update-priority",{id:d,priority:L},"\uC6B0\uC120\uC21C\uC704 \uBCC0\uACBD \uC2E4\uD328").then(()=>w())}function wr(m){O=m.target.value}function Bt(){let m=O.trim();m.length!==0&&ee("label-add",{id:d,label:m},"\uB77C\uBCA8 \uCD94\uAC00 \uC2E4\uD328").then(L=>{L&&(O=""),w()})}function Gt(m){if(m.key==="Escape"){m.stopPropagation(),O="",w();return}m.key==="Enter"&&(m.preventDefault(),Bt())}function wt(m){ee("label-remove",{id:d,label:m},"\uB77C\uBCA8 \uC81C\uAC70 \uC2E4\uD328").then(()=>w())}let nr={onCopyPath:be,onOpenDoc:k};function st(m){return typeof m=="string"?m:m&&typeof m=="object"?String(m.id||m.to||m.issue_id||m.depends_on||""):""}function Mt(m){switch(m&&typeof m=="object"?String(m.dependency_type||m.type||""):""){case"blocks":return"\u26D3";case"discovered-from":return"\u21A9";case"parent-child":return"\u2338";default:return""}}function kr(m){let E=(Array.isArray(m.dependencies)?m.dependencies:[]).map(ne=>({id:st(ne),icon:Mt(ne)})).filter(ne=>ne.id.length>0);return l`
    ...
    `}function Vt(m){let L=m.metadata||{},E=m.workflow||{},ne=E.stages||{},Fe=ne.spec&&ne.spec.stale,Ke=ne.impl&&ne.impl.stale,Qe=ne.plan||null,De=E.route_source==="derived",mt=E.route||L.route||"\u2014";return l`
    ...
    `}let ve={route:["quick_fix","spec_backed","full_plan"]};async function c(m,L){let E=L.target.value;if(m==="route"&&p&&p.metadata&&p.metadata.route==="full_plan"&&E!=="full_plan"&&!window.confirm(`full_plan \u2192 ${E||"(\uBBF8\uC124\uC815)"} \uC804\uD658: \uC800\uC7A5\uB41C plan \uC2B9\uC778\uC740 \uD3EC\uAE30\uB418\uBA70, plan \uD30C\uC77C\xB7\uB9C8\uCEE4 \uC815\uB9AC\uB294 \uC138\uC158 \uACC4\uC57D\uC774 \uC218\uD589\uD569\uB2C8\uB2E4. \uACC4\uC18D\uD560\uAE4C\uC694?`)){w();return}await ee("update-workflow-meta",{id:d,key:m,value:E},"\uC6CC\uD06C\uD50C\uB85C\uC6B0 \uBA54\uD0C0 \uBCC0\uACBD \uC2E4\uD328"),w()}function _(m){let L=m.metadata||{};return l` ${((ne,Fe)=>{let Ke=ve[ne],Qe=typeof L[ne]=="string"?L[ne]:"";return l`<div class="detail-kv">
    ...
    `}function Je(m){let L=Array.isArray(m.labels)?m.labels:[];return l`
    ...
    `}function v(){if(!d)return l``;let m=p||{},L=String(m.id||d),E=m.title||"(\uC81C\uBAA9 \uC5C6\uC74C)",ne=lt(),Fe=m.status||"open",Ke=typeof m.priority=="number"?Math.max(0,Math.min(4,m.priority)):"",Qe=m.description||"",De={...m,metadata:{...m.metadata||{},...f}};return l`
      <div class="detail-overlay" role="dialog" aria-modal="true">
        ...
        <div class="detail-overlay__panel">
          ...
          ${S(E,ne)}
          ${cc(De)}
          ${lc({metadata:De.metadata,workspace_values:b(),catalog:$(),execution_defaults:P(),expanded:I,presets:X()?.presets||[],preset_id:h,preset_busy:R,skipped_orchestration_keys:A},{onToggle:mt=>{I=mt,w()},onEdit:(mt,sr)=>{if(mt==="impl_runtime"||mt==="impl_model"||mt==="impl_effort"){N(mt,sr??"");return}T(mt,sr??"")},onPresetSelect:mt=>{h=mt,A=[],w()},onPresetApply:()=>{Ne()}})}
          ${ie(Fe,Ke)} ${G(m)}
          ${we(Qe)}
          ${Zl(g,he,{expanded:ge,draft:M,sending:D,error:se})}
          ${me(m)} ${Je(m)} ${kr(m)}
          ${Vt(m)} ${_(m)}
          ${Vl(m,nr)}
          ${hc({expanded:Ue,loading:Ze,error:Ee,data:ot},{onToggle:Ve})}
          ${bc(Ie(),fe,{total:ne,expanded:Xe})}
        </div>
      </div>
    `}function w(){Ye(v(),e)}return{load(m){m!==d&&(f={},h="",A=[],I=!1,z(),pe(),Oe()),d=m,p=null,rt(),x()},clear(){d=null,p=null,f={},h="",R=!1,A=[],I=!1,z(),pe(),Oe(),Re.close(),Ae.close(),Ye(l``,e)},destroy(){ke&&(ke(),ke=null),Te&&(Te(),Te=null),ye&&(ye(),ye=null),document.removeEventListener("keydown",He),Re.destroy(),xe.parentNode&&xe.parentNode.removeChild(xe),Ae.destroy(),Be.parentNode&&Be.parentNode.removeChild(Be),d=null,p=null,h="",R=!1,A=[],pe(),Oe(),Ye(l``,e)}}}function vc(e){let t=document.createElement("dialog");t.id="fatal-error-dialog",t.setAttribute("role","alertdialog"),t.setAttribute("aria-modal","true"),t.innerHTML=`
  ...
  >`,A=l`<span class="worker-mini__title">${e.title}</span>`,I=e.pr_url&&e.pr_number?l`<a
        ...
        >`:"",Z=r.map(pe=>pe===e.live_badge?l`<span
        ...
        >`:l`<span
          ...
          >${pe}</span
        >`),Q=e.reason?l`<span class="worker-mini__reason">${e.reason}</span>`:"",U=n.length>0?n.map(pe=>l`<span class="worker-usage" title=${pe.tooltip}
              >${pe.label}</span
      ...
      >
        ...
        >
          ${C?.label||"\uD3D0\uAE30"}
        </button>`:"",se=e.stale_work||null,M=se?l`${se.can_resume||se.can_continue?l`<button
            type="button"
          ...
          >
            다시 확인
          </button>`:""}`:"",D=se?l`<div class="worker-mini__stale">
        ...
        >
          승인하고 진행
        </button>`:"",ge=!!(s||o||e.merge_action||e.cancel_action||e.timeline_action||e.discard_action||C?.operation||e.revise_action||se);return l`<div
  ...
  >
    ${i?l`<div class="worker-mini__row1">${h}${R}${A}</div>
          <div class="worker-mini__row2">
            ...
            <div class="worker-mini__body">${A}${D}</div>
            ...
            ${dn(e)}`:l`<div class="worker-mini__line">
              ${d}${p}${h}${R}${A}${I}${F}${Z}${f}${Q}${U}${W}${O}${z}${g}${H}
            </div>
            ${vr(e)} ${dn(e)}`}
  ...
  >
    ...
    <path d="M8.8 3.6 14 8l-5.2 4.4z" />`)}var Wn=1,tm=6e4,rm={auto_merge:"\uC790\uB3D9 \uBA38\uC9C0",merged:"\uBA38\uC9C0",merge:"\uBA38\uC9C0",pr_stop:"PR \uC911\uB2E8",stopped:"\uC911\uB2E8",failed:"\uC2E4\uD328"},nm=new Set(["auto_merge","merged","merge","done"]),Wc={running:3,paused:2,failed:1};function sm(e,t){let r=null,n=-1/0;for(let s of Object.values(e)){if(!s||s.bead_id!==t||s.status==="running")continue;let o=typeof s.finished_at=="number"?s.finished_at:typeof s.started_at=="number"?s.started_at:0;o>=n&&(n=o,r=s)}return r}function om(e,t){let r=Object.values(e||{}),n=new Set,s=new Map;for(let a of r)!a||typeof a.bead_id!="string"||(typeof a.resumed_from=="string"&&a.resumed_from.length>0&&n.add(a.resumed_from),s.set(a.bead_id,a.attempt_id));let o=new Map;for(let a of r){if(!a||typeof a.bead_id!="string"||a.bead_id.length===0)continue;let i=null;if(a.status==="running")i="running";else if(a.status==="paused"&&!n.has(a.attempt_id))i="paused";else if(a.status==="failed"||a.status==="orphaned"){let f=t.get(a.bead_id),h=typeof f=="number"&&f>0&&typeof a.finished_at=="number"&&f>=a.finished_at;s.get(a.bead_id)===a.attempt_id&&!h&&typeof a.dismissed_at!="number"&&(i="failed")}if(!i)continue;let u=typeof a.started_at=="number"?a.started_at:null,d=o.get(a.bead_id);if(d){let f=Wc[d.run_state],h=Wc[i];if(f>h||f===h&&(d.started_at??0)>(u??0))continue}let p=typeof a.session_id=="string"&&a.session_id.length>0;o.set(a.bead_id,{attempt_id:typeof a.attempt_id=="string"?a.attempt_id:"",run_state:i,started_at:u,last_event_at:typeof a.last_event_at=="number"?a.last_event_at:null,runner:typeof a.runner=="string"?a.runner:null,model:typeof a.model=="string"?a.model:null,effort:typeof a.effort=="string"?a.effort:null,speed:typeof a.speed=="string"?a.speed:null,resumed_from:typeof a.resumed_from=="string"?a.resumed_from:null,continuation_mode:a.continuation_mode==="session"||a.continuation_mode==="fresh"?a.continuation_mode:null,usage:zt(e,a.bead_id),can_pause:i==="running"&&p,can_resume:i!=="running"&&p&&!n.has(a.attempt_id)})}return o}function zc(e,t){let r=e[t];if(!r)return"";if(r.stale===!0)return"\u267B\uFE0F stale\u2192\uC7AC\uB9AC\uBDF0";let n=typeof r.reason=="string"?r.reason:"",s=n.indexOf(":");return s>0&&s<n.length-1?`\u26D4 ${n.slice(0,s)} (${n.slice(s+1)})`:`\u26D4 ${n}`}function At(e){return e&&typeof e=="object"?e:{}}function ka(e,t,r){let n=Array.isArray(e)?e:[],s=Array.isArray(t)?t:[],o=r&&typeof r.done_since=="number"?r.done_since:void 0,a=new Map;for(let g of s)g&&typeof g.root_dir=="string"&&a.set(g.root_dir,g);let i=[],u=[],d=[],p=[],f=[],h=[],R=new Map,A=new Map,I=new Map;for(let g of n){if(!g||typeof g.root_dir!="string")continue;let C=g.root_dir,H=g.name||C,se=a.get(C),M=se&&typeof se.revision=="number"?se.revision:typeof g.revision=="number"?g.revision:0,D=At(g.attempts),ue=At(g.bead_titles),ge=At(g.pr_observations),pe=At(g.admission),qe=At(g.revise_parked),tt=At(g.merge_queue_state),je=At(g.cleanup_failed),_e=At(g.discard_operations),Le=At(g.bead_blocked_by),he=At(g.pr_activity),xe=Array.isArray(g.repo_operations)?g.repo_operations:[],Re=Array.isArray(g.merge_queue)?g.merge_queue:[],Be=new Set(Re.filter(V=>V&&typeof V.bead_id=="string").map(V=>V.bead_id)),Ae=new Map(Re.filter(V=>V&&typeof V.bead_id=="string").map(V=>[V.bead_id,V])),Ue=Array.isArray(g.queue)?g.queue:[],Ze=(Array.isArray(g.serial_lanes)?g.serial_lanes:[]).filter(V=>V&&/^s[1-5]$/.test(V.id)&&Array.isArray(V.entries)),Ee=At(g.lane_states),ot=typeof g.serial_lane_count=="number"?Math.max(0,Math.min(5,Math.floor(g.serial_lane_count))):Math.min(5,Ze.length);I.set(C,ot);let Y=new Map(Ze.map(V=>[V.id,V])),j=new Map;for(let V of Ze)for(let te of V.entries)te&&typeof te.bead_id=="string"&&j.set(te.bead_id,V.id);let oe=Array.isArray(g.done)?g.done:[];for(let V of oe)V&&typeof V.bead_id=="string"&&h.push({id:V.bead_id,root_dir:C,workspace_name:H});let Oe=new Map;for(let V of oe)V&&typeof V.bead_id=="string"&&typeof V.added_at=="number"&&Oe.set(V.bead_id,V.added_at);let We=V=>({id:V,title:ue[V]||V,root_dir:C,workspace_name:H,expected_revision:M,draggable:!1}),Ve=new Set;for(let[V,te]of om(D,Oe))Ve.add(V),u.push({...We(V),lane:"running",...j.has(V)?{serial_lane_id:j.get(V)}:{},attempt_id:te.attempt_id,run_state:te.run_state,can_pause:te.can_pause,can_resume:te.can_resume,started_at:te.started_at,last_event_at:te.last_event_at,runner:te.runner,model:te.model,effort:te.effort,speed:te.speed,resumed_from:te.resumed_from,continuation_mode:te.continuation_mode,usage:te.usage,discard:cr(_e,V,{attempt_id:te.attempt_id}),badges:te.run_state==="paused"?["\u23F8 \uC77C\uC2DC\uC815\uC9C0"]:te.run_state==="failed"?["\u26A0 \uC2E4\uD328"]:[],alert:te.run_state==="failed"});for(let V of Array.isArray(g.pr_wait)?g.pr_wait:[]){let te=V&&V.bead_id;if(typeof te!="string"||Ve.has(te))continue;Ve.add(te);let Me=At(ge[te]),ze=At(Me.pr),fe=Me.gate?At(Me.gate):null,b=Be.has(te),x=Ae.get(te)?.continuation_action||null,$=!!x&&x.continuation===null,P=tt.active===te,K=V.external===!0,X=je[te]||null,ae=At(he[te]),de=Un({bead_id:te,merge_sha:V.merge_sha,cleanup_cursor:V.cleanup_cursor,merge_progress:ae.merge_progress||null,cleanup_failed:X,repo_operations:xe}),Ne=to(de),ke=!!fe&&fe.base_badge==="\uCDA9\uB3CC",Te=!!X&&["child_sweep","branch_cleanup","parent_close"].includes(X.step)&&!!fe&&fe.tier==="merged",ye=K&&!!X&&!!fe&&fe.tier==="merged",He=!!fe&&["closed_unmerged","review","undecidable"].includes(fe.tier),rt=cr(_e,te,{external:K,merge_active:P||de?.step==="merge",merge_queued:b,cleanup_active:Ne,merged:!!X||fe?.tier==="merged"}),B=!!rt.operation;d.push({...We(te),lane:"pr_wait",pr_number:typeof ze.number=="number"?ze.number:null,pr_url:typeof ze.url=="string"?ze.url:void 0,external:K,usage:zt(D,te),merge_step:de,badges:$?["\uC774\uC5B4\uD558\uAE30 \uC120\uD0DD \uD544\uC694"]:de?[fe?.tier==="merged"?"\uBA38\uC9C0\uB428":"\uBA38\uC9C0 \uC911"]:X?[Hr(X.step)?`\uC815\uB9AC \uBA48\uCDA4 \xB7 ${Hr(X.step)}`:"\uC815\uB9AC \uBA48\uCDA4"]:typeof fe?.gate_badge=="string"&&fe.gate_badge.length>0?[fe.gate_badge]:[],alert:de?de.failed===!0:!!X||He,reason:X&&de?.active!==!0?eo(X.step):"PR \uB300\uAE30",merge_action:fe?.tier==="merged"&&!Te&&!ye?!1:!b||$,merge_enabled:!B&&($||fe?.enabled===!0||ke||Te||ye),merge_label:$?"\uC774\uC5B4\uD558\uAE30 \uC120\uD0DD":ye||Te?"\uC815\uB9AC \uC7AC\uAC1C":ke&&!Te?"\uCDA9\uB3CC \uD574\uC18C \uD6C4 \uBA38\uC9C0":void 0,merge_title:$?"\uC2E4\uD589 provider\uAC00 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4 \u2014 \uC774\uC5B4\uAC08 \uBC29\uC2DD\uC744 \uC120\uD0DD\uD558\uC138\uC694":B?rt.error?`\uD3D0\uAE30 \uC2E4\uD328: ${rt.error} \u2014 [\uC7AC\uC2DC\uB3C4]\uD558\uAC70\uB098 \uC0C1\uD0DC\uB97C \uD655\uC778\uD558\uC138\uC694`:`\uD3D0\uAE30 \uC9C4\uD589 \uC911 \u2014 ${rt.progress||"\uC644\uB8CC\uB97C \uAE30\uB2E4\uB9AC\uC138\uC694"}`:ye?"\uBA38\uC9C0 \uC644\uB8CC \u2014 \uD074\uB9AD\uD558\uBA74 \uC2E4\uD328\uD55C \uC815\uB9AC\uB97C \uC7AC\uAC1C\uD569\uB2C8\uB2E4":Te?"\uBA38\uC9C0 \uC644\uB8CC \u2014 \uD074\uB9AD\uD558\uBA74 \uB0A8\uC740 \uC815\uB9AC\uB97C \uC2E4\uD328 \uB2E8\uACC4\uBD80\uD130 \uC7AC\uAC1C\uD569\uB2C8\uB2E4":ke?"\uCDA9\uB3CC \u2014 \uD050\uC5D0 \uB123\uC73C\uBA74 \uD574\uC18C \uC138\uC158\uC744 \uB744\uC6B0\uACE0 \uC644\uB8CC \uD6C4 \uC790\uB3D9\uC73C\uB85C \uC7AC\uBA38\uC9C0\uD569\uB2C8\uB2E4":fe?.enabled===!0?`\uBA38\uC9C0 (${fe.gate_badge}) \u2014 \uD050\uC5D0 \uB123\uC5B4 \uC21C\uC11C\uB300\uB85C \uBA38\uC9C0\uD569\uB2C8\uB2E4`:`\uBA38\uC9C0 \uBD88\uAC00: ${fe?.reason||"\uAD00\uCE21 \uB300\uAE30"}`,cancel_action:b&&!$,cancel_enabled:!P,continuation_mismatch:x?.mismatch||null,discard:rt,discard_action:rt.action,discard_enabled:rt.enabled,discard_title:rt.title})}let Ie=(V,te,Me,ze)=>{let fe=V&&V.bead_id;if(typeof fe!="string"||Ve.has(fe))return null;Ve.add(fe);let b=qe[fe],x=cr(_e,fe),$=x.operation?x:null,P={...We(fe),lane:te,draggable:!$,discard:$||void 0,reason:zc(pe,fe),queue_position:Me+1,queue_index:Me,queue_length:ze,badges:b?["\u23F8 REVISE \uD30C\uD0B9"]:[],alert:!!b,revise_action:!!b,revise_enabled:!!b&&!$,revise_title:b?b.notes_tail?`REVISE findings (\uC790\uC138\uD788\uB294 \uCE74\uB4DC \uD074\uB9AD \u2192 \uC774\uC288 \uC0C1\uC138):
${b.notes_tail}`:"notes\uC758 REVISE finding\uC744 \uC2A4\uD399\uC5D0 \uBC18\uC601\uD558\uB294 \uCC98\uBD84 \uC138\uC158\uC744 \uB744\uC6C1\uB2C8\uB2E4":""};return Object.hasOwn(Le,fe)&&(P.blocked_by=Array.isArray(Le[fe])?Le[fe].filter(K=>typeof K=="string"&&K.length>0):[]),P};for(let V=0;V<Ue.length;V++){let te=Ie(Ue[V],"queue",V,Ue.length);if(!te)continue;p.push(te);let Me=R.get(C);Me?Me.push(te):R.set(C,[te])}let lt=[];for(let V=0;V<Ze.length;V++){let te=Ze[V],Me=[];for(let fe=0;fe<te.entries.length;fe++){let b=Ie(te.entries[fe],te.id,fe,te.entries.length);b&&(Me.push(b),p.push(b))}if(Me.length===0)continue;let ze=At(Ee[te.id]);lt.push({id:te.id,index:V,items:Me,occupied_by:Array.isArray(ze.occupied_by)?ze.occupied_by.filter(fe=>typeof fe=="string"):[],corrections:Array.isArray(ze.corrections)?ze.corrections.length:0,cycle:ze.cycle===!0})}A.set(C,lt);let Xe=Array.from({length:ot},(V,te)=>{let Me=`s${te+1}`,ze=Y.get(Me),fe=ze&&Array.isArray(ze.entries)?ze.entries:[],b=At(Ee[Me]);return{id:Me,index:fe.length,length:fe.length,occupied_by:Array.isArray(b.occupied_by)?b.occupied_by.filter(x=>typeof x=="string"):[]}});for(let V of Array.isArray(g.runnable)?g.runnable:[]){let te=V&&V.bead_id;typeof te!="string"||Ve.has(te)||(Ve.add(te),i.push({...We(te),title:V.title||ue[te]||te,lane:"runnable",draggable:!0,reason:zc(pe,te),created_at:V.created_at??void 0,updated_at:V.updated_at??void 0,labels:Array.isArray(V.labels)?V.labels:[],spec_reviewer:typeof V.spec_reviewer=="string"?V.spec_reviewer:void 0,plan_state:V.plan_state==="approved"||V.plan_state==="authored"?V.plan_state:"none",workflow:V.route?{route:V.route,chips:{route:V.route}}:null,blocked:V.blocked===!0,...Array.isArray(V.blocked_by)?{blocked_by:V.blocked_by.filter(Me=>typeof Me=="string"&&Me.length>0)}:{},place_index:Ue.length,place_lanes:Xe}))}for(let V of oe){let te=V&&V.bead_id;if(typeof te!="string"||Ve.has(te)||(Ve.add(te),o!==void 0&&typeof V.added_at=="number"&&V.added_at<o))continue;let Me=sm(D,te);f.push({...We(te),lane:"done",done:!0,usage:zt(D,te),done_at:typeof V.added_at=="number"?V.added_at:void 0,done_kind:Me&&typeof Me.done_kind=="string"?Me.done_kind:null})}}let F=new Map;s.forEach((g,C)=>{g&&typeof g.root_dir=="string"&&F.set(g.root_dir,C)});let Z=r&&r.running_sort==="repo"?"repo":"started";u.sort((g,C)=>{if(Z==="repo"){let M=F.get(g.root_dir)??Number.MAX_SAFE_INTEGER,D=F.get(C.root_dir)??Number.MAX_SAFE_INTEGER;if(M!==D)return M-D}let H=typeof g.started_at=="number"&&Number.isFinite(g.started_at)?g.started_at:null,se=typeof C.started_at=="number"&&Number.isFinite(C.started_at)?C.started_at:null;return H!==null&&se!==null&&H!==se?H-se:H===null&&se!==null?1:H!==null&&se===null?-1:g.id.localeCompare(C.id)}),f.sort((g,C)=>(C.done_at??0)-(g.done_at??0));let Q=s.length>0?s:n.map(g=>({root_dir:g&&g.root_dir,name:g&&g.name,auto_advance:g&&g.auto_advance,auto_merge:g&&g.auto_merge,slots:g&&g.slots,revision:g&&g.revision,runner_catalog:g&&g.runner_catalog})),U=[];for(let g of Q){if(!g||typeof g.root_dir!="string")continue;let C=R.get(g.root_dir)||[],H=A.get(g.root_dir)||[];U.push({root_dir:g.root_dir,name:g.name||g.root_dir,auto_advance:g.auto_advance===!0,auto_merge:g.auto_merge===!0,slots:typeof g.slots=="number"&&g.slots>=Wn?g.slots:Wn,revision:typeof g.revision=="number"?g.revision:0,runner_catalog:At(g.runner_catalog),items:C,sublanes:{parallel:C,serial:H},serial_lane_count:I.get(g.root_dir)||0})}let W={runnable:i,queue:p,queue_groups:U,running:u,pr_wait:d,done:f,automation:{total:U.length,both_on:U.filter(g=>g.auto_advance&&g.auto_merge).length}},O=fa(W);for(let g of h)O.has(g.id)||O.set(g.id,{root_dir:g.root_dir,workspace_name:g.workspace_name,lane:"done",state:"done"});for(let g of[...W.queue,...W.runnable]){if(!Object.hasOwn(g,"blocked_by"))continue;let C=O.get(g.id);g.blockers=(g.blocked_by||[]).map(H=>Ac(H,C,O,s)),g.blocker_warnings=g.blockers.filter(H=>H.missing_internal).map(H=>`\u26A0 \uC120\uD589 ${H.id}\uAC00 \uC5B4\uB290 \uB808\uC778\uC5D0\uB3C4 \uC5C6\uACE0 \uC2E4\uD589 \uC911\uB3C4 \uC544\uB2D8 \u2014 \uC218\uB3D9 \uAC1C\uC785 \uC804\uAE4C\uC9C0 \uC774 \uC790\uB9AC\uC5D0\uC11C \uC815\uC9C0`),g.blocker_warnings.length>0&&(g.alert=!0)}let z=Sc(W.queue_groups);for(let g of W.queue_groups)for(let C of g.sublanes.serial){let H=z.get(Tc(g.root_dir,C.id));H&&(C.cross_wait_peers=H)}return W}function am(e,t){if(typeof e!="number"||!Number.isFinite(e))return"";let n=t-e<tm;return l`<span
  ...
  >
    ...
    ${r?e.sublanes.serial.map(s=>l`<section
  ...
  </div>`}function nu(e,t){let r=_t("views:monitor"),n=t.gotoIssue,s=t.pipelineStore,o=t.transport,a=t.getWorkspacePath,i=t.switchWorkspace,u=t.now||(()=>Date.now()),d=t.confirm||(b=>typeof globalThis.confirm!="function"||globalThis.confirm(b)),p=gm(),f=hm(),h=fm();function R(){let b=or.find(x=>x.value===p);return b?b.label:""}let A=document.createElement("div");A.className="mon",e.appendChild(A);let I=ka(null,null),F=new Map,Z=null,Q=null;async function U(b,x,$,P,K=!0){if(!o||!$)return null;let X=await o(b,{...x,root_dir:$,expected_revision:P});if(X&&X.conflict&&K){X.queue&&F.set($,X.queue);let ae=X.queue&&typeof X.queue.revision=="number"?X.queue.revision:P;X=await o(b,{...x,root_dir:$,expected_revision:ae})}return X&&X.queue&&$&&F.set($,X.queue),X}function W(b,x){let $=F.get(b),P=s&&s.get?s.get():null,K=(Array.isArray(P)?P:[]).find(ae=>ae?.root_dir===b);return($||K)?.merge_queue?.find(ae=>ae.bead_id===x)?.continuation_action}async function O(b,x,$,P){let K=await U(b,x,$,P),X=F.get($)?.revision??K?.queue?.revision??P;return fr(K,(ae,de)=>U(b,{...x,continuation:ae,decision_token:de},$,X,!1),{refresh:ae=>U(b,x,$,ae?.queue?.revision??F.get($)?.revision??X,!1)})}async function z(b,x,$,P){let K=await fr({continuation_mismatch:P},(ae,de)=>U("worker-merge-queue-add",{bead_id:x,continuation:ae,decision_token:de},b,$,!1)),X=K?.queue?.merge_queue?.find(ae=>ae.bead_id===x)?.continuation_action;K?.applied!==!0&&X?.continuation===null&&X.mismatch&&await z(b,x,K.queue.revision,X.mismatch)}async function g(b,x,$){let P=await U("worker-discard",b,x,$);if(P&&P.discarded===!0){le(Qs(P),"success",5e3);return}if(P&&P.reason){le(`\uD3D0\uAE30 \uC2E4\uD328: ${P.reason}`,"error");return}if(P&&P.accepted&&P.pending==="merged_revert"){le("revert PR \uB300\uAE30 \uC0C1\uD0DC\uB85C \uC804\uD658\uD588\uC2B5\uB2C8\uB2E4","success");return}if(P&&P.accepted){le(`\uD3D0\uAE30 \uC9C4\uD589: ${P.phase||"\uBC31\uC5C5 \uC911"}`,"success");return}P&&!P.conflict&&le("\uD3D0\uAE30 \uAC70\uBD80: unknown","error")}async function C(b,x,$){return!o||!$?null:await o(b,{...x,root_dir:$})}async function H(b){if(!o||!b&&!d("\uC804 \uB808\uD3EC\uC758 \uC790\uB3D9 \uC9C4\uD589\xB7\uC790\uB3D9 \uBA38\uC9C0\uB97C \uB055\uB2C8\uB2E4. \uAC01 \uB808\uD3EC\uC758 \uBA38\uC9C0 \uB300\uAE30\uC5F4\uB3C4 \uD568\uAED8 \uBE44\uC6CC\uC9D1\uB2C8\uB2E4. \uACC4\uC18D\uD560\uAE4C\uC694?"))return;let x=await o("monitor-auto-toggle",{on:b}),$=x&&Array.isArray(x.failed)?x.failed:[];$.length>0&&le(`\uC790\uB3D9\uD654 ${b?"\uCF1C\uAE30":"\uB044\uAE30"} \uC77C\uBD80 \uC2E4\uD328: ${$.map(P=>P.root_dir).join(", ")}`,"error",3200)}async function se(){let b=new Map;for(let x of I.pr_wait)b.has(x.root_dir)||b.set(x.root_dir,x.expected_revision);for(let[x,$]of b)await U("worker-merge-queue-add-all",{},x,$)}let M=null,D=!1,ue=null;function ge(){ue!==null&&clearTimeout(ue),ue=setTimeout(()=>{ue=null,D=!1},0)}function pe(b){let x=b.target;return typeof x?.closest=="function"?x.closest(".mon-group"):null}function qe(b){let x=pe(b);return!x||!M?null:(x.getAttribute("data-root-dir")||"")===M.root_dir?x:null}function tt(){for(let b of Array.from(A.querySelectorAll(".mon-group--drag-over")))b.classList.remove("mon-group--drag-over")}function je(b){let x=b.target,$=typeof x?.closest=="function"?x.closest('.mon-card[draggable="true"]'):null;if($){M={bead_id:$.getAttribute("data-issue-id")||"",lane:$.getAttribute("data-lane")||"",root_dir:$.getAttribute("data-root-dir")||"",revision:Number($.getAttribute("data-revision")||0)||0,queue_index:Number($.getAttribute("data-queue-index")),queue_length:Number($.getAttribute("data-queue-length")),place_index:Number($.getAttribute("data-place-index"))},D=!0;try{b.dataTransfer?.setData("text/plain",M.bead_id),b.dataTransfer&&(b.dataTransfer.effectAllowed="move")}catch{}}}function _e(b){let x=qe(b);x&&(b.preventDefault(),b.dataTransfer&&(b.dataTransfer.dropEffect="move"),x.classList.add("mon-group--drag-over"))}function Le(b){pe(b)?.classList.remove("mon-group--drag-over")}function he(){M=null,tt(),ge()}function xe(b){let x=qe(b),$=M;if(M=null,tt(),!x||!$||!$.bead_id)return;b.preventDefault();let P=b.target,K=typeof P?.closest=="function"?P.closest('.mon-card[data-lane="queue"]'):null,X=K&&x.contains(K)?Number(K.getAttribute("data-queue-index")):NaN;if($.lane==="runnable"){let Ne=Number.isFinite(X)?X:$.place_index;if(!Number.isFinite(Ne))return;U("worker-queue-place",{bead_id:$.bead_id,index:Ne},$.root_dir,$.revision);return}if($.lane!=="queue"||K&&K.getAttribute("data-issue-id")===$.bead_id)return;let ae=$.queue_index,de=Number.isFinite(X)?ae>X?X:X-1:$.queue_length-1;!Number.isFinite(de)||de<0||de===ae||U("worker-queue-reorder",{bead_id:$.bead_id,to_index:de},$.root_dir,$.revision)}function Re(b){let x=mm(I.runnable,h),$={runnable:x.visible,queue:I.queue,running:I.running,pr_wait:I.pr_wait,done:I.done};return l`${Zc({automation:I.automation,counts:{running:I.running.length,queue:I.queue.length,pr_wait:I.pr_wait.length},running_sort:f,done_range:p,token_total:Qc(I.done),token_tooltip:Xc(R())})}
      ...
      </div>`}function Be(){let b=s&&s.get?s.get():null,x=s&&s.getWorkspacesState?s.getWorkspacesState():[],$=u();I=ka(b,x,{done_since:Nr(p,$),running_sort:f}),Ye(Re($),A)}function Ae(b,x){let $=a?a():void 0;if(!x||!$||x===$||!i){n(b);return}i(x).then(()=>{n(b)}).catch(P=>{r("workspace switch for %s failed: %o",x,P)})}function Ue(b){return{root_dir:b.getAttribute("data-root-dir")||"",revision:Number(b.getAttribute("data-revision")||0)||0}}function Ze(b){if(typeof b=="string"&&b.length>0)return b;if(b&&typeof b=="object"){let x=b;if(typeof x.message=="string"&&x.message.length>0)return x.message;if(typeof x.error=="string"&&x.error.length>0)return x.error;if(x.error&&typeof x.error=="object"&&typeof x.error.message=="string")return x.error.message}return"\uC5F0\uACB0\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4"}function Ee(b,x){let $=b.querySelector(".mon-link__trigger"),P=b.querySelector(".mon-link__popover"),K=b.querySelector(".mon-link__error");!$||!P||!K||(Oe(),P.hidden=!1,$.setAttribute("aria-expanded","true"),K.textContent=x,K.hidden=!1)}async function ot(b,x,$){let P=x.getAttribute("data-root-dir")||"",K=x.getAttribute("data-issue-id")||"";if(!(!K||!$||$===K))try{await C(b,{a:K,b:$},P),Oe()}catch(X){Ee(x,Ze(X))}}function Y(b,x){let{root_dir:$,revision:P}=Ue(b),K=b.getAttribute("data-issue-id")||"",X=x.dataset.attemptId||b.getAttribute("data-attempt-id")||"",ae=x.classList;if(ae.contains("mon-link__trigger")){Ve(x);return}if(ae.contains("mon-link__candidate")||ae.contains("mon-link__direct")){let de=x.dataset.targetId||"";ot("dep-add",b,de);return}if(ae.contains("mon-blocker__remove")){let de=x.dataset.blockerId||"";ot("dep-remove",b,de);return}if(ae.contains("mon-place__choice")){let de=x.dataset.lane||"parallel",Ne=Number(x.dataset.placeIndex||0)||0;Oe(),U("worker-queue-place",{bead_id:K,...de==="parallel"?{}:{lane:de},index:Ne},$,P);return}if(ae.contains("worker-card__place")){We(x);return}if(ae.contains("mon-op--up")||ae.contains("mon-op--down")){let de=Number(b.getAttribute("data-queue-index")||0)||0,Ne=ae.contains("mon-op--up")?de-1:de+1;if(Ne<0)return;U("worker-queue-reorder",{bead_id:K,.../^s[1-5]$/.test(b.dataset.lane||"")?{lane:b.dataset.lane}:{},to_index:Ne},$,P);return}if(ae.contains("mon-op--remove")){U("worker-queue-remove",{bead_id:K},$,P);return}if(ae.contains("mon-op--pause")){C("worker-attempt-pause",{attempt_id:X},$);return}if(ae.contains("mon-op--discard")){if(!d(Fn(K,"unmerged")))return;g({bead_id:K,...X?{attempt_id:X}:{},...x.dataset.operationId?{operation_id:x.dataset.operationId}:{}},$,P);return}if(ae.contains("mon-op--resume")){sn().then(de=>{if(de!==null)return O("worker-attempt-resume",{attempt_id:X,...de!==""?{instructions:de}:{}},$,P)});return}if(ae.contains("mon-op--dismiss")){U("worker-attempt-dismiss",{attempt_id:X},$,P);return}if(ae.contains("worker-mini__merge")){let de=W($,K);de?.mismatch&&de.continuation===null?z($,K,P,de.mismatch):U("worker-merge-queue-add",{bead_id:K},$,P);return}if(ae.contains("worker-mini__merge-cancel")){U("worker-merge-queue-remove",{bead_id:K},$,P);return}if(ae.contains("worker-mini__discard")){let de=x.dataset.discardMode==="merged"?"merged":"unmerged";if(!d(Fn(K,de)))return;g({bead_id:K,...X?{attempt_id:X}:{},...x.dataset.operationId?{operation_id:x.dataset.operationId}:{}},$,P);return}if(ae.contains("worker-mini__revise-fix")){O("worker-revise-fix",{bead_id:K},$,P);return}ae.contains("worker-mini__revise-approve")&&U("worker-revise-approve",{bead_id:K},$,P)}function j(b){b.querySelector(".mon-link__list")?.replaceChildren();let $=b.querySelector(".mon-link__search");$&&($.value="");let P=b.querySelector(".mon-link__direct");P&&(P.hidden=!0,P.dataset.targetId="",P.textContent="");let K=b.querySelector(".mon-link__empty");K&&(K.hidden=!0);let X=b.querySelector(".mon-link__error");X&&(X.hidden=!0,X.textContent="")}function oe(b,x){let $=b.querySelector(".mon-link__list");if(!$)return;let P=document.createDocumentFragment(),K=Ec(I).filter(X=>X.id!==x);for(let X of K){let ae=document.createElement("button");ae.type="button",ae.className="mon-link__candidate",ae.dataset.targetId=X.id,ae.dataset.search=`${X.id} ${X.title} ${X.location}`.toLocaleLowerCase();let de=document.createElement("strong");de.textContent=X.id;let Ne=document.createElement("span");Ne.textContent=X.title;let ke=document.createElement("small");ke.textContent=X.location,ae.append(de,Ne,ke),P.append(ae)}$.replaceChildren(P)}function Oe(){for(let b of Array.from(A.querySelectorAll(".mon-card-popover"))){let x=b;x.hidden=!0,x.classList.contains("mon-link__popover")&&j(x)}for(let b of Array.from(A.querySelectorAll('[aria-expanded="true"]')))b.setAttribute("aria-expanded","false")}function We(b){let $=b.closest(".mon-place")?.querySelector(".mon-place__popover")||null;if(!$)return;let P=$.hidden;Oe(),P&&($.hidden=!1,b.setAttribute("aria-expanded","true"))}function Ve(b){let $=b.closest(".mon-link")?.querySelector(".mon-link__popover")||null;if(!$)return;let P=$.hidden;if(Oe(),P){let K=b.closest(".mon-card");oe($,K?.getAttribute("data-issue-id")||""),$.hidden=!1,b.setAttribute("aria-expanded","true");let X=$.querySelector(".mon-link__search");X&&(Ie(X),X.focus())}}function Ie(b){let x=b.closest(".mon-link__popover"),$=b.closest(".mon-card");if(!x||!$)return;let P=b.value.trim(),K=P.toLocaleLowerCase(),X=0,ae=!1;for(let ye of Array.from(x.querySelectorAll(".mon-link__candidate"))){let He=ye,rt=He.dataset.targetId||"",B=K.length===0||(He.dataset.search||"").includes(K);He.hidden=!B,B&&(X+=1),rt.toLocaleLowerCase()===K&&(ae=!0)}let de=x.querySelector(".mon-link__direct"),Ne=$.getAttribute("data-issue-id")||"";if(de){let ye=P.length>0&&!ae&&K!==Ne.toLocaleLowerCase();de.hidden=!ye,de.dataset.targetId=ye?P:"",de.textContent=ye?`\uC9C1\uC811 \uC785\uB825 \xB7 ${P}`:"",ye&&(X+=1)}let ke=x.querySelector(".mon-link__empty");ke&&(ke.hidden=X>0);let Te=x.querySelector(".mon-link__error");Te&&(Te.hidden=!0,Te.textContent="")}function lt(b){let x=b.target;x&&A.contains(x)&&typeof x.closest=="function"&&x.closest(".mon-popover-owner")||Oe()}function Xe(b){if(b.key!=="Escape")return;let x=A.querySelector('[aria-expanded="true"]');Oe(),x?.focus()}function V(b){let x=D;D=!1;let $=b.target;if(!$||typeof $.closest!="function"||$.closest("dialog")||$.closest("a"))return;let P=$.closest(".mon-running-sort");if(P){b.preventDefault(),f=P.getAttribute("data-sort")==="repo"?"repo":"started",ym(f),Be();return}let K=$.closest(".mon-auto-all");if(K){b.preventDefault(),H(K.getAttribute("data-on")==="true");return}if($.closest(".mon-merge-all")){b.preventDefault(),se();return}let ae=$.closest(".mon-ctl--advance");if(ae){b.preventDefault();let{root_dir:ye,revision:He}=Ue(ae);U("worker-automation-toggle",{on:ae.getAttribute("data-on")==="true"},ye,He);return}let de=$.closest(".mon-ctl--merge-auto");if(de){b.preventDefault();let{root_dir:ye,revision:He}=Ue(de);U("worker-merge-auto-toggle",{on:de.getAttribute("data-on")==="true"},ye,He);return}let Ne=$.closest(".mon-card");if(!Ne)return;let ke=$.closest("button");if(ke){b.preventDefault(),Y(Ne,ke);return}let Te=Ne.getAttribute("data-issue-id");Te&&!x&&(b.preventDefault(),Ae(Te,Ne.getAttribute("data-root-dir")||""))}function te(b){let x=b.target;if(!x||typeof x.closest!="function")return;let $=x.closest(".mon-filter__blocked");if($){h={show_blocked:$.checked},_m(h),Be();return}let P=x.closest(".mon-done-range");if(P){p=Wt(P.value)?P.value:Nt,bm(p),Be();return}let K=x.closest(".mon-slots__input");if(!K)return;let{root_dir:X,revision:ae}=Ue(K),de=Number(K.value);if(!Number.isFinite(de))return;let Ne=Math.max(Wn,Math.floor(de));U("worker-queue-set-slots",{slots:Ne},X,ae)}function Me(b){let x=b.target;x?.classList.contains("mon-link__search")&&Ie(x)}e.addEventListener("click",V),e.addEventListener("change",te),e.addEventListener("input",Me),e.addEventListener("dragstart",je),e.addEventListener("dragover",_e),e.addEventListener("dragleave",Le),e.addEventListener("drop",xe),e.addEventListener("dragend",he),document.addEventListener("click",lt),document.addEventListener("keydown",Xe),s&&typeof s.subscribe=="function"&&(Z=s.subscribe(()=>{try{F.clear(),Be()}catch{}}));function ze(){Q!==null&&(clearInterval(Q),Q=null)}function fe(){ue!==null&&(clearTimeout(ue),ue=null)}return{load(){r("load"),Be(),Q===null&&(Q=setInterval(()=>{try{if(A.querySelector(".mon-card-popover:not([hidden])"))return;Be()}catch{}},vm))},pause(){ze()},clear(){ze(),fe(),Z&&(Z(),Z=null),e.removeEventListener("click",V),e.removeEventListener("change",te),e.removeEventListener("input",Me),e.removeEventListener("dragstart",je),e.removeEventListener("dragover",_e),e.removeEventListener("dragleave",Le),e.removeEventListener("drop",xe),e.removeEventListener("dragend",he),document.removeEventListener("click",lt),document.removeEventListener("keydown",Xe),e.replaceChildren()}}}function su(e,t,r){let n=_t("views:nav"),s=null;function o(u){return d=>{d.preventDefault(),n("click tab %s",u),r.gotoView(u)}}function a(){let u=t.getState(),d=u.view==="worker"||u.view==="monitor"?u.view:"board";return l`
  ...
  `}var Am=[{id:"execution",label:"\uC2E4\uD589",glyph:"\u25C6"},{id:"display",label:"\uD45C\uC2DC",glyph:"\u25EB"}],Ft="";function jt(e){return!!e&&typeof e=="object"&&!Array.isArray(e)}function pu(e,t){let{transport:r,policyStore:n,labelOptions:s}=t,o=t.notify||(b=>le(b,"error",4e3)),a=document.createElement("dialog");a.id="settings-dialog",a.className="settings-dialog",a.setAttribute("role","dialog"),a.setAttribute("aria-modal","true"),a.setAttribute("aria-label","\uC124\uC815"),e.appendChild(a);let i="execution",u=!1,d="",p={},f={},h=[],R=!1,A=null,I={},F="",Z="",Q=!1,U=!1,W=!1,O=null;function z(){let b=t.queueStore?.get();return jt(b)?b.runner_catalog:null}function g(){let b=t.queueStore?.get();return jt(b)&&jt(b.execution_defaults)?b.execution_defaults:null}function C(){let b=t.implPresetStore?.get();return jt(b)&&Array.isArray(b.presets)?b:null}async function H(){R=!0,Ie();try{let b=await r("get-session-defaults",{});p=jt(b?.values)?{...b.values}:{},f={...p},h=Array.isArray(b?.warnings)?b.warnings:[]}catch(b){h=["kv_read_failed"],o(`\uC138\uC158 \uAE30\uBCF8\uAC12\uC744 \uC77D\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4: ${b instanceof Error?b.message:String(b)}`)}finally{R=!1,Ie()}}async function se(){let b=rc(p,f);if(Object.keys(b).length!==0){try{let x=await r("set-session-defaults",{values:b});p=jt(x?.values)?{...x.values}:{},f={...p},h=Array.isArray(x?.warnings)?x.warnings:[]}catch(x){o(`\uC138\uC158 \uAE30\uBCF8\uAC12 \uC800\uC7A5 \uC2E4\uD328: ${x instanceof Error?x.message:String(x)}`)}Ie()}}function M(b,x){x===Ft?delete f[b]:f[b]=x,Ie(),se()}async function D(){let b=t.queueStore?.get();if(!jt(b))return;let x={orchestration_model:b.orchestration_model??null,orchestration_effort:b.orchestration_effort??null,orchestration_speed:b.orchestration_speed??null},$=nc(x,{...x,...I});if(Object.keys($).length!==0){try{let P=await r("worker-queue-set-orchestration-defaults",{expected_revision:b.revision,values:$});if(P&&P.applied===!1){o("Worker \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uD074\uB77C\uC774\uC5B8\uD2B8\uC640 \uCDA9\uB3CC");return}I={}}catch(P){o(`Worker \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: ${P instanceof Error?P.message:String(P)}`)}Ie()}}function ue(b,x){I[b]=x===Ft?null:x,Ie(),D()}async function ge(b){let x=t.queueStore?.get();if(!(!jt(x)||b<1)){try{await r("worker-queue-set-slots",{expected_revision:x.revision,slots:b})}catch($){o(`slots \uC800\uC7A5 \uC2E4\uD328: ${$ instanceof Error?$.message:String($)}`)}Ie()}}function pe(){let b={},x=Ue();for(let $ of Jl){let P=Cr.includes($)?x[$]:f[$];typeof P=="string"&&P.length>0&&(b[$]=P)}return b}async function qe(){let b=C();if(!b)return;let x=pe();if(Object.keys(x).length===0){o("\uC800\uC7A5\uD560 \uC2E4\uD589 \uC124\uC815\uC774 \uC5C6\uC2B5\uB2C8\uB2E4 \u2014 \uBA3C\uC800 \uC2E4\uD589 \uAC12\uC744 \uC120\uD0DD\uD558\uC138\uC694");return}let $=(b.presets||[]).find(K=>K.id===F),P=Z.trim()||($?$.name:"");if(!P){o("\uD504\uB9AC\uC14B \uC774\uB984\uC744 \uC785\uB825\uD558\uC138\uC694");return}try{let K=$?await r("impl-preset-update",{expected_revision:b.revision,id:$.id,name:P,settings:x}):await r("impl-preset-create",{expected_revision:b.revision,name:P,settings:x});if(K&&K.applied){if(Z="",!$&&Array.isArray(K.presets)){let X=K.presets.find(ae=>ae.name===P);F=X?X.id:F}Ie()}else o("\uD504\uB9AC\uC14B \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uACF3\uC5D0\uC11C \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4"),Ie()}catch(K){o(`\uD504\uB9AC\uC14B \uC800\uC7A5 \uC2E4\uD328: ${K instanceof Error?K.message:String(K)}`)}}async function tt(){let b=C();if(!(!b||F.length===0))try{let x=await r("impl-preset-delete",{expected_revision:b.revision,id:F});x&&x.applied?(F="",Ie()):(o("\uD504\uB9AC\uC14B \uC0AD\uC81C \uC2E4\uD328: \uB2E4\uB978 \uACF3\uC5D0\uC11C \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4"),Ie())}catch(x){o(`\uD504\uB9AC\uC14B \uC0AD\uC81C \uC2E4\uD328: ${x instanceof Error?x.message:String(x)}`)}}async function je(){let b=C(),x=t.queueStore?.get();if(!(!b||!jt(x)||F.length===0)){try{let $=await r("apply-impl-preset-global",{preset_id:F,expected_revision:b.revision,expected_queue_revision:x.revision});$&&$.applied?(p=jt($.values)?{...$.values}:{},f={...p},h=Array.isArray($.warnings)?$.warnings:[],jt($.queue)&&(t.queueStore?.set?.($.queue),I={}),$.queue_applied===!1&&o("\uC624\uCF00\uC2A4\uD2B8\uB808\uC774\uC158 \uAC12\uC740 \uC801\uC6A9\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4 \u2014 \uB2E4\uC2DC \uC2DC\uB3C4\uD558\uC138\uC694")):$&&$.conflict&&o("\uC2E4\uD589 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328: \uD504\uB9AC\uC14B\uC774 \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4")}catch($){o(`\uC2E4\uD589 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328: ${$ instanceof Error?$.message:String($)}`)}Ie()}}async function _e(){U=!0,W=!1,Ie();try{let b=await r("get-worker-system-prompt",{});!b||typeof b!="object"||Array.isArray(b)?W=!0:O=b}catch{W=!0}finally{U=!1,Ie()}}function Le(){if(Q=!Q,Q&&!O){_e();return}Ie()}function he(){let b=cn({loading:U,error:W});if(b)return b;if(!O)return"";let x=Array.isArray(O.variants)?O.variants:[];return l`<div class="settings-dialog__sp-body">
    ...
    >
      ...
      ${Q?he():""}
    </section>`}function Re(b,x,$,P,K,X){let ae=K[b]??Ft,de=sa(b,$,K,g(),z()),Ne=de.options.find(Te=>Te.value===ae),ke=ae===Ft?de.full_value:Ne?.full_value;return l`<select
        class=${ae===Ft?"settings-dialog__unset":""}
        data-key=${b}
        aria-label=${x}
        title=${ke||""}
        ?disabled=${X===!0||de.disabled}
        .value=${zr(String(ae))}
        @change=${Te=>P(b,String(Te.target.value))}
      >
        <option value=${Ft} ?selected=${ae===Ft}>
          ${de.unset_label}
        </option>
        ${de.options.map(Te=>l`<option
              value=${Te.value}
              title=${Te.full_value||""}
      ...
      ${ae===Ft?l`<span class="settings-dialog__source-badge">기본</span>`:""}`}function Be(b,x,$,P,K,X=!1){return l`<div
    ...
    >
      ...
      <span class="settings-dialog__controls">
        ${Re($,`${b} \uBAA8\uB378`,P,M,f,!1)}
        ${Re(K,`${b} effort`,zs,M,f,!1)}
      </span>
    </div>`}function Ue(){let b=t.queueStore?.get(),x={};for(let $ of Cr)x[$]=Object.prototype.hasOwnProperty.call(I,$)?I[$]:jt(b)&&typeof b[$]=="string"?b[$]:null;return x}function Ze(){let b=z(),x=f.impl_runtime,$=f.impl_model,P=C(),K=t.queueStore?.get(),X=Ue(),ae=Gs(b,A),de=un(b,A||void 0,X.orchestration_model||lr).filter(ye=>ye!==lr),Ne=jt(K)&&typeof K.slots=="number"?K.slots:2,ke=g()?.supported===!0,Te=sa("workflow_mode",Dn,f,g(),b);return l`
      <section
        ...
        ${R?l`<div class="settings-dialog__empty">불러오는 중…</div>`:l`
              <div class="settings-dialog__preset-bar">
                <select
                  aria-label="실행 프리셋"
                  .value=${zr(F)}
                  @change=${ye=>{F=String(ye.target.value),Ie()}}
                >
                  <option value="" ?selected=${F===""}>
                    실행 프리셋…
                  </option>
                  ${(P?.presets||[]).map(ye=>l`<option
                        value=${ye.id}
                        ?selected=${ye.id===F}
                      >
                        ${ye.name}
                ...
                <input
                  ...
                  aria-label="프리셋 이름"
                  .value=${zr(Z)}
                  @input=${ye=>{Z=String(ye.target.value)}}
                />
                <button
                  type="button"
                  class="settings-dialog__btn"
              ...
              <div class="settings-dialog__group">
                ...
                <div class="settings-dialog__row">
                  ...
                  <span class="settings-dialog__controls">
                    <select
                      ...
                      .value=${zr(A||Ft)}
                      @change=${ye=>{let He=String(ye.target.value);A=He===Ft?null:He,Ie()}}
                    >
                      <option
                        value=${Ft}
                        ?selected=${!A}
                ...
                ${Be("orchestration_model","\uBAA8\uB378",ae,ue,X)}
                ${Be("orchestration_effort","effort",de,ue,X)}
                ${Be("orchestration_speed","\uC18D\uB3C4",Pn,ue,X)}
              </div>
                  ...
                  <span class="settings-dialog__controls">
                    <span class="settings-dialog__seg" role="group">
                      ...
                      ${Dn.map(ye=>l`<button
                            type="button"
                            data-mode=${ye}
                            aria-pressed=${String(f.workflow_mode===ye)}
                            @click=${()=>M("workflow_mode",ye)}
                          >
                            ${ye}
                          </button>`)}
                    </span>
              ...
              <div class="settings-dialog__group">
                ...
                ${Be("impl_runtime","\uC704\uC784 \uB300\uC0C1",Us,M,f)}
                ${Be("impl_model","\uBAA8\uB378",Hs(b,x),M,f)}
                ${Be("impl_effort","effort",un(b,x,$),M,f)}
                ${Be("impl_speed","\uC18D\uB3C4",Pn,M,f)}
              </div>
    ...
    `}function Ee(){let b=n.get();return l`
      ...
      >
        ...
        ${b?l`
              ${cu(b,s(),oe)}
              ${uu(b,d,{onDraft:x=>{d=x},onAdd:Oe,onRemove:We})}
              ${du(b,Ve)}
            `:l`<div class="settings-dialog__empty">
              표시 정책을 불러오는 중…
            </div>`}
      </section>
    `}async function ot(b){let x=n.get();if(x)try{let $=await r("display-policy-set",{expected_revision:x.revision,policy:b(x)});Y($),$&&$.conflict&&$.policy&&($=await r("display-policy-set",{expected_revision:$.policy.revision,policy:b($.policy)}),Y($)),$&&$.conflict&&o("\uD45C\uC2DC \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uD074\uB77C\uC774\uC5B8\uD2B8\uC640 \uCDA9\uB3CC")}catch{o("\uD45C\uC2DC \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328")}}function Y(b){b&&b.policy&&typeof b.policy=="object"&&n.set(b.policy)}function j(b){ot(b)}function oe(b){let x=n.get();if(!x)return;let $=!Sm(b,x);j(P=>Em(b,P,$))}function Oe(){let b=d.trim();b.length!==0&&(d="",j(x=>x.hidden_prefixes.includes(b)?{hidden_prefixes:x.hidden_prefixes}:{hidden_prefixes:[...x.hidden_prefixes,b]}),Ie())}function We(b){j(x=>({hidden_prefixes:x.hidden_prefixes.filter($=>$!==b)}))}function Ve(b){let x=n.get();if(!x)return;let $=x.chips[b]===!1;j(()=>({chips:{[b]:$}}))}function Ie(){Ye(l`
    ...
    </span>`}function Z(M,D,ue){let ge=D.available&&typeof D.ageSeconds=="number"&&D.ageSeconds>_u,pe=ge&&typeof D.ageSeconds=="number"?`${Math.floor(D.ageSeconds/60)}\uBD84 \uC804 \uCE21\uC815`:"",qe=D.accounts.filter(Le=>!Le.active).length,tt=`usage-meter__group${ge?" usage-meter__group--stale":""}`,je=l`<span class="usage-meter__provider"
        >${M.label}</span
      >
      ${D.available?D.windows.map(Le=>F(Le,ge,pe,ue)):l`<span class="usage-meter__empty">사용량 없음</span>`}
      ${qe>0?l`<span class="usage-meter__badge">+${qe}</span>`:""}`;if(D.accounts.length===0)return l`<span
        class=${tt}
        aria-label=${`${M.label} usage`}
        >${je}</span
      >`;let _e=r===M.key;return l`<button
      type="button"
    ...
    >
      ${je}
    </button>`}function Q(M,D){return l`<span class="usage-meter" aria-label="Usage">
      ${M.map(ue=>Z(ue.provider,ue.snapshot,D))}
    </span>`}function U(M){let D=bu(M.pct);return l`<span
      class="usage-meter__account-window ${gu(D)}"
      style=${`--progress: ${D}%`}
    >
    ...
    </span>`}function W(M,D){return Cm.includes(D)?`\uD1A0\uD070 \uB9CC\uB8CC \u2014 ${M.tool} \uC7AC\uB85C\uADF8\uC778 \uD544\uC694`:"\uC0AC\uC6A9\uB7C9 \uC5C6\uC74C"}function O(M,D){let ue=D.status==="ok",ge=typeof D.ageSeconds=="number"&&D.ageSeconds>_u,pe=a.get(yu(M.key,D.number)),qe=n.get(M.key),tt=qe!==void 0,je=qe===D.number,_e=["usage-meter__account"];return D.active&&_e.push("usage-meter__account--active"),ue||_e.push("usage-meter__account--unavailable"),ge&&_e.push("usage-meter__account--stale"),l`<div class=${_e.join(" ")}>
      <div class="usage-meter__account-head">
        <span class="usage-meter__account-label" title=${D.email}
          >${D.alias===null?D.email:D.alias}</span
        >
        ${D.plan===null?"":l`<span class="usage-meter__account-tag">${D.plan}</span>`}
        ${D.active?l`<span
              class="usage-meter__account-tag usage-meter__account-tag--active"
              >active</span
            >`:""}
        ${D.ageSeconds===null?"":l`<span class="usage-meter__account-age"
              >${Lm(D.ageSeconds)}</span
            >`}
        ${D.active?"":l`<button
              type="button"
    ...
    </div>`}function z(M,D){let ue=D.accounts.filter(ge=>ge.active).length;return l`<section class="usage-meter__section">
      <h2 class="usage-meter__section-title">
        ${M.label} · 활성 ${ue} / 전체
        ${D.accounts.length}
      </h2>
      ${D.accounts.map(ge=>O(M,ge))}
    </section>`}function g(M){return l`<div
      class="usage-meter__card"
      id=${fu}
      role="dialog"
      aria-label=${`${M.provider.label} \uACC4\uC815 \uC0AC\uC6A9\uB7C9`}
    >
      ${z(M.provider,M.snapshot)}
      <p class="usage-meter__note">전환은 새로 시작하는 세션부터 적용됩니다.</p>
    </div>`}function C(){let M=[];for(let ge of hu){let pe=o.get(ge.key);pe&&M.push({provider:ge,snapshot:pe})}if(M.length===0){p(),u();return}let D=M.find(ge=>ge.provider.key===r&&ge.snapshot.accounts.length>0);D||p();let ue=Date.now();Ye(l`${Q(M,ue)}
      ${D?l`<div
              class="usage-meter__scrim"
              aria-hidden="true"
              @mousedown=${A}
            ></div>
            ${g(D)}`:""}`,e),e.hidden=!1}async function H(M){try{let D=await fetch(M.endpoint);return D.ok?Mm(await D.json()):null}catch{return null}}async function se(){i+=1;let M=i,D=await Promise.all(hu.map(async ue=>({provider:ue,snapshot:await H(ue)})));if(!(t||M!==i)){for(let ue of D)ue.snapshot?o.set(ue.provider.key,ue.snapshot):o.delete(ue.provider.key);C()}}return u(),se(),s=setInterval(()=>{se()},6e4),{destroy(){t=!0,s!==null&&(clearInterval(s),s=null),p(),u()}}}function ku(e){let t=e.attempts?Object.values(e.attempts):[],r=new Map;for(let s of t)s&&r.set(s.bead_id,s.attempt_id);let n=new Map;for(let s of e.done||[])s&&typeof s.bead_id=="string"&&typeof s.added_at=="number"&&n.set(s.bead_id,s.added_at);return s=>{let o=r.get(s.bead_id)!==s.attempt_id,a=n.get(s.bead_id),i=typeof a=="number"&&a>0&&typeof s.finished_at=="number"&&a>=s.finished_at;return!o&&!i&&typeof s.dismissed_at!="number"}}var Pm="worker-ineligible";function Sa(e){return Array.isArray(e)?e.filter(t=>typeof t=="string"):[]}function $u(e){return Sa(e).includes(Pm)}var Dm="worker-serial";function Ea(e){return Sa(e).includes(Dm)}function Ta(e,t,r){if(typeof t!="string"||typeof r!="string")return[];let n=e?.runners;if(!n||!Object.hasOwn(n,t))return[];let s=n[t],o=s?.models;if(!o||!Object.hasOwn(o,r))return[];let a=o[r]?.efforts;return Array.isArray(a)?a.slice():Array.isArray(s.efforts)?s.efforts.slice():[]}var Nm=new Set(["done","failed","orphaned","stopped","discarded"]),qm={spec_missing:"\uC2A4\uD399 \uC5C6\uC74C",route:"route \uBBF8\uB2EC",spec_review:"\uC2A4\uD399 \uB9AC\uBDF0 \uC5C6\uC74C",spec_conflict:"\uC2A4\uD399 \uCDA9\uB3CC",phase_child:"phase child",worker_ineligible:"worker \uC81C\uC678"},Fm={running:"\uC2E4\uD589 \uC911",success:"\uC131\uACF5",failure:"\uC2E4\uD328",cancelled:"\uCDE8\uC18C",interrupted:"\uC911\uB2E8"},jm={running:"running",success:"done",failure:"failed",cancelled:"stopped",interrupted:"orphaned"};function Ca(e){return{runner:e.runner||void 0,model:e.model||void 0,effort:e.effort||void 0,status:jm[e.outcome]||(typeof e.job_id=="string"?"running":void 0),session_id:e.session_id||void 0}}function xu(e,t){let{queueStore:r,analysisStore:n,transport:s,getWorkspacePath:o,onOpenTranscript:a}=t,i=document.createElement("dialog");i.id="worker-parallel-analysis-dialog",i.className="pa",i.setAttribute("role","dialog"),i.setAttribute("aria-modal","true"),e.appendChild(i);let u=new Map,d=new Map,p=!1,f=null,h=null,R=null,A=new Set,I=!1,F=0,Z=null,Q=new Set;function U(){return r&&r.get()||{revision:0,queue:[],serial_lanes:[],serial_lane_count:0,attempts:{},pr_wait:[]}}function W(){return n&&n.get()||{settings:{revision:0,runner:null,model:null,effort:null},job:null,runs:[],last_good:null}}function O(){return o&&o()||""}async function z(){if(!s)return;let k=++F;I=!0,R=null,A.clear(),ke();try{let T=await s("worker-parallel-analysis-targets",{root_dir:O()});if(k!==F||!Te)return;let N=Array.isArray(T?.qualified)?T.qualified:[],ee=Array.isArray(T?.excluded)?T.excluded:[];R={qualified:N,excluded:ee};for(let $e of N)$e&&typeof $e.id=="string"&&A.add($e.id)}catch{k===F&&Te&&(R={qualified:[],excluded:[]},le("\uBD84\uC11D \uB300\uC0C1\uC744 \uBD88\uB7EC\uC624\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4","error",2800))}finally{k===F&&(I=!1,Te&&ke())}}function g(k){return Array.isArray(k.runs)?k.runs:[]}function C(){let k=U(),T=new Set;for(let N of Object.values(k.attempts||{})){let ee=N;ee&&typeof ee.bead_id=="string"&&!Nm.has(ee.status)&&T.add(ee.bead_id)}for(let N of Array.isArray(k.pr_wait)?k.pr_wait:[])N&&typeof N.bead_id=="string"&&T.add(N.bead_id);for(let N of Object.values(k.discard_operations||{})){let ee=N;ee&&ee.phase!=="done"&&typeof ee.bead_id=="string"&&T.add(ee.bead_id)}return T}function H(k){return k.filter(T=>se(T)===null)}function se(k){let T=U();for(let N of Array.isArray(T.serial_lanes)?T.serial_lanes:[])if(Array.isArray(N?.entries)&&N.entries.some(ee=>ee.bead_id===k))return N.id;return(Array.isArray(T.queue)?T.queue:[]).some(N=>N.bead_id===k)?"parallel":null}function M(k,T){let N=u.get(k);return N||[...T.order]}function D(k){if(k.length<2)return!1;let T=se(k[0]);if(!T||T==="parallel")return!1;let N=U(),ee=(Array.isArray(N.serial_lanes)?N.serial_lanes:[]).find(J=>J.id===T)?.entries.map(J=>J.bead_id);if(!Array.isArray(ee))return!1;let $e=k.map(J=>ee.indexOf(J));return $e.every(J=>J>=0)&&$e.every((J,Se)=>Se===0||J>$e[Se-1])}function ue(){let k=U(),T=Array.isArray(k.serial_lanes)?k.serial_lanes:[],N=T.find(ee=>Array.isArray(ee.entries)&&ee.entries.length===0);return N?N.id:T[0]?.id||"s1"}function ge(k){let T=U().bead_titles||{};return typeof T[k]=="string"?T[k]:k}async function pe(k,T){if(!s||p)return null;p=!0,ke();try{return await s(k,T)}finally{p=!1,ke()}}async function qe(k){n?.setPending?.(!0);try{let T=await pe("worker-parallel-analysis-start",{force:k,target_ids:Array.from(A)});T&&T.applied===!1&&T.reason&&(T.reason==="target_not_qualified"&&Array.isArray(T.detail)?le(`\uBD84\uC11D \uB300\uC0C1 \uC790\uACA9 \uBCC0\uACBD: ${T.detail.join(", ")}`,"error",3200):le(`\uBD84\uC11D \uC2E4\uD328: ${T.reason}`,"error",2800))}finally{n?.setPending?.(!1)}}async function tt(){let k=W().job;!s||!k||await s("worker-parallel-analysis-cancel",{job_id:k.job_id})}async function je(k){if(!(!s||Q.has(k))){Q.add(k),ke();try{let T=await s("worker-parallel-analysis-prompt",{root_dir:O(),run_id:k});if(!Te)return;if(T?.ok===!0&&typeof T.prompt=="string"){Z={run_id:k,prompt:T.prompt};return}le(T?.reason==="not_found"?"\uC800\uC7A5\uB41C \uBD84\uC11D \uD504\uB86C\uD504\uD2B8\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4":"\uBD84\uC11D \uD504\uB86C\uD504\uD2B8\uB97C \uBD88\uB7EC\uC624\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4","error",2800)}finally{Q.delete(k),ke()}}}function _e(){Z=null,ke()}async function Le(){if(!Z)return;let k=await Xt(Z.prompt);le(k?"\uBCF5\uC0AC\uB428":"\uBCF5\uC0AC \uC2E4\uD328",k?"success":"error",1400)}function he(k,T){a&&a(k,Ca(T))}function xe(){return U().runner_catalog}function Re(k){return Object.keys(xe()?.runners?.[k]?.models||{})}function Be(k){let T=Re(k),N=xe()?.runners?.[k]?.default_model;return typeof N=="string"&&T.includes(N)?N:T[0]||""}function Ae(){let k=W().settings,T=f||k.runner||"claude",N=Re(T),ee=f?Be(T):k.model||N[0]||"",$e=Ta(xe(),T,ee),J=k.effort||"",Se=$e.includes(J)?J:$e[0]||"";return{runner:T,model:ee,effort:Se,models:N,efforts:$e}}async function Ue(k){let T=W().settings,N=await pe("worker-parallel-analysis-settings-update",{expected_revision:T.revision,runner:k.runner,model:k.model,effort:k.effort});(!N||N.applied!==!0)&&(f=null,ke(),N&&N.reason&&le(`\uBD84\uC11D \uC124\uC815 \uAC70\uBD80: ${N.reason}`,"error",2800))}function Ze(k){f=k,ke();let T=Ae();Ue({runner:k,model:T.model,effort:T.effort})}function Ee(k){let T=Ae(),N=Ta(xe(),T.runner,k);Ue({runner:T.runner,model:k,effort:N.includes(T.effort)?T.effort:N[0]||""})}function ot(k){let T=Ae();Ue({runner:T.runner,model:T.model,effort:k})}async function Y(k,T){if(!s||p)return;let N=M(k,T),ee=W();if(N.length<2||!ee.last_good){le("\uC81C\uCD9C\uD558\uB824\uBA74 2\uAC1C \uC774\uC0C1\uC774\uC5B4\uC57C \uD569\uB2C8\uB2E4","warning");return}let $e=d.get(k)||ue(),J=()=>({snapshot_digest:ee.last_good.identity_digest,group_index:k,lane:$e,ordered_bead_ids:N,expected_revision:U().revision});p=!0,ke();try{let Se=await s("worker-parallel-analysis-submit",J());Se&&Se.queue&&r&&r.set(Se.queue),Se&&Se.applied!==!0&&Se.conflict===!0&&(Se=await s("worker-parallel-analysis-submit",J()),Se&&Se.queue&&r&&r.set(Se.queue)),Se&&Se.applied===!0?(u.delete(k),le(`\uC9C1\uB82C \uB808\uC778 ${$e}\uC5D0 ${N.length}\uAC1C \uBC30\uCE58`,"success")):le(`\uC81C\uCD9C \uAC70\uBD80: ${Se?.reason||"conflict"} (\uD050 \uBB34\uBCC0\uACBD)`,"error",2800)}finally{p=!1,ke()}}function j(k,T,N){u.set(k,M(k,T).filter(ee=>ee!==N)),ke()}function oe(k){u.delete(k),ke()}function Oe(k,T,N,ee){let $e=[...M(k,T)],J=$e.indexOf(N),Se=J+ee;J<0||Se<0||Se>=$e.length||($e.splice(Se,0,...$e.splice(J,1)),u.set(k,$e),ke())}function We(){let k=W().settings,T=Object.keys(xe()?.runners||{}),N=Ae();return l`<div class="pa-settings">
      ...
      >`:k.is_default===!0?l`<span class="pa-settings__default">기본값</span>`:""}function Ie(k){return k.is_default===!0&&k.compatible===!1}function lt(k){return!!(k.runner&&k.model&&k.effort)}function Xe(k){return lt(k)&&k.compatible!==!1}function V(k){let T=Math.max(0,Math.floor(k/1e3)),N=Math.floor(T/60),ee=T%60;return`${N}:${String(ee).padStart(2,"0")}`}function te(k){let T=k.job;if(T){let N=typeof T.started_at=="number"?T.started_at:0,ee=`${T.runner||"?"}/${T.model||"?"}`,$e=N?` \xB7 \uACBD\uACFC ${V(Date.now()-N)}`:"",J=typeof T.session_id=="string"?T.session_id:"",Se=g(k).find(Ce=>Ce.run_id===T.job_id);return l`<span class="pa-meta__progress">
        ...
        >`:""}function Me(){return n?.isPending?.()===!0}function ze(k){let T=!!k.job,N=Xe(k.settings),ee=R!==null&&A.size===0,$e=T||p||Me()||I;return l`<div class="pa-meta">
...
${Z.prompt}</pre
    ...
    </div>`:""}function ae(k,T){let N=M(k,T),ee=C(),$e=N.filter(nt=>ee.has(nt)),J=H(N),Se=D(N),Ce=Array.isArray(U().serial_lanes)?U().serial_lanes:[],ft=d.get(k)||ue(),Et=T.eligible!==!0||N.length<2||$e.length>0||J.length>0||Se||p;return l`<section class="pa-group" data-group-index=${String(k)}>
    ...
    </section>`}function de(k){let T=Array.isArray(k.issues)?k.issues:[],N=T.filter($e=>$e.verdict==="parallel_ok").length,ee=T.filter($e=>$e.verdict==="uncertain").length;return l`<div class="pa-summary">
      <span>parallel_ok ${N}</span>
      <span>uncertain ${ee}</span>
    </div>`}function Ne(){let k=Te&&!!W().job;if(k&&h===null){h=setInterval(()=>ke(),1e3);return}!k&&h!==null&&(clearInterval(h),h=null)}function ke(){let k=W();f&&k.settings.runner===f&&(f=null);let T=k.last_good?.result;Ne(),Ye(l`
        ...
        ${X()}
      `,i)}let Te=!1,ye=()=>{Te=!1,Z=null,F+=1,Ne()},He=k=>{k.target===k.currentTarget&&be()};i.addEventListener("close",ye),i.addEventListener("cancel",ye),i.addEventListener("click",He);let rt=null;r&&r.subscribe&&(rt=r.subscribe(()=>{Te&&ke()}));let B=null;n&&n.subscribe&&(B=n.subscribe(()=>{Te&&ke()}));function re(){Te||(Te=!0,ke(),z(),typeof i.showModal=="function"?i.showModal():i.setAttribute("open",""))}function be(){Te&&(Te=!1,Z=null,F+=1,Ne(),typeof i.close=="function"?i.close():i.removeAttribute("open"))}return{open:re,close:be,destroy(){Te=!1,h!==null&&(clearInterval(h),h=null),i.removeEventListener("close",ye),i.removeEventListener("cancel",ye),i.removeEventListener("click",He),rt&&(rt(),rt=null),B&&(B(),B=null),i.remove()}}}var Au=new Set(["sh","bash","zsh","dash","ksh"]),Su=/('(?:[^']*)'|"(?:\\.|[^"\\])*"|#.*|\$(?:\{[^}\n]*\}|[A-Za-z_][A-Za-z0-9_]*|[?#@*!$0-9-])|\b(?:if|then|else|elif|fi|for|while|until|do|done|case|esac|in|function|select|time)\b)/g;function Eu(e){let t=e.split("/");return t[t.length-1]||""}function Bm(e){let t=e.split(`
`,1)[0];if(!t.startsWith("#!"))return!1;let r=t.slice(2).trim().split(/\s+/).filter(Boolean);if(r.length===0)return!1;let n=Eu(r[0]);if(n!=="env")return Au.has(n);let s=r.slice(1).find(o=>!o.startsWith("-")&&!o.includes("="));return s!==void 0&&Au.has(Eu(s))}function Um(e){return e.startsWith("#")?"comment":e.startsWith("'")||e.startsWith('"')?"string":e.startsWith("$")?"variable":"keyword"}function Wm(e){let t=[],r=0;Su.lastIndex=0;for(let n of e.matchAll(Su)){let s=n.index;s>r&&t.push({text:e.slice(r,s),kind:"plain"}),t.push({text:n[0],kind:Um(n[0])}),r=s+n[0].length}return r<e.length&&t.push({text:e.slice(r),kind:"plain"}),t.length===0&&t.push({text:e,kind:"plain"}),t}function zm(e){return{bad_request:"\uC2A4\uD06C\uB9BD\uD2B8 \uC694\uCCAD\uC774 \uC62C\uBC14\uB974\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4.",forbidden:"\uB4F1\uB85D\uB418\uC9C0 \uC54A\uC740 \uC6CC\uD06C\uC2A4\uD398\uC774\uC2A4\uC758 \uC2A4\uD06C\uB9BD\uD2B8\uB294 \uC77D\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.",lane_not_declared:"\uD604\uC7AC \uACE0\uC815 \uC120\uC5B8\uC5D0 \uD574\uB2F9 \uC2A4\uD06C\uB9BD\uD2B8\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4.",stale_declaration:"\uC800\uC7A5\uC18C \uC791\uC5C5 \uC120\uC5B8\uC774 \uBC14\uB00C\uC5C8\uC2B5\uB2C8\uB2E4. \uCD5C\uC2E0 \uD654\uBA74\uC5D0\uC11C \uB2E4\uC2DC \uC5F4\uC5B4 \uC8FC\uC138\uC694.",too_large:"\uC2A4\uD06C\uB9BD\uD2B8\uAC00 \uB108\uBB34 \uCEE4\uC11C \uD654\uBA74\uC5D0 \uD45C\uC2DC\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.",unsupported_content:"\uD14D\uC2A4\uD2B8 \uD615\uC2DD\uC758 \uC2A4\uD06C\uB9BD\uD2B8\uB9CC \uD45C\uC2DC\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4.",unreadable:"\uACE0\uC815\uB41C \uC2A4\uD06C\uB9BD\uD2B8 \uB0B4\uC6A9\uC744 \uC77D\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4."}[e]||"\uC2A4\uD06C\uB9BD\uD2B8\uB97C \uBD88\uB7EC\uC624\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4."}function Tu(e){let t=e.getWorkspacePath,r=e.fetchImpl||globalThis.fetch?.bind(globalThis),n=document.createElement("div");n.className="repo-ops-script-viewer-root",document.body.appendChild(n);let s=null,o="loading",a="",i="",u=0,d=null,p=!1;function f(O,z){return z?Wm(O).map(g=>g.kind==="plain"?g.text:l`<span
            ...
            >${g.text}</span
          >`):O}function h(){if(!s)return l``;let O=o==="ready"&&Bm(a),z=o==="ready"?a.split(`
`):[];return l`<div
    ...
    </div>`}function R(){Ye(h(),n)}async function A(){if(o!=="ready")return;let O=await Xt(a);le(O?"\uC2A4\uD06C\uB9BD\uD2B8 \uBCF5\uC0AC\uB428":"\uC2A4\uD06C\uB9BD\uD2B8 \uBCF5\uC0AC \uC2E4\uD328",O?"success":"error")}function I(O){O.key==="Escape"&&s&&(O.preventDefault(),U())}function F(){p||(document.addEventListener("keydown",I),p=!0)}function Z(){p&&(document.removeEventListener("keydown",I),p=!1)}async function Q(O,z=null){let g=++u;F(),s={...O},d=z||(document.activeElement instanceof HTMLElement?document.activeElement:null),o="loading",a="",i="",R(),n.querySelector(".repo-ops-script-viewer__close")?.focus();let H=t?t():"";if(!H){o="error",i="\uC6CC\uD06C\uC2A4\uD398\uC774\uC2A4\uAC00 \uC120\uD0DD\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4.",R();return}if(!r){o="error",i="\uC2A4\uD06C\uB9BD\uD2B8 \uC694\uCCAD \uAE30\uB2A5\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.",R();return}let se="/api/repo-ops-script?workspace="+encodeURIComponent(H)+"&lane="+encodeURIComponent(O.lane)+"&base_sha="+encodeURIComponent(O.base_sha);try{let M=await r(se),D=await M.json().catch(()=>({}));if(g!==u)return;if((t?t():"")!==H){U();return}if(!M.ok||!D||D.ok!==!0){o="error",i=zm(D&&typeof D.error=="string"?D.error:""),R();return}s={lane:D.lane,base_sha:D.base_sha,path:D.path,base_ref:D.base_ref},a=String(D.content),o="ready",R()}catch{if(g!==u)return;o="error",i="\uC2A4\uD06C\uB9BD\uD2B8 \uC694\uCCAD\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4.",R()}}function U(){u+=1,Z(),s=null,a="",R();let O=d;d=null,O?.isConnected&&O.focus()}function W(){U(),n.remove()}return{open:Q,close:U,destroy:W}}function Cu(e){let t=e.queueStore,r=e.transport,n=e.onChanged||(()=>{}),s=e.onOpenScript;function o(){return t&&t.get()||{}}function a(){let g=o();return typeof g.revision=="number"?g.revision:0}function i(g){t&&g&&g.queue&&typeof g.queue=="object"&&t.set(g.queue)}function u(){let g=o().workspace_info;return g&&typeof g=="object"?g:{}}function d(g,C){return l`<span
      class="worker-repo-ops__vd-badge worker-repo-ops__vd-badge--${g}"
      >${C}</span
    >`}function p(g){if(typeof g!="number"||!Number.isFinite(g))return"";let C=g/6e4;return Number.isInteger(C)?`timeout ${C}\uBD84`:`timeout ${Math.round(g/1e3)}\uCD08`}function f(g){let C=p(g);return C?d("config",C):""}function h(g,C,H){return l`<button
      type="button"
    ...
    ></button>`}function R(){let g=o().repo_ops_opt_out;return{verify:g?.verify===!0,deploy:g?.deploy===!0}}function A(g,C){return l`<label class="worker-repo-ops__lane-run">
      ...
      />
      이 workspace에서 실행
    </label>`}function I(g){let C=typeof g.base_sha=="string"?g.base_sha:"",H=`${g.source_path||"repo-ops/config.toml"} @ ${g.base_ref||"?"}${C?`@${C.slice(0,7)}`:""}`,se=R(),M=!!g.verify&&se.verify,D=!!g.deploy&&se.deploy;return l`<section class="worker-repo-ops__vd" data-seam="repo-ops">
      ...
      >
        ...
        <span class="worker-repo-ops__lane-v"
          >${g.verify?l`${h("verify",g,g.verify)}
              ${f(g.verify.timeout_ms)}
              ${M?d("skipped","\uC774 workspace\uC5D0\uC11C \uAC74\uB108\uB700"):""}`:l`선언 없음${d("absent","verify \uC5C6\uC774 \uD310\uC815")}`}</span
        >
        <span class="worker-repo-ops__lane-d"
          >${M?"\uC774 workspace\uC5D0\uC11C\uB294 \uAC80\uC99D \uC5C6\uC774 \uD310\uC815\uD569\uB2C8\uB2E4.":g.verify?"\uBA38\uC9C0 \uC804\uC5D0 \uC774 \uC2A4\uD06C\uB9BD\uD2B8\uAC00 \uD1B5\uACFC\uD574\uC57C \uC790\uACA9\uC744 \uC5BB\uC2B5\uB2C8\uB2E4.":"\uBA38\uC9C0 \uC790\uACA9\uC740 PR/base/head \uC2E0\uC120\uB3C4\xB7mergeability\xB7\uB9AC\uBDF0 \uC601\uC218\uC99D\uC73C\uB85C\uB9CC \uD310\uC815\uD569\uB2C8\uB2E4."}</span
        >
      ...
      >
        ...
        <span class="worker-repo-ops__lane-v"
          >${g.deploy?l`${h("deploy",g,g.deploy)}
              ${f(g.deploy.timeout_ms)}
              ${D?d("skipped","\uC774 workspace\uC5D0\uC11C \uAC74\uB108\uB700"):""}`:l`선언 없음${d("absent","\uBC30\uD3EC \uC5C6\uC74C")}`}</span
        >
        <span class="worker-repo-ops__lane-d"
          >${D?"\uC774 workspace\uC5D0\uC11C\uB294 \uBC30\uD3EC \uC5C6\uC774 \uACE7\uBC14\uB85C \uC815\uB9AC\uB85C \uB118\uC5B4\uAC11\uB2C8\uB2E4.":g.deploy?l`Worker가 <code>.worktrees/.repo-ops-deploy</code>에서 대상
...
`),live:s.live===!0,alert:s.alert===!0}};return e.continuation_required?r("\uC774\uC5B4\uD558\uAE30 \uC120\uD0DD \uD544\uC694",{alert:!0}):e.merge_step?e.gate?.tier==="merged"?r("\uBA38\uC9C0\uB428",{title:e.merge_step.label,alert:e.merge_step.failed===!0}):r("\uBA38\uC9C0 \uC911",{title:e.merge_step.label,live:!0}):e.conflict_badge?r(e.conflict_badge,{live:e.conflict_live===!0}):e.head_review&&e.head_review.state!=="failed"?r("\uB9AC\uBDF0 \uC9C4\uD589 \uC911",{title:e.head_review.badge,live:e.head_review.live===!0}):e.recovery?.lock_actions?r(e.recovery.badge,{title:e.recovery.title,live:!0}):e.cleanup_failed?r(e.cleanup_label?`\uC815\uB9AC \uBA48\uCDA4 \xB7 ${e.cleanup_label}`:"\uC815\uB9AC \uBA48\uCDA4",{title:e.cleanup_failed.reason||"",alert:!0}):e.base_exception?r("\uB2E4\uB978 base \uB300\uC0C1",{title:e.base_exception,alert:!0}):e.conflicting?r("\uCDA9\uB3CC \uD574\uACB0 \uD544\uC694",{alert:!0}):e.gate?.reason==="base_behind"?r("base \uAC31\uC2E0 \uD544\uC694",{alert:!0}):e.gate?.reason==="review_receipt_missing"||e.gate?.reason==="review_receipt_stale"?r("\uCD5C\uC885 \uBCC0\uACBD \uB9AC\uBDF0 \uD544\uC694",{title:e.gate.reason==="review_receipt_stale"?"\uB9AC\uBDF0 \uC601\uC218\uC99D\uC774 \uD604\uC7AC head\uC758 \uC870\uC0C1\uC774 \uC544\uB2C8\uAC70\uB098 \uC870\uC0C1 \uD655\uC778\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4 \u2014 \uD788\uC2A4\uD1A0\uB9AC \uC7AC\uC791\uC131\xB7\uBE0C\uB79C\uCE58 \uB9AC\uC14B \uBCF5\uAD6C \uACBD\uB85C\uB85C, \uAD00\uCE21\uB41C \uCD5C\uC885 head \uC804\uCCB4\uB97C \uB2E4\uC2DC \uB9AC\uBDF0\uD569\uB2C8\uB2E4":"\uB9AC\uBDF0 \uC601\uC218\uC99D\uC774 \uC5C6\uC2B5\uB2C8\uB2E4 \u2014 \uAD00\uCE21\uB41C \uCD5C\uC885 head \uC804\uCCB4\uB97C \uB9AC\uBDF0\uD574\uC57C \uBA38\uC9C0\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4",alert:!0}):e.gate?.reason==="spec_id_missing"?r("\uC2A4\uD399 ID \uB204\uB77D",{title:"native spec_id \uBBF8\uAE30\uB85D \u2014 bd update --spec-id \uD544\uC694",alert:!0}):e.gate?.reason==="review_receipt_invalid"?r("\uB9AC\uBDF0 \uAE30\uB85D \uC624\uB958",{title:"review_receipt_invalid",alert:!0}):Bu(e.receipt_check).length>0?r("\uC601\uC218\uC99D \uD655\uC778 \uD544\uC694",{title:`\uC131\uB9BD\uD558\uC9C0 \uC54A\uB294 \uC2E4\uD589 \uC601\uC218\uC99D \u2014 ${Bu(e.receipt_check).join(", ")}`,alert:!0}):e.head_review?.state==="failed"?r("\uB9AC\uBDF0 \uC2E4\uD328",{title:e.head_review.failure_reason||"",alert:!0}):e.recovery?r(e.recovery.badge,{title:e.recovery.title,alert:!0}):e.gate?.tier==="verify"&&e.gate.gate_badge==="\uAC80\uC99D \uC2E4\uD328"?r("\uAC80\uC99D \uC2E4\uD328",{title:e.gate.reason||"",alert:!0}):e.queue_failure?r(`\uBA38\uC9C0 \uC2E4\uD328 \u2014 ${ju(e.queue_failure)}`,{title:e.queue_failure,alert:!0}):e.auto_skip?r(`\uC790\uB3D9 \uC81C\uC678 \u2014 ${ju(e.auto_skip)}`,{title:e.auto_skip,alert:!0}):e.queued&&!e.queue_active?r(`\uBA38\uC9C0 \uB300\uAE30 #${e.queue_position}`):e.gate?.enabled===!0?r("\uBA38\uC9C0 \uAC00\uB2A5"):e.gate?.tier==="merged"?r("\uBA38\uC9C0\uB428"):e.gate?.tier==="closed_unmerged"?r("\uB2EB\uD798",{alert:!0}):e.activity?r("\uD655\uC778 \uC911",{live:!0}):e.gate?.tier==="undecidable"||e.gate?.reason==="mergeability_unknown"?r("\uC0C1\uD0DC \uD655\uC778 \uC2E4\uD328",{title:e.gate.reason||"",alert:!0}):e.gate?.tier==="unobserved"||e.gate?.tier==="verify"||e.gate?.gate_badge==="\uAD00\uCE21 \uB300\uAE30"?r("\uD655\uC778 \uC911"):e.gate?.gate_badge?r(e.gate.gate_badge,{title:e.gate.reason||"",alert:e.gate.enabled!==!0}):null}function Eg(e,t,r,n,s=null,o=null,a=null,i=!1,u=null,d=!0,p=null,f=null,h=null,R={},A=!1,I=!1,F={}){let Z=!!u&&u.position>0,Q=!!u?.continuation_action&&u.continuation_action.continuation===null,U=!!u&&u.active===!0,W=u&&u.failure||null,O=kg(u?u.waiting:null,h),z=r[e]||null,g=z&&z.gate?z.gate:null,C=z&&z.pr?z.pr:null,H=Ag(h),se=$g(u?u.resolution:null),M=xg(u?u.head_review:null),D=u&&u.head_review||null,ue=u&&u.authority||null,ge=!!D&&["pending","reviewing","revising"].includes(D.state),pe=Z&&!U&&(D?.state==="failed"||!ue||ue.source==="automatic"&&!I),qe=a==="paused"?"\uCDA9\uB3CC \uD574\uC18C \uC77C\uC2DC\uC815\uC9C0":se?se.badge:a==="running"?"\uCDA9\uB3CC \uD574\uC18C \uC911":O,tt=!!g&&g.base_badge==="\uCDA9\uB3CC",je=!!g&&g.enabled===!0,_e=Un({bead_id:e,merge_sha:F.merge_sha,cleanup_cursor:F.cleanup_cursor,merge_progress:o&&o.merge_progress?o.merge_progress:null,cleanup_failed:n,repo_operations:F.repo_operations}),Le=to(_e),he=!!n&&["child_sweep","branch_cleanup","parent_close"].includes(n.step)&&!!g&&g.tier==="merged",xe=i&&!!n&&!!g&&g.tier==="merged",Re=pe&&(je||tt||g?.reason==="base_behind"||g?.reason==="review_receipt_missing"||g?.reason==="review_receipt_stale"||he||xe),Be=i&&tt&&d===!1,Ae=cr(R,e,{external:i,merge_active:U||_e?.step==="merge",merge_queued:Z,conflict_active:!!a,cleanup_active:Le,merged:!!n||g?.tier==="merged"}),Ue=!!Ae.operation,Ze=!he&&!!n&&n.step==="repo_operations",Ee=Sg({continuation_required:Q,merge_step:_e,conflict_badge:qe,conflict_live:se?.live===!0||a==="running",head_review:D&&M?{...M,state:D.state,failure_reason:D.failure_reason}:null,recovery:H,cleanup_failed:n,cleanup_label:n?Hr(n.step):null,base_exception:f,conflicting:tt,gate:g,receipt_check:z&&z.receipt_check?z.receipt_check:null,queue_failure:W,auto_skip:p,queued:Z,queue_active:U,queue_position:u?u.position:0,activity:qe?null:o&&o.activity||null}),ot=Ee?.live===!0&&Ee.title?l`<span title=${Ee.title}>${Ee.label}</span>`:Ee?.label||null;return{id:e,title:i?l`${t}<span class="muted"> · 세션</span>`:t,reason:n&&_e?.active!==!0?eo(n.step):"PR \uB300\uAE30",draggable:!1,done:!0,lane:"pr_wait",worker_serial:A,external:i,pr_number:C&&typeof C.number=="number"?C.number:null,pr_url:C&&typeof C.url=="string"?C.url:"",completion_badge:Ee?.live!==!0&&Ee?.title?Ee.label:null,completion_title:Ee?.title||"",completion_repair_pr_url:H?H.repair_pr_url:"",completion_repair_pr_number:H?H.repair_pr_number:null,badges:ot?[ot]:[],live_badge:Ee?.live===!0?ot:null,usage:s,alert:Ee?.alert===!0,merge_action:g?.tier==="merged"&&!he&&!xe||Ze?!1:!Z||Q||pe,timeline_action:Ze,cancel_action:Z&&!Q,cancel_enabled:(!U||ge)&&!(H&&H.lock_actions),cancel_title:H&&H.lock_actions?`${H.badge} \u2014 \uC911\uB2E8\uD558\uB824\uBA74 \uC0C1\uB2E8 \uC790\uB3D9 \uBA38\uC9C0 \uC911\uB2E8\uC744 \uC0AC\uC6A9\uD558\uC138\uC694`:U&&!ge?"\uBA38\uC9C0 \uC9C4\uD589 \uC911 \u2014 \uCDE8\uC18C\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4":ge?"review \uC9C4\uD589\uC744 \uCDE8\uC18C\uD558\uACE0 \uBA38\uC9C0 \uAD8C\uD55C\uC744 \uD3D0\uAE30\uD569\uB2C8\uB2E4":"\uBA38\uC9C0 \uD050\uC5D0\uC11C \uC774 \uD56D\uBAA9\uC744 \uBE8D\uB2C8\uB2E4 (\uB2E4\uC2DC [\uBA38\uC9C0]\uB85C \uB123\uC744 \uC218 \uC788\uC2B5\uB2C8\uB2E4)",discard:Ae,discard_action:Ae.action,merge_step:_e,discard_enabled:Ae.enabled,discard_title:Ae.title,merge_enabled:!_e&&!a&&!Ue&&!f&&!(H&&H.lock_actions)&&!Be&&!Ze&&(je||tt||g?.reason==="base_behind"||g?.reason==="review_receipt_missing"||g?.reason==="review_receipt_stale"||he||xe||Re),merge_label:Q?"\uC774\uC5B4\uD558\uAE30 \uC120\uD0DD":he||xe?"\uC815\uB9AC \uC7AC\uAC1C":tt&&!_e&&!he?"\uCDA9\uB3CC \uD574\uC18C \uD6C4 \uBA38\uC9C0":g?.reason==="base_behind"?"base \uAC31\uC2E0 \uD6C4 \uBA38\uC9C0":g?.reason==="review_receipt_missing"||g?.reason==="review_receipt_stale"?"\uB9AC\uBDF0 \uD6C4 \uBA38\uC9C0":pe?"\uB2E4\uC2DC \uBA38\uC9C0":void 0,merge_title:Ue?Ae.error?`\uD3D0\uAE30 \uC2E4\uD328: ${Ae.error} \u2014 [\uC7AC\uC2DC\uB3C4]\uD558\uAC70\uB098 \uC0C1\uD0DC\uB97C \uD655\uC778\uD558\uC138\uC694`:`\uD3D0\uAE30 \uC9C4\uD589 \uC911 \u2014 ${Ae.progress||"\uC644\uB8CC\uB97C \uAE30\uB2E4\uB9AC\uC138\uC694"}`:Q?"\uC2E4\uD589 provider\uAC00 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4 \u2014 \uC774\uC5B4\uAC08 \uBC29\uC2DD\uC744 \uC120\uD0DD\uD558\uC138\uC694":_e?`\uBA38\uC9C0 \uC9C4\uD589 \uC911 \u2014 ${_e.label}`:xe?"\uBA38\uC9C0 \uC644\uB8CC \u2014 \uD074\uB9AD\uD558\uBA74 \uC2E4\uD328\uD55C \uC815\uB9AC\uB97C \uC7AC\uAC1C\uD569\uB2C8\uB2E4":Be?"\uC6CC\uD06C\uD2B8\uB9AC \uC5C6\uC74C \u2014 \uC138\uC158\uC5D0\uC11C \uC9C1\uC811 \uD574\uC18C\uD558\uC138\uC694":a==="running"?"\uCDA9\uB3CC \uD574\uC18C \uC138\uC158 \uC2E4\uD589 \uC911 \u2014 \uC644\uB8CC \uD6C4 \uB2E4\uC2DC \uBA38\uC9C0\uD558\uC138\uC694":a==="paused"?"\uCDA9\uB3CC \uD574\uC18C \uC138\uC158 \uC77C\uC2DC\uC815\uC9C0 \u2014 \uC7AC\uAC1C \uD6C4 \uC644\uB8CC\uB418\uBA74 \uBA38\uC9C0\uD558\uC138\uC694":he?"\uBA38\uC9C0 \uC644\uB8CC \u2014 \uD074\uB9AD\uD558\uBA74 \uB0A8\uC740 \uC815\uB9AC\uB97C \uC2E4\uD328 \uB2E8\uACC4\uBD80\uD130 \uC7AC\uAC1C\uD569\uB2C8\uB2E4":tt?"\uCDA9\uB3CC \u2014 \uD050\uC5D0 \uB123\uC73C\uBA74 \uD574\uC18C \uC138\uC158\uC744 \uB744\uC6B0\uACE0 \uC644\uB8CC \uD6C4 \uC790\uB3D9\uC73C\uB85C \uC7AC\uBA38\uC9C0\uD569\uB2C8\uB2E4":g?.reason==="base_behind"?"base\uB97C \uC790\uB3D9 \uAC31\uC2E0\uD55C \uB4A4 \uBA38\uC9C0\uD569\uB2C8\uB2E4":g?.reason==="review_receipt_missing"||g?.reason==="review_receipt_stale"?"\uC790\uB3D9 \uB9AC\uBDF0 \uC138\uC158 \uD6C4 \uC2B9\uC778\uB418\uBA74 \uBA38\uC9C0\uD569\uB2C8\uB2E4":g?.reason==="spec_id_missing"?"native spec_id \uBBF8\uAE30\uB85D \u2014 bd update --spec-id\uB85C \uAE30\uB85D\uD55C \uB4A4 \uB2E4\uC2DC \uBA38\uC9C0\uD558\uC138\uC694":je?`\uBA38\uC9C0 (${g.gate_badge}) \u2014 \uD050\uC5D0 \uB123\uC5B4 \uC21C\uC11C\uB300\uB85C \uBA38\uC9C0\uD569\uB2C8\uB2E4 (\uCC28\uB840\uAC00 \uB418\uBA74 \uB2E4\uC2DC \uD655\uC778)`:g&&g.tier==="merged"?"\uBA38\uC9C0\uB428 \u2014 \uBA38\uC9C0 \uD6C4 \uC815\uB9AC \uC9C4\uD589 \uC911":`\uBA38\uC9C0 \uBD88\uAC00: ${g&&g.reason||"\uAD00\uCE21 \uB300\uAE30"}`}}function La(e,t={}){let{transport:r,issueStores:n,queueStore:s,analysisStore:o,sessionLogStore:a,uiOrderStore:i,gotoIssue:u,getWorkspacePath:d,doneRange:p,onDoneRangeChange:f}=t,h=n?fs(n,i):null,R=ms({transport:r,uiOrderStore:i}),A=null,I=[],F=og(),Z=null,Q=ug(),U=Wt(p)?p:pg(),W=new Map;function O(){let c=or.find(_=>_.value===U);return c?c.label:"\uC624\uB298"}let z=mg(),g=!1,C=new Set,H=new Set,se=new Set,M=new Set,D=[],ue=document.createElement("div");ue.className="worker-console";let ge=document.createElement("div");ge.className="worker-top";let pe=document.createElement("div");pe.className="worker-drawer-overlay",pe.hidden=!0;let qe=document.createElement("div");qe.className="worker-drawer-overlay__backdrop";let tt=document.createElement("div");tt.className="worker-drawer-host";let je=document.createElement("div");je.className="worker-drawer-host",je.hidden=!0,pe.append(qe,tt,je);let _e=document.createElement("div");_e.className="worker-lanes-host",ue.append(ge,pe,_e),e.appendChild(ue);let Le=null,he=null,xe=qs(tt,{transport:r,sessionLogStore:a,onClose:()=>{Le=null,he=null,pe.hidden=!0,J()}}),Re=Du(je,{onClose:()=>{je.hidden=!0,pe.hidden=!0,J()}}),Be=Tu({getWorkspacePath:d||(()=>"")}),Ae=d&&d()||"",Ue=Cu({queueStore:s,transport:r,onChanged:()=>J(),onOpenScript:(c,_)=>{Be.open(c,_)}}),Ze=o?xu(ue,{queueStore:s,analysisStore:o,transport:r,getWorkspacePath:d,onOpenTranscript:(c,_)=>kr(c,_)}):null;function Ee(){return s&&s.get()||{revision:0,auto_advance:!1,auto_merge:!1,slots:oo,queue:[],serial_lanes:[],serial_lane_count:0,pr_wait:[],done:[]}}function ot(){let c=Ee(),_=typeof c.serial_lane_count=="number"&&Number.isInteger(c.serial_lane_count)&&c.serial_lane_count>0?Math.min(c.serial_lane_count,5):0,S=Array.isArray(c.serial_lanes)?c.serial_lanes:[],G=[];for(let we of S){if(G.length>=_)break;!we||typeof we.id!="string"||!/^s[1-5]$/.test(we.id)||!Array.isArray(we.entries)||G.push({id:we.id,label:`\uC9C1\uB82C ${we.id.slice(1)}`,count:we.entries.length})}return G.length===0?null:[{id:"parallel",label:"\uBCD1\uB82C",count:(Array.isArray(c.queue)?c.queue:[]).length},...G]}function Y(c){if(!Z||!c.some(S=>S.id===Z))return null;let _=ot();return _?{bead_id:Z,lanes:_}:null}function j(){let c=Ee();return typeof c.revision=="number"?c.revision:0}function oe(c){c&&c.queue&&s&&s.set(c.queue)}function Oe(){let c=Ee().queue;return Array.isArray(c)?c.length:0}async function We(c,_,S){if(!r)return;let G=()=>({bead_id:c,..._==="parallel"?{}:{lane:_},...S===void 0?{}:{index:S},expected_revision:j()}),ie=await r("worker-queue-place",G());oe(ie),ie&&ie.conflict&&await r("worker-queue-place",G()).then(oe)}async function Ve(c,_,S){if(!r)return;let G=()=>({bead_id:c,..._==="parallel"?{}:{lane:_},to_index:S,expected_revision:j()}),ie=await r("worker-queue-reorder",G());oe(ie),ie&&ie.conflict&&await r("worker-queue-reorder",G()).then(oe)}async function Ie(c){if(!r)return;let _=await r("worker-queue-remove",{bead_id:c,expected_revision:j()});oe(_),_&&_.conflict&&await r("worker-queue-remove",{bead_id:c,expected_revision:j()}).then(oe)}async function lt(c){if(!r||!c)return;let _=await r("worker-attempt-pause",{attempt_id:c});_&&_.paused===!1&&_.reason&&le(`\uC77C\uC2DC\uC815\uC9C0 \uAC70\uBD80: ${_.reason}`,"error",2400)}async function Xe(c){if(!r||!c)return;let _=await sn();if(_===null)return;let S=async(ie={})=>await r("worker-attempt-resume",{attempt_id:c,expected_revision:j(),..._!==""?{instructions:_}:{},...ie}),G=await S();oe(G),G&&G.conflict&&(G=await S(),oe(G)),G=await fr(G,(ie,we)=>S({continuation:ie,decision_token:we}),{onResult:oe,refresh:()=>S()}),G&&G.resumed===!1&&!G.conflict&&G.reason&&le(`\uC774\uC5B4\uD558\uAE30 \uAC70\uBD80: ${G.reason}`,"error",2400)}async function V(c){if(!r||!c)return;let _=await r("worker-attempt-dismiss",{attempt_id:c,expected_revision:j()});oe(_),_&&_.conflict&&(_=await r("worker-attempt-dismiss",{attempt_id:c,expected_revision:j()}),oe(_)),_&&_.dismissed===!1&&!_.conflict&&_.reason&&le(`\uBC30\uB108 \uB2EB\uAE30 \uAC70\uBD80: ${_.reason}`,"error",2400)}async function te(c,_,S=!0){if(!r)return null;let G=r,ie=await G(c,{..._,expected_revision:j()});return oe(ie),ie&&ie.conflict&&S&&(ie=await G(c,{..._,expected_revision:j()}),oe(ie)),ie}async function Me(c){if(!r||!c)return;let _=Ee().merge_queue?.find(G=>G.bead_id===c)?.continuation_action;if(_?.mismatch&&_.continuation===null){await fe(c,_.mismatch);return}C.add(c),J();let S;try{S=await te("worker-merge-queue-add",{bead_id:c})}finally{C.delete(c),J()}!S||S.conflict||S.applied||le(wg(S.reason),"error",2400)}async function ze(c){if(!(!r||!c||H.has(c))){H.add(c),J();try{let _=await r("worker-cleanup-retry",{bead_id:c,expected_revision:j()});oe(_),_&&!_.retried&&!_.conflict&&_.reason&&le(`\uC815\uB9AC \uC7AC\uAC1C \uAC70\uBD80: ${_.reason}`,"error",2400)}finally{H.delete(c),J()}}}async function fe(c,_){let S=await fr({continuation_mismatch:_},(ie,we)=>te("worker-merge-queue-add",{bead_id:c,continuation:ie,decision_token:we},!1)),G=S?.queue?.merge_queue?.find(ie=>ie.bead_id===c)?.continuation_action;if(S?.applied!==!0&&G?.continuation===null&&G.mismatch){await fe(c,G.mismatch);return}S&&S.applied===!1&&!S.conflict&&le("\uC774\uC5B4\uD558\uAE30 \uC120\uD0DD\uC774 \uCD5C\uC2E0 \uC0C1\uD0DC\uC640 \uC77C\uCE58\uD558\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4","error",2800)}async function b(c){if(!r)return;let _=await te("worker-merge-auto-toggle",{on:c});!_||_.conflict||le(c?"\uC790\uB3D9 \uBA38\uC9C0 \uCF1C\uC9D0 \u2014 \uC790\uACA9\uC774 \uC0DD\uAE30\uB294 PR\uC744 \uACC4\uC18D \uBA38\uC9C0\uD569\uB2C8\uB2E4":"\uC790\uB3D9 \uBA38\uC9C0 \uAEBC\uC9D0 \u2014 \uB300\uAE30 \uD56D\uBAA9\uC744 \uBE44\uC6E0\uC2B5\uB2C8\uB2E4",c?"success":"info",2400)}async function x(c){if(!r||!c)return;let _=await te("worker-merge-queue-remove",{bead_id:c});_&&!_.conflict&&!_.applied&&_.reason==="merge_active"&&le("\uBA38\uC9C0 \uC9C4\uD589 \uC911 \u2014 \uCDE8\uC18C\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4","error",2400)}async function $(){await te("worker-merge-queue-remove",{all:!0})}async function P(c,_=null,S="unmerged",G=null){if(!r||!c)return;let ie=Fn(c,S);if(!(!!G||typeof globalThis.confirm!="function"||globalThis.confirm(ie)))return;let me=await r("worker-discard",{bead_id:c,..._?{attempt_id:_}:{},...G?{operation_id:G}:{},expected_revision:j()});if(oe(me),me&&me.conflict&&(me=await r("worker-discard",{bead_id:c,..._?{attempt_id:_}:{},...G?{operation_id:G}:{},expected_revision:j()}),oe(me)),me&&me.discarded===!0){le(Qs(me),"success",5e3);return}if(me&&me.reason){le(`\uD3D0\uAE30 \uC2E4\uD328: ${me.reason}`,"error",2800);return}if(me&&me.accepted&&me.pending==="merged_revert"){le("revert PR \uB300\uAE30 \uC0C1\uD0DC\uB85C \uC804\uD658\uD588\uC2B5\uB2C8\uB2E4","success",2400);return}if(me&&me.accepted&&!me.discarded){le(`\uD3D0\uAE30 \uC9C4\uD589: ${me.phase||"\uBC31\uC5C5 \uC911"}`,"success",2400);return}me&&!me.conflict&&le("\uD3D0\uAE30 \uAC70\uBD80: unknown","error",2800)}async function K(c,_,S){if(!(!r||!_||!S||M.has(_))){M.add(_),J();try{let G=await r(c,{bead_id:_,action_id:S,expected_revision:j()});oe(G),G?.conflict?le("\uC774\uC804 \uC791\uC5C5 \uC0C1\uD0DC\uAC00 \uBC14\uB00C\uC5C8\uC2B5\uB2C8\uB2E4. \uCD5C\uC2E0 \uC0C1\uD0DC\uB97C \uD655\uC778\uD558\uC138\uC694.","error",2800):!G?.ok&&G?.reason&&le(`\uC774\uC804 \uC791\uC5C5 \uCC98\uB9AC \uAC70\uBD80: ${String(G.reason)}`,"error",2800)}finally{M.delete(_),J()}}}async function X(c,_){if(!r||!_||se.has(_))return;se.add(_),J();let S;try{let G=async(ie={})=>await r(c,{bead_id:_,expected_revision:j(),...ie});S=await G(),oe(S),S&&S.conflict&&(S=await r(c,{bead_id:_,expected_revision:j()}),oe(S)),c==="worker-revise-fix"&&(S=await fr(S,(ie,we)=>G({continuation:ie,decision_token:we}),{onResult:oe,refresh:()=>G()}))}finally{se.delete(_),J()}if(!(!S||S.conflict)){if(S.ok){le(c==="worker-revise-fix"?"\uCC98\uBD84 \uC138\uC158\uC744 \uB744\uC6E0\uC2B5\uB2C8\uB2E4 \u2014 \uC218\uB9AC \uD6C4 \uAD6C\uD604\uC774 \uC7AC\uB514\uC2A4\uD328\uCE58\uB429\uB2C8\uB2E4":"\uB378\uD0C0 \uC2B9\uC778 \uC644\uB8CC \u2014 \uC601\uC218\uC99D \uAC31\uC2E0 + \uD30C\uD0B9 \uD574\uC81C","success",2800);return}le(`\uCC98\uBD84 \uAC70\uBD80: ${S.reason||""}`,"error",3e3)}}async function ae(c){if(!r)return;let _=await r("worker-automation-toggle",{on:c,expected_revision:j()});oe(_),_&&_.conflict&&await r("worker-automation-toggle",{on:c,expected_revision:j()}).then(oe)}async function de(c){if(!r||!c)return;let _=await r("worker-repo-operation-repair",{operation_id:c});if(oe(_),_&&_.ok===!1){le(`\uD574\uACB0 \uC138\uC158 \uAC70\uBD80: ${_.reason||""}`,"error",3e3);return}_&&_.ok===!0&&le("\uD574\uACB0 \uC138\uC158\uC744 \uB744\uC6E0\uC2B5\uB2C8\uB2E4","success",2400)}async function Ne(c){if(!r||!c)return;let _=await r("worker-repo-operation-dismiss",{operation_id:c});oe(_),_&&_.ok===!1&&le(`\uAE30\uB85D \uB2EB\uAE30 \uAC70\uBD80: ${_.reason||""}`,"error",3e3)}async function ke(c){if(!r||!Number.isFinite(c))return;let _=Math.max(oo,Math.floor(c)),S=await r("worker-queue-set-slots",{slots:_,expected_revision:j()});oe(S),S&&S.conflict&&await r("worker-queue-set-slots",{slots:_,expected_revision:j()}).then(oe)}async function Te(c){if(!r||!Number.isInteger(c)||c<1||c>Nu)return;let _=Ee(),S=(Array.isArray(_.serial_lanes)?_.serial_lanes:[]).slice(c).reduce((we,me)=>we+(Array.isArray(me?.entries)?me.entries.length:0),0),G=()=>({count:c,expected_revision:j()}),ie=await r("worker-queue-set-serial-lane-count",G());oe(ie),ie&&ie.conflict&&(ie=await r("worker-queue-set-serial-lane-count",G()),oe(ie)),ie&&ie.applied&&S>0&&le(`\uC9C1\uB82C \uB808\uC778 \uCD95\uC18C \u2014 ${S}\uAC1C \uD56D\uBAA9\uC774 \uBCD1\uB82C \uB300\uAE30\uB85C \uC774\uB3D9`)}function ye(){let c=Ee(),_=h?h.selectBoardColumn(tg,"ready"):[],S=h?h.selectBoardColumn(rg,"blocked"):[],G=h?h.selectBoardColumn(sg,"closed"):[],ie=h?h.selectBoardColumn(ng,"in_progress"):[],we=new Map;for(let y of ie){let q=yg(y);if(!q)continue;let ce=we.get(q);ce?ce.push(y):we.set(q,[y])}let me=y=>{let q=_s(we.get(y)||[]);return q?q.title||q.id:null},Je=c.bead_titles||{},v=new Map;for(let[y,q]of Object.entries(Je))typeof q=="string"&&q.length>0&&v.set(y,q);for(let y of[..._,...S])v.set(y.id,y.title||y.id);let w=c.bead_times&&typeof c.bead_times=="object"&&!Array.isArray(c.bead_times)?c.bead_times:{},m=c.bead_labels&&typeof c.bead_labels=="object"&&!Array.isArray(c.bead_labels)?c.bead_labels:{},L=new Map;for(let[y,q]of Object.entries(m))Array.isArray(q)&&L.set(y,Ea(q));for(let y of[..._,...S]){let q=y.labels;Array.isArray(q)&&!L.has(y.id)&&L.set(y.id,Ea(q))}let E=new Map,ne=o?.get()?.last_good?.result?.groups;for(let y of Array.isArray(ne)?ne:[]){if(y?.eligible!==!0||!Array.isArray(y.members))continue;let q=y.members.map(Ge=>{let pt=(Array.isArray(c.serial_lanes)?c.serial_lanes:[]).find(Dt=>Dt.entries.some(Ct=>Ct.bead_id===Ge));return pt?pt.id:null});if(!(q.every(Ge=>Ge!==null)&&new Set(q).size===1))for(let Ge of y.members)E.set(Ge,y.members.filter(pt=>pt!==Ge))}let Fe=c.bead_blocked_by&&typeof c.bead_blocked_by=="object"&&!Array.isArray(c.bead_blocked_by)?c.bead_blocked_by:{},Ke=new Map;for(let[y,q]of Object.entries(w))q&&typeof q=="object"&&Ke.set(y,q);for(let y of[..._,...S])Ke.set(y.id,{created_at:y.created_at,updated_at:y.updated_at});let Qe=y=>Ke.get(y)||{},De=c.pr_wait||[],mt=c.pr_observations||{},sr=c.pr_activity||{},Vr=c.cleanup_failed||{},Vn=Object.entries(Vr).map(([y,q])=>({bead_id:y,step:q&&q.step?q.step:"",reason:q&&q.reason?q.reason:"",at:q&&typeof q.at=="number"?q.at:null,detail:q&&typeof q.detail=="string"?q.detail:null,output_tail:q&&typeof q.output_tail=="string"&&q.output_tail?q.output_tail:void 0,log_path:q&&typeof q.log_path=="string"&&q.log_path?q.log_path:void 0,retry_count:q&&typeof q.retry_count=="number"&&Number.isInteger(q.retry_count)&&q.retry_count>0?q.retry_count:0,failure_code:q&&typeof q.failure_code=="string"?q.failure_code:void 0,subject_id:q&&typeof q.subject_id=="string"?q.subject_id:void 0,repair_eligible:!!(q&&q.repair_eligible),repair:q&&q.repair?q.repair:void 0})),pn=c.queue||[],fn=new Set([...pn.map(y=>y.bead_id),...(Array.isArray(c.serial_lanes)?c.serial_lanes:[]).flatMap(y=>(Array.isArray(y?.entries)?y.entries:[]).map(q=>q.bead_id)),...De.map(y=>y.bead_id),...c.done.map(y=>y.bead_id)]),Kn=new Set(S.map(y=>y.id)),Pe=i?i.get()?.order||{}:{},dt=new Set,Kr=[];for(let y of[..._,...S])fn.has(y.id)||dt.has(y.id)||hg(y)||(dt.add(y.id),Kr.push(y));I=bg(Kr,Q,Pe);let sd=c.admission||{},Da=y=>{let q=sd[y];if(!q)return"";if(q.stale===!0)return"\u267B\uFE0F stale\u2192\uC7AC\uB9AC\uBDF0";let ce=typeof q.reason=="string"?q.reason:"",Ge=ce.indexOf(":");return Ge>0&&Ge<ce.length-1?`\u26D4 ${ce.slice(0,Ge)} (${ce.slice(Ge+1)})`:`\u26D4 ${ce}`},od=I.map(y=>{let q=Fs(y),ce=q.path.length>0,Ge=y.workflow?.route==="quick_fix"||y.metadata&&y.metadata.route==="quick_fix",pt=!Object.hasOwn(y,"description")||typeof y.description=="string"&&y.description.trim().length>0,Dt=Object.hasOwn(y,"labels")&&$u(y.labels),Ct=!Dt&&(Ge?pt:ce&&!q.conflict),ut=Kn.has(y.id),Yt=[];ut&&Yt.push(vg(y)),Ge&&!pt?Yt.push("missing_description"):!Ge&&q.conflict?Yt.push("spec_id_conflict"):!Ge&&!ce&&Yt.push("spec \uC5C6\uC74C");let rs=Da(y.id);return rs&&Yt.push(rs),{id:y.id,title:y.title||y.id,reason:Yt.join(" \xB7 "),draggable:Ct,lane:"candidate",created_at:y.created_at,updated_at:y.updated_at,workflow:y.workflow,is_quick_fix:Ge,status:y.status,worker_ineligible:Dt,blocked:ut,has_spec:ce}}),io=ig(od,F),ad=io.visible,id=c.revise_parked||{},Yn=c.discard_operations&&typeof c.discard_operations=="object"&&!Array.isArray(c.discard_operations)?c.discard_operations:{},lo=(y,q)=>y.map((ce,Ge)=>{let pt=q!=="done",Dt=q!=="done"&&q!=="queue",Ct=pt?id[ce.bead_id]:null,ut=pt?cr(Yn,ce.bead_id):null,Yt=ut?.operation?ut:null,rs=pt&&L.get(ce.bead_id)===!0,ai=Fe[ce.bead_id]||[],_o=c.admission&&typeof c.admission=="object"?c.admission[ce.bead_id]:null,mo=pt?$c(_o,!!Yt||M.has(ce.bead_id)):null,vd=pt&&!mo?Da(ce.bead_id):null,wd=pt?[vd]:[],ii=pt&&ai.length>0&&typeof _o?.reason=="string"&&_o.reason.startsWith("not_ready")?[`\u23F8 ${ai.join(", ")} \uC644\uB8CC \uB300\uAE30 (blocks)`]:[],go=pt?E.get(ce.bead_id):void 0;return go&&go.length>0&&ii.push(`\u2733 serial \uAD8C\uC7A5 \xB7 ${go.join(", ")}\uC640`),{id:ce.bead_id,title:v.get(ce.bead_id)||ce.bead_id,reason:wd.filter(Boolean).join(" \xB7 "),draggable:pt&&!Yt&&!mo,done:q==="done",lane:q,seq:Dt?Ge+1:void 0,worker_serial:rs,discard:Yt,stale_work:mo,badges:[...ii,...Ct?["\u23F8 REVISE \uD30C\uD0B9"]:[]],alert:!!Ct,revise_action:!!Ct,revise_enabled:!!Ct&&!Yt&&!se.has(ce.bead_id),revise_title:Ct?Ct.notes_tail?`REVISE findings (\uC790\uC138\uD788\uB294 \uCE74\uB4DC \uD074\uB9AD \u2192 \uC774\uC288 \uC0C1\uC138):
${Ct.notes_tail}`:"notes\uC758 REVISE finding\uC744 \uC2A4\uD399\uC5D0 \uBC18\uC601\uD558\uB294 \uCC98\uBD84 \uC138\uC158\uC744 \uB744\uC6C1\uB2C8\uB2E4":"",usage:q==="done"?zt(c.attempts||{},ce.bead_id):null,work_ms:q==="done"?wc(c.attempts||{},ce.bead_id):null,done_at:q==="done"&&typeof ce.added_at=="number"?ce.added_at:void 0,...Qe(ce.bead_id)}}),Yr=c.attempts?Object.values(c.attempts):[],co=new Set;for(let y of Yr)y&&typeof y.resumed_from=="string"&&y.resumed_from.length>0&&co.add(y.resumed_from);let Na=new Map;for(let y of Yr)Na.set(y.bead_id,y.attempt_id);let Zn=new Map;for(let y of Yr)Zn.set(y.attempt_id,y);function uo(y){let q=new Set,ce=y;for(;ce&&!q.has(ce.attempt_id);){if(ce.conflict_resolution===!0)return!0;q.add(ce.attempt_id),ce=typeof ce.resumed_from=="string"&&ce.resumed_from.length>0&&Zn.get(ce.resumed_from)||null}return!1}let Xn=typeof c.declared_base=="string"?c.declared_base:null;function ld(y){let q=null;for(let ce of Yr)!ce||ce.bead_id!==y||uo(ce)||(q===null||(typeof ce.started_at=="number"?ce.started_at:0)>=(typeof q.started_at=="number"?q.started_at:0))&&(q=ce);return q&&typeof q.target_base=="string"?q.target_base:null}let qa=[],Fa=[],cd=ku(c),ja=y=>{let q=typeof y.session_id=="string"&&y.session_id.length>0,ce=co.has(y.attempt_id);return{eligible:q&&!ce,reason:q?ce?"\uC774\uBBF8 \uC774\uC5B4\uBC1B\uC740 attempt (child attempt \uC874\uC7AC) \u2014 \uC774\uC5B4\uD558\uAE30 \uBD88\uAC00":null:"session_id \uC5C6\uB294 \uAD6C attempt \u2014 \uC774\uC5B4\uD558\uAE30 \uBD88\uAC00"}},Kt=null;for(let y of Yr){let q=y.status==="paused"&&!co.has(y.attempt_id);if(y.status==="running"||q)Fa.push({bead_id:y.bead_id,attempt_id:y.attempt_id,title:v.get(y.bead_id)||y.bead_id,runner:y.runner||null,model:y.model||null,effort:y.effort||null,speed:y.speed||null,continuation_mode:y.continuation_mode||null,started_at:typeof y.started_at=="number"?y.started_at:null,resumed_from:y.resumed_from||null,paused:q,conflict_resolution:uo(y),base_exception:Ia(Xn,y.target_base),can_pause:typeof y.session_id=="string"&&y.session_id.length>0,discard:cr(Yn,y.bead_id,{attempt_id:y.attempt_id}),usage:zt(c.attempts||{},y.bead_id),current_child:me(y.bead_id),...Qe(y.bead_id)});else if((y.status==="failed"||y.status==="orphaned")&&cd(y)){let ce=ja(y);qa.push({bead_id:y.bead_id,attempt_id:y.attempt_id,title:v.get(y.bead_id)||y.bead_id,runner:y.runner||null,model:y.model||null,effort:y.effort||null,speed:y.speed||null,continuation_mode:y.continuation_mode||null,started_at:typeof y.started_at=="number"?y.started_at:null,resumed_from:y.resumed_from||null,failed:!0,status:y.status,status_label:y.status==="orphaned"?"\uC911\uB2E8\uB428":"\uC2E4\uD328",discard:cr(Yn,y.bead_id,{attempt_id:y.attempt_id}),resume_eligible:ce.eligible,resume_reason:ce.reason,conflict_resolution:uo(y),base_exception:Ia(Xn,y.target_base),usage:zt(c.attempts||{},y.bead_id),current_child:me(y.bead_id),...Qe(y.bead_id)}),Kt=y}}let Qn=[...qa,...Fa].map(y=>{let q=Zn.get(y.attempt_id),ce=q?.quickfix_landing;if(q?.quickfix_lane!==!0||!ce||typeof ce!="object")return y;let Ge=typeof ce.reason=="string"&&ce.reason.length>0?ce.reason:null,pt=Un({bead_id:q.bead_id,merge_sha:ce.head_sha,cleanup_cursor:ce.cursor,cleanup_failed:Ge?{step:ce.cursor,reason:Ge}:null,repo_operations:Array.isArray(c.repo_operations)?c.repo_operations:[]});return pt?{...y,landing:pt}:y}),Ba=null;if(Kt){let y=ja(Kt),q=Kt.cause_detail;Ba={bead_id:Kt.bead_id,repo:Kt.repo||"",reason:Kt.cause||Kt.status,cause_detail:q&&typeof q.reason=="string"?{reason:q.reason,command:typeof q.command=="string"?q.command:null}:null,resume_attempt_id:Kt.attempt_id,resume_eligible:y.eligible,resume_reason:y.reason,discard:cr(Yn,Kt.bead_id,{attempt_id:Kt.attempt_id})}}let Ua=new Set(Qn.map(y=>y.bead_id)),po=Array.isArray(c.merge_queue)?c.merge_queue:[],Wa=new Map,za=new Map,Ha=new Map,Ga=new Map,Va=new Map;po.forEach((y,q)=>{y&&typeof y.bead_id=="string"&&(Wa.set(y.bead_id,q+1),za.set(y.bead_id,y.resolution),Ha.set(y.bead_id,y.continuation_action||null),Ga.set(y.bead_id,y.head_review||null),Va.set(y.bead_id,y.authority||null))});let Zr=c.merge_queue_state||{active:null,failures:{}},ud=Zr.failures||{},Ka=Zr.waiting&&typeof Zr.waiting.bead_id=="string"&&typeof Zr.waiting.reason=="string"?Zr.waiting:null,dd=c.auto_merge_skips||{},Ya=y=>{let q=dd[y];if(!q)return null;let ce=mt[y],Ge=ce&&ce.pr?ce.pr.head_sha:null;return Ge&&Ge===q.head_sha?q.reason||"":null},Jn=new Map;for(let y of Qn)y.failed!==!0&&y.conflict_resolution&&(y.paused?Jn.has(y.bead_id)||Jn.set(y.bead_id,"paused"):Jn.set(y.bead_id,"running"));let Za=Qn.filter(y=>!y.paused&&y.failed!==!0).length,Xa=(c.workspace_info||{}).slots,Qa=typeof Xa=="number"?Xa:typeof c.slots=="number"?c.slots:oo,pd=Za>Qa,es=Nr(U),fd=(Array.isArray(c.done)?c.done.slice():[]).filter(y=>es===void 0||typeof y.added_at!="number"||y.added_at>=es).sort((y,q)=>(q.added_at||0)-(y.added_at||0)),_n=lo(fd,"done"),_d=new Set((Array.isArray(c.done)?c.done:[]).map(y=>y?.bead_id).filter(y=>typeof y=="string")),Ja=[],md=d?.()||"";for(let y of G){let q=jr(y.closed_at);if(typeof y.id!="string"||_d.has(y.id)||q===null||es!==void 0&&q<es||typeof y.comment_count!="number"||y.comment_count<=0)continue;let ce=`${md}\0${y.id}\0${String(y.updated_at)}\0${y.comment_count}`,Ge=W.get(ce);Ge===void 0&&r&&(W.set(ce,"pending"),Promise.resolve(r("get-comments",{id:y.id})).then(pt=>{let Dt=Array.isArray(pt)&&pt.some(Ct=>js(typeof Ct?.text=="string"?Ct.text:"")?.lane==="session");W.set(ce,Dt?"session":"not-session"),J()}).catch(()=>{W.set(ce,"failed"),J()})),Ge==="session"&&Ja.push({id:y.id,title:y.title||y.id,reason:"",draggable:!1,done:!0,lane:"done",selectable:!1,selected:!1,worker_serial:!1,badges:["\uC138\uC158 \uC791\uC5C5"],alert:!1,usage:null,work_ms:null,done_at:q,created_at:y.created_at,updated_at:y.updated_at})}_n.push(...Ja),_n.sort((y,q)=>(q.done_at||0)-(y.done_at||0));let ts={};for(let y of _r)ts[y]=0;let ei=!1,ti=0,fo=0,ri=0;for(let y of _n){let q=y.usage;if(q&&typeof q=="object"){let ce=!1;for(let Ge of _r)Number.isFinite(q[Ge])&&(ts[Ge]+=q[Ge],ei=!0,ce=!0);ce&&(fo+=1,Number.isFinite(q.total_cost_usd)&&(ti+=q.total_cost_usd,ri+=1))}}fo>0&&ri===fo&&(ts.total_cost_usd=ti);let ni=_n.map(y=>y.usage).filter(y=>y&&typeof y=="object"&&y.providers),gd=ni.length>0?xt(xs(ni)):ei?Qt(ts):null,bd=c.lane_states&&typeof c.lane_states=="object"&&!Array.isArray(c.lane_states)?c.lane_states:{},hd=Array.isArray(c.serial_lanes)?c.serial_lanes:[],si=y=>{if(De.some(Ge=>Ge.bead_id===y))return"PR \uB300\uAE30 \xB7 \uC810\uC720";let q=Yr.filter(Ge=>Ge&&Ge.bead_id===y),ce=q.length>0?q[q.length-1].status:null;return ce==="failed"||ce==="orphaned"?"\uC2E4\uD328 \xB7 \uC810\uC720 \uC720\uC9C0":ce==="paused"?"\uC77C\uC2DC\uC815\uC9C0 \xB7 \uC810\uC720":"\uC2E4\uD589 \uC911 \xB7 \uC810\uC720"},oi=hd.filter(y=>y&&typeof y.id=="string"&&Array.isArray(y.entries)).map((y,q)=>{let ce=bd[y.id]||{},Ge=new Map((Array.isArray(ce.corrections)?ce.corrections:[]).filter(ut=>ut&&typeof ut.bead_id=="string"&&typeof ut.after=="string").map(ut=>[ut.bead_id,ut.after])),pt=lo(y.entries.filter(ut=>!Ua.has(ut.bead_id)),y.id).map(ut=>Ge.has(ut.id)?{...ut,badges:[`\u{1F517} ${Ge.get(ut.id)} \uB4A4 (blocks \uC790\uB3D9)`,...ut.badges]}:ut),Dt=Array.isArray(ce.occupied_by)?ce.occupied_by.filter(ut=>typeof ut=="string"):[],Ct=Dt.map(ut=>({id:ut,title:v.get(ut)||ut,draggable:!1,lane:y.id,ghost:!0,badges:[si(ut)]}));return{id:y.id,index:q+1,rows:[...Ct,...pt],occupied:Dt.length>0,badge:Dt.length>0?si(Dt[0]):"\uB300\uAE30",cycle:ce.cycle===!0}}),yd=typeof c.serial_lane_count=="number"?c.serial_lane_count:oi.length;return{queue:c,idToTitle:v,candidates:ad,candidate_hidden:{blocked:io.hidden_blocked,spec:io.hidden_spec},running:Qn,live_count:Za,slots:Qa,over_cap:pd,failure:Ba,waiting:lo(pn.filter(y=>!Ua.has(y.bead_id)),"queue"),serial_lanes:oi,serial_lane_count:yd,pr_wait:De.map(y=>Eg(y.bead_id,v.get(y.bead_id)||y.bead_id,mt,Vr[y.bead_id]||null,zt(c.attempts||{},y.bead_id),sr[y.bead_id]||(C.has(y.bead_id)||H.has(y.bead_id)?{activity:null,merge_progress:{step:"merging"}}:null),Jn.get(y.bead_id)||null,y.external===!0,{position:Wa.get(y.bead_id)||0,active:Zr.active===y.bead_id,failure:ud[y.bead_id]||null,waiting:Ka?.bead_id===y.bead_id?Ka.reason:null,resolution:za.get(y.bead_id),continuation_action:Ha.get(y.bead_id),head_review:Ga.get(y.bead_id)||null,authority:Va.get(y.bead_id)||null},y.wt_present!==!1,c.auto_merge===!0?Ya(y.bead_id):null,Ia(Xn,ld(y.bead_id)),c.completion_status&&typeof c.completion_status=="object"&&!Array.isArray(c.completion_status)&&c.completion_status[y.bead_id]||null,c.discard_operations&&typeof c.discard_operations=="object"&&!Array.isArray(c.discard_operations)?c.discard_operations:{},Zn.get(Na.get(y.bead_id)||"")?.worker_serial===!0,c.auto_merge===!0,{merge_sha:y.merge_sha,cleanup_cursor:y.cleanup_cursor,repo_operations:Array.isArray(c.repo_operations)?c.repo_operations:[]})).map(y=>({...y,...Qe(y.id)})),merge_queue_length:po.length,merge_queue_running:po.length>0,auto_excluded:De.map(y=>y.bead_id).filter(y=>Ya(y)!==null),declared_base:Xn,done:_n,token_total:gd,cleanup_failures:Vn,repo_operations:Array.isArray(c.repo_operations)?c.repo_operations:[]}}function He(){let _=!!o?.get()?.job,S=!_&&o?.isPending?.()===!0,G=_?"\uBD84\uC11D \uC911":S?"\uC900\uBE44 \uC911":"";return l`<button
      type="button"
    ...
    >`,Je=l`<label class="worker-tgl worker-slots"
      ...
      ${o?He():""} `,v=Nc({failure:c.failure}),w=kc(c.repo_operations,c.cleanup_failures);return g?l`<div class="worker-ribbon">
          ${S} ${G}
        ...
        ${w}${Ue.template()}${v}`:l`<div class="worker-ctrl">
        <div class="worker-ctrl__ops">${S}${G}${Je}</div>
      ...
      ${w}${Ue.template()}${v}`}function B(c){if(c.running.length===0&&c.pr_wait.length===0)return"";let _=c.running.some(S=>!S.paused&&S.failed!==!0);return l`<section
    ...
    >
      ▶ 자동 머지${G>0?` ${G}`:""}
    </button>`}function ee(c){let _=rr({id:"worker-pane-candidate",lane:"candidate",title:"\uD6C4\uBCF4 \xB7 Board \uC5F0\uB3D9",items:c.candidates,src:!0,empty:"\uD6C4\uBCF4 \uC5C6\uC74C",header_control:be(),controls:re(c),place_menu:Y(c.candidates)});return g?l`<div class="worker-lanes worker-lanes--mobile">
        ${B(c)}
      ...
      ${rr({id:"worker-pane-running",lane:"running",title:`\uC2E4\uD589 \uC911 \xB7 \uC2AC\uB86F ${c.slots}`,items:c.running,live:c.running.some(S=>!S.paused&&S.failed!==!0),body:ya(c.running,Date.now(),Le)})}
    ...
    </div>`}function $e(c){z={...z,[c]:!z[c]},gg(z),J()}function J(){let c=ye();Ye(rt(c),ge),Ye(ee(c),_e)}function Se(){let c=document.querySelector(".app-header");if(!c)return;let _=()=>{let S=Math.round(c.getBoundingClientRect().height);ue.style.setProperty("--worker-ribbon-top",`${S}px`)};if(_(),typeof ResizeObserver=="function"){let S=new ResizeObserver(_);S.observe(c),D.push(()=>S.disconnect())}else window.addEventListener("resize",_),D.push(()=>window.removeEventListener("resize",_))}function Ce(){if(typeof window.matchMedia!="function")return;let c=window.matchMedia(_g);g=!!c.matches;let _=S=>{let G=!!(S&&typeof S.matches=="boolean"?S.matches:c.matches);G!==g&&(g=G,J())};typeof c.addEventListener=="function"?(c.addEventListener("change",_),D.push(()=>c.removeEventListener("change",_))):typeof c.addListener=="function"&&(c.addListener(_),D.push(()=>c.removeListener(_)))}let ft=null;function Et(c){ft=c.target instanceof Element?c.target:null}function nt(c){let S=c.target?.closest?.('.worker-mini[draggable="true"], .worker-card[draggable="true"]');if(!S)return;if(ft&&S.contains(ft)&&ft.closest("input, button, a")){c.preventDefault();return}let G=S.dataset.beadId||"",ie=S.dataset.lane||"";A={bead_id:G,from_lane:ie};try{c.dataTransfer?.setData("text/plain",G),c.dataTransfer&&(c.dataTransfer.effectAllowed="move")}catch{}}function yt(c){let _=c.target?.closest?.(".worker-pane");if(!_)return;let S=_.dataset.lane||"";S!=="candidate"&&S!=="queue"&&!/^s[1-5]$/.test(S)||(c.preventDefault(),c.dataTransfer&&(c.dataTransfer.dropEffect="move"),_.classList.add("worker-pane--drag-over"))}function ur(c){c.target?.closest?.(".worker-pane")?.classList.remove("worker-pane--drag-over")}function vt(c,_){let S=I.find(me=>me.id===c);if(!S)return;let G=I.filter(me=>me.id!==c),ie=G.length;if(_){let me=_.dataset.beadId;if(me===c)return;let Je=G.findIndex(v=>v.id===me);Je>=0&&(ie=Je)}let we=G.slice();we.splice(ie,0,S),R.applyReorder(c,we,ie)}function Tt(c){let _=c.target?.closest?.(".worker-pane");if(!_)return;c.preventDefault(),_.classList.remove("worker-pane--drag-over");let S=_.dataset.lane||"",G=A?.bead_id||c.dataTransfer?.getData("text/plain")||"",ie=A?.from_lane||"";if(A=null,!G)return;let we=c.target?.closest?.(".worker-mini, .worker-card"),me=Array.from(_.querySelectorAll(".worker-mini, .worker-card")),Je=me.length;if(we){let v=me.indexOf(we);v>=0&&(Je=v)}if(Je=Math.max(0,Je-_.querySelectorAll(".worker-mini--ghost").length),_.classList.contains("worker-pane--collapsed")&&(Je=Oe()),S==="candidate"){if(ie==="candidate"){vt(G,we);return}(ie==="queue"||/^s[1-5]$/.test(ie))&&Ie(G);return}if(S==="queue"||/^s[1-5]$/.test(S)){let v=S==="queue"?"parallel":S;ie===S?Ve(G,v,Je):We(G,v)}}function dr(c){F=c,ag(c),J()}function wr(c){Q=c==="board"||c==="created"||c==="spec"?c:ao,dg(Q),J()}function Bt(c){U=Wt(c)?c:Nt,fg(U),f?.(U),J()}function Gt(c){let _=c.target?.closest?.(".worker-serial-lane-count");if(_){let Je=Number.parseInt(_.value,10);Number.isFinite(Je)&&Te(Je).then(J);return}let S=c.target?.closest?.(".worker-filter__blocked");if(S){dr({...F,show_blocked:S.checked});return}let G=c.target?.closest?.(".worker-done-range");if(G){Bt(G.value);return}let ie=c.target?.closest?.(".worker-sort");if(ie){wr(ie.value||ao);return}let we=c.target?.closest?.(".worker-slots__input");if(!we)return;let me=Number.parseInt(we.value,10);if(!Number.isFinite(me)){J();return}ke(me).then(J)}function wt(c){return c?{runner:c.runner||void 0,model:c.model||void 0,effort:c.effort||void 0,worktree:c.worktree||void 0,status:c.status||void 0,session_id:c.session_id||void 0}:{}}function nr(){let c=ye();return{operations:c.repo_operations,cleanup_failures:c.cleanup_failures,repo:d&&d()||""}}function st(){Le&&xe.close(),je.hidden=!1,pe.hidden=!1,Re.open(nr()),J()}function Mt(c){let _=Ee(),S=_.attempts?_.attempts[c]:null;Le=c,he=null,Re.close(),je.hidden=!0,pe.hidden=!1,xe.open({attempt_id:c,meta:wt(S)}),J()}function kr(c,_){Le=null,he=c,Re.close(),je.hidden=!0,pe.hidden=!1,xe.open({attempt_id:c,meta:_,hide_prompt:!0}),J()}function Vt(){if(Re.isOpen()&&Re.refresh(nr()),he){let S=(o?.get()?.runs||[]).find(G=>G.run_id===he);S?xe.updateMeta(Ca(S)):xe.close();return}if(!Le)return;let c=Ee(),_=c.attempts?c.attempts[Le]:null;if(_){xe.updateMeta(wt(_));return}xe.close()}function ve(c){let _=c.target;if(_?.closest?.(".worker-mini__serial, .worker-mini__grip")||_?.closest?.("#worker-parallel-analysis-dialog"))return;if(_?.closest?.(".worker-analysis-btn")){Ze?.open();return}if(_?.closest?.(".worker-repo-strip")||_?.closest?.(".worker-mini__timeline")){st();return}let S=_?.closest?.(".worker-repo-op__session");if(S){let Pe=S.dataset.attemptId;Pe&&Mt(Pe);return}let G=_?.closest?.(".worker-repo-op__resolve");if(G){de(G.dataset.operationId||"");return}let ie=_?.closest?.(".worker-repo-op__dismiss");if(ie){Ne(ie.dataset.operationId||"");return}let we=_?.closest?.(".worker-cleanup__resume");if(we){let Pe=we.dataset.beadId;Pe&&ze(Pe);return}let me=_?.closest?.(".worker-banner__resume");if(me){let Pe=me.dataset.attemptId;Pe&&Xe(Pe);return}let Je=_?.closest?.(".worker-banner__discard");if(Je){let Pe=Je.dataset.confirmation==="merged"?"merged":"unmerged";P(Je.dataset.beadId||"",Je.dataset.attemptId||null,Pe,Je.dataset.operationId||null);return}let v=_?.closest?.(".worker-banner__dismiss");if(v){let Pe=v.dataset.attemptId;Pe&&V(Pe);return}if(_?.closest?.(".worker-play")){ae(!Ee().auto_advance);return}let w=_?.closest?.(".worker-merge-all");if(w){w.classList.contains("worker-merge-all--stop")?Ee().auto_merge===!0?b(!1):$():b(!0);return}let m=_?.closest?.(".worker-pane__hd--toggle");if(m){let Pe=m.dataset.lane;(Pe==="queue"||Pe==="done")&&$e(Pe);return}let L=_?.closest?.(".worker-card__place-lane");if(L){let Pe=L.dataset.beadId,dt=L.dataset.lane;Pe&&(dt==="parallel"||/^s[1-5]$/.test(dt||""))&&(Z=null,J(),We(Pe,dt));return}if(_?.closest?.(".worker-card__place-cancel")){Z=null,J();return}let ne=_?.closest?.(".worker-card__place");if(ne){let Pe=ne.dataset.beadId;Pe&&!ne.disabled&&(ot()?(Z=Pe,J()):We(Pe,"parallel"));return}let Fe=_?.closest?.(".worker-filter__chip");if(Fe){let Pe=Fe.dataset.spec;(Pe==="all"||Pe==="with"||Pe==="without")&&dr({...F,spec:Pe});return}let Ke=_?.closest?.(".worker-mini__merge");if(Ke){let Pe=Ke.dataset.beadId||"";Ee().cleanup_failed?.[Pe]?ze(Pe):Me(Pe);return}let Qe=_?.closest?.(".worker-mini__merge-cancel");if(Qe){x(Qe.dataset.beadId||"");return}let De=_?.closest?.(".worker-mini__discard");if(De){P(De.dataset.beadId||"",De.dataset.attemptId||null,De.dataset.discardMode==="merged"?"merged":"unmerged",De.dataset.operationId||null);return}let mt=_?.closest?.(".worker-mini__stale-continue");if(mt){K("worker-stale-work-continue",mt.dataset.beadId||"",mt.dataset.actionId||"");return}let sr=_?.closest?.(".worker-mini__stale-backup");if(sr){K("worker-stale-work-backup-fresh",sr.dataset.beadId||"",sr.dataset.actionId||"");return}let Vr=_?.closest?.(".worker-mini__stale-recheck");if(Vr){K("worker-stale-work-recheck",Vr.dataset.beadId||"",Vr.dataset.actionId||"");return}let Vn=_?.closest?.(".worker-mini__revise-fix");if(Vn){X("worker-revise-fix",Vn.dataset.beadId||"");return}let pn=_?.closest?.(".worker-mini__revise-approve");if(pn){X("worker-revise-approve",pn.dataset.beadId||"");return}if(_?.closest?.(".worker-mini__pr"))return;if(_?.closest?.(".rtile__discard")){let Pe=_?.closest?.(".rtile"),dt=Pe?.dataset?.beadId,Kr=Pe?.dataset?.attemptId;dt&&P(dt,Kr||null,"unmerged",_?.closest?.(".rtile__discard")?.dataset.operationId||null);return}if(_?.closest?.(".rtile__dismiss")){let dt=_?.closest?.(".rtile")?.dataset?.attemptId;dt&&V(dt);return}if(_?.closest?.(".rtile__pause")){let dt=_?.closest?.(".rtile")?.dataset?.attemptId;dt&&lt(dt);return}if(_?.closest?.(".rtile__resume")){let dt=_?.closest?.(".rtile")?.dataset?.attemptId;dt&&Xe(dt);return}if(_?.closest?.(".rtile__session")){let dt=_?.closest?.(".rtile")?.dataset?.attemptId;dt&&Mt(dt);return}if(_?.closest?.(".worker-drawer-overlay__backdrop")){Re.close(),xe.close();return}if(_?.closest?.(".worker-drawer-host"))return;let fn=_?.closest?.(".rtile");if(fn){if(_?.closest?.(".rtile__id")){let dt=fn.dataset.beadId;dt&&Xt(dt).then(Kr=>{Kr?le("\uBCF5\uC0AC\uB428","success",1200):le("\uBCF5\uC0AC \uC2E4\uD328","error",1600)});return}let Pe=fn.dataset.beadId;Pe&&u&&u(Pe);return}let Kn=_?.closest?.(".worker-mini, .worker-card");if(Kn){let Pe=Kn.dataset.beadId;if(_?.closest?.(".worker-mini__id, .worker-card__id")){Pe&&Xt(Pe).then(dt=>{dt?le("\uBCF5\uC0AC\uB428","success",1200):le("\uBCF5\uC0AC \uC2E4\uD328","error",1600)});return}Pe&&u&&u(Pe)}}return e.addEventListener("pointerdown",Et),e.addEventListener("dragstart",nt),e.addEventListener("dragover",yt),e.addEventListener("dragleave",ur),e.addEventListener("drop",Tt),e.addEventListener("click",ve),e.addEventListener("change",Gt),Ce(),Se(),h&&D.push(h.subscribe(()=>{for(let[c,_]of W)_==="failed"&&W.delete(c);J()})),s&&D.push(s.subscribe(()=>{let c=d&&d()||"";c!==Ae&&(Ae=c,Be.close()),J(),Vt()})),o&&typeof o.subscribe=="function"&&D.push(o.subscribe(()=>{Vt(),J()})),J(),{load(){J()},destroy(){for(let c of D.splice(0))try{c()}catch{}e.removeEventListener("pointerdown",Et),e.removeEventListener("dragstart",nt),e.removeEventListener("dragover",yt),e.removeEventListener("dragleave",ur),e.removeEventListener("drop",Tt),e.removeEventListener("click",ve),e.removeEventListener("change",Gt);try{xe.destroy()}catch{}pe.hidden=!0;try{Ze?.destroy()}catch{}try{Be.destroy()}catch{}Ye(l``,e)}}}function Oa(e){if(!e)return"Unknown";let t=e.split("/").filter(Boolean);return t.length>0?t[t.length-1]:"Unknown"}function Gu(e,t,r,n=async()=>{},s=async()=>{}){let o=_t("views:workspace-picker"),a=null,i=!1,u=!1,d=!1;async function p(z){let C=z.target.value,se=t.getState().workspace?.current?.path||"";if(C&&C!==se){o("switching workspace to %s",C),i=!0,O();try{await r(C)}catch(M){o("workspace switch failed: %o",M)}finally{i=!1,O()}}}async function f(){let z=t.getState(),g=z.workspace?.current?.path||z.workspace?.available?.[0]?.path||"";if(!(!g||u)){o("git-pulling workspace %s",g),u=!0,O();try{await n(g)}catch(C){o("workspace git pull failed: %o",C)}finally{u=!1,O()}}}function h(z){let g=z.target;g&&e.contains(g)||I()}function R(z){z.key==="Escape"&&I()}function A(){d||(d=!0,document.addEventListener("mousedown",h),document.addEventListener("keydown",R),O())}function I(){d&&(d=!1,document.removeEventListener("mousedown",h),document.removeEventListener("keydown",R),O())}function F(){d?I():A()}async function Z(z){let g=z.target,C=g.value,H=g.checked;o("toggling visibility %s \u2192 %s",C,String(H));try{await s(C,H)}catch(se){o("workspace visibility toggle failed: %o",se)}}function Q(z){return z?l`
      <button
        type="button"
    ...
    `}function W(){let z=t.getState(),g=z.workspace?.current,C=z.workspace?.available||[],H=new Set(z.workspace?.hidden||[]),se=g?.path||C[0]?.path||"";if(C.length===0)return l``;let M=C.filter(D=>!H.has(D.path)||D.path===se);if(M.length<=1){let D=M[0]||C[0],ue=Oa(D.path);return l`
    ...
    `}function O(){Ye(W(),e)}return O(),a=t.subscribe(()=>O()),{destroy(){a&&(a(),a=null),document.removeEventListener("mousedown",h),document.removeEventListener("keydown",R),Ye(l``,e)}}}var Vu=["update-status","edit-text","update-priority","create-issue","dep-add","dep-remove","update-assignee","update-exec-settings","update-impl-target","get-session-defaults","set-session-defaults","update-workflow-meta","label-add","label-remove","subscribe-list","unsubscribe-list","snapshot","upsert","delete","get-comments","add-comment","delete-issue","list-workspaces","set-workspace","set-workspace-visibility","get-workspace","workspace-changed","git-pull-workspace","subscribe-worker-queue","unsubscribe-worker-queue","worker-queue-snapshot","worker-queue-place","worker-queue-reorder","worker-queue-toggle","worker-automation-toggle","worker-auto-repair-toggle","worker-repo-ops-opt-out-toggle","worker-repo-operation-repair","worker-repo-operation-dismiss","worker-queue-set-slots","worker-queue-set-serial-lane-count","subscribe-worker-parallel-analysis","unsubscribe-worker-parallel-analysis","worker-parallel-analysis-snapshot","worker-parallel-analysis-targets","worker-parallel-analysis-prompt","worker-parallel-analysis-start","worker-parallel-analysis-cancel","worker-parallel-analysis-settings-update","worker-parallel-analysis-submit","worker-queue-set-orchestration-defaults","worker-queue-remove","worker-attempt-pause","worker-attempt-stop","worker-attempt-resume","worker-attempt-dismiss","worker-cleanup-retry","worker-merge-queue-add","worker-merge-queue-add-all","worker-merge-auto-toggle","worker-merge-queue-remove","worker-discard","worker-stale-work-continue","worker-stale-work-backup-fresh","worker-stale-work-recheck","worker-pr-discard","worker-revise-fix","worker-revise-approve","subscribe-ui-order","unsubscribe-ui-order","ui-order-set","ui-order-snapshot","subscribe-display-policy","unsubscribe-display-policy","display-policy-set","display-policy-snapshot","subscribe-session-log","unsubscribe-session-log","session-log-snapshot","session-log-append","get-attempt-prompt","get-bead-prompt","get-worker-system-prompt","subscribe-monitor-pipeline","unsubscribe-monitor-pipeline","monitor-pipeline-snapshot","subscribe-impl-presets","unsubscribe-impl-presets","impl-presets-snapshot","impl-preset-create","impl-preset-update","impl-preset-delete","apply-impl-preset","apply-impl-preset-global","monitor-auto-toggle"];function Ma(){let e=Date.now().toString(36),t=Math.random().toString(36).slice(2,8);return`${e}-${t}`}function Ku(e,t,r=Ma()){return{id:r,type:e,payload:t}}function Yu(e={}){let t=_t("ws"),r={initialMs:e.backoff?.initialMs??1e3,maxMs:e.backoff?.maxMs??3e4,factor:e.backoff?.factor??2,jitterRatio:e.backoff?.jitterRatio??.2},n=()=>e.url&&e.url.length>0?e.url:typeof location<"u"?(location.protocol==="https:"?"wss://":"ws://")+location.host+"/ws":"ws://localhost/ws",s=null,o="closed",a=0,i=null,u=!0,d=new Map,p=[],f=new Map,h=new Set;function R(W){for(let O of Array.from(h))try{O(W)}catch{}}function A(){if(!u||i)return;o="reconnecting",t("ws reconnecting\u2026"),R(o);let W=Math.min(r.maxMs||0,(r.initialMs||0)*Math.pow(r.factor||1,a)),O=(r.jitterRatio||0)*W,z=Math.max(0,Math.round(W+(Math.random()*2-1)*O));t("ws retry in %d ms (attempt %d)",z,a+1),i=setTimeout(()=>{i=null,U()},z)}function I(W){try{s?.send(JSON.stringify(W))}catch(O){t("ws send failed",O)}}function F(){for(o="open",t("ws open"),R(o),a=0;p.length;){let W=p.shift();W&&I(W)}}function Z(W){let O;try{O=JSON.parse(String(W.data))}catch{t("ws received non-JSON message");return}if(!O||typeof O.id!="string"||typeof O.type!="string"){t("ws received invalid envelope");return}if(d.has(O.id)){let g=d.get(O.id);d.delete(O.id),O.ok?g?.resolve(O.payload):g?.reject(O.error||new Error("ws error"));return}let z=f.get(O.type);if(z&&z.size>0)for(let g of Array.from(z))try{g(O.payload)}catch(C){t("ws event handler error",C)}else t("ws received unhandled message type: %s",O.type)}function Q(){o="closed",t("ws closed"),R(o);for(let[W,O]of d.entries())O.reject(new Error("ws disconnected")),d.delete(W);a+=1,A()}function U(){if(!u)return;let W=n();try{s=new WebSocket(W),t("ws connecting %s",W),o="connecting",R(o),s.addEventListener("open",F),s.addEventListener("message",Z),s.addEventListener("error",()=>{}),s.addEventListener("close",Q)}catch(O){t("ws connect failed %o",O),A()}}return U(),{send(W,O){if(!Vu.includes(W))return Promise.reject(new Error(`unknown message type: ${W}`));let z=Ma(),g=Ku(W,O,z);return t("send %s id=%s",W,z),new Promise((C,H)=>{d.set(z,{resolve:C,reject:H,type:W}),s&&s.readyState===s.OPEN?I(g):(t("queue %s id=%s (state=%s)",W,z,o),p.push(g))})},on(W,O){f.has(W)||f.set(W,new Set);let z=f.get(W);return z?.add(O),()=>{z?.delete(O)}},onConnection(W){return h.add(W),()=>{h.delete(W)}},reconnect(){u=!0,i&&(clearTimeout(i),i=null),a=0,U()},close(){u=!1,i&&(clearTimeout(i),i=null);try{s?.close()}catch{}},getState(){return o}}}function Tg(){let e=window.__BDUI_BOOTSTRAP__;return{workspace_config:{default_workspace:typeof e?.workspace_config?.default_workspace=="string"&&e.workspace_config.default_workspace.length>0?e.workspace_config.default_workspace:null}}}async function Cg(e,t){try{let n=await(await fetch("/api/config")).json();e.setState({config:n})}catch(r){t("config refresh failed",r)}}var Pa=[["tab:board:ready","ready-issues"],["tab:board:blocked","blocked-issues"],["tab:board:in-progress","in-progress-issues"],["tab:board:resolved","resolved-issues"],["tab:board:deferred","deferred-issues"],["tab:board:closed","closed-issues"]],Zu=[["tab:worker:ready","ready-issues"],["tab:worker:blocked","blocked-issues"],["tab:worker:in-progress","in-progress-issues"],["tab:worker:closed","closed-issues"]],Rr="tab:worker:closed",Rg="bdui.worker.done-range",Xu=ru,Qu="worker:queue",Ju="worker:parallel-analysis",ed="ui:order",td="ui:display-policy",rd="exec:presets",Ir="tab:board:closed",nd="beads-ui.board.closed-range";function Ig(e){let t=_t("main");t("bootstrap start");let r=l`
    <section id="board-root" class="route board"></section>
    <section id="worker-root" class="route worker" hidden></section>
    <section id="monitor-root" class="route monitor" hidden></section>
    <section id="detail-panel" class="route detail" hidden></section>
  `;Ye(r,e);let n=document.getElementById("top-nav"),s=document.getElementById("usage-meter"),o=document.getElementById("board-root"),a=document.getElementById("worker-root"),i=document.getElementById("monitor-root"),u=document.getElementById("detail-panel");if(s&&wu(s),o&&a&&i&&u){let je=function(v,w){let m="Request failed",L="";if(v&&typeof v=="object"){let ne=v;if(typeof ne.message=="string"&&ne.message.length>0&&(m=ne.message),typeof ne.details=="string")L=ne.details;else if(ne.details&&typeof ne.details=="object")try{L=JSON.stringify(ne.details,null,2)}catch{L=""}}else typeof v=="string"&&v.length>0&&(m=v);let E=w&&w.length>0?`Failed to load ${w}`:"Request failed";tt.open(E,m,L)},Xe=function(v){return`${st.getState().workspace.current?.path||""}\0${v}`},V=function(){Y&&(Y().catch(()=>{}),Y=null),j=null,oe=null},Me=function(v){Oe=v;let w=()=>{Oe!==v||st.getState().selected_id!==v||(Oe=null,te(v))};if(!Ie){Ve.then(w);return}w()},x=function(v,w,m,L,E){return m!==b[w]?(E().catch(()=>{}),!1):(v.set(L,E),!0)},P=function(){let v=st.getState();Ne(v.view==="board"),B(v.view==="worker"),N(v.view==="monitor"),be(v.view==="board"||v.view==="worker"||$||!!v.selected_id)},ae=function(){let v=Nr(K);return v===void 0?{type:"closed-issues"}:{type:"closed-issues",params:{since:v}}},de=function(){let v=Nr(X);return v===void 0?{type:"closed-issues"}:{type:"closed-issues",params:{since:v}}},Ne=function(v){if(v)for(let[w,m]of Pa){if(ze.has(w)||fe.has(w))continue;let L=w===Ir?ae():{type:m};try{xe.register(w,L)}catch(Fe){t("register %s store failed: %o",w,Fe)}fe.add(w);let E=b.board,ne=!1;he.subscribeList(w,L).then(Fe=>{ne=!x(ze,"board",E,w,Fe)}).catch(Fe=>{t("subscribe %s failed: %o",w,Fe),je(Fe,"board")}).finally(()=>{fe.delete(w),ne&&P()})}else ye()},ye=function(){b.board+=1;for(let[v]of Pa){let w=ze.get(v);w&&(w().catch(()=>{}),ze.delete(v));try{xe.unregister(v)}catch(m){t("unregister %s failed: %o",v,m)}}},B=function(v){if(!v){re();return}for(let[w,m]of Zu){if(He.has(w)||fe.has(w))continue;let L=w===Rr?de():{type:m};try{xe.register(w,L)}catch(Fe){t("register %s store failed: %o",w,Fe)}fe.add(w);let E=b.worker,ne=!1;he.subscribeList(w,L).then(Fe=>{ne=!x(He,"worker",E,w,Fe)}).catch(Fe=>{t("subscribe %s failed: %o",w,Fe),je(Fe,"worker")}).finally(()=>{fe.delete(w),ne&&P()})}},re=function(){b.worker+=1;for(let[v]of Zu){let w=He.get(v);w&&(w().catch(()=>{}),He.delete(v));try{xe.unregister(v)}catch(m){t("unregister %s failed: %o",v,m)}}},be=function(v){if(!v){k();return}rt||(Le("subscribe-worker-queue",{id:Qu}).catch(w=>{t("subscribe-worker-queue failed: %o",w)}),Le("subscribe-worker-parallel-analysis",{id:Ju}).catch(w=>{t("subscribe-worker-parallel-analysis failed: %o",w)}),rt=()=>(Le("unsubscribe-worker-parallel-analysis",{id:Ju}),Le("unsubscribe-worker-queue",{id:Qu})))},k=function(){rt&&(rt().catch(()=>{}),rt=null),Be.clear()},N=function(v){if(!v){ee();return}T||(Le("subscribe-monitor-pipeline",{id:Xu}).catch(w=>{t("subscribe-monitor-pipeline failed: %o",w)}),T=()=>Le("unsubscribe-monitor-pipeline",{id:Xu}))},ee=function(){T&&(T().catch(()=>{}),T=null)},J=function(){$e||(Le("subscribe-ui-order",{id:ed}).catch(v=>{t("subscribe-ui-order failed: %o",v)}),$e=()=>Le("unsubscribe-ui-order",{id:ed}))},Se=function(){$e&&($e().catch(()=>{}),$e=null),Ue.clear()},ft=function(){Ce||(Le("subscribe-display-policy",{id:td}).catch(v=>{t("subscribe-display-policy failed: %o",v)}),Ce=()=>Le("unsubscribe-display-policy",{id:td}))},Et=function(){Ce&&(Ce().catch(()=>{}),Ce=null),Ze.clear()},yt=function(){nt||(Le("subscribe-impl-presets",{id:rd}).catch(v=>{t("subscribe-impl-presets failed: %o",v)}),nt=()=>Le("unsubscribe-impl-presets",{id:rd}))},Bt=function(v){if(!v)return"Unknown";let w=v.split("/").filter(Boolean);return w.length>0?w[w.length-1]:"Unknown"};var d=je,p=Xe,f=V,h=Me,R=x,A=P,I=ae,F=de,Z=Ne,Q=ye,U=B,W=re,O=be,z=k,g=N,C=ee,H=J,se=Se,M=ft,D=Et,ue=yt,ge=Bt;let pe=document.getElementById("header-loading"),qe=zi(pe),tt=vc(e),_e=Yu(),Le=qe.wrapSend((v,w)=>_e.send(v,w)),he=Di(Le),xe=Ni(),Re=ji(),Be=Fi(),Ae=wi(),Ue=qi(),Ze=yi(),Ee=vi(),ot=ki();_e.on("impl-presets-snapshot",v=>{let w=v;w&&typeof w.revision=="number"&&Array.isArray(w.presets)&&Ee.set({revision:w.revision,presets:w.presets})}),_e.on("monitor-pipeline-snapshot",v=>{let w=v;if(!(!w||!Array.isArray(w.workspaces)))try{Ae.set(w.workspaces,w.workspaces_state)}catch{}}),_e.on("ui-order-snapshot",v=>{let w=v;if(w&&typeof w.revision=="number")try{Ue.set({revision:w.revision,order:w.order&&typeof w.order=="object"?w.order:{}})}catch{}}),_e.on("display-policy-snapshot",v=>{let w=v;if(w&&w.policy&&typeof w.policy=="object")try{Ze.set(w.policy)}catch{}}),_e.on("session-log-snapshot",v=>{let w=v;if(w&&typeof w.id=="string")try{ot.set(w.id,Array.isArray(w.lines)?w.lines:[],typeof w.last_event_at=="number"?w.last_event_at:null)}catch{}}),_e.on("session-log-append",v=>{let w=v;if(w&&typeof w.id=="string")try{ot.append(w.id,w.event)}catch{}}),_e.on("snapshot",v=>{let w=v,m=w&&typeof w.id=="string"?w.id:"",L=m?xe.getStore(m):null;if(L&&w&&w.type==="snapshot")try{L.applyPush(w)}catch{}}),_e.on("upsert",v=>{let w=v,m=w&&typeof w.id=="string"?w.id:"",L=m?xe.getStore(m):null;if(L&&w&&w.type==="upsert")try{L.applyPush(w)}catch{}}),_e.on("delete",v=>{let w=v,m=w&&typeof w.id=="string"?w.id:"",L=m?xe.getStore(m):null;if(L&&w&&w.type==="delete")try{L.applyPush(w)}catch{}});let Y=null,j=null,oe=null,Oe=null,We=()=>{},Ve=new Promise(v=>{We=()=>v(void 0)}),Ie=!1,lt=!1;async function te(v){let w=Xe(v);if(w===j||w===oe)return;oe=w;let m=`detail:${v}`,L={type:"issue-detail",params:{id:v}};try{xe.register(m,L)}catch(E){t("register detail store failed: %o",E)}try{let E=await he.subscribeList(m,L);if(st.getState().selected_id!==v||Xe(v)!==w){await E().catch(()=>{});return}Y&&await Y().catch(()=>{}),Y=E,j=w}catch(E){t("detail subscribe failed: %o",E),je(E,"issue details")}finally{oe===w&&(oe=null)}}let ze=new Map,fe=new Set,b={board:0,worker:0},$=!1,K=Nt;try{let v=window.localStorage.getItem(nd);Wt(v)&&(K=v)}catch{}let X=Nt;try{let v=window.localStorage.getItem(Rg);Wt(v)&&(X=v)}catch{}async function ke(v){if(!Wt(v)||v===K)return;K=v;try{window.localStorage.setItem(nd,v)}catch{}let w=ze.get(Ir);if(!w)return;ze.delete(Ir),await w().catch(()=>{});let m=ae();try{xe.register(Ir,m)}catch(L){t("register %s store failed: %o",Ir,L)}try{let L=await he.subscribeList(Ir,m);ze.set(Ir,L)}catch(L){t("re-subscribe %s failed: %o",Ir,L),je(L,"board")}}async function Te(v){if(!Wt(v)||v===X)return;X=v;let w=He.get(Rr);if(!w)return;He.delete(Rr),await w().catch(()=>{});let m=de();try{xe.register(Rr,m)}catch(L){t("register %s store failed: %o",Rr,L)}try{let L=await he.subscribeList(Rr,m);He.set(Rr,L)}catch(L){t("re-subscribe %s failed: %o",Rr,L),je(L,"worker")}}let He=new Map,rt=null,T=null,$e=null,Ce=null,nt=null;async function ur(){Ce=null,Ze.clear(),nt=null,Ee.clear(),rt=null,T=null,ze.clear(),He.clear(),b.board+=1,b.worker+=1,yt();let v=st.getState().workspace.current?.path;if(v)try{await _e.send("set-workspace",{path:v})}catch(m){t("workspace restore after reconnect failed: %o",m);return}ft();let w=st.getState();Ne(w.view==="board"),B(w.view==="worker"),N(w.view==="monitor"),be(w.view==="board"||w.view==="worker"||!!w.selected_id)}async function vt(){t("clearing all subscriptions for workspace switch"),ye(),re(),k(),Re.clear(),Se(),J(),Et(),ft(),V();let v=st.getState();if(v.selected_id)try{xe.unregister(`detail:${v.selected_id}`)}catch{}let w=st.getState();Ne(w.view==="board"),B(w.view==="worker"),N(w.view==="monitor"),be(w.view==="board"||w.view==="worker"||!!w.selected_id),w.selected_id&&Me(w.selected_id)}async function Tt(v){t("requesting workspace switch to %s",v),lt=!0;try{let w=await _e.send("set-workspace",{path:v});t("workspace switch result: %o",w),w&&w.workspace&&(st.setState({workspace:{current:{path:w.workspace.root_dir,database:w.workspace.db_path}}}),window.localStorage.setItem("beads-ui.workspace",v),w.changed&&(await vt(),le("Switched to "+Bt(v),"success",2e3)))}catch(w){throw t("workspace switch failed: %o",w),le("Failed to switch workspace","error",3e3),w}finally{lt=!1}}async function dr(v){t("requesting workspace git pull for %s",v);try{let w=await _e.send("git-pull-workspace",{});t("workspace git pull result: %o",w);let m=w?.status;if(m==="up_to_date"){le("Already up to date","success",2e3);return}if(m==="stash_pop_conflict"){le("Git pulled, but stash pop conflicted (check git stash list)","warning",4e3);return}le("Git pulled "+Bt(v),"success",2e3)}catch(w){t("workspace git pull failed: %o",w);let m=w?.code,L=w?.message;if(m==="rebase_conflict"){le("Git pull conflicts \u2014 reverted (manual resolve required)","error",4e3);return}if(m==="rebase_conflict_abort_failed"){le("Git pull conflicts AND rebase --abort failed \u2014 repo left mid-rebase, run 'git rebase --abort' manually","error",6e3);return}if(m==="busy"){le("Git pull skipped: another operation is running","warning",3e3);return}let E=L?`: ${L}`:"";throw le(`Git pull failed${E}`,"error",3e3),w}}async function wr(v,w){t("setting workspace visibility %s \u2192 %s",v,String(w));try{await _e.send("set-workspace-visibility",{path:v,visible:w}),await Gt()}catch(m){t("workspace visibility update failed: %o",m),le("Failed to update project visibility","error",3e3)}}async function Gt(){try{let v=await _e.send("list-workspaces",{});if(t("workspaces loaded: %o",v),v&&Array.isArray(v.workspaces)){let w=v.workspaces.map(ne=>({path:ne.path,database:ne.database,pid:ne.pid,version:ne.version})),m=v.current?{path:v.current.root_dir,database:v.current.db_path}:null,L=Array.isArray(v.hidden)?v.hidden.filter(ne=>typeof ne=="string"):[];st.setState({workspace:{current:m,available:w,hidden:L}});let E=window.localStorage.getItem("beads-ui.workspace");E&&(!w.some(Fe=>Fe.path===E)||L.includes(E)?window.localStorage.removeItem("beads-ui.workspace"):m&&E!==m.path&&(t("restoring saved workspace preference: %s",E),await Tt(E)))}}catch(v){t("failed to load workspaces: %o",v)}}_e.on("workspace-changed",v=>{t("workspace-changed event: %o",v),v&&v.root_dir&&(st.setState({workspace:{current:{path:v.root_dir,database:v.db_path}}}),Gt(),vt())});let wt=!1;if(typeof _e.onConnection=="function"){let v=w=>{t("ws state %s",w),w==="reconnecting"||w==="closed"?(wt=!0,le("Connection lost. Reconnecting\u2026","error",4e3)):w==="open"&&wt&&(wt=!1,le("Reconnected","success",2200),Cg(st,(m,L)=>{t(`${m}: %o`,L)}),ur())};_e.onConnection(v)}let nr="board";try{let v=window.localStorage.getItem("beads-ui.view");(v==="board"||v==="worker"||v==="monitor")&&(nr=v)}catch(v){t("view parse error: %o",v)}let st=Wi({config:Tg(),view:nr});_e.on("worker-queue-snapshot",v=>{let w=v;if(!w||!w.queue)return;let m=st.getState().workspace.current?.path;if(typeof m=="string"&&m.length>0&&w.root_dir!==m){t("dropping worker-queue snapshot for %s",String(w.root_dir));return}try{Re.set(w.queue)}catch{}}),_e.on("worker-parallel-analysis-snapshot",v=>{let w=v;if(!w)return;let m=st.getState().workspace.current?.path;if(!(typeof m=="string"&&m.length>0&&typeof w.root_dir=="string"&&w.root_dir!==m))try{Be.set({settings:w.settings,job:w.job??null,runs:Array.isArray(w.runs)?w.runs:[],last_good:w.last_good??null})}catch{}});let Mt=Bi(st);Mt.start();let kr=new Set(["get-comments","dep-add","dep-remove","impl-preset-create","impl-preset-update","impl-preset-delete","apply-impl-preset","apply-impl-preset-global","get-session-defaults","set-session-defaults"]),Vt=async(v,w)=>{try{return await Le(v,w)}catch(m){if(kr.has(v))throw m;return[]}};n&&su(n,st,Mt);let ve=document.getElementById("workspace-picker");ve&&Gu(ve,st,Tt,dr,wr);let c=lu(e,(v,w)=>Le(v,w));try{let v=document.getElementById("new-issue-btn");v&&v.addEventListener("click",()=>c.open())}catch{}let _=pu(e,{policyStore:Ze,queueStore:Re,implPresetStore:Ee,transport:(v,w)=>Le(v,w),onOpenChange:v=>{$=v,P()},labelOptions:()=>{let v=new Set;for(let[w]of Pa)for(let m of xe.snapshotFor(w)||[]){let L=m.labels;if(Array.isArray(L))for(let E of L)typeof E=="string"&&E.length>0&&v.add(E)}return Array.from(v).sort()}});try{let v=document.getElementById("display-settings-btn");v&&(v.setAttribute("aria-label","\uC124\uC815"),v.setAttribute("title","\uC124\uC815"),v.addEventListener("click",()=>_.open()))}catch{}let S=tl(o,{gotoIssue:v=>Mt.gotoIssue(v),issueStores:xe,transport:Vt,workerQueueStore:Re,uiOrderStore:Ue,displayPolicyStore:Ze,closedRange:K,onClosedRangeChange:v=>{ke(v)},onNewIssue:()=>c.open()}),G=La(a,{transport:Vt,issueStores:xe,queueStore:Re,analysisStore:Be,sessionLogStore:ot,uiOrderStore:Ue,gotoIssue:v=>st.setState({selected_id:v}),getWorkspacePath:()=>st.getState().workspace.current?.path,doneRange:X,onDoneRangeChange:v=>{Te(v)}}),ie=nu(i,{transport:Vt,pipelineStore:Ae,execPresetStore:Ee,gotoIssue:v=>Mt.gotoIssue(v),getWorkspacePath:()=>st.getState().workspace.current?.path,switchWorkspace:v=>Tt(v)}),we=yc(u,{issueStores:xe,transport:Vt,queueStore:Re,execPresetStore:Ee,sessionLogStore:ot,getWorkspacePath:()=>st.getState().workspace.current?.path,onNavigate:v=>{st.getState().view==="worker"?st.setState({selected_id:v}):Mt.gotoIssue(v)},onClose:()=>{let v=st.getState();st.setState({selected_id:null});try{Mt.gotoView(v.view==="worker"||v.view==="monitor"?v.view:"board")}catch{}},onOpenExecPresets:()=>{_.open("execution")}}),me=st.getState().selected_id;me&&(u.hidden=!1,we.load(me),Me(me)),st.subscribe(v=>{let w=v.selected_id;w?(u.hidden=!1,we.load(w),lt||Me(w)):(we.clear(),u.hidden=!0,V())});let Je=v=>{o.hidden=v.view!=="board",a.hidden=v.view!=="worker",i.hidden=v.view!=="monitor",Ne(v.view==="board"),B(v.view==="worker"),N(v.view==="monitor"),be(v.view==="board"||v.view==="worker"||$||!!v.selected_id),!v.selected_id&&v.view==="board"&&S.load(),v.view==="worker"&&G.load(),v.view==="monitor"?ie.load():ie.pause(),window.localStorage.setItem("beads-ui.view",v.view)};st.subscribe(Je),Je(st.getState()),J(),ft(),yt(),Gt().finally(()=>{Ie=!0,We()}),window.addEventListener("keydown",v=>{let w=v.ctrlKey||v.metaKey,m=String(v.key||"").toLowerCase(),L=v.target,E=L&&L.tagName?String(L.tagName).toLowerCase():"",ne=E==="input"||E==="textarea"||E==="select"||L&&typeof L.isContentEditable=="boolean"&&L.isContentEditable;w&&m==="n"&&(ne||(v.preventDefault(),c.open()))})}}typeof window<"u"&&typeof document<"u"&&window.addEventListener("DOMContentLoaded",()=>{try{let r=window.localStorage.getItem("beads-ui.theme"),n=window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches,s=r==="dark"||r==="light"?r:n?"dark":"light";document.documentElement.setAttribute("data-theme",s);let o=document.getElementById("theme-switch");o&&(o.checked=s==="dark")}catch{}let e=document.getElementById("theme-switch");e&&e.addEventListener("change",()=>{let r=e.checked?"dark":"light";document.documentElement.setAttribute("data-theme",r),window.localStorage.setItem("beads-ui.theme",r)});let t=document.getElementById("app");t&&Ig(t)});export{Ig as bootstrap,Tg as readBootstrapConfig,Cg as refreshConfigSnapshot};
