...
`)}function st(){if(!o)return c``;let H=U(),K=(a?[d.model,d.effort]:[d.runner,d.model,d.effort]).filter(Boolean).join(" \xB7 "),ge=d.session_id||"",Ce=`\uB77C\uC774\uBE0C \uB530\uB77C\uAC00\uAE30 ${p?"ON":"OFF"}`,je=ce(),We=je?Bf(oe(),Date.now()):"",$e=je?Ve(H):null,Je=je?Qe(H):null,rt=jf(H);return c`<div class="sv" data-attempt-id=${o}>
      <div class="sv__bar">
    ...
    ${i?c`<div class="worker-mini__row1">${b}${C}${E}</div>
          <div class="worker-mini__row2">
            ...
            ${on(e)}`:c`<div class="worker-mini__line">
              ${d}${p}${b}${C}${E}${M}${j}${J}${f}${Z}${W}${L}${S}${N}${k}${oe}
            </div>
            ${wr(e)} ${on(e)}`}
...
${J.prompt}</pre
    ...
    </div>`:""}function Y(w,T){let D=Q(w,T),q=U(),be=D.filter(Ge=>q.has(Ge)),we=oe(D),le=se(D),nt=Array.isArray(W().serial_lanes)?W().serial_lanes:[],Ae=d.get(w)||Ie(),gt=T.eligible!==!0||D.length<2||be.length>0||we.length>0||le||p;return c`<section class="pa-group" data-group-index=${String(w)}>
...
`):[];return c`<div
    ...
    </div>`}function C(){Ke(b(),n)}async function E(){if(o!=="ready")return;let S=await Zt(a);ae(S?"\uC2A4\uD06C\uB9BD\uD2B8 \uBCF5\uC0AC\uB428":"\uC2A4\uD06C\uB9BD\uD2B8 \uBCF5\uC0AC \uC2E4\uD328",S?"success":"error")}function M(S){S.key==="Escape"&&s&&(S.preventDefault(),W())}function j(){p||(document.addEventListener("keydown",M),p=!0)}function J(){p&&(document.removeEventListener("keydown",M),p=!1)}async function Z(S,N=null){let k=++l;j(),s={...S},d=N||(document.activeElement instanceof HTMLElement?document.activeElement:null),o="loading",a="",i="",C(),n.querySelector(".repo-ops-script-viewer__close")?.focus();let oe=t?t():"";if(!oe){o="error",i="\uC6CC\uD06C\uC2A4\uD398\uC774\uC2A4\uAC00 \uC120\uD0DD\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4.",C();return}if(!r){o="error",i="\uC2A4\uD06C\uB9BD\uD2B8 \uC694\uCCAD \uAE30\uB2A5\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.",C();return}let ce="/api/repo-ops-script?workspace="+encodeURIComponent(oe)+"&lane="+encodeURIComponent(S.lane)+"&base_sha="+encodeURIComponent(S.base_sha);try{let Q=await r(ce),se=await Q.json().catch(()=>({}));if(k!==l)return;if((t?t():"")!==oe){W();return}if(!Q.ok||!se||se.ok!==!0){o="error",i=Am(se&&typeof se.error=="string"?se.error:""),C();return}s={lane:se.lane,base_sha:se.base_sha,path:se.path,base_ref:se.base_ref},a=String(se.content),o="ready",C()}catch{if(k!==l)return;o="error",i="\uC2A4\uD06C\uB9BD\uD2B8 \uC694\uCCAD\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4.",C()}}function W(){l+=1,J(),s=null,a="",C();let S=d;d=null,S?.isConnected&&S.focus()}function L(){W(),n.remove()}return{open:Z,close:W,destroy:L}}function hd(e){let t=e.queueStore,r=e.transport,n=e.onChanged||(()=>{}),s=e.onOpenScript;function o(){return t&&t.get()||{}}function a(){let L=o();return typeof L.revision=="number"?L.revision:0}function i(L){t&&L&&L.queue&&typeof L.queue=="object"&&t.set(L.queue)}function l(){let L=o().workspace_info;return L&&typeof L=="object"?L:{}}function d(L,S){return c`<span
