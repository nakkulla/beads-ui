...
`);return c`<button
  ...
  >
    ${Un({id:typeof e.pane_id=="string"?e.pane_id:`worker-pane-lane-${e.id}`,lane:e.id,title:e.title,items:[],count:e.count,match_count:e.match_count,empty:"\uBE44\uC5B4 \uC788\uC74C \u2014 \uD589\uC744 \uC5EC\uAE30\uB85C \uB4DC\uB798\uADF8",header_control:c`${n}${e.header_control?e.header_control:""}`,body:c`<div
...
`);else return null;let n=t.replace(pb,"").trim();return n.length>0?{kind:"user",text:n}:null}function El(e){let t=e.split(/\r?\n/).find(r=>r.trim().length>0)||"",n=cb.exec(t);return n?{kind:"gate",gate:n[2]==="implementation"?"impl":n[2],reviewer:n[3],verdict:n[4],time:n[5]?n[5].trim():void 0,text:t.trim()}:ub.test(t)&&t.trim().length<=80?{kind:"phase",text:t.trim()}:{kind:"assistant",text:e}}function fb(e,t){if(e.subtype==="init"){let n=typeof e.model=="string"?e.model:"";return t.progress=null,[{kind:"thinking",text:n?`\uC138\uC158 \uC2DC\uC791 \xB7 ${n}`:"\uC138\uC158 \uC2DC\uC791"}]}if(e.subtype==="thinking_tokens"){let r=`\uC0DD\uAC01 \uC911\u2026 ${typeof e.estimated_tokens=="number"&&Number.isFinite(e.estimated_tokens)?Math.max(0,Math.round(e.estimated_tokens)):0} \uD1A0\uD070`;return t.progress?(t.progress.text=r,[]):(t.progress={kind:"thinking",text:r},[t.progress])}return[]}function _b(e,t){let n=typeof e.parent_tool_use_id=="string"&&e.parent_tool_use_id.length>0?e.parent_tool_use_id:null;if(e.type==="assistant"){let r=e.message,o=r&&Array.isArray(r.content)?r.content:[],s=[];for(let i of o)if(Pn(i)){if(i.type==="text"&&typeof i.text=="string")s.push(El(i.text));else if(i.type==="thinking"){let l=Sl(i.thinking);l&&s.push(l)}else if(i.type==="tool_use"){let l=db(i);typeof i.id=="string"&&t.set(i.id,l),s.push(l)}}return n?lp(s,n):s}if(e.type==="user"){let r=e.message,o=r&&Array.isArray(r.content)?r.content:[];for(let i of o)if(Pn(i)&&i.type==="tool_result"){let l=t.get(String(i.tool_use_id));if(l){let a=up(i.content);l.result=a,l.output=typeof i.content=="string"?i.content:a,i.is_error===!0&&(l.is_error=!0)}}let s=dp(r&&r.content);return s?[s]:[]}if(e.type==="result"){let r=e.is_error===!1&&e.subtype==="success",o={kind:"result",success:r,text:typeof e.result=="string"?e.result:r?"DONE":""};return n?lp([o],n):[o]}return[]}function lp(e,t){for(let n of e)n.parent_tool_use_id=t;return e}function mb(e){let t=typeof e.command=="string"?e.command:"",n=up(e.aggregated_output===void 0?e.output:e.aggregated_output),o=[typeof e.exit_code=="number"&&Number.isFinite(e.exit_code)?`exit ${e.exit_code}`:typeof e.status=="string"&&e.status.length>0?e.status:"",n].filter(i=>i.length>0).join(" \xB7 "),s={kind:"tool",tool:"shell",icon:cp.Bash,command:t,input:{command:t},expandable:!0};return o.length>0&&(s.result=o),typeof e.aggregated_output=="string"&&(s.output=e.aggregated_output),s}function gb(e){if(e.type==="item.completed"&&Pn(e.item)){let t=e.item;if(t.type==="agent_message"&&typeof t.text=="string")return[El(t.text)];if(t.type==="user_message"){let n=dp(t.text);return n?[n]:[]}if(t.type==="reasoning"){let n=Sl(t.text);return n?[n]:[]}return t.type==="error"?[{kind:"error",text:String(t.message||"")}]:t.type==="command_execution"?[mb(t)]:[]}if(e.type==="turn.completed")return[{kind:"result",success:!0,text:"DONE"}];if(e.type==="turn.failed"){let t=e.error;return[{kind:"error",text:t&&typeof t.message=="string"?t.message:"turn failed"}]}return e.type==="error"?[{kind:"error",text:String(e.message||"")}]:[]}function hb(e){if(e.schema!=="codex-delegation-monitor-v1"||!Pn(e.event))return[];let t=e.event;if(t.type==="session.started"||t.type==="turn.started")return[];if((t.type==="item.started"||t.type==="item.completed")&&Pn(t.item)){let n=t.item;if(typeof n.id!="string"||n.id.length===0)return[];if(t.type==="item.completed"&&n.kind==="agent_message"&&typeof n.text=="string"&&n.text.trim().length>0)return[El(n.text)];if(t.type==="item.completed"&&n.kind==="reasoning"){let i=Sl(n.text);return i?[i]:[]}if(t.type!=="item.completed"||n.kind!=="activity"||typeof n.activity!="string")return[];let r=lb[n.activity];if(!r)return[];let o,s;if(n.status==="completed")o="\uC644\uB8CC",s="\u2713";else if(n.status==="failed")o="\uC2E4\uD328",s="\u2717";else return[];return[{kind:"tool",tool:`${r} \xB7 ${o}`,icon:s,expandable:!1,result:""}]}return t.type==="turn.completed"&&t.status==="completed"?[{kind:"result",success:!0,text:"DONE"}]:t.type==="turn.failed"&&(t.status==="failed"||t.status==="interrupted")&&typeof t.error_code=="string"&&t.error_code.length>0?[{kind:"error",text:t.error_code}]:[]}function bb(e){let t=e.type;return typeof t=="string"&&(t==="error"||t.startsWith("thread.")||t.startsWith("turn.")||t.startsWith("item."))}function yb(e){let t=e;if(typeof e=="string"){let n=e.trim();if(n.length===0)return null;try{t=JSON.parse(n)}catch{return null}}return Pn(t)?t:null}function pp(e={}){let t=e.skip_delegated===!0,n=new Map,r={progress:null};return{push(o){let s=yb(o);if(!s)return[];if(t&&typeof s.parent_tool_use_id=="string"&&s.parent_tool_use_id.length>0)return[];if(s.type==="system"&&s.schema!=="codex-delegation-monitor-v1")return fb(s,r);let i=s.schema==="codex-delegation-monitor-v1"?hb(s):bb(s)?gb(s):_b(s,n);return i.length>0&&(r.progress=null),i}}}function Tl(e){let t=[],n=pp(),r=Array.isArray(e)?e:[];for(let o of r)for(let s of n.push(o))t.push(s);return t}var vb=5,wb=10,kb=/Task\s+#(\d+)/,$b=/\bgh\s+pr\s+create\b|\bgit\s+push\b/,xb=/\bnpm\s+(?:run\s+)?(?:test|tsc|lint|build)\b|\bvitest\b/;function ds(e){return typeof e!="string"?"":(e.split(/\r?\n/).find(t=>t.trim().length>0)||"").trim()}function Ab(e){return typeof e!="string"||e.length===0?0:e.split(/\r?\n/).length}function Sb(e){for(let t=e.length-1;t>=0;t-=1){let n=e[t];if(n.kind==="phase"||n.kind==="gate")return n.text||null}return null}function Eb(e){let t=new Map,n=0;for(let o of e){if(o.kind!=="tool")continue;n+=1;let s=o.input||{};if(o.tool==="TaskCreate"){let a=kb.exec(o.output||o.result||""),u=String(s.activeForm||s.subject||"").trim();if(!a||u.length===0)continue;t.set(a[1],{label:u,active:s.status==="in_progress"?n:0});continue}if(o.tool!=="TaskUpdate")continue;let i=t.get(String(s.taskId??""));if(!i)continue;let l=s.activeForm||s.subject;typeof l=="string"&&l.trim().length>0&&(i.label=l.trim()),typeof s.status=="string"&&(i.active=s.status==="in_progress"?n:0)}let r=null;for(let o of t.values())o.active>0&&(!r||o.active>r.active)&&(r=o);return r?r.label:null}function Tb(e){if(e.tool==="Bash"){let t=e.command||"";return $b.test(t)?"~ PR/\uAC8C\uC2DC \uC911":xb.test(t)?"~ \uAC80\uC99D \uC911":null}return e.tool==="Edit"||e.tool==="Write"||e.tool==="MultiEdit"?"~ \uAD6C\uD604 \uC911":e.tool==="Read"||e.tool==="Grep"||e.tool==="Glob"?"~ \uD0D0\uC0C9 \uC911":null}function Cb(e){let t=e.filter(o=>o.kind==="tool").slice(-wb),n=new Map;t.forEach((o,s)=>{let i=Tb(o);if(!i)return;let l=n.get(i)||{count:0,last:-1};l.count+=1,l.last=s,n.set(i,l)});let r=null;for(let[o,s]of n)(!r||s.count>r.count||s.count===r.count&&s.last>r.last)&&(r={label:o,count:s.count,last:s.last});return r?r.label:null}function Rb(e){let t=Sb(e);if(t)return{text:t,guess:!1};let n=Eb(e);if(n)return{text:n,guess:!1};let r=Cb(e);return r?{text:r,guess:!0}:null}function Ob(e,t){if(typeof e!="number")return"";let n=Math.max(0,Math.floor((t-e)/1e3));return n<60?`${n}\uCD08 \uC804`:ln(e,t)}function ho(e,t={}){let{transport:n,sessionLogStore:r,onClose:o}=t,s=null,i=null,l=null,a=null,u=null,d=!1,p={},g=!0,_=new Set,v=new Set,C=null,U=null,X=!1,se=!1,F=!1,N=null,R=null;function P(){X=!1,se=!1,F=!1,N=null,R=null}async function z(Y){if(n){se=!0,F=!1,Oe();try{let Q=await Promise.resolve(n("get-attempt-prompt",{attempt_id:Y,...u?{root_dir:u}:{}}));if(s!==Y)return;!Q||typeof Q!="object"||Array.isArray(Q)?F=!0:(N=Q,R=Y)}catch{s===Y&&(F=!0)}finally{s===Y&&(se=!1,Oe())}}}function V(){if(X=!X,X&&s&&R!==s){z(s);return}Oe()}function Z(){if(!X)return"";let Y=go({loading:se,error:F});if(Y)return c`<div class="sv__prompt" data-seam="attempt-prompt">
    ...
    </div>`}function D(){if(!a||!r)return[];let Y=r.get(a);return Tl(Y?Y.lines:[])}function K(){if(!a||!r)return null;let Y=r.get(a),Q=Y?Y.last_event_at:null;return typeof Q=="number"?Q:null}function H(){return p.status==="running"}function ne(){if(H()&&s){U||(U=setInterval(()=>Oe(),1e3));return}be()}function be(){U&&(clearInterval(U),U=null)}function Ce(Y){let Q=[],Ne=0;for(;Ne<Y.length;){let{idx:at,line:He}=Y[Ne];if(He.kind==="tool"){let Ie=Ne;for(;Ie<Y.length&&Y[Ie].line.kind==="tool"&&Y[Ie].line.tool===He.tool;)Ie+=1;if(Ie-Ne>=vb&&!v.has(at)){Q.push({kind:"group",idx:at,tool:He.tool||"",lines:Y.slice(Ne,Ie)}),Ne=Ie;continue}}Q.push({kind:"line",idx:at,line:He}),Ne+=1}return Q}function B(Y){let Q=[],Ne=new Map;for(let Ie=0;Ie<Y.length;Ie+=1){let k=Y[Ie],J=k.parent_tool_use_id;if(typeof J=="string"&&J.length>0){let Ae=Ne.get(J);Ae||(Ae={kind:"subagent",idx:Ie,launch_id:J,agent_type:null,header:null,lines:[]},Ne.set(J,Ae),Q.push(Ae)),Ae.lines.push({idx:Ie,line:k});continue}if(k.kind==="tool"&&k.tool==="Agent"&&typeof k.launch_id=="string"&&k.launch_id.length>0){let Ae=ee(k),Pe=Ne.get(k.launch_id);if(Pe){Pe.header={idx:Ie,line:k},Pe.agent_type=Ae;continue}let Ye={kind:"subagent",idx:Ie,launch_id:k.launch_id,agent_type:Ae,header:{idx:Ie,line:k},lines:[]};Ne.set(k.launch_id,Ye),Q.push(Ye);continue}Q.push({kind:"entry",idx:Ie,line:k})}let at=[],He=0;for(;He<Q.length;){if(Q[He].kind!=="entry"){at.push(Q[He]),He+=1;continue}let Ie=He;for(;Ie<Q.length&&Q[Ie].kind==="entry";)Ie+=1;at.push(...Ce(Q.slice(He,Ie))),He=Ie}return at}function ee(Y){let Q=Y.input;return Q&&typeof Q.subagent_type=="string"?Q.subagent_type:null}function $e(Y){for(let Q=Y.length-1;Q>=0;Q-=1){let Ne=Y[Q];if(Ne.kind==="result"||Ne.kind==="error")return null;if(Ne.kind==="tool"&&!Object.hasOwn(Ne,"result"))return Ne}return null}function Ee(Y){for(let Q=Y.length-1;Q>=0;Q-=1)if(Y[Q].kind==="thinking")return Y[Q];return null}function E(Y,Q){if(Q.kind==="gate")return c`<div class="sv__gate">${Q.text}</div>`;if(Q.kind==="phase")return c`<div class="sv__phase">${Q.text}</div>`;if(Q.kind==="result")return c`<div
      ...
      >
        ...
        >
      </div>`;if(Q.kind==="thinking"){let Ne=_.has(Y);return c`<div
      ...
      </div>`}if(Q.kind==="user"){let Ne=_.has(Y);return c`<div
      ...
      </div>`}if(Q.kind==="error")return c`<div class="sv__error">⛔ ${Q.text}</div>`;if(Q.kind==="blocker")return c`<div class="sv__error">⛔ ${Q.text}</div>`;if(Q.kind==="tool"){let Ne=_.has(Y),at=Q.tool==="Bash"?Ab(Q.command):0,He=Q.tool==="Bash"?at>1?ds(Q.command):Q.command:Q.path||Q.command||"";return c`<div
      ...
      </div>`}return c`<div class="sv__as">${mr(Q.text||"")}</div>`}function re(Y){let Q=[];if(Y.tool==="Bash"&&typeof Y.command=="string"&&Y.command.length>0)Q.push(Y.command);else if(Y.input!==void 0)try{Q.push(`input: ${JSON.stringify(Y.input,null,2)}`)}catch{}return typeof Y.output=="string"&&Y.output.length>0&&Q.push(`output:
${Y.output}`),Q.join(`

`)}function ye(){if(!s)return c``;let Y=D(),Q=(i?[p.agent_type,p.model,p.effort]:[p.runner,p.model,p.effort]).filter(Boolean).join(" \xB7 "),Ne=p.session_id||"",at=`\uB77C\uC774\uBE0C \uB530\uB77C\uAC00\uAE30 ${g?"ON":"OFF"}`,He=H(),Ie=He?Ob(K(),Date.now()):"",k=He?$e(Y):null,J=He?Ee(Y):null,Ae=Rb(Y);return c`<div class="sv" data-attempt-id=${s}>
      <div class="sv__bar">
        <span class="sv__id"
      ...
      ${i||d?"":Z()}
      <div class="sv__body">
        ${Y.length===0?c`<div class="sv__empty">세션 로그 없음</div>`:B(Y).map(Pe=>Pe.kind==="subagent"?Le(Pe):Pe.kind==="group"?ge(Pe):E(Pe.idx,Pe.line))}
      </div>
    ...
    >
    ...
    </div>`}function Le(Y){let Q=v.has(Y.idx),Ne=Y.header?Y.header.line:null,at=Ne?Ne.is_error===!0?"\u2717":typeof Ne.result=="string"?"\u2713":"\u27F3":"",He=Ne&&Ne.command?Ne.command:"";return c`<div class="sv__sub${Q?" sv__sub--open":""}">
      <div
        class="sv__sub-head"
        role="button"
        tabindex="0"
      ...
      ${Q?c`<div class="sv__sub-body">
            ${Ce(Y.lines).map(Ie=>Ie.kind==="group"?ge(Ie):E(Ie.idx,Ie.line))}
          </div>`:""}
    </div>`}function ce(Y){v.add(Y),Oe()}function Oe(){ot(ye(),e),ne(),g&&et()}function et(){let Y=e.querySelector(".sv__body");Y&&(Y.scrollTop=Y.scrollHeight)}function rt(Y){_.has(Y)?_.delete(Y):_.add(Y),Oe()}function I(){g=!g,Oe()}function pe(Y){cn(Y).then(Q=>{Q?he("\uBCF5\uC0AC\uB428","success",1200):he("\uBCF5\uC0AC \uC2E4\uD328","error",1600)})}function ie(Y){!s||!Y||(p={...p,...Y},Oe())}function ue(Y){let Q=Y.target;if(!Q||!Q.classList||!Q.classList.contains("sv__body"))return;!(Q.scrollHeight-Q.scrollTop-Q.clientHeight<=4)&&g&&(g=!1,Oe())}e.addEventListener("scroll",ue,!0);function Te(Y){let Q=Y.target;!Q||typeof Q.closest!="function"||e.contains(Q)||Q.closest("dialog")||Q.closest(".md-viewer-root")||je()}let de=!1;function De(){de||(document.addEventListener("mousedown",Te),de=!0)}function ze(){de&&(document.removeEventListener("mousedown",Te),de=!1)}function Je(Y){let Q=Y&&Y.attempt_id;if(!Q)return;let Ne=typeof Y.launch_id=="string"&&Y.launch_id.length>0?Y.launch_id:null,at=Y.session_ref&&typeof Y.session_ref=="object"?Y.session_ref:null;if(Ne&&at)return;let He=a;s=Q,i=Ne,l=at,a=i?`session-log:${s}:${i}`:`session-log:${s}`,n&&He&&He!==a&&Promise.resolve(n("unsubscribe-session-log",{id:He})).catch(()=>{}),u=typeof Y.root_dir=="string"&&Y.root_dir.length>0?Y.root_dir:null,p=Y.meta||{},d=Y.hide_prompt===!0,g=!0,_.clear(),v.clear(),P(),!C&&r&&(C=r.subscribe(Oe)),n&&Promise.resolve(n("subscribe-session-log",{id:a,attempt_id:s,...i?{launch_id:i}:{},...l?{session_ref:l}:{},...u?{root_dir:u}:{}})).catch(()=>{}),De(),Oe()}function je(){let Y=a;ze(),s=null,i=null,l=null,a=null,u=null,d=!1,_.clear(),v.clear(),P(),be(),n&&Y&&Promise.resolve(n("unsubscribe-session-log",{id:Y})).catch(()=>{}),ot(c``,e),o&&o()}return{open:Je,updateMeta:ie,close:je,isOpen(){return s!==null},destroy(){be(),ze(),C&&(C(),C=null),e.removeEventListener("scroll",ue,!0),s=null,i=null,l=null,a=null,u=null,d=!1,ot(c``,e)}}}function Lb(e){return["plan_review","plan_approval","plan_check"].some(t=>{let n=e[t];return typeof n=="string"&&n.trim().length>0})}function Ib(e){let t=e&&e.metadata||{},n=ir(e),r=[];return n.path&&r.push({kind:"spec",path:n.path,missing_state:n.evidence==="draft"?"spec_draft":null}),typeof t.plan_path=="string"&&t.plan_path.trim().length>0&&r.push({kind:"plan",path:t.plan_path.trim(),missing_state:Lb(t)?null:"plan_pending"}),r}function fp(e,t){let n=Ib(e);return c`
...
`):void 0}function wr(y){if(!y||typeof y!="object")return;let L=typeof y.status=="string"?y.status:"",G=typeof y.title=="string"?y.title:"";return L.length>0&&G.length>0?`${L} \xB7 ${G}`:void 0}function Dn(){return(t.getWorkspacePath&&t.getWorkspacePath()||"").trim()}function Wn(){return t.depCandidates?t.depCandidates():null}async function Nn(y,L,G){let ke=Dn(),Ge=u;if(!Ge)return;if(ke.length===0){he("\uB808\uD3EC\uB97C \uC54C \uC218 \uC5C6\uC5B4 \uC758\uC874\uC744 \uBC14\uAFC0 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4","error");return}let it=await x(y,{a:Ge,b:L,view_id:Ge,root_dir:ke},G),Bt=it===!0||it!==!1&&it.saved===!0;Bt&&t.onDepChanged&&t.onDepChanged({type:y,a:Ge,b:L}),y==="dep-add"&&Bt&&(K="",H=!1),Ue()}function Xt(y){if(!u)return;let L=globalThis.confirm;typeof L=="function"&&!L(`${y}\uAC00 ${u}\uB97C \uB9C9\uB294 \uC5F0\uACB0\uC744 \uB04A\uC744\uAE4C\uC694?`)||Nn("dep-remove",y,"\uC758\uC874 \uD574\uC81C \uC2E4\uD328")}function zn(y){y.disabled||Nn("dep-add",y.bead_id,"\uC758\uC874 \uCD94\uAC00 \uC2E4\uD328")}function kr(y){K=y.target.value,H=!0,Ue()}function or(){H||(H=!0,Ue())}function En(y,L){if(y.key==="Escape"){y.stopPropagation(),K="",H=!1,Ue();return}y.key==="Enter"&&(y.preventDefault(),L.length===1&&!L[0].disabled&&zn(L[0]))}function sr(y){return c`<div class="detail-dep-add">
      <input
        ...
        @keydown=${L=>En(L,y)}
      />
      ${H||K.length>0?c`<div class="detail-dep-add__list">
    ...
    `}function Ot(){if(!u)return c``;let y=d||{},L=String(y.id||u),G=y.title||"(\uC81C\uBAA9 \uC5C6\uC74C)",ke=Xe(),Ge=y.status||"open",it=typeof y.priority=="number"?Math.max(0,Math.min(4,y.priority)):"",Bt=y.description||"",Ve=i?i.get():null,kt=Ve&&Ge!=="closed"?Fo({...y,id:L},Ve):null,qn=Ve?Bo(Ve):null,Fr={...y,metadata:{...y.metadata||{},...p}};return c`
      <div class="detail-overlay" role="dialog" aria-modal="true">
        ...
        <div class="detail-overlay__panel">
          ...
          ${$n(y)} ${ys(y)}
          ${fp(y,St)}
          ${Ip({expanded:je,loading:Y,error:Q,data:Ne},{onToggle:W})}
          ${Lp(qe(),dt,{total:ke,expanded:Qe},Ae)}
  ...
  >
    ...
    ${_?Dy(a?"parked":u?"retry_wait":"waiting",a?p:g,rt,d?K:""):i?"":c`${H}${e.rollup?js(e.rollup,{parent_id:e.bead_id,expanded:e.rollup_expanded===!0,childChips:ba}):""}
    ...
    </div>`}function Ae(){return c`<section
    ...
    >
      ...
      ${R?J():""}
    </section>`}function Pe(A,q,oe,W,ve,qe,Xe){let Qe=ve[A]??Qt,Fe=Ca(A,oe,ve,H(),K(),Xe),wt=Fe.options.find(xt=>xt.value===Qe),Et=Qe===Qt?Fe.full_value:wt?.full_value;return c`<select
  ...
  >
    ...
    >
      ...
      >
        ${Zp()}
      </button>`}function K(E){let re=Uy(E);return re?c`<div class="mon2-deck__chips">
    ...
    </header>`}function Ae(m,b){return c`<div
      class="mon2-item"
      data-bead-id=${m.id}
    ...
    >
      ${b}
    </div>`}function Pe(m){if(R!==m.id)return null;let b=E.queue_groups.find(j=>j.root_dir===m.root_dir),M=m.place_lanes||[],f=E.cross_lanes_revision!==null,h=[{id:"parallel",label:"\uBCD1\uB82C",count:m.place_index??0}];for(let j of E.chain_lanes)h.push({id:`lane:${j.lane_id}`,label:`\uC5F0\uACB0 ${j.number} (${j.draft?"draft":"\uD655\uC815"}) \uB05D\uC5D0`,count:j.rows.length,group:"\uC5F0\uACB0 \uB808\uC778",disabled:!f});h.push({id:"new-lane",label:"+ \uC0C8 \uC5F0\uACB0 \uB808\uC778",group:"\uC5F0\uACB0 \uB808\uC778",disabled:!f,title:f?"\uC774 \uC774\uC288\uB9CC \uB4E0 draft \uB808\uC778\uC744 \uB9CC\uB4ED\uB2C8\uB2E4":"\uC5F0\uACB0 \uB808\uC778 \uC800\uC7A5\uC18C\uB97C \uC77D\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4"});for(let j of M)h.push({id:`serial:${j.id}`,label:`\uC9C1\uB82C ${Number(j.id.slice(1))}`,count:j.length,group:`${b?b.name:""} \uC9C1\uB82C`});return{bead_id:m.id,lanes:h}}function Ye(m){return Ae(m,c`${za(k(m),Pe(m),{exec_chips_mode:"pinned_only",onOpenDoc:l?(b,M)=>l(M,m.root_dir):void 0})}`)}function tt(){return E.runnable_flat?c`<div class="mon2-flat" data-drop="candidate">
        ${E.runnable.map(m=>Ye(m))}
      </div>`:c`${E.runnable_sections.map(m=>{let b=Y(m.root_dir);return c`<section
    ...
    >
      ...
      >
        ${m.rows.length===0?c`<div class="mon2-clane__hint">
            ...
            </div>`:m.rows.map((_e,Re)=>Rt(m,_e,Re,j))}
      </div>
...
`)}
  ...
  `}var ov=[{id:"execution",label:"\uC2E4\uD589",glyph:"\u25C6"},{id:"display",label:"\uD45C\uC2DC",glyph:"\u25EB"}];function wf(e,t){let{transport:n,policyStore:r,labelOptions:o}=t,s=t.notify||(ne=>he(ne,"error",4e3)),i=document.createElement("dialog");i.id="settings-dialog",i.className="settings-dialog",i.setAttribute("role","dialog"),i.setAttribute("aria-modal","true"),i.setAttribute("aria-label","\uC124\uC815"),e.appendChild(i);let l="execution",a=!1,u="",d=null;function p(){if(d)return d;let ne=i.querySelector('[data-pane="execution"]');return ne?(d=Ji(ne,{root_dir:null,queue:()=>t.queueStore?.get()??null,transport:n,implPresetStore:t.implPresetStore,notify:s,onQueueAdopt:be=>t.queueStore?.set?.(be)}),d):null}function g(){return c`
    ...
    </section>`}function K(B,ee){return c`<div
...
`):[];return c`<div
    ...
    </div>`}function _(){ot(g(),r)}async function v(){if(s!=="ready")return;let R=await cn(i);he(R?"\uC2A4\uD06C\uB9BD\uD2B8 \uBCF5\uC0AC\uB428":"\uC2A4\uD06C\uB9BD\uD2B8 \uBCF5\uC0AC \uC2E4\uD328",R?"success":"error")}function C(R){R.key==="Escape"&&o&&(R.preventDefault(),F())}function U(){d||(document.addEventListener("keydown",C),d=!0)}function X(){d&&(document.removeEventListener("keydown",C),d=!1)}async function se(R,P=null){let z=++a;U(),o={...R},u=P||(document.activeElement instanceof HTMLElement?document.activeElement:null),s="loading",i="",l="",_(),r.querySelector(".repo-ops-script-viewer__close")?.focus();let Z=t?t():"";if(!Z){s="error",l="\uC6CC\uD06C\uC2A4\uD398\uC774\uC2A4\uAC00 \uC120\uD0DD\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4.",_();return}if(!n){s="error",l="\uC2A4\uD06C\uB9BD\uD2B8 \uC694\uCCAD \uAE30\uB2A5\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.",_();return}let D="/api/repo-ops-script?workspace="+encodeURIComponent(Z)+"&lane="+encodeURIComponent(R.lane)+"&base_sha="+encodeURIComponent(R.base_sha);try{let K=await n(D),H=await K.json().catch(()=>({}));if(z!==a)return;if((t?t():"")!==Z){F();return}if(!K.ok||!H||H.ok!==!0){s="error",l=wv(H&&typeof H.error=="string"?H.error:""),_();return}o={lane:H.lane,base_sha:H.base_sha,path:H.path,base_ref:H.base_ref},i=String(H.content),s="ready",_()}catch{if(z!==a)return;s="error",l="\uC2A4\uD06C\uB9BD\uD2B8 \uC694\uCCAD\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4.",_()}}function F(){a+=1,X(),o=null,i="",_();let R=u;u=null,R?.isConnected&&R.focus()}function N(){F(),r.remove()}return{open:se,close:F,destroy:N}}var Gf={deploy_not_declared:"\uC120\uC5B8 \uC5C6\uC74C",deploy_opted_out:"\uC774 workspace\uC5D0\uC11C \uBC30\uD3EC \uC2E4\uD589\uC774 \uAEBC\uC838 \uC788\uC74C",deploy_in_flight:"\uBC30\uD3EC \uC9C4\uD589 \uC911",target_unresolved:"\uB300\uC0C1 tip\uC744 \uD655\uC815\uD558\uC9C0 \uBABB\uD568",remote_history_not_monotonic:"\uBC30\uD3EC \uC6CC\uD06C\uD2B8\uB9AC\uC640 \uC6D0\uACA9 \uC774\uB825\uC774 \uAC08\uB77C\uC9D0"},kv=new Set(["queued","running","retry_pending"]);function Kf(e){let t=e.queueStore,n=e.transport,r=e.onChanged||(()=>{}),o=e.onOpenScript;function s(){return t&&t.get()||{}}function i(){let D=s();return typeof D.revision=="number"?D.revision:0}function l(D){t&&D&&D.queue&&typeof D.queue=="object"&&t.set(D.queue)}function a(){let D=s().workspace_info;return D&&typeof D=="object"?D:{}}function u(D,K){return c`<span
      class="worker-repo-ops__vd-badge worker-repo-ops__vd-badge--${D}"
      >${K}</span
    >`}function d(D){if(typeof D!="number"||!Number.isFinite(D))return"";let K=D/6e4;return Number.isInteger(K)?`timeout ${K}\uBD84`:`timeout ${Math.round(D/1e3)}\uCD08`}function p(D){let K=d(D);return K?u("config",K):""}function g(D,K,H){return c`<button
      type="button"
      class="worker-repo-ops__vd-cmd worker-repo-ops__vd-cmd--link"
      .textContent=${H.script}
      @click=${ne=>{o&&o({lane:D,base_sha:K.base_sha,path:H.script,base_ref:K.base_ref},ne.currentTarget)}}
    ></button>`}function _(){let D=s().repo_operations;return Array.isArray(D)?D:[]}function v(){let D=a().repo_ops,K=D&&typeof D=="object"?D.repo_id:null;return typeof K=="string"&&K?K:null}function C(){return _().some(D=>D&&D.kind==="deploy"&&kv.has(D.state))}function U(){let D=C(),K=v()===null;return c`<button
      type="button"
    ...
    >
      배포 실행
    </button>`}function X(){let D=s().repo_ops_opt_out;return{verify:D?.verify===!0,deploy:D?.deploy===!0}}function se(D,K){return c`<label class="worker-repo-ops__lane-run">
      <input
        type="checkbox"
        .checked=${!K}
      ...
      />
      이 workspace에서 실행
    </label>`}function F(D){let K=typeof D.base_sha=="string"?D.base_sha:"",H=`${D.source_path||"repo-ops/config.toml"} @ ${D.base_ref||"?"}${K?`@${K.slice(0,7)}`:""}`,ne=X(),be=!!D.verify&&ne.verify,Ce=!!D.deploy&&ne.deploy;return c`<section class="worker-repo-ops__vd" data-seam="repo-ops">
    ...
    </section>`}function N(D){let K=D.repo_ops&&typeof D.repo_ops=="object"?D.repo_ops:null;return K&&(K.status==="resolved"||K.status==="absent")?F(K):K&&(K.status==="pending"||K.status==="error")?c`<section class="worker-repo-ops__vd" data-seam="repo-ops">
        <p class="worker-repo-ops__vd-title">
          저장소 작업 선언
    ...
    </section>`}async function R(D,K){if(!n)return;let H=await n("worker-repo-ops-opt-out-toggle",{kind:D,opted_out:K,expected_revision:i()});if(l(H),H&&H.conflict){let ne=await n("worker-repo-ops-opt-out-toggle",{kind:D,opted_out:K,expected_revision:i()});l(ne)}r()}async function P(){let D=v();if(!n||D===null)return;let K=await n("worker-repo-operation-deploy-run",{repo_id:D});if(l(K),!K||K.ok!==!0){let H=K&&typeof K.reason=="string"?K.reason:"",ne=Object.hasOwn(Gf,H)?Gf[H]:H||"\uBC30\uD3EC \uC2E4\uD589\uC744 \uC2DC\uC791\uD558\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4";he(`\uBC30\uD3EC \uC2E4\uD589 \uAC70\uBD80 \u2014 ${ne}`,"error")}else he("\uBC30\uD3EC \uC2E4\uD589\uC744 \uC2DC\uC791\uD588\uC2B5\uB2C8\uB2E4","success");r()}let z={owned_deploy_worktree_fetch_detached_alignment_recreate:"\uC804\uC6A9 \uBC30\uD3EC \uC6CC\uD06C\uD2B8\uB9AC \uC815\uB82C\xB7\uBCF5\uAD6C",recovered_pre_execution_fetch_timeout_retry_once:"fetch \uD0C0\uC784\uC544\uC6C3 1\uD68C \uBCF5\uAD6C",repo_serial_lock_wait:"\uC800\uC7A5\uC18C \uC21C\uCC28 \uC2E4\uD589 \uB300\uAE30",restart_operation_adoption:"\uC7AC\uC2DC\uC791 \uD6C4 \uC791\uC5C5 \uC778\uACC4",exact_input_exit_zero_evidence_adoption:"\uB3D9\uC77C \uC785\uB825 \uC131\uACF5 \uC99D\uAC70 \uC778\uACC4",descendant_success_covers_ancestor_rows:"\uCD5C\uC2E0 SHA \uC131\uACF5\uC774 \uC774\uC804 \uD589 \uCEE4\uBC84",owned_verify_candidate_cleanup:"\uAC80\uC99D \uC784\uC2DC \uCCB4\uD06C\uC544\uC6C3 \uC815\uB9AC",bounded_single_script_retry_exceeded:"\uB2E8\uC77C \uC2A4\uD06C\uB9BD\uD2B8 \uC7AC\uC2DC\uB3C4 \uD55C\uB3C4 \uCD08\uACFC",repair_session_dispatch:"\uC2E4\uD328 \uD574\uACB0 \uC138\uC158 \uC790\uB3D9 \uC2E4\uD589",baseline_failure_ignore:"\uAE30\uC874 \uC2E4\uD328 \uBB34\uC2DC",config_or_script_deletion_to_bypass_gate:"\uC124\uC815\xB7\uC2A4\uD06C\uB9BD\uD2B8 \uC0AD\uC81C\uB85C \uAC8C\uC774\uD2B8 \uC6B0\uD68C",credential_entry:"\uC790\uACA9\uC99D\uBA85 \uC785\uB825\xB7\uCD9C\uB825",destructive_action:"\uD30C\uAD34\uC801 \uC791\uC5C5",history_rewrite:"\uD788\uC2A4\uD1A0\uB9AC \uC7AC\uC791\uC131",agent_self_report_as_success:"\uC138\uC158 \uC790\uAE30\uBCF4\uACE0\uB97C \uC131\uACF5 \uCC98\uB9AC"};function V(D,K,H){return c`<div class="worker-repo-ops__policy-group" data-policy=${H}>
      <div class="worker-repo-ops__policy-label">${D}</div>
      <ul class="worker-repo-ops__policy-list">
        ${K.map(ne=>c`<li data-token=${ne}>
              ${z[ne]||ne}
            </li>`)}
      </ul>
    </div>`}function Z(){let D=s(),K=D.repo_operation_policy&&typeof D.repo_operation_policy=="object"?D.repo_operation_policy:null;return K?c`<section
...
\uC9C1\uC804 \uB9AC\uBDF0 \uC138\uC158 \uC885\uB8CC \uC0AC\uC720: ${e.review_session.failure}`,alert:!0})}return n("\uCD5C\uC885 \uBCC0\uACBD \uB9AC\uBDF0 \uD544\uC694",{title:s,alert:!0})}return e.gate?.reason==="spec_id_missing"?n("\uC2A4\uD399 ID \uB204\uB77D",{title:"native spec_id \uBBF8\uAE30\uB85D \u2014 bd update --spec-id \uD544\uC694",alert:!0}):r.length>0?n(`\uC601\uC218\uC99D \uD655\uC778 \uD544\uC694 \xB7 ${r[0]}`,{title:`\uC131\uB9BD\uD558\uC9C0 \uC54A\uB294 \uC2E4\uD589 \uC601\uC218\uC99D \u2014 ${r.join(", ")}`,alert:!0}):e.recovery?n(e.recovery.badge,{title:e.recovery.title,alert:!0}):e.gate?.tier==="verify"&&e.gate.gate_badge==="\uAC80\uC99D \uC2E4\uD328"?n("\uAC80\uC99D \uC2E4\uD328",{title:e.gate.reason||"",alert:!0}):e.queue_failure?n(`\uBA38\uC9C0 \uC2E4\uD328 \u2014 ${s_(e.queue_failure)}`,{title:e.queue_failure,alert:!0}):e.auto_skip?n(`\uC790\uB3D9 \uC81C\uC678 \u2014 ${s_(e.auto_skip)}`,{title:e.auto_skip,alert:!0}):e.queued&&!e.queue_active?n(`\uBA38\uC9C0 \uB300\uAE30 #${e.queue_position}`):e.gate?.enabled===!0?n("\uBA38\uC9C0 \uAC00\uB2A5"):e.gate?.tier==="merged"?n("\uBA38\uC9C0\uB428"):e.gate?.tier==="closed_unmerged"?n("\uB2EB\uD798",{alert:!0}):e.activity?n("\uD655\uC778 \uC911",{live:!0}):e.gate?.tier==="undecidable"||e.gate?.reason==="mergeability_unknown"?n("\uC0C1\uD0DC \uD655\uC778 \uC2E4\uD328",{title:e.gate.reason||"",alert:!0}):e.gate?.tier==="unobserved"||e.gate?.tier==="verify"||e.gate?.gate_badge==="\uAD00\uCE21 \uB300\uAE30"?n("\uD655\uC778 \uC911"):e.gate?.gate_badge?n(e.gate.gate_badge,{title:e.gate.reason||"",alert:e.gate.enabled!==!0}):null}function $w(e,t,n,r,o=null,s=null,i=null,l=!1,a=null,u=!0,d=null,p=null,g=null,_={},v=!1,C={},U=null,X={active:!1,failure:null,origin:null},se=!1){let F=!!a&&a.position>0,N=!!a?.continuation_action&&a.continuation_action.continuation===null,R=!!a&&a.active===!0,P=a&&a.failure||null,z=fw(a?a.waiting:null),V=n[e]||null,Z=V&&V.gate?V.gate:null,D=V&&V.pr?V.pr:null,K=_w(a?a.resolution:null),H=hw(g),ne=yw(g,H),be=a&&a.authority||null,Ce=a&&a.review_dispatch||null,B=a?.hold?.auto_review_wait==="slot"?"slot":null,ee=!!g&&typeof g=="object"&&gw.has(g.phase),$e=F&&!R&&(!be||ee||be.source==="automatic"&&!v),Ee=i==="paused"?"\uCDA9\uB3CC \uD574\uC18C \uC77C\uC2DC\uC815\uC9C0":K?K.badge:i==="running"?"\uCDA9\uB3CC \uD574\uC18C \uC911":z,E=!!Z&&Z.base_badge==="\uCDA9\uB3CC",re=!!Z&&Z.enabled===!0,ye=Go({bead_id:e,merge_sha:C.merge_sha,cleanup_cursor:C.cleanup_cursor,merge_progress:s&&s.merge_progress?s.merge_progress:null,cleanup_failed:r,repo_operations:C.repo_operations}),ge=wi(ye),Le=s&&!ye&&(s.queueing??null)?s.queueing:null,ce=!!r&&["repo_operations","post_merge_jobs","child_sweep","branch_cleanup","parent_close"].includes(r.step)&&!!Z&&Z.tier==="merged",Oe=r&&r.step==="repo_operations"&&ye?.failed===!0&&(ye.step==="deploy"||ye.step==="verify")?ye.step:null,et=l&&!!r&&!!Z&&Z.tier==="merged",rt=$e&&(re||E||Z?.reason==="base_behind"||Ul.has(Z?.reason)||ce||et),I=Ul.has(Z?.reason),pe=l&&E&&u===!1,ie=Zn(_,e,{external:l,merge_active:R||ye?.step==="merge",merge_queued:F,conflict_active:!!i,cleanup_active:ge,merged:!!r||Z?.tier==="merged"}),ue=!!ie.operation,Te=!!r||g?.phase==="needs_human"||!!ie.error,de=F&&!P&&!N&&!ce&&!(ne&&ne.lock_actions),De=kw({auto_pending:de,continuation_required:N,queueing:Le,merge_step:ye,conflict_badge:Ee,conflict_live:K?.live===!0||i==="running",auto_resolution:H,recovery:ne,cleanup_failed:r,cleanup_label:r?Pr(r.step):null,base_exception:p,conflicting:E,gate:Z,receipt_check:V&&V.receipt_check?V.receipt_check:null,queue_failure:P,auto_skip:d,queued:F,queue_active:R,queue_position:a?a.position:0,review_session:X,review_dispatch:Ce,auto_review_wait:B,activity:Ee?null:s&&s.activity||null}),ze=De?.live===!0&&De.title?c`<span title=${De.title}>${De.label}</span>`:De?.label||null,Je=ww(V&&V.receipt_check?V.receipt_check:null);return{id:e,title:l?c`${t}<span class="muted"> · 세션</span>`:t,reason:r&&ye?.active!==!0?vi(r.step):"PR \uB300\uAE30",draggable:!1,done:!0,lane:"pr_wait",...U?{dependency_chips:U}:{},external:l,pr_number:D&&typeof D.number=="number"?D.number:null,pr_url:D&&typeof D.url=="string"?D.url:"",completion_badge:De?.live!==!0&&De?.title?De.label:null,completion_title:De?.title||"",...g?.phase==="needs_human"&&typeof g.log_path=="string"&&g.log_path.length>0?{log_path:g.log_path}:{},...Je.length>0?{receipt_badge:{codes:Je}}:{},badges:ze?[ze]:[],live_badge:De?.live===!0?ze:null,usage:o,alert:De?.alert===!0,merge_action:Z?.tier==="merged"&&!ce&&!et?!1:!F||N||$e||I,cancel_action:F&&!N,cancel_enabled:!R&&!(ne&&ne.lock_actions),cancel_title:ne&&ne.lock_actions?`${ne.badge} \u2014 \uC911\uB2E8\uD558\uB824\uBA74 \uC0C1\uB2E8 \uC790\uB3D9 \uBA38\uC9C0 \uC911\uB2E8\uC744 \uC0AC\uC6A9\uD558\uC138\uC694`:R?"\uBA38\uC9C0 \uC9C4\uD589 \uC911 \u2014 \uCDE8\uC18C\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4":"\uBA38\uC9C0 \uD050\uC5D0\uC11C \uC774 \uD56D\uBAA9\uC744 \uBE8D\uB2C8\uB2E4 (\uB2E4\uC2DC [\uBA38\uC9C0]\uB85C \uB123\uC744 \uC218 \uC788\uC2B5\uB2C8\uB2E4)",discard:ie,discard_action:ie.action,resolve_action:Te,resolve_enabled:!se,resolve_title:se?"\uC138\uC158 \uAE30\uB3D9 \uC694\uCCAD \uC911 \u2014 \uC11C\uBC84 \uC751\uB2F5\uC744 \uAE30\uB2E4\uB9BD\uB2C8\uB2E4":"\uC774 \uC2E4\uD328\uB97C \uC0AC\uB78C\uC774 \uC774\uC5B4\uBC1B\uB294 \uB300\uD654\uD615 \uC138\uC158\uC744 \uB744\uC6C1\uB2C8\uB2E4 \u2014 \uAE30\uB85D\uB41C \uC138\uC158\uC774 \uC788\uC73C\uBA74 fork\uD558\uACE0, \uC5C6\uC73C\uBA74 \uC0C8 \uC138\uC158\uC5D0 \uC0AC\uC720\uB97C \uC2E3\uC2B5\uB2C8\uB2E4",merge_step:ye,discard_enabled:ie.enabled,discard_title:ie.title,merge_enabled:!ye&&!Le&&!i&&!ue&&!p&&!(ne&&ne.lock_actions)&&!pe&&X.active!==!0&&(re||E||Z?.reason==="base_behind"||I||ce||et||rt||ee&&!R),merge_label:N?"\uC774\uC5B4\uD558\uAE30 \uC120\uD0DD":ce||et?Oe==="deploy"?"\uBC30\uD3EC \uC7AC\uC2DC\uB3C4 \uD6C4 \uC815\uB9AC":Oe==="verify"?"\uAC80\uC99D \uC7AC\uC2DC\uB3C4 \uD6C4 \uC815\uB9AC":"\uC815\uB9AC \uC7AC\uC2DC\uB3C4":E&&!ye&&!ce?"\uCDA9\uB3CC \uD574\uC18C \uD6C4 \uBA38\uC9C0":Z?.reason==="base_behind"?"base \uAC31\uC2E0 \uD6C4 \uBA38\uC9C0":I?"\uB9AC\uBDF0 \uD6C4 \uBA38\uC9C0":$e?"\uB2E4\uC2DC \uBA38\uC9C0":void 0,merge_title:ue?ie.error?`\uD3D0\uAE30 \uC2E4\uD328: ${ie.error} \u2014 [\uC7AC\uC2DC\uB3C4]\uD558\uAC70\uB098 \uC0C1\uD0DC\uB97C \uD655\uC778\uD558\uC138\uC694`:`\uD3D0\uAE30 \uC9C4\uD589 \uC911 \u2014 ${ie.progress||"\uC644\uB8CC\uB97C \uAE30\uB2E4\uB9AC\uC138\uC694"}`:N?"\uC2E4\uD589 provider\uAC00 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4 \u2014 \uC774\uC5B4\uAC08 \uBC29\uC2DD\uC744 \uC120\uD0DD\uD558\uC138\uC694":Le?"\uC694\uCCAD\uC744 \uBCF4\uB0B4\uB294 \uC911 \u2014 \uC11C\uBC84 \uC751\uB2F5\uC744 \uAE30\uB2E4\uB9BD\uB2C8\uB2E4":ye?`\uBA38\uC9C0 \uC9C4\uD589 \uC911 \u2014 ${ye.label}`:Oe?`\uBA38\uC9C0 \uC644\uB8CC \u2014 ${Oe==="deploy"?"\uBC30\uD3EC":"\uAC80\uC99D"} \uC2A4\uD06C\uB9BD\uD2B8\uAC00 \uC2E4\uD328\uD574 \uC815\uB9AC\uAC00 \uBA48\uCDC4\uC2B5\uB2C8\uB2E4. \uD074\uB9AD\uD558\uBA74 \uC800\uC7A5\uC18C \uC791\uC5C5\uBD80\uD130 \uC815\uB9AC\uB97C \uB2E4\uC2DC \uC9C4\uD589\uD569\uB2C8\uB2E4`:et?"\uBA38\uC9C0 \uC644\uB8CC \u2014 \uD074\uB9AD\uD558\uBA74 \uC2E4\uD328\uD55C \uC815\uB9AC\uB97C \uB2E4\uC2DC \uC2DC\uB3C4\uD569\uB2C8\uB2E4":pe?"\uC6CC\uD06C\uD2B8\uB9AC \uC5C6\uC74C \u2014 \uC138\uC158\uC5D0\uC11C \uC9C1\uC811 \uD574\uC18C\uD558\uC138\uC694":i==="running"?"\uCDA9\uB3CC \uD574\uC18C \uC138\uC158 \uC2E4\uD589 \uC911 \u2014 \uC644\uB8CC \uD6C4 \uB2E4\uC2DC \uBA38\uC9C0\uD558\uC138\uC694":i==="paused"?"\uCDA9\uB3CC \uD574\uC18C \uC138\uC158 \uC77C\uC2DC\uC815\uC9C0 \u2014 \uC7AC\uAC1C \uD6C4 \uC644\uB8CC\uB418\uBA74 \uBA38\uC9C0\uD558\uC138\uC694":ce?"\uBA38\uC9C0 \uC644\uB8CC \u2014 \uD074\uB9AD\uD558\uBA74 \uB0A8\uC740 \uC815\uB9AC\uB97C \uC2E4\uD328 \uB2E8\uACC4\uBD80\uD130 \uB2E4\uC2DC \uC2DC\uB3C4\uD569\uB2C8\uB2E4":E?"\uCDA9\uB3CC \u2014 \uD050\uC5D0 \uB123\uC73C\uBA74 \uD574\uC18C \uC138\uC158\uC744 \uB744\uC6B0\uACE0 \uC644\uB8CC \uD6C4 \uC790\uB3D9\uC73C\uB85C \uC7AC\uBA38\uC9C0\uD569\uB2C8\uB2E4":Z?.reason==="base_behind"?"base\uB97C \uC790\uB3D9 \uAC31\uC2E0\uD55C \uB4A4 \uBA38\uC9C0\uD569\uB2C8\uB2E4":X.active===!0?X.origin==="auto"?"\uC790\uB3D9 \uB9AC\uBDF0 \uC138\uC158 \uC2E4\uD589 \uC911 \u2014 \uB05D\uB098\uBA74 \uC601\uC218\uC99D\uC744 \uB2E4\uC2DC \uD310\uC815\uD569\uB2C8\uB2E4":"\uB9AC\uBDF0 \uC138\uC158 \uC2E4\uD589 \uC911 \u2014 \uB05D\uB098\uBA74 \uC601\uC218\uC99D\uC744 \uB2E4\uC2DC \uD310\uC815\uD569\uB2C8\uB2E4":Z?.reason==="review_receipt_missing"?"\uB9AC\uBDF0 \uC601\uC218\uC99D \uC5C6\uC74C \u2014 \uBA38\uC9C0 \uAC8C\uC774\uD2B8 \uBCF4\uB958\uC785\uB2C8\uB2E4. \uD074\uB9AD\uD558\uBA74 \uAE30\uB85D\uB41C \uC138\uC158\uC744 \uC774\uC5B4 \uB9AC\uBDF0\uB9CC \uC218\uD589\uC2DC\uD0A4\uACE0, \uC601\uC218\uC99D\uC774 \uCD5C\uC885 head\uC5D0 \uC720\uD6A8\uD574\uC9C0\uBA74 \uD050\uAC00 \uBA38\uC9C0\uD569\uB2C8\uB2E4":Z?.reason==="review_receipt_stale"?"head \uC7AC\uC791\uC131\uB428(\uC601\uC218\uC99D\uC774 \uD604\uC7AC head\uC758 \uC870\uC0C1\uC774 \uC544\uB2D8) \u2014 \uBA38\uC9C0 \uAC8C\uC774\uD2B8 \uBCF4\uB958\uC785\uB2C8\uB2E4. \uD074\uB9AD\uD558\uBA74 \uAE30\uB85D\uB41C \uC138\uC158\uC744 \uC774\uC5B4 \uCD5C\uC885 head\uB97C \uB2E4\uC2DC \uB9AC\uBDF0\uC2DC\uD0A4\uACE0, \uC601\uC218\uC99D\uC774 \uC720\uD6A8\uD574\uC9C0\uBA74 \uD050\uAC00 \uBA38\uC9C0\uD569\uB2C8\uB2E4":Z?.reason==="review_receipt_invalid"?"\uB9AC\uBDF0 \uC601\uC218\uC99D \uAE30\uB85D\uC774 \uC131\uB9BD\uD558\uC9C0 \uC54A\uC74C \u2014 \uBA38\uC9C0 \uAC8C\uC774\uD2B8 \uBCF4\uB958\uC785\uB2C8\uB2E4. \uD074\uB9AD\uD558\uBA74 \uAE30\uB85D\uB41C \uC138\uC158\uC744 \uC774\uC5B4 \uCD5C\uC885 head\uB97C \uB2E4\uC2DC \uB9AC\uBDF0\uC2DC\uD0A4\uACE0, \uC601\uC218\uC99D\uC774 \uC720\uD6A8\uD574\uC9C0\uBA74 \uD050\uAC00 \uBA38\uC9C0\uD569\uB2C8\uB2E4":Z?.reason==="review_receipt_undetermined"?"\uB9AC\uBDF0 \uC601\uC218\uC99D ancestry probe \uBBF8\uC644\uB8CC \u2014 \uBA38\uC9C0 \uAC8C\uC774\uD2B8 \uBCF4\uB958\uC785\uB2C8\uB2E4. \uD074\uB9AD\uD558\uBA74 \uAE30\uB85D\uB41C \uC138\uC158\uC744 \uC774\uC5B4 \uCD5C\uC885 head\uB97C \uB2E4\uC2DC \uB9AC\uBDF0\uC2DC\uD0A4\uACE0, \uC0C8 \uC601\uC218\uC99D\uC774 \uCD5C\uC885 head\uC5D0 \uC720\uD6A8\uD574\uC9C0\uBA74 \uD050\uAC00 \uBA38\uC9C0\uD569\uB2C8\uB2E4":Z?.reason==="spec_id_missing"?"native spec_id \uBBF8\uAE30\uB85D \u2014 bd update --spec-id\uB85C \uAE30\uB85D\uD55C \uB4A4 \uB2E4\uC2DC \uBA38\uC9C0\uD558\uC138\uC694":re?`\uBA38\uC9C0 (${Z.gate_badge}) \u2014 \uD050\uC5D0 \uB123\uC5B4 \uC21C\uC11C\uB300\uB85C \uBA38\uC9C0\uD569\uB2C8\uB2E4 (\uCC28\uB840\uAC00 \uB418\uBA74 \uB2E4\uC2DC \uD655\uC778)`:Z&&Z.tier==="merged"?"\uBA38\uC9C0\uB428 \u2014 \uBA38\uC9C0 \uD6C4 \uC815\uB9AC \uC9C4\uD589 \uC911":`\uBA38\uC9C0 \uBD88\uAC00: ${Z&&Z.reason||"\uAD00\uCE21 \uB300\uAE30"}`}}function Wl(e,t={}){let{transport:n,issueStores:r,queueStore:o,sessionLogStore:s,gotoIssue:i,getWorkspacePath:l,switchWorkspace:a,openDoc:u,doneRange:d,onDoneRangeChange:p}=t,g=r?Kr(r):null,_=nw(),v=null,C=null,U=to(()=>h()),X=new Map,se=new Map,F=Nf(),N=ql(F)===null,R=d?jn(d):sw();function P(){let $=Ur.find(w=>w.value===R);return $?$.label:"\uC624\uB298"}let z=Ki("beads-ui.worker.lane-collapsed"),V=!1,Z="";function D(){return Z.trim().length>0}function K($){return D()?$.filter(w=>w.search_match===!0).length:void 0}let H=new Set,ne=new Set,be=new Set;function Ce($,w){return!w?.error||!$?{}:{resolve_action:!0,resolve_enabled:!be.has($),resolve_title:be.has($)?"\uC138\uC158 \uAE30\uB3D9 \uC694\uCCAD \uC911 \u2014 \uC11C\uBC84 \uC751\uB2F5\uC744 \uAE30\uB2E4\uB9BD\uB2C8\uB2E4":"\uC2E4\uD328\uD55C \uD3D0\uAE30\uB97C \uC0AC\uB78C\uC774 \uC774\uC5B4\uBC1B\uB294 \uB300\uD654\uD615 \uC138\uC158\uC744 \uB744\uC6C1\uB2C8\uB2E4 \u2014 \uAE30\uB85D\uB41C \uC138\uC158\uC774 \uC788\uC73C\uBA74 fork\uD558\uACE0, \uC5C6\uC73C\uBA74 \uC0C8 \uC138\uC158\uC5D0 \uC0AC\uC720\uB97C \uC2E3\uC2B5\uB2C8\uB2E4"}}let B=new Set,ee=new Set,$e=new Set,Ee=null,E=[],re=n_({queueStore:o,issueStores:r,transport:n,getWorkspacePath:l,onInvalidate:()=>h()});function ye(){re.refreshSessionDefaults()}let ge=document.createElement("div");ge.className="worker-console";let Le=document.createElement("div");Le.className="worker-top";let ce=document.createElement("div");ce.className="worker-drawer-overlay",ce.hidden=!0;let Oe=document.createElement("div");Oe.className="worker-drawer-overlay__backdrop";let et=document.createElement("div");et.className="worker-drawer-host";let rt=document.createElement("div");rt.className="worker-drawer-host",rt.hidden=!0,ce.append(Oe,et,rt);let I=document.createElement("div");I.className="worker-lanes-host",ge.append(Le,ce,I),e.appendChild(ge);let pe=_r(null,null),ie=[],ue=Vi({transport:n,console_el:ge,getLanes:()=>pe,getWorkspaces:()=>ie,getCrossLanes:()=>null,reproject:()=>({lanes:W(),raw_lanes:null}),onCorrection:()=>{},showToast:he,requestRender:()=>h(),adoptQueue:($,w)=>{o&&o.set(w)},onDragBegin:()=>{v=null}}),Te=null,de=ho(et,{transport:n,sessionLogStore:s,onClose:()=>{Te=null,ce.hidden=!0,h()}}),De=Jf(rt,{onClose:()=>{rt.hidden=!0,ce.hidden=!0,h()}}),ze=Hf({getWorkspacePath:l||(()=>"")}),Je=l&&l()||"",je=Kf({queueStore:o,transport:n,onChanged:()=>h(),onOpenScript:($,w)=>{ze.open($,w)}});function Y(){return o&&o.get()||{revision:0,auto_advance:!1,auto_merge:!1,slots:na,queue:[],serial_lanes:[],serial_lane_count:0,pr_wait:[],done:[]}}function Q($){if(!v||!$.some(O=>O.id===v))return null;let w=Bo(Y());return w?{bead_id:v,lanes:w}:null}function Ne(){return l&&l()||""}async function at($,w){await ue.sendOp({type:"worker-queue-place",payload:{bead_id:$,...w==="parallel"?{}:{lane:w}},root_dir:Ne()},$)}function He(){let $=Y();return typeof $.revision=="number"?$.revision:0}function Ie($){$&&$.queue&&o&&o.set($.queue)}async function k($){if(!n||!$)return;let w=await n("worker-attempt-pause",{attempt_id:$});w&&w.paused===!1&&w.reason&&he(`\uC77C\uC2DC\uC815\uC9C0 \uAC70\uBD80: ${w.reason}`,"error",2400)}async function J($,w="session"){if(!n||!$)return;let O=n,fe=Y().attempts?.[$]||null;await Qr({context:{bead_id:fe?.bead_id||"",kind:w,tuple:fe?bn(fe):""},transport:xe=>O("worker-attempt-resume",{attempt_id:$,expected_revision:He(),...xe}),adopt:Ie})}async function Ae($,w,O=!0){if(!n)return null;let fe=n,xe=await fe($,{...w,expected_revision:He()});return Ie(xe),xe&&xe.conflict&&O&&(xe=await fe($,{...w,expected_revision:He()}),Ie(xe)),xe}async function Pe($){if(!n||!$)return;let w=Y().merge_queue?.find(fe=>fe.bead_id===$)?.continuation_action;if(w?.mismatch&&w.continuation===null){await Nt($,w.mismatch);return}H.add($),h();let O;try{O=await Ae("worker-merge-queue-add",{bead_id:$})}catch{he("\uBA38\uC9C0 \uD074\uB9AD\uC774 \uC11C\uBC84\uC5D0 \uC804\uB2EC\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4(\uC5F0\uACB0 \uBB38\uC81C) \u2014 \uC5F0\uACB0 \uBCF5\uAD6C \uD6C4 \uB2E4\uC2DC \uB20C\uB7EC\uC8FC\uC138\uC694","error",3200);return}finally{H.delete($),h()}if(!(!O||O.applied)){if(O.conflict){he("\uD050\uAC00 \uBC14\uB00C\uC5B4 \uBA38\uC9C0 \uD074\uB9AD\uC774 \uC801\uC6A9\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4 \u2014 \uB2E4\uC2DC \uB20C\uB7EC\uC8FC\uC138\uC694","error",2400);return}he(lw(O.reason),"error",2400)}}async function Ye($){if(!(!n||!$||ne.has($))){ne.add($),h();try{let w=await n("worker-cleanup-retry",{bead_id:$,expected_revision:He()});Ie(w),w&&!w.retried&&!w.conflict&&w.reason&&he(`\uC815\uB9AC \uC7AC\uC2DC\uB3C4 \uAC70\uBD80: ${w.reason}`,"error",2400)}finally{ne.delete($),h()}}}async function tt($){if(!(!n||!$||be.has($))){be.add($),h();try{let w=await n("worker-resolve-in-session",{bead_id:$,expected_revision:He()});Ie(w),he(dw(w),pw(w),4e3)}finally{be.delete($),h()}}}async function ct($,w){let O=Y().hold;if(!n||!O||typeof O.since!="number")return;let fe=await n($,{since:O.since});Ie(fe),fe&&fe.ok===!1&&he(`${w}: ${fe.reason==="hold_changed"?"\uD050 \uC0C1\uD0DC\uAC00 \uBC14\uB00C\uC5C8\uC2B5\uB2C8\uB2E4 \u2014 \uB2E4\uC2DC \uD655\uC778\uD558\uC138\uC694":fe.reason||""}`,"error",2800)}async function Rt($,w){if(!n||!$||!w)return;let O=await n("worker-parked-retry",{bead_id:$,attempt_id:w});Ie(O),O&&O.ok===!1&&he(`\uC7AC\uC2DC\uB3C4 \uAC70\uBD80: ${O.reason==="not_latest"?"\uC774 bead\uC5D0 \uB354 \uC0C8\uB85C\uC6B4 \uC2DC\uB3C4\uAC00 \uC788\uC2B5\uB2C8\uB2E4":O.reason||""}`,"error",2800)}async function Nt($,w){let O=await ur({continuation_mismatch:w},(xe,We)=>Ae("worker-merge-queue-add",{bead_id:$,continuation:xe,decision_token:We},!1)),fe=O?.queue?.merge_queue?.find(xe=>xe.bead_id===$)?.continuation_action;if(O?.applied!==!0&&fe?.continuation===null&&fe.mismatch){await Nt($,fe.mismatch);return}O&&O.applied===!1&&!O.conflict&&he("\uC774\uC5B4\uD558\uAE30 \uC120\uD0DD\uC774 \uCD5C\uC2E0 \uC0C1\uD0DC\uC640 \uC77C\uCE58\uD558\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4","error",2800)}async function mt($){if(!n)return;let w=await Ae("worker-merge-auto-toggle",{on:$});!w||w.conflict||he($?"\uC790\uB3D9 \uBA38\uC9C0 \uCF1C\uC9D0 \u2014 \uC790\uACA9\uC774 \uC0DD\uAE30\uB294 PR\uC744 \uACC4\uC18D \uBA38\uC9C0\uD569\uB2C8\uB2E4":"\uC790\uB3D9 \uBA38\uC9C0 \uAEBC\uC9D0 \u2014 \uB300\uAE30 \uD56D\uBAA9\uC744 \uBE44\uC6E0\uC2B5\uB2C8\uB2E4",$?"success":"info",2400)}async function ht($){if(!n||!$)return;let w=await Ae("worker-merge-queue-remove",{bead_id:$});w&&!w.conflict&&!w.applied&&w.reason==="merge_active"&&he("\uBA38\uC9C0 \uC9C4\uD589 \uC911 \u2014 \uCDE8\uC18C\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4","error",2400)}async function gt(){await Ae("worker-merge-queue-remove",{all:!0})}async function It($,w=null,O="unmerged",fe=null){if(!n||!$)return;let xe=Wo($,O);if(!(!!fe||typeof globalThis.confirm!="function"||globalThis.confirm(xe)))return;let Ze=await n("worker-discard",{bead_id:$,...w?{attempt_id:w}:{},...fe?{operation_id:fe}:{},expected_revision:He()});if(Ie(Ze),Ze&&Ze.conflict&&(Ze=await n("worker-discard",{bead_id:$,...w?{attempt_id:w}:{},...fe?{operation_id:fe}:{},expected_revision:He()}),Ie(Ze)),Ze&&Ze.discarded===!0){he(li(Ze),"success",5e3);return}if(Ze&&Ze.reason){he(`\uD3D0\uAE30 \uC2E4\uD328: ${Ze.reason}`,"error",2800);return}if(Ze&&Ze.accepted&&Ze.pending==="merged_revert"){he("revert PR \uB300\uAE30 \uC0C1\uD0DC\uB85C \uC804\uD658\uD588\uC2B5\uB2C8\uB2E4","success",2400);return}if(Ze&&Ze.accepted&&!Ze.discarded){he(`\uD3D0\uAE30 \uC9C4\uD589: ${Ze.phase||"\uBC31\uC5C5 \uC911"}`,"success",2400);return}Ze&&!Ze.conflict&&he("\uD3D0\uAE30 \uAC70\uBD80: unknown","error",2800)}async function bt($,w,O){if(!(!n||!w||!O||ee.has(w))){ee.add(w),h();try{let fe=await n($,{bead_id:w,action_id:O,expected_revision:He()});Ie(fe),fe?.conflict?he("\uC774\uC804 \uC791\uC5C5 \uC0C1\uD0DC\uAC00 \uBC14\uB00C\uC5C8\uC2B5\uB2C8\uB2E4. \uCD5C\uC2E0 \uC0C1\uD0DC\uB97C \uD655\uC778\uD558\uC138\uC694.","error",2800):!fe?.ok&&fe?.reason&&he(`\uC774\uC804 \uC791\uC5C5 \uCC98\uB9AC \uAC70\uBD80: ${String(fe.reason)}`,"error",2800)}finally{ee.delete(w),h()}}}async function le($,w){if(!n||!w||B.has(w))return;B.add(w),h();let O;try{let fe=async(xe={})=>await n($,{bead_id:w,expected_revision:He(),...xe});O=await fe(),Ie(O),O&&O.conflict&&(O=await n($,{bead_id:w,expected_revision:He()}),Ie(O)),$==="worker-revise-fix"&&(O=await ur(O,(xe,We)=>fe({continuation:xe,decision_token:We}),{onResult:Ie,refresh:()=>fe()}))}finally{B.delete(w),h()}if(!(!O||O.conflict)){if(O.ok){he($==="worker-revise-fix"?"\uCC98\uBD84 \uC138\uC158\uC744 \uB744\uC6E0\uC2B5\uB2C8\uB2E4 \u2014 \uC218\uB9AC \uD6C4 \uAD6C\uD604\uC774 \uC7AC\uB514\uC2A4\uD328\uCE58\uB429\uB2C8\uB2E4":"\uB378\uD0C0 \uC2B9\uC778 \uC644\uB8CC \u2014 \uC601\uC218\uC99D \uAC31\uC2E0 + \uD30C\uD0B9 \uD574\uC81C","success",2800);return}he(`\uCC98\uBD84 \uAC70\uBD80: ${O.reason||""}`,"error",3e3)}}async function ae($){if(!n)return;let w=await n("worker-automation-toggle",{on:$,expected_revision:He()});Ie(w),w&&w.conflict&&await n("worker-automation-toggle",{on:$,expected_revision:He()}).then(Ie)}async function A($){if(!n||!$)return;let w=await n("worker-repo-operation-dismiss",{operation_id:$});Ie(w),w&&w.ok===!1&&he(`\uAE30\uB85D \uB2EB\uAE30 \uAC70\uBD80: ${w.reason||""}`,"error",3e3)}async function q($){if(!n||!Number.isFinite($))return;let w=Math.max(na,Math.floor($)),O=await n("worker-queue-set-slots",{slots:w,expected_revision:He()});Ie(O),O&&O.conflict&&await n("worker-queue-set-slots",{slots:w,expected_revision:He()}).then(Ie)}async function oe($){if(!n||!Number.isInteger($)||$<1||$>r_)return;let w=Y(),O=(Array.isArray(w.serial_lanes)?w.serial_lanes:[]).slice($).reduce((We,Ze)=>We+(Array.isArray(Ze?.entries)?Ze.entries.length:0),0),fe=()=>({count:$,expected_revision:He()}),xe=await n("worker-queue-set-serial-lane-count",fe());Ie(xe),xe&&xe.conflict&&(xe=await n("worker-queue-set-serial-lane-count",fe()),Ie(xe)),xe&&xe.applied&&O>0&&he(`\uC9C1\uB82C \uB808\uC778 \uCD95\uC18C \u2014 ${O}\uAC1C \uD56D\uBAA9\uC774 \uBCD1\uB82C \uB300\uAE30\uB85C \uC774\uB3D9`)}function W(){let $=Tr(R),w=re.read({candidate_sort:F,done_since:$});return ie=w.workspaces,pe=_r(w.workspaces,w.workspaces_state,{done_since:$,candidate_filter:_,candidate_hidden_counts:"per_control",candidate_sort:"as_given",groups:"all",search:Z}),pe}function ve($){return $.queue_groups[0]||tw}function qe($){let w=$.dependency_chips||null,O={...w&&w.released?{released:w.released}:{},...w&&w.dependents?{dependents:w.dependents}:{}},fe=X.get($.id),xe=se.get($.id)||null,We=fe&&fe.overlaps.length>0?fe.overlaps:null,Ze=!!fe&&fe.scope_missing;return!xe&&!We&&!Ze&&Object.keys(O).length===0?null:{...O,...xe?{predecessors:xe}:{},...We?{overlaps:We}:{},...Ze?{scope_missing:!0}:{}}}function Xe($){return{...$,workspace_name:"",done_layout:void 0,dependency_chips:qe($)||void 0,chip_popover:Qe($)}}function Qe($){return mi($,w=>U.isOpen({bead_id:$.id,chip_key:w}))}function Fe(){let $=Y(),w=new Map;for(let O of Object.values(hn($.lane_states))){let fe=Array.isArray(O?.corrections)?O.corrections:[];for(let xe of fe)xe&&typeof xe.bead_id=="string"&&typeof xe.after=="string"&&w.set(xe.bead_id,xe.after)}return{admission:hn($.admission),correction_after:w}}function wt($,w){let O=Xe($),fe=Vu(w.admission[$.id]||null,!!$.discard||ee.has($.id)),xe=w.correction_after.get($.id);return{...O,draggable:O.draggable===!0&&!fe,stale_work:fe,reason:fe?"":O.reason,badges:xe?[`\u{1F517} ${xe} \uB4A4 (blocks \uC790\uB3D9)`,...O.badges||[]]:O.badges,revise_enabled:O.revise_enabled===!0&&!B.has($.id)}}function Et($){let w=Fe();return ve($).sublanes.parallel.map(O=>wt(O,w))}function xt($){let w=Fe();return ve($).sublanes.serial.map(O=>{let fe=O.occupants.map(xe=>({id:xe.id,title:xe.title,draggable:!1,lane:O.id,ghost:!0,badges:[xe.badge],...typeof xe.search_match=="boolean"?{search_match:xe.search_match}:{}}));return{id:O.id,index:O.index+1,raw_length:O.raw_length,ghosts:fe,items:O.items.map(xe=>wt(xe,w)),occupied:O.occupied_by.length>0,badge:O.occupants.length>0?O.occupants[0].badge:"\uB300\uAE30",cycle:O.cycle===!0}})}function Zt($){return $.runnable.map(w=>Xe(w))}function qt($){return $.done.map(w=>Xe(w))}function At($){let w=$.running.filter(O=>O.non_occupying!==!0).map(O=>({...O,bead_id:O.id,attempt_id:O.attempt_id||"",paused:O.run_state==="paused",failed:O.run_state==="failed",parked:O.run_state==="parked",retry_wait:O.run_state==="retry_wait",waiting:O.run_state==="waiting",wait:O.wait||null,status_label:O.run_state==="failed"?O.status==="orphaned"?"\uC911\uB2E8\uB428":"\uC2E4\uD328":O.run_state==="parked"?"\uC138\uC158 \uB300\uAE30":O.run_state==="retry_wait"?"\uC7AC\uC2DC\uB3C4 \uB300\uAE30":O.run_state==="waiting"?"\uC120\uD589 \uB300\uAE30":void 0,can_pause:O.can_pause!==!1,workspace_name:"",dependency_chips:qe(O)||void 0,chip_popover:Qe(O),rollup_expanded:$e.has(O.id),failure:O.failure?{...O.failure,open:C===O.attempt_id}:null,...Ce(O.id,O.discard)}));return[...w.filter(O=>O.failed===!0),...w.filter(O=>O.failed!==!0&&O.parked===!0),...w.filter(O=>O.failed!==!0&&O.parked!==!0)]}function Gt($){return dt($).map(w=>({...w,chip_popover:Qe(w)}))}function dt($){if(Ee&&Ee.model===$)return Ee.rows;let w=Y(),O=ve($),fe=hn(w.attempts),xe=Object.values(fe).filter(Xn),We=new Map;for(let Ke of xe)We.set(Ke.attempt_id,Ke);let Ze=new Map;for(let Ke of xe)Ze.set(Ke.bead_id,Ke);let St=new Map;for(let Ke of[...$.pr_wait,...$.running,...$.queue,...$.runnable,...$.done])St.has(Ke.id)||St.set(Ke.id,Ke);let zt=Ke=>{let Pt=null;for(let $n of xe)!$n||$n.bead_id!==Ke||Za($n,We)||(Pt===null||(typeof $n.started_at=="number"?$n.started_at:0)>=(typeof Pt.started_at=="number"?Pt.started_at:0))&&(Pt=$n);return Pt&&typeof Pt.target_base=="string"?Pt.target_base:null},Yt=new Map;for(let Ke of $.running)Ke.run_state==="failed"||Ke.conflict_resolution!==!0||(Ke.run_state!=="paused"?Yt.set(Ke.id,"running"):Yt.has(Ke.id)||Yt.set(Ke.id,"paused"));let Sn=hn(w.auto_merge_skips),kn=new Set(O.merge.auto_excluded),wr=hn(w.pr_observations),Dn=hn(w.pr_activity),Wn=hn(w.cleanup_failed),Nn=hn(w.discard_operations),Xt=hn(w.bead_workflow),zn=hn(w.bead_titles),kr=w.merge_queue_state||{active:null,failures:{}},or=O.merge.state.waiting,En=new Map;for(let Ke of Array.isArray(w.merge_queue)?w.merge_queue:[])Ke&&typeof Ke=="object"&&Ke.bead_id&&En.set(Ke.bead_id,Ke);let sr=(Array.isArray(w.pr_wait)?w.pr_wait:[]).map(Ke=>{let Pt=St.get(Ke.bead_id);return{...$w(Ke.bead_id,Pt?.title||zn[Ke.bead_id]||Ke.bead_id,wr,Wn[Ke.bead_id]||null,Vn(fe,Ke.bead_id),Dn[Ke.bead_id]||(H.has(Ke.bead_id)?{activity:null,merge_progress:null,queueing:"merge"}:ne.has(Ke.bead_id)?{activity:null,merge_progress:null,queueing:"cleanup"}:null),Yt.get(Ke.bead_id)||null,Ke.external===!0,{position:O.merge.positions.get(Ke.bead_id)||0,active:kr.active===Ke.bead_id,failure:hn(kr.failures)[Ke.bead_id]||null,waiting:or&&or.bead_id===Ke.bead_id?or.reason:null,resolution:O.merge.resolutions.get(Ke.bead_id),continuation_action:O.merge.continuations.get(Ke.bead_id),authority:O.merge.authorities.get(Ke.bead_id)||null,hold:En.get(Ke.bead_id)?.hold||null,review_dispatch:En.get(Ke.bead_id)?.review_dispatch||null},Ke.wt_present!==!1,w.auto_merge===!0&&kn.has(Ke.bead_id)?Sn[Ke.bead_id]?.reason||"":null,Qa(O.declared_base,zt(Ke.bead_id)),hn(w.completion_status)[Ke.bead_id]||null,Nn,w.auto_merge===!0,{merge_sha:Ke.merge_sha,cleanup_cursor:Ke.cleanup_cursor,repo_operations:O.repo_operations},Pt?qe(Pt):null,Hu(fe,Ke.bead_id),be.has(Ke.bead_id)),...Pt?.search_match===void 0?{}:{search_match:Pt.search_match},workflow:Xt[Ke.bead_id]||null,priority:Pt?.priority,from_id:Pt?.from_id,...Pt?.created_at===void 0?{}:{created_at:Pt.created_at},...Pt?.updated_at===void 0?{}:{updated_at:Pt.updated_at}}});return Ee={model:$,rows:sr},sr}function Wt($){let w=ve($),O=[];for(let We of $.running)We.non_occupying!==!0&&O.push({id:We.id,title:We.title,location_label:"\uC2E4\uD589\uC911",kind:"running",lane_id:We.serial_lane_id??null});for(let We of $.pr_wait)O.push({id:We.id,title:We.title,location_label:"PR \uB300\uAE30",kind:"pr_wait",lane_id:null});for(let We of w.sublanes.serial)We.items.forEach((Ze,St)=>{O.push({id:Ze.id,title:Ze.title,location_label:`${We.id} #${St+1}`,kind:"serial",lane_id:We.id})});w.sublanes.parallel.forEach((We,Ze)=>{O.push({id:We.id,title:We.title,location_label:`#${Ze+1}`,kind:"parallel",lane_id:null})});for(let We of $.runnable)O.push({id:We.id,title:We.title,location_label:"\uD6C4\uBCF4",kind:"candidate",lane_id:null,queue_placeable:We.queue_placeable===!0});let fe=Y();X=Bf(fe.bead_scope,O);let xe=new Map;for(let We of[...$.running,...$.runnable])Array.isArray(We.blocked_by)&&We.blocked_by.length>0&&xe.set(We.id,We.blocked_by);for(let[We,Ze]of Object.entries(hn(fe.bead_blocked_by)))Array.isArray(Ze)&&xe.set(We,Ze.filter(St=>typeof St=="string"&&St.length>0));se=id(xe,O,hn(fe.blocker_workspaces))}function Jt($){let w=$.hold&&typeof $.hold=="object"?$.hold:null;if(!w||w.kind!=="env"&&w.kind!=="systemic")return"";let O=hr(w.cause)||String(w.cause||""),fe=Array.isArray($.lineages)?$.lineages:[];if(w.kind==="env"){let We=fe.map(St=>St&&St.next_at).filter(St=>typeof St=="number").sort((St,zt)=>St-zt)[0],Ze=typeof We=="number"?` \xB7 \uB2E4\uC74C ${new Date(We).toLocaleTimeString("ko-KR",{hour:"2-digit",minute:"2-digit"})}`:"";return c`<div class="worker-hold worker-hold--env" role="status">
      ...
      </div>`}let xe=(Array.isArray(w.bead_ids)?w.bead_ids:[]).filter(We=>typeof We=="string"&&We.length>0);return c`<div class="worker-hold worker-hold--systemic" role="alert">
      <span class="worker-hold__text"
    ...
    </div>`}function jt($){let w=Y(),O=ve($),fe=O.sublanes.parallel,xe=fe.length>0?fe[0].id:"\u2014",We=c`<button
      type="button"
    ...
    >
    ...
    </div>`}function rn($){let w=ve($).merge,O=Y().auto_merge===!0;if(w.running)return c`<button
        type="button"
      ...
      >
        ⏸ 자동 머지
      </button>`;let fe=new Set(w.auto_excluded),xe=Gt($).filter(We=>We.merge_action&&We.merge_enabled&&!fe.has(We.id)).length;return c`<button
      type="button"
    ...
    </div>`}function S($){let w=Et($),O=Ne();return hi({parallel:{rows:w.map((fe,xe)=>we(fe,{kind:"parallel",root_dir:O,row_index:xe})),count:w.length,collapsed:z.isAreaCollapsed("parallel"),drop:{drop:"parallel",root_dir:O}},serial:{lanes:xt($).map(fe=>({id:fe.id,title:`\uC9C1\uB82C ${fe.index}`,rows:[...fe.ghosts.map(xe=>Rn({...xe,...Ce(xe.id,xe.discard)},{actions:po(xe)})),...fe.items.map((xe,We)=>we(xe,{kind:"repo-serial",root_dir:O,row_index:We,lane_id:fe.id}))],count:fe.ghosts.length+fe.items.length,match_count:K([...fe.ghosts,...fe.items]),empty:fe.ghosts.length+fe.items.length===0,badge:fe.badge,held:fe.occupied,cycle:fe.cycle,drop:{drop:"repo-serial",root_dir:O,lane_id:fe.id,lane_length:String(fe.raw_length)}})),collapsed:z.isAreaCollapsed("serial")}})}function me($){return Yp(At($),Date.now(),Te)}function m($){return $.running.some(w=>w.kind!=="session"&&w.run_state==="running")}function b($){let w=ve($),O=Zt($),fe=Et($),xe=qt($),We=Gt($),Ze=At($),St=Un({id:"worker-pane-candidate",lane:"candidate",title:"\uD6C4\uBCF4",items:O,match_count:K(O),src:!0,empty:"\uD6C4\uBCF4 \uC5C6\uC74C",header_control:nn(),header_row:N?Ut():void 0,controls:sn($),collapsible:!0,collapsed:z.isCollapsed("candidate"),place_menu:Q(O),onOpenDoc:u?(Yt,Sn)=>u(Sn):void 0}),zt=Un({id:"worker-pane-done",lane:"done",title:"\uC644\uB8CC",items:xe,match_count:K(xe),empty:`${P()} \uC644\uB8CC \uC5C6\uC74C`,header_control:an(),collapsible:!0,collapsed:z.isCollapsed("done"),preview:V?Array.isArray(w.token_total)?w.token_total.map(Yt=>Yt.label).join(" \xB7 "):w.token_total||o_(xe):void 0});return V?c`<div class="worker-lanes worker-lanes--mobile">
        ${bi({live:m($),running_body:Ze.length>0?me($):"",pr_wait_rows:We.map(Yt=>Rn(Yt)),count:Ze.length+We.length})}
        ${Un({id:"worker-pane-queue",lane:"queue",title:"\uB300\uAE30",items:fe,count:fe.length,match_count:K(fe),collapsible:!0,collapsed:z.isCollapsed("queue"),preview:o_(fe),body:S($)})}
        ${St} ${zt}
      </div>`:c`<div class="worker-lanes">
      ${St}
      ${Un({id:"worker-pane-queue",lane:"queue",title:"\uB300\uAE30",items:fe,count:fe.length,match_count:K(fe),collapsible:!0,collapsed:z.isCollapsed("queue"),body:S($)})}
      ${Un({id:"worker-pane-running",lane:"running",title:"\uC2E4\uD589 \uC911",items:Ze,match_count:K(Ze),header_control:c`<span class="worker-pane__meta"
          >슬롯 ${w.slots}</span
        >`,live:m($),collapsible:!0,collapsed:z.isCollapsed("running"),body:me($)})}
      ${Un({id:"worker-pane-pr-wait",lane:"pr_wait",title:"PR \uB300\uAE30",items:We,match_count:K(We),empty:"PR \uB300\uAE30 \uC5C6\uC74C",collapsible:!0,collapsed:z.isCollapsed("pr_wait")})}
      ${zt}
    </div>`}function M($){z.toggle($),h()}function f($){z.toggleArea($),h()}function h(){let $=W();Wt($),ot(jt($),Le),ot(b($),I)}function j(){let $=!0,w=Gi(O=>{if(V=O,$){$=!1;return}h()});E.push(w)}function te($){_=$,rw($),h()}function _e($){if($==="custom"){N=!0,h();return}F=jr($),jl(F),N=!1,h()}function Re($){F=jr({chain:$}),jl(F),h()}function pt($){R=jn($),iw(R),p?.(R),h()}function ut($){let w=$.target?.closest?.(".worker-serial-lane-count");if(w){let zt=Number.parseInt(w.value,10);Number.isFinite(zt)&&oe(zt).then(h);return}let O=$.target?.closest?.(".worker-filter__blocked");if(O){te({..._,show_blocked:O.checked});return}let fe=$.target?.closest?.(".worker-sort-chain__key");if(fe){let zt=Number.parseInt(fe.getAttribute("data-step")||"",10);Number.isFinite(zt)&&Re(qf(gs(F),zt,fe.value));return}let xe=$.target?.closest?.(".worker-done-range");if(xe){pt(xe.value);return}let We=$.target?.closest?.(".worker-sort");if(We){_e(We.value);return}let Ze=$.target?.closest?.(".worker-slots__input");if(!Ze)return;let St=Number.parseInt(Ze.value,10);if(!Number.isFinite(St)){h();return}q(St).then(h)}function lt($){return $?{runner:$.runner||void 0,model:$.model||void 0,effort:$.effort||void 0,worktree:$.worktree||void 0,status:$.status||void 0,session_id:$.session_id||void 0}:{}}function Tt(){let $=ve(W()),w=Y().workspace_info,O=w&&typeof w=="object"&&w.repo_ops&&typeof w.repo_ops=="object"?w.repo_ops:null;return{operations:$.repo_operations,cleanup_failures:$.cleanup_failures,repo:l&&l()||"",repo_ops:O}}function x(){Te&&de.close(),rt.hidden=!1,ce.hidden=!1,De.open(Tt()),h()}function T($){let w=Y(),O=w.attempts?w.attempts[$]:null;Te=$,De.close(),rt.hidden=!0,ce.hidden=!1,de.open({attempt_id:$,meta:lt(O)}),h()}function Se($){let w=Y(),O=(Array.isArray(w.session_active)?w.session_active:[]).find(xe=>xe&&xe.bead_id===$),fe=(O&&Array.isArray(O.session_refs)?O.session_refs:[]).find(xe=>xe&&xe.current===!0);fe&&(De.close(),rt.hidden=!0,ce.hidden=!1,de.open(Zr(fe,$,"in_progress")),h())}function Be(){if(De.isOpen()&&De.refresh(Tt()),!Te)return;let $=Y(),w=$.attempts?$.attempts[Te]:null;if(w){de.updateMeta(lt(w));return}de.close()}function nt($,w){if($.length===0||!i)return;let O=l?l():void 0;if(w.length===0||!O||w===O||!a){i($);return}Promise.resolve(a(w)).then(()=>{i($)}).catch(()=>{he("\uB808\uD3EC \uC804\uD658\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4","error",2400)})}function ft($){let w=$.target;if(w?.closest?.(".worker-mini__grip"))return;let O=w?.closest?.(".worker-sort-chain__dir");if(O){let Me=Number.parseInt(O.getAttribute("data-step")||"",10);Number.isFinite(Me)&&Re(jf(gs(F),Me));return}let fe=w?.closest?.(".worker-dep__open");if(fe){nt(fe.getAttribute("data-dep-id")||"",fe.getAttribute("data-root-dir")||"");return}let xe=w?.closest?.(".judgement-chip");if(xe){let Me=xe.closest("[data-bead-id]"),Ot=Me&&Me.getAttribute("data-bead-id")||"",Ue=xe.getAttribute("data-chip-key")||"";Ot&&Ue&&U.toggle({bead_id:Ot,chip_key:Ue});return}if(w?.closest?.(".chip-popover"))return;if(w?.closest?.(".worker-repo-strip")){x();return}let We=w?.closest?.(".worker-repo-op__dismiss");if(We){A(We.dataset.operationId||"");return}let Ze=w?.closest?.(".worker-cleanup__resume");if(Ze){let Me=Ze.dataset.beadId;Me&&Ye(Me);return}let St=w?.closest?.(".worker-cleanup__resolve");if(St){let Me=St.dataset.beadId;Me&&tt(Me);return}if(w?.closest?.(".worker-hold__retry")){ct("worker-queue-hold-retry-now","\uC9C0\uAE08 \uC7AC\uC2DC\uB3C4 \uAC70\uBD80");return}if(w?.closest?.(".worker-hold__resume")){ct("worker-queue-hold-resume","\uC7AC\uAC1C \uAC70\uBD80");return}if(w?.closest?.(".worker-play")){ae(!Y().auto_advance);return}let zt=w?.closest?.(".worker-merge-all");if(zt){zt.classList.contains("worker-merge-all--stop")?Y().auto_merge===!0?mt(!1):gt():mt(!0);return}let Yt=w?.closest?.(".worker-pane__toggle[data-lane]");if(Yt){let Me=Yt.dataset.lane;(Me==="candidate"||Me==="queue"||Me==="running"||Me==="pr_wait"||Me==="done")&&M(Me);return}let Sn=w?.closest?.(".worker-wait__area-toggle[data-area]");if(Sn){let Me=Sn.dataset.area;(Me==="parallel"||Me==="serial")&&f(Me);return}let kn=w?.closest?.(".worker-card__place-lane");if(kn){let Me=kn.dataset.beadId,Ot=kn.dataset.lane;Me&&(Ot==="parallel"||/^s[1-5]$/.test(Ot||""))&&(v=null,h(),at(Me,Ot));return}if(w?.closest?.(".worker-card__place-cancel")){v=null,h();return}let Dn=w?.closest?.(".worker-card__place");if(Dn){let Me=Dn.dataset.beadId;Me&&!Dn.disabled&&(Bo(Y())?(v=Me,h()):at(Me,"parallel"));return}let Wn=w?.closest?.(".worker-filter__chip");if(Wn){let Me=Wn.dataset.spec;(Me==="all"||Me==="with"||Me==="without")&&te({..._,spec:Me});return}let Nn=w?.closest?.('[data-action="queue-remove"]');if(Nn){let Me=Nn.dataset.beadId||"";Me&&ue.sendOp({type:"worker-queue-remove",payload:{bead_id:Me},root_dir:Ne()},Me);return}let Xt=w?.closest?.(".worker-mini__merge");if(Xt){let Me=Xt.dataset.beadId||"";Y().cleanup_failed?.[Me]?Ye(Me):Pe(Me);return}let zn=w?.closest?.(".worker-mini__merge-cancel");if(zn){ht(zn.dataset.beadId||"");return}let kr=w?.closest?.(".worker-mini__resolve");if(kr){tt(kr.dataset.beadId||"");return}let or=w?.closest?.(".rtile__resolve");if(or){let Me=or.closest(".rtile");tt(Me?.dataset.beadId||"");return}let En=w?.closest?.(".worker-mini__discard");if(En){It(En.dataset.beadId||"",En.dataset.attemptId||null,En.dataset.discardMode==="merged"?"merged":"unmerged",En.dataset.operationId||null);return}let sr=w?.closest?.(".worker-mini__stale-continue");if(sr){bt("worker-stale-work-continue",sr.dataset.beadId||"",sr.dataset.actionId||"");return}let Ke=w?.closest?.(".worker-mini__stale-backup");if(Ke){bt("worker-stale-work-backup-fresh",Ke.dataset.beadId||"",Ke.dataset.actionId||"");return}let Pt=w?.closest?.(".worker-mini__stale-recheck");if(Pt){bt("worker-stale-work-recheck",Pt.dataset.beadId||"",Pt.dataset.actionId||"");return}let $n=w?.closest?.(".worker-mini__revise-fix");if($n){le("worker-revise-fix",$n.dataset.beadId||"");return}let hs=w?.closest?.(".worker-mini__revise-approve");if(hs){le("worker-revise-approve",hs.dataset.beadId||"");return}if(w?.closest?.(".worker-mini__pr"))return;let bs=w?.closest?.(".rtile__failure-badge");if(bs){let Me=bs.dataset.attemptId||"";C=C===Me?null:Me,h();return}let ys=w?.closest?.(".rtile__attempt-copy");if(ys){let Me=ys.dataset.attemptId||"";Me&&cn(Me).then(Ot=>{he(Ot?"\uBCF5\uC0AC\uB428":"\uBCF5\uC0AC \uC2E4\uD328",Ot?"success":"error",1400)});return}if(w?.closest?.(".rtile__parked-retry")){let Me=w?.closest?.(".rtile");Rt(Me?.dataset?.beadId||"",Me?.dataset?.attemptId||"");return}let vo=w?.closest?.(".rtile__discard");if(vo){let Me=w?.closest?.(".rtile"),Ot=Me?.dataset?.beadId,Ue=Me?.dataset?.attemptId;Ot&&It(Ot,Ue||null,vo.dataset.confirmation==="merged"?"merged":"unmerged",vo.dataset.operationId||null);return}if(w?.closest?.(".rtile__pause")){let Ot=w?.closest?.(".rtile")?.dataset?.attemptId;Ot&&k(Ot);return}if(w?.closest?.(".rtile__resume")){let Me=w?.closest?.(".rtile__resume"),Ue=w?.closest?.(".rtile")?.dataset?.attemptId;Ue&&J(Ue,Me?.dataset?.resumeKind==="settlement"?"settlement":"session");return}if(w?.closest?.(".rtile__session")){let Me=w?.closest?.(".rtile"),Ot=Me?.dataset?.attemptId;if(Ot){T(Ot);return}let Ue=Me?.dataset?.beadId;Ue&&Se(Ue);return}if(w?.closest?.(".rtile__failure-pop"))return;if(w?.closest?.(".worker-drawer-overlay__backdrop")){De.close(),de.close();return}if(w?.closest?.(".worker-drawer-host"))return;let vs=w?.closest?.(".rtile .board-card__roll-toggle");if(vs){let Me=vs.dataset.rollParent;Me&&($e.has(Me)?$e.delete(Me):$e.add(Me),h());return}let ws=w?.closest?.(".rtile .board-card__roll-child");if(ws){let Me=ws.dataset.childId;Me&&i&&i(Me);return}let wo=w?.closest?.(".rtile");if(wo){if(w?.closest?.(".rtile__id")){let Ot=wo.dataset.beadId;Ot&&cn(Ot).then(Ue=>{Ue?he("\uBCF5\uC0AC\uB428","success",1200):he("\uBCF5\uC0AC \uC2E4\uD328","error",1600)});return}let Me=wo.dataset.beadId;Me&&i&&i(Me);return}let ks=w?.closest?.(".worker-mini, .worker-card");if(ks){let Me=ks.dataset.beadId;if(w?.closest?.('[data-seam="log-path-copy"]'))return;if(w?.closest?.(".worker-mini__id, .worker-card__id")){Me&&cn(Me).then(Ue=>{Ue?he("\uBCF5\uC0AC\uB428","success",1200):he("\uBCF5\uC0AC \uC2E4\uD328","error",1600)});return}let Ot=w?.closest?.(".ctl-chip--from");if(Ot){let Ue=Ot.dataset.fromId;Ue&&i&&i(Ue);return}Me&&i&&i(Me)}}function Ft($){let w=$.target;w?.closest?.(".worker-search")&&(Z=w.value,h())}function yo($){let w=$.target;$.key!=="Escape"||!w?.closest?.(".worker-search")||Z.length===0||(Z="",h())}ue.attach(e),e.addEventListener("click",ft),e.addEventListener("change",ut),e.addEventListener("input",Ft),e.addEventListener("keydown",yo);function _n($){let w=$.target,O=w&&typeof w.closest=="function"?fe=>w.closest(fe):()=>null;C&&!O(".rtile__failure-pop, .rtile__failure-badge")&&(C=null,h())}function rr($){$.key!=="Escape"||C===null||(C=null,h())}return document.addEventListener("click",_n),document.addEventListener("keydown",rr),U.attach(),E.push(()=>{document.removeEventListener("click",_n),document.removeEventListener("keydown",rr),U.detach()}),j(),g&&E.push(g.subscribe(()=>{re.notifyIssuesChanged(),h()})),o&&E.push(o.subscribe(()=>{let $=l&&l()||"";$!==Je&&(Je=$,ze.close()),h(),Be()})),h(),{load(){re.ensureSessionDefaults(),h()},refreshSessionDefaults:ye,destroy(){for(let $ of E.splice(0))try{$()}catch{}ue.detach(),e.removeEventListener("click",ft),e.removeEventListener("change",ut),re.destroy();try{de.destroy()}catch{}ce.hidden=!0;try{ze.destroy()}catch{}ot(c``,e)}}}function zl(e){if(!e)return"Unknown";let t=e.split("/").filter(Boolean);return t.length>0?t[t.length-1]:"Unknown"}function l_(e,t,n,r=async()=>{},o=async()=>{}){let s=Lt("views:workspace-picker"),i=null,l=!1,a=!1,u=!1;async function d(P){let V=P.target.value,D=t.getState().workspace?.current?.path||"";if(V&&V!==D){s("switching workspace to %s",V),l=!0,R();try{await n(V)}catch(K){s("workspace switch failed: %o",K)}finally{l=!1,R()}}}async function p(){let P=t.getState(),z=P.workspace?.current?.path||P.workspace?.available?.[0]?.path||"";if(!(!z||a)){s("git-pulling workspace %s",z),a=!0,R();try{await r(z)}catch(V){s("workspace git pull failed: %o",V)}finally{a=!1,R()}}}function g(P){let z=P.target;z&&e.contains(z)||C()}function _(P){P.key==="Escape"&&C()}function v(){u||(u=!0,document.addEventListener("mousedown",g),document.addEventListener("keydown",_),R())}function C(){u&&(u=!1,document.removeEventListener("mousedown",g),document.removeEventListener("keydown",_),R())}function U(){u?C():v()}async function X(P){let z=P.target,V=z.value,Z=z.checked;s("toggling visibility %s \u2192 %s",V,String(Z));try{await o(V,Z)}catch(D){s("workspace visibility toggle failed: %o",D)}}function se(P){return P?c`
      <button
        type="button"
    ...
    `}function N(){let P=t.getState(),z=P.workspace?.current,V=P.workspace?.available||[],Z=new Set(P.workspace?.hidden||[]),D=z?.path||V[0]?.path||"";if(V.length===0)return c``;let K=V.filter(H=>!Z.has(H.path)||H.path===D);if(K.length<=1){let H=K[0]||V[0],ne=zl(H.path);return c`
  ...
  `;ot(n,e);let r=document.getElementById("global-nav"),o=document.getElementById("top-nav"),s=document.getElementById("repo-scope"),i=document.getElementById("usage-meter"),l=document.getElementById("board-root"),a=document.getElementById("worker-root"),u=document.getElementById("monitor-root"),d=document.getElementById("detail-panel");if(i&&Of(i),l&&a&&u&&d){let ye=function(x,T){let Se="Request failed",Be="";if(x&&typeof x=="object"){let ft=x;if(typeof ft.message=="string"&&ft.message.length>0&&(Se=ft.message),typeof ft.details=="string")Be=ft.details;else if(ft.details&&typeof ft.details=="object")try{Be=JSON.stringify(ft.details,null,2)}catch{Be=""}}else typeof x=="string"&&x.length>0&&(Se=x);let nt=T&&T.length>0?`Failed to load ${T}`:"Request failed";re.open(nt,Se,Be)},Ne=function(x){return`${we.getState().workspace.current?.path||""}\0${x}`},at=function(){Te&&(Te().catch(()=>{}),Te=null),de=null,De=null},Ie=function(x){ze=x;let T=()=>{ze!==x||we.getState().selected_id!==x||(ze=null,He(x))};if(!Y){je.then(T);return}T()},Pe=function(x,T,Se,Be,nt){return Se!==Ae[T]?(nt().catch(()=>{}),!1):(x.set(Be,nt),!0)},tt=function(){let x=we.getState();ht(x.view==="board"),A(x.view==="worker"),Xe(qe(x)),oe(x.view==="board"||x.view==="worker"||Ye||!!x.selected_id)},Nt=function(){let x=Tr(ct);return x===void 0?{type:"closed-issues"}:{type:"closed-issues",params:{since:x}}},mt=function(){let x=Tr(Rt);return x===void 0?{type:"closed-issues"}:{type:"closed-issues",params:{since:x}}},ht=function(x){if(x)for(let[T,Se]of Gl){if(k.has(T)||J.has(T))continue;let Be=T===vr?Nt():{type:Se};try{Oe.register(T,Be)}catch(Ft){t("register %s store failed: %o",T,Ft)}J.add(T);let nt=Ae.board,ft=!1;ce.subscribeList(T,Be).then(Ft=>{ft=!Pe(k,"board",nt,T,Ft)}).catch(Ft=>{t("subscribe %s failed: %o",T,Ft),ye(Ft,"board")}).finally(()=>{J.delete(T),ft&&tt()})}else bt()},bt=function(){Ae.board+=1;for(let[x]of Gl){let T=k.get(x);T&&(T().catch(()=>{}),k.delete(x));try{Oe.unregister(x)}catch(Se){t("unregister %s failed: %o",x,Se)}}},A=function(x){if(!x){q();return}for(let[T,Se]of p_){if(le.has(T)||J.has(T))continue;let Be=T===yr?mt():{type:Se};try{Oe.register(T,Be)}catch(Ft){t("register %s store failed: %o",T,Ft)}J.add(T);let nt=Ae.worker,ft=!1;ce.subscribeList(T,Be).then(Ft=>{ft=!Pe(le,"worker",nt,T,Ft)}).catch(Ft=>{t("subscribe %s failed: %o",T,Ft),ye(Ft,"worker")}).finally(()=>{J.delete(T),ft&&tt()})}},q=function(){Ae.worker+=1;for(let[x]of p_){let T=le.get(x);T&&(T().catch(()=>{}),le.delete(x));try{Oe.unregister(x)}catch(Se){t("unregister %s failed: %o",x,Se)}}},oe=function(x){if(!x){W();return}ae||(Le("subscribe-worker-queue",{id:__}).catch(T=>{t("subscribe-worker-queue failed: %o",T)}),ae=()=>Le("unsubscribe-worker-queue",{id:__}))},W=function(){ae&&(ae().catch(()=>{}),ae=null)},qe=function(x){return x.view==="monitor"||x.selected_id!=null},Xe=function(x){if(!x){Qe();return}ve||(Le("subscribe-monitor-pipeline",{id:f_}).catch(T=>{t("subscribe-monitor-pipeline failed: %o",T)}),ve=()=>Le("unsubscribe-monitor-pipeline",{id:f_}))},Qe=function(){ve&&(ve().catch(()=>{}),ve=null)},wt=function(){Fe||(Le("subscribe-ui-order",{id:m_}).catch(x=>{t("subscribe-ui-order failed: %o",x)}),Fe=()=>Le("unsubscribe-ui-order",{id:m_}))},Et=function(){Fe&&(Fe().catch(()=>{}),Fe=null),I.clear()},Zt=function(){xt||(Le("subscribe-display-policy",{id:g_}).catch(x=>{t("subscribe-display-policy failed: %o",x)}),xt=()=>Le("unsubscribe-display-policy",{id:g_}))},qt=function(){xt&&(xt().catch(()=>{}),xt=null),pe.clear()},Gt=function(){At||(Le("subscribe-impl-presets",{id:h_}).catch(x=>{t("subscribe-impl-presets failed: %o",x)}),At=()=>Le("unsubscribe-impl-presets",{id:h_}))},nn=function(x){if(!x)return"Unknown";let T=x.split("/").filter(Boolean);return T.length>0?T[T.length-1]:"Unknown"},te=function(x,T){j.open(x.path,{missing_state:x.missing_state,...T?{workspace:T}:{}})};var p=ye,g=Ne,_=at,v=Ie,C=Pe,U=tt,X=Nt,se=mt,F=ht,N=bt,R=A,P=q,z=oe,V=W,Z=qe,D=Xe,K=Qe,H=wt,ne=Et,be=Zt,Ce=qt,B=Gt,ee=nn,$e=te;let Ee=document.getElementById("header-loading"),E=Mc(Ee),re=Np(e),ge=d_(),Le=E.wrapSend((x,T)=>ge.send(x,T)),ce=Tc(Le),Oe=Cc(),et=Oc(),rt=ac(),I=Rc(),pe=sc(),ie=ic(),ue=lc();ge.on("impl-presets-snapshot",x=>{let T=x;T&&typeof T.revision=="number"&&Array.isArray(T.presets)&&ie.set({revision:T.revision,presets:T.presets})}),ge.on("monitor-pipeline-snapshot",x=>{let T=x;if(!(!T||!Array.isArray(T.workspaces)))try{rt.set(T.workspaces,T.workspaces_state,T.cross_lanes)}catch{}}),ge.on("ui-order-snapshot",x=>{let T=x;if(T&&typeof T.revision=="number")try{I.set({revision:T.revision,order:T.order&&typeof T.order=="object"?T.order:{}})}catch{}}),ge.on("display-policy-snapshot",x=>{let T=x;if(T&&T.policy&&typeof T.policy=="object")try{pe.set(T.policy)}catch{}}),ge.on("session-log-snapshot",x=>{let T=x;if(T&&typeof T.id=="string")try{ue.set(T.id,Array.isArray(T.lines)?T.lines:[],typeof T.last_event_at=="number"?T.last_event_at:null)}catch{}}),ge.on("session-log-append",x=>{let T=x;if(T&&typeof T.id=="string")try{ue.append(T.id,T.event)}catch{}}),ge.on("snapshot",x=>{let T=x,Se=T&&typeof T.id=="string"?T.id:"",Be=Se?Oe.getStore(Se):null;if(Be&&T&&T.type==="snapshot")try{Be.applyPush(T)}catch{}}),ge.on("upsert",x=>{let T=x,Se=T&&typeof T.id=="string"?T.id:"",Be=Se?Oe.getStore(Se):null;if(Be&&T&&T.type==="upsert")try{Be.applyPush(T)}catch{}}),ge.on("delete",x=>{let T=x,Se=T&&typeof T.id=="string"?T.id:"",Be=Se?Oe.getStore(Se):null;if(Be&&T&&T.type==="delete")try{Be.applyPush(T)}catch{}});let Te=null,de=null,De=null,ze=null,Je=()=>{},je=new Promise(x=>{Je=()=>x(void 0)}),Y=!1,Q=!1;async function He(x){let T=Ne(x);if(T===de||T===De)return;De=T;let Se=`detail:${x}`,Be={type:"issue-detail",params:{id:x}};try{Oe.register(Se,Be)}catch(nt){t("register detail store failed: %o",nt)}try{let nt=await ce.subscribeList(Se,Be);if(we.getState().selected_id!==x||Ne(x)!==T){await nt().catch(()=>{});return}Te&&await Te().catch(()=>{}),Te=nt,de=T}catch(nt){t("detail subscribe failed: %o",nt),ye(nt,"issue details")}finally{De===T&&(De=null)}}let k=new Map,J=new Set,Ae={board:0,worker:0},Ye=!1,ct=Rs;try{let x=window.localStorage.getItem(b_);ua(x)&&(ct=x)}catch{}let Rt="today";try{let x=window.localStorage.getItem(Sw);x!==null&&(Rt=jn(x))}catch{}async function gt(x){if(!ua(x)||x===ct)return;ct=x;try{window.localStorage.setItem(b_,x)}catch{}let T=k.get(vr);if(!T)return;k.delete(vr),await T().catch(()=>{});let Se=Nt();try{Oe.register(vr,Se)}catch(Be){t("register %s store failed: %o",vr,Be)}try{let Be=await ce.subscribeList(vr,Se);k.set(vr,Be)}catch(Be){t("re-subscribe %s failed: %o",vr,Be),ye(Be,"board")}}async function It(x){let T=jn(x);if(T===Rt)return;Rt=T;let Se=le.get(yr);if(!Se)return;le.delete(yr),await Se().catch(()=>{});let Be=mt();try{Oe.register(yr,Be)}catch(nt){t("register %s store failed: %o",yr,nt)}try{let nt=await ce.subscribeList(yr,Be);le.set(yr,nt)}catch(nt){t("re-subscribe %s failed: %o",yr,nt),ye(nt,"worker")}}let le=new Map,ae=null,ve=null,Fe=null,xt=null,At=null;async function dt(){xt=null,pe.clear(),At=null,ie.clear(),ae=null,ve=null,k.clear(),le.clear(),Ae.board+=1,Ae.worker+=1,Gt();let x=we.getState().workspace.current?.path;if(x)try{await ge.send("set-workspace",{path:x})}catch(Se){t("workspace restore after reconnect failed: %o",Se);return}Zt();let T=we.getState();ht(T.view==="board"),A(T.view==="worker"),Xe(qe(T)),oe(T.view==="board"||T.view==="worker"||!!T.selected_id)}async function Wt(){t("clearing all subscriptions for workspace switch"),bt(),q(),W(),et.clear(),Et(),wt(),qt(),Zt(),at();let x=we.getState();if(x.selected_id)try{Oe.unregister(`detail:${x.selected_id}`)}catch{}let T=we.getState();ht(T.view==="board"),A(T.view==="worker"),Xe(qe(T)),oe(T.view==="board"||T.view==="worker"||!!T.selected_id),T.selected_id&&Ie(T.selected_id)}async function Jt(x){t("requesting workspace switch to %s",x),Q=!0;try{let T=await ge.send("set-workspace",{path:x});t("workspace switch result: %o",T),T&&T.workspace&&(we.setState({workspace:{current:{path:T.workspace.root_dir,database:T.workspace.db_path}}}),window.localStorage.setItem("beads-ui.workspace",x),T.changed&&(await Wt(),he("Switched to "+nn(x),"success",2e3)))}catch(T){throw t("workspace switch failed: %o",T),he("Failed to switch workspace","error",3e3),T}finally{Q=!1}}async function jt(x){t("requesting workspace git pull for %s",x);try{let T=await ge.send("git-pull-workspace",{});t("workspace git pull result: %o",T);let Se=T?.status;if(Se==="up_to_date"){he("Already up to date","success",2e3);return}if(Se==="stash_pop_conflict"){he("Git pulled, but stash pop conflicted (check git stash list)","warning",4e3);return}he("Git pulled "+nn(x),"success",2e3)}catch(T){t("workspace git pull failed: %o",T);let Se=T?.code,Be=T?.message;if(Se==="rebase_conflict"){he("Git pull conflicts \u2014 reverted (manual resolve required)","error",4e3);return}if(Se==="rebase_conflict_abort_failed"){he("Git pull conflicts AND rebase --abort failed \u2014 repo left mid-rebase, run 'git rebase --abort' manually","error",6e3);return}if(Se==="busy"){he("Git pull skipped: another operation is running","warning",3e3);return}let nt=Be?`: ${Be}`:"";throw he(`Git pull failed${nt}`,"error",3e3),T}}async function sn(x,T){t("setting workspace visibility %s \u2192 %s",x,String(T));try{await ge.send("set-workspace-visibility",{path:x,visible:T}),await Ut()}catch(Se){t("workspace visibility update failed: %o",Se),he("Failed to update project visibility","error",3e3)}}async function Ut(){try{let x=await ge.send("list-workspaces",{});if(t("workspaces loaded: %o",x),x&&Array.isArray(x.workspaces)){let T=x.workspaces.map(ft=>({path:ft.path,database:ft.database,pid:ft.pid,version:ft.version})),Se=x.current?{path:x.current.root_dir,database:x.current.db_path}:null,Be=Array.isArray(x.hidden)?x.hidden.filter(ft=>typeof ft=="string"):[];we.setState({workspace:{current:Se,available:T,hidden:Be}});let nt=window.localStorage.getItem("beads-ui.workspace");nt&&(!T.some(Ft=>Ft.path===nt)||Be.includes(nt)?window.localStorage.removeItem("beads-ui.workspace"):Se&&nt!==Se.path&&(t("restoring saved workspace preference: %s",nt),await Jt(nt)))}}catch(x){t("failed to load workspaces: %o",x)}}ge.on("workspace-changed",x=>{t("workspace-changed event: %o",x),x&&x.root_dir&&(we.setState({workspace:{current:{path:x.root_dir,database:x.db_path}}}),Ut(),Wt())});let an=!1;if(typeof ge.onConnection=="function"){let x=T=>{t("ws state %s",T),T==="reconnecting"||T==="closed"?(an=!0,he("Connection lost. Reconnecting\u2026","error",4e3)):T==="open"&&an&&(an=!1,he("Reconnected","success",2200),Aw(we,(Se,Be)=>{t(`${Se}: %o`,Be)}),dt())};ge.onConnection(x)}let rn="board";try{let x=window.localStorage.getItem("beads-ui.view");(x==="board"||x==="worker"||x==="monitor")&&(rn=x)}catch(x){t("view parse error: %o",x)}let we=Pc({config:xw(),view:rn});ge.on("worker-queue-snapshot",x=>{let T=x;if(!T||!T.queue)return;let Se=we.getState().workspace.current?.path;if(typeof Se=="string"&&Se.length>0&&T.root_dir!==Se){t("dropping worker-queue snapshot for %s",String(T.root_dir));return}try{et.set(T.queue)}catch{}});let S=Lc(we);S.start();let me=new Set(["get-comments","dep-add","dep-remove","impl-preset-create","impl-preset-update","impl-preset-delete","apply-impl-preset","apply-impl-preset-global","get-session-defaults","set-session-defaults","monitor-lane-create","monitor-lane-update","monitor-lane-confirm","monitor-lane-remove"]),m=async(x,T)=>{try{return await Le(x,T)}catch(Se){if(me.has(x))throw Se;return[]}};ff({global_element:r,repo_element:o},we,S);let b=document.getElementById("workspace-picker");b&&l_(b,we,Jt,jt,sn);let M=hf(e,(x,T)=>Le(x,T));try{let x=document.getElementById("new-issue-btn");x&&x.addEventListener("click",()=>M.open())}catch{}let f=wf(e,{policyStore:pe,queueStore:et,implPresetStore:ie,transport:(x,T)=>Le(x,T),onOpenChange:x=>{let T=Ye;Ye=x,tt(),T&&x===!1&&Re.refreshSessionDefaults()},labelOptions:()=>{let x=new Set;for(let[T]of Gl)for(let Se of Oe.snapshotFor(T)||[]){let Be=Se.labels;if(Array.isArray(Be))for(let nt of Be)typeof nt=="string"&&nt.length>0&&x.add(nt)}return Array.from(x).sort()}});try{let x=document.getElementById("display-settings-btn");x&&(x.setAttribute("aria-label","\uC124\uC815"),x.setAttribute("title","\uC124\uC815"),x.addEventListener("click",()=>f.open()))}catch{}let h=document.createElement("div");h.className="md-viewer-root",document.body.appendChild(h);let j=zi(h,{getWorkspacePath:()=>we.getState().workspace.current?.path}),_e=Qc(l,{gotoIssue:x=>S.gotoIssue(x),issueStores:Oe,transport:m,workerQueueStore:et,uiOrderStore:I,displayPolicyStore:pe,closedRange:ct,onClosedRangeChange:x=>{gt(x)},onNewIssue:()=>M.open(),openDoc:te}),Re=Wl(a,{transport:m,issueStores:Oe,queueStore:et,sessionLogStore:ue,gotoIssue:x=>we.setState({selected_id:x}),getWorkspacePath:()=>we.getState().workspace.current?.path,switchWorkspace:x=>Jt(x),openDoc:te,doneRange:Rt,onDoneRangeChange:x=>{It(x)}}),pt=pf(u,{transport:m,pipelineStore:rt,execPresetStore:ie,sessionLogStore:ue,router:S,gotoIssue:x=>S.gotoIssue(x),getWorkspacePath:()=>we.getState().workspace.current?.path,switchWorkspace:x=>Jt(x),openDoc:te}),ut=Dp(d,{issueStores:Oe,transport:m,queueStore:et,execPresetStore:ie,sessionLogStore:ue,getWorkspacePath:()=>we.getState().workspace.current?.path,mdViewer:j,depCandidates:()=>{let x=rt.get();if(x===null)return null;let T=rt.getWorkspacesState(),Se=we.getState();if(Se.view==="monitor")return nl(x,T);let Be=Se.workspace.current?.path;return Be?nl(x,T,{root_dir:Be}):null},subscribeCandidates:x=>rt.subscribe(x),onDepChanged:({type:x,a:T,b:Se})=>{let Be=pt;x==="dep-add"&&Be&&typeof Be.recorrectSharedLane=="function"&&Be.recorrectSharedLane(x,T,Se)},onNavigate:(x,T)=>{let Se=()=>{we.getState().view==="worker"?we.setState({selected_id:x}):S.gotoIssue(x)},Be=we.getState().workspace.current?.path;if(typeof T!="string"||T.length===0||!Be||T===Be){Se();return}Promise.resolve(Jt(T)).then(Se).catch(()=>{he("\uB808\uD3EC \uC804\uD658\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4","error",2400)})},onClose:()=>{let x=we.getState();we.setState({selected_id:null});try{S.gotoView(x.view==="worker"||x.view==="monitor"?x.view:"board")}catch{}},onOpenExecPresets:()=>{f.open("execution")}}),lt=we.getState().selected_id;lt&&(d.hidden=!1,ut.load(lt),Ie(lt)),we.subscribe(x=>{let T=x.selected_id;T?(d.hidden=!1,ut.load(T),Q||Ie(T)):(ut.clear(),d.hidden=!0,at())});let Tt=x=>{l.hidden=x.view!=="board",a.hidden=x.view!=="worker",u.hidden=x.view!=="monitor",s&&s.classList.toggle("is-quiet",x.view==="monitor"),ht(x.view==="board"),A(x.view==="worker"),Xe(qe(x)),oe(x.view==="board"||x.view==="worker"||Ye||!!x.selected_id),!x.selected_id&&x.view==="board"&&_e.load(),x.view==="worker"&&Re.load(),x.view==="monitor"?pt.load():pt.pause(),window.localStorage.setItem("beads-ui.view",x.view)};we.subscribe(Tt),Tt(we.getState()),wt(),Zt(),Gt(),Ut().finally(()=>{Y=!0,Je()}),window.addEventListener("keydown",x=>{let T=x.ctrlKey||x.metaKey,Se=String(x.key||"").toLowerCase(),Be=x.target,nt=Be&&Be.tagName?String(Be.tagName).toLowerCase():"",ft=nt==="input"||nt==="textarea"||nt==="select"||Be&&typeof Be.isContentEditable=="boolean"&&Be.isContentEditable;T&&Se==="n"&&(ft||(x.preventDefault(),M.open()))})}}typeof window<"u"&&typeof document<"u"&&window.addEventListener("DOMContentLoaded",()=>{try{let n=window.localStorage.getItem("beads-ui.theme"),r=window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches,o=n==="dark"||n==="light"?n:r?"dark":"light";document.documentElement.setAttribute("data-theme",o);let s=document.getElementById("theme-switch");s&&(s.checked=o==="dark")}catch{}let e=document.getElementById("theme-switch");e&&e.addEventListener("change",()=>{let n=e.checked?"dark":"light";document.documentElement.setAttribute("data-theme",n),window.localStorage.setItem("beads-ui.theme",n)});let t=document.getElementById("app");t&&Cw(t)});export{Cw as bootstrap,xw as readBootstrapConfig,Aw as refreshConfigSnapshot};
