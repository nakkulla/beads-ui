...
Please report this to https://github.com/markedjs/marked.`,t){let n="<p>An error occurred:</p><pre>"+ut(r.message+"",!0)+"</pre>";return e?Promise.resolve(n):n}if(e)return Promise.reject(r);throw r}}},xt=new Ha;function de(t,e){return xt.parse(t,e)}de.options=de.setOptions=function(t){return xt.setOptions(t),de.defaults=xt.defaults,Xs(de.defaults),de};de.getDefaults=hn;de.defaults=St;de.use=function(...t){return xt.use(...t),de.defaults=xt.defaults,Xs(de.defaults),de};de.walkTokens=function(t,e){return xt.walkTokens(t,e)};de.parseInline=xt.parseInline;de.Parser=ot;de.parser=ot.parse;de.Renderer=xr;de.TextRenderer=vn;de.Lexer=st;de.lexer=st.lex;de.Tokenizer=$r;de.Hooks=er;de.parse=de;var Oc=de.options,Mc=de.setOptions,Nc=de.use,Pc=de.walkTokens,Fc=de.parseInline;var Bc=ot.parse,qc=st.lex;function lo(t){let e=de.parse(t),r=zs.sanitize(e);return Ws(r)}function Wa(t){return String(t||"").replace(/^docs\/(superpowers\/)?/,"")}function co(t,e){let r=e.getWorkspacePath,n=e.fetchImpl||globalThis.fetch?.bind(globalThis),s=null,o="loading",i="",l="";function a(k){k.key==="Escape"&&s&&(k.preventDefault(),b())}document.addEventListener("keydown",a);function c(){return s?p`
  ...
  `;ue(r,t);let n=document.getElementById("top-nav"),s=document.getElementById("board-root"),o=document.getElementById("worker-root"),i=document.getElementById("detail-panel");if(s&&o&&i){let $=function(f,g){let Z="Request failed",j="";if(f&&typeof f=="object"){let ke=f;if(typeof ke.message=="string"&&ke.message.length>0&&(Z=ke.message),typeof ke.details=="string")j=ke.details;else if(ke.details&&typeof ke.details=="object")try{j=JSON.stringify(ke.details,null,2)}catch{j=""}}else typeof f=="string"&&f.length>0&&(Z=f);let te=g&&g.length>0?`Failed to load ${g}`:"Request failed";x.open(te,Z,j)},ae=function(f){return`${R.getState().workspace.current?.path||""}\0${f}`},Pe=function(){Ge&&(Ge().catch(()=>{}),Ge=null),$e=null,Ce=null},Re=function(f){Ze=f;let g=()=>{Ze!==f||R.getState().selected_id!==f||(Ze=null,Ke(f))};if(!pe){et.then(g);return}g()},_=function(){let f=Yn(Fe);return f===void 0?{type:"closed-issues"}:{type:"closed-issues",params:{since:f}}},w=function(f){if(f)for(let[g,Z]of An){if(De.has(g)||xe.has(g))continue;let j=g===mt?_():{type:Z};try{J.register(g,j)}catch(te){e("register %s store failed: %o",g,te)}xe.add(g),X.subscribeList(g,j).then(te=>{De.set(g,te)}).catch(te=>{e("subscribe %s failed: %o",g,te),$(te,"board")}).finally(()=>{xe.delete(g)})}else ne()},ne=function(){for(let[f]of An){let g=De.get(f);g&&(g().catch(()=>{}),De.delete(f));try{J.unregister(f)}catch(Z){e("unregister %s failed: %o",f,Z)}}},ce=function(f){if(!f){be();return}for(let[g,Z]of Eo)if(!(me.has(g)||xe.has(g))){try{J.register(g,{type:Z})}catch(j){e("register %s store failed: %o",g,j)}xe.add(g),X.subscribeList(g,{type:Z}).then(j=>{me.set(g,j)}).catch(j=>{e("subscribe %s failed: %o",g,j),$(j,"worker")}).finally(()=>{xe.delete(g)})}le||(Y("subscribe-worker-queue",{id:Co}).catch(g=>{e("subscribe-worker-queue failed: %o",g)}),le=()=>Y("unsubscribe-worker-queue",{id:Co}))},be=function(){for(let[f]of Eo){let g=me.get(f);g&&(g().catch(()=>{}),me.delete(f));try{J.unregister(f)}catch(Z){e("unregister %s failed: %o",f,Z)}}le&&(le().catch(()=>{}),le=null)},ge=function(){ee||(Y("subscribe-ui-order",{id:Ro}).catch(f=>{e("subscribe-ui-order failed: %o",f)}),ee=()=>Y("unsubscribe-ui-order",{id:Ro}))},tt=function(){ee&&(ee().catch(()=>{}),ee=null),qe.clear()},Oe=function(){ye||(Y("subscribe-display-policy",{id:Lo}).catch(f=>{e("subscribe-display-policy failed: %o",f)}),ye=()=>Y("unsubscribe-display-policy",{id:Lo}))},Ae=function(){ye&&(ye().catch(()=>{}),ye=null),Le.clear()},P=function(f){if(!f)return"Unknown";let g=f.split("/").filter(Boolean);return g.length>0?g[g.length-1]:"Unknown"};var l=$,a=ae,c=Pe,u=Re,h=_,b=w,S=ne,k=ce,C=be,N=ge,O=tt,F=Oe,H=Ae,I=P;let v=document.getElementById("header-loading"),y=gs(v),x=mo(t),q=To(),Y=y.wrapSend((f,g)=>q.send(f,g)),X=cs(Y),J=ds(),Ie=ps(),qe=us(),Le=Vn(),Xe=Zn();q.on("worker-queue-snapshot",f=>{let g=f;if(g&&g.queue)try{Ie.set(g.queue)}catch{}}),q.on("ui-order-snapshot",f=>{let g=f;if(g&&typeof g.revision=="number")try{qe.set({revision:g.revision,order:g.order&&typeof g.order=="object"?g.order:{}})}catch{}}),q.on("display-policy-snapshot",f=>{let g=f;if(g&&g.policy&&typeof g.policy=="object")try{Le.set(g.policy)}catch{}}),q.on("session-log-snapshot",f=>{let g=f;if(g&&typeof g.attempt_id=="string")try{Xe.set(g.attempt_id,Array.isArray(g.lines)?g.lines:[])}catch{}}),q.on("session-log-append",f=>{let g=f;if(g&&typeof g.attempt_id=="string")try{Xe.append(g.attempt_id,g.event)}catch{}}),q.on("snapshot",f=>{let g=f,Z=g&&typeof g.id=="string"?g.id:"",j=Z?J.getStore(Z):null;if(j&&g&&g.type==="snapshot")try{j.applyPush(g)}catch{}}),q.on("upsert",f=>{let g=f,Z=g&&typeof g.id=="string"?g.id:"",j=Z?J.getStore(Z):null;if(j&&g&&g.type==="upsert")try{j.applyPush(g)}catch{}}),q.on("delete",f=>{let g=f,Z=g&&typeof g.id=="string"?g.id:"",j=Z?J.getStore(Z):null;if(j&&g&&g.type==="delete")try{j.applyPush(g)}catch{}});let Ge=null,$e=null,Ce=null,Ze=null,se=()=>{},et=new Promise(f=>{se=()=>f(void 0)}),pe=!1,je=!1;async function Ke(f){let g=ae(f);if(g===$e||g===Ce)return;Ce=g;let Z=`detail:${f}`,j={type:"issue-detail",params:{id:f}};try{J.register(Z,j)}catch(te){e("register detail store failed: %o",te)}try{let te=await X.subscribeList(Z,j);if(R.getState().selected_id!==f||ae(f)!==g){await te().catch(()=>{});return}Ge&&await Ge().catch(()=>{}),Ge=te,$e=g}catch(te){e("detail subscribe failed: %o",te),$(te,"issue details")}finally{Ce===g&&(Ce=null)}}let De=new Map,xe=new Set,Fe=or;try{let f=window.localStorage.getItem(Io);qr(f)&&(Fe=f)}catch{}async function z(f){if(!qr(f)||f===Fe)return;Fe=f;try{window.localStorage.setItem(Io,f)}catch{}let g=De.get(mt);if(!g)return;De.delete(mt),await g().catch(()=>{});let Z=_();try{J.register(mt,Z)}catch(j){e("register %s store failed: %o",mt,j)}try{let j=await X.subscribeList(mt,Z);De.set(mt,j)}catch(j){e("re-subscribe %s failed: %o",mt,j),$(j,"board")}}let me=new Map,le=null,ee=null,ye=null;async function Me(){ye=null,Le.clear();let f=R.getState().workspace.current?.path;if(f)try{await q.send("set-workspace",{path:f})}catch(g){e("workspace restore after reconnect failed: %o",g);return}Oe()}async function E(){e("clearing all subscriptions for workspace switch"),ne(),be(),Ie.clear(),tt(),ge(),Ae(),Oe(),Pe();let f=R.getState();if(f.selected_id)try{J.unregister(`detail:${f.selected_id}`)}catch{}let g=R.getState();w(g.view==="board"),ce(g.view==="worker"),g.selected_id&&Re(g.selected_id)}async function D(f){e("requesting workspace switch to %s",f),je=!0;try{let g=await q.send("set-workspace",{path:f});e("workspace switch result: %o",g),g&&g.workspace&&(R.setState({workspace:{current:{path:g.workspace.root_dir,database:g.workspace.db_path}}}),window.localStorage.setItem("beads-ui.workspace",f),g.changed&&(await E(),K("Switched to "+P(f),"success",2e3)))}catch(g){throw e("workspace switch failed: %o",g),K("Failed to switch workspace","error",3e3),g}finally{je=!1}}async function V(f){e("requesting workspace git pull for %s",f);try{let g=await q.send("git-pull-workspace",{});e("workspace git pull result: %o",g);let Z=g?.status;if(Z==="up_to_date"){K("Already up to date","success",2e3);return}if(Z==="stash_pop_conflict"){K("Git pulled, but stash pop conflicted (check git stash list)","warning",4e3);return}K("Git pulled "+P(f),"success",2e3)}catch(g){e("workspace git pull failed: %o",g);let Z=g?.code,j=g?.message;if(Z==="rebase_conflict"){K("Git pull conflicts \u2014 reverted (manual resolve required)","error",4e3);return}if(Z==="rebase_conflict_abort_failed"){K("Git pull conflicts AND rebase --abort failed \u2014 repo left mid-rebase, run 'git rebase --abort' manually","error",6e3);return}if(Z==="busy"){K("Git pull skipped: another operation is running","warning",3e3);return}let te=j?`: ${j}`:"";throw K(`Git pull failed${te}`,"error",3e3),g}}async function W(f,g){e("setting workspace visibility %s \u2192 %s",f,String(g));try{await q.send("set-workspace-visibility",{path:f,visible:g}),await m()}catch(Z){e("workspace visibility update failed: %o",Z),K("Failed to update project visibility","error",3e3)}}async function m(){try{let f=await q.send("list-workspaces",{});if(e("workspaces loaded: %o",f),f&&Array.isArray(f.workspaces)){let g=f.workspaces.map(ke=>({path:ke.path,database:ke.database,pid:ke.pid,version:ke.version})),Z=f.current?{path:f.current.root_dir,database:f.current.db_path}:null,j=Array.isArray(f.hidden)?f.hidden.filter(ke=>typeof ke=="string"):[];R.setState({workspace:{current:Z,available:g,hidden:j}});let te=window.localStorage.getItem("beads-ui.workspace");te&&(!g.some(nr=>nr.path===te)||j.includes(te)?window.localStorage.removeItem("beads-ui.workspace"):Z&&te!==Z.path&&(e("restoring saved workspace preference: %s",te),await D(te)))}}catch(f){e("failed to load workspaces: %o",f)}}q.on("workspace-changed",f=>{e("workspace-changed event: %o",f),f&&f.root_dir&&(R.setState({workspace:{current:{path:f.root_dir,database:f.db_path}}}),m(),E())});let T=!1;if(typeof q.onConnection=="function"){let f=g=>{e("ws state %s",g),g==="reconnecting"||g==="closed"?(T=!0,K("Connection lost. Reconnecting\u2026","error",4e3)):g==="open"&&T&&(T=!1,K("Reconnected","success",2200),dl(R,(Z,j)=>{e(`${Z}: %o`,j)}),Me())};q.onConnection(f)}let A="board";try{let f=window.localStorage.getItem("beads-ui.view");(f==="board"||f==="worker")&&(A=f)}catch(f){e("view parse error: %o",f)}let R=ms({config:cl(),view:A}),Q=fs(R);Q.start();let fe=async(f,g)=>{try{return await Y(f,g)}catch{return[]}};n&&go(n,R,Q);let U=document.getElementById("workspace-picker");U&&xo(U,R,D,V,W);let Te=wo(t,(f,g)=>Y(f,g));try{let f=document.getElementById("new-issue-btn");f&&f.addEventListener("click",()=>Te.open())}catch{}let at=ho(t,{policyStore:Le,transport:(f,g)=>Y(f,g),labelOptions:()=>{let f=new Set;for(let[g]of An)for(let Z of J.snapshotFor(g)||[]){let j=Z.labels;if(Array.isArray(j))for(let te of j)typeof te=="string"&&te.length>0&&f.add(te)}return Array.from(f).sort()}});try{let f=document.getElementById("display-settings-btn");f&&f.addEventListener("click",()=>at.open())}catch{}let pt=$s(s,{gotoIssue:f=>Q.gotoIssue(f),issueStores:J,transport:fe,uiOrderStore:qe,displayPolicyStore:Le,closedRange:Fe,onClosedRangeChange:f=>{z(f)},onNewIssue:()=>Te.open()}),M=$n(o,{transport:fe,issueStores:J,queueStore:Ie,sessionLogStore:Xe,uiOrderStore:qe,gotoIssue:f=>R.setState({selected_id:f})}),oe=po(i,{issueStores:J,transport:fe,queueStore:Ie,sessionLogStore:Xe,getWorkspacePath:()=>R.getState().workspace.current?.path,onNavigate:f=>{R.getState().view==="worker"?R.setState({selected_id:f}):Q.gotoIssue(f)},onClose:()=>{let f=R.getState();R.setState({selected_id:null});try{Q.gotoView(f.view==="worker"?"worker":"board")}catch{}}}),we=R.getState().selected_id;we&&(i.hidden=!1,oe.load(we),Re(we)),R.subscribe(f=>{let g=f.selected_id;g?(i.hidden=!1,oe.load(g),je||Re(g)):(oe.clear(),i.hidden=!0,Pe())});let Qe=f=>{s.hidden=f.view!=="board",o.hidden=f.view!=="worker",w(f.view==="board"),ce(f.view==="worker"),!f.selected_id&&f.view==="board"&&pt.load(),f.view==="worker"&&M.load(),window.localStorage.setItem("beads-ui.view",f.view)};R.subscribe(Qe),Qe(R.getState()),ge(),Oe(),m().finally(()=>{pe=!0,se()}),window.addEventListener("keydown",f=>{let g=f.ctrlKey||f.metaKey,Z=String(f.key||"").toLowerCase(),j=f.target,te=j&&j.tagName?String(j.tagName).toLowerCase():"",ke=te==="input"||te==="textarea"||te==="select"||j&&typeof j.isContentEditable=="boolean"&&j.isContentEditable;g&&Z==="n"&&(ke||(f.preventDefault(),Te.open()))})}}typeof window<"u"&&typeof document<"u"&&window.addEventListener("DOMContentLoaded",()=>{try{let r=window.localStorage.getItem("beads-ui.theme"),n=window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches,s=r==="dark"||r==="light"?r:n?"dark":"light";document.documentElement.setAttribute("data-theme",s);let o=document.getElementById("theme-switch");o&&(o.checked=s==="dark")}catch{}let t=document.getElementById("theme-switch");t&&t.addEventListener("change",()=>{let r=t.checked?"dark":"light";document.documentElement.setAttribute("data-theme",r),window.localStorage.setItem("beads-ui.theme",r)});let e=document.getElementById("app");e&&ul(e)});export{ul as bootstrap,cl as readBootstrapConfig,dl as refreshConfigSnapshot};
