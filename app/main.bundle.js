...
`).trim()}}var Kl=20;function Yl(e){if(e==null||e==="")return"";let t=new Date(e);if(Number.isNaN(t.getTime()))return"";let r=String(t.getMonth()+1).padStart(2,"0"),n=String(t.getDate()).padStart(2,"0"),s=String(t.getHours()).padStart(2,"0"),o=String(t.getMinutes()).padStart(2,"0");return`${r}-${n} ${s}:${o}`}function a_(e){return e.length>Kl?`${e.slice(0,Kl)}\u2026`:e}function i_(e,t,r,n){let s=`${t.lane} ${a_(t.identifier)}`;return l`<div class="detail-report">
  ...
  >
  ...
  </details>`}function g_(e){let t=[];if(e.workflow_mode&&t.push(e.workflow_mode.display),e.impl_dispatch?.value==="main")t.push("\uBA54\uC778");else if(e.impl_dispatch?.value==="delegated"){let r=e.impl_runtime?` ${e.impl_runtime.display}`:"";t.push(`\uC704\uC784${r}`)}for(let r of["impl_model","impl_effort","impl_speed"])e[r]?.resolution!=="not_applicable"&&t.push(e[r]?.display||"\uAE30\uBCF8\uAC12 \uD655\uC778 \uBD88\uAC00");return t.join(" \xB7 ")}function b_(e){if(!e||typeof e!="object")return null;let{kind:t,actor:r,effort:n,sha:s}=e;return typeof t!="string"||typeof r!="string"||typeof s!="string"?null:{kind:t,actor:r,effort:typeof n=="string"?n:null,sha:s}}function cc(e){let t=e&&typeof e.metadata=="object"&&e.metadata?e.metadata:{},r=e&&typeof e.workflow=="object"&&e.workflow?e.workflow:{},n=r.stages||{},s=r.route||t.route||null,o=typeof t.pr_url=="string"?t.pr_url:"",a=typeof t.exec_receipt=="string"?t.exec_receipt:"",i=b_(r.exec_receipt),u=i?Br(i):a,d=i?`${i.kind}:${i.actor}`:a.split("@")[0],f=ys(r.planned_execution,r.exec_receipt);return l`<section class="detail-summary" data-seam="detail-summary">
    ...
    `:l``}function f(){Ge(d(),e)}async function _(A,I={}){s=A,o="loading",a="",i="",f();let D=r?r():"";if(!D){o="error",i="\uC6CC\uD06C\uC2A4\uD398\uC774\uC2A4\uAC00 \uC120\uD0DD\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4",f();return}if(!n){o="error",i="fetch\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4",f();return}let V="/api/doc?workspace="+encodeURIComponent(D)+"&path="+encodeURIComponent(A);try{let Y=await n(V),W=await Y.json().catch(()=>({}));if(!Y.ok||!W||W.ok!==!0){if(W?.error==="not_found"&&I.missing_state==="plan_pending"){o="pending",i="\uACC4\uD68D \uC791\uC131 \uC804 \xB7 \uACBD\uB85C\uB9CC \uC608\uC57D\uB418\uC5B4 \uC788\uC2B5\uB2C8\uB2E4",f();return}o="error",i="\uBB38\uC11C\uB97C \uBD88\uB7EC\uC624\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4 ("+String(W&&W.error||Y.status)+")",f();return}a=String(W.content||""),o="ready",f()}catch{o="error",i="\uBB38\uC11C \uC694\uCCAD \uC2E4\uD328",f()}}function h(){s=null,Ge(l``,e)}function C(){document.removeEventListener("keydown",u),h()}return{open:_,close:h,destroy:C}}var w_=[{key:"input_tokens",label:"\uC785\uB825"},{key:"output_tokens",label:"\uCD9C\uB825"},{key:"cache_read_input_tokens",label:"\uCE90\uC2DC \uC77D\uAE30"}],gc="\uC11C\uBC84 \uC7AC\uC2DC\uC791 \uBCF5\uAD6C \u2014 \uBD80\uBD84 \uC9D1\uACC4",Ks=["implementation","review-consult"],k_=["running","done","failed","interrupted"],$_={running:"\u25CF",done:"\u2713",failed:"\u2717",interrupted:"\u26A0"};function x_(e){return typeof e=="number"&&Number.isFinite(e)?e:0}function A_(e){let t=xt(e);if(t.length>0)return t.map(s=>l`<span class="detail-usage-total" title=${s.tooltip}
          >${s.label}</span
  ...
  >
    ...
    <div class="detail-sessions" data-seam="session-history">
      ${n.map(d=>{let f=Lo(d),_=mc(f),h=xt(_);return l`<div class="detail-session-row">
          <button
            type="button"
...
${b.notes_tail}`:"notes\uC758 REVISE finding\uC744 \uC2A4\uD399\uC5D0 \uBC18\uC601\uD558\uB294 \uCC98\uBD84 \uC138\uC158\uC744 \uB744\uC6C1\uB2C8\uB2E4":""};return Object.hasOwn(Ue,de)&&(O.blocked_by=Array.isArray(Ue[de])?Ue[de].filter(G=>typeof G=="string"&&G.length>0):[]),O};for(let H=0;H<We.length;H++){let ee=Ce(We[H],"queue",H,We.length);if(!ee)continue;f.push(ee);let De=C.get(B);De?De.push(ee):C.set(B,[ee])}let ct=[];for(let H=0;H<et.length;H++){let ee=et[H],De=[];for(let de=0;de<ee.entries.length;de++){let b=Ce(ee.entries[de],ee.id,de,ee.entries.length);b&&(De.push(b),f.push(b))}if(De.length===0)continue;let rt=At(Te[ee.id]);ct.push({id:ee.id,index:H,items:De,occupied_by:Array.isArray(rt.occupied_by)?rt.occupied_by.filter(de=>typeof de=="string"):[],corrections:Array.isArray(rt.corrections)?rt.corrections.length:0,cycle:rt.cycle===!0})}A.set(B,ct);let Ye=Array.from({length:tt},(H,ee)=>{let De=`s${ee+1}`,rt=K.get(De),de=rt&&Array.isArray(rt.entries)?rt.entries:[],b=At(Te[De]);return{id:De,index:de.length,length:de.length,occupied_by:Array.isArray(b.occupied_by)?b.occupied_by.filter($=>typeof $=="string"):[]}});for(let H of Array.isArray(w.runnable)?w.runnable:[]){let ee=H&&H.bead_id;typeof ee!="string"||ze.has(ee)||(ze.add(ee),i.push({...Fe(ee),title:H.title||ue[ee]||ee,lane:"runnable",draggable:!0,reason:zc($e,ee),created_at:H.created_at??void 0,updated_at:H.updated_at??void 0,labels:Array.isArray(H.labels)?H.labels:[],spec_reviewer:typeof H.spec_reviewer=="string"?H.spec_reviewer:void 0,plan_state:H.plan_state==="approved"||H.plan_state==="authored"?H.plan_state:"none",workflow:H.route?{route:H.route,chips:{route:H.route}}:null,blocked:H.blocked===!0,...Array.isArray(H.blocked_by)?{blocked_by:H.blocked_by.filter(De=>typeof De=="string"&&De.length>0)}:{},place_index:We.length,place_lanes:Ye}))}for(let H of re){let ee=H&&H.bead_id;if(typeof ee!="string"||ze.has(ee)||(ze.add(ee),o!==void 0&&typeof H.added_at=="number"&&H.added_at<o))continue;let De=nm(L,ee);_.push({...Fe(ee),lane:"done",done:!0,usage:Wt(L,ee),done_at:typeof H.added_at=="number"?H.added_at:void 0,done_kind:De&&typeof De.done_kind=="string"?De.done_kind:null})}}let D=new Map;s.forEach((w,B)=>{w&&typeof w.root_dir=="string"&&D.set(w.root_dir,B)});let V=r&&r.running_sort==="repo"?"repo":"started";u.sort((w,B)=>{if(V==="repo"){let M=D.get(w.root_dir)??Number.MAX_SAFE_INTEGER,L=D.get(B.root_dir)??Number.MAX_SAFE_INTEGER;if(M!==L)return M-L}let te=typeof w.started_at=="number"&&Number.isFinite(w.started_at)?w.started_at:null,le=typeof B.started_at=="number"&&Number.isFinite(B.started_at)?B.started_at:null;return te!==null&&le!==null&&te!==le?te-le:te===null&&le!==null?1:te!==null&&le===null?-1:w.id.localeCompare(B.id)}),_.sort((w,B)=>(B.done_at??0)-(w.done_at??0));let Y=s.length>0?s:n.map(w=>({root_dir:w&&w.root_dir,name:w&&w.name,auto_advance:w&&w.auto_advance,auto_merge:w&&w.auto_merge,slots:w&&w.slots,revision:w&&w.revision,runner_catalog:w&&w.runner_catalog})),W=[];for(let w of Y){if(!w||typeof w.root_dir!="string")continue;let B=C.get(w.root_dir)||[],te=A.get(w.root_dir)||[];W.push({root_dir:w.root_dir,name:w.name||w.root_dir,auto_advance:w.auto_advance===!0,auto_merge:w.auto_merge===!0,slots:typeof w.slots=="number"&&w.slots>=Wn?w.slots:Wn,revision:typeof w.revision=="number"?w.revision:0,runner_catalog:At(w.runner_catalog),items:B,sublanes:{parallel:B,serial:te},serial_lane_count:I.get(w.root_dir)||0})}let R={runnable:i,queue:f,queue_groups:W,running:u,pr_wait:d,done:_,automation:{total:W.length,both_on:W.filter(w=>w.auto_advance&&w.auto_merge).length}},S=fa(R);for(let w of h)S.has(w.id)||S.set(w.id,{root_dir:w.root_dir,workspace_name:w.workspace_name,lane:"done",state:"done"});for(let w of[...R.queue,...R.runnable]){if(!Object.hasOwn(w,"blocked_by"))continue;let B=S.get(w.id);w.blockers=(w.blocked_by||[]).map(te=>Ac(te,B,S,s)),w.blocker_warnings=w.blockers.filter(te=>te.missing_internal).map(te=>`\u26A0 \uC120\uD589 ${te.id}\uAC00 \uC5B4\uB290 \uB808\uC778\uC5D0\uB3C4 \uC5C6\uACE0 \uC2E4\uD589 \uC911\uB3C4 \uC544\uB2D8 \u2014 \uC218\uB3D9 \uAC1C\uC785 \uC804\uAE4C\uC9C0 \uC774 \uC790\uB9AC\uC5D0\uC11C \uC815\uC9C0`),w.blocker_warnings.length>0&&(w.alert=!0)}let P=Sc(R.queue_groups);for(let w of R.queue_groups)for(let B of w.sublanes.serial){let te=P.get(Tc(w.root_dir,B.id));te&&(B.cross_wait_peers=te)}return R}function om(e,t){if(typeof e!="number"||!Number.isFinite(e))return"";let n=t-e<em;return l`<span
  ...
  `}var xm=[{id:"execution",label:"\uC2E4\uD589",glyph:"\u25C6"},{id:"display",label:"\uD45C\uC2DC",glyph:"\u25EB"}],Ft="";function jt(e){return!!e&&typeof e=="object"&&!Array.isArray(e)}function pu(e,t){let{transport:r,policyStore:n,labelOptions:s}=t,o=t.notify||(b=>se(b,"error",4e3)),a=document.createElement("dialog");a.id="settings-dialog",a.className="settings-dialog",a.setAttribute("role","dialog"),a.setAttribute("aria-modal","true"),a.setAttribute("aria-label","\uC124\uC815"),e.appendChild(a);let i="execution",u=!1,d="",f={},_={},h=[],C=!1,A=null,I={},D="",V="",Y=!1,W=!1,R=!1,S=null;function P(){let b=t.queueStore?.get();return jt(b)?b.runner_catalog:null}function w(){let b=t.queueStore?.get();return jt(b)&&jt(b.execution_defaults)?b.execution_defaults:null}function B(){let b=t.implPresetStore?.get();return jt(b)&&Array.isArray(b.presets)?b:null}async function te(){C=!0,Ce();try{let b=await r("get-session-defaults",{});f=jt(b?.values)?{...b.values}:{},_={...f},h=Array.isArray(b?.warnings)?b.warnings:[]}catch(b){h=["kv_read_failed"],o(`\uC138\uC158 \uAE30\uBCF8\uAC12\uC744 \uC77D\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4: ${b instanceof Error?b.message:String(b)}`)}finally{C=!1,Ce()}}async function le(){let b=rc(f,_);if(Object.keys(b).length!==0){try{let $=await r("set-session-defaults",{values:b});f=jt($?.values)?{...$.values}:{},_={...f},h=Array.isArray($?.warnings)?$.warnings:[]}catch($){o(`\uC138\uC158 \uAE30\uBCF8\uAC12 \uC800\uC7A5 \uC2E4\uD328: ${$ instanceof Error?$.message:String($)}`)}Ce()}}function M(b,$){$===Ft?delete _[b]:_[b]=$,Ce(),le()}async function L(){let b=t.queueStore?.get();if(!jt(b))return;let $={orchestration_model:b.orchestration_model??null,orchestration_effort:b.orchestration_effort??null,orchestration_speed:b.orchestration_speed??null},k=nc($,{...$,...I});if(Object.keys(k).length!==0){try{let O=await r("worker-queue-set-orchestration-defaults",{expected_revision:b.revision,values:k});if(O&&O.applied===!1){o("Worker \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uD074\uB77C\uC774\uC5B8\uD2B8\uC640 \uCDA9\uB3CC");return}I={}}catch(O){o(`Worker \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: ${O instanceof Error?O.message:String(O)}`)}Ce()}}function ue(b,$){I[b]=$===Ft?null:$,Ce(),L()}async function ge(b){let $=t.queueStore?.get();if(!(!jt($)||b<1)){try{await r("worker-queue-set-slots",{expected_revision:$.revision,slots:b})}catch(k){o(`slots \uC800\uC7A5 \uC2E4\uD328: ${k instanceof Error?k.message:String(k)}`)}Ce()}}function $e(){let b={},$=We();for(let k of Jl){let O=Cr.includes(k)?$[k]:_[k];typeof O=="string"&&O.length>0&&(b[k]=O)}return b}async function Be(){let b=B();if(!b)return;let $=$e();if(Object.keys($).length===0){o("\uC800\uC7A5\uD560 \uC2E4\uD589 \uC124\uC815\uC774 \uC5C6\uC2B5\uB2C8\uB2E4 \u2014 \uBA3C\uC800 \uC2E4\uD589 \uAC12\uC744 \uC120\uD0DD\uD558\uC138\uC694");return}let k=(b.presets||[]).find(G=>G.id===D),O=V.trim()||(k?k.name:"");if(!O){o("\uD504\uB9AC\uC14B \uC774\uB984\uC744 \uC785\uB825\uD558\uC138\uC694");return}try{let G=k?await r("impl-preset-update",{expected_revision:b.revision,id:k.id,name:O,settings:$}):await r("impl-preset-create",{expected_revision:b.revision,name:O,settings:$});if(G&&G.applied){if(V="",!k&&Array.isArray(G.presets)){let Z=G.presets.find(ne=>ne.name===O);D=Z?Z.id:D}Ce()}else o("\uD504\uB9AC\uC14B \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uACF3\uC5D0\uC11C \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4"),Ce()}catch(G){o(`\uD504\uB9AC\uC14B \uC800\uC7A5 \uC2E4\uD328: ${G instanceof Error?G.message:String(G)}`)}}async function Qe(){let b=B();if(!(!b||D.length===0))try{let $=await r("impl-preset-delete",{expected_revision:b.revision,id:D});$&&$.applied?(D="",Ce()):(o("\uD504\uB9AC\uC14B \uC0AD\uC81C \uC2E4\uD328: \uB2E4\uB978 \uACF3\uC5D0\uC11C \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4"),Ce())}catch($){o(`\uD504\uB9AC\uC14B \uC0AD\uC81C \uC2E4\uD328: ${$ instanceof Error?$.message:String($)}`)}}async function Ve(){let b=B(),$=t.queueStore?.get();if(!(!b||!jt($)||D.length===0)){try{let k=await r("apply-impl-preset-global",{preset_id:D,expected_revision:b.revision,expected_queue_revision:$.revision});k&&k.applied?(f=jt(k.values)?{...k.values}:{},_={...f},h=Array.isArray(k.warnings)?k.warnings:[],jt(k.queue)&&(t.queueStore?.set?.(k.queue),I={}),k.queue_applied===!1&&o("\uC624\uCF00\uC2A4\uD2B8\uB808\uC774\uC158 \uAC12\uC740 \uC801\uC6A9\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4 \u2014 \uB2E4\uC2DC \uC2DC\uB3C4\uD558\uC138\uC694")):k&&k.conflict&&o("\uC2E4\uD589 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328: \uD504\uB9AC\uC14B\uC774 \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4")}catch(k){o(`\uC2E4\uD589 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328: ${k instanceof Error?k.message:String(k)}`)}Ce()}}async function Me(){W=!0,R=!1,Ce();try{let b=await r("get-worker-system-prompt",{});!b||typeof b!="object"||Array.isArray(b)?R=!0:S=b}catch{R=!0}finally{W=!1,Ce()}}function Ue(){if(Y=!Y,Y&&!S){Me();return}Ce()}function ie(){let b=cn({loading:W,error:R});if(b)return b;if(!S)return"";let $=Array.isArray(S.variants)?S.variants:[];return l`<div class="settings-dialog__sp-body">
    ...
    >
      ...
      ${Y?ie():""}
    </section>`}function Ie(b,$,k,O,G,Z){let ne=G[b]??Ft,ce=sa(b,k,G,w(),P()),je=ce.options.find(Se=>Se.value===ne),me=ne===Ft?ce.full_value:je?.full_value;return l`<select
        class=${ne===Ft?"settings-dialog__unset":""}
        data-key=${b}
        aria-label=${$}
        title=${me||""}
      ...
      >
        <option value=${Ft} ?selected=${ne===Ft}>
          ${ce.unset_label}
        </option>
      ...
      ${ne===Ft?l`<span class="settings-dialog__source-badge">기본</span>`:""}`}function Ee(b,$,k,O,G,Z=!1){return l`<div
    ...
    </div>`}function We(){let b=t.queueStore?.get(),$={};for(let k of Cr)$[k]=Object.prototype.hasOwnProperty.call(I,k)?I[k]:jt(b)&&typeof b[k]=="string"?b[k]:null;return $}function et(){let b=P(),$=_.impl_runtime,k=_.impl_model,O=B(),G=t.queueStore?.get(),Z=We(),ne=Gs(b,A),ce=un(b,A||void 0,Z.orchestration_model||dr).filter(he=>he!==dr),je=jt(G)&&typeof G.slots=="number"?G.slots:2,me=w()?.supported===!0,Se=sa("workflow_mode",Dn,_,w(),b);return l`
      <section
                ...
                <div class="settings-dialog__row">
                  ...
                  <span class="settings-dialog__controls">
                    <select
                      ...
                      .value=${zr(A||Ft)}
                      @change=${he=>{let st=String(he.target.value);A=st===Ft?null:st,Ce()}}
                    >
                      <option
                        value=${Ft}
                        ?selected=${!A}
    ...
    `}function Te(){let b=n.get();return l`
      ...
      >
        ...
        ${b?l`
              ${cu(b,s(),re)}
              ${uu(b,d,{onDraft:$=>{d=$},onAdd:Oe,onRemove:Fe})}
              ${du(b,ze)}
    ...
    </span>`}function V(M,L,ue){let ge=L.available&&typeof L.ageSeconds=="number"&&L.ageSeconds>_u,$e=ge&&typeof L.ageSeconds=="number"?`${Math.floor(L.ageSeconds/60)}\uBD84 \uC804 \uCE21\uC815`:"",Be=L.accounts.filter(Ue=>!Ue.active).length,Qe=`usage-meter__group${ge?" usage-meter__group--stale":""}`,Ve=l`<span class="usage-meter__provider"
        >${M.label}</span
      >
    ...
    </span>`}function W(M){let L=bu(M.pct);return l`<span
      class="usage-meter__account-window ${gu(L)}"
      style=${`--progress: ${L}%`}
    >
...
${Tt.notes_tail}`:"notes\uC758 REVISE finding\uC744 \uC2A4\uD399\uC5D0 \uBC18\uC601\uD558\uB294 \uCC98\uBD84 \uC138\uC158\uC744 \uB744\uC6C1\uB2C8\uB2E4":"",usage:q==="done"?Wt(c.attempts||{},ae.bead_id):null,work_ms:q==="done"?wc(c.attempts||{},ae.bead_id):null,done_at:q==="done"&&typeof ae.added_at=="number"?ae.added_at:void 0,...Re(ae.bead_id)}}),Yr=c.attempts?Object.values(c.attempts):[],co=new Set;for(let y of Yr)y&&typeof y.resumed_from=="string"&&y.resumed_from.length>0&&co.add(y.resumed_from);let Na=new Map;for(let y of Yr)Na.set(y.bead_id,y.attempt_id);let Zn=new Map;for(let y of Yr)Zn.set(y.attempt_id,y);function uo(y){let q=new Set,ae=y;for(;ae&&!q.has(ae.attempt_id);){if(ae.conflict_resolution===!0)return!0;q.add(ae.attempt_id),ae=typeof ae.resumed_from=="string"&&ae.resumed_from.length>0&&Zn.get(ae.resumed_from)||null}return!1}let Xn=typeof c.declared_base=="string"?c.declared_base:null;function id(y){let q=null;for(let ae of Yr)!ae||ae.bead_id!==y||uo(ae)||(q===null||(typeof ae.started_at=="number"?ae.started_at:0)>=(typeof q.started_at=="number"?q.started_at:0))&&(q=ae);return q&&typeof q.target_base=="string"?q.target_base:null}let qa=[],Fa=[],ld=ku(c),ja=y=>{let q=typeof y.session_id=="string"&&y.session_id.length>0,ae=co.has(y.attempt_id);return{eligible:q&&!ae,reason:q?ae?"\uC774\uBBF8 \uC774\uC5B4\uBC1B\uC740 attempt (child attempt \uC874\uC7AC) \u2014 \uC774\uC5B4\uD558\uAE30 \uBD88\uAC00":null:"session_id \uC5C6\uB294 \uAD6C attempt \u2014 \uC774\uC5B4\uD558\uAE30 \uBD88\uAC00"}},Vt=null;for(let y of Yr){let q=y.status==="paused"&&!co.has(y.attempt_id);if(y.status==="running"||q)Fa.push({bead_id:y.bead_id,attempt_id:y.attempt_id,title:nt.get(y.bead_id)||y.bead_id,runner:y.runner||null,model:y.model||null,effort:y.effort||null,speed:y.speed||null,continuation_mode:y.continuation_mode||null,started_at:typeof y.started_at=="number"?y.started_at:null,resumed_from:y.resumed_from||null,paused:q,conflict_resolution:uo(y),base_exception:Ia(Xn,y.target_base),can_pause:typeof y.session_id=="string"&&y.session_id.length>0,discard:pr(Yn,y.bead_id,{attempt_id:y.attempt_id}),usage:Wt(c.attempts||{},y.bead_id),current_child:fe(y.bead_id),...Re(y.bead_id)});else if((y.status==="failed"||y.status==="orphaned")&&ld(y)){let ae=ja(y);qa.push({bead_id:y.bead_id,attempt_id:y.attempt_id,title:nt.get(y.bead_id)||y.bead_id,runner:y.runner||null,model:y.model||null,effort:y.effort||null,speed:y.speed||null,continuation_mode:y.continuation_mode||null,started_at:typeof y.started_at=="number"?y.started_at:null,resumed_from:y.resumed_from||null,failed:!0,status:y.status,status_label:y.status==="orphaned"?"\uC911\uB2E8\uB428":"\uC2E4\uD328",discard:pr(Yn,y.bead_id,{attempt_id:y.attempt_id}),resume_eligible:ae.eligible,resume_reason:ae.reason,conflict_resolution:uo(y),base_exception:Ia(Xn,y.target_base),usage:Wt(c.attempts||{},y.bead_id),current_child:fe(y.bead_id),...Re(y.bead_id)}),Vt=y}}let Qn=[...qa,...Fa].map(y=>{let q=Zn.get(y.attempt_id),ae=q?.quickfix_landing;if(q?.quickfix_lane!==!0||!ae||typeof ae!="object")return y;let He=typeof ae.reason=="string"&&ae.reason.length>0?ae.reason:null,pt=Un({bead_id:q.bead_id,merge_sha:ae.head_sha,cleanup_cursor:ae.cursor,cleanup_failed:He?{step:ae.cursor,reason:He}:null,repo_operations:Array.isArray(c.repo_operations)?c.repo_operations:[]});return pt?{...y,landing:pt}:y}),Ba=null;if(Vt){let y=ja(Vt),q=Vt.cause_detail;Ba={bead_id:Vt.bead_id,repo:Vt.repo||"",reason:Vt.cause||Vt.status,cause_detail:q&&typeof q.reason=="string"?{reason:q.reason,command:typeof q.command=="string"?q.command:null}:null,resume_attempt_id:Vt.attempt_id,resume_eligible:y.eligible,resume_reason:y.reason,discard:pr(Yn,Vt.bead_id,{attempt_id:Vt.attempt_id})}}let Ua=new Set(Qn.map(y=>y.bead_id)),po=Array.isArray(c.merge_queue)?c.merge_queue:[],Wa=new Map,za=new Map,Ha=new Map,Ga=new Map,Va=new Map;po.forEach((y,q)=>{y&&typeof y.bead_id=="string"&&(Wa.set(y.bead_id,q+1),za.set(y.bead_id,y.resolution),Ha.set(y.bead_id,y.continuation_action||null),Ga.set(y.bead_id,y.head_review||null),Va.set(y.bead_id,y.authority||null))});let Zr=c.merge_queue_state||{active:null,failures:{}},cd=Zr.failures||{},Ka=Zr.waiting&&typeof Zr.waiting.bead_id=="string"&&typeof Zr.waiting.reason=="string"?Zr.waiting:null,ud=c.auto_merge_skips||{},Ya=y=>{let q=ud[y];if(!q)return null;let ae=_t[y],He=ae&&ae.pr?ae.pr.head_sha:null;return He&&He===q.head_sha?q.reason||"":null},Jn=new Map;for(let y of Qn)y.failed!==!0&&y.conflict_resolution&&(y.paused?Jn.has(y.bead_id)||Jn.set(y.bead_id,"paused"):Jn.set(y.bead_id,"running"));let Za=Qn.filter(y=>!y.paused&&y.failed!==!0).length,Xa=(c.workspace_info||{}).slots,Qa=typeof Xa=="number"?Xa:typeof c.slots=="number"?c.slots:oo,dd=Za>Qa,es=Nr(W),pd=(Array.isArray(c.done)?c.done.slice():[]).filter(y=>es===void 0||typeof y.added_at!="number"||y.added_at>=es).sort((y,q)=>(q.added_at||0)-(y.added_at||0)),_n=lo(pd,"done"),fd=new Set((Array.isArray(c.done)?c.done:[]).map(y=>y?.bead_id).filter(y=>typeof y=="string")),Ja=[],_d=d?.()||"";for(let y of z){let q=jr(y.closed_at);if(typeof y.id!="string"||fd.has(y.id)||q===null||es!==void 0&&q<es||typeof y.comment_count!="number"||y.comment_count<=0)continue;let ae=`${_d}\0${y.id}\0${String(y.updated_at)}\0${y.comment_count}`,He=R.get(ae);He===void 0&&r&&(R.set(ae,"pending"),Promise.resolve(r("get-comments",{id:y.id})).then(pt=>{let Dt=Array.isArray(pt)&&pt.some(Tt=>js(typeof Tt?.text=="string"?Tt.text:"")?.lane==="session");R.set(ae,Dt?"session":"not-session"),Q()}).catch(()=>{R.set(ae,"failed"),Q()})),He==="session"&&Ja.push({id:y.id,title:y.title||y.id,reason:"",draggable:!1,done:!0,lane:"done",selectable:!1,selected:!1,worker_serial:!1,badges:["\uC138\uC158 \uC791\uC5C5"],alert:!1,usage:null,work_ms:null,done_at:q,created_at:y.created_at,updated_at:y.updated_at})}_n.push(...Ja),_n.sort((y,q)=>(q.done_at||0)-(y.done_at||0));let ts={};for(let y of gr)ts[y]=0;let ei=!1,ti=0,fo=0,ri=0;for(let y of _n){let q=y.usage;if(q&&typeof q=="object"){let ae=!1;for(let He of gr)Number.isFinite(q[He])&&(ts[He]+=q[He],ei=!0,ae=!0);ae&&(fo+=1,Number.isFinite(q.total_cost_usd)&&(ti+=q.total_cost_usd,ri+=1))}}fo>0&&ri===fo&&(ts.total_cost_usd=ti);let ni=_n.map(y=>y.usage).filter(y=>y&&typeof y=="object"&&y.providers),md=ni.length>0?xt(xs(ni)):ei?Xt(ts):null,gd=c.lane_states&&typeof c.lane_states=="object"&&!Array.isArray(c.lane_states)?c.lane_states:{},bd=Array.isArray(c.serial_lanes)?c.serial_lanes:[],si=y=>{if(qe.some(He=>He.bead_id===y))return"PR \uB300\uAE30 \xB7 \uC810\uC720";let q=Yr.filter(He=>He&&He.bead_id===y),ae=q.length>0?q[q.length-1].status:null;return ae==="failed"||ae==="orphaned"?"\uC2E4\uD328 \xB7 \uC810\uC720 \uC720\uC9C0":ae==="paused"?"\uC77C\uC2DC\uC815\uC9C0 \xB7 \uC810\uC720":"\uC2E4\uD589 \uC911 \xB7 \uC810\uC720"},oi=bd.filter(y=>y&&typeof y.id=="string"&&Array.isArray(y.entries)).map((y,q)=>{let ae=gd[y.id]||{},He=new Map((Array.isArray(ae.corrections)?ae.corrections:[]).filter(ut=>ut&&typeof ut.bead_id=="string"&&typeof ut.after=="string").map(ut=>[ut.bead_id,ut.after])),pt=lo(y.entries.filter(ut=>!Ua.has(ut.bead_id)),y.id).map(ut=>He.has(ut.id)?{...ut,badges:[`\u{1F517} ${He.get(ut.id)} \uB4A4 (blocks \uC790\uB3D9)`,...ut.badges]}:ut),Dt=Array.isArray(ae.occupied_by)?ae.occupied_by.filter(ut=>typeof ut=="string"):[],Tt=Dt.map(ut=>({id:ut,title:nt.get(ut)||ut,draggable:!1,lane:y.id,ghost:!0,badges:[si(ut)]}));return{id:y.id,index:q+1,rows:[...Tt,...pt],occupied:Dt.length>0,badge:Dt.length>0?si(Dt[0]):"\uB300\uAE30",cycle:ae.cycle===!0}}),hd=typeof c.serial_lane_count=="number"?c.serial_lane_count:oi.length;return{queue:c,idToTitle:nt,candidates:od,candidate_hidden:{blocked:io.hidden_blocked,spec:io.hidden_spec},running:Qn,live_count:Za,slots:Qa,over_cap:dd,failure:Ba,waiting:lo(pn.filter(y=>!Ua.has(y.bead_id)),"queue"),serial_lanes:oi,serial_lane_count:hd,pr_wait:qe.map(y=>$g(y.bead_id,nt.get(y.bead_id)||y.bead_id,_t,Vr[y.bead_id]||null,Wt(c.attempts||{},y.bead_id),ir[y.bead_id]||(B.has(y.bead_id)||te.has(y.bead_id)?{activity:null,merge_progress:{step:"merging"}}:null),Jn.get(y.bead_id)||null,y.external===!0,{position:Wa.get(y.bead_id)||0,active:Zr.active===y.bead_id,failure:cd[y.bead_id]||null,waiting:Ka?.bead_id===y.bead_id?Ka.reason:null,resolution:za.get(y.bead_id),continuation_action:Ha.get(y.bead_id),head_review:Ga.get(y.bead_id)||null,authority:Va.get(y.bead_id)||null},y.wt_present!==!1,c.auto_merge===!0?Ya(y.bead_id):null,Ia(Xn,id(y.bead_id)),c.completion_status&&typeof c.completion_status=="object"&&!Array.isArray(c.completion_status)&&c.completion_status[y.bead_id]||null,c.discard_operations&&typeof c.discard_operations=="object"&&!Array.isArray(c.discard_operations)?c.discard_operations:{},Zn.get(Na.get(y.bead_id)||"")?.worker_serial===!0,c.auto_merge===!0,{merge_sha:y.merge_sha,cleanup_cursor:y.cleanup_cursor,repo_operations:Array.isArray(c.repo_operations)?c.repo_operations:[]})).map(y=>({...y,...Re(y.id)})),merge_queue_length:po.length,merge_queue_running:po.length>0,auto_excluded:qe.map(y=>y.bead_id).filter(y=>Ya(y)!==null),declared_base:Xn,done:_n,token_total:md,cleanup_failures:Vn,repo_operations:Array.isArray(c.repo_operations)?c.repo_operations:[]}}function st(){let m=!!o?.get()?.job,E=!m&&o?.isPending?.()===!0,z=m?"\uBD84\uC11D \uC911":E?"\uC900\uBE44 \uC911":"";return l`<button
    ...
    >`,Ke=l`<label class="worker-tgl worker-slots"
      ...
      ${o?st():""} `,nt=Nc({failure:c.failure}),Ne=kc(c.repo_operations,c.cleanup_failures);return w?l`<div class="worker-ribbon">
          ${E} ${z}
    ...
    `}function R(){let P=t.getState(),w=P.workspace?.current,B=P.workspace?.available||[],te=new Set(P.workspace?.hidden||[]),le=w?.path||B[0]?.path||"";if(B.length===0)return l``;let M=B.filter(L=>!te.has(L.path)||L.path===le);if(M.length<=1){let L=M[0]||B[0],ue=Oa(L.path);return l`
