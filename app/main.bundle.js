...
`,e=e.substring(h.length+1),c=!0),!c){let P=this.rules.other.nextBulletRegex(k),N=this.rules.other.hrRegex(k),A=this.rules.other.fencesBeginRegex(k),$=this.rules.other.headingBeginRegex(k),D=this.rules.other.htmlBeginRegex(k);for(;e;){let I=e.split(`
`,1)[0],S;if(h=I,this.options.pedantic?(h=h.replace(this.rules.other.listReplaceNesting,"  "),S=h):S=h.replace(this.rules.other.tabCharGlobal,"    "),A.test(h)||$.test(h)||D.test(h)||P.test(h)||N.test(h))break;if(S.search(this.rules.other.nonSpaceChar)>=k||!h.trim())_+=`
...
`)}function He(){if(!o)return i``;let W=S(),U=[a.runner,a.model,a.effort].filter(Boolean).join(" \xB7 "),ge=a.session_id||"",De=`\uB77C\uC774\uBE0C \uB530\uB77C\uAC00\uAE30 ${l?"ON":"OFF"}`,O=V(),j=O?Wp(B(),Date.now()):"",R=O?de(W):null,ee=O?ke(W):null,J=jp(W);return i`<div class="sv" data-attempt-id=${o}>
      <div class="sv__bar">
...
`).trim()}}var dl=20;function ul(e){if(e==null||e==="")return"";let t=new Date(e);if(Number.isNaN(t.getTime()))return"";let r=String(t.getMonth()+1).padStart(2,"0"),n=String(t.getDate()).padStart(2,"0"),s=String(t.getHours()).padStart(2,"0"),o=String(t.getMinutes()).padStart(2,"0");return`${r}-${n} ${s}:${o}`}function Kp(e){return e.length>dl?`${e.slice(0,dl)}\u2026`:e}function Zp(e,t,r,n){let s=`${t.lane} ${Kp(t.identifier)}`;return i`<div class="detail-report">
  ...
  >
    ...
    `:i``}function _(){ze(d(),e)}async function m(k,P={}){s=k,o="loading",a="",l="",_();let N=r?r():"";if(!N){o="error",l="\uC6CC\uD06C\uC2A4\uD398\uC774\uC2A4\uAC00 \uC120\uD0DD\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4",_();return}if(!n){o="error",l="fetch\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4",_();return}let A="/api/doc?workspace="+encodeURIComponent(N)+"&path="+encodeURIComponent(k);try{let $=await n(A),D=await $.json().catch(()=>({}));if(!$.ok||!D||D.ok!==!0){if(D?.error==="not_found"&&P.missing_state==="plan_pending"){o="pending",l="\uACC4\uD68D \uC791\uC131 \uC804 \xB7 \uACBD\uB85C\uB9CC \uC608\uC57D\uB418\uC5B4 \uC788\uC2B5\uB2C8\uB2E4",_();return}o="error",l="\uBB38\uC11C\uB97C \uBD88\uB7EC\uC624\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4 ("+String(D&&D.error||$.status)+")",_();return}a=String(D.content||""),o="ready",_()}catch{o="error",l="\uBB38\uC11C \uC694\uCCAD \uC2E4\uD328",_()}}function h(){s=null,ze(i``,e)}function E(){document.removeEventListener("keydown",c),h()}return{open:m,close:h,destroy:E}}var df=[{key:"input_tokens",label:"\uC785\uB825"},{key:"output_tokens",label:"\uCD9C\uB825"},{key:"cache_read_input_tokens",label:"\uCE90\uC2DC \uC77D\uAE30"}],Dl="\uC11C\uBC84 \uC7AC\uC2DC\uC791 \uBCF5\uAD6C \u2014 \uBD80\uBD84 \uC9D1\uACC4";function uf(e){return typeof e=="number"&&Number.isFinite(e)?e:0}function pf(e){let t=wt(e);if(t.length>0)return t.map(s=>i`<span class="detail-usage-total" title=${s.tooltip}
          >${s.label}</span
    ...
    >${e.replayed?i`<span class="detail-usage-partial" title=${Dl}
          >부분 집계</span
        >`:""}`}function Ll(e){return!e||!e.roles.orchestrator?null:{providers:e.roles.orchestrator,roles:{}}}function Ol(e){if(typeof e!="string")return"";let t=Date.parse(e);return Number.isFinite(t)?Ml(t):""}function ff(e){return e?["implementation","review-consult"].flatMap(r=>{let n=e.roles[r]?.codex;return n?n.legs.map(s=>{let a=wt({providers:{codex:{subtotal:s.subtotal,breakdown:s.usage,...s.replayed?{replayed:!0}:{}}},roles:{}})[0];return i`<div class="detail-session__leg detail-session__usage-detail">
  ...
  </div>`}var mf={running:"\u25CF",done:"\u2713",failed:"\u2717",orphaned:"\u26A0"};function Ml(e){if(typeof e!="number"||!Number.isFinite(e))return"";let t=new Date(e),r=String(t.getHours()).padStart(2,"0"),n=String(t.getMinutes()).padStart(2,"0");return`${r}:${n}`}function gf(e){if(typeof e.exec_default_preset_id!="string"||e.exec_default_preset_id.length===0)return"";let t=e.exec_values&&typeof e.exec_values=="object"?Object.entries(e.exec_values).filter(([,n])=>typeof n=="string"&&n.length>0).map(([n,s])=>`${n}=${s}`).join(" \xB7 "):"",r=typeof e.exec_default_preset_revision=="number"?` r${e.exec_default_preset_revision}`:"";return i`<div
      ...
      ${n.map(d=>{let _=oo(d),m=Ll(_),h=wt(m);return i`<div class="detail-session-row">
          ...
          >
            ...
            ${h.length>0?h.map(E=>i`<span
                    ...
                    >${zr(d.usage)}</span
                  >`:""}
            <span class="detail-session__time">${Ml(d.started_at)}</span>
          </button>
          ${c(d)} ${a(d)} ${l(d)} ${gf(d)}
    ...
    `}function Ze(){if(!d)return i``;let v=_||{},f=String(v.id||d),u=v.title||"(\uC81C\uBAA9 \uC5C6\uC74C)",C=fe(),K=v.status||"open",he=typeof v.priority=="number"?Math.max(0,Math.min(4,v.priority)):"",Oe=v.description||"",$e={...v,metadata:{...v.metadata||{},...m}};return i`
      <div class="detail-overlay" role="dialog" aria-modal="true">
        ...
        <div class="detail-overlay__panel">
          ...
          ${G(u,C)}
          ${Al($e)}
          ${Sl({metadata:$e.metadata,workspace_values:re(),catalog:M(),expanded:k,presets:Me()?.presets||[],preset_id:h,preset_busy:E},{onToggle:()=>{k=!k,we()},onEdit:(_t,Ce)=>{if(_t==="impl_runtime"||_t==="impl_model"||_t==="impl_effort"){ne(_t,Ce??"");return}X(_t,Ce??"")},onPresetSelect:_t=>{h=_t,we()},onPresetApply:()=>{Te()}})}
          ${q(K,he)} ${b(v)}
          ${pe(Oe)}
  ...
  >`,k=i`<span class="worker-mini__title">${e.title}</span>`,P=e.pr_url&&e.pr_number?i`<a
        ...
        >`:i`<span
          ...
          >${He}</span
        >`),$=e.reason?i`<span class="worker-mini__reason">${e.reason}</span>`:"",D=n.length>0?n.map(He=>i`<span class="worker-usage" title=${He.tooltip}
              >${He.label}</span
    ...
    ${l?i`<div class="worker-mini__row1">${h}${E}${k}</div>
          <div class="worker-mini__row2">
            ...
            ${Kr(e)}`:i`<div class="worker-mini__line">
              ${d}${_}${h}${E}${k}${P}${N}${A}${m}${$}${D}${I}${S}${B}${V}${ye}
            </div>
            ${dr(e)} ${Kr(e)}`}
...
${ae.notes_tail}`:"notes\uC758 REVISE finding\uC744 \uC2A4\uD399\uC5D0 \uBC18\uC601\uD558\uB294 \uCC98\uBD84 \uC138\uC158\uC744 \uB744\uC6C1\uB2C8\uB2E4":""};_.push(U);let ge=h.get($);ge?ge.push(U):h.set($,[U])}for(let Y of Array.isArray(A.runnable)?A.runnable:[]){let H=Y&&Y.bead_id;typeof H!="string"||be.has(H)||(be.add(H),l.push({...Ae(H),title:Y.title||V[H]||H,lane:"runnable",draggable:!0,reason:rc(ye,H),created_at:Y.created_at??void 0,updated_at:Y.updated_at??void 0,labels:Array.isArray(Y.labels)?Y.labels:[],spec_reviewer:typeof Y.spec_reviewer=="string"?Y.spec_reviewer:void 0,plan_state:Y.plan_state==="approved"||Y.plan_state==="authored"?Y.plan_state:"none",workflow:Y.route?{route:Y.route,chips:{route:Y.route}}:null,place_index:Ve.length}))}for(let Y of Fe){let H=Y&&Y.bead_id;if(typeof H!="string"||be.has(H)||(be.add(H),o!==void 0&&typeof Y.added_at=="number"&&Y.added_at<o))continue;let xe=Lf(B,H);m.push({...Ae(H),lane:"done",done:!0,usage:Mt(B,H),done_at:typeof Y.added_at=="number"?Y.added_at:void 0,done_kind:xe&&typeof xe.done_kind=="string"?xe.done_kind:null})}}let E=new Map;s.forEach((A,$)=>{A&&typeof A.root_dir=="string"&&E.set(A.root_dir,$)});let k=r&&r.running_sort==="repo"?"repo":"started";c.sort((A,$)=>{if(k==="repo"){let S=E.get(A.root_dir)??Number.MAX_SAFE_INTEGER,B=E.get($.root_dir)??Number.MAX_SAFE_INTEGER;if(S!==B)return S-B}let D=typeof A.started_at=="number"&&Number.isFinite(A.started_at)?A.started_at:null,I=typeof $.started_at=="number"&&Number.isFinite($.started_at)?$.started_at:null;return D!==null&&I!==null&&D!==I?D-I:D===null&&I!==null?1:D!==null&&I===null?-1:A.id.localeCompare($.id)}),m.sort((A,$)=>($.done_at??0)-(A.done_at??0));let P=s.length>0?s:n.map(A=>({root_dir:A&&A.root_dir,name:A&&A.name,auto_advance:A&&A.auto_advance,auto_merge:A&&A.auto_merge,slots:A&&A.slots,revision:A&&A.revision,runner_catalog:A&&A.runner_catalog})),N=[];for(let A of P)!A||typeof A.root_dir!="string"||N.push({root_dir:A.root_dir,name:A.name||A.root_dir,auto_advance:A.auto_advance===!0,auto_merge:A.auto_merge===!0,slots:typeof A.slots=="number"&&A.slots>=$n?A.slots:$n,revision:typeof A.revision=="number"?A.revision:0,runner_catalog:Ft(A.runner_catalog),items:h.get(A.root_dir)||[]});return{runnable:l,queue:_,queue_groups:N,running:c,pr_wait:d,done:m,automation:{total:N.length,both_on:N.filter(A=>A.auto_advance&&A.auto_merge).length}}}function Df(e,t){if(typeof e!="number"||!Number.isFinite(e))return"";let n=t-e<Cf;return i`<span
  ...
  >`}function Rs(e){return e.workspace_name?i`<span class="mon-c__repo" title=${e.root_dir||""}
      ...
      >`:""}function zo(e){return(Array.isArray(e.badges)?e.badges:[]).map(r=>i`<span class="mon-c__badge${e.alert?" mon-c__badge--alert":""}"
        >${r}</span
      >`)}function Mf(e){return i`<span class="mon-c__ops">
  ...
  `}var Kf=[{id:"session",label:"\uC138\uC158",glyph:"\u25C6"},{id:"worker",label:"Worker",glyph:"\u25A4"},{id:"display",label:"\uD45C\uC2DC",glyph:"\u25EB"}],Kt="";function Zt(e){return!!e&&typeof e=="object"&&!Array.isArray(e)}function wc(e,t){let{transport:r,policyStore:n,labelOptions:s}=t,o=t.notify||(w=>le(w,"error",4e3)),a=document.createElement("dialog");a.id="settings-dialog",a.className="settings-dialog",a.setAttribute("role","dialog"),a.setAttribute("aria-modal","true"),a.setAttribute("aria-label","\uC124\uC815"),e.appendChild(a);let l="session",c=!1,d="",_={},m={},h=[],E=!1,k=null,P={},N="",A="",$=!1,D=!1,I=!1,S=null;function B(){let w=t.queueStore?.get();return Zt(w)?w.runner_catalog:null}function V(){let w=t.implPresetStore?.get();return Zt(w)&&Array.isArray(w.presets)?w:null}async function se(){E=!0,J();try{let w=await r("get-session-defaults",{});_=Zt(w?.values)?{...w.values}:{},m={..._},h=Array.isArray(w?.warnings)?w.warnings:[]}catch(w){h=["kv_read_failed"],o(`\uC138\uC158 \uAE30\uBCF8\uAC12\uC744 \uC77D\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4: ${w instanceof Error?w.message:String(w)}`)}finally{E=!1,J()}}async function ye(){let w=bl(_,m);if(Object.keys(w).length!==0){try{let M=await r("set-session-defaults",{values:w});_=Zt(M?.values)?{...M.values}:{},m={..._},h=Array.isArray(M?.warnings)?M.warnings:[]}catch(M){o(`\uC138\uC158 \uAE30\uBCF8\uAC12 \uC800\uC7A5 \uC2E4\uD328: ${M instanceof Error?M.message:String(M)}`)}J()}}function te(w,M){M===Kt?delete m[w]:m[w]=M,J(),ye()}async function de(){let w=t.queueStore?.get();if(!Zt(w))return;let M={orchestration_model:w.orchestration_model??null,orchestration_effort:w.orchestration_effort??null,orchestration_speed:w.orchestration_speed??null},Z=yl(M,{...M,...P});if(Object.keys(Z).length!==0){try{let Me=await r("worker-queue-set-orchestration-defaults",{expected_revision:w.revision,values:Z});if(Me&&Me.applied===!1){o("Worker \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uD074\uB77C\uC774\uC5B8\uD2B8\uC640 \uCDA9\uB3CC");return}P={}}catch(Me){o(`Worker \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: ${Me instanceof Error?Me.message:String(Me)}`)}J()}}function ke(w,M){P[w]=M===Kt?null:M,J(),de()}async function Ue(w){let M=t.queueStore?.get();if(!(!Zt(M)||w<1)){try{await r("worker-queue-set-slots",{expected_revision:M.revision,slots:w})}catch(Z){o(`slots \uC800\uC7A5 \uC2E4\uD328: ${Z instanceof Error?Z.message:String(Z)}`)}J()}}function Ie(){let w={};for(let M of ml){let Z=m[M];typeof Z=="string"&&Z.length>0&&(w[M]=Z)}return w}async function He(){let w=V();if(!w)return;let M=Ie();if(Object.keys(M).length===0){o("\uC800\uC7A5\uD560 \uAD6C\uD604 \uAC12\uC774 \uC5C6\uC2B5\uB2C8\uB2E4 \u2014 \uBA3C\uC800 \uAD6C\uD604 \uADF8\uB8F9\uC744 \uC120\uD0DD\uD558\uC138\uC694");return}let Z=(w.presets||[]).find(Le=>Le.id===N),Me=A.trim()||(Z?Z.name:"");if(!Me){o("\uD504\uB9AC\uC14B \uC774\uB984\uC744 \uC785\uB825\uD558\uC138\uC694");return}try{let Le=Z?await r("impl-preset-update",{expected_revision:w.revision,id:Z.id,name:Me,settings:M}):await r("impl-preset-create",{expected_revision:w.revision,name:Me,settings:M});if(Le&&Le.applied){if(A="",!Z&&Array.isArray(Le.presets)){let qe=Le.presets.find(Te=>Te.name===Me);N=qe?qe.id:N}J()}else o("\uD504\uB9AC\uC14B \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uACF3\uC5D0\uC11C \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4"),J()}catch(Le){o(`\uD504\uB9AC\uC14B \uC800\uC7A5 \uC2E4\uD328: ${Le instanceof Error?Le.message:String(Le)}`)}}async function Ye(){let w=V();if(!(!w||N.length===0))try{let M=await r("impl-preset-delete",{expected_revision:w.revision,id:N});M&&M.applied?(N="",J()):(o("\uD504\uB9AC\uC14B \uC0AD\uC81C \uC2E4\uD328: \uB2E4\uB978 \uACF3\uC5D0\uC11C \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4"),J())}catch(M){o(`\uD504\uB9AC\uC14B \uC0AD\uC81C \uC2E4\uD328: ${M instanceof Error?M.message:String(M)}`)}}async function Ve(){let w=V();if(!(!w||N.length===0)){try{let M=await r("apply-impl-preset-global",{preset_id:N,expected_revision:w.revision});M&&M.applied?(_=Zt(M.values)?{...M.values}:{},m={..._},h=Array.isArray(M.warnings)?M.warnings:[]):M&&M.conflict&&o("\uAD6C\uD604 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328: \uD504\uB9AC\uC14B\uC774 \uBC29\uAE08 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4")}catch(M){o(`\uAD6C\uD604 \uD504\uB9AC\uC14B \uC801\uC6A9 \uC2E4\uD328: ${M instanceof Error?M.message:String(M)}`)}J()}}async function Fe(){D=!0,I=!1,J();try{let w=await r("get-worker-system-prompt",{});!w||typeof w!="object"||Array.isArray(w)?I=!0:S=w}catch{I=!0}finally{D=!1,J()}}function ve(){if($=!$,$&&!S){Fe();return}J()}function Ae(){let w=Vr({loading:D,error:I});if(w)return w;if(!S)return"";let M=Array.isArray(S.variants)?S.variants:[];return i`<div class="settings-dialog__sp-body">
      ${S.target_base_placeholder?i`<div class="prompt-block__meta">
            \`${S.target_base_placeholder}\`는 디스패치 시점에 해석된
            base로 치환됩니다.
          </div>`:""}
      ${M.map(Z=>i`<div class="settings-dialog__sp-variant" data-variant=${Z.key}>
    ...
    >
      ...
      ${$?Ae():""}
    </section>`}function Y(w,M,Z,Me,Le,qe){let Te=Le[w]??Kt;return i`<select
      class=${Te===Kt?"settings-dialog__unset":""}
      data-key=${w}
      aria-label=${M}
      ?disabled=${qe===!0}
      .value=${Dr(String(Te))}
      @change=${Qe=>Me(w,String(Qe.target.value))}
    >
      <option value=${Kt} ?selected=${Te===Kt}>(기본)</option>
      ${Z.map(Qe=>i`<option value=${Qe} ?selected=${Qe===Te}>
            ${Qe===Nt?"\uC790\uB3D9":Qe}
          </option>`)}
    </select>`}function H(w,M,Z,Me,Le,qe=!1){return i`<div
    ...
    </div>`}function xe(w,M,Z,Me,Le){return i`<div class="settings-dialog__row">
    ...
    </div>`}function ae(){let w=B(),M=hl(m),Z=m.impl_runtime,Me=m.impl_model,Le=V();return i`
      <section
      ...
      >
        ...
        ${E?i`<div class="settings-dialog__empty">불러오는 중…</div>`:i`
              ...
              <div class="settings-dialog__group">
                ...
                ${H("impl_runtime","\uC704\uC784 \uB300\uC0C1",hs,te,m,M)}
                ${H("impl_model","\uBAA8\uB378",ws(w,Z),te,m,M)}
                ${H("impl_effort","effort",Yr(w,Z,Me),te,m,M)}
                ${H("impl_speed","\uC18D\uB3C4",bn,te,m,M)}
              </div>
    ...
    `}function ie(){let w=t.queueStore?.get(),M=B(),Z={orchestration_model:P.orchestration_model??(Zt(w)?w.orchestration_model:null),orchestration_effort:P.orchestration_effort??(Zt(w)?w.orchestration_effort:null),orchestration_speed:P.orchestration_speed??(Zt(w)?w.orchestration_speed:null)},Me=ks(M,k),Le=Yr(M,k||void 0,Z.orchestration_model||Nt).filter(Te=>Te!==Nt),qe=Zt(w)&&typeof w.slots=="number"?w.slots:2;return i`
      <section
      ...
      >
        ...
        <div class="settings-dialog__group">
          ...
          ${H("orchestration_model","\uBAA8\uB378",Me,ke,Z)}
    ...
    `}function W(){let w=n.get();return i`
      ...
      >
        ...
        ${w?i`
              ${bc(w,s(),O)}
              ${yc(w,d,{onDraft:M=>{d=M},onAdd:j,onRemove:R})}
              ${vc(w,ee)}
            `:i`<div class="settings-dialog__empty">
              표시 정책을 불러오는 중…
            </div>`}
      </section>
    `}async function U(w){let M=n.get();if(M)try{let Z=await r("display-policy-set",{expected_revision:M.revision,policy:w(M)});ge(Z),Z&&Z.conflict&&Z.policy&&(Z=await r("display-policy-set",{expected_revision:Z.policy.revision,policy:w(Z.policy)}),ge(Z)),Z&&Z.conflict&&o("\uD45C\uC2DC \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328: \uB2E4\uB978 \uD074\uB77C\uC774\uC5B8\uD2B8\uC640 \uCDA9\uB3CC")}catch{o("\uD45C\uC2DC \uC124\uC815 \uC800\uC7A5 \uC2E4\uD328")}}function ge(w){w&&w.policy&&typeof w.policy=="object"&&n.set(w.policy)}function De(w){U(w)}function O(w){let M=n.get();if(!M)return;let Z=!Zf(w,M);De(Me=>Xf(w,Me,Z))}function j(){let w=d.trim();w.length!==0&&(d="",De(M=>M.hidden_prefixes.includes(w)?{hidden_prefixes:M.hidden_prefixes}:{hidden_prefixes:[...M.hidden_prefixes,w]}),J())}function R(w){De(M=>({hidden_prefixes:M.hidden_prefixes.filter(Z=>Z!==w)}))}function ee(w){let M=n.get();if(!M)return;let Z=M.chips[w]===!1;De(()=>({chips:{[w]:Z}}))}function J(){ze(i`
      ...
      >`}return ie()?i`<span class="pa-meta__progress"
        ...
        >`:""}function ie(){return n?.isPending?.()===!0}function W(T){let F=m(),Q=(Array.isArray(F.queue)?F.queue.length:0)+(Array.isArray(F.serial_lanes)?F.serial_lanes:[]).reduce((Z,Me)=>Z+(Array.isArray(Me.entries)?Me.entries.length:0),0),re=!!T.job,w=H(T.settings),M=re||c||ie();return i`<div class="pa-meta">
    ...
    </div>`}function U(T,F){let Q=N(T,F),re=E(),w=Q.filter(Te=>re.has(Te)),M=k(Q),Z=A(Q),Me=Array.isArray(m().serial_lanes)?m().serial_lanes:[],Le=l.get(T)||$(),qe=F.eligible!==!0||Q.length<2||w.length>0||M.length>0||Z||c;return i`<section class="pa-group" data-group-index=${String(T)}>
      ...
      <ol class="pa-group__members">
        ${Q.map((Te,Qe)=>i`<li class="pa-member" data-bead-id=${Te}>
              ...
              <span class="pa-member__title">${D(Te)}</span>
    ...
    </div>`}function De(){let T=j&&!!h().job;if(T&&_===null){_=setInterval(()=>O(),1e3);return}!T&&_!==null&&(clearInterval(_),_=null)}function O(){let T=h();d&&T.settings.runner===d&&(d=null);let F=T.last_good?.result;De(),ze(i`
      ...
      `,o)}let j=!1,R=()=>{j=!1,De()},ee=T=>{T.target===T.currentTarget&&Ee()};o.addEventListener("close",R),o.addEventListener("cancel",R),o.addEventListener("click",ee);let J=null;r&&r.subscribe&&(J=r.subscribe(()=>{j&&O()}));let fe=null;n&&n.subscribe&&(fe=n.subscribe(()=>{j&&O()}));function ue(){j||(j=!0,O(),typeof o.showModal=="function"?o.showModal():o.setAttribute("open",""))}function Ee(){j&&(j=!1,De(),typeof o.close=="function"?o.close():o.removeAttribute("open"))}return{open:ue,close:Ee,destroy(){j=!1,_!==null&&(clearInterval(_),_=null),o.removeEventListener("close",R),o.removeEventListener("cancel",R),o.removeEventListener("click",ee),J&&(J(),J=null),fe&&(fe(),fe=null),o.remove()}}}function Ec(e){let t=e.queueStore,r=e.transport,n=e.onChanged||(()=>{});function s(){return t&&t.get()||{}}function o(){let $=s();return typeof $.revision=="number"?$.revision:0}function a($){t&&$&&$.queue&&typeof $.queue=="object"&&t.set($.queue)}function l(){let $=s().workspace_info;return $&&typeof $=="object"?$:{}}function c($,D){return i`<span
      class="worker-repo-ops__vd-badge worker-repo-ops__vd-badge--${$}"
      >${D}</span
    >`}function d($){if(typeof $!="number"||!Number.isFinite($))return"";let D=$/6e4;return Number.isInteger(D)?`timeout ${D}\uBD84`:`timeout ${Math.round($/1e3)}\uCD08`}function _($){let D=d($);return D?c("config",D):""}function m($){let D=typeof $.base_sha=="string"?$.base_sha:"",I=`${$.source_path||"repo-ops/config.toml"} @ ${$.base_ref||"?"}${D?`@${D.slice(0,7)}`:""}`;return i`<section class="worker-repo-ops__vd" data-seam="repo-ops">
    ...
    </section>`}function h($){let D=$.repo_ops&&typeof $.repo_ops=="object"?$.repo_ops:null;return D&&(D.status==="resolved"||D.status==="absent")?m(D):D&&(D.status==="pending"||D.status==="error")?i`<section class="worker-repo-ops__vd" data-seam="repo-ops">
        <p class="worker-repo-ops__vd-title">
          저장소 작업 선언
    ...
    </section>`}async function E($){if(!r)return;let D=await r("worker-auto-repair-toggle",{on:$,expected_revision:o()});if(a(D),D&&D.conflict){let I=await r("worker-auto-repair-toggle",{on:$,expected_revision:o()});a(I)}n()}let k={owned_deploy_worktree_fetch_detached_alignment_recreate:"\uC804\uC6A9 \uBC30\uD3EC \uC6CC\uD06C\uD2B8\uB9AC \uC815\uB82C\xB7\uBCF5\uAD6C",recovered_pre_execution_fetch_timeout_retry_once:"fetch \uD0C0\uC784\uC544\uC6C3 1\uD68C \uBCF5\uAD6C",repo_serial_lock_wait:"\uC800\uC7A5\uC18C \uC21C\uCC28 \uC2E4\uD589 \uB300\uAE30",restart_operation_adoption:"\uC7AC\uC2DC\uC791 \uD6C4 \uC791\uC5C5 \uC778\uACC4",exact_input_exit_zero_evidence_adoption:"\uB3D9\uC77C \uC785\uB825 \uC131\uACF5 \uC99D\uAC70 \uC778\uACC4",descendant_success_covers_ancestor_rows:"\uCD5C\uC2E0 SHA \uC131\uACF5\uC774 \uC774\uC804 \uD589 \uCEE4\uBC84",owned_verify_candidate_cleanup:"\uAC80\uC99D \uC784\uC2DC \uCCB4\uD06C\uC544\uC6C3 \uC815\uB9AC",script_retry:"\uC2A4\uD06C\uB9BD\uD2B8 \uC7AC\uC2DC\uB3C4",auto_repair_session:"\uC790\uB3D9 \uD574\uACB0 \uC138\uC158",user_triggered_session:"\uC0AC\uC6A9\uC790 \uD574\uACB0 \uC138\uC158",automatic:"\uC790\uB3D9",user_action_only:"\uC0AC\uC6A9\uC790 \uD074\uB9AD",script_identity_present:"\uC2A4\uD06C\uB9BD\uD2B8\uAC00 \uC788\uC744 \uB54C\uB9CC",per_completion_chain:"\uC644\uB8CC \uCCB4\uC778\uB2F9",unbounded:"\uD69F\uC218 \uC81C\uD55C \uC5C6\uC74C",bounded_single_script_retry_exceeded:"\uB2E8\uC77C \uC2A4\uD06C\uB9BD\uD2B8 \uC7AC\uC2DC\uB3C4 \uD55C\uB3C4 \uCD08\uACFC",baseline_failure_ignore:"\uAE30\uC874 \uC2E4\uD328 \uBB34\uC2DC",config_or_script_deletion_to_bypass_gate:"\uC124\uC815\xB7\uC2A4\uD06C\uB9BD\uD2B8 \uC0AD\uC81C\uB85C \uAC8C\uC774\uD2B8 \uC6B0\uD68C",credential_entry:"\uC790\uACA9\uC99D\uBA85 \uC785\uB825\xB7\uCD9C\uB825",destructive_action:"\uD30C\uAD34\uC801 \uC791\uC5C5",history_rewrite:"\uD788\uC2A4\uD1A0\uB9AC \uC7AC\uC791\uC131",agent_self_report_as_success:"\uC138\uC158 \uC790\uAE30\uBCF4\uACE0\uB97C \uC131\uACF5 \uCC98\uB9AC",unbounded_repair_session_retry:"\uBB34\uD55C \uD574\uACB0 \uC138\uC158 \uBC18\uBCF5"};function P($,D,I){return i`<div class="worker-repo-ops__policy-group" data-policy=${I}>
      <div class="worker-repo-ops__policy-label">${$}</div>
      <ul class="worker-repo-ops__policy-list">
        ${D.map(S=>i`<li data-token=${S}>
              ${k[S]||S}
            </li>`)}
      </ul>
    ...
    >
      ...
      <ol class="worker-repo-ops__policy-list">
        ${$.map(D=>{let I=[k[D.trigger]||D.trigger];return Number.isInteger(D.attempts_per_operation_attempt)?I.push(`operation\uB2F9 ${D.attempts_per_operation_attempt}\uD68C`):Number.isInteger(D.attempts)?I.push(`${k[D.budget]||D.budget} ${D.attempts}\uD68C`):Number.isInteger(D.sessions_per_user_action)&&I.push(`${D.sessions_per_user_action}\uD68C`,k[D.user_actions]||D.user_actions),D.applies_when&&I.push(k[D.applies_when]||D.applies_when),i`<li data-token=${D.id}>
            <strong>${k[D.id]||D.id}</strong>
            <span>${I.filter(Boolean).join(" \xB7 ")}</span>
          </li>`})}
      </ol>
    </div>`}function A(){let $=s(),D=$.auto_repair!==!1,I=$.repo_operation_policy&&typeof $.repo_operation_policy=="object"?$.repo_operation_policy:null,S=Array.isArray($.repo_operations)?$.repo_operations:[],B=S.find(te=>te.state==="repairing"),V=S.filter(te=>te.state==="failed"||te.state==="repairing"),se=V.length?Math.min(...V.map(te=>typeof te.repair?.remaining=="number"?te.repair.remaining:0)):I?.auto_repair?.resolution_ladder?.find(te=>te.id==="auto_repair_session")?.attempts??1,ye=Array.isArray(I?.auto_repair?.resolution_ladder)?I.auto_repair.resolution_ladder:[];return i`<section
  ...
  </li>`}function d_(e){let t=e.cleanup,r=Mr(t.step);return i`<li
    class="worker-ev"
...
${pt.notes_tail}`:"notes\uC758 REVISE finding\uC744 \uC2A4\uD399\uC5D0 \uBC18\uC601\uD558\uB294 \uCC98\uBD84 \uC138\uC158\uC744 \uB744\uC6C1\uB2C8\uB2E4":"",usage:L==="done"?Mt(p.attempts||{},ce.bead_id):null,done_at:L==="done"&&typeof ce.added_at=="number"?ce.added_at:void 0,...u(ce.bead_id)}}),na=new Map;for(let g of p.done)g&&typeof g.bead_id=="string"&&typeof g.added_at=="number"&&na.set(g.bead_id,g.added_at);let Nr=p.attempts?Object.values(p.attempts):[],Ms=new Set;for(let g of Nr)g&&typeof g.resumed_from=="string"&&g.resumed_from.length>0&&Ms.add(g.resumed_from);let Ps=new Map;for(let g of Nr)Ps.set(g.bead_id,g.attempt_id);let Ns=new Map;for(let g of Nr)Ns.set(g.attempt_id,g);function Fs(g){let L=new Set,ce=g;for(;ce&&!L.has(ce.attempt_id);){if(ce.conflict_resolution===!0)return!0;L.add(ce.attempt_id),ce=typeof ce.resumed_from=="string"&&ce.resumed_from.length>0&&Ns.get(ce.resumed_from)||null}return!1}let Tn=typeof p.declared_base=="string"?p.declared_base:null;function sd(g){let L=null;for(let ce of Nr)!ce||ce.bead_id!==g||Fs(ce)||(L===null||(typeof ce.started_at=="number"?ce.started_at:0)>=(typeof L.started_at=="number"?L.started_at:0))&&(L=ce);return L&&typeof L.target_base=="string"?L.target_base:null}let sa=[],oa=[],od=g=>{let L=Ps.get(g.bead_id)!==g.attempt_id,ce=na.get(g.bead_id),je=typeof ce=="number"&&ce>0&&typeof g.finished_at=="number"&&ce>=g.finished_at;return!L&&!je&&typeof g.dismissed_at!="number"},aa=g=>{let L=typeof g.session_id=="string"&&g.session_id.length>0,ce=Ms.has(g.attempt_id);return{eligible:L&&!ce,reason:L?ce?"\uC774\uBBF8 \uC774\uC5B4\uBC1B\uC740 attempt (child attempt \uC874\uC7AC) \u2014 \uC774\uC5B4\uD558\uAE30 \uBD88\uAC00":null:"session_id \uC5C6\uB294 \uAD6C attempt \u2014 \uC774\uC5B4\uD558\uAE30 \uBD88\uAC00"}},jt=null;for(let g of Nr){let L=g.status==="paused"&&!Ms.has(g.attempt_id);if(g.status==="running"||L)oa.push({bead_id:g.bead_id,attempt_id:g.attempt_id,title:q.get(g.bead_id)||g.bead_id,runner:g.runner||null,model:g.model||null,effort:g.effort||null,speed:g.speed||null,continuation_mode:g.continuation_mode||null,started_at:typeof g.started_at=="number"?g.started_at:null,resumed_from:g.resumed_from||null,paused:L,conflict_resolution:Fs(g),base_exception:Ko(Tn,g.target_base),can_pause:typeof g.session_id=="string"&&g.session_id.length>0,discard:er(En,g.bead_id,{attempt_id:g.attempt_id}),usage:Mt(p.attempts||{},g.bead_id),current_child:G(g.bead_id),...u(g.bead_id)});else if((g.status==="failed"||g.status==="orphaned")&&od(g)){let ce=aa(g);sa.push({bead_id:g.bead_id,attempt_id:g.attempt_id,title:q.get(g.bead_id)||g.bead_id,runner:g.runner||null,model:g.model||null,effort:g.effort||null,speed:g.speed||null,continuation_mode:g.continuation_mode||null,started_at:typeof g.started_at=="number"?g.started_at:null,resumed_from:g.resumed_from||null,failed:!0,status:g.status,status_label:g.status==="orphaned"?"\uC911\uB2E8\uB428":"\uC2E4\uD328",discard:er(En,g.bead_id,{attempt_id:g.attempt_id}),resume_eligible:ce.eligible,resume_reason:ce.reason,conflict_resolution:Fs(g),base_exception:Ko(Tn,g.target_base),usage:Mt(p.attempts||{},g.bead_id),current_child:G(g.bead_id),...u(g.bead_id)}),jt=g}}let Cn=[...sa,...oa],ia=null;if(jt){let g=aa(jt),L=jt.cause_detail;ia={bead_id:jt.bead_id,repo:jt.repo||"",reason:jt.cause||jt.status,cause_detail:L&&typeof L.reason=="string"?{reason:L.reason,command:typeof L.command=="string"?L.command:null}:null,resume_attempt_id:jt.attempt_id,resume_eligible:g.eligible,resume_reason:g.reason,discard:er(En,jt.bead_id,{attempt_id:jt.attempt_id})}}let la=new Set(Cn.map(g=>g.bead_id)),qs=Array.isArray(p.merge_queue)?p.merge_queue:[],ca=new Map,da=new Map,ua=new Map,pa=new Map,fa=new Map;qs.forEach((g,L)=>{g&&typeof g.bead_id=="string"&&(ca.set(g.bead_id,L+1),da.set(g.bead_id,g.resolution),ua.set(g.bead_id,g.continuation_action||null),pa.set(g.bead_id,g.head_review||null),fa.set(g.bead_id,g.authority||null))});let _a=p.merge_queue_state||{active:null,failures:{}},ad=_a.failures||{},id=p.auto_merge_skips||{},ma=g=>{let L=id[g];if(!L)return null;let ce=K[g],je=ce&&ce.pr?ce.pr.head_sha:null;return je&&je===L.head_sha?L.reason||"":null},Rn=new Map;for(let g of Cn)g.failed!==!0&&g.conflict_resolution&&(g.paused?Rn.has(g.bead_id)||Rn.set(g.bead_id,"paused"):Rn.set(g.bead_id,"running"));let ga=Cn.filter(g=>!g.paused&&g.failed!==!0).length,ha=(p.workspace_info||{}).slots,ba=typeof ha=="number"?ha:typeof p.slots=="number"?p.slots:Is,ld=ga>ba,In=Er($),cd=(Array.isArray(p.done)?p.done.slice():[]).filter(g=>In===void 0||typeof g.added_at!="number"||g.added_at>=In).sort((g,L)=>(L.added_at||0)-(g.added_at||0)),Xr=Ds(cd,"done"),dd=new Set((Array.isArray(p.done)?p.done:[]).map(g=>g?.bead_id).filter(g=>typeof g=="string")),ya=[],ud=d?.()||"";for(let g of oe){let L=Rr(g.closed_at);if(typeof g.id!="string"||dd.has(g.id)||L===null||In!==void 0&&L<In||typeof g.comment_count!="number"||g.comment_count<=0)continue;let ce=`${ud}\0${g.id}\0${String(g.updated_at)}\0${g.comment_count}`,je=D.get(ce);je===void 0&&r&&(D.set(ce,"pending"),Promise.resolve(r("get-comments",{id:g.id})).then(ut=>{let Rt=Array.isArray(ut)&&ut.some(pt=>_s(typeof pt?.text=="string"?pt.text:"")?.lane==="session");D.set(ce,Rt?"session":"not-session"),ne()}).catch(()=>{D.set(ce,"failed"),ne()})),je==="session"&&ya.push({id:g.id,title:g.title||g.id,reason:"",draggable:!1,done:!0,lane:"done",selectable:!1,selected:!1,worker_serial:!1,badges:["\uC138\uC158 \uC791\uC5C5"],alert:!1,usage:null,done_at:L,created_at:g.created_at,updated_at:g.updated_at})}Xr.push(...ya),Xr.sort((g,L)=>(L.done_at||0)-(g.done_at||0));let Ln={};for(let g of sr)Ln[g]=0;let va=!1,wa=0,Bs=0,ka=0;for(let g of Xr){let L=g.usage;if(L&&typeof L=="object"){let ce=!1;for(let je of sr)Number.isFinite(L[je])&&(Ln[je]+=L[je],va=!0,ce=!0);ce&&(Bs+=1,Number.isFinite(L.total_cost_usd)&&(wa+=L.total_cost_usd,ka+=1))}}Bs>0&&ka===Bs&&(Ln.total_cost_usd=wa);let $a=Xr.map(g=>g.usage).filter(g=>g&&typeof g=="object"&&g.providers),pd=$a.length>0?wt(Jn($a)):va?zt(Ln):null,fd=p.lane_states&&typeof p.lane_states=="object"&&!Array.isArray(p.lane_states)?p.lane_states:{},_d=Array.isArray(p.serial_lanes)?p.serial_lanes:[],xa=g=>{if(C.some(je=>je.bead_id===g))return"PR \uB300\uAE30 \xB7 \uC810\uC720";let L=Nr.filter(je=>je&&je.bead_id===g),ce=L.length>0?L[L.length-1].status:null;return ce==="failed"||ce==="orphaned"?"\uC2E4\uD328 \xB7 \uC810\uC720 \uC720\uC9C0":ce==="paused"?"\uC77C\uC2DC\uC815\uC9C0 \xB7 \uC810\uC720":"\uC2E4\uD589 \uC911 \xB7 \uC810\uC720"},Sa=_d.filter(g=>g&&typeof g.id=="string"&&Array.isArray(g.entries)).map((g,L)=>{let ce=fd[g.id]||{},je=new Map((Array.isArray(ce.corrections)?ce.corrections:[]).filter(st=>st&&typeof st.bead_id=="string"&&typeof st.after=="string").map(st=>[st.bead_id,st.after])),ut=Ds(g.entries.filter(st=>!la.has(st.bead_id)),g.id).map(st=>je.has(st.id)?{...st,badges:[`\u{1F517} ${je.get(st.id)} \uB4A4 (blocks \uC790\uB3D9)`,...st.badges]}:st),Rt=Array.isArray(ce.occupied_by)?ce.occupied_by.filter(st=>typeof st=="string"):[],pt=Rt.map(st=>({id:st,title:q.get(st)||st,draggable:!1,lane:g.id,ghost:!0,badges:[xa(st)]}));return{id:g.id,index:L+1,rows:[...pt,...ut],occupied:Rt.length>0,badge:Rt.length>0?xa(Rt[0]):"\uB300\uAE30",cycle:ce.cycle===!0}}),md=typeof p.serial_lane_count=="number"?p.serial_lane_count:Sa.length;return{queue:p,idToTitle:q,candidates:rd,candidate_hidden:{blocked:Os.hidden_blocked,spec:Os.hidden_spec},running:Cn,live_count:ga,slots:ba,over_cap:ld,failure:ia,waiting:Ds(_t.filter(g=>!la.has(g.bead_id)),"queue"),serial_lanes:Sa,serial_lane_count:md,pr_wait:C.map(g=>F_(g.bead_id,q.get(g.bead_id)||g.bead_id,K,Oe[g.bead_id]||null,Mt(p.attempts||{},g.bead_id),he[g.bead_id]||(V.has(g.bead_id)||se.has(g.bead_id)?{activity:null,merge_progress:{step:"merging"}}:null),Rn.get(g.bead_id)||null,g.external===!0,{position:ca.get(g.bead_id)||0,active:_a.active===g.bead_id,failure:ad[g.bead_id]||null,resolution:da.get(g.bead_id),continuation_action:ua.get(g.bead_id),head_review:pa.get(g.bead_id)||null,authority:fa.get(g.bead_id)||null},g.wt_present!==!1,p.auto_merge===!0?ma(g.bead_id):null,Ko(Tn,sd(g.bead_id)),p.completion_status&&typeof p.completion_status=="object"&&!Array.isArray(p.completion_status)&&p.completion_status[g.bead_id]||null,p.discard_operations&&typeof p.discard_operations=="object"&&!Array.isArray(p.discard_operations)?p.discard_operations:{},Ns.get(Ps.get(g.bead_id)||"")?.worker_serial===!0,p.auto_merge===!0)).map(g=>({...g,...u(g.id)})),merge_queue_length:qs.length,merge_queue_running:qs.length>0,auto_excluded:C.map(g=>g.bead_id).filter(g=>ma(g)!==null),declared_base:Tn,done:Xr,token_total:pd,cleanup_failures:$e,repo_operations:Array.isArray(p.repo_operations)?p.repo_operations:[]}}function Qe(){let y=!!o?.get()?.job,x=!y&&o?.isPending?.()===!0,oe=y?"\uBD84\uC11D \uC911":x?"\uC900\uBE44 \uC911":"";return i`<button
    ...
    >`,b=i`<label class="worker-tgl worker-slots"
      ...
      <label
        ...
        <select class="worker-serial-lane-count" aria-label="직렬 레인 수">
          ${Array.from({length:Dc},(Be,Ke)=>Ke+1).map(Be=>i`<option
                value=${String(Be)}
    ...
    >
      ▶ 자동 머지${oe>0?` ${oe}`:""}
        ...
        ${yt(p)}
        ${Yt({id:"worker-pane-queue",lane:"queue",title:"\uBCD1\uB82C \uB300\uAE30",items:p.waiting,empty:"\uB4DC\uB798\uADF8 \uB610\uB294 [\uB300\uAE30\uB85C \u21B4]\uB85C \uBC30\uCE58",collapsible:!0,collapsed:S.queue,preview:Pc(p.waiting)})}
        ${p.serial_lanes.map(x=>ot(x))}
        ${y}
        ${Yt({id:"worker-pane-done",lane:"done",title:"\uC644\uB8CC",items:p.done,empty:`${I()} \uC644\uB8CC \uC5C6\uC74C`,controls:at(),collapsible:!0,collapsed:S.done,preview:Array.isArray(p.token_total)?p.token_total.map(x=>x.label).join(" \xB7 "):p.token_total||Pc(p.done)})}
