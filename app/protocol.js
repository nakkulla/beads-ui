...
export const MESSAGE_TYPES = /** @type {const} */ ([
  ...
  // The INDEPENDENT RepoOperation repair axis (master spec §9.3). Separate from
  // `worker-automation-toggle` on purpose: `auto_advance`/`auto_merge` decide
  // whether work starts and merges, `auto_repair` decides whether a terminal
  // verify/deploy failure may buy its one automatic repair session. Neither
  // toggle reads or writes the other's value.
  ...
  // Per-failure-kind resolve click for one RepoOperation (master spec §10).
  // NOT a retry: it dispatches a repair session through the coordinator, and a
  // new attempt exists only once that session produced evidence.
  'worker-repo-operation-repair',
  ...
  'worker-parallel-analysis-targets',
  'worker-parallel-analysis-prompt',
