...
vi.mock('./ws.js', () => {
  ...
  const singleton = {
    ...
    /** @param {string} type - @param {any} reply */
    _reply(type, reply) {
...
describe('monitor tab direct entry (UI-nprg)', () => {
  ...
  test('keeps the latest picker selection when replies arrive in reverse', async () => {
    ...
    expect(
      client
        ._sent()
        .filter(
          (/** @type {any} */ message) => message.type === 'set-workspace'
        )
    ).toHaveLength(2);
  });
  ...
  test.each([
    null,
    { changed: true, workspace: { root_dir: '/tmp/wrong', db_path: '/tmp/x' } }
  ])('stays on Monitor for an invalid picker response %#', async (reply) => {
    const client = /** @type {any} */ (createWsClient());
    client._reply('list-workspaces', {
      workspaces: [
        { path: '/tmp/ws-a', database: '/tmp/ws-a/.beads/a.db' },
  ...
  test.each(['queue', 'running'])(
    'renders external work on the %s consumer',
    async (lane) => {
      const client = /** @type {any} */ (createWsClient());
      window.location.hash = '#/monitor';
      document.body.innerHTML = '<main id="app"></main>';
        ...
        workspaces: [
          {
            ...
            queue:
              lane === 'queue'
                ? [{ bead_id: record.bead_id, added_at: NOW }]
                : [],
            pr_wait: [],
            done: [],
            pr_observations: {},
            attempts:
              lane === 'running'
                ...
                : {},
            bead_titles: { [record.bead_id]: '분석' },
            external_waits: [record],
            wait_reasons: [
              {
                kind: 'external_job',
                subject: { root_dir: record.root_dir, bead_id: record.bead_id },
