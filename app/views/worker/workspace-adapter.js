...
/**
 * 실행 핀으로 인정하는 metadata 키 집합 — 이슈별 실행 핀 17키와 계정 2키,
 * 그리고 칩 바인딩 판정이 그 핀들과 **한 객체에서** 읽는 정체성 두 키
 * (`applied_exec_preset`·`chip_preset_source`, UI-wg68 §5.1)다. 서버
 * `runnable-cache.js`의 `EXEC_PIN_KEYS`와 같은 집합이어야 두 원천의 후보 행이
 * 같은 칩을 얻는다. quick_fix 저장 키는 여기 없다: 그 값은 워크스페이스 kv와
 * 큐에 살고 Bead metadata에는 쓰이지 않는다 (design §5).
 *
 * @type {ReadonlyArray<string>}
 */
const EXEC_PIN_KEYS = [
