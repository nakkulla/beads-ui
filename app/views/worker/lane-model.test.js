...
describe('monitor 저장 연결 레인 투영 (UI-j92s §5.1·§5.2)', () => {
  ...
  test('leaves 재적용 off for a confirmed member in no queue', () => {
    const lanes = buildLanes(
      [
        workspace({
          queue: [{ bead_id: 'A-1' }],
  ...
  test('labels an out-of-queue confirmed member as 진행 대기', () => {
    const lanes = buildLanes(
      [
        workspace({
          queue: [{ bead_id: 'A-1' }],
  ...
  test('labels a queued member with a proven prerequisite wait as ⛓ 선행 대기', () => {
        ...
        workspace({
          ...
          admission: {
            'A-2': {
              ...
              at: 1,
              blockers: [{ id: 'A-1', rig: null, status: 'open' }]
            }
          }
        })
      ],
    ...
    expect(lanes.chain_lanes[0].rows[1].location_label).toBe('⛓ 선행 대기');
  });

  test('keeps 🔒 대기 for a queued member with only a cached blocker', () => {
    const lanes = buildLanes(
      [
        workspace({
          queue: [{ bead_id: 'A-1' }, { bead_id: 'A-2' }],
    ...
    expect(lanes.chain_lanes[0].rows[1].location_label).toBe('🔒 대기');
  });

  test('drops the 의존 없음 verdict when the preceding member is done', () => {
    const lanes = buildLanes(
      [
        workspace({
          done: [{ bead_id: 'A-1' }],
    ...
    expect(lanes.chain_lanes[0].rows[1].mismatch).toBe(false);
  });
